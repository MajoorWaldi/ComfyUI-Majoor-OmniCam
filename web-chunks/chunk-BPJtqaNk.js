    ...
    beforeRegisterNodeDef(t, a) {
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        if (n.save(), this.selected) {
          ...
          d.addColorStop(0, `rgba(229, 72, 77, ${p})`), d.addColorStop(1, "rgba(229, 72, 77, 0)"), n.fillStyle = d, n.beginPath(), n.arc(m, c, G * 1.15, 0, Math.PI * 2), n.fill();
        }
...
function Yr(e) {
  return !!e?.sequence?.enabled && Me(e).length > 0;
}
function xt(e, t) {
  const a = Me(e);
...
function Ja(e, t, a = null) {
  ...
  const s = xt(e, r)?.camera_id || o[0].camera_id;
...
const Mt = Object.freeze(["select", "track", "anchor", "project", "erase"]), Ct = Object.freeze(["manual_2d", "static_anchor", "world_point", "object_point", "camera_field"]), Dt = Object.freeze(["linear", "smooth", "hold"]), le = (e, t = 0) => Number.isFinite(Number(e)) ? Number(e) : t, ct = (e) => Math.max(0, Math.min(1, le(e)));
function to(e, t) {
  return {
...
function ao(e) {
  ...
  }).filter((o) => o.keys.length), e.motion_tool = Mt.includes(e.motion_tool) ? e.motion_tool : "select", e.selected_motion_layer_id = e.motion_layers.some((o) => o.id === e.selected_motion_layer_id) ? e.selected_motion_layer_id : e.motion_layers[0]?.id || null, e;
}
function oo(e) {
...
function L(e) {
  ...
  if (Math.abs(e.roll || 0) > 1e-9) {
    const s = e.roll * Math.PI / 180, i = Math.cos(s), l = Math.sin(s), m = x(C(r, i), C(n, l));
    n = x(C(n, i), C(r, -l)), r = m;
  }
...
function fe(e, t, a = null) {
  ...
  if (u && Array.isArray(h)) {
    ...
    if (f && f.enabled !== !1) {
      const M = Tt(h, f, t), T = p?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
...
const j = (e, t, a) => Math.max(t, Math.min(a, e)), so = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, se = (e, t = null) => typeof e == "string" && so.test(e.trim()) ? e.trim() : t, x = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], O = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], C = (e, t) => [e[0] * t, e[1] * t, e[2] * t], ee = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], me = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], $ = (e) => Math.sqrt(Math.max(1e-12, ee(e, e))), de = (e) => C(e, 1 / $(e));
function io(e, t, a) {
...
function I(e, t, a, o, r = !1, n = null) {
  ...
  if (l.interpolation === "bezier" || m.interpolation === "bezier") {
    const b = lt(l, a, c, m, o), S = lt(m, a, l, p, o), D = h, w = h + (b.out_y || 0), k = g + (S.in_y || 0), f = g, M = j(Number(b.out_x ?? 1 / 3), 0, 1), T = j(1 + Number(S.in_x ?? -1 / 3), 0, 1);
    let N = 0, X = 1;
    for (let it = 0; it < 32; it++) {
      const V = (N + X) * 0.5, ze = 1 - V;
      3 * ze * ze * V * M + 3 * ze * V * V * T + V * V * V < u ? N = V : X = V;
    }
...
function en(e = "balanced", t = "all_views", a = null) {
  const o = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  for (let d = 0; d < r; d++) {
    ...
    if (t === "ground_focus")
      ...
      const k = u * Math.PI * 2, f = 1 - 2 * h, M = Math.sqrt(Math.max(0, 1 - f * f)), T = 1.5 + Math.cbrt(g) * 20;
      v = Math.cos(k) * M * T, b = Math.sin(k) * M * T, y = Math.max(0.01, f * T * 0.75 + 2.5), S = 0.72, D = 0.78, w = 0.88;
    } else {
      const k = d % 4;
      if (k === 0) {
...
function tn(e) {
  ...
  if (!e || typeof e != "object") return t;
  const a = { ...t, ...e };
  a.fps = Math.round(ge(a.fps, 24, 1, 120)), a.duration_frames = Math.round(ge(a.duration_frames, 120, 1, he.maxDurationFrames)), a.width = Math.round(ge(a.width, 1280, 64, 4096)), a.height = Math.round(ge(a.height, 720, 64, 4096));
  const o = (c, p) => (Array.isArray(c) ? c : []).slice(0, he.maxKeysPerTrack).map((d) => ({
  ...
  a.cameras = s.slice(0, he.maxCameras).map((c, p) => {
  ...
  a.camera = l.camera, a.keyframes = l.keyframes, a.target_object_id = l.target_object_id || null, a.target_offset = l.target_offset || [0, 0, 0], a.aim_bone = l.aim_bone || null, a.objects = (Array.isArray(a.objects) ? a.objects : t.objects).slice(0, he.maxObjects).map((c) => ({
    ...c,
    color: se(c?.color),
    locked: !!c.locked,
...
function kt(e, t) {
  ...
  return { position: x(uo(e.position.map((r, n) => r * t.size[n]), a), t.position), rotation: ho(o), quaternion: o, size: e.size.map((r, n) => r * t.size[n]) };
}
function go(e, t) {
...
function It(e, t = {}, a = null, o = "generic") {
  ...
  for (let f = 0; f < n; f++) {
    ...
    !g[f] && !y && (M.push("over"), T.push("framing_loss")), b.push(ft(M)), S.push(T);
  }
...
function To(e, t, a = 0.6) {
  const o = je(e), r = Math.min(1, Math.max(0, Number(a) || 0));
...
async function cn() {
  ...
  try {
    ...
    return e.ok ? (ye = await e.json(), ye) : null;
  } catch {
    return null;
  }
}
...
function ie(e, t, a, o) {
  ...
  return `
    <div class="oc-health-metric" data-grade="${o}">
      ...
      <span class="oc-health-metric-name">${Ae(e)}</span>
...
function Ro(e) {
  ...
  return t.length ? t.slice(0, 6).map((a) => {
    const o = a.metrics.map((n) => Ae(n)).join(", "), r = a.start === a.end ? _("Frame {frame}").replace("{frame}", String(a.start)) : _("Frames {start}-{end}").replace("{start}", String(a.start)).replace("{end}", String(a.end));
    return `
...
function Fo(e) {
  ...
  const n = [
    ie(
      ...
      o.max_speed,
      o.limits.max_speed,
      be(o.max_speed, o.limits.max_speed, r)
    ),
    ie(
      "angular_speed",
      o.max_angular_speed,
      o.limits.max_angular_speed,
      be(o.max_angular_speed, o.limits.max_angular_speed, r)
    ),
    ie(
      "acceleration",
      o.max_acceleration,
      o.limits.max_acceleration,
      be(o.max_acceleration, o.limits.max_acceleration, r)
    ),
    ie(
      "jerk",
      o.max_jerk,
      o.limits.max_jerk,
      be(o.max_jerk, o.limits.max_jerk, r)
    ),
    ie("fov_drift", o.max_fov_change, o.limits.max_fov_change, o.track_grades.fov_drift)
         ...
         <span class="oc-health-metric-name">${Ae("framing_loss")}</span>
...
function ln(e, t) {
  ...
  if (a) {
    ...
    for (const o of a.segments) {
      ...
      s.className = "oc-health-band", s.dataset.grade = o.grade, s.style.left = `${r}%`, s.style.width = `${Math.max(0.4, n - r)}%`, s.title = o.metrics.map((i) => Ae(i)).join(", "), t.appendChild(s);
    }
  }
}
function ke(e, t, a, o) {
...
function mn(e) {
  ...
  if (n.max_speed <= a) {
    ke(e, r, "Slow to limits", _("Speed flattened; the shot keeps its length."));
    return;
  }
  const s = n.max_speed / a * (e.state.duration_frames / Math.max(1, e.state.fps));
  ke(e, r, "Slow to limits", _("Speed flattened, still over: this path needs about {seconds}s to fit the limit.").replace("{seconds}", s.toFixed(1)));
}
function dn(e) {
  const t = ue(e);
  if (!t) return;
...
function Wo() {
  return {
    ...
    loopPlayback: A(xa, !1),
    uiDensity: z(Ma, "advanced", ["basic", "animation", "advanced"]),
...
function Qo(e, t, a, o, r) {
  const { right: n, up: s, forward: i } = L(t), l = t.position, m = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = Zo(m, i);
  let p, d;
  if (t.camera_type === "orthographic") {
...
function er(e, t, a) {
  ...
  return e.state.gizmo_space === "local" ? o.map((n) => De(n, r)) : o;
}
function tr(e) {
...
function ar(e) {
  ...
  const n = Math.max(0.7, $(O(a.position, o)) * 0.12), s = t.type === "object" ? er(e, t.object, t) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      entity: t,
      center: r,
      worldLength: n,
      handles: s.map((l, m) => ({ index: m, axis: l, points: [r, F(x(o, C(l, n)), a, e.canvas.width, e.canvas.height)] })).filter((l) => l.points[1] && Number.isFinite(l.points[1][0]) && Number.isFinite(l.points[1][1]))
    };
  const i = s.map((l, m) => {
    const c = Math.abs(l[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = de(me(l, c)), d = de(me(l, p)), u = [];
    for (let h = 0; h <= 48; h++) {
      const g = h / 48 * Math.PI * 2, v = F(x(o, x(C(p, Math.cos(g) * n), C(d, Math.sin(g) * n))), a, e.canvas.width, e.canvas.height);
...
function or(e, t) {
  ...
  for (const s of e.state.objects) {
    ...
    const i = s.keyframes?.length ? Ie(s, e.frame) : s, l = F(i.position || [0, 0, 0], o, e.canvas.width, e.canvas.height);
    if (!l) continue;
...
function sr(e, t, a, o, r) {
  ...
  if (n === "vertex" && o) {
    ...
    if (s?.point && !t.snapshots.some((i) => i.object.id === s.objectId)) {
      const i = a.reduce((m, c) => x(m, c), [0, 0, 0]).map((m) => m / a.length), l = O(s.point, i);
...
function Se(e, t) {
  ...
  const n = r[0] - a.start[0], s = r[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = nr(a), m = a.axis ? We[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], p = L(c), d = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...O(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((S) => [...S.transform.position]);
  if (a.mode === "translate") {
    const S = l ?? (n - s) * d * i, D = m ? C(m, S) : x(C(p.right, n * d * i), C(p.up, -s * d * i));
    u = u.map((w) => x(w, D)), u = sr(e, a, u, r, t.ctrlKey || t.metaKey);
  }
  ...
  a.snapshots.forEach((S, D) => {
    ...
    if (a.mode === "translate" && (w.position = u[D]), a.mode === "rotate") {
      ...
      w.position = x(a.pivot, De(O(S.transform.position, a.pivot), f)), w.rotation = x(S.transform.rotation, f);
    }
    if (a.mode === "scale") {
      const k = y ? $e(g, 0.1) : g, f = m ? m.map((T) => T ? k : 1) : [k, k, k], M = O(S.transform.position, a.pivot);
...
function ir(e, t) {
  ...
  return o === "escape" ? Pa(e) : o === "enter" || o === " " ? Oa(e) : We[o] ? (a.axis = a.axis === o ? null : o, Se(e, a.lastEvent), !0) : /^[0-9]$/.test(o) || o === "." || o === "," || o === "-" && !a.numeric ? (a.numeric += o === "," ? "." : o, Se(e, a.lastEvent), !0) : (o === "backspace" && (a.numeric = a.numeric.slice(0, -1), Se(e, a.lastEvent)), !0);
}
function _e(e, t, a) {
...
const ce = (e, t) => Math.round(e / t) * t, lr = (e, t) => e.map((a) => ce(a, t)), Ue = (e, t) => e.camera_type === "orthographic" ? 10 / (Math.max(0.01, e.zoom || 1) * Math.max(1, t)) : $(O(e.position, e.target)) * 25e-4;
function Z(e, t, a, o = []) {
  const n = e.currentTransformEvent?.ctrlKey || e.currentTransformEvent?.metaKey ? "grid" : e.state.spatial_snap_mode;
  if (n === "grid") return lr(t, e.state.spatial_grid_size || 0.5);
  if (n === "vertex" && a) {
...
function hn(e, t) {
  ...
  if (m) {
    ...
    if (m.entity.type === "object") {
      ...
      const k = w.reduce((f, M) => x(f, M.transform.position), [0, 0, 0]).map((f) => f / w.length);
      e.gizmoDrag = {
        ...b,
        type: "object",
        historyCheckpointed: !0,
  ...
  s || (e.checkpoint("Move camera"), e.beginCameraEdit()), s && !e.state.editor_views && (e.state.editor_views = Ce()), e.drag = {
    x: t.clientX,
    y: t.clientY,
    shift: d,
    dolly: u,
    fly: h,
    camera: R(n),
    target: s ? e.state.editor_views[e.state.view_mode] || (e.state.editor_views[e.state.view_mode] = Ce()[e.state.view_mode]) : e.camera,
    editorView: s,
...
function gn(e, t) {
  ...
  if (e.pathDrag) {
    ...
    m && (m.camera.position = Qo(
      [s, i],
      re(e),
      e.pathDrag.anchor,
      e.canvas.width,
      e.canvas.height
    ), m.interpolation = Jo(m.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  if (e.boxSelection) {
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        ...
        const d = m ? ce(l * 0.015, Math.PI / 12) : l * 0.015, u = O(e.gizmoDrag.target, e.gizmoDrag.position), h = De(u, C(e.gizmoDrag.axis, d * (180 / Math.PI)));
    ...
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
        const d = (s[0] - e.gizmoDrag.pointer[0]) * i, u = (s[1] - e.gizmoDrag.pointer[1]) * i, h = x(
    ...
    if (c.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        ...
        for (const u of c) u.object.position = x(u.transform.position, d);
      } else if (e.state.gizmo_mode === "rotate") {
        const d = O(e.gizmoDrag.object.rotation, p.rotation);
        for (const u of c)
          u.object.position = x(e.gizmoDrag.groupPivot, De(O(u.transform.position, e.gizmoDrag.groupPivot), d)), u.object.rotation = x(u.transform.rotation, d);
      } else {
  ...
  if (e.objectDrag) {
    _e(e, e.objectDrag, "Move object");
    const n = t.clientX - e.objectDrag.x, s = t.clientY - e.objectDrag.y, { right: i, up: l } = L(e.objectDrag.camera), m = Ue(e.objectDrag.camera, e.canvas.height) * (t.shiftKey ? 0.1 : 1), c = x(e.objectDrag.position, x(C(i, n * m), C(l, -s * m)));
  ...
  _e(e, e.drag, e.drag.editorView ? "Navigate viewport" : "Move camera");
  ...
  if (e.drag.dolly) {
    ...
    const { right: n, up: s } = L(r), i = Ue(r, e.canvas.height), l = x(C(n, -a * i), C(s, o * i));
...
function yn(e, t) {
  ...
  if (e.boxSelection) {
    ...
    for (const d of e.state.objects) {
      ...
      const u = d.keyframes?.length ? Ie(d, e.frame) : d, h = F(u.position || [0, 0, 0], s, e.canvas.width, e.canvas.height);
...
function Te(e, { sourceKind: t = "manual_2d", label: a, keys: o, source: r = {} }) {
  if (!Ct.includes(t)) throw new Error(`Unsupported motion source: ${t}`);
  const n = dr(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: o.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...r } };
...
function at(e, t) {
  return e.motion_tool = Mt.includes(t) ? t : "select", e.motion_tool;
}
function pr(e, t) {
  if (Dt.includes(t))
...
function Le(e, t) {
  return ur(t, e.interactionElement);
}
function br(e, t) {
...
function _r(e, t) {
  const a = e.state.objects.find((m) => m.id === e.selectedObjectId), o = e.motionCreationKind, n = (o === "object" || o !== "world" && !!a) && a ? "object_point" : "world_point", s = n === "object_point" ? { object_id: a.id, local_point: [0, 0, 0] } : { point: e.webgl?.intersectScenePoint?.(t.x * e.canvas.width, t.y * e.canvas.height, e.canvas.width, e.canvas.height) || [...e.camera.target] }, i = hr(e.state, s, e.frame, e.canvas.width, e.canvas.height) || t, l = n === "object_point" ? `${a.name || a.id} Track` : "World Anchor";
  return Te(e.state, { sourceKind: n, label: l, keys: [{ time_seconds: e.frame / e.state.fps, x: i.x, y: i.y, visible: i.visible !== !1 }], source: s });
}
function _n(e, t) {
  for (const r of e.root.querySelectorAll("[data-motion-tool]"))
    r.addEventListener("click", () => {
      e.motionCreationKind = "", at(e.state, r.dataset.motionTool), Ee(e), e.render();
    }, { signal: t });
  for (const r of e.root.querySelectorAll("[data-motion-preset]"))
    r.addEventListener("click", () => {
      e.checkpoint("Add camera field"), Te(e.state, { sourceKind: "camera_field", label: `${r.dataset.motionPreset} Field`, keys: [{ time_seconds: 0, x: 0.5, y: 0.5 }], source: { preset: r.dataset.motionPreset, point: [...e.camera.target] } }), Q(e, `Camera field: ${r.dataset.motionPreset}`);
    }, { signal: t });
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (r) => {
    const n = Fe(e.state);
    n && (e.checkpoint("Set motion interpolation"), pr(n, r.target.value), Q(e, `Motion interpolation: ${r.target.value}`));
  ...
  a.addEventListener("pointerdown", (r) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? Te(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? _r(e, s) : n === "erase" && br(e.state, s), Q(e, `Motion tool: ${n}`);
  ...
  const o = (r) => {
    ...
    const [s, i] = gt(e), l = yr(e.state, n.points, s, i);
...
function wr(e, t) {
  ...
  if (a) {
    ...
    e.checkpoint?.(`Motion: ${a.label}`), at(e.state, a.tool), e.motionCreatingLabel = a.label, e.motionCreationKind = t, Ee(e), e.render(), e.setStatus(a.hint);
  }
}
function Ra(e) {
  return (e.state.motion_tool || "select") === "select" && !e.motionTrackDraft ? !1 : (at(e.state, "select"), e.motionTrackDraft = null, e.motionCreatingLabel = "", e.motionCreationKind = "", Ee(e), e.render(), e.setStatus("Motion creation cancelled."), !0);
}
function vn(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => wr(e, a.dataset.motionCreate), { signal: t });
  e.root.querySelector("[data-motion-create-cancel]")?.addEventListener("click", () => Ra(e), { signal: t });
}
const yt = { t: "translate", r: "rotate", s: "scale" }, Sr = [
...
function jr(e, t) {
  return Dr(e) || t?.lastKeyZone || "viewport";
}
let bt = !1;
function kr() {
  bt || typeof window > "u" || (bt = !0, window.addEventListener("keydown", (e) => {
    const t = e.composedPath?.()[0] || e.target, a = Bo(t);
    !a || a.disposed || Tr(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function Tr(e, t) {
  const a = t.composedPath?.()[0] || t.target;
  if (Cr(a) || (t.code === "Space" || t.key === "Enter") && Mr(a)) return !1;
...
function Or(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!Me(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: Za(e.state) }, Ve(e)) : (e.checkpoint("Split shot"), Ja(e.state, e.frame, null) ? Ve(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
    if (t.repeat) return !0;
    const o = Me(e.state), r = xt(e.state, e.frame), n = r ? o.findIndex((s) => s.start === r.start) : -1;
...
function Nr([e, t], [a, o]) {
  ...
  const r = Math.round((window.innerWidth || e) * Pr), n = Math.round((window.innerHeight || t) * zr);
  return [
...
function Rr(e, t) {
  const a = Fa[t];
  return !a || !e?.setSize ? !1 : (e.setSize(Nr(a.default, a.min)), !0);
}
function Fr(e, t, a) {
  const o = Fa[t];
...
const _t = "oc-help-css", ve = "#8b7bd8", La = /* @__PURE__ */ new Map();
function rt(e, t) {
  e && t && La.set(e, t);
}
function Qe(e) {
...
.oc-help-h-icon{width:18px;height:18px;flex:none;border-radius:50%;background:${ve};
...
.oc-help-close:hover{background:${ve};color:#fff}
...
.oc-help-h{margin:0 0 6px;font-size:10px;font-weight:700;color:${ve};
...
.oc-help-tip{margin-top:2px;padding:8px 10px;background:rgba(139,123,216,.12);
  border-left:2px solid ${ve};border-radius:3px;color:#ddd;font-size:11.5px}
`;
function Kr() {
  if (document.getElementById(_t)) return;
  const e = document.createElement("style");
  e.id = _t, e.textContent = Lr, document.head.appendChild(e);
}
function J(e) {
...
const wt = "MajoorOmniCam.ShowHelp", Je = "oc-help-toolbar-icon", St = "oc-help-toolbar-css", Br = "#8b7bd8";
function qr() {
...
U.registerExtension({
  ...
  async nodeCreated(e) {
    if (Pe(e) !== Xe) return;
    we("director:nodeCreated", e);
    const t = !ae, a = t ? null : st(e);
    await ot(e, async () => {
      we("director:import:start", e);
      const { attachDirector: r } = await import("./chunk-CynrCD75.js").then((n) => n.f);
      return we("director:import:resolved", e), r;
    });
    const o = e.__majoorOmniCam;
    o && (we("director:attach:complete", e), Xo(o), t && Yo(o), nt(e, Xe, t, a?.()));
  }
});
U.registerExtension({
  ...
  async nodeCreated(e) {
    if (Pe(e) !== Ye) return;
    const t = !ae, a = t ? null : st(e);
    await ot(e, async () => (await import("./chunk-B8sSbAQ1.js")).attachExtractor), e.__majoorOmniCamExtractor && nt(e, Ye, t, a?.());
  }
});
U.registerExtension({
  ...
  async nodeCreated(e) {
    if (Pe(e) !== Ze) return;
    const t = !ae, a = t ? null : st(e);
    await ot(e, async () => (await import("./chunk-x26LrT_w.js")).attachMonitor), e.__majoorOmniCamMonitor && nt(e, Ze, t, a?.());
  }
});
export {
  bn as $,
