...
function _l(e, { type: t = "handheld_subtle", intensity: a = 1, duration_frames: o = null, subdivide: r = !0 } = {}) {
  ...
  return [...b].sort((y, x) => y - x).map((y) => {
    const x = Ee(u, y), S = Pt(y * m, 1) * l, g = Pt(y * m, 2) * l, _ = Pt(y * m, 3) * l * 0.5, A = Pt(y * m, 4) * p, F = Pt(y * m, 5) * (p * 0.35), L = [...x.position], X = [...x.target];
    return L[0] += S, L[1] += g, L[2] += _, X[0] += S * 0.35, X[1] += g * 0.35, {
      frame: y,
      camera: {
        ...x,
        position: L,
...
function Pl(e, t) {
  const a = Math.max(1, e.state.duration_frames - 1), o = Y(Number(e.timelineZoom) || 1, 0.1, 50), r = Number(e.timelinePan) || 0, n = a / o, i = as(n, t), l = (i >= Hr ? i / Hr : 0) || i, p = [], m = Math.max(0, Math.floor(r / l) * l);
  for (let d = m; d <= a + 1e-6; d += l) {
    const h = Math.round(d), u = be(e, h);
    if (!(u < -1)) {
      if (u > 101) break;
...
function ql(e, t) {
  ...
  if (e.curveHover = null, t.preventDefault(), t.stopPropagation(), e.curveDrag.historyCheckpointed || (e.checkpoint?.(e.curveDrag.handle ? "Edit curve tangent" : "Edit curve"), e.curveDrag.historyCheckpointed = !0), e.curveDrag.handle) {
    ...
    f.tangents.channels[d.id] = g, e.scheduleSerialize(), e.camera = Ee(e.state, e.frame), e.applyObjectAnimationFrame(), e.render(), e.drawCurveEditor();
    return;
  }
  ...
  if (e.curveDrag.object) {
    ...
    const f = he(e.curveDrag.key.camera);
...
function rs(e, { selectedOnly: t = !1 } = {}) {
  const a = Math.max(1, (e.state?.duration_frames ?? 120) - 1), o = e.timelineKeyframes() || [], r = e.timelineObject(), n = ot(e), i = e.selectedKeyFrames?.size ? [...e.selectedKeyFrames] : e.selectedKeyFrame != null ? [e.selectedKeyFrame] : [], c = i.length > 0, l = t || c && i.length < o.length ? o.filter((b) => i.includes(b.frame)) : o;
  if (!l.length) {
  ...
  if (h.length > 0) {
    const b = Math.min(...h), v = Math.max(...h), y = Math.max(1e-4, v - b), x = (b + v) / 2, S = (V) => r ? Oo(r, V) : Ee(e.state, V), g = [], _ = Math.max(1, Math.floor(a / 40));
    for (let V = 0; V <= a; V += _) {
      const se = S(V);
      for (const j of n) {
        const z = j.get(se);
...
function Ql(e) {
  ...
  const i = e.timelineObject(), c = e.timelineKeyframes(), l = ot(e), p = 44, m = 14, f = 16, d = 22, h = Math.max(1, a - p - m), u = Math.max(1, o - f - d), b = Math.max(1, e.state.duration_frames - 1), v = Y(Number(e.curveZoomX) || 1, 0.1, 50), y = Number(e.curvePanX) || 0, x = b / v, S = y, g = y + x, _ = [], A = Math.max(1, Math.ceil(x / Math.max(80, h))), F = (T) => i ? Oo(i, T) : Ee(e.state, T);
  for (let T = 0; T <= b; T += A) _.push({ frame: T, value: F(T) });
  _[_.length - 1]?.frame !== b && _.push({ frame: b, value: F(b) });
  const L = _.flatMap((T) => l.map((H) => H.get(T.value)));
  let X = Math.min(...L), W = Math.max(...L);
  (!Number.isFinite(X) || !Number.isFinite(W)) && (X = -1, W = 1), Math.abs(W - X) < 1e-6 && (X -= 1, W += 1);
  const V = (W - X) * 0.1;
  X -= V, W += V;
  const se = Y(Number(e.curveZoom) || 1, 0.1, 50), j = (W + X) / 2 + (Number(e.curvePanY) || 0), z = (W - X) / se;
  X = j - z / 2, W = j + z / 2;
  const B = (T) => p + (T - S) / Math.max(1e-6, g - S) * h, G = (T) => f + u * (W - T) / Math.max(1e-6, W - X);
  if (n.fillStyle = "#111114", n.fillRect(0, 0, a, o), n.strokeStyle = "#222228", n.lineWidth = 1, n.font = "9px system-ui, -apple-system, sans-serif", n.fillStyle = "#6e727a", Kl(n, {
    ...
    xFor: B,
    frame: e.frame
  }), Dl(n, { left: p, right: m, top: f, width: a, graphHeight: u, minimum: X, maximum: W, yFor: G }), X <= 0 && W >= 0) {
    const T = G(0);
  ...
  for (const T of l) {
    ...
    let H = !1;
    _.forEach((P) => {
      const U = B(P.frame), J = G(T.get(P.value));
      U >= p - 50 && U <= a - m + 50 && (H ? n.lineTo(U, J) : (n.moveTo(U, J), H = !0));
    }), n.stroke();
    for (const P of c) {
      const U = i ? P.transform : P.camera, J = B(P.frame), de = G(T.get(U)), me = P.frame === e.selectedKeyFrame || e.selectedKeyFrames?.has(P.frame);
      me && (n.fillStyle = "rgba(242, 208, 107, 0.35)", n.beginPath(), n.arc(J, de, 8.5, 0, Math.PI * 2), n.fill()), n.fillStyle = me ? "#ffd75e" : T.color, n.strokeStyle = "#0d0d10", n.lineWidth = 1.6, n.beginPath(), n.arc(J, de, me ? 5.2 : 3.8, 0, Math.PI * 2), n.fill(), n.stroke(), e.curveHitPoints.push({
        x: J,
        y: de,
        key: P,
        channel: T,
    ...
    if (e.showCurveHandles)
      for (let P = 0; P < c.length; P++) {
        ...
        const me = i ? U.transform || i : U.camera || U, ae = B(U.frame), pe = G(T.get(me)), fe = c[P - 1], ue = c[P + 1], ve = Math.max(1, U.frame - (fe?.frame ?? U.frame - 1)), ge = Math.max(1, (ue?.frame ?? U.frame + 1) - U.frame), ye = Qs(
          U,
          T.id,
          fe,
          ue,
          (ne) => T.get(i ? ne.transform || i : ne.camera || U)
        ), N = (W - X) / Math.max(1, u), D = h * ge / Math.max(1, x), re = h * ve / Math.max(1, x), ce = [];
        (fe || P > 0) && ce.push({ side: "in", x: ae + ye.in_x * re, y: pe - ye.in_y / N }), (ue || P < c.length - 1 || c.length === 1) && ce.push({ side: "out", x: ae + ye.out_x * D, y: pe - ye.out_y / N });
        for (const ne of ce) {
          if (n.strokeStyle = T.color, n.lineWidth = J ? 1.5 : 1, n.beginPath(), n.moveTo(ae, pe), n.lineTo(ne.x, ne.y), n.stroke(), n.fillStyle = J ? "#2a2233" : "#171720", n.strokeStyle = J ? "#ffd75e" : T.color, n.lineWidth = J ? 2 : 1.2, n.beginPath(), ne.side === "in")
          ...
          n.fill(), n.stroke(), e.curveHitPoints.push({
            ...
            object: i,
            handle: ne.side,
            pixelPerSegment: ne.side === "in" ? re : D,
            valuePerPixel: N,
            startHandles: { ...ye }
          });
        }
  ...
  if (e.curveBoxSelect) {
    const T = Math.min(e.curveBoxSelect.startX, e.curveBoxSelect.currentX), H = Math.min(e.curveBoxSelect.startY, e.curveBoxSelect.currentY), P = Math.abs(e.curveBoxSelect.currentX - e.curveBoxSelect.startX), U = Math.abs(e.curveBoxSelect.currentY - e.curveBoxSelect.startY);
    n.fillStyle = "rgba(56, 189, 248, 0.15)", n.fillRect(T, H, P, U), n.strokeStyle = "#38bdf8", n.lineWidth = 1, n.setLineDash([4, 4]), n.strokeRect(T, H, P, U), n.setLineDash([]);
  }
  const te = B(e.frame);
  if (te >= p && te <= a - m && (n.strokeStyle = "#a78bfa", n.lineWidth = 1.5, n.beginPath(), n.moveTo(te, f), n.lineTo(te, f + u), n.stroke(), n.fillStyle = "#a78bfa", n.beginPath(), n.moveTo(te - 4, f), n.lineTo(te + 4, f), n.lineTo(te, f + 6), n.closePath(), n.fill()), e.curveDrag || e.curveHover) {
    ...
    if (e.curveDrag)
      if (e.curveDrag.handle) {
        ...
        T = `F${e.curveDrag.key.frame} · ${e.curveDrag.channel.name} (${P})`, H = "Tangent edit";
        ...
        T = `${e.curveDrag.group.length} keys · ΔF: ${de} · ΔVal: ${me}`, H = `${e.curveDrag.channel.name}: ${U.toFixed(2)}`;
      } else {
        const P = e.curveDrag.key.frame - e.curveDrag.startFrame, U = e.curveDrag.channel.get(e.curveDrag.object ? e.curveDrag.key.transform : e.curveDrag.key.camera), J = U - e.curveDrag.startValue, de = P >= 0 ? `+${P}` : `${P}`, me = J >= 0 ? `+${J.toFixed(2)}` : `${J.toFixed(2)}`;
        T = `F${e.curveDrag.key.frame} (${de}) · ${e.curveDrag.channel.name}: ${U.toFixed(2)} (${me})`;
      }
    else if (e.curveHover)
      if (e.curveHover.channelName) {
        const P = Number.isFinite(e.curveHover.value) ? e.curveHover.value.toFixed(2) : "";
        T = `F${e.curveHover.frame} · ${e.curveHover.channelName}: ${P}`, e.curveHover.isHandle && (H = `Handle ${e.curveHover.handleSide}`);
      } else
        T = `Frame ${e.curveHover.frame}`;
    if (T) {
      n.save(), n.font = "11px system-ui, -apple-system, sans-serif";
      const P = n.measureText(T), U = H ? n.measureText(H) : { width: 0 }, J = Math.max(P.width, U.width) + 16, de = H ? 32 : 20, me = a - m - J - 6, ae = f + 6;
      n.fillStyle = "rgba(18, 18, 24, 0.88)", n.strokeStyle = "#38384a", n.lineWidth = 1, n.beginPath(), n.roundRect ? n.roundRect(me, ae, J, de, 4) : n.rect(me, ae, J, de), n.fill(), n.stroke(), n.fillStyle = "#e2e8f0", n.fillText(T, me + 8, ae + (H ? 13 : 14)), H && (n.fillStyle = "#94a3b8", n.font = "9.5px system-ui, -apple-system, sans-serif", n.fillText(H, me + 8, ae + 26)), n.restore();
    }
  }
...
function ad(e) {
  e.checkpoint("Auto-split shots"), e.state.sequence = {
    ...
    enabled: !0,
    cuts: ai(e.state)
  }, ut(e), e.setStatus(s("Split into {count} shots").replace("{count}", String(e.state.sequence.cuts.length)));
}
...
function od(e, t) {
  ...
  if (a.appendChild(co(
    ...
    () => {
      e.checkpoint("Split shot"), zn(e.state, e.frame, null) ? ut(e) : e.setStatus(s("Move the playhead inside a shot first"));
    },
    { disabled: o }
  )), a.appendChild(co(
    s("Clear edit"),
    ...
    () => {
      e.checkpoint("Clear edit"), e.state.sequence = { ...e.state.sequence, enabled: !1, cuts: [] }, ut(e), e.setStatus(s("Multi-camera edit cleared"));
    }
  ))), a.appendChild(co(
...
function nd(e, t, a, o, r) {
  ...
  e.contextMenu?.show(t, n?.name || s("Shot"), [
    ...e.state.cameras.map((i) => ({
      ...
      run: () => {
        e.checkpoint("Change shot camera"), e.state.sequence.cuts[o].camera_id = i.id, ut(e);
      }
    })),
    null,
    {
      ...
      run: () => {
        e.checkpoint("Split shot"), zn(e.state, e.frame, null) && ut(e);
      }
    },
    {
      label: s("Remove shot"),
      ...
      disabled: r === 1,
      run: () => {
        e.checkpoint("Remove shot"), ei(e.state, o) && ut(e);
      }
    }
  ]);
}
...
function cs(e, t) {
  ...
  if (n.className = "oc-sequence-lane", n.dataset.role = "sequence-lane", n.setAttribute("aria-label", s("Multi-camera edit")), o.length)
    for (const [c, l] of o.entries()) {
      ...
      be(e, l.end + 1) < -5 || p > 105 || n.appendChild(sd(e, l, c, r));
    }
  else {
    const c = document.createElement("span");
...
function hd(e) {
  ...
  if (e.audioWaveformPeaks && e.audioWaveformPeaks.length) {
    ...
    for (let L = 0; L < g.length; L++) {
      const W = (L / (g.length - 1) * r - l) / Math.max(1e-6, c) * _;
      if (W >= -5 && W <= _ + 5) {
        const V = g[L] * (A * 0.85);
  ...
  if (e.state.playback_range) {
    ...
    const S = be(e, e.state.playback_range[0]), g = be(e, e.state.playback_range[1]);
    x.style.left = `${S}%`, x.style.width = `${Math.max(0, g - S)}%`, t.appendChild(x);
  }
  oi(e, t);
  for (const x of e.state.markers || []) {
    const S = be(e, x.frame);
  ...
  if (o.length > 1) {
    const x = be(e, o[0].frame), S = be(e, o[o.length - 1].frame), g = document.createElement("span");
  ...
  for (const x of o) {
    ...
    _.className = "key-label", _.textContent = String(x.frame), g.appendChild(_), g.addEventListener("pointerdown", (A) => {
      if (A.preventDefault(), A.stopPropagation(), g.focus({ preventScroll: !0 }), A.altKey) {
        ...
        const L = a ? { frame: x.frame, transform: Xe(x.transform), interpolation: x.interpolation } : { frame: x.frame, camera: he(x.camera), interpolation: x.interpolation }, X = e.timelineKeyframes();
...
const at = 1, en = 50, Ae = 120, it = 160, tm = Object.freeze(["perspective", "orthographic"]), K = Object.freeze({
...
function sm(e, t) {
  ...
  switch (t?.type) {
    ...
    case xe.CAMERA_GET: {
      ...
      return we(e, { version: 1, type: t.type, camera: qe(r) });
    }
    case xe.CAMERA_LIST: {
      const o = a.cameras || [], { offset: r, limit: n, end: i } = ct(t, o.length);
      return we(e, {
        version: 1,
        type: t.type,
    ...
    case xe.ASSET_LIST: {
      ...
      return we(e, { version: 1, type: t.type, items: qe(r), total: r.length });
    }
    case xe.ASSET_GET: {
    ...
    case xe.OBJECT_LIST: {
      const o = a.objects || [], { offset: r, limit: n, end: i } = ct(t, o.length);
      return we(e, {
        version: 1,
        type: t.type,
    ...
    case xe.OBJECT_SEARCH: {
      ...
      }, p = (a.objects || []).filter(l), { offset: m, limit: f, end: d } = ct(t, p.length);
      return we(e, {
        version: 1,
        type: t.type,
    ...
    case xe.CHARACTER_LIST: {
      const o = (a.objects || []).filter((c) => c.asset_kind === "character"), { offset: r, limit: n, end: i } = ct(t, o.length);
      return we(e, {
        version: 1,
        type: t.type,
    ...
    case xe.SHOT_LIST: {
      ...
      })), { offset: i, limit: c, end: l } = ct(t, n.length);
      return we(e, {
        version: 1,
        type: t.type,
        items: n.slice(i, l),
    ...
    case xe.KEYFRAME_LIST: {
      ...
      if (!r) throw new I("UNKNOWN_CAMERA", `Unknown camera: ${o}`);
      const n = r.keyframes || [], { offset: i, limit: c, end: l } = ct(t, n.length);
      return we(e, {
        version: 1,
        type: t.type,
        cameraId: r.id,
...
const et = (e) => typeof e == "number" && Number.isFinite(e);
function Ce(e, t, a) {
  if (!Array.isArray(e) || e.length !== 3 || !e.every(et))
...
function _m(e, t) {
  ...
  if (e.position !== void 0 && Ce(e.position, "camera.position", t), e.target !== void 0 && Ce(e.target, "camera.target", t), e.up !== void 0 && Ce(e.up, "camera.up", t), e.fov !== void 0 && (Lt(e.fov, "camera.fov", t), e.fov < 1 || e.fov > 179))
...
function Cm(e, t) {
  ...
  switch (a) {
    case K.ASSET_INSTANTIATE: {
      ...
      e.point !== void 0 && Ce(e.point, "point", t), e.id !== void 0 && ee(e.id, "id", t);
      break;
    }
    ...
    case K.CAMERA_CREATE:
      if (e.id !== void 0 && ee(e.id, "id", t, Ae), e.name !== void 0 && ee(e.name, "name", t, it), e.camera !== void 0) {
    ...
    case K.CAMERA_DUPLICATE:
      ee(e.cameraId, "cameraId", t, Ae), e.id !== void 0 && ee(e.id, "id", t, Ae), e.name !== void 0 && ee(e.name, "name", t, it);
      break;
    ...
    case K.CAMERA_RENAME:
      ee(e.cameraId, "cameraId", t, Ae), ee(e.name, "name", t, it);
      break;
    case K.OBJECT_CREATE:
      ...
      e.id !== void 0 && ee(e.id, "id", t, Ae), e.name !== void 0 && ee(e.name, "name", t, it), e.position !== void 0 && Ce(e.position, "position", t), e.rotation !== void 0 && Ce(e.rotation, "rotation", t);
      break;
    case K.OBJECT_DUPLICATE:
      ee(e.objectId, "objectId", t, Ae), e.id !== void 0 && ee(e.id, "id", t, Ae), e.name !== void 0 && ee(e.name, "name", t, it), e.offset !== void 0 && Ce(e.offset, "offset", t);
      break;
    case K.OBJECT_DELETE:
      ee(e.objectId, "objectId", t, Ae);
      break;
    case K.OBJECT_RENAME:
      ee(e.objectId, "objectId", t, Ae), ee(e.name, "name", t, it);
      break;
    ...
    case K.CAMERA_TRANSFORM:
      if (e.cameraId !== void 0 && ee(e.cameraId, "cameraId", t), e.position !== void 0 && Ce(e.position, "position", t), e.target !== void 0 && Ce(e.target, "target", t), e.frame !== void 0 && Le(e.frame, "frame", t), e.position === void 0 && e.target === void 0)
        throw new I("EMPTY_OPERATION", "camera.transform needs position and/or target", t);
      break;
    case K.CAMERA_LOOK_AT:
      if (e.cameraId !== void 0 && ee(e.cameraId, "cameraId", t), e.point !== void 0 && Ce(e.point, "point", t), e.objectId !== void 0 && e.objectId !== null && ee(e.objectId, "objectId", t), e.point === void 0 && e.objectId === void 0)
        throw new I("EMPTY_OPERATION", "camera.look_at needs a point or an objectId", t);
      break;
    case K.OBJECT_TRANSFORM:
      if (ee(e.objectId, "objectId", t), e.position !== void 0 && Ce(e.position, "position", t), e.rotation !== void 0 && Ce(e.rotation, "rotation", t), e.scale !== void 0 && Ce(e.scale, "scale", t), e.position === void 0 && e.rotation === void 0 && e.scale === void 0)
    ...
    case K.CHARACTER_SET_JOINT_ROTATION:
      if (ee(e.objectId, "objectId", t), ee(e.joint, "joint", t), !Array.isArray(e.rotation) || e.rotation.length !== 4 || !e.rotation.every(et))
    ...
    case K.CHARACTER_SET_MOTION: {
      ...
      for (const r of ["start_frame", "end_frame", "speed", "offset_seconds"])
        if (o[r] !== void 0 && !et(o[r]))
          throw new I("BAD_VALUE", `motion.${r} must be a finite number`, t);
      if (et(o.start_frame) && et(o.end_frame) && o.end_frame > 0 && o.end_frame <= o.start_frame)
    ...
    case K.KEYFRAME_UPSERT:
      ...
      if (e.camera !== void 0) {
        ...
        e.camera.position !== void 0 && Ce(e.camera.position, "camera.position", t), e.camera.target !== void 0 && Ce(e.camera.target, "camera.target", t);
        for (const o of ["fov", "roll", "zoom", "near", "far"])
          if (e.camera[o] !== void 0 && !et(e.camera[o]))
...
const zm = {
  ...
  [K.CAMERA_TRANSFORM](e, t) {
    const a = dt(e, t.cameraId), o = on(a);
  ...
  [K.OBJECT_SET_ENABLED](e, t) {
    return Qe(e, t.objectId).enabled = t.value, { dirtyMask: C.viewport | C.previews | C.outliner | C.inspector };
  },
  [K.OBJECT_SET_LOCKED](e, t) {
    return or(e, t.objectId).locked = t.value, { dirtyMask: C.outliner | C.inspector };
  },
  [K.OBJECT_SET_TAGS](e, t) {
    const a = Qe(e, t.objectId), o = si(t.tags), r = o.length !== t.tags.length ? "some tags were dropped or normalised" : void 0;
    return o.length ? a.tags = o : delete a.tags, { dirtyMask: C.outliner | C.inspector | C.viewport, warning: r };
  },
  [K.OBJECT_SET_ANNOTATION](e, t) {
    const a = Qe(e, t.objectId), o = t.annotation === null ? null : Fn(t.annotation);
    if (t.annotation && !o)
  ...
  [K.KEYFRAME_REMOVE](e, t) {
    const a = dt(e, t.cameraId), o = (a.keyframes || []).length;
  ...
  [K.KEYFRAME_SET_INTERPOLATION](e, t) {
    const a = dt(e, t.cameraId), o = mo(a, t.frame);
...
function Dm(e, t, a) {
  const o = He(e?.cameras), r = He(t?.cameras);
...
function Um(e, t, a) {
  const o = He(e?.objects), r = He(t?.objects);
  for (const [n, i] of r) {
...
function gp(e, t = {}) {
  return bs.map((a) => {
    ...
    return `<button type="button" class="oc-asset-kind${a === (e || "all") ? " active" : ""}" data-asset-kind="${a}">${ht(o)}${r}</button>`;
  }).join("");
}
function yp(e, { selected: t = !1, thumbUrl: a = "" } = {}) {
  const o = up[e.kind] || "pi-box", r = e.kind === "character" && Nn(e.rig) === "rigged" ? `<span class="oc-asset-badge">${s("RIGGED")}</span>` : "", n = a ? `<img class="oc-asset-thumb" src="${ht(a)}" alt="" loading="lazy">` : `<span class="oc-asset-thumb oc-asset-thumb--glyph"><i class="pi ${o}"></i></span>`;
  return `<button type="button" class="oc-asset-card${t ? " selected" : ""}" data-asset-id="${ht(e.id)}" title="${ht(e.name)}">
    ${n}
    <span class="oc-asset-name">${ht(e.name)}</span>
    <span class="oc-asset-kind-tag">${ht((e.kind || "").toUpperCase())}</span>
    ${r}
  </button>`;
}
...
function xp(e, t = {}) {
  const a = e.root, o = t.fetchApi || ((N, D) => (e.api || e.app?.api).fetchApi(N, D)), r = t.apiClient || Er({ fetchApi: o }), n = t.store || lp(r), i = dp(), c = t.previewQueue || mp(), l = e.api || e.app?.api || null;
  let p = t.thumbnailRenderer || null, m = null;
  const f = /* @__PURE__ */ new Set();
  function d() {
    return p ? Promise.resolve(p) : (m || (m = fp().then((N) => p = dn(N)).catch(() => p = dn({}))), m);
  }
  function h(N) {
    return Kr(l, us(N));
  }
  function u(N) {
    return N.thumbnail ? Kr(l, `omnicam/library/${N.thumbnail} [input]`) : "";
  }
  function b() {
    for (const N of n.state.items) {
      ...
      D && c.enqueue(N.id, async () => {
        const ce = await (await d()).render(D, N.format || "glb");
        if (!ce) {
          f.add(N.id);
          return;
        }
        if (i.set(N.id, ce), T(), !(N.source && N.source !== "user"))
          try {
            const ne = await r.uploadThumbnail(N.id, hp(ce));
  ...
  const v = (N) => a.querySelector(`[data-role="${N}"]`), y = v("assets-panel"), x = v("asset-grid"), S = v("asset-kinds"), g = v("asset-search"), _ = v("asset-status"), A = v("asset-import-file"), F = v("scene-tab"), L = v("assets-tab"), X = v("agent-tab"), W = { assets: L, agent: X };
  let V = "", se = null, j = !0, z = 0;
  function B(N, { sticky: D = !1 } = {}) {
    _ && (!D && z > Date.now() || (_.textContent = N || "", z = D ? Date.now() + 9e3 : 0));
  }
  function G(N) {
    if (V = N, !!x)
  ...
  function T() {
    if (S && (S.innerHTML = gp(n.state.filter.kind, n.state.kinds)), x) {
      ...
      for (const D of n.state.items) {
        const re = i.get(D.id) || u(D);
        re && (N[D.id] = re);
      }
      x.innerHTML = vp(n.state.items, { selectedId: V, thumbUrls: N });
    }
    if (!te) {
      te = !0;
      try {
  ...
  function H(N) {
    ...
    ce && (e.selectedEntity = "object", e.selectedObjectId = ce, e.selectedObjectIds = /* @__PURE__ */ new Set([ce]), e.selectedKeyFrame = null), e.restoreAssets?.(), e.refreshObjects?.(), e.refreshInspector?.(), e.render?.(), e.setStatus?.(s("{name} added").replace("{name}", N.name));
  }
  async function P(N) {
    if (N) {
      B(s("Importing {name}...").replace("{name}", N.name));
      try {
        const D = await r.importModel(N, { kind: "prop", name: N.name.replace(/\.[^.]+$/, "") });
        D.asset && n.upsert(D.asset), B(s("Imported {name}").replace("{name}", D.asset?.name || N.name));
      } catch (D) {
        B(D.message || s("Import failed"));
      }
    }
  }
  ...
  async function ae(N) {
    const D = (J?.value || "").trim();
    if (!D) {
      B(s("Point to the folder you extracted the pack into."), { sticky: !0 });
      return;
    }
    z = 0, B(N ? s("Scanning {folder}...").replace("{folder}", D) : s("Installing characters from {folder}...").replace("{folder}", D));
    try {
      ...
      if (N) {
        const Ie = (re.candidates || []).map((rt) => rt.name).join(", ");
        B(
          (re.candidates || []).length ? s("{n} rig-complete character(s): {names}").replace("{n}", re.candidates.length).replace("{names}", Ie) : s("No rig-complete .glb/.fbx character found ({n} skipped)").replace("{n}", ce),
          { sticky: !0 }
        );
        return;
      }
      const ne = (re.installed || []).filter((Ie) => Ie.status !== "conflict").length, ke = (re.installed || []).filter((Ie) => Ie.status === "conflict").length;
      B(
  ...
  function pe(N) {
    F && (F.hidden = N !== "scene");
    for (const [D, re] of Object.entries(W))
      re && (re.hidden = D !== N);
    for (const D of a.querySelectorAll("[data-asset-view]"))
      D.classList.toggle("active", D.dataset.assetView === N);
    N === "assets" && j && (j = !1, n.refresh());
  }
  function fe(N) {
    const D = mn(N.target);
    if (D) {
      if (D.action === "switch-view") return pe(D.view);
      if (D.action === "filter-kind") return void n.setFilter({ kind: D.kind });
      if (D.action === "asset-add")
        return H(n.get(V));
      if (D.action === "asset-import")
        return A?.click();
      if (D.action === "local-toggle") return me();
      if (D.action === "local-scan") return void ae(!0);
      if (D.action === "local-install") return void ae(!1);
      D.action === "card" && G(D.assetId);
    }
  }
  function ue(N) {
    const D = mn(N.target);
    D?.action === "card" && H(n.get(D.assetId));
  }
  function ve() {
  ...
  const ye = n.subscribe(T);
  return y?.addEventListener("click", fe), y?.addEventListener("dblclick", ue), a.querySelector('[data-role="left-tabs"]')?.addEventListener("click", fe), g?.addEventListener("input", ve), A?.addEventListener("change", ge), T(), {
    store: n,
    switchView: pe,
    refresh: () => n.refresh(),
    dispose() {
      ye(), clearTimeout(se), y?.removeEventListener("click", fe), y?.removeEventListener("dblclick", ue), a.querySelector('[data-role="left-tabs"]')?.removeEventListener("click", fe), g?.removeEventListener("input", ve), A?.removeEventListener("change", ge), c.clear(), p?.dispose?.(), i.clear();
    }
  };
}
function wp(e, t = {}) {
...
function Ep(e, t = {}) {
  ...
  function S() {
    ...
    l && (l.textContent = j);
    const B = h(), G = y(B?.character)[j] || [0, 0, 0, 1], te = li(G);
    p.forEach((T, H) => {
      T && document.activeElement !== T && (T.value = String(Math.round(te[H] * 100) / 100));
    });
  }
  function g() {
    ...
    if (i && (i.classList.toggle("active", f), i.disabled = z, i.title = z ? s("Clear the motion clip to edit the pose") : s("Toggle FK pose editing")), n) {
      ...
      n.dataset.sig !== G && (n.dataset.sig = G, n.replaceChildren(...B.map(([te, T]) => {
        const H = document.createElement("option");
        return H.value = te, H.textContent = T, H;
  ...
  function A() {
    const j = b();
    if (!j) return;
    const z = p.map((te) => Number(te?.value) || 0), B = ii(z), G = e.directorApi?.execute({
      version: 1,
  ...
  async function L() {
    ...
    const B = z.toLowerCase().replace(Cp, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "pose";
    try {
      ...
      d.set(G.pose.id, v(G.pose)), e.setStatus?.(s("Pose saved: {name}").replace("{name}", z)), g();
    } catch (G) {
  ...
  n?.addEventListener("change", F), i?.addEventListener("click", X), r.querySelector('[data-pose-act="save"]')?.addEventListener("click", L);
  for (const j of p)
...
const hn = ["#4aa3ef", "#f2a93b", "#48c774", "#b565d8", "#ec4899"], We = [4, 8, 12, 20, 35, 60, 100];
function yt(e) {
...
function Lo(e, t, a) {
  ...
  const o = yt(e), r = o.expanded ? 220 : 138, n = Math.min(r, Math.max(80, Math.min(t, a) - 20)), i = 10, c = Math.max(0, t - n - i), l = Math.max(0, a - n - i), p = e.viewportCamera(), m = p?.position || [0, 1.5, 5], f = p?.target || [0, 0, 0], d = e.activeCameraTrack?.(), h = e.state.cameras?.length ? e.state.cameras : d ? [d] : [], u = h.flatMap(
    (_) => (_.keyframes || []).map((A) => A.camera?.position).filter(Boolean)
  );
  let b = 12;
  if (o.rangeIndex >= 0 && o.rangeIndex < We.length)
    b = We[o.rangeIndex];
  else {
    const _ = Math.max(
      Math.abs(m[0] || 0),
...
function Lp(e, t, a, o) {
  const r = Lo(e, a, o);
  if (!r) return;
  const { rx: n, ry: i, radarSize: c, cx: l, cy: p, innerRadius: m, range: f, scale: d, camPos: h, camTgt: u, cameras: b, activeTrack: v } = r, y = yt(e), x = e.state.active_camera_id || v?.id, S = (ae, pe) => ys(r, ae, pe);
  ...
  for (const ae of _) {
    const pe = m * ae;
    t.beginPath(), t.arc(l, p, pe, 0, Math.PI * 2), t.stroke(), c >= 120 && (t.font = "8px monospace", t.fillStyle = "rgba(0, 210, 211, 0.35)", t.textAlign = "left", t.fillText(`${Math.round(f * ae)}m`, l + pe + 2, p - 2));
  }
  ...
  for (const ae of e.state.objects || []) {
    ...
    const pe = vr(e.state.objects, ae).position || [0, 0, 0], [fe, ue] = S(pe[0], pe[2]);
  ...
  for (let ae = 0; ae < b.length; ae++) {
    const pe = b[ae], fe = pe.keyframes || [], ue = pe.id === x, ve = pe.color || hn[ae % hn.length];
    t.save(), t.strokeStyle = ve, t.globalAlpha = ue ? 0.95 : 0.4, t.lineWidth = ue ? 2 : 1, t.setLineDash(ue ? [] : [2, 2]), t.beginPath();
    let ge = !1;
    const ye = fe[0]?.frame, N = fe[fe.length - 1]?.frame;
    for (let D = ye; Number.isFinite(D) && D <= N; D++) {
      const re = Ee(pe, D, e.state.objects)?.position;
    ...
    for (const D of fe) {
      const re = D.camera?.position;
      if (!re) continue;
      const [ce, ne] = S(re[0], re[2]);
      if (ce < n + 4 || ce > n + c - 4 || ne < i + 4 || ne > i + c - 4) continue;
      const ke = D.frame === e.frame && ue;
  ...
  const [A, F] = S(h[0] || 0, h[2] || 0), [L, X] = S(u[0] || 0, u[2] || 0), W = 8, V = Y(A, n + W, n + c - W), se = Y(F, i + W, i + c - W), j = Y(L, n + W, n + c - W), z = Y(X, i + W, i + c - W), B = h[1] || 0, G = Op(B), te = Pp(B);
  t.strokeStyle = "rgba(255, 255, 255, 0.40)", t.lineWidth = 1, t.setLineDash([3, 3]), t.beginPath(), t.moveTo(V, se), t.lineTo(j, z), t.stroke(), t.setLineDash([]), t.fillStyle = "#ffffff", t.beginPath(), t.arc(j, z, 2.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "rgba(255, 255, 255, 0.7)", t.lineWidth = 1, t.beginPath(), t.arc(j, z, 4.5, 0, Math.PI * 2), t.stroke();
  const T = u[0] - h[0], H = u[2] - h[2], P = Math.atan2(H, T), J = (e.viewportCamera().fov || 35) * Math.PI / 360, de = Y(24 * (d / (m / 8)), 16, 38), me = t.createRadialGradient(V, se, 2, V, se, de);
  me.addColorStop(0, G + "55"), me.addColorStop(1, G + "08"), t.fillStyle = me, t.strokeStyle = G, t.lineWidth = 1.2, t.beginPath(), t.moveTo(V, se), t.lineTo(V + Math.cos(P - J) * de, se + Math.sin(P - J) * de), t.arc(V, se, de, P - J, P + J), t.closePath(), t.fill(), t.stroke(), t.fillStyle = G + "44", t.beginPath(), t.arc(V, se, 6.5, 0, Math.PI * 2), t.fill(), t.fillStyle = G, t.beginPath(), t.arc(V, se, 3.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "#ffffff", t.lineWidth = 1.5, t.beginPath(), t.moveTo(V, se), t.lineTo(V + Math.cos(P) * 7, se + Math.sin(P) * 7), t.stroke(), zp(e, t, r, G, te, P, Math.hypot(T, H)), y.hover && Fp(t, r, y.hover), t.restore();
}
function zp(e, t, a, o, r, n, i) {
  const { rx: c, ry: l, radarSize: p } = a, m = yt(e);
...
function Np(e, t, a, o) {
  ...
  if (a < n || a > n + c || o < i || o > i + c)
    return !1;
  t.preventDefault?.(), t.stopPropagation?.();
  const l = yt(e), p = vs(r, a, o);
  if (p)
    return p === "zoom_in" ? (l.rangeIndex === -1 && (l.rangeIndex = We.findIndex((h) => h >= r.range), l.rangeIndex < 0 && (l.rangeIndex = We.length - 1)), l.rangeIndex = Math.max(0, (l.rangeIndex === -1 ? 2 : l.rangeIndex) - 1)) : p === "zoom_out" ? (l.rangeIndex === -1 && (l.rangeIndex = We.findIndex((h) => h >= r.range), l.rangeIndex < 0 && (l.rangeIndex = 0)), l.rangeIndex = Math.min(We.length - 1, l.rangeIndex + 1)) : p === "center" ? l.centerMode = l.centerMode === "camera" ? "origin" : "camera" : p === "size" && (l.expanded = !l.expanded), e.render?.(), !0;
  const m = xs(e, r, a, o);
  if (m) {
    if (m.type === "camera")
...
function Rp(e, t, a, o) {
  ...
  const r = Lo(e, e.canvas.width, e.canvas.height);
  if (!r) return !1;
  const n = yt(e), { rx: i, ry: c, radarSize: l } = r, p = a >= i && a <= i + l && o >= c && o <= c + l;
  if (n.drag) {
...
function Kp(e, t, a, o) {
  ...
  if (a < n || a > n + c || o < i || o > i + c)
    return !1;
  t.preventDefault?.(), t.stopPropagation?.();
  const l = yt(e), p = Math.sign(t.deltaY || 0);
  return l.rangeIndex === -1 && (l.rangeIndex = We.findIndex((m) => m >= r.range), l.rangeIndex < 0 && (l.rangeIndex = 2)), p > 0 ? l.rangeIndex = Math.min(We.length - 1, l.rangeIndex + 1) : p < 0 && (l.rangeIndex = Math.max(0, l.rangeIndex - 1)), e.render?.(), !0;
}
function Dp(e, t, a) {
...
const ho = () => import("./chunk-CNVm0y7G.js");
function vf(e, t) {
...
function Tf(e, t, a) {
  ...
  for (const n of e.root.querySelectorAll('[data-role="viewport-bg-color"]')) {
    const i = (c) => {
      e.state.viewport_bg_color !== c.target.value && uo(e, "viewport_bg_color", "Change background colour"), e.state.viewport_bg_color = c.target.value, Se(e.root, "viewport-bg-color", c.target), e.scheduleSerialize(), e.render();
    };
    n.addEventListener("input", i, { signal: a }), n.addEventListener("change", i, { signal: a });
  }
...
function Kf(e) {
  ...
  for (let d = m; d < i.length; d++) {
    const h = i[d], u = Nf(i, d, p, e.planeAxis), b = he(c);
...
const tt = 1024 * 1024, mh = Object.freeze({
  card: 128 * tt,
  // MAX_CARD_BYTES
  model: 256 * tt,
  // MAX_MODEL_BYTES
...
function Cn(e) {
  return `${(e / tt).toFixed(e >= 10 * tt ? 0 : 1)} MB`;
}
function Ht(e, t) {
  const a = mh[t];
  if (!e || !a) return null;
...
async function wh(e, t) {
  if (!bt?.fetchApi || !/\.(mp4|mov|webm|mkv|m4v|avi)(?:\s|$)/i.test(e)) return null;
  const a = await bt.fetchApi("/majoor/omnicam/extractor/source", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function kh(e) {
  ...
  if (e.state.viewport_bg_image) {
    const o = new Image();
    o.src = Ve(e.state.viewport_bg_image), o.decode().catch(() => {
    }), e.viewportBgImage = o;
  }
  e.viewportBgSequenceImages = (e.state.viewport_bg_sequence || []).map((o) => {
    const r = new Image();
    return r.src = Ve(o), r.decode().catch(() => {
    }), r;
  });
...
async function jh(e, t) {
  ...
  try {
    const c = await Cr(bt, { route: "/majoor/omnicam/upload_model", field: "asset", file: t });
...
async function _h(e, t) {
  ...
  if (o) {
    ...
    try {
      const r = await Cr(bt, { route: "/majoor/omnicam/upload_asset", field: "asset", file: t });
...
function Eh(e) {
  ...
  const a = new Image();
  a.onload = () => {
    e.disposed || (Ge(e, "subject", a, !1, a.src), e.render(), e.setStatus(s("Upstream media refreshed")));
  }, a.src = bt.apiURL(`/view?${new URLSearchParams(t).toString()}`);
}
async function Ah(e) {
  if (!e.node) return;
  ...
  for (const f of i) {
    ...
    if (h) {
      if (d === "image" || d === "video") {
        ...
        if (u && u.value) {
          const b = String(u.value), v = /\.(mp4|mov|webm|mkv|m4v|avi)(?:\s|$)/i.test(b), y = h.widgets?.find((g) => String(g.name).toLowerCase() === "subfolder")?.value || "", x = Ve(mt(b, y)), S = e.state.objects.find((g) => g.id === "subject");
          if (S) {
          ...
          }), Ge(e, "subject", b, !1, b.currentSrc || b.src || ""), jn(e, b, { frameCount: b instanceof HTMLVideoElement ? 0 : 1 }), e.upstreamImageConnected = !0, n = !0, e.render(), e.setStatus(b instanceof HTMLVideoElement ? s("Upstream video preview synced") : s("Upstream image preview synced")));
        }
      }
      if (d === "audio") {
        l = !0;
        ...
        if (u && u.value) {
          const b = String(u.value), v = h.widgets?.find((x) => String(x.name).toLowerCase() === "subfolder")?.value || "", y = Ve(mt(b, v));
          try {
      ...
      if (d === "scene_3d" || d === "model" || d === "mesh") {
        ...
        if (u && u.value) {
          ...
          if (["glb", "gltf", "obj", "fbx", "stl", "ply"].includes(v)) {
            const y = h.widgets?.find((_) => String(_.name).toLowerCase() === "subfolder")?.value || "", x = Ve(mt(b, y)), S = `upstream_scene_${h.id}`;
            p.add(S);
            let g = e.state.objects.find((_) => _.id === S);
            g ? (g.asset = mt(b, y), g.format = v === "gltf" ? "glb" : v) : (g = {
              id: S,
              type: "model",
              format: v === "gltf" ? "glb" : v,
...
async function Lh(e) {
  ...
  try {
    let n = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: l } = await import("./chunk-Cp3jBqFs.js");
...
async function Nh(e, t) {
  ...
  try {
    ...
    e.checkpoint?.("Set background sequence"), e.state.viewport_bg_sequence = i, e.state.viewport_bg_image = "", e.viewportBgImage = null, e.viewportBgSequenceImages = i.map((c) => {
      const l = new Image();
      return l.src = Ve(c), l.decode().catch(() => {
      }), l;
...
function Kh(e) {
  ...
  for (const m of a) {
    ...
    r.fillStyle = "rgba(16,17,22,0.82)", Dh(r, A, F, g, _, 4 * c), r.fill(), v && (r.strokeStyle = v, r.lineWidth = Math.max(1, c), r.stroke()), r.fillStyle = v || "#e6e6ec", r.fillText(f, A + y, F + _ - x - 2 * c);
  }
  r.restore();
}
...
function Gh(e, t) {
  ...
  le(e, c, l, "#aaa", 2), le(e, l, b, "#aaa", 2), le(e, p, m, "#aaa", 2), le(e, p, f, "#aaa", 2), le(e, f, h, "#aaa", 2), le(e, m, d, "#aaa", 2), le(e, d, u, "#aaa", 2), le(e, v, y, "#aaa", 2), le(e, v, x, "#aaa", 2), le(e, x, g, "#aaa", 2), le(e, y, S, "#aaa", 2), le(e, S, _, "#aaa", 2);
...
function Yh(e, t) {
  ...
  for (let u = 0; u < f; u++) {
    const b = (u + 1) % f;
    le(e, d[u], d[b], "#aaa", 1.5), le(e, h[u], h[b], "#aaa", 1.5);
  }
  for (let u = 0; u < f; u += 3)
    le(e, d[u], h[u], "#aaa", 1.5);
}
function Xh(e, t) {
  ...
  for (let h = 0; h < m; h++) {
    const u = (h + 1) % m;
    le(e, f[h], f[u], "#aaa", 1.5), le(e, d[h], d[u], "#aaa", 1.5), h % 4 === 0 && le(e, f[h], d[h], "#888", 1);
  }
}
function Jh(e, t) {
  const a = t.position || [0, 1, 0], o = 0.25;
  le(e, Ue(a, [-o, 0, 0]), Ue(a, [o, 0, 0]), "#bbb", 2), le(e, Ue(a, [0, -o, 0]), Ue(a, [0, o, 0]), "#bbb", 2), le(e, Ue(a, [0, 0, -o]), Ue(a, [0, 0, o]), "#bbb", 2);
}
function Zh(e, t) {
  ...
  (e.state.cameras || []).forEach((a, o) => {
    ...
    if (!(i && e.state.view_mode === "camera")) {
      ...
      if (e.state.view_mode !== "camera") {
        const c = Ee(a, e.frame, e.state.objects), l = Re(c.position, e.viewportCamera(), e.canvas.width, e.canvas.height);
...
function cu(e, { perFrame: t = !1 } = {}) {
  ...
  if (t) {
    ...
    for (let m = c; m <= l; m++) {
      const d = p.get(m) || { frame: m, camera: Ee(a, m, e.state.objects), interpolation: "linear" };
  ...
  if (m) {
    ...
    if (t.appendChild(x), !S)
      for (const { object: g, level: _ } of y) {
        ...
        z.style.cssText = V ? "color:#fca5a5" : W ? "" : "opacity:.5;text-decoration:line-through", z.textContent = g.name || g.type, z.title = s("Double-click to rename"), z.addEventListener("dblclick", (T) => {
          T.preventDefault(), T.stopPropagation(), du(e, g, z);
        }), j.appendChild(z);
        const B = Array.isArray(g.tags) ? g.tags : [];
        if (B.length) {
          const T = document.createElement("span");
          T.className = "scene-item-tags";
          for (const H of B.slice(0, 2)) {
            const P = document.createElement("span");
            P.className = "scene-item-tag", P.textContent = H, T.appendChild(P);
          }
          if (B.length > 2) {
            const H = document.createElement("span");
            H.className = "scene-item-tag scene-item-tag-more", H.textContent = `+${B.length - 2}`, T.appendChild(H);
          }
          j.appendChild(T);
        }
        if (V) {
        ...
        G.className = "scene-item-actions", G.appendChild(r(W ? "pi-eye" : "pi-eye-slash", W ? "Hide object (Alt+Click to Isolate)" : "Show object (Alt+Click to Isolate)", !W, (T) => {
          if (T?.altKey) {
            ...
            e.toggleObject(g.id);
        }, "color:#ef4444;opacity:.7")), G.appendChild(r(g.locked ? "pi-lock" : "pi-lock-open", "Lock object", g.locked, () => $r(e, g))), G.appendChild(r("pi-copy", "Duplicate object", !1, () => e.duplicateObject?.(g.id))), g.id !== "subject" && G.appendChild(r("pi-trash", "Delete object", !1, () => e.deleteObject?.(g.id))), G.appendChild(r("pi-ellipsis-v", "Object actions", !1, (T) => e.openObjectContext(T, g.id))), A.append(se, j, G), A.title = s("Click to select · Double-click to toggle visibility · Right-click for actions");
        const te = (T = {}) => {
          ...
          if (e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectIds ||= /* @__PURE__ */ new Set(), T.ctrlKey || T.metaKey)
            ...
            const P = e.state.objects.map((de) => de.id), U = P.indexOf(e.outlinerAnchorId), J = P.indexOf(g.id);
        ...
        A.addEventListener("dblclick", () => e.toggleObject(g.id)), A.addEventListener("contextmenu", (T) => {
          ...
          (T.key === "Enter" || T.key === " ") && (T.preventDefault(), te(T));
        }), t.appendChild(A);
      }
  }
...
function Ks(e, t) {
  ...
  o.id = `${a.type}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`, o.name = `${a.name || a.type} Copy`, o.position = Ue(o.position || [0, 0, 0], [0.35, 0, 0.35]), (o.type === "model" || o.type === "glb") && e.modelUrlsById.has(a.id) ? e.modelUrlsById.set(o.id, e.modelUrlsById.get(a.id)) : o.type === "card" && e.cardMediaById.has(a.id) && Ge(e, o.id, e.cardMediaById.get(a.id), !1, o.asset || e.cardMediaAssetById?.get?.(a.id) || ""), e.state.objects.push(o), e.selectedEntity = "object", e.selectedObjectId = o.id, e.selectedObjectIds = /* @__PURE__ */ new Set([o.id]), e.serialize(), e.refreshObjects(), e.refreshKeys(), e.render(), e.setStatus(s(`${o.name} added`));
}
function fu(e, t) {
  const a = e.state.objects.find((o) => o.id === t);
...
function bu(e) {
  ...
  if (n) {
    const j = r.target_object_id || e.state.target_object_id || "", z = `T${qr()}${e.state.objects.map((B) => `${B.id}\0${B.name || B.type}`).join("|")}`;
    Xo(n, z, () => [
      Nt("", s("Manual Target (No Tracking)")),
      ...e.state.objects.map((B) => Nt(B.id, `${s("Track:")} ${B.name || B.type}`))
    ], j);
  }
  lu(e);
  ...
  ["camera-px", "camera-py", "camera-pz", "camera-tx", "camera-ty", "camera-tz", "camera-fov", "camera-roll", "camera-near", "camera-far", "camera-rx", "camera-ry", "camera-rz"].forEach((j, z) => {
    for (const B of e.root.querySelectorAll(`[data-role="${j}"]`))
      document.activeElement !== B && (B.value = String(Math.round(i[z] * 1e4) / 1e4));
  });
  ...
  for (const [j, z] of Object.entries(h))
    for (const B of e.root.querySelectorAll(`[data-role="${j}"]`))
      document.activeElement !== B && (B.value = String(Math.round(z * 1e4) / 1e4));
  ...
  if (v) {
    ...
    for (; B; ) {
      ...
      for (const T of e.state.objects)
        !z.has(T.id) && T.parent_id && z.has(T.parent_id) && (z.add(T.id), B = !0);
    }
    const G = e.state.objects.filter((T) => !z.has(T.id)), te = `P${qr()}${j}${G.map((T) => `${T.id} ${T.name || T.type}`).join("|")}`;
    Xo(v, te, () => [
      Nt("", s("No parent")),
      ...G.map((T) => Nt(T.id, T.name || T.type))
    ], t.parent_id || "");
  }
  ...
  if (u && (u.hidden = !L?.animations), b) {
    ...
    Xo(b, `A${j.join("|")}`, () => j.map((z, B) => Nt(String(B), z)), String(t.animation_index || 0));
  }
...
function wu(e) {
  ...
  e.camera.position = [a("camera-px", e.camera.position[0]), a("camera-py", e.camera.position[1]), a("camera-pz", e.camera.position[2])], e.camera.target = [a("camera-tx", e.camera.target[0]), a("camera-ty", e.camera.target[1]), a("camera-tz", e.camera.target[2])], e.camera.fov = Y(a("camera-fov", e.camera.fov), 5, 150), e.camera.roll = Y(a("camera-roll", e.camera.roll || 0), -180, 180), e.camera.near = Math.max(1e-4, a("camera-near", e.camera.near)), e.camera.far = Math.max(e.camera.near + 1e-4, a("camera-far", e.camera.far)), e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit(), qs(e), e.render();
}
function ku(e, t) {
  const a = Yt(e);
  if (!a) return;
...
function _u(e) {
  ...
  return t.forEach((o, r) => {
    ...
    const c = 0.35 + r * 0.15;
    i.position = Ue(i.position || [0, 0, 0], [c, 0, c]), (i.type === "model" || i.type === "glb") && e.modelUrlsById.has(n.id) ? e.modelUrlsById.set(i.id, e.modelUrlsById.get(n.id)) : i.type === "card" && e.cardMediaById.has(n.id) && Ge(e, i.id, e.cardMediaById.get(n.id), !1, i.asset || e.cardMediaAssetById?.get?.(n.id) || ""), e.state.objects.push(i), a.push(i.id);
...
function Iu(e) {
  ...
  const t = e.root.querySelector('[data-role="key-interp"]')?.value || e.root.querySelector('[data-role="interp"]')?.value || "ease", a = Me(e), o = je(e), r = a ? { frame: e.frame, transform: Xe(a), interpolation: t } : { frame: e.frame, camera: he(e.camera), interpolation: t }, n = o.findIndex((i) => i.frame === e.frame);
...
function Pu(e) {
  ...
  e.editingKeyFrame === n && (e.editingKeyFrame = null), e.selectedKeyFrame = r.length ? r.reduce((i, c) => Math.abs(c.frame - n) < Math.abs(i.frame - n) ? c : i).frame : null, e.camera = Ee(e.state, e.frame), e.applyObjectAnimationFrame(), e.serialize(), e.refreshKeys(), e.render(), e.setStatus(s(`${t?.name || "Camera"} key deleted @ ${n}`));
}
function Lu(e) {
  const t = Me(e), a = Ke(e) || je(e).find((o) => o.frame === e.frame);
...
function zu(e) {
  ...
  const o = t ? { frame: e.frame, transform: Xe(e.copiedKeyframe.transform), interpolation: e.copiedKeyframe.interpolation } : { frame: e.frame, camera: he(e.copiedKeyframe.camera), interpolation: e.copiedKeyframe.interpolation }, r = je(e), n = r.findIndex((i) => i.frame === e.frame);
...
function Yu(e) {
  ...
  if (e.checkpoint("Edit keyframe"), e.editingKeyFrame = t.frame, Me(e)) {
    t.interpolation = e.root.querySelector('[data-role="key-interp"]').value, t.transform = Xe(Me(e)), e.serialize(), e.setFrame(t.frame), e.setStatus(s(`Object keyframe updated @ ${t.frame}`));
    return;
  }
  const a = (o, r) => {
...
function Vs(e, t, a) {
  ...
  gt(e, "out", Ut(r.out, o)), gt(e, "in", Ut(r.in, o));
}
...
function tb(e, t, { prevKey: a = null, nextKey: o = null } = {}) {
  ...
  if (t === "corner") {
    const r = Ws(e, a, o);
    return pr(e), gt(e, "out", r.out), gt(e, "in", r.in), e;
  }
...
function rb(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: l, dispatchDirectorKey: p, activeCameraTrack: m, bindWidgetCallbacks: f, playblastCameraTrack: d, restoreFromWidgets: h, serializeEditorState: u, syncActiveCameraTrack: b, syncFromWidgets: v, bindEditorEvents: y, activateCamera: x, addCamera: S, deleteCamera: g, drawPreviewOverlays: _, duplicateCamera: A, maximizeCameraPreview: F, refreshCameraPreviews: L, refreshCameraSelectors: X, renameCamera: W, setPlayblastCamera: V, toggleCameraView: se, captureRealtime: j, makePlayblast: z, uploadDirectorPlayblast: B, waitForMediaFrame: G, computeAudioPeaks: te, loadAudioFile: T, releaseAudio: H, stopPlay: P, togglePlay: U, applyCameraPreset: J, applyCameraShake: de, applyProxyPreset: me, clearViewportBgImage: ae, loadViewportBgFile: pe, loadViewportBgSequence: fe, drawCameraPath: ue, drawCard: ve, drawCube: ge, drawGrid: ye, drawHuman: N, drawLine3D: D, drawNull: re, drawOverlays: ce, drawPointField: ne, drawSpeedHeatmap: ke, drawSphere: Ie, curveChannels: rt, drawCurveEditor: vt, onCurvePointerDown: xt, onCurvePointerMove: wt, onCurvePointerUp: kt, onTimelinePointerDown: Xt, onTimelinePointerMove: Jt, onTimelinePointerUp: Zt, refreshKeys: St, resetCurveZoom: jt, resetTimelineZoom: nt, setChannelFilter: _t, setCurveInterpolation: Ct, setTangentMode: Et, timelineFrameFromEvent: Qt, toggleCurveHandles: At, zoomCurve: ea, drawTransformGizmo: ta, frameTarget: aa, gizmoAxes: oa, gizmoGeometry: ra, onPointerDown: na, onPointerMove: sa, onPointerUp: ia, onWheel: ca, pickGizmo: la, pickSceneObject: da, resetCamera: $t, setTransformMode: Tt, setViewMode: ma, viewportCamera: pa, loadCardFile: Fo, loadExecutionPreview: No, loadMediaUrl: fa, loadModelFile: ha, loadSelectedReference: ua, onModelLoaded: ba, restoreAssets: ga, syncUpstreamInputs: ya, configureDomMedia: va, refreshSetupDiagnostic: Mt, addMediaCard: xa, addPrimitive: wa, applyObjectAnimationFrame: ka, beginCameraEdit: Sa, beginObjectEdit: ja, commitCameraEdit: _a, commitObjectEdit: Ca, copyKeyframe: Ea, deleteKeyframe: Aa, deleteObject: $a, duplicateObject: Ta, exitKeyEdit: Ma, finishCameraEdit: Ia, goToAdjacentKey: Oa, insertKeyframe: Pa, loadSelectedKeyView: La, pasteKeyframe: za, playblastCameraAtFrame: Fa, refreshInspector: Na, refreshKeyEditor: Ra, refreshObjects: Ka, removeObjectResources: Da, renameObject: Ba, retimeSelectedKey: qa, selectKeyframe: Ua, selectedKeyframe: Wa, selectedObject: Va, selectObjectAnimation: Ha, setKeyInterpolation: Ga, setObjectParent: Ya, timelineKeyframes: Xa, timelineObject: Je, toggleAutoKey: Ja, toggleObject: Za, updateCameraFromHud: Qa, updateEditState: eo, updateKeyVisualState: to, updateSelectedKey: ao, updateSelectedObject: oo, clamp: It, cloneCamera: ro, configureCore: no, defaultCamera: so, sampleCamera: Ot, sampleObjectTransform: Ro, sanitizeState: st, worldTransform: De } = e;
  return {
    setSelectMode(O) {
    ...
    restoreHistorySnapshot(O) {
      ...
      const Z = new Set(this.timelineKeyframes().map((ie) => ie.frame)), q = Array.isArray($.selectedKeyFrames) ? $.selectedKeyFrames : [$.selectedKeyFrame].filter((ie) => ie != null);
    ...
    onContextMenu(O) {
      ...
      if ($.closest?.(".viewport-wrap")) {
        const M = this.interactionElement.getBoundingClientRect(), R = (O.clientX - M.left) * this.canvas.width / Math.max(1, M.width), Z = (O.clientY - M.top) * this.canvas.height / Math.max(1, M.height), q = this.pickSceneObject([R, Z]);
        if (q) {
    ...
    openTimelineContext(O, $) {
      ...
      this.showContextMenu(O, $ ? `Keyframe F${this.selectedKeyFrame}` : `Timeline F${this.frame}`, [
        { label: s("Fit timeline view (F)"), icon: "pi-arrows-alt", shortcut: "F", run: () => nt(this) },
    ...
    resizeCanvas() {
      ...
      for (const R of this.cameraPreviewCanvases.values()) {
        const Z = R.clientWidth || 220, q = R.clientHeight || 124, ie = Math.max($, ob / Math.max(1, Z)), _e = Math.max(1, Math.round(Z * ie)), Pe = Math.max(1, Math.round(q * ie));
        (R.width !== _e || R.height !== Pe) && (R.width = _e, R.height = Pe);
      }
...
function hb(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: l, dispatchDirectorKey: p, activeCameraTrack: m, bindWidgetCallbacks: f, playblastCameraTrack: d, restoreFromWidgets: h, serializeEditorState: u, syncActiveCameraTrack: b, syncFromWidgets: v, bindEditorEvents: y, activateCamera: x, addCamera: S, deleteCamera: g, drawPreviewOverlays: _, duplicateCamera: A, maximizeCameraPreview: F, refreshCameraPreviews: L, refreshCameraSelectors: X, renameCamera: W, setPlayblastCamera: V, toggleCameraView: se, captureRealtime: j, makePlayblast: z, uploadDirectorPlayblast: B, waitForMediaFrame: G, computeAudioPeaks: te, loadAudioFile: T, releaseAudio: H, stopPlay: P, togglePlay: U, applyCameraPreset: J, applyCameraShake: de, applyProxyPreset: me, clearViewportBgImage: ae, loadViewportBgFile: pe, loadViewportBgSequence: fe, drawCameraPath: ue, drawCard: ve, drawCube: ge, drawGrid: ye, drawHuman: N, drawLine3D: D, drawNull: re, drawOverlays: ce, drawPointField: ne, drawSpeedHeatmap: ke, drawSphere: Ie, curveChannels: rt, drawCurveEditor: vt, onCurvePointerDown: xt, onCurvePointerMove: wt, onCurvePointerUp: kt, onTimelinePointerDown: Xt, onTimelinePointerMove: Jt, onTimelinePointerUp: Zt, refreshKeys: St, resetCurveZoom: jt, resetTimelineZoom: nt, setChannelFilter: _t, setCurveInterpolation: Ct, setTangentMode: Et, timelineFrameFromEvent: Qt, toggleCurveHandles: At, zoomCurve: ea, drawTransformGizmo: ta, frameTarget: aa, gizmoAxes: oa, gizmoGeometry: ra, onPointerDown: na, onPointerMove: sa, onPointerUp: ia, onWheel: ca, pickGizmo: la, pickSceneObject: da, resetCamera: $t, setTransformMode: Tt, setViewMode: ma, viewportCamera: pa, loadCardFile: Fo, loadExecutionPreview: No, loadMediaUrl: fa, loadModelFile: ha, loadSelectedReference: ua, onModelLoaded: ba, restoreAssets: ga, syncUpstreamInputs: ya, configureDomMedia: va, refreshSetupDiagnostic: Mt, addMediaCard: xa, addPrimitive: wa, applyObjectAnimationFrame: ka, beginCameraEdit: Sa, beginObjectEdit: ja, commitCameraEdit: _a, commitObjectEdit: Ca, copyKeyframe: Ea, deleteKeyframe: Aa, deleteObject: $a, deleteSelectedObjects: Ta, duplicateObject: Ma, exitKeyEdit: Ia, finishCameraEdit: Oa, goToAdjacentKey: Pa, insertKeyframe: La, loadSelectedKeyView: za, pasteKeyframe: Fa, playblastCameraAtFrame: Na, refreshInspector: Ra, refreshKeyEditor: Ka, refreshObjects: Da, removeObjectResources: Ba, renameObject: qa, retimeSelectedKey: Ua, selectKeyframe: Wa, selectedKeyframe: Va, selectedObject: Ha, selectObjectAnimation: Ga, setKeyInterpolation: Ya, setKeyTangentMode: Xa, setObjectParent: Je, timelineKeyframes: Ja, timelineObject: Za, toggleAutoKey: Qa, toggleObject: eo, updateCameraFromHud: to, updateCameraRotationFromHud: ao, updateEditState: oo, updateKeyVisualState: It, updateSelectedKey: ro, updateSelectedObject: no, clamp: so, cloneCamera: Ot, configureCore: Ro, defaultCamera: st, sampleCamera: De, sampleObjectTransform: O, sanitizeState: $, worldTransform: k } = e;
  return {
    setChannelFilter(w) {
      _t(this, w);
    },
    setFrame(w, E = !1, M = !0) {
      this.frame = so(Math.round(w), 0, this.state.duration_frames - 1), this.editingKeyFrame !== this.frame && (this.editingKeyFrame = null), this.camera = De(this.activeCameraTrack(), this.frame, this.state.objects), Kt(this, this.activeCameraTrack(), this.camera, this.frame), this.applyObjectAnimationFrame();
      ...
      const q = Math.floor(Z / 60), ie = Math.floor(Z % 60), _e = Math.floor(Z % 1 * 1e3), Pe = this.frame % Math.max(1, Math.round(this.state.fps)), Ze = Math.floor(this.frame / this.state.fps);
      if ((R.time || this.root.querySelector('[data-role="time"]')).textContent = this.state.timecode_mode === "timecode" ? `${String(Math.floor(Ze / 3600)).padStart(2, "0")}:${String(Math.floor(Ze / 60) % 60).padStart(2, "0")}:${String(Ze % 60).padStart(2, "0")}:${String(Pe).padStart(2, "0")}` : `${String(q).padStart(2, "0")}:${String(ie).padStart(2, "0")}.${String(_e).padStart(3, "0")}`, M) this.refreshKeys();
      else {
        _o(this);
    ...
    transformCameraPath(w) {
      ...
      return E.keyframes = M, E.id === this.state.active_camera_id && (this.state.keyframes = M), this.camera = De(E, this.frame, this.state.objects), E.camera = Ot(this.camera), this.serialize(), this.refreshKeys(), this.refreshInspector(), this.render(), this.renderCameraView?.(), !0;
    },
    toggleCurveHandles() {
      At(this);
    },
    ...
    playblastCameraAtFrame() {
      return Kt(this, d(this), Na(this, De), this.frame);
    },
    viewportCamera() {
      return pa(this);
    },
    ...
    lookAtObject(w) {
      ...
      if (E) {
        ...
        this.camera = De(this.state, this.frame), this.serialize(), this.refreshKeys(), this.render(), this.setStatus(`Cameras look at ${E.name || E.type}`);
      }
    },
    setTransformMode(w) {
      Tt(this, w);
    ...
    aimAtSelectedObject(w) {
      ...
      const E = this.activeCameraTrack(), M = w && this.state.objects.find((q) => q.id === w) || this.selectedObject() || this.state.objects.find((q) => q.id === "subject") || this.state.objects[0];
      if (!M) return;
    ...
    setCameraTrackingTarget(w) {
      ...
      const E = this.activeCameraTrack();
      E.target_object_id !== (w || null) && (E.aim_bone = null), E.target_object_id = w || null, E.id === this.state.active_camera_id && (this.state.target_object_id = w || null, this.state.aim_bone = E.aim_bone), this.camera = De(E, this.frame, this.state.objects), Kt(this, E, this.camera, this.frame), this.serialize(), this.refreshInspector(), this.render(), this.setStatus(w ? `Camera tracking: ${w}` : "Camera tracking disabled (manual target)");
    },
    bakeAimToKeyframes() {
      ...
      for (const Z of w.keyframes) {
        const q = (M.type === "model" || M.type === "glb") && R && !M.keyframes?.length ? R : M.keyframes?.length ? O(M, Z.frame).position : M.position || [0, 1.5, 0];
        Z.camera.target = [...q];
      }
...
const xb = "http://www.w3.org/2000/svg", pt = 26, In = 17, wb = 5.4;
function ft(e, t) {
...
function kb(e) {
  ...
  for (const n of yb(gb(a))) {
    const i = pt + n.x * In, c = pt + n.y * In, l = vb(n.depth);
    t.appendChild(ft("line", {
      x1: pt,
      y1: pt,
      x2: i,
      y2: c,
...
function Ob(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: l, dispatchDirectorKey: p, activeCameraTrack: m, bindWidgetCallbacks: f, playblastCameraTrack: d, restoreFromWidgets: h, serializeEditorState: u, syncActiveCameraTrack: b, syncFromWidgets: v, bindEditorEvents: y, activateCamera: x, addCamera: S, deleteCamera: g, drawPreviewOverlays: _, duplicateCamera: A, maximizeCameraPreview: F, refreshCameraPreviews: L, refreshCameraSelectors: X, renameCamera: W, setPlayblastCamera: V, toggleCameraView: se, captureRealtime: j, makePlayblast: z, uploadDirectorPlayblast: B, waitForMediaFrame: G, computeAudioPeaks: te, loadAudioFile: T, releaseAudio: H, stopPlay: P, togglePlay: U, applyCameraPreset: J, applyCameraShake: de, applyProxyPreset: me, clearViewportBgImage: ae, loadViewportBgFile: pe, loadViewportBgSequence: fe, drawCameraPath: ue, drawCard: ve, drawCube: ge, drawCylinder: ye, drawGrid: N, drawHuman: D, drawLine3D: re, drawNull: ce, drawOverlays: ne, drawPointField: ke, drawSpeedHeatmap: Ie, drawSphere: rt, drawTorus: vt, curveChannels: xt, drawCurveEditor: wt, onCurvePointerDown: kt, onCurvePointerMove: Xt, onCurvePointerUp: Jt, onTimelinePointerDown: Zt, onTimelinePointerMove: St, onTimelinePointerUp: jt, refreshKeys: nt, resetCurveZoom: _t, resetTimelineZoom: Ct, setChannelFilter: Et, setCurveInterpolation: Qt, setTangentMode: At, timelineFrameFromEvent: ea, toggleCurveHandles: ta, zoomCurve: aa, drawTransformGizmo: oa, frameTarget: ra, gizmoAxes: na, gizmoGeometry: sa, onPointerDown: ia, onPointerMove: ca, onPointerUp: la, onWheel: da, pickGizmo: $t, pickSceneObject: Tt, resetCamera: ma, setTransformMode: pa, setViewMode: Fo, viewportCamera: No, loadCardFile: fa, loadExecutionPreview: ha, loadMediaUrl: ua, loadModelFile: ba, loadSelectedReference: ga, onModelLoaded: ya, restoreAssets: va, syncUpstreamInputs: Mt, configureDomMedia: xa, refreshSetupDiagnostic: wa, addMediaCard: ka, addPrimitive: Sa, applyObjectAnimationFrame: ja, beginCameraEdit: _a, beginObjectEdit: Ca, commitCameraEdit: Ea, commitObjectEdit: Aa, copyKeyframe: $a, deleteKeyframe: Ta, deleteObject: Ma, duplicateObject: Ia, exitKeyEdit: Oa, finishCameraEdit: Pa, goToAdjacentKey: La, insertKeyframe: za, loadSelectedKeyView: Fa, pasteKeyframe: Na, playblastCameraAtFrame: Ra, refreshInspector: Ka, refreshKeyEditor: Da, refreshObjects: Ba, removeObjectResources: qa, renameObject: Ua, retimeSelectedKey: Wa, selectKeyframe: Va, selectedKeyframe: Ha, selectedObject: Ga, selectObjectAnimation: Ya, setKeyInterpolation: Xa, setObjectParent: Je, timelineKeyframes: Ja, timelineObject: Za, toggleAutoKey: Qa, toggleObject: eo, updateCameraFromHud: to, updateEditState: ao, updateKeyVisualState: oo, updateSelectedKey: It, updateSelectedObject: ro, clamp: no, cloneCamera: so, configureCore: Ot, defaultCamera: Ro, sampleCamera: st, sampleObjectTransform: De, sanitizeState: O, worldTransform: $ } = e;
  return {
    ...
    renderViewportOnly() {
      ...
      if (k.fillStyle = this.state.viewport_bg_color || "#121212", k.fillRect(0, 0, w, E), this.viewportBgSequenceImages && this.viewportBgSequenceImages.length) {
        const oe = this.frame % this.viewportBgSequenceImages.length, Be = this.viewportBgSequenceImages[oe];
        if (Be?.complete && Be.naturalWidth)
          try {
            k.drawImage(Be, 0, 0, w, E);
          } catch {
          }
      ...
      if (this.webgl)
        try {
          const oe = this.recording ? 1 : this.webgl.supersampleFactor?.() ?? 1, Be = oe > 1 ? Math.min(oe, 4096 / Math.max(1, w, E)) : 1, io = Be > 1 ? Math.round(w * Be) : w, Ko = Be > 1 ? Math.round(E * Be) : E;
          this.webgl.render(Ze, R, this.cardMediaById, io, Ko, this.modelUrlsById, this.frame, this.recording, this.selectedEntity, this.selectedObjectId, this.subSelection, this.selectedKeyFrame ?? null), k.imageSmoothingEnabled = !0, k.imageSmoothingQuality = "high", io !== w || Ko !== E ? k.drawImage(this.webgl.canvas, 0, 0, io, Ko, 0, 0, w, E) : k.drawImage(this.webgl.canvas, 0, 0, w, E), Q = !0;
        } catch (oe) {
    ...
    requestUiUpdate(k = C.viewport, w = "unknown") {
      (this.renderReasons ||= /* @__PURE__ */ new Set()).add(w), this.uiDirtyMask = Am(this.uiDirtyMask, k), this.renderInvalidations = (this.renderInvalidations || 0) + 1, !this.renderScheduled && (this.renderScheduled = !0, this.renderFrame = requestAnimationFrame(() => {
        ...
        this.uiDirtyMask = 0, this.lastRenderReasons = [...this.renderReasons || []], this.renderReasons?.clear(), this.rendersCoalesced = (this.rendersCoalesced || 0) + 1, this.perf && (this.perf.renderCount = (this.perf.renderCount || 0) + 1), lt(E, C.outliner) && this.refreshObjects(), lt(E, C.timeline) && this.refreshKeys(), lt(E, C.inspector) && this.refreshInspector(), lt(E, C.viewport) && this.renderViewportOnly(), lt(E, C.previews) && this.renderCameraView(), lt(E, C.motion) && this.renderMotionUiOnly();
      }));
    },
    renderCameraView() {
      if (this.perf && (this.perf.previewRenderCount = (this.perf.previewRenderCount || 0) + 1), this.state.camera_view_visible) {
        ...
        let M = null;
        if (E) {
          const R = this.state.active_camera_id, Z = w.filter((q) => q.id !== R);
          M = Z.length ? Z[this.cameraPreviewTick % Z.length] : null;
        }
        for (const R of w) {
          const Z = this.cameraPreviewCanvases.get(R.id), q = this.cameraPreviewContexts.get(R.id);
          if (!Z?.width || !q) continue;
          const ie = Z.width, _e = Z.height, Pe = this.root.querySelector(`[data-camera-frame="${R.id}"]`);
          if (Pe && (Pe.textContent = `F${this.frame}`), E && R.id !== this.state.active_camera_id && R !== M) continue;
          const Ze = Kt(this, R, st(R, this.frame, this.state.objects), this.frame);
          if (q.fillStyle = "#111", q.fillRect(0, 0, ie, _e), this.cameraWebgl)
            try {
              this.cameraWebgl.render({ ...this.state, keyframes: [], playblast_grid: !1, viewport_bg_image: this.viewportBgImage?.src || "", viewport_bg_sequence: (this.viewportBgSequenceImages || []).map((Q) => Q.src), __omnicamRevision: this.renderRevision || 0 }, Ze, this.cardMediaById, ie, _e, this.modelUrlsById, this.frame, !0), q.drawImage(this.cameraWebgl.canvas, 0, 0, ie, _e);
            } catch (Q) {
...
function Wb() {
  return {
    ...
    deleteSelectedKeyframes() {
      ...
      const t = this._activeTrack(), a = je(this), o = t.kind === "camera" ? 1 : 0, { keys: r, removed: n } = Kb(a, e, { minKeys: o });
    ...
    nudgeSelectedKeyframes(e) {
      ...
      const a = this._activeTrack(), o = Math.max(0, this.state.duration_frames - 1), r = Db(je(this), t, e, { lastFrame: o });
    ...
    setSelectedKeysInterpolation(e) {
      ...
      this.checkpoint(s("Interpolation on {n} keys").replace("{n}", t.length)), a.write(Bb(je(this), t, e)), this.serialize(), this.refreshKeys(), this.refreshKeyEditor(), this.render(), this.drawCurveEditor(), this.setStatus(s("{mode} interpolation on {n} keys").replace("{mode}", e.replace(/_/g, " ")).replace("{n}", t.length));
    },
    setSelectedKeysTangentMode(e) {
      const t = this.resolveSelectedFrames();
      if (t.length < 2) return this.setTangentMode(e);
      const a = this._activeTrack(), o = ot(this).map((r) => r.id);
      this.checkpoint(s("Tangents on {n} keys").replace("{n}", t.length)), a.write(qb(je(this), t, e, o)), this.serialize(), this.refreshKeys(), this.render(), this.drawCurveEditor(), this.setStatus(s("{mode} tangents on {n} keys").replace("{mode}", e).replace("{n}", t.length));
    },
    /**
    ...
    simplifyActiveKeys({ mode: e = "simplify", tolerance: t = 0, target: a = 0, scope: o = "camera", fromKeys: r = null, silent: n = !1 } = {}) {
      ...
      const l = this.resolveSelectedFrames(), p = o !== "all_cameras" && l.length >= 2 ? [l[0], l.at(-1)] : null, m = p ? [] : l, f = (u) => {
        ...
        let _ = S, A = 0;
        if (S.length > 2) {
          const F = e === "reduce" ? Nb(S, i, { target: a || Math.ceil(S.length / 2), keepFrames: m }) : e === "clean" ? Rb(S, i, { keepFrames: m }) : Fb(S, i, { tolerance: t * Ub, keepFrames: m });
      ...
      return tr(this), this.selectedKeyFrame = null, this.selectedKeyFrames = /* @__PURE__ */ new Set(), this.camera = Ee(this.state, this.frame), this.applyObjectAnimationFrame(), this.serialize(), this.refreshKeys(), this.setFrame(this.frame, !1, !1), this.render(), n || this.setStatus(d ? s("Removed {n} keyframes").replace("{n}", d) : s("No keyframes to remove")), d;
    },
...
class Zs {
  constructor(t) {
    this.app = Pn, this.api = Ye, this.node = t, this.root = ps(), this.root.tabIndex = -1, this.dom = gs(this.root), this.canvas = this.root.querySelector(".viewport-wrap > canvas"), this.cameraPreviewCanvases = /* @__PURE__ */ new Map(), this.cameraPreviewContexts = /* @__PURE__ */ new Map(), this.cameraPreviewSignature = "", this.interactionElement = this.canvas, this.interactionElement.tabIndex = 0, this.interactionElement.dataset.captureWheel = "true", this.ctx = this.canvas.getContext("2d", { alpha: !1 }), this.disposed = !1, this.renderRevision = 0, this.directorRevision = 0, this.webgl = null, this.cameraWebgl = null, this.webglReady = this.loadWebGLViewports(), this.stateWidget = t.widgets?.find((o) => o.name === "state_json"), this.recordingWidget = t.widgets?.find((o) => o.name === "recording_path"), this.cardWidget = t.widgets?.find((o) => o.name === "card_asset"), this.widthWidget = t.widgets?.find((o) => o.name === "width"), this.heightWidget = t.widgets?.find((o) => o.name === "height"), this.fpsWidget = t.widgets?.find((o) => o.name === "fps"), this.durationWidget = t.widgets?.find((o) => o.name === "duration_seconds"), this.modeWidget = t.widgets?.find((o) => o.name === "render_mode");
    let a = null;
    try {
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-Cp3jBqFs.js"));
    } catch (a) {
