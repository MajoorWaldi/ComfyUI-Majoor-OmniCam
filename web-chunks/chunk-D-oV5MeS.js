import { bQ as t } from "./chunk-yjHTInyb.js";
import { H as n } from "./chunk-EFYmn-AY.js";
const d = `
  .majoor-omnicam{
    ${t}
    ${n}
...
`, m = '<svg class="oc-mark" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><circle class="oc-mark-disc" cx="16" cy="16" r="16"/><circle class="oc-mark-ring" cx="16" cy="16" r="7.6"/><circle class="oc-mark-core" cx="16" cy="16" r="5.8"/></svg>';
function p(r) {
  return `<div class="oc-heading"><span class="oc-brand">${m}</span><span class="oc-title">${r}</span></div>`;
}
function x(r) {
  return (a) => {
    if (!a.ctrlKey)
      for (let o = a.composedPath?.()[0] || a.target; o && o !== r; o = o.parentNode) {
        if (!(o instanceof HTMLElement)) continue;
        const i = getComputedStyle(o);
        if (/(auto|scroll)/.test(i.overflowY) && o.scrollHeight - o.clientHeight > 1) {
          const c = o.scrollTop <= 0, e = o.scrollTop + o.clientHeight >= o.scrollHeight - 1;
          (a.deltaY < 0 && !c || a.deltaY > 0 && !e) && a.stopPropagation();
          return;
        }
        if (/(auto|scroll)/.test(i.overflowX) && o.scrollWidth - o.clientWidth > 1 && a.deltaX !== 0) {
          a.stopPropagation();
          return;
        }
      }
