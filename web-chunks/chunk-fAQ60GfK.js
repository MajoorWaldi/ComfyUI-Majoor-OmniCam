...
import { d as $e, u as Pe, l as Le, S as Fe, b as Oe, p as De } from "./chunk-CUiAG3lm.js";
import { M as qe, E as je } from "./chunk-CbqXtcpr.js";
import { u as f, v as fe, x as Ve, y as Ue } from "./chunk-Cgt_u7Uc.js";
import { c as me, S as ge, F as be, a as L, m as z, b as K, n as Ge, L as Be, e as We, r as He, d as Qe, f as ze, s as Ke, g as Ye, p as Xe } from "./chunk-CghSef64.js";
import { w as Je } from "./chunk-BvpYpyx-.js";
function Ze(t) {
...
async function Mt(t, { selectElement: e = null, statusElement: r = null, checkpointSelectElement: o = null } = {}) {
  ...
  if (o) {
    ...
    d(l("auto", "Auto"));
    const h = Array.isArray(c?.metadata?.checkpoints) ? c.metadata.checkpoints : [];
    for (const u of h)
      d(l(u, u));
    o.value = "auto";
  }
...
function $t(t, {
  ...
  const i = [], s = (p, g, b) => {
    n(p, g, b), i.push(() => p?.removeEventListener?.(g, b));
  }, c = [
  ...
  const h = t.querySelector('[data-role="reconstruction-run"]');
  h && s(h, "click", e);
...
function jt(t, e) {
  ...
  if (d)
    if (e?.summary) {
      d.hidden = !1;
      const u = e.summary, m = u.triangle_count != null ? u.triangle_count : u.mesh_triangles, p = m != null ? Number(m).toLocaleString() : null, g = u.camera_fov_x != null ? u.camera_fov_x : u.camera_fov, b = g != null ? Number(g).toFixed(1) : null, w = Number(u.ground_confidence) > 0 ? f("ground plane detected") : null, x = [];
      p && x.push(`${p} ${f("triangles")}`), b && x.push(`FOV ${b}°`), w && x.push(w), d.textContent = x.join(" • ");
    } else
      d.hidden = !0, d.textContent = "";
  const h = t.querySelector('[data-role="reconstruction-warnings"]');
  if (h) {
    const u = e?.warnings || [];
    if (u.length > 0) {
      h.hidden = !1, h.replaceChildren();
      for (const m of u) {
        const p = document.createElement("div");
        p.className = "oc-warning-item", p.textContent = `⚠ ${m}`, h.appendChild(p);
      }
    } else
      h.hidden = !0, h.replaceChildren();
  }
}
class Vt {
  constructor({
    ...
    on: l = (d, h, u) => d?.addEventListener?.(h, u)
  }) {
    ...
    d && (this.state.source = d), this.unbindControls = $t(this.root, {
      ...
      onSettingsChange: (p) => {
        P(this.node, this.root), this.dispatch({ type: "SETTINGS", settings: p });
      },
      on: this.on
    }), this.syncFromWidgets(), P(this.node, this.root), this.preview = null, this.previewLoad = null, this.previewOpen = !1;
    const h = this.root?.querySelector?.('[data-role="reconstruction-preview-toggle"]');
    h && this.on(h, "click", () => this.togglePreview());
  ...
  async ensurePreview() {
    return this.preview || this.disposed ? this.preview : (this.previewLoad ||= import("./chunk-D85bX2uq.js").then(({ TrackViewer: e }) => {
...
function Jt({ pitch: t = 0, yaw: e = 0, roll: r = 0 } = {}) {
  ...
  const [o, a, n] = [t, e, r].map((u) => (Number(u) || 0) * (Math.PI / 180) * 0.5), [i, s, c, l, d, h] = [
    Math.cos(o),
    Math.sin(o),
    Math.cos(a),
    Math.sin(a),
...
function ur(t, e, r = e?.width, o = e?.height) {
  ...
  const l = Math.min(s / n, c / i), d = Math.round(n * l), h = Math.round(i * l);
  return a.clearRect(0, 0, s, c), a.drawImage(e, Math.round((s - d) / 2), Math.round((c - h) / 2), d, h), !0;
}
class dr {
  ...
  async load(e, r, { maxDimension: o = 960 } = {}) {
    ...
    try {
      ...
      if (a !== this.generation || n.signal.aborted)
        return l?.close?.(), !1;
      const d = I(s.headers, "X-OmniCam-Width", l?.width), h = I(s.headers, "X-OmniCam-Height", l?.height);
      let u = !1;
      try {
        u = ur(this.canvas, l, d, h);
      } finally {
        l?.close?.();
      }
...
function yr(t, e = t?.graph, r = "camera_track") {
  const o = r === "scene_reconstruct", a = o ? hr : pr, n = o ? mr : fr, i = o ? "Load Image" : "Load Video", s = o ? "an image" : "a video", c = o ? "reconstruct" : "track", l = gr(o), d = vr(t, e);
  if (d) {
    const u = a[ie(d)];
    if (!u) {
      const p = se(t);
...
function Dr(t, { y: e, height: r, width: o, frameCount: a, colorAt: n, style: i }) {
  ...
  for (let h = 0; h < s; h += l) {
    const u = n(h, Math.min(s, h + l));
    u && (t.fillStyle = u, t.fillRect(i.labelWidth + h / s * c, e, d, r));
  }
}
function qr(t, e, r, o) {
...
function Br(t, {
  ...
  const l = Math.max(1, Number(i) || Number(e?.duration_frames) || 1), d = Ae(e, s), { rows: h, style: u } = Me(s, c), m = {
    total: l,
    labelWidth: u.labelWidth,
    lanes: h.filter((v) => v.kind === "lane").map((v) => ({
      key: v.key,
      top: v.top,
    ...
    anomalies: Fr(a, l)
  }, p = t?.getContext?.("2d"), g = t?.width || 0, b = t?.height || 0;
  if (!p || !g || !b) return { ...m, keys: d };
  p.clearRect(0, 0, g, b);
  const w = Array.isArray(r?.frame_grades) ? r.frame_grades : [], x = {
    solve: (v, y) => Ne[qr(o, w, v, y)]
  };
  for (const v of h) {
    u.rowChrome && Gr(p, { row: v, width: g, style: u });
    const y = v.top + v.height / 2;
    if (u.labels && Vr(p, v.label, y, "#9a9aad"), v.kind === "band") {
      const A = x[v.key];
      if (!A) continue;
      ...
      Dr(p, {
        ...
        colorAt: A,
        style: u
      }), v.key === "solve" && Or(p, m.anomalies, v, g, l, u);
      continue;
    }
    const k = d[v.key] || [];
    k.length > 1 && !u.rowChrome && (p.strokeStyle = "#2c2c38", p.lineWidth = 1, p.beginPath(), p.moveTo(_(k[0], g, l, u), y), p.lineTo(_(k[k.length - 1], g, l, u), y), p.stroke()), p.fillStyle = Nr[v.key] || "#8b7bd8";
    ...
    for (const A of k) {
      ...
      p.beginPath(), p.moveTo(S, y - N), p.lineTo(S + N, y), p.lineTo(S, y + N), p.lineTo(S - N, y), p.closePath(), p.fill();
    }
  }
  if (u.ruler) {
    p.fillStyle = "#3a3a48";
    ...
    for (let y = 0; y <= v; y += 1) {
      ...
      p.fillRect(_(k, g, l, u), b - 6, 1, 5);
    }
  }
  if (u.playhead) {
    const v = _(Math.max(0, Math.min(l - 1, Number(n) || 0)), g, l, u);
    p.fillStyle = "#e6e6f0", p.fillRect(Math.round(v), 0, 1, b);
  }
  return { ...m, keys: d };
}
function $(t) {
...
function Yr(t, {
  ...
  const n = (u) => t?.querySelector?.(Qr[u]) || null, i = () => r() || {}, s = () => zr(i(), o()), c = (u) => W(i()) < 1 ? !1 : (e?.seek?.(u, "transport"), !0), l = (u) => {
    ...
    "next-key": () => l(1),
    "last-frame": () => c(W(i()) - 1),
    "toggle-loop": () => (e?.setLoop?.(!e?.loop), h(), !0)
  };
  for (const u of Hr) {
    const m = n(u);
    m && a(m, "click", () => d[u]());
  ...
  function h() {
    const u = Number(i().frame) || 0, m = s(), p = n("previous-key");
    p && (p.disabled = B(u, -1, m) === null);
    const g = n("next-key");
...
class Zr {
  ...
  draw({ sourceWidth: e = 0, sourceHeight: r = 0 } = {}) {
    ...
    for (const s of this.vectors) {
      const [c, l] = H(s.from ?? s, i), [d, h] = H(s.to ?? s, i);
      o.strokeStyle = T[s.state] || T.accepted, o.beginPath(), o.moveTo(c, l), o.lineTo(d, h), o.stroke();
    }
...
function ro(t, e, { onAction: r = () => {
  ...
  for (const i of e) {
    ...
    const l = n.createElement("strong"), d = Number(i.start_frame ?? i.frame), h = Number(i.end_frame ?? i.frame);
    l.textContent = d === h ? `Frame ${d}` : `Frames ${d}-${h}`, l.tabIndex = 0, l.setAttribute("role", "button"), l.addEventListener("click", () => o(i.frame)), l.addEventListener("keydown", (p) => {
      (p.key === "Enter" || p.key === " ") && (p.preventDefault(), o(i.frame));
    });
    ...
    for (const p of ["interpolate", "ignore", "exclude"]) {
      ...
      g.type = "button", g.textContent = p.toUpperCase(), g.dataset.action = p, g.dataset.frame = String(i.frame), p === m && g.setAttribute("aria-selected", "true"), g.addEventListener("click", () => r(i, p)), s.append(g);
    }
    t.append(s);
  }
  return e.length;
...
function ao(t) {
  return import("./chunk-D85bX2uq.js").then(({ TrackViewer: e }) => (t.viewerLoad = null, t.disposed || t.viewer || (t.viewer = new e(t.$("track-canvas")), t.pushTracksToViewer()), t.viewer)).catch((e) => (t.viewerLoad = null, console.warn("OmniCam track viewer unavailable", e), null));
}
function de(t) {
  const e = t.$("frame");
...
function go(t) {
  ...
  }, e.unwatchGraphConnections = Je(t, () => setTimeout(a, 0));
...
class so {
  constructor(e) {
    ...
    }), this.overlay = new Zr(this.$("tracking-overlay")), this.viewer = null, this.viewerLoad = null, this.queuePromptId = "", this.unbindQueueEvents = vt(this, C), this.extractMode = String(he(this.node, "extract_mode")?.value || "camera_track"), this.reconstruction = new Vt({
      root: this.root,
      node: this.node,
      api: C,
      app: Z,
      getSource: () => this.state.source?.ref || null,
      onAdopt: (n) => Qe(this.node, n),
  ...
  acceptSolvedResult(e) {
    ...
    return K(this.node, { motionScene: i, fingerprint: a }), e?.source && ze(this.node, e.source), this.node.__majoorOmniCamStatus = Ke({ track: o, confidence: n }), this.dispatch({ type: "APPLIED", fingerprint: a }), e?.source && this.refreshSource(), !0;
  }
  /**
  ...
  async estimateUp() {
    ...
    for (const [d, h] of [["pitch", c], ["yaw", 0], ["roll", l]]) {
  ...
  render() {
    ...
    e && (e.dataset.tone = pt(this.state.solveState), this.$("solve-status-text").textContent = ht(this.state));
    ...
    const p = this.$("extractor-fps");
    p && (p.textContent = String(this.sourceViewer.fps || 24)), ro(this.$("anomalies"), this.state.anomalies, {
    ...
    }), this.renderTimeline(), this.transport.render(), de(this), pe(this);
  ...
  setExtractMode(e) {
    ...
    const s = he(this.node, "extract_mode");
