...
function si(e, { mode: t, selectedIds: a } = {}) {
...
const ci = "omnicam_humanoid_v1", ht = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function pi({ preset: e, overrides: t } = {}) {
  const a = Ne(e), r = Ne(t);
  return {
...
const Lt = ["speed", "angular_speed", "acceleration", "jerk"], Qe = ["ok", "warn", "over"], ya = 0.8, Qo = [0, 1.5, 0];
function Ft(e, t) {
  const a = [0];
  ...
  return r.map((i, l) => {
    ...
    if (m.position = s[l], Array.isArray(d))
      if (n)
        ...
        const p = G(A(d, c)) || 5, u = $n(s, l);
        m.target = C(m.position, k(u, p));
      }
    return { ...i, camera: m };
  });
}
function Ki(e, t, { delta: a = [0, 0, 0] } = {}) {
...
function Xn(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function Yn(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  s.keyframes = Wn(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = ce(s, e.frame, e.state.objects), s.camera = B(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function Zt(e, t) {
  const a = [];
...
function Qn(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => Zn(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((M) => M[b]))), m = [0, 1, 2].map((b) => Math.max(...i.map((M) => M[b]))), c = l.map((b, M) => (b + m[M]) / 2), { right: d, up: p, forward: u } = V(t), h = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), _ = f * h;
  let y = 2, S = 0.1;
  for (const b of i) {
...
function qi(e, t = {}) {
  ...
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), Qn(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? v("Framed: all objects") : v("Framed: {name}").replace("{name}", s.name || s.type || v("Subject")));
}
function es(e, t, a, { forceLocal: r = !1 } = {}) {
...
function Rr(e) {
  const t = ts(e);
  if (!t || e.transformControlsWiring && as.has(t.type)) return null;
  ...
  const n = Math.max(0.7, G(A(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? es(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
...
function Hi(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : os(e.ctx, s[0], i);
    }
  }
...
function Oe(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = ss(a), m = a.axis ? lt[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], d = V(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...A(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, M = m ? k(m, b) : C(k(d.right, n * p * i), k(d.up, -s * p * i));
    u = u.map((w) => C(w, M)), u = is(e, a, u, o, t.ctrlKey || t.metaKey, m, a.snapshots.map((w) => w.transform.position));
  }
  ...
  a.snapshots.forEach((b, M) => {
    ...
    if (a.mode === "translate" && (w.position = u[M]), a.mode === "rotate") {
      const D = y ? qe(h, 15) : h, g = k(_, D);
      w.position = C(a.pivot, Ce(A(b.transform.position, a.pivot), g)), w.rotation = C(b.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const D = y ? qe(f, 0.1) : f, g = m ? m.map((E) => E ? D : 1) : [D, D, D], x = A(b.transform.position, a.pivot);
...
const oe = (e, t) => Math.round(e / t) * t, ms = (e, t) => e.map((a) => oe(a, t));
function ds(e, t, a) {
...
function ge(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, m) => Math.abs(o.axis[m]) > 1e-6 ? oe(l, i) : o.base[m]) : ms(t, i);
  if (s === "vertex" && a && !o) {
...
function Wi(e, t) {
  ...
  if (c) {
    ...
    if (c.entity.type === "camera_path" && Xn(e, { baseDrag: b, viewCamera: n, entityPosition: c.entity.position })) return;
...
function $i(e, t) {
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_path") return void Yn(e, { pointer: i, deltaPixels: m, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
...
function He(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = ps(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function tt(e, t) {
  return hs(t, e.interactionElement);
}
function _s(e, t) {
...
function Yi(e, t) {
  for (const o of e.root.querySelectorAll("[data-motion-tool]"))
    o.addEventListener("click", () => {
      e.motionCreationKind = "", Ct(e.state, o.dataset.motionTool), We(e), e.render();
    }, { signal: t });
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = et(e.state);
    n && (e.checkpoint("Set motion interpolation"), fs(n, o.target.value), re(e, `Motion interpolation: ${o.target.value}`));
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? He(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? vs(e, s) : n === "erase" && _s(e.state, s), re(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = Jt(e), l = bs(e.state, n.points, s, i);
  ...
  a.addEventListener("pointerup", r, { capture: !0, signal: t }), a.addEventListener("pointercancel", r, { capture: !0, signal: t }), a.addEventListener("click", (o) => {
    ...
    const n = qr(e.state.motion_layers, tt(e, o));
...
function ws(e, t) {
  ...
  if (a) {
    ...
    e.checkpoint?.(`Motion: ${a.label}`), Ct(e.state, a.tool), e.motionCreatingLabel = a.label, e.motionCreationKind = t, We(e), e.render(), e.setStatus(a.hint);
  }
}
function Hr(e) {
  return (e.state.motion_tool || "select") === "select" && !e.motionTrackDraft ? !1 : (Ct(e.state, "select"), e.motionTrackDraft = null, e.motionCreatingLabel = "", e.motionCreationKind = "", We(e), e.render(), e.setStatus("Motion creation cancelled."), !0);
}
function Zi(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => ws(e, a.dataset.motionCreate), { signal: t });
...
const ye = { t: "translate", r: "rotate", s: "scale" }, ks = [
...
function Os() {
  ta || typeof window > "u" || (ta = !0, window.addEventListener("keydown", (e) => {
    ...
    !a || a.disposed || Is(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function Is(e, t) {
  if (!An()) return !1;
...
function Ls(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!ze(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: so(e.state) }, rt(e)) : (e.checkpoint("Split shot"), lo(e.state, e.frame, null) ? rt(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
    if (t.repeat) return !0;
    const r = ze(e.state), o = sa(e.state, e.frame), n = o ? r.findIndex((s) => s.start === o.start) : -1;
    return n >= 0 && (e.checkpoint("Remove shot"), co(e.state, n) && rt(e)), !0;
  }
  return !1;
}
function kt(e, t) {
...
function Bs([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * Fs), n = Math.round((window.innerHeight || t) * Ks);
  return [
...
function Vs(e, t) {
  const a = Wr[t];
  return !a || !e?.setSize ? !1 : (e.setSize(Bs(a.default, a.min)), !0);
}
function Gs(e, t, a) {
  const r = Wr[t];
...
function Hs() {
  ...
  e.id = aa, e.textContent = qs, document.head.appendChild(e);
}
function ne(e) {
...
const oa = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", na = "oc-help-toolbar-css", Us = "#8b7bd8";
function Xs() {
  ...
  e.id = na, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${Us};color:#fff;
...
ee.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await kt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-5Pe4mfao.js").then((n) => n.g);
...
ee.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== mt) return;
    const t = !le, a = t ? null : At(e);
    await kt(e, async () => (await import("./chunk-DcLBm4FH.js")).attachExtractor), e.__majoorOmniCamExtractor && Tt(e, mt, t, a?.());
  }
});
ee.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== dt) return;
    const t = !le, a = t ? null : At(e);
    await kt(e, async () => (await import("./chunk-DuB0st1_.js")).attachMonitor), e.__majoorOmniCamMonitor && Tt(e, dt, t, a?.());
  }
});
export {
  Ki as $,
