import { v as p, e as _, a as T, bn as D, L as O, S as L, s as U, be as z, bP as ee } from "./chunk-CUzpmyH7.js";
import { a as te } from "./chunk-D6j0koKF.js";
import { g as $, l as ae } from "./chunk-CGAuv3Kp.js";
function Re(e, t) {
...
function Pe({ title: e, items: t = [], onDelete: a = null, owner: o = null }) {
    ...
    const i = (d) => {
      ...
      if (S.textContent = d.sublabel || "", Object.assign(S.style, { opacity: "0.6", fontSize: "11px" }), u.append(j, S), u.addEventListener("click", () => i(d.id)), b.appendChild(u), a) {
        ...
        y.type = "button", y.title = "Delete", y.textContent = "✕", Object.assign(y.style, {
          ...
          background: "transparent",
          color: "inherit"
        }), y.addEventListener("click", async (Z) => {
          Z.stopPropagation(), y.disabled = !0;
          try {
...
async function oe(e, t, a, o) {
  ...
  return m?.prompt ? m.prompt({ title: c, message: l, defaultValue: s }) : W({ title: c, message: l, withInput: !0, defaultValue: s, owner: n });
}
async function re(e, t, a) {
  ...
  return l?.confirm ? l.confirm({ title: n, message: c }) : W({ title: n, message: c, withInput: !1, owner: r });
}
const ne = "omnicam_extractor_result_v2", E = "omnicam_extracted_motion_scene_json", A = "omnicam_extracted_track_fingerprint", B = "omnicam_extractor_source";
function H(e) {
...
const K = 24, V = 5, G = 150, J = Math.PI / 180;
function X(e, t, a) {
  return Math.min(a, Math.max(t, e));
}
function le(e, t = K) {
  const a = X(Number(e) || 0, V, G);
  return t / (2 * Math.tan(a * J / 2));
}
function me(e, t = K) {
  const a = Math.max(1e-6, Number(e) || 0), o = 2 * Math.atan(t / (2 * a)) / J;
  return X(o, V, G);
}
function de(e) {
  const t = le(e);
...
}, pe = { "16:9": 16 / 9, "4:3": 4 / 3, "1:1": 1, "9:16": 9 / 16, "2.39:1": 2.39 };
function ue(e) {
  if (!e) return null;
  const t = pe[e.aspect_ratio];
  if (t) return t;
...
function ge(e, t, { label: a = "Extracted Camera" } = {}) {
  const o = Array.isArray(t?.keyframes) ? t.keyframes : [];
  if (!o.length) throw new Error(p("no camera keys in this solve"));
  const r = Number(e.state.fps) || 24, n = Number(t.fps) || r, c = n > 0 ? r / n : 1, l = R(e.state), s = e.state.cameras.length, m = fe(e.state, a || "Extracted Camera"), i = k[s % k.length], h = o.map((f) => ({
    ...f,
...
function Ge(e) {
  ...
  for (const a of e.root.querySelectorAll('[data-role="playblast-camera"]')) {
    ...
    o.value = L, o.textContent = t.length ? p("Sequence ({count} shots)").replace("{count}", String(t.length)) : p("Sequence (no shots yet)"), o.disabled = t.length === 0, a.appendChild(o), a.value = e.state.playblast_camera_id;
  }
...
function Y(e) {
  ...
  const { tracks: c, overflow: l } = ve(e), s = `${c.map((i) => `${i.id}:${i.name}:${i.muted ? 1 : 0}:${i.solo ? 1 : 0}:${i.color || ""}`).join("|")}#${l}`;
  let m = !1;
  if (s !== e.cameraPreviewSignature && (m = !0, e.cameraPreviewSignature = s, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), c.forEach((i, h) => {
    ...
    S.className = "camera-view-badge", S.textContent = p("CAMERA PREVIEW"), g.append(v, d, b, u), f.append(j, g, S), t.appendChild(f), f.addEventListener("click", () => {
      ...
      y.button === 1 && (y.preventDefault(), be(e, i.id));
...
async function Xe(e, t) {
  ...
  if (!a) return;
  const o = (await oe(e.app, p("Rename camera"), p("Camera name"), a.name))?.trim();
...
async function Qe(e, t) {
  ...
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a || !await re(e.app, p("Delete camera"), p("Delete {value1} and its {value2} keyframe(s)?", { value1: a.name, value2: a.keyframes.length }))) return;
...
function be(e, t) {
  e.state.maximized_camera_id = e.state.maximized_camera_id === t ? null : t, e.serialize(), Y(e), requestAnimationFrame(() => {
    e.resizeCanvas(), e.renderCameraView();
...
function at(e, t) {
  ...
  for (const o of e.root.querySelectorAll('[data-role="camera-focal"]')) o.value = de(a);
...
function Se(e, t, a = {}) {
  ...
  const r = a.mode || (je(e) ? "replace" : "merge"), n = a.reconMode || we(t);
  if (r === "replace") {
    e.checkpoint?.("Adopt reconstructed scene (replace)"), e.state = U(
      ke(JSON.parse(JSON.stringify(o)))
    ), e.camera = T(e.state, e.frame || 0);
...
function Ee(e) {
  ...
  for (const o of t.inputs || []) {
    if (String(o?.name || "").toLowerCase() !== _e || o.link == null) continue;
    const r = ae(a, o.link);
    if (r && Ce(r) === ee) return r;
  }
  return null;
}
function Ae(e) {
...
function rt(e) {
  const t = Ee(e), a = t ? ce(t) : null;
  let o = !1;
  return a ? a.fingerprint !== Ae(e) && e.pendingExtractorImport?.fingerprint !== a.fingerprint && (e.pendingExtractorImport = {
    track: a.track,
    fingerprint: a.fingerprint,
    originNodeId: t.id,
    label: String(t.title || p("OmniCam Extractor")),
    keyCount: a.track.keyframes?.length || 0
  }, Ne(e, a.fingerprint, t), o = !0) : e.pendingExtractorImport && (e.pendingExtractorImport = null, o = !0), I(e), o;
}
function nt(e) {
  const t = e.pendingExtractorImport;
  return t ? (e.checkpoint("Import extracted camera"), ge(e, t.track, { label: t.label }), e.pendingExtractorImport = null, I(e), e.setStatus?.(p("Imported {count} camera keys from {name}").replace("{count}", String(t.keyCount)).replace("{name}", t.label)), e.scheduleSerialize(), e.render(), !0) : !1;
}
function it(e) {
