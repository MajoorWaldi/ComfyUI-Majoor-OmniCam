...
import { M as Et, E as Ct } from "./chunk-CbqXtcpr.js";
import { u as p, v as rt, bI as _t, x as Mt } from "./chunk-BehqUNE0.js";
import { d as Tt } from "./chunk-eq1tqQ9i.js";
import { h as at, D as Nt, L as $t, E as At } from "./chunk-BJ59IIsk.js";
import { a as Rt, F as Ft, S as Pt, m as nt, c as it } from "./chunk-sVL03mTG.js";
import { c as Lt, q as It, a as st, r as Ot, s as Dt, b as qt, d as jt, e as Vt, p as Ut, f as Bt } from "./chunk-BYLFPjEE.js";
function Wt(e) {
...
class ue {
  ...
  async ensurePreview() {
    return this.preview || this.disposed ? this.preview : (this.previewLoad ||= import("./chunk-ePcXfPcu.js").then(({ TrackViewer: t }) => {
  ...
  async discard() {
    ...
    if (o)
      try {
        ...
        return this.dispatch({ type: "ERROR", error: { message: r.message } }), !1;
      }
...
class _e {
  ...
  seek(t, o = "manual") {
    ...
    return Ce(o) && this.setFollow(!1), o !== "media" && this.media?.seekFrame?.(r), this.getViewer?.()?.setFrame?.(r), this.showDiagnostics(r), this.frame = r, this.dispatch({ type: "FRAME", frame: r }), o !== "playback" && (this.playbackStartFrame = r, this.playbackStartTime = null), r;
  }
  play() {
...
function Me(e, { track: t, state: o } = {}) {
  if (o !== "COMPLETED")
    throw new N("Only a completed solve can be applied to the Director.");
  const r = t?.keyframes;
  if (!Array.isArray(r) || !r.length)
    throw new N("This solve produced no camera keys to apply.");
  const a = String(t?.metadata?.extractor_fingerprint || "");
  if (!a)
    throw new N("This track carries no extractor fingerprint.");
  const n = nt(t);
  if (!n)
    throw new N("This solve cannot be wrapped in a canonical motion scene.");
  it(e, {
    motionScene: n,
    fingerprint: a,
...
const $e = "/majoor/omnicam/extractor/frame";
function Ae(e) {
...
class Le {
  ...
  async load(t, o, { maxDimension: r = 960 } = {}) {
    ...
    const i = Ae(o);
    try {
      const s = await this.api?.fetchApi?.($e, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: t, frame: i, max_dimension: r }),
        signal: n.signal
      });
      if (!s?.ok) throw new Error(await Re(s));
      const l = await s.blob(), c = await this.decodeImage(l);
      if (a !== this.generation || n.signal.aborted)
        return c?.close?.(), !1;
      const u = A(s.headers, "X-OmniCam-Width", c?.width), m = A(s.headers, "X-OmniCam-Height", c?.height);
      let d = !1;
      try {
      ...
      return this.frame = A(s.headers, "X-OmniCam-Frame", i), this.frameCount = A(s.headers, "X-OmniCam-Frame-Count", this.frameCount), this.error = "", !0;
    } catch (s) {
...
function Je(e, t, o, r, a, n) {
  for (const i of t) {
    const s = C(i.start, r, a, n), l = C(i.end, r, a, n), c = Math.max(2, l - s + 2);
...
function no(e, {
  ...
  for (const v of m) {
    ...
    const k = u[v.key] || [];
    k.length > 1 && !d.rowChrome && (h.strokeStyle = "#2c2c38", h.lineWidth = 1, h.beginPath(), h.moveTo(C(k[0], f, c, d), y), h.lineTo(C(k[k.length - 1], f, c, d), y), h.stroke()), h.fillStyle = Ue[v.key] || "#8b7bd8";
    const _ = d.rowChrome ? 5.5 : 3.5;
    for (const $ of k) {
      const S = Math.max(
        d.labelWidth + _,
        Math.min(f - _, C($, f, c, d))
      );
      h.beginPath(), h.moveTo(S, y - _), h.lineTo(S + _, y), h.lineTo(S, y + _), h.lineTo(S - _, y), h.closePath(), h.fill();
    }
  }
  if (d.ruler) {
    h.fillStyle = "#3a3a48";
    ...
    for (let y = 0; y <= v; y += 1) {
      ...
      h.fillRect(C(k, f, c, d), g - 6, 1, 5);
    }
  }
  if (d.playhead) {
    const v = C(Math.max(0, Math.min(c - 1, Number(n) || 0)), f, c, d);
...
function xo(e) {
  return import("./chunk-ePcXfPcu.js").then(({ TrackViewer: t }) => (e.viewerLoad = null, e.disposed || e.viewer || (e.viewer = new t(e.$("track-canvas")), e.pushTracksToViewer()), e.viewer)).catch((t) => (e.viewerLoad = null, console.warn("OmniCam track viewer unavailable", t), null));
}
function et(e) {
  const t = e.$("frame");
...
class xt {
  ...
  constructor(t) {
    this.runtime = t;
    const o = t.node;
    this.node = o, this.app = H, this.api = T, this.root = je(), this.disposed = !1, this.events = new Ct(), this.requests = new Gt(), this.diagnostics = new Se(), this.upstreamPreviewActive = !1, this.motionLimits = null, this.refine = new we({ onRefine: (i) => this.requestRefine(i) }), this.fallbackViewer = new Le(this.$("fallback-preview"), { api: T }), this.sourceViewer = new Ne(this.$("source-video"), {
  ...
  applyRefined() {
    try {
      ...
      const o = t instanceof N ? t.message : String(t?.message || t);
...
function Ro(e) {
  const t = new xt(e);
  return e.attachWorkbench(t), e.node.__majoorOmniCamExtractor = t, e.pendingSourceResync && (e.pendingSourceResync = !1, t.refreshSource()), t;
}
function Fo(e) {
