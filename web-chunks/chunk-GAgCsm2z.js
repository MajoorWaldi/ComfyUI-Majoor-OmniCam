import { c as w, bw as g, e as k, a$ as A, bx as oe, y as D, s as M, by as re, b5 as O, a as y, g as H, b0 as W, bz as ne } from "./chunk-B6C7k7m4.js";
...
function B({ title: e, message: t, withInput: a = !1, defaultValue: o = "", owner: r = null }) {
  return typeof document > "u" || !document.body ? Promise.resolve(a ? null : !1) : new Promise((s) => {
    ...
    v.addEventListener("click", () => b(a ? null : !1)), n.addEventListener("click", () => b(a ? m.value : !0)), l.addEventListener("mousedown", (u) => {
...
function Ze(e, t = !0) {
  ...
  v && (v.value = String(e.state.fps)), e.frame = w(e.frame, 0, e.state.duration_frames - 1), t && e.serialize(), (a !== e.state.duration_frames || o !== e.state.fps) && (e.computeAudioPeaks?.(), e.setFrame(e.frame, !1, !0), e.setStatus(`Timeline: ${e.state.duration_frames} frames · ${(e.state.duration_frames / e.state.fps).toFixed(2)} s`));
}
function Ue(e) {
  let t = null;
  try {
...
function we(e, t, { label: a = "Extracted Camera" } = {}) {
  ...
  if (!o.length) throw new Error(y("no camera keys in this solve"));
...
function et(e) {
  ...
  for (const a of e.root.querySelectorAll('[data-role="playblast-camera"]')) {
    ...
    o.value = A, o.textContent = t.length ? y("Sequence ({count} shots)").replace("{count}", String(t.length)) : y("Sequence (no shots yet)"), o.disabled = t.length === 0, a.appendChild(o), a.value = e.state.playblast_camera_id;
  }
...
function Z(e) {
  ...
  d !== e.cameraPreviewSignature && (i = !0, e.cameraPreviewSignature = d, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), l.forEach((c, m) => {
    ...
    f.style.setProperty("--camera-color", v), f.title = y(`Click: set ${c.name} as primary · Double-click: edit · Right-click: preview actions`);
    const n = document.createElement("div");
    ...
    x.className = "pi pi-circle-fill output-mark", x.title = y("Playblast camera");
    const u = document.createElement("canvas");
    u.dataset.cameraPreview = c.id;
    const j = document.createElement("span");
    j.className = "camera-view-badge", j.textContent = y("CAMERA PREVIEW"), n.append(p, b, h, x), f.append(u, n, j), t.appendChild(f), f.addEventListener("click", () => {
...
function tt(e) {
  ...
  }), e.cameraPreviewSignature = "", e.activateCamera(t), e.setStatus(y(`${o} added`));
}
async function at(e, t) {
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a) return;
  const o = (await ce(e.app, y("Rename camera"), y("Camera name"), a.name))?.trim();
  !o || o === a.name || (e.checkpoint("Rename camera"), a.name = o.slice(0, 80), e.cameraPreviewSignature = "", e.serialize(), e.refreshObjects(), e.refreshKeys(), e.setStatus(y(`Camera renamed: ${a.name}`)));
}
function ot(e, t) {
  ...
  e.state.cameras.push(o), e.cameraPreviewSignature = "", e.activateCamera(o.id), e.setStatus(y(`${o.name} added`));
}
async function rt(e, t) {
  if (e.state.cameras.length <= 1) return e.setStatus(y("At least one camera is required"));
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a || !await le(e.app, y("Delete camera"), y(`Delete ${a.name} and its ${a.keyframes.length} keyframe(s)?`))) return;
...
function it(e) {
  ...
  }), e.setStatus(y(`Camera previews ${e.state.camera_view_visible ? "shown" : "hidden"}`));
}
function _e(e, t) {
  e.state.maximized_camera_id = e.state.maximized_camera_id === t ? null : t, e.serialize(), Z(e), requestAnimationFrame(() => {
    e.resizeCanvas(), e.renderCameraView();
  }), e.setStatus(e.state.maximized_camera_id ? y("Preview maximized") : y("Preview restored"));
}
function ct(e, t, a, o) {
  if (e.state.guides !== !1) {
...
function dt(e, t, {
  ...
  if (!Array.isArray(c) || !c.length)
    throw new Error(y("no camera keys in this file"));
  d && e.checkpoint(a);
  const m = N(e);
  ...
  }), e.syncActiveCameraTrack(), e.setFrame(0), e.refreshKeys(), e.render(), e.scheduleSerialize(), i && e.setStatus(y("Imported {count} camera keys from {name}").replace("{count}", String(c.length)).replace("{name}", a)), c.length;
}
...
function Le(e, t, a = {}) {
  ...
  if (r === "replace") {
    e.checkpoint?.("Adopt reconstructed scene (replace)"), e.state = D(
...
function P(e) {
  ...
  o && (o.textContent = y("{count} camera keys ready from {name} — import as a new camera?").replace("{count}", String(a.keyCount)).replace("{name}", a.label));
}
function yt(e) {
  ...
  return a ? a.fingerprint !== Pe(e) && e.pendingExtractorImport?.fingerprint !== a.fingerprint && (e.pendingExtractorImport = {
    ...
    originNodeId: t.id,
    label: String(t.title || y("OmniCam Extractor")),
...
function vt(e) {
  ...
  return t ? (e.checkpoint("Import extracted camera"), we(e, t.track, { label: t.label }), e.pendingExtractorImport = null, P(e), e.setStatus?.(y("Imported {count} camera keys from {name}").replace("{count}", String(t.keyCount)).replace("{name}", t.label)), e.scheduleSerialize(), e.render(), !0) : !1;
}
function xt(e) {
  return e.pendingExtractorImport ? (e.pendingExtractorImport = null, P(e), e.render(), e.setStatus?.(y("Extracted camera preview dismissed")), !0) : !1;
}
function wt(e) {
  const t = e?.graph;
  if (!t) return 0;
