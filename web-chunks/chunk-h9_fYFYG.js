import { u as a, aa as j, bQ as te, bR as J, bS as w } from "./chunk-KSpmkRWt.js";
...
function he(e, n = {}) {
  ...
  function E() {
    ...
    if (H) {
      ...
      H.textContent = `${p} · ${t.model || a("(no model set)")}`;
    }
  ...
  async function _() {
    ...
    try {
      ...
      if (l) return;
      let p = t.model;
      !p && r.length && (p = r[0], j(J(t.provider), p)), m.innerHTML = Y(r, p), E();
    } catch {
      if (l) return;
  ...
  return f?.addEventListener("click", G), y?.addEventListener("change", B), m?.addEventListener("change", R), d?.addEventListener("input", D), E(), Q(), I(), _(), {
    get state() {
      return g;
    },
    dispose() {
      l = !0, L?.abort?.(), f?.removeEventListener("click", G), y?.removeEventListener("change", B), m?.removeEventListener("change", R), d?.removeEventListener("input", D);
    }
  };
}
export {
