import { a as p, e as g, _ as S } from "./chunk-DVQY0KQ6.js";
...
function q(o, e, { label: a = "Extracted Camera" } = {}) {
  ...
  const t = Number(o.state.fps) || 24, n = Number(e.fps) || t, s = n > 0 ? t / n : 1, i = z(o.state), m = o.state.cameras.length, c = M(o.state, a || "Extracted Camera"), d = w[m % w.length], v = r.map((h) => ({
    ...h,
    frame: Math.round((Number(h.frame) || 0) * s),
    camera: g(h.camera)
  }));
  if (o.state.cameras.push({ id: i, name: c, color: d, camera: g(v[0].camera), keyframes: v }), Number.isFinite(Number(e.duration_frames))) {
    const h = Math.round(Number(e.duration_frames) * s);
...
function B(o, e) {
  ...
  for (const t of o.root.querySelectorAll('[data-role="camera-focal"]')) t.value = A(a);
  o.setStatus(`Lens: ${e}mm (FOV ${a.toFixed(1)}°)`);
}
function F(o) {
  return o?.state?.cameras?.length || (o.state.cameras = [{ id: "camera_1", name: "Camera 1", color: "#4aa3ef", camera: g(o?.camera), keyframes: o?.state?.keyframes || [] }]), o.state.cameras.find((e) => e.id === o.state.active_camera_id) || o.state.cameras[0];
}
const u = "upstream_camera_track";
function Y(o, e, {
  ...
  }), o.syncActiveCameraTrack(), o.setFrame(0), o.refreshKeys(), o.render(), o.scheduleSerialize(), m && o.setStatus(p("Imported {count} camera keys from {name}").replace("{count}", String(c.length)).replace("{name}", a)), c.length;
}
const R = "omnicam_extractor_result_v1", f = "omnicam_extracted_track_json", x = "omnicam_extracted_track_fingerprint", O = "omnicam_extractor_source";
function Q(o) {
  ...
  const t = r.track;
  return !t || !Array.isArray(t.keyframes) ? null : {
    track: t,
...
function Z(o, e) {
  ...
  return a && (a.value = JSON.stringify(e.track)), r && (r.value = e.fingerprint), t;
}
function oo(o, e) {
  const a = l(o, O);
  if (!a || !e) return !1;
...
function K(o) {
  ...
  for (const r of e.inputs || []) {
    if (String(r?.name || "").toLowerCase() !== $ || r.link == null) continue;
    const t = I(a, r.link);
    if (t && P(t) === S) return t;
  }
  return null;
...
function G(o) {
  return String(o?.state?.metadata?.[u]?.fingerprint || "");
}
function U(o, e, a) {
  o.state.metadata = {
    ...o.state.metadata,
...
function ao(o) {
  const e = K(o), a = e ? W(e) : null;
  let r = !1;
  return a ? a.fingerprint !== G(o) && o.pendingExtractorImport?.fingerprint !== a.fingerprint && (o.pendingExtractorImport = {
    track: a.track,
    fingerprint: a.fingerprint,
    originNodeId: e.id,
    label: String(e.title || p("OmniCam Extractor")),
    keyCount: a.track.keyframes?.length || 0
  }, U(o, a.fingerprint, e), r = !0) : o.pendingExtractorImport && (o.pendingExtractorImport = null, r = !0), b(o), r;
}
function ro(o) {
  const e = o.pendingExtractorImport;
  return e ? (o.checkpoint("Import extracted camera"), q(o, e.track, { label: e.label }), o.pendingExtractorImport = null, b(o), o.setStatus?.(p("Imported {count} camera keys from {name}").replace("{count}", String(e.keyCount)).replace("{name}", e.label)), o.scheduleSerialize(), o.render(), !0) : !1;
}
function to(o) {
...
function no(o) {
  ...
  for (const n of a)
    for (const s of n?.links || []) {
      const i = C(e, s), m = i?.target_id ?? i?.targetId;
