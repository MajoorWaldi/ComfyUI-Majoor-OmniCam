...
function vd(e, { type: t = "handheld_subtle", intensity: a = 1, duration_frames: o = null, subdivide: r = !0 } = {}) {
  ...
  return [...b].sort((y, x) => y - x).map((y) => {
    const x = $e(u, y), k = Kt(y * m, 1) * l, g = Kt(y * m, 2) * l, j = Kt(y * m, 3) * l * 0.5, T = Kt(y * m, 4) * p, K = Kt(y * m, 5) * (p * 0.35), z = [...x.position], J = [...x.target];
    return z[0] += k, z[1] += g, z[2] += j, J[0] += k * 0.35, J[1] += g * 0.35, {
      frame: y,
      camera: {
        ...x,
        position: z,
...
function Fd(e, t) {
  ...
  if (e.curveScrub && t.pointerId === e.curveScrub.pointerId) {
    t.preventDefault();
    const f = Math.max(1, e.state.duration_frames - 1), d = f / (Number(e.curveZoomX) || 1), h = Number(e.curvePanX) || 0, u = Math.round(te(h + (o - 44) / Math.max(1, a.clientWidth - 58) * d, 0, f));
    e.setFrame(u);
    return;
  }
  ...
  if (!e.curveDrag || t.pointerId !== e.curveDrag.pointerId) {
    const f = (e.curveHitPoints || []).map((x) => ({ point: x, distance: Math.hypot(o - x.x, r - x.y) })).sort((x, k) => x.distance - k.distance)[0], d = Math.max(1, e.state.duration_frames - 1), h = d / (Number(e.curveZoomX) || 1), u = Number(e.curvePanX) || 0, b = Math.max(1, a.clientWidth - 58), v = te(Math.round(u + (o - 44) / b * h), 0, d);
    let y = null;
    if (f && f.distance <= 14) {
  ...
  if (e.curveHover = null, t.preventDefault(), t.stopPropagation(), e.curveDrag.historyCheckpointed || (e.checkpoint?.(e.curveDrag.handle ? "Edit curve tangent" : "Edit curve"), e.curveDrag.historyCheckpointed = !0), e.curveDrag.handle) {
    ...
    if (h === "in") {
      if (g.in_x = te((o - v) / Math.max(1, u), -0.99, -0.01), g.in_y = (y - r) * b, k === "aligned") {
        const j = Math.hypot(g.in_x, g.in_y) || 1e-6, T = Math.hypot(e.curveDrag.startHandles.out_x, e.curveDrag.startHandles.out_y) || 1e-6;
        g.out_x = -g.in_x / j * T, g.out_y = -g.in_y / j * T;
      }
    } else if (g.out_x = te((o - v) / Math.max(1, u), 0.01, 0.99), g.out_y = (y - r) * b, k === "aligned") {
      const j = Math.hypot(g.out_x, g.out_y) || 1e-6, T = Math.hypot(e.curveDrag.startHandles.in_x, e.curveDrag.startHandles.in_y) || 1e-6;
      g.in_x = -g.out_x / j * T, g.in_y = -g.out_y / j * T;
    }
  ...
  if (e.curveDrag.group) {
    ...
    if (d) {
      const u = e.curveDrag.group.map((v) => te(Math.round(v.startFrame + d), 0, e.curveDrag.lastFrame));
...
function Ns(e, { selectedOnly: t = !1 } = {}) {
  const a = Math.max(1, (e.state?.duration_frames ?? 120) - 1), o = e.timelineKeyframes() || [], r = e.timelineObject(), n = ut(e), i = e.selectedKeyFrames?.size ? [...e.selectedKeyFrames] : e.selectedKeyFrame != null ? [e.selectedKeyFrame] : [], c = i.length > 0, l = t || c && i.length < o.length ? o.filter((b) => i.includes(b.frame)) : o;
  if (!l.length) {
  ...
  if (h.length > 0) {
    const b = Math.min(...h), v = Math.max(...h), y = Math.max(1e-4, v - b), x = (b + v) / 2, k = ($) => r ? qo(r, $) : $e(e.state, $), g = [], j = Math.max(1, Math.floor(a / 40));
    for (let $ = 0; $ <= a; $ += j) {
      const P = k($);
      for (const _ of n) {
        const R = _.get(P);
    ...
    let T = Math.min(...g), K = Math.max(...g);
    (!Number.isFinite(T) || !Number.isFinite(K)) && (T = -1, K = 1), Math.abs(K - T) < 1e-6 && (T -= 1, K += 1);
    const z = (K - T) * 0.1;
    T -= z, K += z;
    const J = K - T, H = (T + K) / 2;
    if (l.length < o.length && y < J * 0.75) {
      const $ = y * 1.35;
      e.curveZoom = te(J / $, 0.2, 30), e.curvePanY = x - H;
    } else
...
function Hd(e) {
  ...
  const i = e.timelineObject(), c = e.timelineKeyframes(), l = ut(e), p = 44, m = 14, f = 16, d = 22, h = Math.max(1, a - p - m), u = Math.max(1, o - f - d), b = Math.max(1, e.state.duration_frames - 1), v = te(Number(e.curveZoomX) || 1, 0.1, 50), y = Number(e.curvePanX) || 0, x = b / v, k = y, g = y + x, j = [], T = Math.max(1, Math.ceil(x / Math.max(80, h))), K = (C) => i ? qo(i, C) : $e(e.state, C);
  for (let C = 0; C <= b; C += T) j.push({ frame: C, value: K(C) });
  j[j.length - 1]?.frame !== b && j.push({ frame: b, value: K(b) });
  const z = j.flatMap((C) => l.map((Y) => Y.get(C.value)));
  let J = Math.min(...z), H = Math.max(...z);
  (!Number.isFinite(J) || !Number.isFinite(H)) && (J = -1, H = 1), Math.abs(H - J) < 1e-6 && (J -= 1, H += 1);
  const $ = (H - J) * 0.1;
  J -= $, H += $;
  const P = te(Number(e.curveZoom) || 1, 0.1, 50), _ = (H + J) / 2 + (Number(e.curvePanY) || 0), R = (H - J) / P;
  J = _ - R / 2, H = _ + R / 2;
  const q = (C) => p + (C - k) / Math.max(1e-6, g - k) * h, G = (C) => f + u * (H - C) / Math.max(1e-6, H - J);
  if (n.fillStyle = "#111114", n.fillRect(0, 0, a, o), n.strokeStyle = "#222228", n.lineWidth = 1, n.font = "9px system-ui, -apple-system, sans-serif", n.fillStyle = "#6e727a", Pd(n, {
    ...
    xFor: q,
    frame: e.frame
  }), Ld(n, { left: p, right: m, top: f, width: a, graphHeight: u, minimum: J, maximum: H, yFor: G }), J <= 0 && H >= 0) {
    const C = G(0);
    n.strokeStyle = "#383842", n.lineWidth = 1.2, n.beginPath(), n.moveTo(p, C), n.lineTo(a - m, C), n.stroke();
  }
  e.curveHitPoints = [];
  for (const C of l) {
    n.strokeStyle = C.color, n.lineWidth = 2, n.beginPath();
    let Y = !1;
    j.forEach((D) => {
      const X = q(D.frame), ae = G(C.get(D.value));
      X >= p - 50 && X <= a - m + 50 && (Y ? n.lineTo(X, ae) : (n.moveTo(X, ae), Y = !0));
    }), n.stroke();
    for (const D of c) {
      const X = i ? D.transform : D.camera, ae = q(D.frame), le = G(C.get(X)), se = D.frame === e.selectedKeyFrame || e.selectedKeyFrames?.has(D.frame);
      se && (n.fillStyle = "rgba(242, 208, 107, 0.35)", n.beginPath(), n.arc(ae, le, 8.5, 0, Math.PI * 2), n.fill()), n.fillStyle = se ? "#ffd75e" : C.color, n.strokeStyle = "#0d0d10", n.lineWidth = 1.6, n.beginPath(), n.arc(ae, le, se ? 5.2 : 3.8, 0, Math.PI * 2), n.fill(), n.stroke(), e.curveHitPoints.push({
        x: ae,
        y: le,
        key: D,
        channel: C,
    ...
    if (e.showCurveHandles)
      for (let D = 0; D < c.length; D++) {
        const X = c[D], ae = X.frame === e.selectedKeyFrame || e.selectedKeyFrames?.has(X.frame);
        if (!(ae || e.curveChannelFilter !== "all" || c.length <= 4) || X.interpolation !== "bezier") continue;
        const se = i ? X.transform || i : X.camera || X, re = q(X.frame), de = G(C.get(se)), ye = c[D - 1], he = c[D + 1], ve = Math.max(1, X.frame - (ye?.frame ?? X.frame - 1)), xe = Math.max(1, (he?.frame ?? X.frame + 1) - X.frame), pe = Ni(
          X,
          C.id,
          ye,
          he,
          (N) => C.get(i ? N.transform || i : N.camera || X)
        ), Ie = (H - J) / Math.max(1, u), we = h * xe / Math.max(1, x), ke = h * ve / Math.max(1, x), be = [];
        (ye || D > 0) && be.push({ side: "in", x: re + pe.in_x * ke, y: de - pe.in_y / Ie }), (he || D < c.length - 1 || c.length === 1) && be.push({ side: "out", x: re + pe.out_x * we, y: de - pe.out_y / Ie });
        for (const N of be) {
          if (n.strokeStyle = C.color, n.lineWidth = ae ? 1.5 : 1, n.beginPath(), n.moveTo(re, de), n.lineTo(N.x, N.y), n.stroke(), n.fillStyle = ae ? "#2a2233" : "#171720", n.strokeStyle = ae ? "#ffd75e" : C.color, n.lineWidth = ae ? 2 : 1.2, n.beginPath(), N.side === "in")
            n.arc(N.x, N.y, ae ? 5 : 3.8, 0, Math.PI * 2);
          else {
            const V = ae ? 4.5 : 3.2;
          ...
          n.fill(), n.stroke(), e.curveHitPoints.push({
            ...
            object: i,
            handle: N.side,
            pixelPerSegment: N.side === "in" ? ke : we,
            valuePerPixel: Ie,
            startHandles: { ...pe }
          });
        }
  ...
  if (e.curveBoxSelect) {
    const C = Math.min(e.curveBoxSelect.startX, e.curveBoxSelect.currentX), Y = Math.min(e.curveBoxSelect.startY, e.curveBoxSelect.currentY), D = Math.abs(e.curveBoxSelect.currentX - e.curveBoxSelect.startX), X = Math.abs(e.curveBoxSelect.currentY - e.curveBoxSelect.startY);
    n.fillStyle = "rgba(56, 189, 248, 0.15)", n.fillRect(C, Y, D, X), n.strokeStyle = "#38bdf8", n.lineWidth = 1, n.setLineDash([4, 4]), n.strokeRect(C, Y, D, X), n.setLineDash([]);
  }
  const L = q(e.frame);
  if (L >= p && L <= a - m && (n.strokeStyle = "#a78bfa", n.lineWidth = 1.5, n.beginPath(), n.moveTo(L, f), n.lineTo(L, f + u), n.stroke(), n.fillStyle = "#a78bfa", n.beginPath(), n.moveTo(L - 4, f), n.lineTo(L + 4, f), n.lineTo(L, f + 6), n.closePath(), n.fill()), e.curveDrag || e.curveHover) {
    ...
    if (e.curveDrag)
      if (e.curveDrag.handle) {
        ...
        C = `F${e.curveDrag.key.frame} · ${e.curveDrag.channel.name} (${D})`, Y = "Tangent edit";
      } else if (e.curveDrag.group && e.curveDrag.group.length > 1) {
        const D = e.curveDrag.key.frame - e.curveDrag.startFrame, X = e.curveDrag.channel.get(e.curveDrag.object ? e.curveDrag.key.transform : e.curveDrag.key.camera), ae = X - e.curveDrag.startValue, le = D >= 0 ? `+${D}` : `${D}`, se = ae >= 0 ? `+${ae.toFixed(2)}` : `${ae.toFixed(2)}`;
        C = `${e.curveDrag.group.length} keys · ΔF: ${le} · ΔVal: ${se}`, Y = `${e.curveDrag.channel.name}: ${X.toFixed(2)}`;
      } else {
        const D = e.curveDrag.key.frame - e.curveDrag.startFrame, X = e.curveDrag.channel.get(e.curveDrag.object ? e.curveDrag.key.transform : e.curveDrag.key.camera), ae = X - e.curveDrag.startValue, le = D >= 0 ? `+${D}` : `${D}`, se = ae >= 0 ? `+${ae.toFixed(2)}` : `${ae.toFixed(2)}`;
        C = `F${e.curveDrag.key.frame} (${le}) · ${e.curveDrag.channel.name}: ${X.toFixed(2)} (${se})`;
      }
    else if (e.curveHover)
      if (e.curveHover.channelName) {
        const D = Number.isFinite(e.curveHover.value) ? e.curveHover.value.toFixed(2) : "";
        C = `F${e.curveHover.frame} · ${e.curveHover.channelName}: ${D}`, e.curveHover.isHandle && (Y = `Handle ${e.curveHover.handleSide}`);
      } else
        C = `Frame ${e.curveHover.frame}`;
    if (C) {
      n.save(), n.font = "11px system-ui, -apple-system, sans-serif";
      const D = n.measureText(C), X = Y ? n.measureText(Y) : { width: 0 }, ae = Math.max(D.width, X.width) + 16, le = Y ? 32 : 20, se = a - m - ae - 6, re = f + 6;
      n.fillStyle = "rgba(18, 18, 24, 0.88)", n.strokeStyle = "#38384a", n.lineWidth = 1, n.beginPath(), n.roundRect ? n.roundRect(se, re, ae, le, 4) : n.rect(se, re, ae, le), n.fill(), n.stroke(), n.fillStyle = "#e2e8f0", n.fillText(C, se + 8, re + (Y ? 13 : 14)), Y && (n.fillStyle = "#94a3b8", n.font = "9.5px system-ui, -apple-system, sans-serif", n.fillText(Y, se + 8, re + 26)), n.restore();
    }
  }
  for (const C of e.root.querySelectorAll("[data-tangent-mode]")) {
    ...
    C.classList.toggle("active", C.dataset.tangentMode === D);
  }
  for (const C of e.root.querySelectorAll("[data-channel-filter]"))
    C.classList.toggle("active", C.dataset.channelFilter === (e.curveChannelFilter || "all"));
  for (const C of e.root.querySelectorAll("[data-curve-mode]"))
    C.classList.toggle("active", C.dataset.curveMode === e.selectedKeyframe()?.interpolation);
}
...
function cm(e) {
  ...
  if (e.audioWaveformPeaks && e.audioWaveformPeaks.length) {
    ...
    const k = x.getContext("2d"), g = e.audioWaveformPeaks, j = x.width, T = x.height, K = T / 2;
    k.fillStyle = "#f2d06b";
    for (let z = 0; z < g.length; z++) {
      const H = (z / (g.length - 1) * r - l) / Math.max(1e-6, c) * j;
      if (H >= -5 && H <= j + 5) {
        const $ = g[z] * (T * 0.85);
        k.fillRect(H, K - $ / 2, Math.max(1, j / g.length * n - 0.5), $);
      }
    }
    t.appendChild(x);
  }
  ...
  for (const x of o) {
    ...
    j.className = "key-label", j.textContent = String(x.frame), g.appendChild(j), g.addEventListener("pointerdown", (T) => {
      if (T.preventDefault(), T.stopPropagation(), g.focus({ preventScroll: !0 }), T.altKey) {
        e.checkpoint("Duplicate keyframe");
        const z = a ? { frame: x.frame, transform: Xe(x.transform), interpolation: x.interpolation } : { frame: x.frame, camera: ce(x.camera), interpolation: x.interpolation }, J = e.timelineKeyframes();
        J.push(z), J.sort((H, $) => H.frame - $.frame), e.selectedKeyFrame = z.frame, e.selectedKeyFrames = /* @__PURE__ */ new Set([z.frame]), e.keyDrag = { key: z, box: t, isDuplicate: !0, historyCheckpointed: !0, moving: [{ key: z, startFrame: z.frame }], startPointerFrame: x.frame, startClientX: T.clientX, startClientY: T.clientY }, e.setFrame(z.frame, !1, !1), e.setStatus(s(`Duplicating key from ${x.frame}...`));
        return;
      }
      if (T.shiftKey) {
        e.selectedKeyFrames = new Set(e.selectedKeyFrames || [e.selectedKeyFrame].filter((z) => z !== null)), e.selectedKeyFrames.has(x.frame) ? e.selectedKeyFrames.delete(x.frame) : e.selectedKeyFrames.add(x.frame), e.selectedKeyFrame = e.selectedKeyFrames.has(x.frame) ? x.frame : [...e.selectedKeyFrames].at(-1) ?? null, e.setFrame(x.frame, !1, !1), e.updateKeyVisualState(), e.refreshKeyEditor();
        return;
      }
      e.selectedKeyFrames?.has(x.frame) || (e.selectedKeyFrames = /* @__PURE__ */ new Set([x.frame])), e.selectedKeyFrame = x.frame;
      const K = e.timelineKeyframes().filter((z) => e.selectedKeyFrames.has(z.frame));
      e.keyDrag = { key: x, box: t, historyCheckpointed: !1, moving: K.map((z) => ({ key: z, startFrame: z.frame })), startPointerFrame: x.frame, startClientX: T.clientX, startClientY: T.clientY }, e.setFrame(x.frame, !1, !1);
    }), g.addEventListener("click", (T) => {
      T.preventDefault(), T.stopPropagation(), !T.shiftKey && (T.shiftKey || (e.selectedKeyFrames = /* @__PURE__ */ new Set([x.frame])), e.selectKeyframe(x));
    }), t.appendChild(g);
  }
  ...
  if (f) {
    ...
    const g = o.filter((j) => j.frame > e.state.duration_frames - 1).length;
    if (g) {
      const j = document.createElement("span");
      j.className = "oc-dormant-keys", j.textContent = ` · ${g} beyond end`, j.title = s("Keys past the end of the timeline are kept. Lengthen the shot to reach them again."), f.append(j);
    }
  }
  ...
  if (u) {
    ...
    for (const x of e.state.cameras) {
      ...
      const j = document.createElement("span");
      j.textContent = `${x.name} · ${x.keyframes.length} key${x.keyframes.length === 1 ? "" : "s"}${x.id === e.state.playblast_camera_id ? " · PLAYBLAST" : ""}`, k.append(g, j), k.addEventListener("click", () => {
...
const ot = 1, kn = 50, Oe = 120, yt = 160, Ym = Object.freeze(["perspective", "orthographic"]), W = Object.freeze({
...
}), Ws = Object.freeze(Object.values(W)), Se = Object.freeze({
  SCENE_GET: "scene.get",
...
function go(e, { degrees: t = 180, direction: a = "cw", radius: o, radiusEnd: r, heightOffset: n = 0, samples: i = 5, close: c = !1 } = {}) {
  ...
  for (let g = 0; g < y; g += 1) {
    const j = c ? g / y : g / (y - 1), T = d + v * j, K = h + (u - h) * j;
    k.push({
      position: [l[0] + Math.cos(T) * K, x, l[2] + Math.sin(T) * K],
      target: [...l]
    });
  }
  return k;
...
function Hs({ type: e, camera: t, target: a, startFrame: o, endFrame: r, params: n = {} } = {}) {
  ...
  const l = up(t, a), p = Number(n.distance) > 0 ? Number(n.distance) : 1, m = (b, v, y) => ({
...
function Tn(e, t, a) {
  if (!Array.isArray(e) || e.length === 0 || !e.every((o) => Number.isInteger(o) && o >= 0))
    throw new I("BAD_VALUE", `${t} must be a non-empty array of non-negative integer frames`, a);
}
...
function _p(e, t) {
  ...
  switch (a) {
    ...
    case W.CUT_SET_CAMERA:
      Te(e.start, "start", t), ee(e.cameraId, "cameraId", t);
      break;
    case W.CAMERA_PATH_TRANSFORM_KEYS: {
      e.cameraId !== void 0 && ee(e.cameraId, "cameraId", t), Tn(e.frames, "frames", t);
      const o = e.transform;
      if (!o || typeof o != "object" || Array.isArray(o))
        throw new I("BAD_VALUE", "camera.path.transform_keys needs a transform object", t);
      if (!wp.has(o.mode))
        throw new I("BAD_VALUE", "transform.mode must be translate, rotate or scale", t);
...
function Qs(e, { leftFrame: t, rightFrame: a, t: o = 0.5 } = {}) {
  ...
  if (b) {
    const k = [...f.camera.position], g = vr(f, h, d).out, j = vr(d, f, u).in, T = [...d.camera.position], K = Vp(k, g, j, T, m);
    y.camera = { ...ce(v), position: [...K.point] };
    const z = ft(f);
    (z === "free" || z === "aligned") && vo(f, "out", K.left[1]);
    const J = ft(d);
    (J === "free" || J === "aligned") && vo(d, "in", K.right[2]), vo(y, "in", K.left[2]), vo(y, "out", K.right[1]);
  }
...
const qt = A.viewport | A.previews | A.timeline | A.inspector;
function On(e, t, a) {
  const o = new Set((e.keyframes || []).map((n) => n.frame)), r = t.find((n) => !o.has(n));
  if (r !== void 0)
    throw new I("UNKNOWN_KEYFRAME", `${a}: camera has no key at frame ${r}`);
}
const Yp = {
  ...
  [W.CAMERA_SET_ACTIVE](e, t) {
    return xr(e, t.cameraId), e.active_camera_id = t.cameraId, { dirtyMask: A.viewport | A.previews | A.inspector | A.outliner | A.timeline };
  },
  [W.CAMERA_SET_LOCKED](e, t) {
    return xr(e, t.cameraId).locked = t.value, { dirtyMask: A.outliner | A.inspector | A.viewport };
  },
  [W.CAMERA_CREATE](e, t) {
    const a = ap(e, t);
  ...
  [W.OBJECT_SET_ENABLED](e, t) {
    return dt(e, t.objectId).enabled = t.value, { dirtyMask: A.viewport | A.previews | A.outliner | A.inspector };
  },
  [W.OBJECT_SET_LOCKED](e, t) {
    return wr(e, t.objectId).locked = t.value, { dirtyMask: A.outliner | A.inspector };
  },
  ...
  [W.CAMERA_PATH_TRANSFORM_KEYS](e, t) {
    const a = ze(e, t.cameraId);
  ...
  [W.CAMERA_PATH_DELETE_KEYS](e, t) {
    const a = ze(e, t.cameraId);
    On(a, t.frames, "camera.path.delete_keys");
    const o = Gp(a.keyframes || [], t.frames);
    if (!o.ok)
      throw new I("CANNOT_DELETE", "camera.path.delete_keys: a camera track needs at least one key");
    return Bt(e, a, o.keys), { dirtyMask: qt, outcome: { removed: o.removed } };
  },
  [W.CAMERA_PATH_REDISTRIBUTE_TIMING](e, t) {
    ...
    if (!i.ok) {
      ...
      throw new I(c[i.reason] || "BAD_RANGE", `camera.path.redistribute_timing: ${i.reason}`);
    }
    return Bt(e, a, i.keys), { dirtyMask: qt };
  },
  [W.CAMERA_PATH_APPLY_PRESET](e, t) {
    ...
    if (!r.ok) {
      ...
      throw new I(n[r.reason] || "BAD_VALUE", `camera.path.apply_preset: ${r.reason}`);
    }
...
function ff(e, t) {
  ...
  try {
    ...
    if (f instanceof I) return rr(e, t?.id, f);
    throw f;
  }
  const o = Sr(e);
...
const Pn = "omnicam-agent/1", Ln = "majoor.omnicam.agent.request", bf = 1, Ut = Object.freeze({
  ...
  ...Ws.filter((e) => e !== W.ASSET_INSTANTIATE),
  ei
]);
function _r(e) {
...
function kf(e, t, a) {
  ...
  async function d() {
    ...
    try {
      const j = await Wt(a, Ut.register, {
        protocol: Pn,
        client_id: g,
        node_id: String(t.id),
      ...
      if (o || k !== p) return;
      r = j.session_id, n = j.session_token, i = g, u();
    } catch {
      if (o || k !== p) return;
      h();
    }
  ...
  async function v(k) {
    ...
    try {
      if (g.kind === "query")
        j = g.payload?.type === ti ? await wf(e, g.payload) : e.directorApi.query(g.payload);
      else if (g.kind === "transaction") {
        const T = g.payload, K = (T?.operations || []).find(
          (z) => !Nn.includes(z?.type)
        );
        if (!Number.isInteger(T?.baseRevision) || T.baseRevision < 0)
          j = Vt(
            e,
          ...
          const z = await xf(e, T.operations);
          z.ok ? (j = e.directorApi.execute({ ...T, operations: z.operations }), await j?._reconciliation) : j = Vt(e, z.code, z.message);
        }
      } else
        j = Vt(e, "UNKNOWN_AGENT_REQUEST", `Unsupported Agent request kind: ${g.kind}`);
    } catch (T) {
      j = Vt(e, T?.code || "INTERNAL", T?.message || "OmniCam Agent request failed");
    }
    try {
      await Wt(a, Ut.reply, {
        session_id: r,
  ...
  return a.addEventListener?.(Ln, v), a.addEventListener?.("status", x), d(), {
    ...
    dispose() {
      ...
      const k = r, g = n;
      r = null, n = null, k && g && Wt(a, Ut.close, {
        session_id: k,
        session_token: g
      }).catch(() => {
      });
...
function Ff(e, t = {}) {
  const a = e.root, o = t.fetchApi || ((N, V) => (e.api || e.app?.api).fetchApi(N, V)), r = t.apiClient || Yr({ fetchApi: o }), n = t.store || Ef(r), i = Cf(), c = t.previewQueue || Af(), l = e.api || e.app?.api || null;
  let p = t.thumbnailRenderer || null, m = null;
  const f = /* @__PURE__ */ new Set();
  function d() {
    return p ? Promise.resolve(p) : (m || (m = Mf().then((N) => p = Rn(N)).catch(() => p = Rn({}))), m);
  }
  function h(N) {
    return sn(l, Gs(N));
  }
  function u(N) {
    return N.thumbnail ? sn(l, `omnicam/library/${N.thumbnail} [input]`) : "";
  }
  function b() {
    for (const N of n.state.items) {
      ...
      V && c.enqueue(N.id, async () => {
        const Ee = await (await d()).render(V, N.format || "glb");
        if (!Ee) {
          f.add(N.id);
          return;
        }
        if (i.set(N.id, Ee), X(), !(N.source && N.source !== "user"))
          try {
            const Le = await r.uploadThumbnail(N.id, $f(Ee));
  ...
  const v = (N) => a.querySelector(`[data-role="${N}"]`), y = v("assets-panel"), x = v("asset-grid"), k = v("asset-kinds"), g = v("asset-search"), j = v("asset-status"), T = v("asset-import-file"), K = v("scene-tab"), z = v("assets-tab"), J = v("agent-tab"), H = a.querySelector('[data-asset-view="agent"]'), $ = { assets: z, agent: J };
  let P = "", _ = null, R = !0, q = !0, G = "scene", L = 0;
  function C(N, { sticky: V = !1 } = {}) {
    j && (!V && L > Date.now() || (j.textContent = N || "", L = V ? Date.now() + 9e3 : 0));
  }
  function Y(N) {
    if (P = N, !!x)
  ...
  let D = !1;
  function X() {
    if (k && (k.innerHTML = Pf(n.state.filter.kind, n.state.kinds)), x) {
      const N = {};
      for (const V of n.state.items) {
        const me = i.get(V.id) || u(V);
        me && (N[V.id] = me);
      }
      x.innerHTML = Nf(n.state.items, { selectedId: P, thumbUrls: N });
    }
    if (!D) {
      D = !0;
      try {
    ...
    n.state.error ? C(n.state.error.message) : n.state.loading ? C(s("Loading assets...")) : C(s("{n} of {total} assets").replace("{n}", n.state.items.length).replace("{total}", n.state.total));
  }
  function ae(N) {
    if (!N) return;
    const V = Op({
    ...
    Ee && (e.selectedEntity = "object", e.selectedObjectId = Ee, e.selectedObjectIds = /* @__PURE__ */ new Set([Ee]), e.selectedKeyFrame = null), e.restoreAssets?.(), e.refreshObjects?.(), e.refreshInspector?.(), e.render?.(), e.setStatus?.(s("{name} added").replace("{name}", N.name));
  }
  async function le(N) {
    if (N) {
      C(s("Importing {name}...").replace("{name}", N.name));
      try {
        const V = await r.importModel(N, { kind: "prop", name: N.name.replace(/\.[^.]+$/, "") });
        V.asset && n.upsert(V.asset), C(s("Imported {name}").replace("{name}", V.asset?.name || N.name));
      } catch (V) {
        C(V.message || s("Import failed"));
      }
    }
  }
  ...
  async function he(N) {
    ...
    if (!V) {
      C(s("Point to the folder you extracted the pack into."), { sticky: !0 });
      return;
    }
    L = 0, C(N ? s("Scanning {folder}...").replace("{folder}", V) : s("Installing characters from {folder}...").replace("{folder}", V));
    try {
      ...
      C(
        ...
        { sticky: !0 }
      ), Le && (n.refresh(), se && (se.hidden = !0));
    } catch (me) {
      C(me.message || s("Import failed"), { sticky: !0 });
    }
  }
  function ve(N) {
    N === "agent" && !mr() && (N = "scene"), G = N, K && (K.hidden = N !== "scene");
    for (const [V, me] of Object.entries($))
      me && (me.hidden = V !== N);
  ...
  function xe() {
    const N = mr();
    H && (H.hidden = !N), !N && G === "agent" && ve("scene");
  }
  function pe(N) {
    const V = Dn(N.target);
    if (V) {
      if (V.action === "switch-view") return ve(V.view);
      if (V.action === "filter-kind") return void n.setFilter({ kind: V.kind });
      if (V.action === "asset-add")
        return ae(n.get(P));
      if (V.action === "asset-import")
        return T?.click();
      if (V.action === "local-toggle") return ye();
  ...
  function Ie(N) {
    const V = Dn(N.target);
    V?.action === "card" && ae(n.get(V.assetId));
  }
  function we() {
    clearTimeout(_), _ = setTimeout(() => n.setFilter({ search: g.value }), 200);
  }
  function ke(N) {
  ...
  return y?.addEventListener("click", pe), y?.addEventListener("dblclick", Ie), a.querySelector('[data-role="left-tabs"]')?.addEventListener("click", pe), g?.addEventListener("input", we), T?.addEventListener("change", ke), X(), xe(), {
    store: n,
    switchView: ve,
    syncAgentAvailability: xe,
    refresh: () => n.refresh(),
    dispose() {
      be(), clearTimeout(_), y?.removeEventListener("click", pe), y?.removeEventListener("dblclick", Ie), a.querySelector('[data-role="left-tabs"]')?.removeEventListener("click", pe), g?.removeEventListener("input", we), T?.removeEventListener("change", ke), c.clear(), p?.dispose?.(), i.clear();
    }
  };
}
function zf(e, t = {}) {
  ...
  function l() {
    ...
    for (const v of u) {
      ...
      const x = ys(u, v, h) || {
        position: v.position,
        size: v.size
      }, k = e.webgl.projectWorldToScreen(vs(x, v.type));
      ...
      g.style.setProperty("--oc-label-accent", j || ""), g.classList.toggle("is-annotation", n.content === "annotation" && !!j);
    }
...
function Uf(e, t = {}) {
  const a = e.root, o = t.apiClient || Yr({
    fetchApi: (_, R) => (e.api || e.app?.api).fetchApi(_, R)
  }), r = a.querySelector('[data-role="pose-editor"]');
  if (!r) return { sync() {
  }, update() {
  }, dispose() {
  } };
  const n = r.querySelector('[data-role="pose-preset"]'), i = r.querySelector('[data-pose-act="edit"]'), c = r.querySelector('[data-role="pose-joint-row"]'), l = r.querySelector('[data-role="pose-joint-name"]'), p = ["x", "y", "z"].map((_) => r.querySelector(`[data-role="pose-rot-${_}"]`));
  let m = null, f = !1, d = /* @__PURE__ */ new Map();
  const h = () => e.state?.objects?.find((_) => _.id === m) || null, u = () => e.rigMapper?.boneMap && Object.keys(e.rigMapper.boneMap).length ? e.rigMapper.boneMap : null, b = () => e.subSelection?.type === "character_joint" && e.subSelection.objectId === m ? e.subSelection.jointId : null;
  function v(_) {
    const R = Io({ ..._, preset_id: _.id });
    return { id: _.id, name: _.name || _.id, root_offset: R.root_offset, joints: R.joints };
  }
  function y(_) {
    const R = d.get(_?.pose?.preset_id) || null;
    return Vi({ preset: R, overrides: _?.pose }).joints;
  }
  function x() {
    const _ = h();
    !_?.character || !u() || e.webgl?.applyCharacterPose?.(m, u(), f || _.character.pose?.joints ? y(_.character) : {});
  }
  function k() {
    const _ = b(), R = f && !!_;
    if (c && (c.hidden = !R), !R) return;
    l && (l.textContent = _);
    const q = h(), G = y(q?.character)[_] || [0, 0, 0, 1], L = Hi(G);
    p.forEach((C, Y) => {
      C && document.activeElement !== C && (C.value = String(Math.round(L[Y] * 100) / 100));
    });
  }
  function g() {
    const _ = h(), R = !!_?.character?.motion;
    if (i && (i.classList.toggle("active", f), i.disabled = R, i.title = R ? s("Clear the motion clip to edit the pose") : s("Toggle FK pose editing")), n) {
      ...
      n.dataset.sig !== G && (n.dataset.sig = G, n.replaceChildren(...q.map(([L, C]) => {
        const Y = document.createElement("option");
        return Y.value = L, Y.textContent = C, Y;
      }))), document.activeElement !== n && (n.value = _?.character?.pose?.preset_id || "neutral");
    }
    k(), x();
  }
  async function j() {
    try {
      const _ = await o.listPoses();
      d = new Map((_.poses || []).filter((R) => R?.id).map((R) => [R.id, v(R)]));
    } catch {
      d = /* @__PURE__ */ new Map();
    }
  ...
  function T() {
    const _ = b();
    if (!_) return;
    const R = p.map((L) => Number(L?.value) || 0), q = Wi(R), G = e.directorApi?.execute({
      version: 1,
      id: `tx_pose_${Date.now().toString(36)}`,
      description: "Pose joint",
      operations: [{ type: "character.set_joint_rotation", objectId: m, joint: _, rotation: q }]
    });
    G && !G.ok && e.setStatus?.(G.error?.message || s("Could not set the joint")), g();
  }
  function K() {
    const _ = n?.value || "neutral", R = d.get(_) || Io({ preset_id: _ });
    e.directorApi?.execute({
      version: 1,
      id: `tx_preset_${Date.now().toString(36)}`,
      description: "Pose preset",
      operations: [{ type: "character.set_pose", objectId: m, pose: { preset_id: _, root_offset: R.root_offset, joints: {} } }]
    }), g();
  }
  async function z() {
    const _ = h();
    if (!_?.character) return;
    const R = (await Zt(e, s("Save Pose"), s("Pose name"), ""))?.trim();
    if (!R || e.disposed) return;
    const q = R.toLowerCase().replace(qf, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "pose";
    try {
      const G = await o.savePose({
        id: q,
        name: R,
        profile: "omnicam_humanoid_v1",
        root_offset: _.character.pose?.root_offset || [0, 0, 0],
        joints: y(_.character)
      });
  ...
  function J() {
    const _ = h();
    !_?.character || _.character.motion || (f = !f, !f && e.subSelection?.type === "character_joint" && (e.subSelection = null), g(), e.rigOverlay?.update?.());
  }
  function H(_) {
    f && (e.subSelection = { type: "character_joint", objectId: m, jointId: _ }, g(), e.rigOverlay?.update?.());
  }
  const $ = Bf(e, {
    onPick: H,
  ...
  e.rigOverlay = $;
  function P() {
    const _ = e.selectedObject?.(), R = _?.asset_kind === "character";
    if (r.hidden = !R, !R) {
      f && (f = !1), m = null, $.update();
      return;
    }
    _.id !== m && (m = _.id, f = !1, e.subSelection?.type === "character_joint" && (e.subSelection = null), d.size || j()), g(), $.update();
  }
  n?.addEventListener("change", K), i?.addEventListener("click", J), r.querySelector('[data-pose-act="save"]')?.addEventListener("click", z);
  for (const _ of p)
    _?.addEventListener("change", T), _?.addEventListener("input", T);
  return j(), {
    sync: P,
    update() {
      $.update(), x();
    ...
    dispose() {
      $.dispose(), n?.removeEventListener("change", K), i?.removeEventListener("click", J);
      for (const _ of p)
        _?.removeEventListener("change", T), _?.removeEventListener("input", T);
    }
  };
}
function Bn(e, t) {
...
function Wf(e) {
  ...
  function u() {
    ...
    nr(r, k?.start_frame ?? 0), nr(n, k?.end_frame ?? 0), nr(i, k?.speed ?? 1), c && document.activeElement !== c && (c.checked = k ? k.loop !== !1 : !0);
    const g = !!k;
    for (const j of [r, n, i, c]) j && (j.disabled = !g);
    l && (l.disabled = !g);
  }
  function b() {
...
function Yf({
  ...
  function x(L) {
    return u ? {
      position: L.position.map((C, Y) => C - u.position[Y]),
      rotationDeg: L.rotationDeg.map((C, Y) => C - u.rotationDeg[Y]),
      scaleFactors: L.scale.map((C, Y) => u.scale[Y] === 0 ? 1 : C / u.scale[Y])
  ...
  f.addEventListener?.("mouseDown", k), f.addEventListener?.("objectChange", g), f.addEventListener?.("mouseUp", j), f.addEventListener?.("dragging-changed", T);
  function K(L) {
    h = L, m.position.set(...L.position || [0, 0, 0]);
    const C = L.rotation || [0, 0, 0];
    m.rotation.set(C[0] / Ve, C[1] / Ve, C[2] / Ve), m.scale.set(...L.scale || [1, 1, 1]), f.attach(m), f.visible = !0;
  }
  function z() {
  ...
  function H(L) {
    if (!Vf.has(L)) throw new Error(`createTransformControlsAdapter: unknown mode "${L}"`);
    f.setMode ? f.setMode(L) : f.mode = L;
  }
  function $(L) {
    if (!Hf.has(L)) throw new Error(`createTransformControlsAdapter: unknown space "${L}"`);
    f.space = L;
  }
  function P(L) {
  ...
  function G() {
    f.removeEventListener?.("mouseDown", k), f.removeEventListener?.("objectChange", g), f.removeEventListener?.("mouseUp", j), f.removeEventListener?.("dragging-changed", T), o?.remove && d && o.remove(d), o?.remove && o.remove(m), f.dispose?.(), h = null, u = null;
  }
  return {
    attach: K,
    detach: z,
...
function Jf(e, { controlsFactory: t, anchorFactory: a } = {}) {
  ...
  function f() {
    return o || (!e.webgl || !e.interactionElement ? null : (o = Yf({
      ...
      onDragEnd: T,
      onDraggingChanged: ($) => {
        e.transformControlsDragging = $;
      }
    }), p(), o));
  }
  function d({ targetSpec: $ }) {
    if ($) {
      if (c(i), $.type === "object") {
        e.checkpoint("Transform object");
        const P = Yi(e), _ = (P.length ? P : [$.object]).map((q) => ({
          object: q,
          transform: Xe(q)
        }));
        for (const q of _) e.beginObjectEdit(q.object);
        const R = _.reduce((q, G) => fe(q, G.transform.position), [0, 0, 0]).map((q) => q / _.length);
        r = { type: "object", group: _, pivot: R };
        return;
      }
      if ($.type === "camera") {
      ...
      if ($.type === "camera_target") {
        ...
        const P = e.activeCameraTrack?.(), _ = !!P?.target_object_id;
        r = {
          type: "camera_target",
          tracking: _,
          base: _ ? [...P.target_offset || [0, 0, 0]] : [...e.camera.target]
        };
        return;
      }
      if ($.type === "camera_path") {
        ...
        e.checkpoint("Transform camera path"), r = {
          type: "camera_path",
          trackId: P.id,
          origin: Or(P.keyframes),
          baseKeys: P.keyframes.map((_) => ({ ..._, camera: ce(_.camera) }))
        };
        return;
      }
      if ($.type === "path_point" || $.type === "path_group") {
        const P = $.track;
        if (!P || P.locked) return;
        const _ = $.type === "path_point" ? [$.frame] : $.frames;
        e.checkpoint($.type === "path_point" ? "Transform path point" : "Transform path selection"), r = {
          type: $.type,
          trackId: P.id,
          origin: $.position,
      ...
      if ($.type === "path_point_target") {
        const P = $.track;
        if (!P || P.locked || $.readOnly) return;
  ...
  function h($) {
    ...
    return P === "translate" ? { mode: P, delta: $.position } : P === "scale" ? { mode: P, origin: r.origin, factors: $.scaleFactors } : { mode: P, origin: r.origin, rotationDeg: $.rotationDeg };
  }
  function u($) {
    $.id === e.state.active_camera_id && (e.state.keyframes = $.keyframes), e.camera = $e($, e.frame, e.state.objects), $.camera = ce(e.camera), e.refreshKeys();
  }
  function b($) {
    const P = e.state.cameras.find((_) => _.id === r.trackId);
    P && (P.keyframes = ws(r.baseKeys, h($)), u(P));
  }
  function v($) {
    const P = e.state.cameras.find((R) => R.id === r.trackId);
    if (!P) return;
    const _ = { ...h($), lookAtActive: r.lookAtActive };
    P.keyframes = fs(r.baseKeys, r.selectedFrames, _), u(P);
  }
  function y($) {
    const P = e.state.cameras.find((_) => _.id === r.trackId);
    P && (P.keyframes = Ji(r.baseKeys, [r.frame], { delta: $.position }), u(P));
  }
  function x({ delta: $ }) {
    r && (r.type === "object" ? k($) : r.type === "camera" ? g($) : r.type === "camera_target" ? j($) : r.type === "camera_path" ? b($) : r.type === "path_point" || r.type === "path_group" ? v($) : r.type === "path_point_target" && y($), e.render());
  }
  function k($) {
    const { group: P, pivot: _ } = r, R = e.state.gizmo_mode;
    for (const q of P)
      if (R === "translate")
        q.object.position = fe(q.transform.position, $.position);
      else if (R === "rotate")
        q.object.position = fe(_, dn(De(q.transform.position, _), $.rotationDeg)), q.object.rotation = fe(q.transform.rotation, $.rotationDeg);
      else if (R === "scale") {
        const G = De(q.transform.position, _);
        q.object.position = fe(_, G.map((L, C) => L * $.scaleFactors[C])), q.object.size = q.transform.size.map((L, C) => Math.max(0.01, L * $.scaleFactors[C]));
      }
    for (const q of P) e.commitObjectEdit(q.object);
  }
  function g($) {
    if (e.state.gizmo_mode === "translate")
      e.camera.position = fe(r.position, $.position);
    else {
      const P = De(r.target, r.position);
      e.camera.target = fe(r.position, dn(P, $.rotationDeg));
    }
    e.commitCameraEdit();
  }
  function j($) {
    const P = fe(r.base, $.position);
    r.tracking ? Xi(e, P) : e.camera.target = P, e.commitCameraEdit();
  }
  function T({ cancelled: $ }) {
    if (!r) return;
    const P = r.type;
    $ ? (e.undo(), (P === "camera" || P === "camera_target") && e.finishCameraEdit()) : P === "camera" || P === "camera_target" ? e.finishCameraEdit() : (e.editingKeyFrame = null, e.updateKeyVisualState?.(), e.drawCurveEditor?.()), r = null, e.refreshInspector(), e.render();
  }
  function K() {
    if (!e.webgl || !e.interactionElement) return;
    const $ = e.recording ? null : Gi(e), P = e.state.gizmo_mode || "translate";
    if (!(!!$ && !$.readOnly && Xf.has($.type) && $.allowedModes.includes(P))) {
      o?.detach();
      return;
    }
    f() && (o.isDragging() || (o.setCamera(e.webgl.activeCamera), o.setMode(P), o.setSpace(e.state.gizmo_space === "local" ? "local" : "world"), o.attach($)));
  }
  function z() {
  ...
  return { sync: K, isPointerOverHandle: z, cancelDrag: J, dispose: H };
}
function Zf(e, t) {
...
function Ho(e, t, a) {
  ...
  const o = Mt(e), r = o.expanded ? 220 : 138, n = Math.min(r, Math.max(80, Math.min(t, a) - 20)), i = 10, c = Math.max(0, t - n - i), l = Math.max(0, a - n - i), p = e.viewportCamera(), m = p?.position || [0, 1.5, 5], f = p?.target || [0, 0, 0], d = e.activeCameraTrack?.(), h = e.state.cameras?.length ? e.state.cameras : d ? [d] : [], u = h.flatMap(
    (j) => (j.keyframes || []).map((T) => T.camera?.position).filter(Boolean)
  );
  let b = 12;
  if (o.rangeIndex >= 0 && o.rangeIndex < tt.length)
    ...
    const j = Math.max(
      ...
      Math.abs(f[0] || 0),
      Math.abs(f[2] || 0),
      ...u.flatMap((T) => [Math.abs(T[0] || 0), Math.abs(T[2] || 0)]),
      4
    );
    b = Math.max(6, Math.ceil((j + 1.5) / 4) * 4);
  }
...
function rh(e, t, a, o) {
  const r = Ho(e, a, o);
  if (!r) return;
  const { rx: n, ry: i, radarSize: c, cx: l, cy: p, innerRadius: m, range: f, scale: d, camPos: h, camTgt: u, cameras: b, activeTrack: v } = r, y = Mt(e), x = e.state.active_camera_id || v?.id, k = (re, de) => ri(r, re, de);
  ...
  for (const re of e.state.objects || []) {
    ...
    const de = Fr(e.state.objects, re).position || [0, 0, 0], [ye, he] = k(de[0], de[2]);
  ...
  for (let re = 0; re < b.length; re++) {
    const de = b[re], ye = de.keyframes || [], he = de.id === x, ve = de.color || Un[re % Un.length];
    ...
    for (let we = pe; Number.isFinite(we) && we <= Ie; we++) {
      ...
      const [be, N] = k(ke[0], ke[2]);
      xe ? t.lineTo(be, N) : (t.moveTo(be, N), xe = !0);
    }
    ...
    for (const we of ye) {
      ...
      const V = we.frame === e.frame && he;
      t.fillStyle = V ? "#ffffff" : ve, t.globalAlpha = he ? 0.95 : 0.6, t.beginPath(), t.moveTo(be, N - 3), t.lineTo(be + 3, N), t.lineTo(be, N + 3), t.lineTo(be - 3, N), t.closePath(), t.fill(), V && (t.strokeStyle = "#00d2d3", t.lineWidth = 1.2, t.stroke());
    }
    t.restore();
  }
  const [T, K] = k(h[0] || 0, h[2] || 0), [z, J] = k(u[0] || 0, u[2] || 0), H = 8, $ = te(T, n + H, n + c - H), P = te(K, i + H, i + c - H), _ = te(z, n + H, n + c - H), R = te(J, i + H, i + c - H), q = h[1] || 0, G = ah(q), L = oh(q);
  t.strokeStyle = "rgba(255, 255, 255, 0.40)", t.lineWidth = 1, t.setLineDash([3, 3]), t.beginPath(), t.moveTo($, P), t.lineTo(_, R), t.stroke(), t.setLineDash([]), t.fillStyle = "#ffffff", t.beginPath(), t.arc(_, R, 2.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "rgba(255, 255, 255, 0.7)", t.lineWidth = 1, t.beginPath(), t.arc(_, R, 4.5, 0, Math.PI * 2), t.stroke();
  const C = u[0] - h[0], Y = u[2] - h[2], D = Math.atan2(Y, C), ae = (e.viewportCamera().fov || 35) * Math.PI / 360, le = te(24 * (d / (m / 8)), 16, 38), se = t.createRadialGradient($, P, 2, $, P, le);
  se.addColorStop(0, G + "55"), se.addColorStop(1, G + "08"), t.fillStyle = se, t.strokeStyle = G, t.lineWidth = 1.2, t.beginPath(), t.moveTo($, P), t.lineTo($ + Math.cos(D - ae) * le, P + Math.sin(D - ae) * le), t.arc($, P, le, D - ae, D + ae), t.closePath(), t.fill(), t.stroke(), t.fillStyle = G + "44", t.beginPath(), t.arc($, P, 6.5, 0, Math.PI * 2), t.fill(), t.fillStyle = G, t.beginPath(), t.arc($, P, 3.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "#ffffff", t.lineWidth = 1.5, t.beginPath(), t.moveTo($, P), t.lineTo($ + Math.cos(D) * 7, P + Math.sin(D) * 7), t.stroke(), nh(e, t, r, G, L, D, Math.hypot(C, Y)), y.hover && sh(t, r, y.hover), t.restore();
}
function nh(e, t, a, o, r, n, i) {
  ...
  const u = m.expanded ? "⤡" : "⤢";
  ko(t, c + 96, f, 14, d, u, m.hover?.button === "size"), t.font = "bold 8.5px monospace", t.fillStyle = o, t.textAlign = "right", t.fillText(`Y:${r}`, c + p - 5, l + 9), t.fillStyle = "rgba(15, 23, 42, 0.70)", t.fillRect(c, l + p - 15, p, 15), t.strokeStyle = "rgba(0, 210, 211, 0.15)", t.beginPath(), t.moveTo(c, l + p - 15), t.lineTo(c + p, l + p - 15), t.stroke();
...
function si(e, t, a, o) {
  ...
  for (const v of e.state.objects || []) {
    ...
    const y = Fr(e.state.objects, v).position || [0, 0, 0], [x, k] = m(y[0], y[2]);
...
function ch(e, t, a, o) {
  ...
  if (n.drag) {
    const [b, v] = jr(r, a, o), y = e.viewportCamera();
...
function dh(e, t, a) {
  ...
  const o = () => {
    ...
    const u = String(t('[data-role="object-annotation"]')?.value || "").trim(), b = u ? hs({
      text: u,
  ...
  for (let r = 1; r < a.length - 1; r++) {
    ...
    if (i.camera) {
      for (const m of yh) {
        ...
        if (Array.isArray(f) && Array.isArray(h) && Array.isArray(d)) {
          const u = Vn(f, d, h, p);
          a[r].camera[m] = Gn(d.map(Number), u, o);
        }
      }
      for (const m of vh) {
        const f = n.camera?.[m], d = i.camera?.[m], h = c.camera?.[m];
    ...
    if (Array.isArray(i.position) && Array.isArray(n.position) && Array.isArray(c.position)) {
      for (const m of wh) {
        ...
        if (Array.isArray(f) && Array.isArray(h) && Array.isArray(d)) {
          const u = Vn(f, d, h, p);
          a[r][m] = Gn(d.map(Number), u, o);
        }
      }
      for (const m of kh) {
        const f = n[m], d = i[m], h = c[m];
        if (Array.isArray(f) && Array.isArray(h) && Array.isArray(d)) {
          const u = [0, 1, 2].map((b) => Hn(f[b], d[b], h[b], p));
          a[r][m] = d.map((b, v) => Yn(Number(b), u[v], o));
        }
      }
    }
  }
...
function Mh(e, t) {
  ...
  const o = (r) => {
    ...
    const u = (v) => {
      ...
      i.value = g.toFixed(j), i.dispatchEvent(new Event("input", { bubbles: !0 })), i.dispatchEvent(new Event("change", { bubbles: !0 }));
    }, b = (v) => {
...
function ii(e) {
  ...
  if (m)
    if (o?.target && Array.isArray(o.target) && Array.isArray(o.position)) {
      const u = He(De(o.position, o.target));
...
const So = () => import("./chunk-Bqfw91eb.js");
function Kh(e, t) {
...
function xy(e, t, a = ci) {
  ...
  const h = new e.FogExp2(1250588, 8e-3), u = Hh(e), b = new e.PMREMGenerator(t);
  b.compileEquirectangularShader();
  const v = new yd(), y = b.fromScene(v, 0.04).texture;
  return v.traverse((x) => {
    x.geometry?.dispose?.();
...
function cu(e) {
  ...
  for (let d = m; d < i.length; d++) {
    const h = i[d], u = su(i, d, p, e.planeAxis), b = ce(c);
    b.position = [...h], b.target = iu(h, u, l, e.planeAxis), f.push({
...
function Jn(e, t = {}) {
  ...
  if (a === "extend") {
    ...
    const u = Math.max(0, Math.round(Number(e.state.duration_frames) || 1) - 1), [, b] = Ar(e.state), v = d.length > 1 ? d[d.length - 1].frame - d[0].frame : 24;
...
function Cu(e) {
  ...
  for (const v of e.state.motion_layers || []) {
    ...
    if (c.strokeStyle = k ? "#ffcc4d" : "rgba(65,217,197,0.6)", c.lineWidth = (k ? 2.4 : 1.5) * r, c.beginPath(), x.forEach((j, T) => {
      const K = p(j.x), z = m(j.y);
      T ? c.lineTo(K, z) : c.moveTo(K, z);
    }), c.stroke(), !y) {
  ...
  for (const f of i) {
    ...
    if (h) {
      if (d === "image" || d === "video") {
        ...
        if (u && u.value) {
          const b = String(u.value), v = /\.(mp4|mov|webm|mkv|m4v|avi)(?:\s|$)/i.test(b), y = h.widgets?.find((g) => String(g.name).toLowerCase() === "subfolder")?.value || "", x = at(kt(b, y)), k = e.state.objects.find((g) => g.id === "subject");
          if (k) {
            const g = await yi(e, k, x, r, v);
            if (!r()) return;
            k.asset = kt(b, y);
            let j = null;
            if (v)
              try {
                j = await qu(b, o.signal);
              } catch (T) {
          ...
          }), nt(e, "subject", b, !1, b.currentSrc || b.src || ""), Qn(e, b, { frameCount: b instanceof HTMLVideoElement ? 0 : 1 }), e.upstreamImageConnected = !0, n = !0, e.render(), e.setStatus(b instanceof HTMLVideoElement ? s("Upstream video preview synced") : s("Upstream image preview synced")));
        }
      }
      if (d === "audio") {
        l = !0;
        ...
        if (u && u.value) {
          const b = String(u.value), v = h.widgets?.find((x) => String(x.name).toLowerCase() === "subfolder")?.value || "", y = at(kt(b, v));
          try {
      ...
      if (d === "scene_3d" || d === "model" || d === "mesh") {
        ...
        if (u && u.value) {
          ...
          if (["glb", "gltf", "obj", "fbx", "stl", "ply"].includes(v)) {
            const y = h.widgets?.find((j) => String(j.name).toLowerCase() === "subfolder")?.value || "", x = at(kt(b, y)), k = `upstream_scene_${h.id}`;
            p.add(k);
            let g = e.state.objects.find((j) => j.id === k);
            g ? (g.asset = kt(b, y), g.format = v === "gltf" ? "glb" : v) : (g = {
              id: k,
              type: "model",
              format: v === "gltf" ? "glb" : v,
  ...
  try {
    for (const k of Ju)
      if (!(n.isTypeSupported && !n.isTypeSupported(k)))
        try {
          f = new n(m, { mimeType: k, videoBitsPerSecond: Zu(r) });
          break;
        } catch {
        }
...
async function ob(e) {
  ...
  try {
    let n = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: l } = await import("./chunk-BEUwwIUD.js");
...
function cb(e) {
  ...
  for (const m of a) {
    ...
    if (!h) continue;
    const [u, b] = h, v = t.content === "annotation" && m.annotation?.color || "", y = 6 * c, x = 4 * c, g = r.measureText(f).width + y * 2, j = 12 * c + x * 2, T = Math.round(u - g / 2), K = Math.round(b - j - 6 * c);
    r.fillStyle = "rgba(16,17,22,0.82)", lb(r, T, K, g, j, 4 * c), r.fill(), v && (r.strokeStyle = v, r.lineWidth = Math.max(1, c), r.stroke()), r.fillStyle = v || "#e6e6ec", r.fillText(f, T + y, K + j - x - 2 * c);
  }
  r.restore();
}
...
function bb(e, t) {
  const [a, o, r] = t.position || [0, 0, 0], n = t.size?.[1] || 1.8, i = (t.size?.[0] || 0.7) * 0.5, c = [a, o + n * 0.88, r], l = [a, o + n * 0.76, r], p = [a - i * 0.55, o + n * 0.73, r], m = [a + i * 0.55, o + n * 0.73, r], f = [a - i * 0.72, o + n * 0.52, r], d = [a + i * 0.72, o + n * 0.52, r], h = [a - i * 0.82, o + n * 0.34, r], u = [a + i * 0.82, o + n * 0.34, r], b = [a, o + n * 0.44, r], v = [a - i * 0.28, o + n * 0.44, r], y = [a + i * 0.28, o + n * 0.44, r], x = [a - i * 0.28, o + n * 0.22, r], k = [a + i * 0.28, o + n * 0.22, r], g = [a - i * 0.28, o, r + 0.05], j = [a + i * 0.28, o, r + 0.05];
  ie(e, c, l, "#aaa", 2), ie(e, l, b, "#aaa", 2), ie(e, p, m, "#aaa", 2), ie(e, p, f, "#aaa", 2), ie(e, f, h, "#aaa", 2), ie(e, m, d, "#aaa", 2), ie(e, d, u, "#aaa", 2), ie(e, v, y, "#aaa", 2), ie(e, v, x, "#aaa", 2), ie(e, x, g, "#aaa", 2), ie(e, y, k, "#aaa", 2), ie(e, k, j, "#aaa", 2);
  const T = Ye(c, e.viewportCamera(), e.canvas.width, e.canvas.height);
  T && (e.ctx.strokeStyle = "#aaa", e.ctx.beginPath(), e.ctx.arc(T[0], T[1], te(28 / T[2], 3, 12), 0, Math.PI * 2), e.ctx.stroke());
}
function gb(e, t) {
  ...
  if (b)
    try {
      ...
      if (e.ctx.fillStyle = "#111", e.ctx.fillRect(f, h, v, y), g === "stretch" || !x || !k)
        ...
        const j = Math.min(v / x, y / k), T = x * j, K = k * j;
        e.ctx.drawImage(b, f + (v - T) / 2, h + (y - K) / 2, T, K);
      } else {
        const j = Math.max(v / x, y / k), T = v / j, K = y / j;
        e.ctx.drawImage(b, (x - T) / 2, (k - K) / 2, T, K, f, h, v, y);
      }
    } catch {
    }
  else
...
function ji(e) {
  ...
  const r = (d, h, u, b, v = "") => {
    ...
    `, v.addEventListener("click", () => {
      e.outlinerCollapsedSections ||= /* @__PURE__ */ new Set(), e.outlinerCollapsedSections.has(u) ? e.outlinerCollapsedSections.delete(u) : e.outlinerCollapsedSections.add(u), ji(e);
    ...
    if (t.appendChild(h), !u)
      for (const b of d) {
        ...
        if (j.className = "scene-item-label", k || y) {
          ...
          z.style.cssText = `color:${k ? "#f59e0b" : "#58cc6b"};font-weight:700`, z.textContent = k ? "● " : "○ ", j.appendChild(z);
        }
        if (j.appendChild(document.createTextNode(b.name)), x) {
          const z = document.createElement("span");
          z.style.cssText = "color:#f2d06b;font-size:10px", z.title = "Playblast Output", z.textContent = " ★", j.appendChild(z);
        }
        if (b.muted) {
          const z = document.createElement("span");
          z.style.opacity = ".6", z.textContent = " (muted)", j.appendChild(z);
        }
        const T = document.createElement("div");
        T.className = "scene-item-actions", T.appendChild(r("pi-star", "Solo track", b.solo, () => {
          e.checkpoint("Solo track"), b.solo = !b.solo, e.serialize(), e.refreshObjects(), e.renderCameraView();
        }, "color:#fbbf24;border-color:#78350f;background:rgba(245,158,11,0.2)")), T.appendChild(r("pi-volume-off", "Mute track", b.muted, () => {
          e.checkpoint("Mute track"), b.muted = !b.muted, e.serialize(), e.refreshObjects(), e.renderCameraView();
        }, "color:#f87171;border-color:#7f1d1d;background:rgba(239,68,68,0.15)")), T.appendChild(r("pi-lock", "Lock track", b.locked, () => {
          e.checkpoint("Lock track"), b.locked = !b.locked, e.serialize(), e.refreshObjects(), e.renderCameraView();
        })), (b.keyframes || []).length >= 1 && T.appendChild(r(
          "pi-arrows-alt",
        ...
        )), T.appendChild(r("pi-ellipsis-v", "Camera actions", !1, (z) => e.openCameraContext(z, b.id, !1))), v.append(g, j, T), v.title = k ? s("Currently selected for editing") : x ? s("Active playblast camera") : s("Click to select & activate this camera");
        const K = () => {
        ...
        v.addEventListener("contextmenu", (z) => {
          z.preventDefault(), z.stopPropagation(), e.openCameraContext(z, b.id, !1);
        }), v.addEventListener("keydown", (z) => {
          (z.key === "Enter" || z.key === " ") && (z.preventDefault(), K());
        }), t.appendChild(v);
      }
  }
  if (m) {
    ...
    const b = [], v = (g, j) => {
      b.push({ object: g, level: j });
      const T = h.get(g.id) || [];
      for (const K of T) v(K, j + 1);
    };
    for (const g of u) v(g, 0);
    const y = b.filter(({ object: g }) => {
      const j = [g.name, g.type, ...g.tags || [], g.asset_kind, g.asset_id].filter(Boolean).join(" ");
      return !(!i(j) || a === "lights" && !l(g.type) || a === "objects" && l(g.type) || a === "hidden" && g.enabled !== !1);
    }), { header: x, isCollapsed: k } = c(a === "lights" ? s("Lights") : s("Objects"), y.length, "objects");
    if (t.appendChild(x), !k)
      for (const { object: g, level: j } of y) {
        const T = document.createElement("div");
        T.role = "button", T.tabIndex = 0, T.dataset.objectId = g.id;
        const K = e.selectedEntity === "object" && (g.id === e.selectedObjectId || e.selectedObjectIds?.has?.(g.id)), z = e.selectedEntity === "object" && g.id === e.selectedObjectId;
        T.setAttribute("aria-selected", String(K)), T.className = `scene-item${K ? " selected" : ""}${z ? " primary" : ""}${j > 0 && !n ? " scene-item-child" : ""}`, j > 0 && !n && (T.style.paddingLeft = `${j * 16 + 6}px`);
        const J = g.type === "card" ? { icon: "pi-image", color: "#38bdf8" } : g.type === "model" || g.type === "glb" ? { icon: "pi-box", color: "#c084fc" } : g.type === "ground" ? { icon: "pi-minus", color: "#fbbf24" } : g.type === "cube" ? { icon: "pi-stop", color: "#fbbf24" } : g.type === "sphere" ? { icon: "pi-circle", color: "#fbbf24" } : g.type === "cylinder" ? { icon: "pi-database", color: "#fbbf24" } : g.type === "torus" ? { icon: "pi-circle", color: "#fbbf24" } : g.type === "pyramid" ? { icon: "pi-play", color: "#fbbf24" } : g.type === "sun_light" ? { icon: "pi-sun", color: "#f59e0b" } : g.type === "point_light" ? { icon: "pi-bolt", color: "#fbbf24" } : g.type === "spot_light" ? { icon: "pi-compass", color: "#38bdf8" } : g.type === "human" ? { icon: "pi-user", color: "#34d399" } : { icon: "pi-plus", color: "#94a3b8" }, H = g.enabled !== !1, $ = !!g.load_error, P = document.createElement("i");
        P.className = `pi ${$ ? "pi-exclamation-triangle" : J.icon}`, P.style.cssText = $ ? "color:#f87171" : H ? `color:${J.color}` : "opacity:.4";
        const _ = document.createElement("span");
        _.className = "scene-item-label";
        const R = document.createElement("span");
        R.style.cssText = $ ? "color:#fca5a5" : H ? "" : "opacity:.5;text-decoration:line-through", R.textContent = g.name || g.type, R.title = s("Double-click to rename"), R.addEventListener("dblclick", (C) => {
          C.preventDefault(), C.stopPropagation(), qb(e, g, R);
        }), _.appendChild(R);
        const q = Array.isArray(g.tags) ? g.tags : [];
        if (q.length) {
          const C = document.createElement("span");
          C.className = "scene-item-tags";
          for (const Y of q.slice(0, 2)) {
            const D = document.createElement("span");
            D.className = "scene-item-tag", D.textContent = Y, C.appendChild(D);
          }
          if (q.length > 2) {
            const Y = document.createElement("span");
            Y.className = "scene-item-tag scene-item-tag-more", Y.textContent = `+${q.length - 2}`, C.appendChild(Y);
          }
          _.appendChild(C);
        }
        if ($) {
          const C = document.createElement("span");
          C.style.cssText = "color:#ef4444;font-size:9px;font-weight:700", C.textContent = " [Format!]", _.appendChild(C);
        }
        const G = document.createElement("div");
        G.className = "scene-item-actions", G.appendChild(r(H ? "pi-eye" : "pi-eye-slash", H ? "Hide object (Alt+Click to Isolate)" : "Show object (Alt+Click to Isolate)", !H, (C) => {
          if (C?.altKey) {
            ...
            e.toggleObject(g.id);
        }, "color:#ef4444;opacity:.7")), G.appendChild(r(g.locked ? "pi-lock" : "pi-lock-open", "Lock object", g.locked, () => Jr(e, g))), G.appendChild(r("pi-copy", "Duplicate object", !1, () => e.duplicateObject?.(g.id))), g.id !== "subject" && G.appendChild(r("pi-trash", "Delete object", !1, () => e.deleteObject?.(g.id))), G.appendChild(r("pi-ellipsis-v", "Object actions", !1, (C) => e.openObjectContext(C, g.id))), T.append(P, _, G), T.title = s("Click to select · Double-click to toggle visibility · Right-click for actions");
        const L = (C = {}) => {
          if (C.altKey && g.id !== "subject") return void e.deleteObject(g.id);
          if (e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectIds ||= /* @__PURE__ */ new Set(), C.ctrlKey || C.metaKey)
            e.selectedObjectIds.has(g.id) ? e.selectedObjectIds.delete(g.id) : e.selectedObjectIds.add(g.id), e.outlinerAnchorId = g.id;
          else if (C.shiftKey && e.outlinerAnchorId && e.state.objects.some((D) => D.id === e.outlinerAnchorId)) {
            const D = e.state.objects.map((le) => le.id), X = D.indexOf(e.outlinerAnchorId), ae = D.indexOf(g.id);
            e.selectedObjectIds = new Set(D.slice(Math.min(X, ae), Math.max(X, ae) + 1));
          } else
          ...
          for (const D of t.querySelectorAll(".scene-item")) {
            const X = !!(D.dataset.objectId && e.selectedObjectIds.has(D.dataset.objectId)), ae = !!(D.dataset.objectId && D.dataset.objectId === e.selectedObjectId);
            D.classList.toggle("selected", X), D.classList.toggle("primary", ae), D.dataset.objectId && D.setAttribute("aria-selected", String(X));
          }
        ...
        T.addEventListener("dblclick", () => e.toggleObject(g.id)), T.addEventListener("contextmenu", (C) => {
          C.preventDefault(), C.stopPropagation(), e.openObjectContext(C, g.id);
...
function Yb(e) {
  ...
  a && (a.hidden = !t);
  const o = (_) => e.root.querySelector(_), r = e.activeCameraTrack(), n = o('[data-role="camera-target-object"]');
  if (n) {
    const _ = r.target_object_id || e.state.target_object_id || "", R = `T${mn()}${e.state.objects.map((q) => `${q.id}\0${q.name || q.type}`).join("|")}`;
    lr(n, R, () => [
      Ht("", s("Manual Target (No Tracking)")),
      ...e.state.objects.map((q) => Ht(q.id, `${s("Track:")} ${q.name || q.type}`))
    ], _);
  }
  Bb(e);
  const i = [...e.camera.position, ...e.camera.target, e.camera.fov, e.camera.roll || 0, e.camera.near, e.camera.far, ...Dr(e.camera)];
  ["camera-px", "camera-py", "camera-pz", "camera-tx", "camera-ty", "camera-tz", "camera-fov", "camera-roll", "camera-near", "camera-far", "camera-rx", "camera-ry", "camera-rz"].forEach((_, R) => {
    for (const q of e.root.querySelectorAll(`[data-role="${_}"]`))
      document.activeElement !== q && (q.value = String(Math.round(i[R] * 1e4) / 1e4));
  });
  for (const _ of e.root.querySelectorAll('[data-role="camera-type"]'))
    document.activeElement !== _ && (_.value = e.camera.camera_type || "perspective");
  for (const _ of e.root.querySelectorAll('[data-role="speed"]'))
    document.activeElement !== _ && (_.value = String(e.cameraSpeed || 1));
  for (const _ of e.root.querySelectorAll('[data-role="active-camera-select"]'))
    document.activeElement !== _ && (_.value = e.state.active_camera_id);
  for (const _ of e.root.querySelectorAll('[data-role="camera-color"]'))
    document.activeElement !== _ && (_.value = r?.color || "#4aa3ef");
  if (!t) {
    const _ = o('[data-role="object-recon-badge"]');
    _ && (_.hidden = !0);
    const R = o('[data-role="selected-name"]');
    R && (R.textContent = `${r.name} · F${e.frame}`), ns(o, {
  ...
  if (c) {
    ...
    if (_) {
      c.hidden = !1;
      const R = _.semantic ? `${_.semantic} · ` : "";
      c.textContent = `${R}${_.label} (${Math.round(_.confidence * 100)}%)`, c.title = _.title, c.className = `oc-recon-badge oc-badge-${_.band}`;
    } else
      c.hidden = !0;
  }
  ...
  if (l) {
    ...
    const _ = l.querySelector("i");
    _ && (_.className = `pi ${t.locked ? "pi-lock" : "pi-lock-open"}`);
  }
  ...
  for (const [_, R] of Object.entries(h))
    for (const q of e.root.querySelectorAll(`[data-role="${_}"]`))
      document.activeElement !== q && (q.value = String(Math.round(R * 1e4) / 1e4));
  for (const _ of e.root.querySelectorAll('[data-role="object-material"]'))
    document.activeElement !== _ && (_.value = t.material_mode || "textured");
  for (const _ of e.root.querySelectorAll('[data-role="object-color"]'))
    document.activeElement !== _ && (_.value = t.color || "#8c929b");
  for (const _ of e.root.querySelectorAll('[data-role="object-light-color"]'))
    document.activeElement !== _ && (_.value = t.color || "#ffffff");
  for (const _ of e.root.querySelectorAll("[data-transform-mode]")) _.classList.toggle("active", _.dataset.transformMode === (e.state.gizmo_mode || "translate"));
  const u = o('[data-role="animation-row"]'), b = o('[data-role="animation-select"]'), v = o('[data-role="object-parent"]');
  if (v) {
    const _ = t.id, R = /* @__PURE__ */ new Set([_]);
    let q = !0;
    for (; q; ) {
      q = !1;
      for (const C of e.state.objects)
        !R.has(C.id) && C.parent_id && R.has(C.parent_id) && (R.add(C.id), q = !0);
    }
    const G = e.state.objects.filter((C) => !R.has(C.id)), L = `P${mn()}${_}${G.map((C) => `${C.id} ${C.name || C.type}`).join("|")}`;
    lr(v, L, () => [
      Ht("", s("No parent")),
      ...G.map((C) => Ht(C.id, C.name || C.type))
    ], t.parent_id || "");
  }
  ...
  j && (j.hidden = y);
  const T = o('[data-role="scale-row"]');
  T && (T.hidden = y);
  const K = o('[data-role="rotation-row"]');
  if (K && (K.hidden = t.type === "point_light"), y) {
    const _ = o('[data-role="object-intensity"]');
    _ && document.activeElement !== _ && (_.value = String(t.intensity ?? (t.type === "sun_light" ? 2.2 : t.type === "spot_light" ? 3 : 2)));
  ...
  const z = e.modelInfoById.get(t.id);
  if (u && (u.hidden = !z?.animations), b) {
    const _ = z?.animationNames || [];
    lr(b, `A${_.join("|")}`, () => _.map((R, q) => Ht(String(q), R)), String(t.animation_index || 0));
  }
  ...
  const $ = o('[data-role="object-annotation-color"]');
  $ && document.activeElement !== $ && ($.value = t.annotation?.color || "#8d7ee8");
...
function mg(e) {
  ...
  const t = e.root.querySelector('[data-role="key-interp"]')?.value || e.root.querySelector('[data-role="interp"]')?.value || "ease", a = Pe(e), o = Ae(e), r = a ? { frame: e.frame, transform: Xe(a), interpolation: t } : { frame: e.frame, camera: ce(e.camera), interpolation: t }, n = o.findIndex((i) => i.frame === e.frame);
...
function ug(e) {
  ...
  const o = t ? { frame: e.frame, transform: Xe(e.copiedKeyframe.transform), interpolation: e.copiedKeyframe.interpolation } : { frame: e.frame, camera: ce(e.copiedKeyframe.camera), interpolation: e.copiedKeyframe.interpolation }, r = Ae(e), n = r.findIndex((i) => i.frame === e.frame);
...
function Pg(e) {
  ...
  return {
    ...
    hideInternalWidgets() {
      for (const F of ["state_json", "recording_path", "card_asset"]) {
        const O = this.node.widgets?.find((S) => S.name === F);
        O && (O.computeSize = () => [0, -4], O.draw = () => {
        }, O.hidden = !0, O.options = { ...O.options || {}, hideInVueNodes: !0 });
      }
    },
    restoreFromWidgets() {
      h(this);
    },
    restoreHistorySnapshot(F) {
      ...
      const S = ss(this.state), w = new Set(this.state.objects.map((Q) => Q.id));
      this.state = gt(O.state);
      const E = new Set(this.state.objects.map((Q) => Q.id));
      for (const Q of w) E.has(Q) || this.removeObjectResources(Q);
      this.frame = Dt(O.frame, 0, this.state.duration_frames - 1);
      const M = new Set(this.state.objects.map((Q) => Q.id)), U = Array.isArray(O.selectedObjectIds) ? O.selectedObjectIds : [O.selectedObjectId].filter(Boolean);
      this.selectedObjectIds = new Set(U.filter((Q) => M.has(Q))), this.selectedObjectId = this.selectedObjectIds.has(O.selectedObjectId) ? O.selectedObjectId : [...this.selectedObjectIds].at(-1) || null, this.selectedEntity = this.selectedObjectIds.size ? "object" : O.selectedEntity || "camera";
      const Z = new Set(this.timelineKeyframes().map((Q) => Q.frame)), B = Array.isArray(O.selectedKeyFrames) ? O.selectedKeyFrames : [O.selectedKeyFrame].filter((Q) => Q != null);
      this.selectedKeyFrames = new Set(B.filter((Q) => Z.has(Q))), this.selectedKeyFrame = this.selectedKeyFrames.has(O.selectedKeyFrame) ? O.selectedKeyFrame : [...this.selectedKeyFrames].at(-1) ?? null, this.pathSelection = il(this.pathSelection, this.activeCameraTrack()), this.subSelection = O.subSelection || null, this.camera = Ze(this.state, this.frame), Yt(this, this.activeCameraTrack(), this.camera, this.frame), this.cameraPreviewSignature = "", this.serialize(), S !== ss(this.state) && this.restoreAssets(), this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render();
    },
    checkpoint(F) {
      this.history.checkpoint(F);
    },
    undo() {
      const F = this.history.undo();
      F && this.setStatus(`Undo: ${F}`);
    },
    redo() {
      const F = this.history.redo();
      F && this.setStatus(`Redo: ${F}`);
    },
    bindEditorEvents() {
      y(this);
    },
    ...
    async renameCamera(F) {
      return H(this, F);
    },
    duplicateCamera(F) {
      T(this, F);
    },
    async deleteCamera(F) {
      return g(this, F);
    },
    activateCamera(F) {
      x(this, F);
    },
    setPlayblastCamera(F) {
      $(this, F);
    },
    closeMenus(F = null) {
      for (const O of this.root.querySelectorAll(".toolbar-menu")) O !== F && (O.open = !1);
    ...
    showContextMenu(F, O, S) {
      return this.contextMenu.show(F, O, S);
    },
    onContextMenu(F) {
      if (F.preventDefault(), F.stopPropagation(), F.stopImmediatePropagation?.(), F.altKey || this.state.navigation_profile === "simple" && F.target?.closest?.(".viewport-wrap")) return;
      const O = F.target, S = O.closest?.(".camera-preview-tile"), w = O.closest?.(".scene-item"), E = O.closest?.(".key");
      if (S) return this.openCameraContext(F, S.dataset.cameraId, !0);
      if (w?.dataset.cameraId) return this.openCameraContext(F, w.dataset.cameraId, !1);
      if (w?.dataset.objectId) return this.openObjectContext(F, w.dataset.objectId);
      if (E) {
        const M = this.timelineKeyframes().find((U) => U.frame === Number(E.dataset.keyFrame));
        return M && this.selectKeyframe(M), this.openTimelineContext(F, !0);
      }
      if (O.closest?.('[data-role="keys"]'))
        return this.setFrame(this.timelineFrameFromEvent(F, O.closest('[data-role="keys"]'))), this.openTimelineContext(F, !1);
      if (O.closest?.(".curve-editor")) return this.openCurveContext(F);
      if (O.closest?.(".viewport-wrap")) {
        const M = this.interactionElement.getBoundingClientRect(), U = (F.clientX - M.left) * this.canvas.width / Math.max(1, M.width), Z = (F.clientY - M.top) * this.canvas.height / Math.max(1, M.height), B = this.pickSceneObject([U, Z]);
        if (B) {
    ...
    openObjectContext(F, O) {
      const S = this.state.objects.find((M) => M.id === O);
      if (!S) return;
      this.selectedEntity = "object", this.selectedObjectId = O, this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render();
      const w = ["sun_light", "point_light", "spot_light"].includes(S.type), E = this.selectedObjectIds?.size || 0;
      if (E >= 2 && this.selectedObjectIds.has(O)) {
        this.showContextMenu(F, `${E} ${s("objects selected")}`, [
      ...
      this.showContextMenu(F, S.name || S.type, [
        { label: s("Set key"), icon: "pi-key", shortcut: "I", run: () => this.insertKeyframe() },
        { label: s("Frame subject"), icon: "pi-search", shortcut: "F", run: () => this.frameTarget() },
        { label: s("Rename object…"), icon: "pi-pencil", run: () => this.renameObject(O) },
        { label: s("Duplicate object"), icon: "pi-copy", run: () => this.duplicateObject(O) },
        { label: S.enabled === !1 ? s("Show object") : s("Hide object"), icon: S.enabled === !1 ? "pi-eye" : "pi-eye-slash", run: () => this.toggleObject(O) },
        { label: S.locked ? s("Unlock object") : s("Lock object"), icon: S.locked ? "pi-lock" : "pi-lock-open", run: () => Jr(this, S) },
        null,
        {
          label: s("Transform mode"),
          icon: "pi-arrows-alt",
        ...
        },
        {
          ...
          icon: "pi-bullseye",
          items: [
            { label: s("Camera tracks this object (Look-At)"), icon: "pi-bullseye", help: s("Lock camera live look-at tracking to this moving object"), run: () => this.aimAtSelectedObject(O) },
            ...
            null,
            { label: s("Select hierarchy"), icon: "pi-sitemap", shortcut: "Shift+G", help: s("Select this object and all descendants"), run: () => this.selectHierarchy(O) }
          ]
        },
        ...w ? [
          {
        ...
        null,
        { label: s("Reset entire animation"), icon: "pi-replay", danger: !0, help: s("Delete every animation key and return position/rotation to zero"), run: () => this.resetObjectAnimation(O) },
        null,
        { label: s("Delete object"), icon: "pi-trash", danger: !0, disabled: O === "subject", help: O === "subject" ? s("The canonical subject card cannot be deleted") : s("Delete this object and its animation keys"), run: () => this.deleteObject(O) }
      ]);
    },
    openCameraContext(F, O, S = !1) {
      const w = this.state.cameras.find((E) => E.id === O);
      w && (this.selectedEntity = "camera", this.selectedObjectId = null, this.activateCamera(O), this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render(), this.showContextMenu(F, `${w.name}${S ? " preview" : ""}`, [
        { label: s("Edit this camera"), icon: "pi-video", run: () => this.activateCamera(O) },
        {
          label: s("Select whole path — move / scale / rotate"),
          icon: "pi-arrows-alt",
          disabled: (w.keyframes || []).length < 1,
          run: () => {
            this.activateCamera(O), this.selectCameraPath() && this.setStatus(`${w.name} · ${s("whole path selected — move / scale / rotate")}`);
          }
        },
        { label: s("Set as primary / playblast"), icon: "pi-star", disabled: O === this.state.playblast_camera_id, run: () => this.setPlayblastCamera(O) },
        { label: s("Set key at playhead"), icon: "pi-key", shortcut: "I", run: () => {
          this.activateCamera(O), this.insertKeyframe();
        } },
        { label: s("Record this preview"), icon: "pi-circle-fill", run: () => {
          this.setPlayblastCamera(O), this.makePlayblast();
        } },
        { label: this.state.maximized_camera_id === O ? s("Restore preview size") : s("Maximize preview"), icon: "pi-window-maximize", run: () => this.maximizeCameraPreview(O) },
        null,
        {
          label: s("Shot order & handles"),
          icon: "pi-sliders-h",
          items: [
            { label: s("Shot: move earlier"), icon: "pi-arrow-up", disabled: this.state.cameras.findIndex((E) => E.id === O) <= 0, run: () => this.moveShot(O, -1) },
            { label: s("Shot: move later"), icon: "pi-arrow-down", disabled: this.state.cameras.findIndex((E) => E.id === O) >= this.state.cameras.length - 1, run: () => this.moveShot(O, 1) },
            null,
            { label: s("Shot handles…"), icon: "pi-sliders-h", run: () => this.editShotHandles(O) }
          ]
        },
        null,
        { label: s("Rename camera…"), icon: "pi-pencil", run: () => this.renameCamera(O) },
        { label: s("Duplicate camera"), icon: "pi-copy", run: () => this.duplicateCamera(O) },
        { label: s("Create camera from current view"), icon: "pi-plus", run: () => this.addCamera() },
        null,
        { label: s("Reset entire animation"), icon: "pi-replay", danger: !0, help: s("Delete every camera key and return to a static zero pose at frame 0"), run: () => this.resetCameraAnimation(O) },
        null,
        { label: s("Delete camera"), icon: "pi-trash", danger: !0, disabled: this.state.cameras.length <= 1, run: () => this.deleteCamera(O) }
      ]));
    },
    openPathKeyContext(F, O, S) {
      const w = this.state.cameras.find((Q) => Q.id === O);
      if (!w) return;
      this.selectedEntity = "camera", this.selectedObjectId = null, this.activateCamera(O);
      const E = (w.keyframes || []).find((Q) => Q.frame === S) || null;
      E && this.selectKeyframe(E), this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render();
      const M = E ? ft(E) : "auto", U = this.selectedKeyFrames?.size || 0, Z = this.pathSelection?.component === "target", B = Pr(w);
      this.showContextMenu(F, `Path key F${S}`, [
    ...
    moveShot(F, O) {
      const S = this.state.cameras.findIndex((M) => M.id === F), w = S + O;
    ...
    async editShotHandles(F) {
      const O = this.state.cameras.find((M) => M.id === F);
      if (!O) return;
      const S = O.handles || { in: 0, out: 0 }, w = await i(t, s("Shot handles…"), "Handle frames: in,out", `${S.in},${S.out}`);
      if (w == null) return;
      const E = String(w).match(/^\s*(\d+)\s*[,;\s]\s*(\d+)\s*$/);
      if (!E) return this.setStatus("Handles must be two integers: in,out");
      this.checkpoint("Shot handles"), O.handles = { in: Math.min(600, Number(E[1])), out: Math.min(600, Number(E[2])) }, this.serialize(), this.setStatus(`${O.name} handles: ${O.handles.in} / ${O.handles.out}`);
    },
    openTimelineContext(F, O) {
      const S = this.selectedKeyFrames?.size || 0, w = this.selectedKeyframe(), E = w?.interpolation || "ease", M = w && ft(w) || "auto", U = ["ease", "linear", "bezier", "smooth", "ease_in", "ease_out", "sine", "cubic", "quintic", "expo", "back"], Z = ["auto", "clamped", "vector", "free", "aligned", "flat"];
      this.showContextMenu(F, O ? `Keyframe F${this.selectedKeyFrame}` : `Timeline F${this.frame}`, [
    ...
    addMarker() {
      if ((this.state.markers || []).find((O) => O.frame === this.frame)) return this.setStatus(`Marker already at F${this.frame}`);
      this.checkpoint("Add marker"), this.state.markers = [...this.state.markers || [], { frame: this.frame, name: `Marker ${(this.state.markers || []).length + 1}`, color: "#f2d06b" }].sort((O, S) => O.frame - S.frame), this.serialize(), this.refreshKeys(), this.setStatus(`Marker @ F${this.frame}`);
    },
    removeNearestMarker() {
      const F = this.state.markers || [];
      if (!F.length) return;
      const O = F.reduce((S, w) => Math.abs(w.frame - this.frame) < Math.abs(S.frame - this.frame) ? w : S);
      this.checkpoint("Remove marker"), this.state.markers = F.filter((S) => S !== O), this.serialize(), this.refreshKeys(), this.setStatus(`Marker removed @ F${O.frame}`);
    },
    openCurveContext(F) {
      const O = this.selectedKeyFrames?.size || 0, S = O < 2 && !this.selectedKeyframe(), w = this.selectedKeyframe(), E = w?.interpolation || "ease", M = w && ft(w) || "auto", U = ["bezier", "smooth", "linear", "ease_in", "ease_out", "ease", "sine", "cubic", "quintic", "expo", "back"], Z = ["auto", "clamped", "vector", "free", "aligned", "flat"];
      this.showContextMenu(F, s("Curve editor"), [
        ...
        null,
        { label: O >= 2 ? s("Delete {count} keys").replace("{count}", String(O)) : s("Delete selected key"), icon: "pi-trash", danger: !0, disabled: S, run: () => this.deleteSelectedKeyframes() }
      ]);
    },
    ...
    refitNode() {
      ...
      const F = this.node;
      try {
        if (F && typeof F.computeSize == "function" && typeof F.setSize == "function") {
          const O = F.computeSize();
          Array.isArray(O) && F.setSize([F.size?.[0] ?? O[0], O[1]]);
        }
        F?.graph?.setDirtyCanvas?.(!0, !0);
      } catch {
      }
      this.scheduleResizeAndRender();
    },
    resizeCanvas() {
      const F = this.root.querySelector(".viewport-wrap");
      if (!F) return;
      const O = Math.min(2, window.devicePixelRatio || 1), S = F.clientWidth || 320, w = F.clientHeight || 180, E = Math.max(320, Math.round(S * O)), M = Math.max(180, Math.round(w * O));
      ...
      for (const U of this.cameraPreviewCanvases.values()) {
        const Z = U.clientWidth || 220, B = U.clientHeight || 124, Q = Math.max(O, Og / Math.max(1, Z)), _e = Math.max(1, Math.round(Z * Q)), Ue = Math.max(1, Math.round(B * Q));
        (U.width !== _e || U.height !== Ue) && (U.width = _e, U.height = Ue);
      }
...
function Ug(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: l, dispatchDirectorKey: p, activeCameraTrack: m, bindWidgetCallbacks: f, playblastCameraTrack: d, restoreFromWidgets: h, serializeEditorState: u, syncActiveCameraTrack: b, syncFromWidgets: v, bindEditorEvents: y, activateCamera: x, addCamera: k, deleteCamera: g, drawPreviewOverlays: j, duplicateCamera: T, maximizeCameraPreview: K, refreshCameraPreviews: z, refreshCameraSelectors: J, renameCamera: H, setPlayblastCamera: $, toggleCameraView: P, captureRealtime: _, makePlayblast: R, uploadDirectorPlayblast: q, waitForMediaFrame: G, computeAudioPeaks: L, loadAudioFile: C, releaseAudio: Y, stopPlay: D, togglePlay: X, applyCameraPreset: ae, applyCameraShake: le, applyProxyPreset: se, clearViewportBgImage: re, loadViewportBgFile: de, loadViewportBgSequence: ye, drawCameraPath: he, drawCard: ve, drawCube: xe, drawGrid: pe, drawHuman: Ie, drawLine3D: we, drawNull: ke, drawOverlays: be, drawPointField: N, drawSpeedHeatmap: V, drawSphere: me, curveChannels: Ee, drawCurveEditor: Le, onCurvePointerDown: qe, onCurvePointerMove: Ne, onCurvePointerUp: it, onTimelinePointerDown: oa, onTimelinePointerMove: ra, onTimelinePointerUp: na, refreshKeys: $t, resetCurveZoom: It, resetTimelineZoom: bt, setChannelFilter: Ot, setCurveInterpolation: Pt, setTangentMode: Lt, timelineFrameFromEvent: sa, toggleCurveHandles: Nt, zoomCurve: ia, drawTransformGizmo: ca, frameTarget: la, gizmoAxes: da, gizmoGeometry: ma, onPointerDown: pa, onPointerMove: fa, onPointerUp: ha, onWheel: ua, pickGizmo: ba, pickSceneObject: ga, resetCamera: Ft, setTransformMode: zt, setViewMode: ya, viewportCamera: va, loadCardFile: Go, loadExecutionPreview: Yo, loadMediaUrl: xa, loadModelFile: wa, loadSelectedReference: ka, onModelLoaded: Sa, restoreAssets: _a, syncUpstreamInputs: ja, configureDomMedia: Ea, refreshSetupDiagnostic: Rt, addMediaCard: Ca, addPrimitive: Aa, applyObjectAnimationFrame: Ta, beginCameraEdit: Ma, beginObjectEdit: $a, commitCameraEdit: Ia, commitObjectEdit: Oa, copyKeyframe: Pa, deleteKeyframe: La, deleteObject: Na, deleteSelectedObjects: Fa, duplicateObject: za, exitKeyEdit: Ra, finishCameraEdit: Da, goToAdjacentKey: Ka, insertKeyframe: Ba, loadSelectedKeyView: qa, pasteKeyframe: Ua, playblastCameraAtFrame: Wa, refreshInspector: Va, refreshKeyEditor: Ha, refreshObjects: Ga, removeObjectResources: Ya, renameObject: Xa, retimeSelectedKey: Ja, selectKeyframe: Za, selectedKeyframe: Qa, selectedObject: eo, selectObjectAnimation: to, setKeyInterpolation: ao, setKeyTangentMode: oo, setObjectParent: ct, timelineKeyframes: ro, timelineObject: no, toggleAutoKey: so, toggleObject: io, updateCameraFromHud: co, updateCameraRotationFromHud: lo, updateEditState: mo, updateKeyVisualState: Dt, updateSelectedKey: po, updateSelectedObject: fo, clamp: ho, cloneCamera: Ze, configureCore: Xo, defaultCamera: gt, sampleCamera: Fe, sampleObjectTransform: F, sanitizeState: O, worldTransform: S } = e;
  return {
    setChannelFilter(w) {
      Ot(this, w);
    },
    setFrame(w, E = !1, M = !0) {
      ...
      const U = this.dom ||= oi(this.root);
      for (const oe of U.frames) document.activeElement !== oe && (oe.value = String(this.frame));
      for (const oe of U.scrubs) oe.value = String(this.frame);
      for (const oe of U.cameraFov) document.activeElement !== oe && (oe.value = String(Math.round(this.camera.fov * 100) / 100));
      for (const oe of U.cameraRoll) document.activeElement !== oe && (oe.value = String(Math.round((this.camera.roll || 0) * 100) / 100));
      for (const oe of U.cameraFocal) document.activeElement !== oe && (oe.value = Kr(this.camera.fov));
      for (const oe of U.viewportZoom) oe.textContent = `${(Number(this.camera.zoom) || 1).toFixed(2)}x`;
      for (const oe of U.cameraType) document.activeElement !== oe && (oe.value = this.camera.camera_type || "perspective");
      for (const oe of U.cameraNear) document.activeElement !== oe && (oe.value = String(this.camera.near ?? 0.01));
      for (const oe of U.cameraFar) document.activeElement !== oe && (oe.value = String(this.camera.far ?? 1e4));
      const Z = this.frame / this.state.fps;
      for (const oe of this.cardMediaById.values()) oe instanceof HTMLVideoElement && Number.isFinite(oe.duration) && oe.duration > 0 && (oe.currentTime = Z % oe.duration);
      const B = Math.floor(Z / 60), Q = Math.floor(Z % 60), _e = Math.floor(Z % 1 * 1e3), Ue = this.frame % Math.max(1, Math.round(this.state.fps)), lt = Math.floor(this.frame / this.state.fps);
      if ((U.time || this.root.querySelector('[data-role="time"]')).textContent = this.state.timecode_mode === "timecode" ? `${String(Math.floor(lt / 3600)).padStart(2, "0")}:${String(Math.floor(lt / 60) % 60).padStart(2, "0")}:${String(lt % 60).padStart(2, "0")}:${String(Ue).padStart(2, "0")}` : `${String(B).padStart(2, "0")}:${String(Q).padStart(2, "0")}.${String(_e).padStart(3, "0")}`, M) this.refreshKeys();
      else {
        Oo(this);
        for (const oe of this.root.querySelectorAll("[data-key-frame]")) {
          const ne = Number(oe.dataset.keyFrame);
          oe.classList.toggle("at-playhead", ne === this.frame), oe.classList.toggle("selected", ne === this.selectedKeyFrame), oe.classList.toggle("editing", ne === this.editingKeyFrame);
        }
    ...
    setSpatialHandleMode(w) {
      ...
      const M = this.activeCameraTrack()?.keyframes || [], U = M.findIndex((Z) => Z.frame === this.selectedKeyFrame);
      if (U < 0) {
        this.setStatus(s("Select a camera keyframe first"));
        return;
      }
      this.checkpoint(s("Camera path handle: {mode}").replace("{mode}", w)), Np(M[U], w, {
        prevKey: M[U - 1] || null,
        nextKey: M[U + 1] || null
    ...
    transformCameraPath(w) {
      ...
      const M = ws(E.keyframes, { origin: Or(E.keyframes), ...w });
      return E.keyframes = M, E.id === this.state.active_camera_id && (this.state.keyframes = M), this.camera = Fe(E, this.frame, this.state.objects), E.camera = Ze(this.camera), this.serialize(), this.refreshKeys(), this.refreshInspector(), this.render(), this.renderCameraView?.(), !0;
    },
    ...
    async openCameraPathPresetPicker() {
      ...
      const E = Po.map((U) => ({ id: U, label: s(_n[U] || U) })), M = await Es({ title: s("Camera Path Preset"), items: E, owner: this });
      return M ? this.applyCameraPathPreset(M) : !1;
    },
    applyCameraPathPreset(w, E = {}) {
      const M = this.activeCameraTrack();
      if (!M || M.locked)
        return this.setStatus(s("{name} is locked").replace("{name}", M?.name || s("Camera"))), !1;
      const [U, Z] = Ar(this.state), B = Hs({ type: w, camera: this.camera, startFrame: U, endFrame: Z, params: E });
      return B.ok ? (this.checkpoint(s("Apply camera path preset")), M.keyframes = B.keyframes, M.id === this.state.active_camera_id && (this.state.keyframes = B.keyframes), this.camera = Fe(M, this.frame, this.state.objects), M.camera = Ze(this.camera), this.serialize(), this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render(), this.setStatus(s("{preset} camera path generated").replace("{preset}", s(_n[w] || w))), !0) : (this.setStatus(B.reason === "insufficient_frame_slots" ? s("Not enough frames in the playback range for this preset") : s("Could not generate that camera path preset")), !1);
    },
    toggleCurveHandles() {
      Nt(this);
    },
    ...
    toggleInspector(w) {
      ...
      for (const U of this.root.querySelectorAll('[data-act="toggle-inspector"]'))
        U.classList.toggle("active", !M), U.setAttribute("aria-pressed", String(!M));
    ...
    selectHierarchy(w = this.selectedObjectId) {
      ...
      for (; M; ) {
        ...
        for (const U of this.state.objects)
          U.parent_id && E.has(U.parent_id) && !E.has(U.id) && (E.add(U.id), M = !0);
      }
    ...
    lookAtObject(w) {
      ...
      if (E) {
        ...
        for (const M of this.state.cameras)
          for (const U of M.keyframes) U.camera.target = [...E.position || [0, 1.5, 0]];
        this.camera = Fe(this.state, this.frame), this.serialize(), this.refreshKeys(), this.render(), this.setStatus(`Cameras look at ${E.name || E.type}`);
      }
    },
    setTransformMode(w) {
      zt(this, w);
    ...
    aimAtSelectedObject(w) {
      ...
      const Z = (M.type === "model" || M.type === "glb" ? this.webgl?.getObjectWorldCenter?.(M.id) : null) || (M.keyframes?.length ? F(M, this.frame).position : M.position || [0, 1.5, 0]);
    ...
    setCameraTrackingTarget(w) {
      ...
      const E = this.activeCameraTrack();
      E.target_object_id !== (w || null) && (E.aim_bone = null), E.target_object_id = w || null, E.id === this.state.active_camera_id && (this.state.target_object_id = w || null, this.state.aim_bone = E.aim_bone), this.camera = Fe(E, this.frame, this.state.objects), Yt(this, E, this.camera, this.frame), this.serialize(), this.refreshInspector(), this.render(), this.setStatus(w ? `Camera tracking: ${w}` : "Camera tracking disabled (manual target)");
    },
    bakeAimToKeyframes() {
      ...
      const U = M.type === "model" || M.type === "glb" ? this.webgl?.getObjectWorldCenter?.(M.id) : null;
      for (const Z of w.keyframes) {
        const B = (M.type === "model" || M.type === "glb") && U && !M.keyframes?.length ? U : M.keyframes?.length ? F(M, Z.frame).position : M.position || [0, 1.5, 0];
        Z.camera.target = [...B];
      }
...
function Wg(e) {
  ...
  return {
    ...
    async loadMediaUrl(S, w, E, M) {
      return ka(this, S, w, E, M);
    },
    restoreAssets() {
      Ea(this);
    },
...
function ly(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: l, dispatchDirectorKey: p, activeCameraTrack: m, bindWidgetCallbacks: f, playblastCameraTrack: d, restoreFromWidgets: h, serializeEditorState: u, syncActiveCameraTrack: b, syncFromWidgets: v, bindEditorEvents: y, activateCamera: x, addCamera: k, deleteCamera: g, drawPreviewOverlays: j, duplicateCamera: T, maximizeCameraPreview: K, refreshCameraPreviews: z, refreshCameraSelectors: J, renameCamera: H, setPlayblastCamera: $, toggleCameraView: P, captureRealtime: _, makePlayblast: R, uploadDirectorPlayblast: q, waitForMediaFrame: G, computeAudioPeaks: L, loadAudioFile: C, releaseAudio: Y, stopPlay: D, togglePlay: X, applyCameraPreset: ae, applyCameraShake: le, applyProxyPreset: se, clearViewportBgImage: re, loadViewportBgFile: de, loadViewportBgSequence: ye, drawCameraPath: he, drawCard: ve, drawCube: xe, drawCylinder: pe, drawGrid: Ie, drawHuman: we, drawLine3D: ke, drawNull: be, drawOverlays: N, drawPointField: V, drawSpeedHeatmap: me, drawSphere: Ee, drawTorus: Le, curveChannels: qe, drawCurveEditor: Ne, onCurvePointerDown: it, onCurvePointerMove: oa, onCurvePointerUp: ra, onTimelinePointerDown: na, onTimelinePointerMove: $t, onTimelinePointerUp: It, refreshKeys: bt, resetCurveZoom: Ot, resetTimelineZoom: Pt, setChannelFilter: Lt, setCurveInterpolation: sa, setTangentMode: Nt, timelineFrameFromEvent: ia, toggleCurveHandles: ca, zoomCurve: la, drawTransformGizmo: da, frameTarget: ma, gizmoAxes: pa, gizmoGeometry: fa, onPointerDown: ha, onPointerMove: ua, onPointerUp: ba, onWheel: ga, pickGizmo: Ft, pickSceneObject: zt, resetCamera: ya, setTransformMode: va, setViewMode: Go, viewportCamera: Yo, loadCardFile: xa, loadExecutionPreview: wa, loadMediaUrl: ka, loadModelFile: Sa, loadSelectedReference: _a, onModelLoaded: ja, restoreAssets: Ea, syncUpstreamInputs: Rt, configureDomMedia: Ca, refreshSetupDiagnostic: Aa, addMediaCard: Ta, addPrimitive: Ma, applyObjectAnimationFrame: $a, beginCameraEdit: Ia, beginObjectEdit: Oa, commitCameraEdit: Pa, commitObjectEdit: La, copyKeyframe: Na, deleteKeyframe: Fa, deleteObject: za, duplicateObject: Ra, exitKeyEdit: Da, finishCameraEdit: Ka, goToAdjacentKey: Ba, insertKeyframe: qa, loadSelectedKeyView: Ua, pasteKeyframe: Wa, playblastCameraAtFrame: Va, refreshInspector: Ha, refreshKeyEditor: Ga, refreshObjects: Ya, removeObjectResources: Xa, renameObject: Ja, retimeSelectedKey: Za, selectKeyframe: Qa, selectedKeyframe: eo, selectedObject: to, selectObjectAnimation: ao, setKeyInterpolation: oo, setObjectParent: ct, timelineKeyframes: ro, timelineObject: no, toggleAutoKey: so, toggleObject: io, updateCameraFromHud: co, updateEditState: lo, updateKeyVisualState: mo, updateSelectedKey: Dt, updateSelectedObject: po, clamp: fo, cloneCamera: ho, configureCore: Ze, defaultCamera: Xo, sampleCamera: gt, sampleObjectTransform: Fe, sanitizeState: F, worldTransform: O } = e;
  return {
    ...
    renderViewportOnly() {
      ...
      const M = this.state.render_mode, U = this.viewportCamera(), Z = this.state.objects.some((ne) => ne.parent_id) ? this.state.objects.map((ne) => ne.parent_id ? { ...ne, ...O(this.state.objects, ne) } : ne) : this.state.objects, B = (this.viewportBgSequenceImages || []).map((ne) => ne.src), Q = this.viewportBgImage?.src || "", _e = this.pendingExtractorImport, Ue = _e ? [...this.state.cameras, {
      ...
      if (this.webgl) {
        try {
          const ne = this.recording ? 1 : this.webgl.supersampleFactor?.() ?? 1, Qe = ne > 1 ? Math.min(ne, 4096 / Math.max(1, w, E)) : 1, uo = Qe > 1 ? Math.round(w * Qe) : w, Jo = Qe > 1 ? Math.round(E * Qe) : E;
          this.webgl.render(lt, U, this.cardMediaById, uo, Jo, this.modelUrlsById, this.frame, this.recording, this.selectedEntity, this.selectedObjectId, this.subSelection, this.selectedKeyFrame ?? null, this.selectedKeyFrames ? [...this.selectedKeyFrames] : null), S.imageSmoothingEnabled = !0, S.imageSmoothingQuality = "high", uo !== w || Jo !== E ? S.drawImage(this.webgl.canvas, 0, 0, uo, Jo, 0, 0, w, E) : S.drawImage(this.webgl.canvas, 0, 0, w, E), oe = !0;
        } catch (ne) {
      ...
      if (!oe) {
        ...
        for (const ne of Z)
          ne.enabled !== !1 && (ne.type === "card" && ["omni_ref", "card_grid", "graybox", "wireframe"].includes(M) ? this.drawCard(ne) : ["cube", "ground", "glb", "model"].includes(ne.type) && M !== "grid" && M !== "point_field" ? this.drawCube(ne) : ne.type === "sphere" && M !== "grid" && M !== "point_field" ? this.drawSphere(ne) : ne.type === "cylinder" && M !== "grid" && M !== "point_field" ? this.drawCylinder(ne) : ne.type === "torus" && M !== "grid" && M !== "point_field" ? this.drawTorus(ne) : ne.type === "human" && M !== "grid" && M !== "point_field" ? this.drawHuman(ne) : ne.type === "null" && this.drawNull(ne));
    ...
    requestUiUpdate(S = A.viewport, w = "unknown") {
      (this.renderReasons ||= /* @__PURE__ */ new Set()).add(w), this.uiDirtyMask = Ep(this.uiDirtyMask, S), this.renderInvalidations = (this.renderInvalidations || 0) + 1, !this.renderScheduled && (this.renderScheduled = !0, this.renderFrame = requestAnimationFrame(() => {
        if (this.renderScheduled = !1, this.disposed) return;
        const E = this.uiDirtyMask || A.viewport;
        this.uiDirtyMask = 0, this.lastRenderReasons = [...this.renderReasons || []], this.renderReasons?.clear(), this.rendersCoalesced = (this.rendersCoalesced || 0) + 1, this.perf && (this.perf.renderCount = (this.perf.renderCount || 0) + 1), xt(E, A.outliner) && this.refreshObjects(), xt(E, A.timeline) && this.refreshKeys(), xt(E, A.inspector) && this.refreshInspector(), xt(E, A.viewport) && this.renderViewportOnly(), xt(E, A.previews) && this.renderCameraView(), xt(E, A.motion) && this.renderMotionUiOnly();
      }));
    },
    renderCameraView() {
      if (this.perf && (this.perf.previewRenderCount = (this.perf.previewRenderCount || 0) + 1), this.state.camera_view_visible) {
        ...
        for (const U of w) {
          ...
          const Q = Z.width, _e = Z.height, Ue = this.root.querySelector(`[data-camera-frame="${U.id}"]`);
          if (Ue && (Ue.textContent = `F${this.frame}`), E && U.id !== this.state.active_camera_id && U !== M) continue;
          const lt = Yt(this, U, gt(U, this.frame, this.state.objects), this.frame);
          if (B.fillStyle = "#111", B.fillRect(0, 0, Q, _e), this.cameraWebgl)
            try {
              this.cameraWebgl.render({ ...this.state, keyframes: [], playblast_grid: !1, viewport_bg_image: this.viewportBgImage?.src || "", viewport_bg_sequence: (this.viewportBgSequenceImages || []).map((oe) => oe.src), __omnicamRevision: this.renderRevision || 0 }, lt, this.cardMediaById, Q, _e, this.modelUrlsById, this.frame, !0), B.drawImage(this.cameraWebgl.canvas, 0, 0, Q, _e);
            } catch (oe) {
              console.error("[OmniCam Preview Render Error]", oe);
            }
          j(this, B, Q, _e);
        }
      }
  ...
  return {
    ...
    simplifyActiveKeys({ mode: e = "simplify", tolerance: t = 0, target: a = 0, scope: o = "camera", fromKeys: r = null, silent: n = !1 } = {}) {
      ...
      const l = this.resolveSelectedFrames(), p = o !== "all_cameras" && l.length >= 2 ? [l[0], l.at(-1)] : null, m = p ? [] : l, f = (u) => {
        const b = [...u].sort((K, z) => K.frame - z.frame), v = p ? p[0] : -1 / 0, y = p ? p[1] : 1 / 0, x = b.filter((K) => K.frame < v), k = b.filter((K) => K.frame >= v && K.frame <= y), g = b.filter((K) => K.frame > y);
        let j = k, T = 0;
        if (k.length > 2) {
          const K = e === "reduce" ? Kp(k, i, { target: a || Math.ceil(k.length / 2), keepFrames: m }) : e === "clean" ? Bp(k, i, { keepFrames: m }) : Dp(k, i, { tolerance: t * dy, keepFrames: m });
          j = K.keys, T = K.removed;
        }
        return { keys: [...x, ...j, ...g].sort((K, z) => K.frame - z.frame), removed: T };
      };
      let d = 0;
      const h = c.map((u) => {
...
class Li {
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-BEUwwIUD.js"));
    } catch (a) {
  ...
  try {
    t.assetBrowser = Ff(t, {
      ...
      onAgentFirstOpen: async () => {
        if (mr())
          try {
            const { createDirectorAgentPanel: m } = await import("./chunk-C81k1OF3.js");
            t.agentPanel = m(t);
          } catch (m) {
