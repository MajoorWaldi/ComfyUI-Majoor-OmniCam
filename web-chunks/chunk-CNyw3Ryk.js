  ...
  e.registerExtension({
    ...
    beforeRegisterNodeDef(t, a) {
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        const i = Math.max(4, Number(this.size?.[0] || 160) - Q - 6), l = -26, m = i + Q / 2, c = l + Q / 2;
        if (n.save(), this.selected) {
          const p = ro(), f = n.createRadialGradient(m, c, Q * 0.35, m, c, Q * 1.15);
          f.addColorStop(0, `rgba(136, 115, 253, ${p})`), f.addColorStop(1, "rgba(136, 115, 253, 0)"), n.fillStyle = f, n.beginPath(), n.arc(m, c, Q * 1.15, 0, Math.PI * 2), n.fill();
        }
        n.globalAlpha = 0.96, n.drawImage(s, i, l, Q, Q), n.restore();
      };
    }
  });
}
...
function di(e) {
  return !!e?.sequence?.enabled && Ne(e).length > 0;
}
function la(e, t) {
  const a = Ne(e);
...
function gi(e, { mode: t, selectedIds: a } = {}) {
...
const Nt = Object.freeze([0.05, 8]), Ao = 80, pa = (e, t, a) => Math.max(t, Math.min(a, e));
function fa(e) {
  if (!e || typeof e != "object") return null;
  const t = String(e.clip_id ?? "").trim();
  if (!t || t.length > Ao) return null;
  const a = Math.max(0, Math.round(Number(e.start_frame) || 0)), r = Math.round(Number(e.end_frame) || 0), o = r > a ? r : 0, n = Number(e.speed), s = pa(Number.isFinite(n) && n > 0 ? n : 1, Nt[0], Nt[1]), i = Number(e.offset_seconds);
  return {
    clip_id: t,
    start_frame: a,
    end_frame: o,
...
function bi(e, t, a, r) {
  ...
  if (!o || n <= 0) return 0;
  const i = (p) => o.loop ? (p % n + n) % n : pa(p, 0, n);
...
const _i = "omnicam_humanoid_v1", ht = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function vi(e) {
  const a = (e || []).map(String).filter((n) => n.trim()).map((n) => [n, No(Po(n))]), r = /* @__PURE__ */ new Set(), o = {};
  for (const n of [!0, !1])
...
const Fo = 128, zt = "omnicam_humanoid_v1", gt = /^[a-z0-9][a-z0-9_-]*$/, Ko = 1e-3, Bo = (e) => Array.isArray(e) && e.length === 3 && e.every((t) => Number.isFinite(Number(t)));
function yt(e) {
...
function wi({ preset: e, overrides: t } = {}) {
  const a = ze(e), r = ze(t);
  return {
...
function Ci(e, { startFrame: t, endFrame: a } = {}) {
  ...
  const n = [...e].sort((d, u) => d.frame - u.frame);
  if (Math.floor(o) - Math.floor(r) + 1 < n.length)
    return { ok: !1, reason: "insufficient_frame_slots" };
  const i = n.map((d) => qo(d)), l = [];
  for (let d = 1; d < n.length; d += 1) {
    const u = Ho(Rt(n[d - 1]), Rt(n[d])), w = (i[d - 1] + i[d]) / 2;
    l.push(u * w);
  }
  const m = l.reduce((d, u) => d + u, 0), c = o - r, p = [0];
  if (m > 0) {
    let d = 0;
    for (const u of l)
      d += u, p.push(d / m);
  } else
    for (let d = 1; d < n.length; d += 1) p.push(d / (n.length - 1));
  const f = p.map((d) => Math.round(r + d * c));
  f[0] = r, f[f.length - 1] = o;
  for (let d = 1; d < f.length; d += 1)
    f[d] <= f[d - 1] && (f[d] = f[d - 1] + 1);
  for (let d = f.length - 1; d > 0; d -= 1)
    f[d] > o - (f.length - 1 - d) && (f[d] = o - (f.length - 1 - d));
  return f[f.length - 1] = o, f[0] = r, { ok: !0, keys: n.map((d, u) => ({ ...d, frame: f[u] })) };
}
function Wo(e) {
...
function q(e) {
  const t = A(e.target, e.position), a = Math.sqrt(Y(t, t)) < 1e-6 ? [0, 0, -1] : se(t);
  let r = e.up || [0, 1, 0], o = Me(a, r);
  Math.sqrt(Y(o, o)) < 1e-6 && (r = Math.abs(a[1]) > 0.9 ? [0, 0, a[1] > 0 ? -1 : 1] : [0, 1, 0], o = Me(a, r)), o = se(o);
  let n = se(Me(o, a));
  if (Math.abs(e.roll || 0) > 1e-9) {
    const s = e.roll * Math.PI / 180, i = Math.cos(s), l = Math.sin(s), m = x(D(o, i), D(n, l));
    n = x(D(n, i), D(o, -l)), o = m;
  }
  return { right: o, up: n, forward: a };
}
function Ti(e) {
  const t = A(e.target, e.position), a = H(t), r = a < 1e-6 ? [0, 0, -1] : D(t, 1 / a), o = Math.asin(T(r[1], -1, 1)) * 180 / Math.PI, n = Math.atan2(r[0], -r[2]) * 180 / Math.PI;
  return [o, n, e.roll || 0];
}
function Ai(e, t) {
  const [a, r, o] = t, n = Math.max(1e-4, H(A(e.target, e.position))), s = a * Math.PI / 180, i = r * Math.PI / 180, l = [Math.sin(i) * Math.cos(s), Math.sin(s), -Math.cos(i) * Math.cos(s)];
  e.target = x(e.position, D(l, n)), e.roll = o;
}
function B(e, t, a, r) {
  const { right: o, up: n, forward: s } = q(t), i = A(e, t.position), l = Y(i, s);
  if (l <= Math.max(1e-4, t.near || 0.01) || l >= (t.far || 1e4)) return null;
  const m = Y(i, o), c = Y(i, n);
  if (t.camera_type === "orthographic") {
    const f = 5 / Math.max(0.01, t.zoom || 1), h = f * a / Math.max(1, r);
    return [a * (0.5 + m / (2 * h)), r * (0.5 - c / (2 * f)), l];
  }
  const p = 0.5 * r / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
  return [a * 0.5 + m * p / l, r * 0.5 - c * p / l, l];
}
function ce(e, t, a = null) {
  const r = (e.keyframes || []).map((g) => ({
    ...g,
    camera: G(g.camera || g || e.camera || ie())
  }));
  if (!r.length) return G(e.camera || ie());
  const o = Uo(e, r, t), n = z(r, t, "pos_x", (g) => (g.camera || g).position[0], !1, o), s = z(r, t, "pos_y", (g) => (g.camera || g).position[1], !1, o), i = z(r, t, "pos_z", (g) => (g.camera || g).position[2], !1, o);
  ...
  const p = e.constraints?.look_at, h = p?.status === void 0 || p?.status === "active" ? p?.object_id || e.target_object_id || e.camera?.target_object_id : null, d = a || e.objects;
  if (h && Array.isArray(d)) {
    const g = d.find((C) => C.id === h);
    if (g && g.enabled !== !1) {
      const C = vt(d, g, t), j = p?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
      l = (C.position?.[0] ?? 0) + (j[0] || 0), m = (C.position?.[1] ?? 1.5) + (j[1] || 0), c = (C.position?.[2] ?? 0) + (j[2] || 0);
    }
  }
  const u = z(r, t, "fov", (g) => Number((g.camera || g).fov ?? 35), !1, o), w = z(r, t, "roll", (g) => Number((g.camera || g).roll ?? 0), !0, o), b = z(r, t, "zoom", (g) => Number((g.camera || g).zoom ?? 1), !1, o), S = z(r, t, "near", (g) => Number((g.camera || g).near ?? 0.01), !1, o), y = z(r, t, "far", (g) => Number((g.camera || g).far ?? 1e4), !1, o), M = r[0]?.camera || r[0] || ie();
  let v = r[0];
  for (const g of r)
    if ((g.frame ?? 0) <= t) v = g;
    else break;
  const k = (v.camera || v).camera_type;
  return {
    position: [n, s, i],
    target: [l, m, c],
    fov: T(u, 5, 150),
...
const T = (e, t, a) => Math.max(t, Math.min(a, e)), Yo = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, fe = (e, t = null) => typeof e == "string" && Yo.test(e.trim()) ? e.trim() : t, x = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], A = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], D = (e, t) => [e[0] * t, e[1] * t, e[2] * t], Y = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], Me = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], H = (e) => Math.sqrt(Math.max(1e-12, Y(e, e))), se = (e) => D(e, 1 / H(e));
function _a(e, t, a) {
...
function Ft(e, t, a, r, o) {
  const n = Jo(e, t), s = Qo.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", i = o ? o(e) : 0, l = a && o ? o(a) : i, m = r && o ? o(r) : i, c = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), p = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), f = (S = !1) => {
    const y = (i - l) / c, M = (m - i) / p;
    let v = 0;
    if (!a && !r)
      v = 0;
    else if (!a)
      ...
      const k = p / (c + p), g = c / (c + p);
      if (v = y * k + M * g, S) {
        const C = 3 * Math.min(Math.abs(y), Math.abs(M));
        v = T(v, -C, C);
      }
    }
    return {
  ...
  if (s === "auto")
    return { ...f(!1), mode: s };
  const h = f(!1), d = T(Number(n.out_x ?? h.out_x), 0.01, 0.99), u = Number(n.out_y ?? h.out_y);
  let w = T(Number(n.in_x ?? h.in_x), -0.99, -0.01), b = Number(n.in_y ?? h.in_y);
  if (s === "aligned") {
    const S = Math.hypot(d, u) || 1e-6, y = Math.hypot(w, b) || 1e-6;
    w = -d / S * y, b = -u / S * y;
  }
  return { out_x: d, out_y: u, in_x: w, in_y: b, mode: s };
}
function _t(e, t) {
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  const s = n || _t(e, t), { leftIndex: i, left: l, right: m } = s, c = i > 0 ? e[i - 1] : null, p = i + 2 < e.length ? e[i + 2] : null, f = Math.max(1, m.frame - l.frame), h = T((t - l.frame) / f, 0, 1);
  let d = r(l), u = r(m);
  if (o) {
    const S = ((u - d + 540) % 360 + 360) % 360 - 180;
    u = d + S;
  }
  if (l.interpolation === "bezier" || m.interpolation === "bezier") {
    const S = Ft(l, a, c, m, r), y = Ft(m, a, l, p, r), M = d, v = d + (S.out_y || 0), k = u + (y.in_y || 0), g = u, C = T(Number(S.out_x ?? 1 / 3), 0, 1), j = T(1 + Number(y.in_x ?? -1 / 3), 0, 1);
    let V = 0, L = 1;
    for (let Et = 0; Et < 32; Et++) {
      const X = (V + L) * 0.5, Ye = 1 - X;
      3 * Ye * Ye * X * C + 3 * Ye * X * X * j + X * X * X < h ? V = X : L = X;
    }
    const U = (V + L) * 0.5, ae = 1 - U;
    return ae * ae * ae * M + 3 * ae * ae * U * v + 3 * ae * U * U * k + U * U * U * g;
  }
  const b = Zo(h, l.interpolation);
  return d + (u - d) * b;
}
...
function $e(e, t) {
  ...
  const r = me(e), o = (b, S) => (b.transform?.position || r.position)[S] ?? 0, n = (b, S) => (b.transform?.rotation || r.rotation)[S] ?? 0, s = (b, S) => (b.transform?.size || r.size)[S] ?? (S === 2 ? 0.01 : 1), i = _t(a, t), l = z(a, t, "pos_x", (b) => o(b, 0), !1, i), m = z(a, t, "pos_y", (b) => o(b, 1), !1, i), c = z(a, t, "pos_z", (b) => o(b, 2), !1, i), p = z(a, t, "rot_x", (b) => n(b, 0), !0, i), f = z(a, t, "rot_y", (b) => n(b, 1), !0, i), h = z(a, t, "rot_z", (b) => n(b, 2), !0, i), d = z(a, t, "scale_x", (b) => s(b, 0), !1, i), u = z(a, t, "scale_y", (b) => s(b, 1), !1, i), w = z(a, t, "scale_z", (b) => s(b, 2), !1, i);
  return {
    position: [Number.isFinite(l) ? l : r.position[0], Number.isFinite(m) ? m : r.position[1], Number.isFinite(c) ? c : r.position[2]],
    rotation: [Number.isFinite(p) ? p : r.rotation[0], Number.isFinite(f) ? f : r.rotation[1], Number.isFinite(h) ? h : r.rotation[2]],
    size: [
      Math.max(0.01, Number.isFinite(d) ? d : r.size[0]),
      Math.max(0.01, Number.isFinite(u) ? u : r.size[1]),
      Math.max(0.01, Number.isFinite(w) ? w : r.size[2])
    ]
  };
}
function ji(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  if (typeof a == "string" && a.startsWith("#")) {
    const f = a.replace("#", "");
    f.length === 6 && (i = parseInt(f.slice(0, 2), 16) / 255, l = parseInt(f.slice(2, 4), 16) / 255, m = parseInt(f.slice(4, 6), 16) / 255);
  }
  const c = 0.618033988749895, p = 0.324717957244746;
  for (let f = 0; f < o; f++) {
    const h = f * c % 1, d = f * p % 1, u = (f + 0.5) * 0.7548776662466927 % 1;
    let w = 0, b = 0, S = 0, y = 0.65, M = 0.72, v = 0.82;
    if (t === "ground_focus")
      if (h < 0.6) {
        const k = 0.4 + Math.sqrt(d) * 24, g = u * Math.PI * 2 + f * 2.399963229728653;
        w = Math.cos(g) * k, S = Math.sin(g) * k, b = 0.01 + h * 0.75, y = 0.86, M = 0.9, v = 0.98;
      } else {
        const k = 1 + Math.sqrt(d) * 18, g = u * Math.PI * 2 + f * 2.399963229728653;
        w = Math.cos(g) * k, S = Math.sin(g) * k, b = 0.75 + (h - 0.6) * 8.5, y = 0.62, M = 0.7, v = 0.82;
      }
    else if (t === "dome") {
      const k = h * Math.PI * 2, g = 1 - 2 * d, C = Math.sqrt(Math.max(0, 1 - g * g)), j = 1.5 + Math.cbrt(u) * 20;
      w = Math.cos(k) * C * j, S = Math.sin(k) * C * j, b = Math.max(0.01, g * j * 0.75 + 2.5), y = 0.72, M = 0.78, v = 0.88;
    } else {
      const k = f % 4;
      if (k === 0) {
        const g = 0.3 + Math.sqrt(d) * 28, C = f * 2.399963229728653;
        w = Math.cos(C) * g, S = Math.sin(C) * g, b = 0.01 + u * 0.34, y = 0.9, M = 0.94, v = 1;
      } else if (k === 1) {
        const g = 0.6 + Math.sqrt(d) * 18, C = f * 2.399963229728653;
        w = Math.cos(C) * g, S = Math.sin(C) * g, b = 0.35 + u * 3.15, y = 0.68, M = 0.76, v = 0.86;
      } else if (k === 2) {
        const g = 2 + Math.sqrt(d) * 24, C = f * 2.399963229728653;
        w = Math.cos(C) * g, S = Math.sin(C) * g, b = 3.5 + u * 11.5, y = 0.55, M = 0.65, v = 0.78;
      } else {
        const g = 0.5 + d * 6.5, C = f * 2.399963229728653;
        w = Math.cos(C) * g, S = Math.sin(C) * g, b = 0.05 + u * 4.95, y = 0.8, M = 0.86, v = 0.94;
      }
    }
    n.push(w, b, S), s.push(a ? y * i : y, a ? M * l : M, a ? v * m : v);
  }
  return { points: n, colors: s };
}
function an() {
  const e = ie(), t = [{ frame: 0, camera: G(e), interpolation: "ease" }];
  return {
    schema_version: 1,
    fps: 24,
    duration_frames: 120,
    ...
    camera: e,
    keyframes: t,
    cameras: [{ id: "camera_1", name: "Camera 1", color: "#4aa3ef", camera: G(e), keyframes: t }],
    ...
    resolution_gate: !1,
    aspect_ratio: "auto",
    outliner_height: E.outlinerHeight.default,
    preview_width: E.previewWidth.default,
    side_width: E.sideWidth.default,
    left_width: E.leftWidth.default,
    graph_height: E.graphHeight.default,
    assets_height: E.assetsHeight.default,
    agent_height: E.agentHeight.default,
...
function Ei(e) {
  ...
  if (!e || typeof e != "object") return t;
  const a = { ...t, ...e };
  a.fps = Math.round(W(a.fps, 24, 1, 120)), a.duration_frames = Math.round(W(a.duration_frames, 120, 1, De.maxDurationFrames)), a.width = Math.round(W(a.width, 1280, 64, 4096)), a.height = Math.round(W(a.height, 720, 64, 4096));
  const r = (c, p) => (Array.isArray(c) ? c : []).slice(0, De.maxKeysPerTrack).map((f) => ({
    frame: Math.max(0, Math.round(Number(f.frame || 0))),
    camera: G(f.camera || f || p),
    interpolation: Lt.includes(f.interpolation) ? f.interpolation : "ease",
    ...f.tangents && typeof f.tangents == "object" ? { tangents: { ...f.tangents } } : {},
    ...Array.isArray(f.references) ? { references: f.references.map((h) => ({ ...h })) } : {},
    ...Xo(f)
  })), o = G(a.camera || t.camera);
  let n = r(a.keyframes, o);
  n = [...new Map(n.map((c) => [c.frame, c])).values()].sort((c, p) => c.frame - p.frame), n.length || (n = [{ frame: 0, camera: G(o), interpolation: "ease" }]);
  ...
  a.cameras = s.slice(0, De.maxCameras).map((c, p) => {
    let f = String(c?.id || `camera_${p + 1}`);
    i.has(f) && (f = `camera_${p + 1}`), i.add(f);
    const h = G(c?.camera || c?.keyframes?.[0]?.camera || o);
    let d = r(c?.keyframes, h);
    return d = [...new Map(d.map((u) => [u.frame, u])).values()].sort((u, w) => u.frame - w.frame), d.length || (d = [{ frame: 0, camera: G(h), interpolation: "ease" }]), {
      id: f,
...
function va(e, [t, a, r, o]) {
  const [n, s, i] = e, l = o * n + a * i - r * s, m = o * s + r * n - t * i, c = o * i + t * s - a * n, p = -t * n - a * s - r * i;
  return [l * o - p * t - m * r + c * a, m * o - p * a - c * t + l * r, c * o - p * r - l * a + m * t];
}
function on([e, t, a, r]) {
  const o = 1 - 2 * (t * t + a * a), n = 2 * (e * t - a * r), s = 2 * (e * a + t * r), i = 1 - 2 * (e * e + a * a), l = 2 * (t * a - e * r), m = 2 * (t * a + e * r), c = 1 - 2 * (e * e + t * t), p = Math.asin(Math.max(-1, Math.min(1, s))), [f, h] = Math.abs(s) < 0.9999999 ? [Math.atan2(-l, c), Math.atan2(-n, o)] : [Math.atan2(m, i), 0];
  return [f, p, h].map((d) => d * 180 / Math.PI);
}
function Sa(e, t) {
  const a = t.quaternion || Le(t.rotation), r = rn(a, e.quaternion || Le(e.rotation));
  return { position: x(va(e.position.map((o, n) => o * t.size[n]), a), t.position), rotation: on(r), quaternion: r, size: e.size.map((o, n) => o * t.size[n]) };
}
function nn(e, t) {
...
function cn(e, t) {
  const a = [0];
  for (let r = 1; r < e.length; r++) {
    const o = q(e[r - 1]), n = q(e[r]), s = ["right", "up", "forward"].reduce(
...
function Ma(e, t = {}, a = null, r = "generic") {
  ...
  const m = ln(l, o), c = cn(l, o), p = Bt(m, o), f = Bt(p, o), h = { speed: m, angular_speed: c, acceleration: p, jerk: f }, d = mn(e, a), u = dn(l, d, s, i), w = l.map((g) => g.fov), b = t.allow_framing_loss === !0, S = [], y = [];
  for (let g = 0; g < n; g++) {
    const C = [], j = [];
    for (const V of Kt) {
      const L = Vt(h[V][g], t[`max_${V}`]);
      C.push(L), L !== "ok" && j.push(V);
    }
    !u[g] && !b && (C.push("over"), j.push("framing_loss")), S.push(Gt(C)), y.push(j);
  }
  const M = u.filter((g) => !g).length, v = {
    profile: r,
    warn_ratio: wa,
    limits: t,
    subject: d,
  ...
  for (const g of [...Kt, "fov_drift"]) {
    const C = g === "fov_drift" ? "max_fov_change" : `max_${g}`, j = t[C];
    j != null && v[C] > Number(j) && v.violations.push({ metric: C, value: v[C], recommended_max: Number(j) });
  }
  M && !b && v.violations.push({ metric: "framing_loss_frames", value: M, recommended_max: 0 });
  const k = Vt(v.max_fov_change, t.max_fov_change);
  return v.track_grades = { fov_drift: k }, v.grade = Gt([...S, k]), v.trajectory_valid = v.violations.length === 0, v.ok = v.trajectory_valid, v;
}
function un(e) {
...
function Ii(e, t) {
  ...
  if (t.shiftKey)
    ...
    const n = t.currentTarget.getBoundingClientRect(), s = (t.clientX - n.left) / Math.max(1, n.width), i = T(Number(e.timelineZoom) || 1, 0.2, 30), l = T(i * r, 0.2, 30), m = a / i, c = a / l, p = (Number(e.timelinePan) || 0) + s * m;
    e.timelinePan = T(p - s * c, -a * 0.5, a), e.timelineZoom = l;
  }
...
function zi(e, t) {
  ...
  if (e.boxSelect && t.pointerId === e.boxSelect.pointerId) {
    t.preventDefault(), t.stopPropagation();
    const a = e.boxSelect.box.getBoundingClientRect(), r = Math.max(1, e.state.duration_frames - 1), o = T(Number(e.timelineZoom) || 1, 0.1, 50), n = Number(e.timelinePan) || 0, s = r / o, i = (p) => T(n + p / Math.max(1, a.width) * s, 0, r), l = Math.min(i(e.boxSelect.startX), i(e.boxSelect.currentX)), m = Math.max(i(e.boxSelect.startX), i(e.boxSelect.currentX));
...
function gn(e, t) {
  ...
  if (c.style.left = `${p}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${m !== 0 ? ` (${m > 0 ? "+" : ""}${m})` : ""}`, a.moving && a.moving.length > 1) {
    ...
    a.lastDelta = m;
    const f = e.timelineKeyframes(), h = new Set(f.filter((d) => !e.selectedKeyFrames.has(d.frame)).map((d) => d.frame));
    for (const d of a.moving) {
      let u = T(d.startFrame + m, 0, e.state.duration_frames - 1);
      for (; h.has(u) && u > 0 && u < e.state.duration_frames - 1; ) u += Math.sign(m || 1);
      d.key.frame = h.has(u) ? d.key.frame : u;
    }
    f.sort((d, u) => d.frame - u.frame), e.editingKeyFrame = a.key.frame, e.scheduleSerialize(), e.setFrame(a.key.frame, !1, !0);
    return;
  }
  ...
  for (let s = 1; s < r.length - 1; s++)
    if (Ca(r[s].frame ?? 0, t))
      for (const i of ["position", "target"]) {
        const l = [r[s - 1], r[s], r[s + 1]].map((p) => p.camera?.[i]).filter((p) => Array.isArray(p) && p.length >= 3), m = r[s].camera?.[i];
        if (l.length < 3 || !Array.isArray(m)) continue;
        const c = [0, 1, 2].map((p) => l.reduce((f, h) => f + Number(h[p] || 0), 0) / l.length);
        n[s].camera[i] = m.map((p, f) => Number(p) + (c[f] - Number(p)) * o);
      }
  return n;
}
function _n(e, t, a) {
...
function ue(e, t, a, r) {
  const o = a == null ? _("no limit") : `${qt(t)} / ${qt(a)}`, n = a != null && a > 0, s = n ? Math.min(100, Math.round(t / a * 100)) : 0, i = r === "over" ? "#ef4444" : r === "warn" ? "#f59e0b" : "#22c55e";
  return `
...
function Dn(e) {
  ...
  return t.length ? t.slice(0, 6).map((a) => {
    const r = a.metrics.map((n) => Ue(n)).join(", "), o = a.start === a.end ? _("Frame {frame}").replace("{frame}", String(a.start)) : _("Frames {start}-{end}").replace("{start}", String(a.start)).replace("{end}", String(a.end));
    return `
...
function wt(e) {
  return `MajoorOmniCam.Agent.${zr(e).settingKey}.Model`;
}
function Mt(e) {
  return `MajoorOmniCam.Agent.${zr(e).settingKey}.BaseUrl`;
}
function I(e, t, a, r, o) {
  return { id: e, category: [...J, t, a], name: a, tooltip: r, type: "boolean", defaultValue: o };
}
function P(e, t, a, r, o, n) {
  return { id: e, category: [...J, t, a], name: a, tooltip: r, type: "combo", options: o, defaultValue: n };
}
function F(e, t, a, r, o, n) {
  return { id: e, category: [...J, t, a], name: a, tooltip: r, type: "slider", attrs: o, defaultValue: n };
}
function $t(e, t, a, r, o = "") {
  return { id: e, category: [...J, t, a], name: a, tooltip: r, type: "text", defaultValue: o };
}
function In({
  onLocaleChange: e,
  onQualityChange: t,
  ...
  onPanSensitivityChange: p,
  onDollySensitivityChange: f,
  onCameraViewVisibleChange: h,
  onAgentEnabledChange: d
} = {}) {
  return [
    {
      id: Da,
      category: [...J, "Language", "Viewport language"],
    ...
    {
      id: Ka,
      category: [...J, "Proxy", "Default point colour"],
    ...
    {
      id: Va,
      category: [...J, "Viewport", "Studio quality"],
      name: "Studio quality",
    ...
    {
      id: qa,
      category: [...J, "Viewport", "Default background colour"],
...
function Fn() {
  const e = N(St, "ollama", ["openai", "openai_compatible", "anthropic", "ollama"]);
  return {
    enabled: O(Ir, !0),
    provider: e,
    ...
    maxOutputTokens: K(Or, 4096, 512, 32768, !0),
    maxPlannerSteps: K(Pr, 6, 1, 12, !0),
    requestTimeoutSeconds: K(Nr, 120, 15, 300, !0)
  };
}
function Kn() {
  return {
    fps: K(Ta, 24, 1, 120, !0),
    durationSeconds: K(Aa, 5, 1, 120, !0),
    width: K(ja, 1280, 64, 4096, !0),
    height: K(Ea, 720, 64, 4096, !0),
    renderMode: String(R(Ia, "omni_ref")),
    encoder: String(R(Oa, "auto")),
    playblastResolution: N(Pa, "output", ["viewport", "half", "output", "double"]),
    playblastQuality: N(Na, "balanced", ["low", "balanced", "high"]),
    playblastGrid: O(za, !1),
    playblastLabels: O(Ra, !1),
    pointDensity: N(La, "balanced", ["none", "sparse", "balanced", "dense", "ultra"]),
    pointSpread: N(Fa, "all_views", ["all_views", "ground_focus", "dome"]),
    pointColor: Ut(Ka, "#cbd5e1"),
    cardFit: N(Ba, "contain", ["contain", "cover", "stretch"]),
    ...
    aspectRatio: N(tr, "auto", ["auto", "16:9", "4:3", "1:1", "9:16", "2.39:1"]),
    burnIn: O(ar, !1),
    speedHeatmap: O(rr, !1),
    showWireframe: O(or, !1),
    showVertices: O(nr, !1),
    selectMode: N(sr, "object", ["object", "vertex", "edge", "face"]),
    gizmoMode: N(ir, "translate", ["translate", "rotate", "scale"]),
    gizmoSpace: N(lr, "world", ["world", "local"]),
    spatialSnapMode: N(cr, "none", ["none", "grid", "vertex"]),
    spatialGridSize: K(mr, 0.5, 0.01, 100),
    navigationProfile: N(dr, "simple", ["maya", "blender", "simple"]),
    flySpeed: K(pr, 1, 0.05, 5),
    invertOrbitY: O(fr, !1),
    zoomSensitivity: K(ur, 1, 0.2, 3),
    orbitSensitivity: K(hr, 1, 0.2, 3),
    panSensitivity: K(gr, 1, 0.2, 3),
    dollySensitivity: K(yr, 1, 0.2, 3),
    viewMode: N(br, "perspective", ["camera", "perspective", "front", "back", "top", "bottom", "right", "left"]),
    snapEnabled: O(vr, !0),
    snapFrames: K(Sr, 1, 1, 24, !0),
    autoKey: O(wr, !1),
    defaultInterpolation: N(Mr, "ease", ["ease", "smooth", "bezier", "linear", "ease_in", "ease_out", "hold"]),
    timecodeMode: N(xr, "time", ["time", "timecode"]),
    loopPlayback: O(Cr, !1),
    uiDensity: N(kr, "animation", ["basic", "animation", "advanced"]),
    previewLayout: N(Dr, "auto", ["auto", "1", "2", "4"]),
    cameraViewVisible: O(Tr, !0),
    undoLimit: K(Ar, 100, 10, 500, !0),
    extractorBackend: N(jr, "dpvo", ["dpvo", "pycolmap"]),
    monitorProfile: N(Er, "wan_camera_native", ["wan_camera_native", "minimax_h3", "ltx_motion", "generic_video"])
  };
}
function Bn() {
...
function qn(e, { cameraId: t, frame: a, additive: r = !1 } = {}) {
...
function Yt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = q(t), l = t.position, m = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = Wn(m, i);
  let p, f;
  if (t.camera_type === "orthographic") {
    const h = 5 / Math.max(0.01, t.zoom || 1), d = h * r / Math.max(1, o);
    p = (e[0] / Math.max(1, r) - 0.5) * 2 * d, f = (0.5 - e[1] / Math.max(1, o)) * 2 * h;
  } else {
    const h = 0.5 * o / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
    p = (e[0] - r / 2) * c / h, f = (o / 2 - e[1]) * c / h;
  }
  return [0, 1, 2].map((h) => l[h] + i[h] * c + n[h] * p + s[h] * f);
}
function $n(e) {
...
function Zt(e, { cameraId: t, frame: a, additive: r = !1 }) {
  e.pathSelection = qn(e.pathSelection, { cameraId: t, frame: a, additive: r }), e.selectedKeyFrames = new Set(e.pathSelection.frames), e.selectedKeyFrame = e.pathSelection.primaryFrame, e.editingKeyFrame = null, e.pathSelection.primaryFrame != null && e.setFrame(e.pathSelection.primaryFrame), e.refreshKeys(), e.refreshInspector();
}
function Un(e, { pointerX: t, pointerY: a, shiftKey: r, altKey: o }) {
...
const Xn = 3;
function Yn(e, t, a, r, o) {
  const n = B(e, a, r, o), s = B(t, a, r, o);
  return !n || !s ? !1 : Math.hypot(n[0] - s[0], n[1] - s[1]) < Xn;
}
function Zn(e, { pointerX: t, pointerY: a, overHandle: r, viewCamera: o, e: n }) {
  if (!e.webgl?.pickCurveHandle) return !1;
  const s = e.webgl.pickCurveHandle([t, a]);
  if (!s) return !1;
  const i = (e.state.cameras || []).find((c) => c.id === s.cameraId), l = (i?.keyframes || []).findIndex((c) => c.frame === s.frame), m = l >= 0 ? i.keyframes[l] : null;
  return !m || Yn(s.position, m.camera.position, o, e.canvas.width, e.canvas.height) ? !1 : (r && n.stopImmediatePropagation?.(), e.curveHandleDrag = {
    cameraId: s.cameraId,
    frame: s.frame,
    side: s.side,
    anchor: [...m.camera.position],
  ...
  }, e.interactionElement.style && (e.interactionElement.style.cursor = "grabbing"), e.selectKeyframe?.(m), !0);
}
...
function Ve(e, t) {
  return (e.camera_type === "orthographic" ? 10 / Math.max(0.01, e.zoom || 1) : 2 * H(A(e.position, e.target)) * Math.tan((e.fov || 35) * Math.PI / 360)) / Math.max(1, t);
}
function Se(e) {
...
function Ge(e, { mode: t, origin: a, delta: r, factors: o, rotationDeg: n }) {
  ...
  const s = A(e, a);
  return t === "scale" ? x(a, [s[0] * o[0], s[1] * o[1], s[2] * o[2]]) : x(a, Ce(s, n));
}
function as(e, t) {
...
function Qi(e, t, a) {
  ...
  return r.map((i, l) => {
    ...
    const c = m.position, p = m.target;
    if (m.position = s[l], Array.isArray(p))
      if (n)
        m.target = Ge(p, a);
      else {
        const f = H(A(p, c)) || 5, h = rs(s, l);
        m.target = x(m.position, D(h, f));
      }
    return { ...i, camera: m };
  });
}
function Ji(e, t, { delta: a = [0, 0, 0] } = {}) {
...
function ns(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
  return !o || o.locked || !(o.keyframes?.length >= 1) ? !1 : (e.checkpoint("Transform camera path"), e.gizmoDrag = {
    ...
    baseKeys: o.keyframes.map((n) => ({ ...n, camera: G(n.camera) })),
    viewRight: q(a).right,
    viewUp: q(a).up,
    freeScale: a.camera_type === "orthographic" ? Ve(a, e.canvas.height) : H(A(a.position, r)) * (2 * Math.tan((a.fov || 35) * Math.PI / 360)) / e.canvas.height
  }, !0);
}
function ss(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  if (e.state.gizmo_mode === "translate") {
    let m;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, p = (t[1] - n.pointer[1]) * r;
      m = x(D(n.viewRight, c * n.freeScale), D(n.viewUp, -p * n.freeScale));
    } else
      m = D(n.axis, a * n.worldLength / n.screenLength);
    l = { mode: "translate", delta: m };
  } else if (e.state.gizmo_mode === "scale") {
    let m;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, p = (t[1] - n.pointer[1]) * r, f = Math.max(0.01, 1 + (c - p) * n.freeScale * 0.35);
      m = [f, f, f];
    } else {
  ...
  s.keyframes = as(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = ce(s, e.frame, e.state.objects), s.camera = G(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function Jt(e, t) {
  const a = [];
...
function is(e, t) {
  ...
  return Jt(n.map((i) => -i), n).map((i) => x(o, va(i, s)));
}
function ls(e, t, a, r = {}) {
  const o = e.state.objects.filter((y) => y.enabled !== !1), n = r.all ? o : o.filter((y) => e.selectedObjectIds?.has(y.id)), i = (n.length ? n : [a]).flatMap((y) => is(e, y)), l = [0, 1, 2].map((y) => Math.min(...i.map((M) => M[y]))), m = [0, 1, 2].map((y) => Math.max(...i.map((M) => M[y]))), c = l.map((y, M) => (y + m[M]) / 2), { right: p, up: f, forward: h } = q(t), d = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), u = Math.tan((t.fov || 35) * Math.PI / 360), w = u * d;
  let b = 2, S = 0.1;
  for (const y of i) {
    const M = A(y, c), v = Math.abs(Y(M, p)), k = Math.abs(Y(M, f)), g = Y(M, h);
    b = Math.max(b, 1.15 * v / w - g, 1.15 * k / u - g, (t.near || 0.01) * 2 - g), S = Math.max(S, 1.15 * k, 1.15 * v / d);
  }
  t.target = c, t.position = A(c, D(h, b)), t.camera_type === "orthographic" && (t.zoom = Math.max(0.01, 5 / S));
}
const ee = {
      ...
      if (a.length === 1) {
        if (e.pathSelection?.component === "target") {
          ...
          return {
            ...
            rotation: [0, 0, 0],
            scale: [1, 1, 1],
            allowedModes: ee.path_point_target,
            track: t,
            frame: r.frame,
            readOnly: ts(t, e.state.objects)
          };
        }
        return {
...
function el(e, t) {
  if (["camera", "perspective", "iso", "front", "back", "top", "right", "left", "bottom"].includes(t)) {
    ...
    e.serialize(), e.render(), e.setStatus(_(`View: ${t[0].toUpperCase()}${t.slice(1)}`));
  }
}
function tl(e, t) {
  if (["translate", "rotate", "scale"].includes(t)) {
    ...
    e.serialize(), e.render(), e.setStatus(_(`${t[0].toUpperCase()}${t.slice(1)} · ${t === "translate" ? "W" : t === "rotate" ? "E" : "R"}`));
  }
}
function al(e, t) {
...
function rl(e, t = {}) {
  const a = Z(e), r = e.state.view_mode !== "camera", o = [...a.target];
  if (e.subSelection?.point && !t.all) {
    e.checkpoint("Frame selection");
    const i = e.subSelection.point, l = se(A(a.position, o)), m = Number.isFinite(l[0]) && H(l) > 0.1 ? l : [0.707, 0.4, 0.707], c = 2;
    a.target = [...i], a.position = x(a.target, D(m, c)), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const p = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(_(`Focused on ${p} at [${i.map((f) => Math.round(f * 100) / 100).join(", ")}]`));
    return;
  }
  const s = e.selectedObject() || e.state.objects.find((i) => i.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] };
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), ls(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? _("Framed: all objects") : _("Framed: {name}").replace("{name}", s.name || s.type || _("Subject")));
}
function ms(e, t, a, { forceLocal: r = !1 } = {}) {
...
function Vr(e) {
  ...
  const t = ds(e);
  if (!t || e.transformControlsWiring && ps.has(t.type)) return null;
  const a = Z(e), r = t.position;
  ...
  const n = Math.max(0.7, H(A(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? ms(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: i.map((m, c) => ({ index: c, axis: m, points: [o, B(x(r, D(m, n)), a, e.canvas.width, e.canvas.height)] })).filter((m) => m.points[1] && Number.isFinite(m.points[1][0]) && Number.isFinite(m.points[1][1]))
    };
  const l = i.map((m, c) => {
    const p = Math.abs(m[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], f = se(Me(m, p)), h = se(Me(m, f)), d = [];
    for (let u = 0; u <= 48; u++) {
      const w = u / 48 * Math.PI * 2, b = B(x(r, x(D(f, Math.cos(w) * n), D(h, Math.sin(w) * n))), a, e.canvas.width, e.canvas.height);
      b && Number.isFinite(b[0]) && Number.isFinite(b[1]) && d.push(b);
    }
    return { index: c, axis: m, points: d };
  });
...
function Gr(e, t) {
  ...
  for (const i of a.handles)
    for (let l = 0; l < i.points.length - 1; l++) {
      const m = i.points[l], c = i.points[l + 1], p = _a(t, m, c);
...
function fs(e, t) {
  ...
  if (e.state.view_mode !== "camera") {
    ...
    if (e.state.show_camera_paths !== !1)
      for (const s of e.state.cameras) {
        ...
        const l = i.map((m) => B(m.camera?.position, r, e.canvas.width, e.canvas.height)).filter((m) => m && Number.isFinite(m[0]) && Number.isFinite(m[1]));
    ...
    for (const s of e.state.objects)
      if (s.enabled !== !1)
        for (const i of s.keyframes || []) {
          ...
          if (!l) continue;
          const m = B(l, r, e.canvas.width, e.canvas.height);
  ...
  for (const s of e.state.objects) {
    ...
    const i = s.keyframes?.length ? $e(s, e.frame) : s, l = B(i.position || [0, 0, 0], r, e.canvas.width, e.canvas.height);
    if (!l) continue;
...
function us(e, t, a, r = 15) {
  const o = a[0] - t[0], n = a[1] - t[1], s = Math.hypot(o, n) || 1, i = o / s, l = n / s, m = -l, c = i, p = r * 0.42, f = a[0] - i * r, h = a[1] - l * r;
  e.beginPath(), e.moveTo(a[0], a[1]), e.lineTo(f + m * p, h + c * p), e.lineTo(f - m * p, h - c * p), e.closePath(), e.fill(), e.save(), e.strokeStyle = "rgba(15, 23, 42, 0.65)", e.lineWidth = 1, e.stroke(), e.restore();
}
function ol(e) {
  const t = Vr(e);
  if (!t || !t.handles) return;
  if (t.entity?.type === "camera_path") {
    const o = Z(e), n = (t.entity.track?.keyframes || []).map((s) => B(s.camera?.position, o, e.canvas.width, e.canvas.height)).filter((s) => s && Number.isFinite(s[0]) && Number.isFinite(s[1]));
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : us(e.ctx, s[0], i);
    }
  }
...
function ys(e, t, a, r, o, n, s) {
  const i = o ? "grid" : e.state.spatial_snap_mode;
  if (i === "grid") {
    const l = e.state.spatial_grid_size || 0.5;
    return n ? a.map((m, c) => m.map((p, f) => Math.abs(n[f]) > 1e-6 ? qe(p, l) : s[c][f])) : a.map((m) => m.map((c) => qe(c, l)));
  }
  if (i === "vertex" && r && !n) {
    ...
    if (l?.point && !t.snapshots.some((m) => m.object.id === l.objectId)) {
      const m = a.reduce((p, f) => x(p, f), [0, 0, 0]).map((p) => p / a.length), c = A(l.point, m);
      return a.map((p) => x(p, c));
    }
  }
  return a;
}
function Ie(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = gs(a), m = a.axis ? lt[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], p = q(c), f = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...A(c.position, c.target)) * 25e-4;
  let h = a.snapshots.map((y) => [...y.transform.position]);
  if (a.mode === "translate") {
    const y = l ?? (n - s) * f * i, M = m ? D(m, y) : x(D(p.right, n * f * i), D(p.up, -s * f * i));
    h = h.map((v) => x(v, M)), h = ys(e, a, h, o, t.ctrlKey || t.metaKey, m, a.snapshots.map((v) => v.transform.position));
  }
  const d = a.mode === "rotate" ? l ?? (n - s) * 0.5 * i : 0, u = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - s) * 0.01 * i) : 1, w = m || lt.z, b = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
  a.snapshots.forEach((y, M) => {
    const v = y.object;
    if (a.mode === "translate" && (v.position = h[M]), a.mode === "rotate") {
      const k = b ? qe(d, 15) : d, g = D(w, k);
      v.position = x(a.pivot, Ce(A(y.transform.position, a.pivot), g)), v.rotation = x(y.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const k = b ? qe(u, 0.1) : u, g = m ? m.map((j) => j ? k : 1) : [k, k, k], C = A(y.transform.position, a.pivot);
      v.position = x(a.pivot, C.map((j, V) => j * g[V])), v.size = y.transform.size.map((j, V) => Math.max(0.01, j * g[V]));
    }
    e.commitObjectEdit(v);
...
const oe = (e, t) => Math.round(e / t) * t, vs = (e, t) => e.map((a) => oe(a, t));
function Ss(e, t, a) {
  ...
  if (n)
    ...
    const f = (r.size || [1, 1, 1]).map((h) => Math.max(0.01, Math.abs(h)) / 2);
    s = f.map((h, d) => o[d] - h), i = f.map((h, d) => o[d] + h);
  }
  let l = 1 / 0, m = 1 / 0, c = -1 / 0, p = -1 / 0;
  for (const f of [s[0], i[0]]) for (const h of [s[1], i[1]]) for (const d of [s[2], i[2]]) {
    const u = B([f, h, d], a, e.canvas.width, e.canvas.height);
    u && (l = Math.min(l, u[0]), c = Math.max(c, u[0]), m = Math.min(m, u[1]), p = Math.max(p, u[1]));
  }
  return Number.isFinite(l) ? { minX: l, minY: m, maxX: c, maxY: p } : null;
}
function ge(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, m) => Math.abs(o.axis[m]) > 1e-6 ? oe(l, i) : o.base[m]) : vs(t, i);
  if (s === "vertex" && a && !o) {
...
function nl(e, t) {
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = Z(e), s = e.state.view_mode !== "camera", i = Qt(e, t), l = t.button === 0 && !i, m = l && !t.altKey && !t.shiftKey, c = e.transformControlsWiring?.isPointerOverHandle?.();
  !m && c && t.stopImmediatePropagation?.();
  const p = e.transformControlsWiring?.currentLiveType?.();
  if (m && (!c || p === "path_point" || p === "camera") && Zn(e, { pointerX: r, pointerY: o, overHandle: c, viewCamera: n, e: t }) || m && c || l && Un(e, { pointerX: r, pointerY: o, shiftKey: t.shiftKey, altKey: t.altKey })) return;
  const h = m ? Gr(e, [r, o]) : null;
  if (h) {
    const [y, M] = h.segment, v = Math.max(1, Math.hypot(M[0] - y[0], M[1] - y[1])), k = {
      pointer: [r, o],
      axis: h.axis,
      axisIndex: h.index,
      screen: [(M[0] - y[0]) / v, (M[1] - y[1]) / v],
    ...
    if (e.interactionElement.style && (e.interactionElement.style.cursor = "grabbing"), h.entity.type === "camera_target") {
      ...
      const g = e.activeCameraTrack?.(), C = !!g?.target_object_id;
      e.gizmoDrag = {
        ...k,
        type: "camera_target",
        historyCheckpointed: !0,
        tracking: C,
        target: C ? [...g.target_offset || [0, 0, 0]] : [...h.entity.position || e.camera.target]
      };
      return;
    }
    ...
    if (h.entity.type === "camera_path" && ns(e, { baseDrag: k, viewCamera: n, entityPosition: h.entity.position })) return;
    if (h.entity.type === "object") {
      const g = h.entity.object;
      e.checkpoint("Transform object");
      const C = qr(e), j = (C.length ? C : [g]).map((L) => ({ object: L, transform: me(L) }));
      for (const L of j) e.beginObjectEdit(L.object);
      const V = j.reduce((L, U) => x(L, U.transform.position), [0, 0, 0]).map((L) => L / j.length);
      e.gizmoDrag = {
        ...k,
        type: "object",
        historyCheckpointed: !0,
        ...
        viewRight: q(n).right,
        viewUp: q(n).up,
        freeScale: n.camera_type === "orthographic" ? Ve(n, e.canvas.height) : H(A(n.position, h.entity.position)) * (2 * Math.tan((n.fov || 35) * Math.PI / 360)) / e.canvas.height
      };
      return;
    }
  }
  const d = l ? fs(e, [r, o]) : null;
  if (e.pointerHit = !!(h || d), d) {
    if (d.type === "camera_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.editingKeyFrame = null, e.activateCamera(d.camera.id), e.setFrame(d.keyframe.frame), e.selectKeyframe(d.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(_(`${d.camera.name} · Keyframe @ F${d.keyframe.frame} selected`));
      return;
    }
    if (d.type === "object_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectId = d.object.id, e.editingKeyFrame = null, e.setFrame(d.keyframe.frame), e.selectKeyframe(d.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(_(`${d.object.name || d.object.type} · Keyframe @ F${d.keyframe.frame} selected`));
      return;
    }
    if (d.type === "camera_target") {
      if (e.finishCameraEdit(), e.selectedEntity = "camera_target", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(d.camera.id), d.camera.locked) {
        e.setStatus(_("{name} is locked").replace("{name}", d.camera.name)), e.refreshObjects(), e.refreshInspector(), e.render();
        return;
      }
      e.checkpoint("Move camera target"), e.beginCameraEdit();
      const { right: y, up: M } = q(n), v = [...e.camera.target], k = e.activeCameraTrack?.(), g = !!k?.target_object_id;
      e.targetFreeDrag = {
        pointer: [r, o],
        target: g ? [...k.target_offset || [0, 0, 0]] : v,
        tracking: g,
        right: y,
        up: M,
        ...
        historyCheckpointed: !0
      }, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(_(`${d.camera.name} · Target aim selected`));
      return;
    }
    if (d.type === "camera") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(d.camera.id), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(_(`${d.camera.name} selected`));
      return;
    }
  ...
  s && !e.state.editor_views && (e.state.editor_views = Re()), e.drag = {
    ...
    navigationOnly: i,
    historyCheckpointed: !1
  }, e.interactionElement.style && (e.interactionElement.style.cursor = S ? "ns-resize" : b ? "move" : "grabbing"), e.setStatus?.(_(u ? "Fly" : S ? "Dolly" : b ? "Pan" : "Orbit"));
}
function sl(e, t) {
  ...
  if (e.pathDrag) {
    ...
    const c = ((e.state.cameras || []).find((p) => p.id === e.pathDrag.cameraId)?.keyframes || []).find((p) => p.frame === e.pathDrag.frame);
    c && (c.camera.position = Yt(
      ...
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = $n(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  if (e.curveHandleDrag) {
    ...
    const c = ((e.state.cameras || []).find((p) => p.id === e.curveHandleDrag.cameraId)?.keyframes || []).find((p) => p.frame === e.curveHandleDrag.frame);
    if (c) {
      const p = Yt(
        [i, l],
        Z(e),
      ...
      e.dragCurveHandle?.(c, e.curveHandleDrag.side, p, {
  ...
  if (e.targetFreeDrag) {
    ...
    const s = e.interactionElement.getBoundingClientRect(), i = (t.clientX - s.left) * e.canvas.width / Math.max(1, s.width), l = (t.clientY - s.top) * e.canvas.height / Math.max(1, s.height), m = i - e.targetFreeDrag.pointer[0], c = l - e.targetFreeDrag.pointer[1], p = t.shiftKey ? 0.1 : 1, f = x(D(e.targetFreeDrag.right, m * e.targetFreeDrag.scale * p), D(e.targetFreeDrag.up, -c * e.targetFreeDrag.scale * p)), h = ge(e, x(e.targetFreeDrag.target, f), [i, l]);
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_target") {
      const h = x(e.gizmoDrag.target, D(e.gizmoDrag.axis, m * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), d = ge(e, h, i, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
      e.gizmoDrag.tracking ? ea(e, d) : e.camera.target = d, e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        const h = x(e.gizmoDrag.position, D(e.gizmoDrag.axis, m * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.camera.position = ge(e, h, i, [], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      } else {
        const h = c ? oe(m * 0.015, Math.PI / 12) : m * 0.015, d = A(e.gizmoDrag.target, e.gizmoDrag.position), u = Ce(d, D(e.gizmoDrag.axis, h * (180 / Math.PI)));
        e.camera.target = x(e.gizmoDrag.position, u);
      }
      e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera_path") return void ss(e, { pointer: i, deltaPixels: m, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
        const h = (i[0] - e.gizmoDrag.pointer[0]) * l, d = (i[1] - e.gizmoDrag.pointer[1]) * l, u = x(
          e.gizmoDrag.position,
          x(D(e.gizmoDrag.viewRight, h * e.gizmoDrag.freeScale), D(e.gizmoDrag.viewUp, -d * e.gizmoDrag.freeScale))
        );
      ...
      if (e.gizmoDrag.free) {
        const h = (i[0] - e.gizmoDrag.pointer[0]) * l, d = (i[1] - e.gizmoDrag.pointer[1]) * l, u = (h - d) * e.gizmoDrag.freeScale, w = e.gizmoDrag.size.map((b) => {
          const S = b + u;
          return Math.max(0.01, c ? oe(S, 0.1) : S);
        });
        e.gizmoDrag.object.size = w;
      } else {
        const h = [...e.gizmoDrag.size], d = h[e.gizmoDrag.axisIndex] + m * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength;
        h[e.gizmoDrag.axisIndex] = Math.max(0.01, c ? oe(d, 0.1) : d), e.gizmoDrag.object.size = h;
      }
    else {
      const h = [...e.gizmoDrag.rotation], d = h[e.gizmoDrag.axisIndex] + m * 0.75;
      h[e.gizmoDrag.axisIndex] = c ? oe(d, 15) : d, e.gizmoDrag.object.rotation = h;
    }
    const p = e.gizmoDrag.group || [], f = p.find((h) => h.object === e.gizmoDrag.object)?.transform;
    if (p.length > 1 && f)
      if (e.state.gizmo_mode === "translate") {
        const h = A(e.gizmoDrag.object.position, f.position);
        for (const d of p) d.object.position = x(d.transform.position, h);
      } else if (e.state.gizmo_mode === "rotate") {
        const h = A(e.gizmoDrag.object.rotation, f.rotation);
        for (const d of p)
          d.object.position = x(e.gizmoDrag.groupPivot, Ce(A(d.transform.position, e.gizmoDrag.groupPivot), h)), d.object.rotation = x(d.transform.rotation, h);
      } else {
        const h = e.gizmoDrag.object.size.map((d, u) => d / Math.max(0.01, f.size[u]));
        for (const d of p) {
          const u = A(d.transform.position, e.gizmoDrag.groupPivot);
          d.object.position = x(e.gizmoDrag.groupPivot, u.map((w, b) => w * h[b])), d.object.size = d.transform.size.map((w, b) => Math.max(0.01, w * h[b]));
        }
      }
    for (const h of p.length ? p : [{ object: e.gizmoDrag.object }]) e.commitObjectEdit(h.object);
    e.refreshInspector(), e.render();
    return;
  }
  if (!e.drag) {
  ...
  if (e.drag.dolly) {
    ...
    const s = A(n.target, n.position), i = H(s);
    ...
    const { right: s, up: i } = q(n), l = Ve(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), m = x(D(s, -a * l), D(i, r * l));
    e.drag.target.position = x(n.position, m), e.drag.target.target = x(n.target, m);
  } else {
    const s = A(n.position, n.target), i = H(s);
    let l = Math.atan2(s[0], s[2]), m = Math.asin(T(s[1] / i, -0.999, 0.999));
...
function $r(e) {
  ...
  return e.keyDrag?.badge?.remove?.(), e.boxSelect?.overlay?.remove?.(), e.drag?.camera && e.drag?.target && (Object.assign(e.drag.target, e.drag.camera), e.drag.editorView && e.scheduleSerialize()), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.boxSelection = null, e.pathDrag = null, e.keyDrag = null, e.curveDrag = null, e.timelineDrag = null, e.timelinePanDrag = null, e.boxSelect = null, e.curvePanDrag = null, e.curveScrub = null, e.curveBoxSelect = null, Se(e), t && e.undo(), e.finishCameraEdit(), e.refreshInspector(), e.render(), e.setStatus(_("Interaction cancelled")), !0;
}
function il(e, t) {
  ...
  if (e.pathDrag) {
    ...
    e.pathDrag = null, Se(e), s && (e.scheduleSerialize(), e.refreshKeys(), e.setStatus(_("Path key moved")));
    return;
  }
  if (e.curveHandleDrag) {
    const s = e.curveHandleDrag.moved;
    e.curveHandleDrag = null, Se(e), s && (e.webgl && (e.webgl.pathKey = ""), e.scheduleSerialize(), e.refreshKeys(), e.setStatus(_("Curve handle updated")));
    return;
  }
  if (e.boxSelection) {
    const s = e.boxSelection, i = Z(e), l = Math.min(s.start[0], s.current[0]), m = Math.max(s.start[0], s.current[0]), c = Math.min(s.start[1], s.current[1]), p = Math.max(s.start[1], s.current[1]), f = s.additive ? new Set(s.initial) : /* @__PURE__ */ new Set();
    for (const h of e.state.objects) {
      if (h.enabled === !1) continue;
      const d = Ss(e, h, i);
      d && d.maxX >= l && d.minX <= m && d.maxY >= c && d.minY <= p && f.add(h.id);
    }
    e.selectedObjectIds = f, e.selectedObjectId = [...f].at(-1) || null, e.selectedEntity = f.size ? "object" : "camera", e.boxSelection = null, Se(e), e.refreshObjects(), e.refreshInspector(), e.render(), e.setStatus(_("{count} object(s) selected").replace("{count}", String(f.size)));
    return;
  }
  const a = e.keyDrag, r = !!(e.drag && !e.drag.editorView || e.targetFreeDrag), o = !!e.gizmoDrag;
  e.gizmoDrag?.type === "camera_path" && (e.serialize?.(), e.refreshKeys?.(), e.setStatus(_("Camera path transformed"))), !e.pointerHit && !e.gizmoDrag && !e.targetFreeDrag && e.drag && !e.drag.navigationOnly && t && Math.hypot(t.clientX - e.drag.x, t.clientY - e.drag.y) < 5 && (t.button === 0 || t.button === void 0) && (e.selectedEntity === "object" || e.selectedObjectId !== null || e.selectedEntity === "camera_target" || e.selectedEntity === "camera_path") && (e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = null, e.subSelection = null, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(_("Deselected"))), Se(e), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.keyDrag = null, e.pointerHit = !1, e.canvas.classList.remove("dragging"), e.interactionElement.style && (e.interactionElement.style.cursor = "default"), a && (a.badge?.remove(), e.editingKeyFrame = null, e.updateKeyVisualState(), e.root.focus({ preventScroll: !0 })), r && e.finishCameraEdit(), o && (e.editingKeyFrame = null, e.updateKeyVisualState(), e.drawCurveEditor());
}
function ll(e, t) {
  ...
  const a = es(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
  if (e.isNavigatingFly) {
    e.cameraSpeed = T(e.cameraSpeed * Math.exp(-a * 1e-3), 0.05, 20), e.setStatus(_(`Fly speed: ${e.cameraSpeed.toFixed(2)}x`));
    return;
  }
  _s(e);
...
function He(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = ws(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function ks(e, t, a, r, o) {
  const n = ce(e, a, e.objects);
  let s = t?.point;
  if (t?.object_id) {
    const c = e.objects.find((h) => h.id === t.object_id);
    if (!c) return null;
    const p = nn(e.objects, c), f = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
    s = [p.position[0] + f[0] * p.size[0], p.position[1] + f[1] * p.size[1], p.position[2] + f[2] * p.size[2]];
  }
...
function tt(e, t) {
  return Cs(t, e.interactionElement);
}
function As(e, t) {
...
function cl(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = et(e.state);
    n && (e.checkpoint("Set motion interpolation"), Ms(n, o.target.value), re(e, `Motion interpolation: ${o.target.value}`));
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? He(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? js(e, s) : n === "erase" && As(e.state, s), re(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = ta(e), l = Ts(e.state, n.points, s, i);
...
function aa(e, t, a) {
  const r = e.viewportCamera(), o = e.state.view_mode !== "camera", n = A(r.position, r.target), s = H(n);
  if (!(s > 1e-4)) return;
...
const ye = { t: "translate", r: "rotate", s: "scale" }, zs = [
...
], Rs = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function Ls(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(Rs) : !1;
}
function Fs(e) {
...
function Bs(e, t) {
  return Ks(e) || t?.lastKeyZone || "viewport";
}
let ra = !1;
function Vs() {
  ra || typeof window > "u" || (ra = !0, window.addEventListener("keydown", (e) => {
    if (!Pn()) return;
    const t = e.composedPath?.()[0] || e.target, a = Nn(t);
...
function Gs(e, t) {
  ...
  if (Fs(a) || (t.code === "Space" || t.key === "Enter") && Ls(a)) return !1;
...
function Ws(e, t) {
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const i = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: m, up: c, forward: p } = q(i), f = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, h = { w: D(p, f), s: D(p, -f), d: D(m, f), a: D(m, -f), e: D(c, f), q: D(c, -f) }[a];
...
function Us(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!Ne(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: po(e.state) }, rt(e)) : (e.checkpoint("Split shot"), uo(e.state, e.frame, null) ? rt(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
    if (t.repeat) return !0;
    const r = Ne(e.state), o = la(e.state, e.frame), n = o ? r.findIndex((s) => s.start === o.start) : -1;
...
function Zs([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * Xs), n = Math.round((window.innerHeight || t) * Ys);
  return [
...
function Qs(e, t) {
  const a = Zr[t];
  return !a || !e?.setSize ? !1 : (e.setSize(Zs(a.default, a.min)), !0);
}
function Js(e, t, a) {
  const r = Zr[t];
...
function ti() {
  ...
  e.id = oa, e.textContent = ei, document.head.appendChild(e);
}
function ne(e) {
...
function na(e) {
  ...
  a.appendChild(l);
  let c = !1;
  const p = document.activeElement instanceof HTMLElement ? document.activeElement : null, f = () => {
    document.removeEventListener("keydown", d, !0), t.remove(), xe === f && (xe = null), p && p.isConnected && typeof p.focus == "function" && p.focus({ preventScroll: !0 });
  };
  xe = f;
  const h = () => Array.from(
    a.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
  ).filter((u) => !u.disabled && u.offsetParent !== null), d = (u) => {
    if (u.key === "Escape") {
      u.stopPropagation(), u.preventDefault(), f();
      return;
    }
    if (u.key === "Tab") {
      const w = h();
      ...
      u.shiftKey && (y === b || !a.contains(y)) ? (u.preventDefault(), S.focus()) : !u.shiftKey && (y === S || !a.contains(y)) && (u.preventDefault(), b.focus());
    }
  };
  return document.addEventListener("keydown", d, !0), i.addEventListener("click", (u) => {
    u.stopPropagation(), f();
  }), t.addEventListener("mousedown", (u) => {
    c = u.target === t;
  }), t.addEventListener("click", (u) => {
    u.target === t && c && f(), c = !1;
  }), a.addEventListener("mousedown", (u) => u.stopPropagation()), document.body.appendChild(t), (i.isConnected ? i : a).focus({ preventScroll: !0 }), f;
}
...
const sa = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", ia = "oc-help-toolbar-css", oi = "#8b7bd8";
function ni() {
  ...
  e.id = ia, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${oi};color:#fff;
...
te.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Dt(e, async () => {
      ...
      const { attachDirectorShell: o } = await import("./chunk-D_a6cG2W.js").then((n) => n.q);
      return Ee("director:import:resolved", e), o;
    });
    const r = e.__majoorOmniCamDirectorRuntime;
    r && (Ee("director:attach:complete", e), t && Gn(r), At(e, ct, t, a?.()));
  }
});
te.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== mt) return;
    const t = !le, a = t ? null : jt(e);
    await Dt(e, async () => (await import("./chunk-B8L_LuFB.js").then((r) => r.g)).attachExtractorShell), e.__majoorOmniCamExtractorRuntime && At(e, mt, t, a?.());
  }
});
te.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== dt) return;
    const t = !le, a = t ? null : jt(e);
    await Dt(e, async () => (await import("./chunk-KwKKkI7U.js")).attachMonitor), e.__majoorOmniCamMonitor && At(e, dt, t, a?.());
  }
});
export {
  on as $,
