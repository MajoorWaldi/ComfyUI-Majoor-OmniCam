import { c as w, bn as g, e as k, aR as A, bo as ae, y as $, s as M, bp as oe, a as v, g as O, aS as P, bq as re } from "./chunk-B56R8UMR.js";
import { m as ne, g as H, l as se } from "./chunk-BOI_2OKd.js";
function Fe(e, t) {
    ...
    for (const r of a) {
      ...
      if (Array.isArray(d) && d.length) {
        ...
        const x = () => {
          ...
          let y = p.right + 2;
          y + b.width > window.innerWidth - h && (y = Math.max(h, p.left - b.width - 2));
          let u = p.top - 4;
          u + b.height > window.innerHeight - h && (u = Math.max(h, window.innerHeight - b.height - h)), c.style.left = `${y}px`, c.style.top = `${u}px`;
        }, n = () => {
        ...
        s.addEventListener("pointerenter", () => {
          clearTimeout(f), m = setTimeout(x, 60);
          ...
          p.preventDefault(), p.stopPropagation(), c.hidden ? x() : n();
        });
      } else {
        if (r.shortcut) {
...
function B({ title: e, message: t, withInput: a = !1, defaultValue: o = "", owner: r = null }) {
  return typeof document > "u" || !document.body ? Promise.resolve(a ? null : !1) : new Promise((s) => {
    ...
    n.style.background = "var(--oc-accent, #4c6ef5)", n.style.borderColor = "transparent", n.style.color = "#fff", f.append(x, n), d.append(i, c), m && d.append(m), d.append(f), l.append(d);
    let p = !1;
    const b = (u) => {
    ...
    x.addEventListener("click", () => b(a ? null : !1)), n.addEventListener("click", () => b(a ? m.value : !0)), l.addEventListener("mousedown", (u) => {
      u.target === l && b(a ? null : !1);
    }), document.addEventListener("keydown", y, !0), document.body.appendChild(l), (m || n).focus();
  });
}
function Oe({ title: e, items: t = [], onDelete: a = null, owner: o = null }) {
  return typeof document > "u" || !document.body ? Promise.resolve(null) : new Promise((r) => {
    ...
    for (const h of t) i.appendChild(x(h));
...
function Ge(e, t) {
  ...
  for (const [a, o] of Object.entries(fe)) {
    ...
    r.addEventListener("pointerup", x, { signal: t }), r.addEventListener("pointercancel", x, { signal: t }), r.addEventListener("dblclick", (n) => {
...
function Ye(e) {
  ...
  e.directorRevision = (Number.isInteger(e.directorRevision) ? Math.max(0, e.directorRevision) : 0) + 1, e.renderRevision = (e.renderRevision || 0) + 1, ue(e);
...
function Qe(e, t = !0) {
  ...
  const x = r('[data-role="timeline-fps"]');
  x && (x.value = String(e.state.fps)), e.frame = w(e.frame, 0, e.state.duration_frames - 1), t && e.serialize(), (a !== e.state.duration_frames || o !== e.state.fps) && (e.computeAudioPeaks?.(), e.setFrame(e.frame, !1, !0), e.setStatus(`Timeline: ${e.state.duration_frames} frames · ${(e.state.duration_frames / e.state.fps).toFixed(2)} s`));
}
function Ze(e) {
  let t = null;
  try {
...
function ye(e, t, { label: a = "Extracted Camera" } = {}) {
  ...
  const r = Number(e.state.fps) || 24, s = Number(t.fps) || r, l = s > 0 ? r / s : 1, d = T(e.state), i = e.state.cameras.length, c = xe(e.state, a || "Extracted Camera"), m = S[i % S.length], f = o.map((x) => ({
    ...x,
    frame: Math.round((Number(x.frame) || 0) * l),
    camera: k(x.camera)
  }));
  if (e.state.cameras.push({ id: d, name: c, color: m, camera: k(f[0].camera), keyframes: f }), Number.isFinite(Number(t.duration_frames))) {
    const x = Math.round(Number(t.duration_frames) * l);
...
function Q(e) {
  ...
  d !== e.cameraPreviewSignature && (i = !0, e.cameraPreviewSignature = d, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), l.forEach((c, m) => {
    ...
    const x = c.color || S[m % S.length];
    f.style.setProperty("--camera-color", x), f.title = v(`Click: set ${c.name} as primary · Double-click: edit · Right-click: preview actions`);
    const n = document.createElement("div");
    ...
    y.className = "pi pi-circle-fill output-mark", y.title = v("Playblast camera");
    const u = document.createElement("canvas");
    u.dataset.cameraPreview = c.id;
    const j = document.createElement("span");
    j.className = "camera-view-badge", j.textContent = v("CAMERA PREVIEW"), n.append(p, b, h, y), f.append(u, n, j), t.appendChild(f), f.addEventListener("click", () => {
...
async function tt(e, t) {
  ...
  if (!a) return;
  const o = (await ie(e.app, v("Rename camera"), v("Camera name"), a.name))?.trim();
...
async function ot(e, t) {
  ...
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a || !await ce(e.app, v("Delete camera"), v(`Delete ${a.name} and its ${a.keyframes.length} keyframe(s)?`))) return;
  ...
  if (e.state.cameras = e.state.cameras.filter((r) => r.id !== t), t === e.state.playblast_camera_id && (e.state.playblast_camera_id = e.state.cameras[0].id), e.cameraPreviewSignature = "", o) {
    ...
    e.state.active_camera_id = r.id, e.state.keyframes = r.keyframes, e.state.camera = k(r.camera), e.camera = M(r, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = r.keyframes.find((s) => s.frame === e.frame)?.frame ?? null, e.editingKeyFrame = null;
  }
  e.serialize(), e.refreshCameraSelectors(), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v(`${a.name} deleted`));
}
function rt(e, t) {
...
function nt(e, t) {
  const a = O(e.state), o = t === A && a.length > 0, r = o ? null : e.state.cameras.find((s) => s.id === t);
...
const _e = "omnicam_extractor_result_v2", L = "omnicam_extracted_motion_scene_json", z = "omnicam_extracted_track_fingerprint", Z = "omnicam_extractor_source";
function U(e) {
...
function Me(e, t, a = {}) {
  ...
  const r = a.mode || (Ne(e) ? "replace" : "merge"), s = a.reconMode || Ae(t);
  if (r === "replace") {
    ...
    for (const l of e.state.objects || [])
      F(l, s), (l.type === "glb" || l.type === "model") && l.asset && e.modelUrlsById?.set(l.id, P(l.asset));
  } else {
    ...
    for (const m of c)
      m.parent_id && i.has(m.parent_id) && (m.parent_id = i.get(m.parent_id)), F(m, s), e.state.objects.push(m), (m.type === "glb" || m.type === "model") && m.asset && e.modelUrlsById?.set(m.id, P(m.asset));
    for (const m of ee(o)) {
      const f = D(d, m.id);
...
function Te(e) {
  ...
  for (const o of t.inputs || []) {
    if (String(o?.name || "").toLowerCase() !== Le || o.link == null) continue;
    const r = se(a, o.link);
...
function bt(e) {
  ...
  let o = !1;
  return a ? a.fingerprint !== Re(e) && e.pendingExtractorImport?.fingerprint !== a.fingerprint && (e.pendingExtractorImport = {
    track: a.track,
    fingerprint: a.fingerprint,
    originNodeId: t.id,
    label: String(t.title || v("OmniCam Extractor")),
    keyCount: a.track.keyframes?.length || 0
  }, We(e, a.fingerprint, t), o = !0) : e.pendingExtractorImport && (e.pendingExtractorImport = null, o = !0), W(e), o;
}
function vt(e) {
  const t = e.pendingExtractorImport;
  return t ? (e.checkpoint("Import extracted camera"), ye(e, t.track, { label: t.label }), e.pendingExtractorImport = null, W(e), e.setStatus?.(v("Imported {count} camera keys from {name}").replace("{count}", String(t.keyCount)).replace("{name}", t.label)), e.scheduleSerialize(), e.render(), !0) : !1;
}
function xt(e) {
...
function yt(e) {
  ...
  for (const s of a)
    for (const l of s?.links || []) {
      const d = H(t, l), i = d?.target_id ?? d?.targetId;
...
function wt(e, t) {
  ...
  for (const l of o)
    for (const d of l?.links || []) {
      const i = H(a, d), c = i?.target_id ?? i?.targetId;
