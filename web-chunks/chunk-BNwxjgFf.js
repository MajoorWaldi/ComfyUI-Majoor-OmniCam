import { G as Q, D as ze, M as re, P as xe, a as Be, S as ht, b as ft, C as pt, c as pe, E as gt, A as wt, d as de, N as yt, e as We, f as Mt, B as je, g as xt, h as bt, i as vt, j as Ct, k as St, l as Bt, m as Ne, n as ue, F as Lt, o as Gt, H as Ie, L as Pt, p as Vt, q as _t, r as kt, s as Dt, t as At, u as Ot, v as me, O as Ue, w as Re, x as qe, y as Tt, z as $e, I as Ke, Q as Ft, R as Xe, J as Ye, K as Ze, T as zt, U as Qe, V as Je, W as Wt, X as jt, Y as He, Z as Nt, _ as It, $ as Le, a0 as be, a1 as Ee, a2 as et, a3 as Ut, a4 as Rt, a5 as qt, a6 as $t, a7 as Kt, a8 as tt, a9 as rt, aa as st, ab as at, ac as ot } from "./chunk-B7ZTbDAV.js";
import { a3 as Xt, D as Yt, s as Zt, f as Qt } from "./chunk-DXtw26Dt.js";
import { a as Ae, s as nt, D as Oe, c as Jt, b as Ht, d as Et } from "./chunk-D2gOZ0kR.js";
import { O as er, B as tr, W as rr, C as sr, Q as ar, c as or } from "./chunk-CcqF7PHi.js";
...
function nr(t, { position: e, forward: n, up: l, color: v, scale: h = 1, active: S = !0 }) {
  const C = new t.Group(), O = S ? 0.95 : 0.5, G = new t.MeshBasicMaterial({
    color: v,
    transparent: !0,
    opacity: O,
    depthTest: !1
  }), A = new t.Mesh(new t.BoxGeometry(0.34, 0.24, 0.42), G);
  A.renderOrder = 912, C.add(A);
  const _ = new t.Mesh(new t.ConeGeometry(0.17, 0.26, 20), G);
  return _.rotation.x = -Math.PI / 2, _.position.z = -0.32, _.renderOrder = 912, C.add(_), C.scale.setScalar(h), C.position.copy(e), C.up.copy(l), C.lookAt(e.clone().add(n)), C;
}
function ir(t, { position: e, color: n = 15903035, radius: l = 0.28, bold: v = !1 }) {
  const h = new t.Group(), S = v ? 16773544 : n, C = new t.LineBasicMaterial({ color: S, transparent: !0, opacity: v ? 1 : 0.95, depthTest: !1 }), O = (_) => {
    const k = [];
    for (let q = 0; q <= 48; q++) {
  ...
  const G = l * 1.55, A = new t.LineSegments(
  ...
  return A.renderOrder = 915, h.add(A), h.position.copy(e), h.userData.omnicamBillboard = !0, h;
}
const Ce = 3718648, cr = 12e3;
function he(t) {
...
function lr(t, e) {
  ...
  return { overlay: fe(n), parent: e };
}
function dr(t, e) {
  const n = new t.PointsMaterial({ color: Ce, size: 0.05, sizeAttenuation: !0 });
  if (!he(e)) {
    const _ = new t.Points(e.geometry, n);
    return { overlay: fe(_), parent: e };
  }
  const l = e.geometry.getAttribute("position")?.count || 0, v = Math.max(1, Math.ceil(l / cr)), h = Math.ceil(l / v), S = new Float32Array(h * 3), C = new t.BufferGeometry();
  C.setAttribute("position", new t.Float32BufferAttribute(S, 3));
  const O = new t.Points(C, n);
  it(O, e);
  const G = new t.Vector3(), A = C.getAttribute("position");
  return O.onBeforeRender = () => {
    for (let _ = 0; _ < h; _++)
      e.getVertexPosition(_ * v, G), A.setXYZ(_, G.x, G.y, G.z);
    A.needsUpdate = !0;
  }, { overlay: fe(O), parent: e.parent || e };
}
function ur(t, e, n) {
  const l = he(e) ? new t.SkinnedMesh(e.geometry.clone(), n) : new t.Mesh(e.geometry.clone(), n);
...
function mr(t, e, { wireframe: n = !1, vertices: l = !1 } = {}) {
  if (!n && !l) return;
  const v = [];
  e.traverse((h) => {
  ...
  for (const h of v) {
    ...
    if (l) {
      const { overlay: S, parent: C } = dr(t, h);
      C.add(S);
    }
  }
}
function hr(t) {
  const { THREE: e, FBXLoader: n, GLTFLoader: l, OBJLoader: v, PLYLoader: h, STLLoader: S, neutral: C, wire: O, checkerMaterial: G, objectMaterial: A, applyModelMaterial: _, disposeObject: k, textureFor: X, cardMesh: N, generatePointField: q, sampleCamera: I, sampleObjectTransform: J } = t;
  return {
    removeModel(x) {
    ...
    rebuild(x, y, a) {
      ...
      if (m.userData.omnicamCaptureGuide = !0, m.frustumCulled = !1, this.content.add(m), ["omni_ref", "point_field"].includes(c)) {
        const { points: s, colors: r } = q(x.point_density || "balanced", x.point_spread || "all_views", x.point_color || null);
        if (s.length > 0) {
          ...
          i.frustumCulled = !1, this.content.add(i);
        }
      }
      if (!["grid", "point_field"].includes(c))
        for (const s of x.objects) {
          ...
          if (s.type === "glb" || s.type === "model") {
            const u = a.get(s.id), i = this.models.get(s.id), f = s.format || (s.type === "glb" ? "glb" : "");
            u && (i?.url !== u || i?.format !== f) && this.loadModel(s.id, u, f), i?.url === u ? (g = i.scene, _(g, s.material_mode || "textured")) : g = new e.Mesh(new e.BoxGeometry(r[0], r[1], r[2] || 1), O.clone());
          } else if (s.type === "sphere") g = new e.Mesh(new e.SphereGeometry(0.5, 24, 16), A(s, c));
          else if (s.type === "ground") g = new e.Mesh(new e.BoxGeometry(1, 1, 1), A(s, c));
          else if (s.type === "card")
            g = s.material_mode && s.material_mode !== "textured" ? new e.Mesh(new e.PlaneGeometry(r[0], r[1]), A(s, c)) : N(s, y.get(s.id), x.card_fit || "contain");
          else if (s.type === "null") {
            ...
            g = new e.Mesh(new e.BoxGeometry(1, 1, 1), A(s, c));
          ...
          }), mr(e, g, { wireframe: x.show_wireframe, vertices: x.show_vertices }), this.objectNodes.set(s.id, g), this.content.add(g);
        }
    },
    rebuildPath(x, y = "camera", a = null) {
      ...
      (x.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: x.keyframes || [] }]).forEach((r, g) => {
        ...
        const i = r.color ? { line: new e.Color(r.color), marker: new e.Color(r.color), frustum: new e.Color(r.color) } : c[g % c.length], f = r.id === x.active_camera_id, o = f && y === "camera";
        if (u.length >= 2) {
          ...
          if (V.renderOrder = 900, V.userData.omnicamWidget = "path", this.path.add(V), f) {
            const T = new e.Mesh(
              ...
              new e.MeshBasicMaterial({ color: i.line, transparent: !0, opacity: o ? 0.3 : 0.18, depthTest: !1 })
            );
        ...
        for (const w of u) {
          const b = new e.Mesh(
            ...
            new e.MeshBasicMaterial({ color: i.marker, depthTest: !1 })
          );
          ...
          if (L) {
            ...
            const D = new e.BufferGeometry().setFromPoints(d), $ = new e.LineSegments(D, new e.LineBasicMaterial({
              color: i.marker,
              transparent: !0,
              opacity: 1,
              depthTest: !1
      ...
      (x.objects || []).forEach((r, g) => {
        ...
        const i = r.color ? new e.Color(r.color) : s[g % s.length], f = u.map((b) => new e.Vector3().fromArray(b.transform?.position || [0, 0, 0])), o = new e.CatmullRomCurve3(f, !1, "centripetal"), w = new e.Mesh(
          new e.TubeGeometry(o, Math.max(32, u.length * 16), 0.035, 8, !1),
          new e.MeshBasicMaterial({ color: i, transparent: !0, opacity: 0.9, depthTest: !1 })
        );
        w.renderOrder = 900, w.userData.omnicamWidget = "path", this.path.add(w);
        for (const b of u) {
          const p = new e.Mesh(
            new e.BoxGeometry(0.14, 0.14, 0.14),
            new e.MeshBasicMaterial({ color: i, depthTest: !1 })
          );
...
function fr(t) {
  const { THREE: e, FBXLoader: n, GLTFLoader: l, OBJLoader: v, PLYLoader: h, STLLoader: S, neutral: C, wire: O, checkerMaterial: G, objectMaterial: A, applyModelMaterial: _, disposeObject: k, textureFor: X, cardMesh: N, generatePointField: q, sampleCamera: I, sampleObjectTransform: J, hasOutlineMesh: x } = t;
  return {
    updateLiveCameras(y, a, c, m, s = "camera", r = null) {
      ...
      (y.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: y.keyframes || [] }]).forEach((i, f) => {
        const o = i.color ? { line: new e.Color(i.color), marker: new e.Color(i.color), frustum: new e.Color(i.color), body: new e.Color(i.color).multiplyScalar(0.35) } : g[f % g.length], w = i.id === y.active_camera_id, b = w && s === "camera";
        if (m === "camera" && w) return;
        const p = I(i, a, y.objects), M = new e.Vector3().fromArray(p.position || [0, 0, 0]), L = new e.Vector3().fromArray(p.target || [0, 0, 0]), B = L.clone().sub(M), P = B.length();
        ...
        D.position.set(0, 0.07, -0.08), F.add(D);
        const $ = new e.Matrix4().makeBasis(V, T, B.clone().negate());
        F.quaternion.setFromRotationMatrix($), F.position.copy(M), F.userData.omnicamWidget = "gizmo", this.liveCameras.add(F);
        const R = new e.SphereGeometry(0.35, 8, 6), z = new e.MeshBasicMaterial({ transparent: !0, opacity: 0, depthWrite: !1 }), K = new e.Mesh(R, z);
        K.position.copy(M), K.userData = { omnicamType: "camera", omnicamId: i.id }, this.liveCameras.add(K);
        ...
        if (_e.userData.omnicamWidget = "gizmo", this.liveCameras.add(_e), P > 0.01) {
          ...
          if (we.position.copy(L), we.userData = { omnicamType: "camera_target", omnicamId: i.id }, this.liveCameras.add(we), (W || b) && m !== "camera") {
            const De = new e.RingGeometry(0.14, 0.18, 24);
            De.rotateX(Math.PI / 2);
            const mt = new e.MeshBasicMaterial({ color: 9133302, side: e.DoubleSide, transparent: !0, opacity: 0.9 }), ye = new e.Mesh(De, mt);
      ...
      if (g !== this.selectionKey) {
        if (this.selectionKey = g, k(this.selectionGroup), this.selectionGroup.clear(), a === "object" && c) {
          ...
          if (u) {
            ...
            try {
              const i = new e.Box3(), f = [];
              if (u.traverse((o) => {
                o.isBone && f.push(o);
              }), f.length > 0) {
                const o = new e.Vector3();
                for (const w of f)
                  w.getWorldPosition(o), i.expandByPoint(o);
                i.expandByScalar(0.2);
              } else
                i.setFromObject(u);
              if (!x(u) && !i.isEmpty() && Number.isFinite(i.min.x) && Number.isFinite(i.max.x) && Number.isFinite(i.min.y) && Number.isFinite(i.max.y) && Number.isFinite(i.min.z) && Number.isFinite(i.max.z)) {
                i.expandByScalar(0.04);
                const o = new e.Box3Helper(i, new e.Color(9133302));
            ...
            if (y.show_wireframe) {
              let i = 0;
              u.traverse((f) => {
                if (!f.isMesh || !f.geometry || f.userData.omnicamHelper || i >= 64) return;
                const o = ur(e, f, new e.MeshBasicMaterial({
                  color: 9133302,
                  transparent: !0,
                  opacity: 0.2,
                  depthTest: !0,
                ...
                o.renderOrder = 9998, this.selectionGroup.add(o), i += 1;
              });
            }
            if (m && m.objectId === c && m.point) {
              if (m.mode === "vertex") {
                const i = new e.SphereGeometry(0.08, 16, 12), f = new e.MeshBasicMaterial({ color: 16096779, depthTest: !1 }), o = new e.Mesh(i, f);
                ...
                const [i, f] = m.edge, o = new e.BufferGeometry().setFromPoints([new e.Vector3(...i), new e.Vector3(...f)]), w = new e.LineBasicMaterial({ color: 16096779, linewidth: 5, depthTest: !1 }), b = new e.Line(o, w);
                b.renderOrder = 1e4, this.selectionGroup.add(b);
              } else if (m.mode === "face" && m.vertices) {
                const [i, f, o] = m.vertices, w = new e.BufferGeometry().setFromPoints([
                  new e.Vector3(...i),
                  new e.Vector3(...f),
                  new e.Vector3(...o)
                ]);
        ...
        if (a === "object")
          for (const u of y.__selectedObjectIds || []) {
            if (u === c) continue;
            const i = this.objectNodes.get(u);
            if (i) {
              i.updateMatrixWorld(!0);
              try {
                const f = new e.Box3().setFromObject(i);
                if (!x(i) && !f.isEmpty() && Number.isFinite(f.min.x)) {
    ...
    sampleModelPoint(y, a, c, m = 24) {
      ...
      if (a) {
        ...
        i = this.getObjectWorldCenter(y);
      return g && Number.isFinite(u) && (r.mixer.setTime(u), s.updateMatrixWorld(!0)), i;
    },
...
function pr(t) {
  ...
  return {
    ...
    configureCamera(x, y) {
      ...
      let i = a.up ? new e.Vector3().fromArray(a.up) : new e.Vector3(0, 1, 0), f = new e.Vector3().crossVectors(u, i);
      if (f.lengthSq() < 1e-6 && (i = Math.abs(u.y) > 0.9 ? new e.Vector3(0, 0, u.y > 0 ? -1 : 1) : new e.Vector3(0, 1, 0), f.crossVectors(u, i)), f.normalize(), i.crossVectors(f, u).normalize(), a.roll) {
        const o = e.MathUtils.degToRad(a.roll);
        f.applyAxisAngle(u, o), i.applyAxisAngle(u, o);
      }
      return s.position.copy(r), s.up.copy(i), s.lookAt(g), s.updateMatrixWorld(), s;
    },
    pick(x, y, a, c) {
    ...
    pickSubElement(x, y, a, c, m = "vertex") {
      ...
      for (const r of s) {
        ...
        const i = g.userData.omnicamId, o = u.geometry.getAttribute("position");
        if (!o) continue;
        u.updateMatrixWorld(!0);
...
const Me = ["high", "balanced", "low"], gr = 25, Te = 30, wr = 0.6;
...
function Mr(t, e) {
  if (!Number.isFinite(e) || e < 0 || (t.samples.push(e), t.samples.length > Te && t.samples.shift(), t.samples.length < Te) || t.samples.filter((v) => v > gr).length / t.samples.length < wr) return null;
  const l = yr(t.quality);
...
function br(t) {
  const { THREE: e, FBXLoader: n, GLTFLoader: l, OBJLoader: v, PLYLoader: h, STLLoader: S, neutral: C, wire: O, checkerMaterial: G, objectMaterial: A, applyModelMaterial: _, disposeObject: k, textureFor: X, cardMesh: N, generatePointField: q, sampleCamera: I, sampleObjectTransform: J, hasOutlineMesh: x, SelectionOutlineRenderer: y } = t;
  return {
    render(a, c, m, s, r, g = /* @__PURE__ */ new Map(), u = 0, i = !1, f = "camera", o = "subject", w = null, b = null) {
      const p = !i || (a.render_mode || "") === "beauty";
      ...
      if (M) {
        ...
        if (d)
          ...
          this.bgTextureLoads.set(M, D), new e.TextureLoader().load(M, (R) => {
            if (this.bgTextureLoads.delete(M), this.disposed || D !== this.bgLoadGeneration) {
              R.dispose();
              return;
            }
      ...
      const L = JSON.stringify([
        ...
        !!a.show_vertices,
        a.objects.map((d) => {
          const { position: D, rotation: $, keyframes: R, size: z, ...K } = d;
          return d.type === "card" && (K.size = z), K;
        })
      ]), B = [...m.entries()].map(([d, D]) => `${d}:${D?.src || ""}`).join("|"), P = [...g.entries()].map(([d, D]) => `${d}:${D}`).join("|");
      ...
      for (const d of a.objects) {
        ...
        if (!D) continue;
        const $ = d.keyframes?.length ? J(d, u) : d;
        D.position.fromArray($.position || [0, 0, 0]), D.rotation.set(...($.rotation || [0, 0, 0]).map(e.MathUtils.degToRad)), d.type !== "card" && d.type !== "null" && D.scale.fromArray($.size || [1, 1, 1]), d.type === "null" && (D.visible = i ? !0 : a.show_helper_axes !== !1);
      }
      this.path.visible = !i;
      const j = a.show_grid !== !1 && a.render_mode !== "point_field";
      this.content.traverse((d) => {
        d.userData.omnicamCaptureGuide && (d.visible = i ? !!a.playblast_grid : j);
      });
      ...
      if (V !== this.pathKey && (this.pathKey = V, this.rebuildPath(a, f, b)), this.updateLiveCameras(a, u, i, a.view_mode || "camera", f, b), this.liveCameras.visible = !i, !i) {
        const d = a.show_camera_paths !== !1, D = a.show_camera_gizmos !== !1, $ = a.show_look_at !== !1;
        for (const R of [this.path, this.liveCameras])
          R.traverse((z) => {
            const K = z.userData.omnicamWidget;
            K === "path" ? z.visible = d : K === "gizmo" ? z.visible = D : K === "lookat" && (z.visible = $);
          });
      }
      i ? this.selectionGroup.visible = !1 : (this.updateSelection(a, f, o, w, `${a.__omnicamRevision ?? "legacy"}:${u}`), this.selectionGroup.visible = !0), this.studioEnabled && this.contentShadowKey !== this.sceneKey && (this.contentShadowKey = this.sceneKey, this.content.traverse((d) => {
      ...
      if (Y || this.renderer.render(this.scene, F), !i && this.adaptiveQuality !== !1) {
        this.qualityMonitor ||= Fe(this.studio?.quality);
        const d = Mr(this.qualityMonitor, performance.now() - U);
        d && (Ae(this.studio, this.renderer, d), this.onQualityDowngrade?.(d));
      }
    },
    setViewportQuality(a) {
      Ae(this.studio, this.renderer, a), this.qualityMonitor = xr(this.qualityMonitor || Fe(a), a);
    },
    dispose() {
      if (!this.disposed) {
...
function Br(t, e) {
  ...
  const n = se.clone();
  return t.color && (n.color = new de(t.color)), n;
}
function Lr(t, e) {
  t.traverse((n) => {
    if (n.isMesh) {
      ...
      e === "textured" ? n.material = n.userData.omnicamOriginalMaterial : (n.material = e === "checker" ? Pe() : e === "wireframe" ? Ge.clone() : se.clone(), n.userData.omnicamOverrideMaterial = !0);
    }
  });
}
function Se(t, e = !1) {
...
function Gr(t, e, n) {
  ...
  const C = ct(e);
  if (!C) return h;
  const O = e.videoWidth || e.naturalWidth || e.width || l, G = e.videoHeight || e.naturalHeight || e.height || v, A = O / Math.max(1, G), _ = l / Math.max(0.01, v);
  let k = l, X = v;
  n === "contain" ? A > _ ? X = l / A : k = v * A : n === "cover" && (A > _ ? (C.repeat.x = _ / A, C.offset.x = (1 - C.repeat.x) * 0.5) : (C.repeat.y = A / _, C.offset.y = (1 - C.repeat.y) * 0.5));
  const N = new re(
    new xe(k, X),
    new me({
      color: 16777215,
...
class Pr {
  constructor(e = () => {
    ...
    l.position.set(5, 8, 4), this.scene.add(l), this.flatLights = [this.scene.children.at(-2), l], this.studio = Jt(ve, this.renderer, Et), this.scene.add(this.studio.group), this.studioEnabled = !0, nt(ve, this.scene, this.renderer, this.studio, !0), this.content = new Q(), this.scene.add(this.content), this.path = new Q(), this.scene.add(this.path), this.liveCameras = new Q(), this.scene.add(this.liveCameras), this.selectionGroup = new Q(), this.scene.add(this.selectionGroup), this.selectionKey = "", this.perspective = new qe(35, 16 / 9, 0.01, 1e4), this.orthographic = new Ue(-5, 5, 2.8125, -2.8125, 0.01, 1e4), this.sceneKey = "", this.mediaSignature = "", this.bgImageUrl = "", this.bgTexture = null, this.bgTextureCache = /* @__PURE__ */ new Map(), this.bgTextureLoads = /* @__PURE__ */ new Map(), this.bgLoadGeneration = 0, this.disposed = !1, this.invalidate = e, this.onModelLoaded = n, this.modelUrls = /* @__PURE__ */ new Map(), this.models = /* @__PURE__ */ new Map(), this.modelLoads = /* @__PURE__ */ new Map(), this.objectNodes = /* @__PURE__ */ new Map(), this.raycaster = new Ye(), this.pointer = new Le(), this.activeCamera = this.perspective;
  }
  async loadModel(e, n, l = "glb") {
    ...
    if (!(!n || this.modelLoads.get(e) === v)) {
      ...
      try {
        let h, S = [];
        if (l === "obj") h = await new tt().loadAsync(n);
        else if (l === "fbx")
          h = await new rt().loadAsync(n), S = h.animations || [];
        else if (l === "stl") h = new re(await new st().loadAsync(n), se.clone());
        else if (l === "ply") {
          const c = await new at().loadAsync(n);
          c.index ? (c.getAttribute("normal") || c.computeVertexNormals(), h = new re(c, se.clone())) : h = new $e(c, new Ke({ color: 11449792, size: 0.025 }));
        } else {
          const c = await new ot().loadAsync(n);
        ...
        let O = 0, G = 0, A = 0, _ = 0;
        h.traverse((c) => {
          c.isMesh && (O += 1, _ += c.geometry?.getAttribute?.("position")?.count || 0), c.isPoints && (G += 1), c.isBone && (A += 1);
        });
        const k = new Q();
        if (k.frustumCulled = !1, k.add(h), !O && !G && A) {
          const c = new Je(h);
        ...
        const X = new je().setFromObject(k), N = X.getSize(new be()), q = Math.max(N.x, N.y, N.z), I = Number.isFinite(q) && q > 1e-6 ? 2.5 / q : 1, J = X.getCenter(new be());
        ...
        const a = { url: n, format: l, scene: x, mixer: y, clips: S, selectedClip: 0, duration: S[0]?.duration || 0, meshes: O, points: G, bones: A, vertices: _, animations: S.length, normalizationScale: I };
        this.models.set(e, a), this.onModelLoaded({ id: e, format: l, meshes: O, points: G, bones: A, vertices: _, animations: S.length, animationNames: S.map((c, m) => c.name || `Clip ${m + 1}`), duration: a.duration, normalizationScale: I }), this.sceneKey = "", this.invalidate();
      } catch (h) {
...
const ce = { THREE: ve, FBXLoader: rt, GLTFLoader: ot, OBJLoader: tt, PLYLoader: at, STLLoader: st, neutral: se, wire: Ge, checkerMaterial: Pe, objectMaterial: Br, applyModelMaterial: Lr, disposeObject: Se, textureFor: ct, cardMesh: Gr, generatePointField: Yt, sampleCamera: Zt, sampleObjectTransform: Qt, hasOutlineMesh: Cr, SelectionOutlineRenderer: Sr };
Object.assign(
  Pr.prototype,
  hr(ce),
  fr(ce),
...
async function Vr(t, e) {
  ...
  for (const n of ["vp9", "vp8"])
    try {
      if (await le(or(n, { width: t, height: e }), 5e3, `Checking ${n} support`)) return n;
    } catch {
    }
  return null;
}
...
async function Tr(t, e, n, l, v) {
  const h = await Vr(t.width, t.height);
  if (!h) throw new Error("No supported WebCodecs WebM encoder");
  const S = new er({ format: new rr(), target: new tr() }), C = new sr(t, { codec: h, quality: new ar("high"), keyFrameInterval: 1 });
  S.addVideoTrack(C, { frameRate: n }), await le(S.start(), 1e4, "Starting deterministic encoder");
  try {
    const O = 1 / n;
    for (let G = 0; G < e; G++) {
      if (v?.aborted) throw new DOMException("Playblast cancelled", "AbortError");
      await l(G), await le(C.add(G * O, O, { keyFrame: G % n === 0 }), 1e4, `Encoding frame ${G + 1}`);
    }
  ...
  return Ht(new Blob([S.target.buffer], { type: await S.getMimeType() }), {
    encoder: "webcodecs",
    requestedFrames: e,
...
export {
  Pr as OmniWebGLViewport,
  Tr as encodeDeterministicPlayblast,
  Vr as supportsDeterministicEncoding
};
