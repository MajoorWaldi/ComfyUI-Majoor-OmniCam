...
function io(e) {
  e.registerExtension({
    ...
    beforeRegisterNodeDef(t, a) {
      if (!String(a?.name || a?.node_id || t?.comfyClass || t?.type || "").startsWith(ro)) return;
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        const i = Math.max(4, Number(this.size?.[0] || 160) - J - 6), l = -26, d = i + J / 2, c = l + J / 2;
...
function bo(e, t, a = null) {
  ...
  return r.push({ camera_id: a || go(e, s), start: o }), r.sort((i, l) => i.start - l.start), !0;
}
function yo(e, t) {
...
function _o(e) {
  ...
  return e.motion_layers = (Array.isArray(e.motion_layers) ? e.motion_layers : []).slice(0, 256).map((r, o) => {
    ...
    const s = ua.includes(r?.source_kind) ? r.source_kind : "manual_2d", i = (Array.isArray(r?.keys) ? r.keys : []).slice(0, 1e4).map((l) => vo(l, t)).sort((l, d) => l.time_seconds - d.time_seconds);
    return {
      id: n,
...
const So = 32, wo = 64, Co = 128, Mo = Object.freeze(["off", "selected", "all"]), xo = Object.freeze(["annotation", "name", "tag"]), Pt = Object.freeze({ mode: "selected", content: "annotation" }), ko = Object.freeze(["top", "center", "bottom"]), Ao = /^[a-z0-9][a-z0-9_-]*$/, To = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/, Do = /[<>]|:\/\/|javascript:|expression\(|&#/i;
function nt(e) {
  ...
  for (const r of e) {
    ...
    if (!(!o || o.length > wo || !Ao.test(o) || a.has(o)) && (a.add(o), t.push(o), t.length >= So))
      break;
  }
  return t;
}
...
function Nt(e) {
  ...
  if (!t || t.length > Co || Do.test(t)) return null;
  const a = typeof e.color == "string" ? e.color.trim() : "", r = To.test(a) ? a.toLowerCase() : "#8d7ee8", o = ko.includes(e.anchor) ? e.anchor : "top";
  return { text: t, visible: e.visible !== !1, color: r, anchor: o };
}
function Ci(e) {
  return {
    mode: Mo.includes(e?.mode) ? e.mode : Pt.mode,
    content: xo.includes(e?.content) ? e.content : Pt.content
  };
}
function jo(e) {
...
function Mi(e, t) {
  ...
  if (t === "tag") return jo(e);
  const a = e.annotation;
  return a && a.visible !== !1 ? String(a.text || "") : "";
}
function xi(e, { mode: t, selectedIds: a } = {}) {
  return !e || e.enabled === !1 || t === "off" ? !1 : t === "all" ? !0 : (a instanceof Set ? a : new Set(a || [])).has(e.id);
}
const Eo = /* @__PURE__ */ new Set(["camera", "null", "sun_light", "point_light", "spot_light"]);
function ki(e, t) {
  const a = Array.isArray(e?.position) ? e.position : [0, 0, 0], r = Array.isArray(e?.size) ? e.size : [1, 1, 1], o = Eo.has(t) ? 0.35 : Math.max(0.2, (Number(r[1]) || 1) * 0.5 + 0.2);
  return [Number(a[0]) || 0, (Number(a[1]) || 0) + o, Number(a[2]) || 0];
}
const zt = Object.freeze([0.05, 8]), Io = 80, fa = (e, t, a) => Math.max(t, Math.min(a, e));
function ha(e) {
  if (!e || typeof e != "object") return null;
  const t = String(e.clip_id ?? "").trim();
  if (!t || t.length > Io) return null;
  const a = Math.max(0, Math.round(Number(e.start_frame) || 0)), r = Math.round(Number(e.end_frame) || 0), o = r > a ? r : 0, n = Number(e.speed), s = fa(Number.isFinite(n) && n > 0 ? n : 1, zt[0], zt[1]), i = Number(e.offset_seconds);
  return {
    clip_id: t,
    start_frame: a,
    end_frame: o,
...
function Ai(e, t, a, r) {
  ...
  if (!o || n <= 0) return 0;
  const i = (u) => o.loop ? (u % n + n) % n : fa(u, 0, n);
  if (t <= o.start_frame) return i(o.offset_seconds);
  const l = o.end_frame > o.start_frame ? o.end_frame : null, d = l !== null && !o.loop ? Math.min(t, l) : t, c = o.offset_seconds + (d - o.start_frame) * o.speed / s;
  return i(c);
}
const Ti = "omnicam_humanoid_v1", gt = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
  ...
  "foot_r",
  "toe_r"
]), Oo = Object.freeze(["eye_l", "eye_r", "hand_tip_l", "hand_tip_r"]);
Object.freeze([...gt, ...Oo]);
new Map(gt.map((e, t) => [e, t]));
const Po = /^mixamorig[:_ ]?/i, No = /[\s_\-.:|]+/g, zo = {
...
function Ro(e) {
  return String(e || "").trim().replace(Po, "").replace(No, "").toLowerCase();
}
function Lo(e) {
  const t = [e];
...
function Di(e) {
  const a = (e || []).map(String).filter((n) => n.trim()).map((n) => [n, Lo(Ro(n))]), r = /* @__PURE__ */ new Set(), o = {};
  for (const n of [!0, !1])
...
function Ko(e) {
  return Bo(e).length === 0;
}
function ji(e) {
  const t = e?.bone_map || e?.rig?.bone_map || null;
  return !t || !Object.keys(t).length ? "none" : Ko(t) ? "rigged" : "incomplete";
}
const Vo = 128, Rt = "omnicam_humanoid_v1", bt = /^[a-z0-9][a-z0-9_-]*$/, Go = 1e-3, qo = (e) => Array.isArray(e) && e.length === 3 && e.every((t) => Number.isFinite(Number(t)));
function yt(e) {
  ...
  return a <= 1e-8 || Math.abs(a - 1) > Go && !(a > 0.5 && a < 2) ? null : t.map((r) => r / a);
}
...
function Ho(e) {
  ...
  for (const [r, o] of Object.entries(e)) {
    ...
    if (!bt.test(n) || n.length > 64) continue;
    const s = yt(o);
    !s || ga(s) || (t[n] = s, a += 1);
  }
  return t;
...
function Re(e) {
  ...
  return {
    preset_id: bt.test(a) && a.length <= 80 ? a : "neutral",
    root_offset: qo(t.root_offset) ? t.root_offset.map(Number) : [0, 0, 0],
    joints: Ho(t.joints)
  };
}
function Ei({ preset: e, overrides: t } = {}) {
  const a = Re(e), r = Re(t);
  return {
...
function Ii(e, t, a) {
  const r = Re(e), o = String(t || "").trim().toLowerCase();
  if (!bt.test(o)) return r;
  const n = yt(a);
  return !n || ga(n) ? delete r.joints[o] : r.joints[o] = n, r;
}
function Wo(e) {
  return !e || typeof e != "object" ? null : {
    rig_profile: e.rig_profile === Rt ? Rt : null,
    pose: Re(e.pose),
    motion: ha(e.motion)
  };
}
const vt = 1, ba = 0.1, ya = 10;
function va(e) {
  return Number.isFinite(e) && e >= ba && e <= ya;
}
function $o(e) {
...
function Pi(e, { startFrame: t, endFrame: a } = {}) {
  ...
  const i = n.map((m) => $o(m)), l = [];
  for (let m = 1; m < n.length; m += 1) {
    const f = Uo(Lt(n[m - 1]), Lt(n[m])), w = (i[m - 1] + i[m]) / 2;
    l.push(f * w);
  }
...
function H(e) {
  ...
  Math.sqrt(Z(o, o)) < 1e-6 && (r = Math.abs(a[1]) > 0.9 ? [0, 0, a[1] > 0 ? -1 : 1] : [0, 1, 0], o = Me(a, r)), o = ie(o);
  let n = ie(Me(o, a));
  if (Math.abs(e.roll || 0) > 1e-9) {
    const s = e.roll * Math.PI / 180, i = Math.cos(s), l = Math.sin(s), d = M(T(o, i), T(n, l));
    n = M(T(n, i), T(o, -l)), o = d;
  }
...
function de(e, t, a = null) {
  ...
  const o = Zo(e, r, t), n = R(r, t, "pos_x", (g) => (g.camera || g).position[0], !1, o), s = R(r, t, "pos_y", (g) => (g.camera || g).position[1], !1, o), i = R(r, t, "pos_z", (g) => (g.camera || g).position[2], !1, o);
  let l = R(r, t, "target_x", (g) => (g.camera || g).target[0], !1, o), d = R(r, t, "target_y", (g) => (g.camera || g).target[1], !1, o), c = R(r, t, "target_z", (g) => (g.camera || g).target[2], !1, o);
  ...
  if (h && Array.isArray(m)) {
    const g = m.find((x) => x.id === h);
    if (g && g.enabled !== !1) {
      const x = St(m, g, t), E = u?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
      l = (x.position?.[0] ?? 0) + (E[0] || 0), d = (x.position?.[1] ?? 1.5) + (E[1] || 0), c = (x.position?.[2] ?? 0) + (E[2] || 0);
    }
  }
  const f = R(r, t, "fov", (g) => Number((g.camera || g).fov ?? 35), !1, o), w = R(r, t, "roll", (g) => Number((g.camera || g).roll ?? 0), !0, o), y = R(r, t, "zoom", (g) => Number((g.camera || g).zoom ?? 1), !1, o), S = R(r, t, "near", (g) => Number((g.camera || g).near ?? 0.01), !1, o), b = R(r, t, "far", (g) => Number((g.camera || g).far ?? 1e4), !1, o), C = r[0]?.camera || r[0] || le();
  let _ = r[0];
  for (const g of r)
    if ((g.frame ?? 0) <= t) _ = g;
    else break;
...
function Qo(e) {
  ...
  return !Number.isFinite(t) || t < ba || t > ya || t === vt ? {} : { timing: { weight: t } };
}
const D = (e, t, a) => Math.max(t, Math.min(a, e)), Jo = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, fe = (e, t = null) => typeof e == "string" && Jo.test(e.trim()) ? e.trim() : t, M = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], j = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], T = (e, t) => [e[0] * t, e[1] * t, e[2] * t], Z = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], Me = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], W = (e) => Math.sqrt(Math.max(1e-12, Z(e, e))), ie = (e) => T(e, 1 / W(e));
function Sa(e, t, a) {
...
const Ft = [
  ...
  "ease_expo",
  "ease_back"
], tn = ["auto", "clamped", "vector", "free", "aligned", "flat"];
function an(e, t) {
  const a = e?.tangents;
  return !a || typeof a != "object" ? {} : a.channels && typeof a.channels == "object" && a.channels[t] ? a.channels[t] : a;
}
function Bt(e, t, a, r, o) {
  const n = an(e, t), s = tn.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", i = o ? o(e) : 0, l = a && o ? o(a) : i, d = r && o ? o(r) : i, c = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), u = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), p = (S = !1) => {
    const b = (i - l) / c, C = (d - i) / u;
    let _ = 0;
    if (!a && !r)
      _ = 0;
    else if (!a)
      ...
      if (_ = b * A + C * g, S) {
        const x = 3 * Math.min(Math.abs(b), Math.abs(C));
        _ = D(_, -x, x);
      }
    }
    return {
  ...
  if (s === "auto")
    return { ...p(!1), mode: s };
  const h = p(!1), m = D(Number(n.out_x ?? h.out_x), 0.01, 0.99), f = Number(n.out_y ?? h.out_y);
  let w = D(Number(n.in_x ?? h.in_x), -0.99, -0.01), y = Number(n.in_y ?? h.in_y);
  if (s === "aligned") {
    const S = Math.hypot(m, f) || 1e-6, b = Math.hypot(w, y) || 1e-6;
    w = -m / S * b, y = -f / S * b;
  }
  return { out_x: m, out_y: f, in_x: w, in_y: y, mode: s };
}
function _t(e, t) {
...
function R(e, t, a, r, o = !1, n = null) {
  if (!e.length) return 0;
  if (t <= e[0].frame) return r(e[0]);
  if (t >= e[e.length - 1].frame) return r(e[e.length - 1]);
  const s = n || _t(e, t), { leftIndex: i, left: l, right: d } = s, c = i > 0 ? e[i - 1] : null, u = i + 2 < e.length ? e[i + 2] : null, p = Math.max(1, d.frame - l.frame), h = D((t - l.frame) / p, 0, 1);
  let m = r(l), f = r(d);
  if (o) {
  ...
  if (l.interpolation === "bezier" || d.interpolation === "bezier") {
    const S = Bt(l, a, c, d, r), b = Bt(d, a, l, u, r), C = m, _ = m + (S.out_y || 0), A = f + (b.in_y || 0), g = f, x = D(Number(S.out_x ?? 1 / 3), 0, 1), E = D(1 + Number(b.in_x ?? -1 / 3), 0, 1);
    let G = 0, F = 1;
  ...
  const y = en(h, l.interpolation);
  return m + (f - m) * y;
}
const rn = 0.01, on = 1e4;
function le() {
  return {
    position: [6, 4, 6],
    target: [0, 1.5, 0],
...
function Ue(e, t) {
  ...
  const r = me(e), o = (y, S) => (y.transform?.position || r.position)[S] ?? 0, n = (y, S) => (y.transform?.rotation || r.rotation)[S] ?? 0, s = (y, S) => (y.transform?.size || r.size)[S] ?? (S === 2 ? 0.01 : 1), i = _t(a, t), l = R(a, t, "pos_x", (y) => o(y, 0), !1, i), d = R(a, t, "pos_y", (y) => o(y, 1), !1, i), c = R(a, t, "pos_z", (y) => o(y, 2), !1, i), u = R(a, t, "rot_x", (y) => n(y, 0), !0, i), p = R(a, t, "rot_y", (y) => n(y, 1), !0, i), h = R(a, t, "rot_z", (y) => n(y, 2), !0, i), m = R(a, t, "scale_x", (y) => s(y, 0), !1, i), f = R(a, t, "scale_y", (y) => s(y, 1), !1, i), w = R(a, t, "scale_z", (y) => s(y, 2), !1, i);
  return {
...
function Fi(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  const n = [], s = [];
  let i = 0.65, l = 0.72, d = 0.82;
  if (typeof a == "string" && a.startsWith("#")) {
    const p = a.replace("#", "");
    p.length === 6 && (i = parseInt(p.slice(0, 2), 16) / 255, l = parseInt(p.slice(2, 4), 16) / 255, d = parseInt(p.slice(4, 6), 16) / 255);
  }
  const c = 0.618033988749895, u = 0.324717957244746;
  for (let p = 0; p < o; p++) {
    const h = p * c % 1, m = p * u % 1, f = (p + 0.5) * 0.7548776662466927 % 1;
    let w = 0, y = 0, S = 0, b = 0.65, C = 0.72, _ = 0.82;
    if (t === "ground_focus")
      if (h < 0.6) {
        const A = 0.4 + Math.sqrt(m) * 24, g = f * Math.PI * 2 + p * 2.399963229728653;
        w = Math.cos(g) * A, S = Math.sin(g) * A, y = 0.01 + h * 0.75, b = 0.86, C = 0.9, _ = 0.98;
      } else {
        const A = 1 + Math.sqrt(m) * 18, g = f * Math.PI * 2 + p * 2.399963229728653;
        w = Math.cos(g) * A, S = Math.sin(g) * A, y = 0.75 + (h - 0.6) * 8.5, b = 0.62, C = 0.7, _ = 0.82;
      }
    else if (t === "dome") {
      const A = h * Math.PI * 2, g = 1 - 2 * m, x = Math.sqrt(Math.max(0, 1 - g * g)), E = 1.5 + Math.cbrt(f) * 20;
      w = Math.cos(A) * x * E, S = Math.sin(A) * x * E, y = Math.max(0.01, g * E * 0.75 + 2.5), b = 0.72, C = 0.78, _ = 0.88;
    } else {
      const A = p % 4;
      if (A === 0) {
        const g = 0.3 + Math.sqrt(m) * 28, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, y = 0.01 + f * 0.34, b = 0.9, C = 0.94, _ = 1;
      } else if (A === 1) {
        const g = 0.6 + Math.sqrt(m) * 18, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, y = 0.35 + f * 3.15, b = 0.68, C = 0.76, _ = 0.86;
      } else if (A === 2) {
        const g = 2 + Math.sqrt(m) * 24, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, y = 3.5 + f * 11.5, b = 0.55, C = 0.65, _ = 0.78;
      } else {
        const g = 0.5 + m * 6.5, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, y = 0.05 + f * 4.95, b = 0.8, C = 0.86, _ = 0.94;
      }
    }
    n.push(w, y, S), s.push(a ? b * i : b, a ? C * l : C, a ? _ * d : _);
  }
...
function Bi(e) {
  ...
  const r = (c, u) => (Array.isArray(c) ? c : []).slice(0, Te.maxKeysPerTrack).map((p) => ({
  ...
  a.cameras = s.slice(0, Te.maxCameras).map((c, u) => {
    let p = String(c?.id || `camera_${u + 1}`);
    i.has(p) && (p = `camera_${u + 1}`), i.add(p);
  ...
  }), a.active_camera_id = a.cameras.some((c) => c.id === a.active_camera_id) ? a.active_camera_id : a.cameras[0].id, a.sequence = fo(a.sequence, a.cameras.map((c) => c.id)), a.playblast_camera_id = a.playblast_camera_id === uo && a.sequence.cuts.length || a.cameras.some((c) => c.id === a.playblast_camera_id) ? a.playblast_camera_id : a.active_camera_id;
  const l = a.cameras.find((c) => c.id === a.active_camera_id);
  a.camera = l.camera, a.keyframes = l.keyframes, a.target_object_id = l.target_object_id || null, a.target_offset = l.target_offset || [0, 0, 0], a.aim_bone = l.aim_bone || null, a.objects = (Array.isArray(a.objects) ? a.objects : t.objects).slice(0, Te.maxObjects).map((c) => ({
    ...c,
    color: fe(c?.color),
    locked: !!c.locked,
    ...
    ...c?.cone_angle !== void 0 ? { cone_angle: D(Number(c.cone_angle) || 45, 1, 90) } : {},
    ...c?.penumbra !== void 0 ? { penumbra: D(Number(c.penumbra) || 0.25, 0, 1) } : {},
...
function Fe(e = [0, 0, 0]) {
  const [t, a, r] = e.map((c) => c * Math.PI / 360), o = Math.cos(t), n = Math.sin(t), s = Math.cos(a), i = Math.sin(a), l = Math.cos(r), d = Math.sin(r);
  return [n * s * l + o * i * d, o * i * l - n * s * d, o * s * d + n * i * l, o * s * l - n * i * d];
}
function sn(e, t) {
...
function wa(e, [t, a, r, o]) {
  const [n, s, i] = e, l = o * n + a * i - r * s, d = o * s + r * n - t * i, c = o * i + t * s - a * n, u = -t * n - a * s - r * i;
  return [l * o - u * t - d * r + c * a, d * o - u * a - c * t + l * r, c * o - u * r - l * a + d * t];
}
function ln([e, t, a, r]) {
  const o = 1 - 2 * (t * t + a * a), n = 2 * (e * t - a * r), s = 2 * (e * a + t * r), i = 1 - 2 * (e * e + a * a), l = 2 * (t * a - e * r), d = 2 * (t * a + e * r), c = 1 - 2 * (e * e + t * t), u = Math.asin(Math.max(-1, Math.min(1, s))), [p, h] = Math.abs(s) < 0.9999999 ? [Math.atan2(-l, c), Math.atan2(-n, o)] : [Math.atan2(d, i), 0];
  return [p, u, h].map((m) => m * 180 / Math.PI);
}
function Ca(e, t) {
  const a = t.quaternion || Fe(t.rotation), r = sn(a, e.quaternion || Fe(e.rotation));
  return { position: M(wa(e.position.map((o, n) => o * t.size[n]), a), t.position), rotation: ln(r), quaternion: r, size: e.size.map((o, n) => o * t.size[n]) };
}
function cn(e, t) {
...
const Kt = ["speed", "angular_speed", "acceleration", "jerk"], Je = ["ok", "warn", "over"], Ma = 0.8, dn = [0, 1.5, 0];
function Vt(e, t) {
  const a = [0];
...
function pn(e, t = null) {
  ...
  return Array.isArray(a?.position) ? a.position.slice(0, 3).map(Number) : [...dn];
}
function fn(e, t, a, r) {
  return e.map((o) => {
    const n = V(t, o, a, r);
...
function gn(e, t) {
  ...
  for (let r = 0; r < e.length; r++) {
    ...
    if (n && n.grade === e[r] && hn(n.metrics, o)) {
      n.end = r;
      continue;
    }
...
function xa(e, t = {}, a = null, r = "generic") {
  ...
  const d = mn(l, o), c = un(l, o), u = Vt(d, o), p = Vt(u, o), h = { speed: d, angular_speed: c, acceleration: u, jerk: p }, m = pn(e, a), f = fn(l, m, s, i), w = l.map((g) => g.fov), y = t.allow_framing_loss === !0, S = [], b = [];
  for (let g = 0; g < n; g++) {
    const x = [], E = [];
    for (const G of Kt) {
    ...
    !f[g] && !y && (x.push("over"), E.push("framing_loss")), S.push(qt(x)), b.push(E);
  }
  ...
  for (const g of [...Kt, "fov_drift"]) {
    ...
    E != null && _[x] > Number(E) && _.violations.push({ metric: x, value: _[x], recommended_max: Number(E) });
  }
  C && !y && _.violations.push({ metric: "framing_loss_frames", value: C, recommended_max: 0 });
...
function Ki(e, t) {
  ...
  if (t.shiftKey)
    ...
    const n = t.currentTarget.getBoundingClientRect(), s = (t.clientX - n.left) / Math.max(1, n.width), i = D(Number(e.timelineZoom) || 1, 0.2, 30), l = D(i * r, 0.2, 30), d = a / i, c = a / l, u = (Number(e.timelinePan) || 0) + s * d;
    e.timelinePan = D(u - s * c, -a * 0.5, a), e.timelineZoom = l;
  }
...
function Hi(e, t) {
  ...
  if (e.boxSelect && t.pointerId === e.boxSelect.pointerId) {
    t.preventDefault(), t.stopPropagation();
    const a = e.boxSelect.box.getBoundingClientRect(), r = Math.max(1, e.state.duration_frames - 1), o = D(Number(e.timelineZoom) || 1, 0.1, 50), n = Number(e.timelinePan) || 0, s = r / o, i = (u) => D(n + u / Math.max(1, a.width) * s, 0, r), l = Math.min(i(e.boxSelect.startX), i(e.boxSelect.currentX)), d = Math.max(i(e.boxSelect.startX), i(e.boxSelect.currentX));
...
function vn(e, t) {
  ...
  if (!a) return;
  if (!a.engaged) {
    if (Math.hypot(t.clientX - (a.startClientX ?? t.clientX), t.clientY - (a.startClientY ?? t.clientY)) < yn) return;
    a.engaged = !0;
  }
  ...
  if (c.style.left = `${u}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${d !== 0 ? ` (${d > 0 ? "+" : ""}${d})` : ""}`, a.moving && a.moving.length > 1) {
    ...
    for (const m of a.moving) {
      let f = D(m.startFrame + d, 0, e.state.duration_frames - 1);
...
function Sn(e, t) {
  ...
  for (let l = 1; l < a.length - 1; l++) {
    const d = n + Math.round(s * (r[l] / o));
    a[l].frame = Math.min(t - 1, Math.max(i + 1, d)), i = a[l].frame;
  }
  return a;
}
function Aa(e, t) {
  ...
  for (let s = 1; s < r.length - 1; s++)
    if (Aa(r[s].frame ?? 0, t))
      for (const i of ["position", "target"]) {
        const l = [r[s - 1], r[s], r[s + 1]].map((u) => u.camera?.[i]).filter((u) => Array.isArray(u) && u.length >= 3), d = r[s].camera?.[i];
        if (l.length < 3 || !Array.isArray(d)) continue;
        const c = [0, 1, 2].map((u) => l.reduce((p, h) => p + Number(h[u] || 0), 0) / l.length);
        n[s].camera[i] = d.map((u, p) => Number(u) + (c[p] - Number(u)) * o);
      }
  return n;
}
function wn(e, t, a) {
...
function he(e, t, a, r) {
  const o = a == null ? v("no limit") : `${Ht(t)} / ${Ht(a)}`, n = a != null && a > 0, s = n ? Math.min(100, Math.round(t / a * 100)) : 0, i = r === "over" ? "var(--oc-danger)" : r === "warn" ? "var(--oc-warn)" : "var(--oc-ok)";
  return `
...
function En(e) {
  ...
  if (a.className = `oc-health-badge ${o.grade}`, a.textContent = xn(o.grade), r) {
    const { score: l, letter: d } = Dn(o);
  ...
  t.innerHTML = `
    ...
    <div class="oc-health-zones" data-role="health-zones">${jn(o)}</div>
...
function Ae(e, t, a, r) {
  ...
  o && (e.checkpoint(a), o.keyframes = t, e.state.keyframes = t, e.syncActiveCameraTrack(), e.refreshKeys(), e.setFrame(e.frame, !1, !1), e.setStatus(r), En(e));
}
function Ui(e) {
  const t = ue(e);
  if (!t) return;
  ...
  const r = Math.max(1, e.state.duration_frames - 1), o = Sn(e.state.keyframes, r), n = xa({ ...e.state, keyframes: o }, t.limits, null, t.profile);
  if (n.max_speed <= a) {
  ...
  Ae(e, o, "Slow to limits", v("Speed flattened, still over: this path needs about {seconds}s to fit the limit.").replace("{seconds}", s.toFixed(1)));
}
function Xi(e) {
  const t = ue(e);
  if (!t) return;
...
function Yi(e) {
  const t = ue(e);
  if (!t) return;
  const a = Cn(t, "framing_loss");
  if (!a.length) {
    e.setStatus(v("The subject stays in frame on this shot."));
    return;
  }
  const r = wn(e.state.keyframes, a, t.subject);
  Ae(
    e,
    r,
    "Recenter subject",
...
const In = {
  ...
  "No source connected": "Aucune source connectée",
  anomalies: "anomalies",
  "OPEN EXTRACTOR": "OUVRIR EXTRACTOR",
  Source: "Source",
  Track: "Trajectoire",
  Solve: "Résolution",
  Refine: "Affiner",
  "OmniCam Monitor": "OmniCam Monitor",
  Select: "Sélectionner",
  Orbit: "Orbite",
  ...
  ...In,
  ...On
}, ee = ["OmniCam"], Da = "MajoorOmniCam.Locale", ja = "MajoorOmniCam.Defaults.Fps", Ea = "MajoorOmniCam.Defaults.DurationSeconds", Ia = "MajoorOmniCam.Defaults.Width", Oa = "MajoorOmniCam.Defaults.Height", Pa = "MajoorOmniCam.Defaults.RenderMode", Na = "MajoorOmniCam.Defaults.Encoder", za = "MajoorOmniCam.Defaults.PlayblastResolution", Ra = "MajoorOmniCam.Playblast.Quality", La = "MajoorOmniCam.Defaults.PlayblastGrid", Fa = "MajoorOmniCam.Defaults.PlayblastLabels", Ba = "MajoorOmniCam.Proxy.PointDensity", Ka = "MajoorOmniCam.Proxy.PointSpread", Va = "MajoorOmniCam.Proxy.PointColor", Ga = "MajoorOmniCam.Proxy.CardFit", qa = "MajoorOmniCam.Viewport.Quality", Ha = "MajoorOmniCam.Viewport.Adaptive", Wa = "MajoorOmniCam.Viewport.BackgroundColor", $a = "MajoorOmniCam.Display.Grid", Ua = "MajoorOmniCam.Display.Radar", Xa = "MajoorOmniCam.Display.CameraPaths", Ya = "MajoorOmniCam.Display.CameraGizmos", Za = "MajoorOmniCam.Display.LookAt", Qa = "MajoorOmniCam.Display.HelperAxes", Ja = "MajoorOmniCam.Display.Gizmo", er = "MajoorOmniCam.Display.Guides", tr = "MajoorOmniCam.Display.SafeAreas", ar = "MajoorOmniCam.Display.ResolutionGate", rr = "MajoorOmniCam.Display.AspectRatio", or = "MajoorOmniCam.Display.BurnIn", nr = "MajoorOmniCam.Display.SpeedHeatmap", sr = "MajoorOmniCam.Display.Wireframe", ir = "MajoorOmniCam.Display.Vertices", lr = "MajoorOmniCam.Tools.SelectMode", cr = "MajoorOmniCam.Tools.GizmoMode", dr = "MajoorOmniCam.Tools.GizmoSpace", mr = "MajoorOmniCam.Tools.SpatialSnapMode", ur = "MajoorOmniCam.Tools.SpatialGridSize", pr = "MajoorOmniCam.Navigation.Profile", fr = "MajoorOmniCam.Navigation.FlySpeed", hr = "MajoorOmniCam.Navigation.InvertOrbitY", gr = "MajoorOmniCam.Navigation.ZoomSensitivity", br = "MajoorOmniCam.Navigation.OrbitSensitivity", yr = "MajoorOmniCam.Navigation.PanSensitivity", vr = "MajoorOmniCam.Navigation.DollySensitivity", _r = "MajoorOmniCam.Navigation.ViewMode", Sr = "MajoorOmniCam.Controls.EnableShortcuts", wr = "MajoorOmniCam.Timeline.SnapEnabled", Cr = "MajoorOmniCam.Timeline.SnapFrames", Mr = "MajoorOmniCam.Timeline.AutoKey", xr = "MajoorOmniCam.Timeline.DefaultInterpolation", kr = "MajoorOmniCam.Timeline.TimecodeMode", Ar = "MajoorOmniCam.Timeline.LoopPlayback", Tr = "MajoorOmniCam.Interface.Density", Dr = "MajoorOmniCam.Interface.PreviewLayout", jr = "MajoorOmniCam.Interface.CameraPreviews", Er = "MajoorOmniCam.History.Limit", Ir = "MajoorOmniCam.Extractor.DefaultBackend", Or = "MajoorOmniCam.Monitor.DefaultProfile", Pr = "MajoorOmniCam.Agent.Enabled", wt = "MajoorOmniCam.Agent.Provider", Wt = "MajoorOmniCam.Agent.Model", $t = "MajoorOmniCam.Agent.BaseUrl", Nr = "MajoorOmniCam.Agent.MaxOutputTokens", zr = "MajoorOmniCam.Agent.MaxPlannerSteps", Rr = "MajoorOmniCam.Agent.RequestTimeoutSeconds", Ke = [
...
function Ct(e) {
  return `MajoorOmniCam.Agent.${Lr(e).settingKey}.Model`;
}
function Mt(e) {
  return `MajoorOmniCam.Agent.${Lr(e).settingKey}.BaseUrl`;
}
...
function Br() {
  const e = String(L(Da, "auto")), t = String(L("Comfy.Locale", "en") || "en").slice(0, 2).toLowerCase(), a = e === "auto" ? t : e, r = a !== mo();
  if (co(a), !!r) {
    for (const o of U)
      if (!o.disposed)
        try {
...
function Yt(e = Kr()) {
  for (const t of U)
    t.disposed || (Vr(t), t.requestRender ? t.requestRender("quality") : t.render?.(), t.renderCameraView?.());
}
function tl() {
  return Hn().enabled;
}
...
function Hn() {
  const e = z(wt, "ollama", ["openai", "openai_compatible", "anthropic", "ollama"]);
  return {
    enabled: P(Pr, !0),
    provider: e,
    ...
    maxOutputTokens: K(Nr, 4096, 512, 32768, !0),
    maxPlannerSteps: K(zr, 6, 1, 12, !0),
    requestTimeoutSeconds: K(Rr, 120, 15, 300, !0)
  };
}
function Wn() {
  return {
    ...
    playblastResolution: z(za, "output", ["viewport", "half", "output", "double"]),
    playblastQuality: z(Ra, "balanced", ["low", "balanced", "high"]),
    playblastGrid: P(La, !1),
    playblastLabels: P(Fa, !1),
    pointDensity: z(Ba, "balanced", ["none", "sparse", "balanced", "dense", "ultra"]),
    pointSpread: z(Ka, "all_views", ["all_views", "ground_focus", "dome"]),
    pointColor: Xt(Va, "#cbd5e1"),
    cardFit: z(Ga, "contain", ["contain", "cover", "stretch"]),
    backgroundColor: Xt(Wa, "#121212"),
    showGrid: P($a, !0),
    showRadar: P(Ua, !0),
    showCameraPaths: P(Xa, !0),
    showCameraGizmos: P(Ya, !0),
    showLookAt: P(Za, !0),
    showHelperAxes: P(Qa, !0),
    showGizmo: P(Ja, !0),
    guides: P(er, !0),
    safeAreas: P(tr, !1),
    resolutionGate: P(ar, !1),
    aspectRatio: z(rr, "auto", ["auto", "16:9", "4:3", "1:1", "9:16", "2.39:1"]),
    burnIn: P(or, !1),
    speedHeatmap: P(nr, !1),
    showWireframe: P(sr, !1),
    showVertices: P(ir, !1),
    selectMode: z(lr, "object", ["object", "vertex", "edge", "face"]),
    gizmoMode: z(cr, "translate", ["translate", "rotate", "scale"]),
    gizmoSpace: z(dr, "world", ["world", "local"]),
    spatialSnapMode: z(mr, "none", ["none", "grid", "vertex"]),
    spatialGridSize: K(ur, 0.5, 0.01, 100),
    navigationProfile: z(pr, "simple", ["maya", "blender", "simple"]),
    flySpeed: K(fr, 1, 0.05, 5),
    invertOrbitY: P(hr, !1),
    zoomSensitivity: K(gr, 1, 0.2, 3),
    orbitSensitivity: K(br, 1, 0.2, 3),
    panSensitivity: K(yr, 1, 0.2, 3),
    dollySensitivity: K(vr, 1, 0.2, 3),
    viewMode: z(_r, "perspective", ["camera", "perspective", "front", "back", "top", "bottom", "right", "left"]),
    snapEnabled: P(wr, !0),
    snapFrames: K(Cr, 1, 1, 24, !0),
    autoKey: P(Mr, !1),
    defaultInterpolation: z(xr, "ease", ["ease", "smooth", "bezier", "linear", "ease_in", "ease_out", "hold"]),
    timecodeMode: z(kr, "time", ["time", "timecode"]),
    loopPlayback: P(Ar, !1),
    uiDensity: z(Tr, "animation", ["basic", "animation", "advanced"]),
    previewLayout: z(Dr, "auto", ["auto", "1", "2", "4"]),
    cameraViewVisible: P(jr, !0),
    undoLimit: K(Er, 100, 10, 500, !0),
    extractorBackend: z(Ir, "dpvo", ["dpvo", "pycolmap"]),
    monitorProfile: z(Or, "wan_camera_native", ["wan_camera_native", "minimax_h3", "ltx_motion", "generic_video"])
  };
}
function $n() {
...
function Yn(e, { cameraId: t, frame: a, additive: r = !1 } = {}) {
...
function Zt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = H(t), l = t.position, d = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = Qn(d, i);
  let u, p;
  if (t.camera_type === "orthographic") {
  ...
  return [0, 1, 2].map((h) => l[h] + i[h] * c + n[h] * u + s[h] * p);
}
function Jn(e) {
...
function Qt(e, { cameraId: t, frame: a, additive: r = !1 }) {
  e.pathSelection = Yn(e.pathSelection, { cameraId: t, frame: a, additive: r }), e.selectedKeyFrames = new Set(e.pathSelection.frames), e.selectedKeyFrame = e.pathSelection.primaryFrame, e.editingKeyFrame = null, e.pathSelection.primaryFrame != null && e.setFrame(e.pathSelection.primaryFrame), e.refreshKeys(), e.refreshInspector();
}
function es(e, { pointerX: t, pointerY: a, shiftKey: r, altKey: o }) {
...
const ts = 3;
function as(e, t, a, r, o) {
  const n = V(e, a, r, o), s = V(t, a, r, o);
  return !n || !s ? !1 : Math.hypot(n[0] - s[0], n[1] - s[1]) < ts;
}
function rs(e, { pointerX: t, pointerY: a, overHandle: r, viewCamera: o, e: n }) {
  if (!e.webgl?.pickCurveHandle) return !1;
  const s = e.webgl.pickCurveHandle([t, a]);
  if (!s) return !1;
  const i = (e.state.cameras || []).find((c) => c.id === s.cameraId), l = (i?.keyframes || []).findIndex((c) => c.frame === s.frame), d = l >= 0 ? i.keyframes[l] : null;
  return !d || as(s.position, d.camera.position, o, e.canvas.width, e.canvas.height) ? !1 : (r && n.stopImmediatePropagation?.(), e.curveHandleDrag = {
    cameraId: s.cameraId,
    frame: s.frame,
    side: s.side,
...
function Gr(e, t, { includeCtrlFallback: a, includeSimpleLeft: r = a }) {
...
function os(e, t, a) {
  const r = kt(e), o = Gr(r, t, { includeCtrlFallback: !0 });
  return o === _e && a?.camera_type === "orthographic" ? Se : o;
}
function Jt(e, t) {
  return e.isNavigatingFly ? !0 : Gr(kt(e), t, { includeCtrlFallback: !1 }) !== null;
}
const ns = ["maya", "blender", "simple"];
function kt(e) {
  const t = e.state?.navigation_profile;
  return ns.includes(t) ? t : "maya";
}
function ss(e, t) {
...
function qe(e, { mode: t, origin: a, delta: r, factors: o, rotationDeg: n }) {
  ...
  const s = j(e, a);
  return t === "scale" ? M(a, [s[0] * o[0], s[1] * o[1], s[2] * o[2]]) : M(a, ke(s, n));
}
function ls(e, t) {
...
function il(e, t, a) {
  ...
  return r.map((i, l) => {
    const d = { ...i.camera };
    if (!o.has(i.frame) || !Array.isArray(d.position)) return { ...i, camera: d };
    const c = d.position, u = d.target;
    if (d.position = s[l], Array.isArray(u))
      if (n)
        d.target = qe(u, a);
      else {
        const p = W(j(u, c)) || 5, h = cs(s, l);
        d.target = M(d.position, T(h, p));
      }
    return { ...i, camera: d };
  });
}
function ll(e, t, { delta: a = [0, 0, 0] } = {}) {
...
function ms(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function us(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  const n = e.gizmoDrag, s = e.state.cameras.find((d) => d.id === n.trackId);
  if (!s) return;
  const i = n.origin;
  let l;
  if (e.state.gizmo_mode === "translate") {
    ...
    if (n.free) {
      ...
      d = M(T(n.viewRight, c * n.freeScale), T(n.viewUp, -u * n.freeScale));
    } else
      d = T(n.axis, a * n.worldLength / n.screenLength);
    ...
    l = { mode: "scale", origin: i, factors: d };
  } else {
    const d = o ? Math.round(a * 0.75 / 15) * 15 : a * 0.75, c = [0, 0, 0];
    c[n.axisIndex] = d, l = { mode: "rotate", origin: i, rotationDeg: c };
  }
  s.keyframes = ls(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = de(s, e.frame, e.state.objects), s.camera = q(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
...
`, qr = 1e-4;
function ps(e, t) {
  return !Array.isArray(e) || !Array.isArray(t) ? e !== t : e.some((a, r) => Math.abs(Number(a) - Number(t[r])) > qr);
}
function ea(e, t) {
  return Math.abs(Number(e) - Number(t)) > qr;
}
const fs = [
  {
    id: "camera",
  ...
  },
  {
    ...
    label: "Look At",
    color: k.typeLookAt,
    changed: (e, t) => ps(e?.target, t?.target),
    read: (e) => e?.target
  },
  {
    id: "focal_length",
...
function dl(e, t = null) {
  return fs.filter((a) => !t || t.has(a.id)).map((a) => ({
    id: a.id,
    label: a.label,
    color: a.color,
    frames: hs(e, a)
...
function bs(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => gs(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((C) => C[b]))), d = [0, 1, 2].map((b) => Math.max(...i.map((C) => C[b]))), c = l.map((b, C) => (b + d[C]) / 2), { right: u, up: p, forward: h } = H(t), m = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), w = f * m;
  let y = 2, S = 0.1;
  for (const b of i) {
    const C = j(b, c), _ = Math.abs(Z(C, u)), A = Math.abs(Z(C, p)), g = Z(C, h);
    y = Math.max(y, 1.15 * _ / w - g, 1.15 * A / f - g, (t.near || 0.01) * 2 - g), S = Math.max(S, 1.15 * A, 1.15 * _ / m);
  }
  t.target = c, t.position = j(c, T(h, y)), t.camera_type === "orthographic" && (t.zoom = Math.max(0.01, 5 / S));
}
const te = {
...
function fl(e, t = {}) {
  ...
  if (e.subSelection?.point && !t.all) {
    ...
    const i = e.subSelection.point, l = ie(j(a.position, o)), d = Number.isFinite(l[0]) && W(l) > 0.1 ? l : [0.707, 0.4, 0.707], c = 2;
    a.target = [...i], a.position = M(a.target, T(d, c)), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const u = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(v("Focused on {value1} at [{value2}]", { value1: u, value2: i.map((p) => Math.round(p * 100) / 100).join(", ") }));
    return;
  }
  const s = e.selectedObject() || e.state.objects.find((i) => i.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] };
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), bs(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? v("Framed: all objects") : v("Framed: {name}").replace("{name}", s.name || s.type || v("Subject")));
}
function vs(e, t, a, { forceLocal: r = !1 } = {}) {
  const o = [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n = a?.rotation || t?.rotation || [0, 0, 0];
  return r || e.state.gizmo_space === "local" ? o.map((s) => ke(s, n)) : o;
}
function _s(e) {
  return ys(e);
}
...
function Hr(e) {
  ...
  const t = _s(e);
  if (!t || e.transformControlsWiring && Ss.has(t.type)) return null;
  ...
  const n = Math.max(0.7, W(j(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? vs(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: i.map((d, c) => ({ index: c, axis: d, points: [o, V(M(r, T(d, n)), a, e.canvas.width, e.canvas.height)] })).filter((d) => d.points[1] && Number.isFinite(d.points[1][0]) && Number.isFinite(d.points[1][1]))
    };
  const l = i.map((d, c) => {
    const u = Math.abs(d[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = ie(Me(d, u)), h = ie(Me(d, p)), m = [];
    for (let f = 0; f <= 48; f++) {
      const w = f / 48 * Math.PI * 2, y = V(M(r, M(T(p, Math.cos(w) * n), T(h, Math.sin(w) * n))), a, e.canvas.width, e.canvas.height);
      y && Number.isFinite(y[0]) && Number.isFinite(y[1]) && m.push(y);
    }
...
function Wr(e, t) {
  ...
  for (const i of a.handles)
    for (let l = 0; l < i.points.length - 1; l++) {
      const d = i.points[l], c = i.points[l + 1], u = Sa(t, d, c);
      (!s || u < s.distance) && (s = { ...i, distance: u, segment: [d, c], worldLength: a.worldLength, entity: a.entity });
    }
...
function ws(e, t) {
  ...
  if (e.state.view_mode !== "camera") {
    for (const s of e.state.cameras) {
      ...
      const d = V(i.position || [6, 4, 6], r, e.canvas.width, e.canvas.height);
    ...
    if (e.state.show_camera_paths !== !1)
      for (const s of e.state.cameras) {
        ...
        const l = i.map((d) => V(d.camera?.position, r, e.canvas.width, e.canvas.height)).filter((d) => d && Number.isFinite(d[0]) && Number.isFinite(d[1]));
  ...
  for (const s of e.state.objects) {
    ...
    (!n || d < n.distance) && (n = { object: s, distance: d });
  }
  return n?.distance <= 22 * Math.min(2, window.devicePixelRatio || 1) ? { type: "object", object: n.object } : null;
}
function Cs(e, t, a, r = 15) {
  const o = a[0] - t[0], n = a[1] - t[1], s = Math.hypot(o, n) || 1, i = o / s, l = n / s, d = -l, c = i, u = r * 0.42, p = a[0] - i * r, h = a[1] - l * r;
...
function hl(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : Cs(e.ctx, s[0], i);
    }
  }
...
function Ms(e, t) {
  ...
  const o = a.map((c) => ({ object: c, transform: me(c) })), n = o.reduce((c, u) => M(c, u.transform.position), [0, 0, 0]).map((c) => c / o.length), s = [e.canvas.width * 0.5, e.canvas.height * 0.5], i = e.lastViewportPointer || s, l = e.interactionElement.getBoundingClientRect(), d = e.lastPointerEvent || { clientX: l.left + i[0] * l.width / e.canvas.width, clientY: l.top + i[1] * l.height / e.canvas.height };
  return e.modalTransform = { mode: t, axis: null, numeric: "", start: i, lastEvent: d, snapshots: o, pivot: n }, e.setTransformMode(t), e.setStatus(`${t.toUpperCase()} · move mouse · X/Y/Z constrain · type value · Enter confirm · Esc cancel`), e.render(), !0;
}
function xs(e) {
...
function ks(e, t, a, r, o, n, s) {
  const i = o ? "grid" : e.state.spatial_snap_mode;
  if (i === "grid") {
    const l = e.state.spatial_grid_size || 0.5;
    return n ? a.map((d, c) => d.map((u, p) => Math.abs(n[p]) > 1e-6 ? He(u, l) : s[c][p])) : a.map((d) => d.map((c) => He(c, l)));
  }
  if (i === "vertex" && r && !n) {
    ...
    if (l?.point && !t.snapshots.some((d) => d.object.id === l.objectId)) {
      const d = a.reduce((u, p) => M(u, p), [0, 0, 0]).map((u) => u / a.length), c = j(l.point, d);
      return a.map((u) => M(u, c));
    }
  }
  return a;
}
function Oe(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = xs(a), d = a.axis ? ct[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], u = H(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...j(c.position, c.target)) * 25e-4;
  let h = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, C = d ? T(d, b) : M(T(u.right, n * p * i), T(u.up, -s * p * i));
    h = h.map((_) => M(_, C)), h = ks(e, a, h, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((_) => _.transform.position));
  }
  const m = a.mode === "rotate" ? l ?? (n - s) * 0.5 * i : 0, f = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - s) * 0.01 * i) : 1, w = d || ct.z, y = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
  a.snapshots.forEach((b, C) => {
    const _ = b.object;
    if (a.mode === "translate" && (_.position = h[C]), a.mode === "rotate") {
      const A = y ? He(m, 15) : m, g = T(w, A);
      _.position = M(a.pivot, ke(j(b.transform.position, a.pivot), g)), _.rotation = M(b.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const A = y ? He(f, 0.1) : f, g = d ? d.map((E) => E ? A : 1) : [A, A, A], x = j(b.transform.position, a.pivot);
      _.position = M(a.pivot, x.map((E, G) => E * g[G])), _.size = b.transform.size.map((E, G) => Math.max(0.01, E * g[G]));
    }
...
function As(e, t) {
  ...
  return r === "escape" ? Xr(e) : r === "enter" || r === " " ? Ur(e) : ct[r] ? (a.axis = a.axis === r ? null : r, Oe(e, a.lastEvent), !0) : /^[0-9]$/.test(r) || r === "." || r === "," || r === "-" && !a.numeric ? (a.numeric += r === "," ? "." : r, Oe(e, a.lastEvent), !0) : (r === "backspace" && (a.numeric = a.numeric.slice(0, -1), Oe(e, a.lastEvent)), !0);
}
function ge(e, t, a) {
...
const ne = (e, t) => Math.round(e / t) * t, Ds = (e, t) => e.map((a) => ne(a, t));
function js(e, t, a) {
  ...
  if (n)
    ...
    s = p.map((h, m) => o[m] - h), i = p.map((h, m) => o[m] + h);
  }
  let l = 1 / 0, d = 1 / 0, c = -1 / 0, u = -1 / 0;
  for (const p of [s[0], i[0]]) for (const h of [s[1], i[1]]) for (const m of [s[2], i[2]]) {
...
function be(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, d) => Math.abs(o.axis[d]) > 1e-6 ? ne(l, i) : o.base[d]) : Ds(t, i);
  if (s === "vertex" && a && !o) {
...
function gl(e, t) {
  if (e.modalTransform) {
    t.preventDefault?.(), t.stopPropagation?.(), t.button === 0 ? Ur(e) : t.button === 2 && Xr(e);
    return;
  }
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = Q(e), s = e.state.view_mode !== "camera", i = Jt(e, t), l = t.button === 0 && !i, d = l && !t.altKey && !t.shiftKey, c = e.transformControlsWiring?.isPointerOverHandle?.();
  !d && c && t.stopImmediatePropagation?.();
  const u = e.transformControlsWiring?.currentLiveType?.();
  if (d && (!c || u === "path_point" || u === "camera") && rs(e, { pointerX: r, pointerY: o, overHandle: c, viewCamera: n, e: t }) || d && c || l && es(e, { pointerX: r, pointerY: o, shiftKey: t.shiftKey, altKey: t.altKey })) return;
  const h = d ? Wr(e, [r, o]) : null;
  if (h) {
    const [b, C] = h.segment, _ = Math.max(1, Math.hypot(C[0] - b[0], C[1] - b[1])), A = {
      pointer: [r, o],
      axis: h.axis,
      axisIndex: h.index,
      screen: [(C[0] - b[0]) / _, (C[1] - b[1]) / _],
    ...
    if (h.entity.type === "camera_path" && ms(e, { baseDrag: A, viewCamera: n, entityPosition: h.entity.position })) return;
    if (h.entity.type === "object") {
      const g = h.entity.object;
      e.checkpoint("Transform object");
      const x = $r(e), E = (x.length ? x : [g]).map((F) => ({ object: F, transform: me(F) }));
  ...
  if (e.pointerHit = !!(h || m), m) {
    ...
    if (m.type === "camera_target") {
      ...
      e.checkpoint("Move camera target"), e.beginCameraEdit();
      const { right: b, up: C } = H(n), _ = [...e.camera.target], A = e.activeCameraTrack?.(), g = !!A?.target_object_id;
      e.targetFreeDrag = {
        pointer: [r, o],
  ...
  s && !e.state.editor_views && (e.state.editor_views = Le()), e.drag = {
    ...
    navigationOnly: i,
    historyCheckpointed: !1
  }, e.interactionElement.style && (e.interactionElement.style.cursor = S ? "ns-resize" : y ? "move" : "grabbing"), e.setStatus?.(v(f ? "Fly" : S ? "Dolly" : y ? "Pan" : "Orbit"));
}
function bl(e, t) {
  ...
  if (e.pathDrag) {
    ...
    c && (c.camera.position = Zt(
      ...
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = Jn(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  ...
  if (e.targetFreeDrag) {
    ...
    const s = e.interactionElement.getBoundingClientRect(), i = (t.clientX - s.left) * e.canvas.width / Math.max(1, s.width), l = (t.clientY - s.top) * e.canvas.height / Math.max(1, s.height), d = i - e.targetFreeDrag.pointer[0], c = l - e.targetFreeDrag.pointer[1], u = t.shiftKey ? 0.1 : 1, p = M(T(e.targetFreeDrag.right, d * e.targetFreeDrag.scale * u), T(e.targetFreeDrag.up, -c * e.targetFreeDrag.scale * u)), h = be(e, M(e.targetFreeDrag.target, p), [i, l]);
  ...
  if (e.gizmoDrag) {
    ...
    const s = e.interactionElement.getBoundingClientRect(), i = [
      (t.clientX - s.left) * e.canvas.width / Math.max(1, s.width),
      (t.clientY - s.top) * e.canvas.height / Math.max(1, s.height)
    ], l = t.shiftKey ? 0.1 : 1, d = ((i[0] - e.gizmoDrag.pointer[0]) * e.gizmoDrag.screen[0] + (i[1] - e.gizmoDrag.pointer[1]) * e.gizmoDrag.screen[1]) * l, c = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
    if (e.gizmoDrag.type === "camera_target") {
      const h = M(e.gizmoDrag.target, T(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), m = be(e, h, i, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
      e.gizmoDrag.tracking ? aa(e, m) : e.camera.target = m, e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        const h = M(e.gizmoDrag.position, T(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.camera.position = be(e, h, i, [], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      } else {
        const h = c ? ne(d * 0.015, Math.PI / 12) : d * 0.015, m = j(e.gizmoDrag.target, e.gizmoDrag.position), f = ke(m, T(e.gizmoDrag.axis, h * (180 / Math.PI)));
        e.camera.target = M(e.gizmoDrag.position, f);
      }
      e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera_path") return void us(e, { pointer: i, deltaPixels: d, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
        const h = (i[0] - e.gizmoDrag.pointer[0]) * l, m = (i[1] - e.gizmoDrag.pointer[1]) * l, f = M(
          e.gizmoDrag.position,
          M(T(e.gizmoDrag.viewRight, h * e.gizmoDrag.freeScale), T(e.gizmoDrag.viewUp, -m * e.gizmoDrag.freeScale))
        );
        e.gizmoDrag.object.position = be(e, f, i, [e.gizmoDrag.object.id]);
      } else {
        const h = M(e.gizmoDrag.position, T(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.gizmoDrag.object.position = be(e, h, i, [e.gizmoDrag.object.id], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      }
    else if (e.state.gizmo_mode === "scale")
      if (e.gizmoDrag.free) {
        const h = (i[0] - e.gizmoDrag.pointer[0]) * l, m = (i[1] - e.gizmoDrag.pointer[1]) * l, f = (h - m) * e.gizmoDrag.freeScale, w = e.gizmoDrag.size.map((y) => {
          const S = y + f;
    ...
    if (u.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        ...
        for (const m of u) m.object.position = M(m.transform.position, h);
      } else if (e.state.gizmo_mode === "rotate") {
        const h = j(e.gizmoDrag.object.rotation, p.rotation);
        for (const m of u)
          m.object.position = M(e.gizmoDrag.groupPivot, ke(j(m.transform.position, e.gizmoDrag.groupPivot), h)), m.object.rotation = M(m.transform.rotation, h);
      } else {
        const h = e.gizmoDrag.object.size.map((m, f) => m / Math.max(0.01, p.size[f]));
        for (const m of u) {
          const f = j(m.transform.position, e.gizmoDrag.groupPivot);
          m.object.position = M(e.gizmoDrag.groupPivot, f.map((w, y) => w * h[y])), m.object.size = m.transform.size.map((w, y) => Math.max(0.01, w * h[y]));
        }
      }
  ...
  if (e.drag.dolly) {
    ...
    let l = Math.atan2(s[0], s[2]), d = Math.asin(D(s[1] / i, -0.999, 0.999));
    l -= a * 8e-3, d = D(d - r * 8e-3, -1.45, 1.45), e.drag.target.target = [
      n.position[0] + i * Math.sin(l) * Math.cos(d),
      n.position[1] + i * Math.sin(d),
      n.position[2] + i * Math.cos(l) * Math.cos(d)
    ];
  } else if (e.drag.shift) {
    const { right: s, up: i } = H(n), l = Ge(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), d = M(T(s, -a * l), T(i, r * l));
    e.drag.target.position = M(n.position, d), e.drag.target.target = M(n.target, d);
  } else {
    const s = j(n.position, n.target), i = W(s);
    let l = Math.atan2(s[0], s[2]), d = Math.asin(D(s[1] / i, -0.999, 0.999));
    l -= a * 8e-3, d = D(d + r * 8e-3, -1.45, 1.45), e.drag.target.position = [
      n.target[0] + i * Math.sin(l) * Math.cos(d),
      n.target[1] + i * Math.sin(d),
      n.target[2] + i * Math.cos(l) * Math.cos(d)
    ];
  }
...
function yl(e, t) {
  if (t?.type === "pointercancel" || t?.type === "lostpointercapture") {
    t.pointerId === e.activePointerId && Yr(e);
    return;
  }
  if (e.pathDrag) {
    const s = e.pathDrag.moved;
    e.pathDrag = null, we(e), s && (e.scheduleSerialize(), e.refreshKeys(), e.setStatus(v("Path key moved")));
    return;
  }
  if (e.curveHandleDrag) {
    const s = e.curveHandleDrag.moved;
    e.curveHandleDrag = null, we(e), s && (e.webgl && (e.webgl.pathKey = ""), e.scheduleSerialize(), e.refreshKeys(), e.setStatus(v("Curve handle updated")));
    return;
  }
  if (e.boxSelection) {
    const s = e.boxSelection, i = Q(e), l = Math.min(s.start[0], s.current[0]), d = Math.max(s.start[0], s.current[0]), c = Math.min(s.start[1], s.current[1]), u = Math.max(s.start[1], s.current[1]), p = s.additive ? new Set(s.initial) : /* @__PURE__ */ new Set();
...
function vl(e, t) {
  ...
  const a = ss(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
  if (e.isNavigatingFly) {
    e.cameraSpeed = D(e.cameraSpeed * Math.exp(-a * 1e-3), 0.05, 20), e.setStatus(v("Fly speed: {value1}x", { value1: e.cameraSpeed.toFixed(2) }));
    return;
  }
  Ts(e);
...
function We(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Es(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function Ns(e, t, a, r, o) {
  const n = de(e, a, e.objects);
  let s = t?.point;
  if (t?.object_id) {
    const c = e.objects.find((h) => h.id === t.object_id);
    if (!c) return null;
    const u = cn(e.objects, c), p = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
    s = [u.position[0] + p[0] * u.size[0], u.position[1] + p[1] * u.size[1], u.position[2] + p[2] * u.size[2]];
  }
  if (!Array.isArray(s)) return null;
  const i = V(s, n, r, o);
  if (!i) return null;
  const l = i[0] / r, d = i[1] / o;
  return { x: l, y: d, visible: l >= 0 && l <= 1 && d >= 0 && d <= 1 };
}
function zs(e, t = 6e-3) {
...
function Fs(e, t) {
  const a = e.state.objects.find((d) => d.id === e.selectedObjectId), r = e.motionCreationKind, n = (r === "object" || r !== "world" && !!a) && a ? "object_point" : "world_point", s = n === "object_point" ? { object_id: a.id, local_point: [0, 0, 0] } : { point: e.webgl?.intersectScenePoint?.(t.x * e.canvas.width, t.y * e.canvas.height, e.canvas.width, e.canvas.height) || [...e.camera.target] }, i = Ns(e.state, s, e.frame, e.canvas.width, e.canvas.height) || t, l = n === "object_point" ? `${a.name || a.id} Track` : "World Anchor";
...
function _l(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    n && (e.checkpoint("Set motion interpolation"), Is(n, o.target.value), oe(e, `Motion interpolation: ${o.target.value}`));
  }, { signal: t }), e.root.querySelector('[data-role="motion-key-visible"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    if (!n?.keys?.length) return;
    const s = e.frame / e.state.fps, i = n.keys.reduce((l, d) => Math.abs(d.time_seconds - s) < Math.abs(l.time_seconds - s) ? d : l);
  ...
  for (const o of e.root.querySelectorAll("[data-motion-layer-action]"))
    o.addEventListener("click", () => {
      ...
      if (e.checkpoint(s === "delete" ? "Delete motion layer" : s === "retime" ? "Retime motion layer" : "Toggle motion layer"), s === "delete") Zr(e.state, n.id);
      else if (s === "retime") {
        const [i, l] = ra(e);
        Os(n, i, l);
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? We(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? Fs(e, s) : n === "erase" && Ls(e.state, s), oe(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = ra(e), l = Rs(e.state, n.points, s, i);
  ...
  a.addEventListener("pointerup", r, { capture: !0, signal: t }), a.addEventListener("pointercancel", r, { capture: !0, signal: t }), a.addEventListener("click", (o) => {
    ...
    const n = Qr(e.state.motion_layers, at(e, o));
...
function Sl(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => Ks(e, a.dataset.motionCreate), { signal: t });
  e.root.querySelector("[data-motion-create-cancel]")?.addEventListener("click", () => Jr(e), { signal: t });
}
...
const ye = { t: "translate", r: "rotate", s: "scale" }, Hs = [
...
], Ws = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function $s(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(Ws) : !1;
}
function Us(e) {
...
function Ys(e, t) {
  return Xs(e) || t?.lastKeyZone || "viewport";
}
let na = !1;
function Zs() {
  na || typeof window > "u" || (na = !0, window.addEventListener("keydown", (e) => {
    if (!Bn()) return;
    const t = e.composedPath?.()[0] || e.target, a = Kn(t);
    !a || a.disposed || Qs(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function Qs(e, t) {
  if (!Gn()) return !1;
  const a = t.composedPath?.()[0] || t.target;
  if (Us(a) || (t.code === "Space" || t.key === "Enter") && $s(a)) return !1;
...
function ti(e, t) {
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const i = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: d, up: c, forward: u } = H(i), p = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, h = { w: T(u, p), s: T(u, -p), d: T(d, p), a: T(d, -p), e: T(c, p), q: T(c, -p) }[a];
...
function ri(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!ze(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: ho(e.state) }, ot(e)) : (e.checkpoint("Split shot"), bo(e.state, e.frame, null) ? ot(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
    if (t.repeat) return !0;
    const r = ze(e.state), o = da(e.state, e.frame), n = o ? r.findIndex((s) => s.start === o.start) : -1;
    return n >= 0 && (e.checkpoint("Remove shot"), yo(e.state, n) && ot(e)), !0;
  }
  return !1;
}
function Tt(e, t) {
...
function si([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * oi), n = Math.round((window.innerHeight || t) * ni);
  return [
...
function ii(e, t) {
  const a = eo[t];
  return !a || !e?.setSize ? !1 : (e.setSize(si(a.default, a.min)), !0);
}
function li(e, t, a) {
  const r = eo[t];
  if (!r || !e?.setSize) return !1;
  const o = Array.isArray(a) ? a : Array.isArray(e.size) ? e.size : [0, 0], n = Array.isArray(e.size) ? e.size : [0, 0], [s, i] = r.min, l = Math.max(Number(o[0]) || 0, s), d = Math.max(Number(o[1]) || 0, i);
  return l === n[0] && d === n[1] ? !1 : (e.setSize([l, d]), !0);
}
const sa = "oc-help-css", Ee = "#8b7bd8", to = /* @__PURE__ */ new Map();
function Dt(e, t) {
  e && t && to.set(e, t);
}
function pt(e) {
...
function di() {
  ...
  e.id = sa, e.textContent = ci, document.head.appendChild(e);
}
function se(e) {
...
function ia(e) {
  ...
  const u = document.activeElement instanceof HTMLElement ? document.activeElement : null, p = () => {
    document.removeEventListener("keydown", m, !0), t.remove(), xe === p && (xe = null), u && u.isConnected && typeof u.focus == "function" && u.focus({ preventScroll: !0 });
  };
  xe = p;
  const h = () => Array.from(
    ...
    if (f.key === "Tab") {
      ...
      const y = w[0], S = w[w.length - 1], b = document.activeElement;
      f.shiftKey && (b === y || !a.contains(b)) ? (f.preventDefault(), S.focus()) : !f.shiftKey && (b === S || !a.contains(b)) && (f.preventDefault(), y.focus());
    }
  };
  return document.addEventListener("keydown", m, !0), i.addEventListener("click", (f) => {
    f.stopPropagation(), p();
...
const la = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", ca = "oc-help-toolbar-css", pi = "#8b7bd8";
function fi() {
  ...
  e.id = ca, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${pi};color:#fff;
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Tt(e, async () => {
      ...
      const { attachDirectorShell: o } = await import("./chunk-CPxEa9w2.js").then((n) => n.o);
      return Ie("director:import:resolved", e), o;
    });
    const r = e.__majoorOmniCamDirectorRuntime;
    r && (Ie("director:attach:complete", e), t && Xn(r), jt(e, dt, t, a?.()));
  }
});
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== mt) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-8DWlRDM4.js").then((r) => r.g)).attachExtractorShell), e.__majoorOmniCamExtractorRuntime && jt(e, mt, t, a?.());
  }
});
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== ut) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-C1Zfp71g.js").then((r) => r.s)).attachMonitorShell), !(!e.__majoorOmniCamMonitorShell && !e.__majoorOmniCamMonitor) && jt(e, ut, t, a?.());
  }
});
export {
  Fe as $,
