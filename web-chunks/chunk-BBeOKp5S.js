import { e as E, a as A, bg as I, v as p, G as q, b6 as _, s as G, b7 as N, bM as X } from "./chunk-CP16H5xA.js";
import { a as Y } from "./chunk-tXB5k9Zz.js";
    ...
    const i = (d) => {
      ...
      if (C.textContent = d.sublabel || "", Object.assign(C.style, { opacity: "0.6", fontSize: "11px" }), u.append(j, C), u.addEventListener("click", () => i(d.id)), v.appendChild(u), o) {
        ...
        y.type = "button", y.title = "Delete", y.textContent = "✕", Object.assign(y.style, {
          ...
          background: "transparent",
          color: "inherit"
        }), y.addEventListener("click", async (J) => {
          J.stopPropagation(), y.disabled = !0;
          try {
            await o(d.id), v.remove(), s.children.length || i(null);
          } catch (S) {
            y.disabled = !1, console.warn("[OmniCam] delete failed", S), a?.setStatus?.(String(S?.message || S).slice(0, 120));
          }
        }), v.appendChild(y);
      }
      return v;
...
function ce(e, t, { label: o = "Extracted Camera" } = {}) {
  ...
  if (e.state.cameras.push({ id: m, name: l, color: i, camera: E(h[0].camera), keyframes: h }), Number.isFinite(Number(t.duration_frames))) {
...
function W(e) {
  ...
  if (s !== e.cameraPreviewSignature && (l = !0, e.cameraPreviewSignature = s, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), c.forEach((i, h) => {
    ...
    f.style.setProperty("--camera-color", x), f.title = p(`Click: set ${i.name} as primary · Double-click: edit · Right-click: preview actions`);
...
function Pe(e) {
  ...
  const t = L(e.state), o = e.state.cameras.length, a = `Camera ${o + 1}`, r = E(e.camera), n = [
  ...
  e.state.cameras.push({
    ...
    color: m,
    camera: r,
    keyframes: [{ frame: 0, camera: E(r), interpolation: s }]
  }), e.cameraPreviewSignature = "", e.activateCamera(t), e.setStatus(p(`${a} added`));
}
async function Ie(e, t) {
  const o = e.state.cameras.find((r) => r.id === t);
  if (!o) return;
  const a = (await U(e.app, p("Rename camera"), p("Camera name"), o.name))?.trim();
  !a || a === o.name || (e.checkpoint("Rename camera"), o.name = a.slice(0, 80), e.cameraPreviewSignature = "", e.serialize(), e.refreshObjects(), e.refreshKeys(), e.setStatus(p(`Camera renamed: ${o.name}`)));
}
function qe(e, t) {
  ...
  e.state.cameras.push(a), e.cameraPreviewSignature = "", e.activateCamera(a.id), e.setStatus(p(`${a.name} added`));
}
async function Fe(e, t) {
  if (e.state.cameras.length <= 1) return e.setStatus(p("At least one camera is required"));
  const o = e.state.cameras.find((r) => r.id === t);
  if (!o || !await ee(e.app, p("Delete camera"), p(`Delete ${o.name} and its ${o.keyframes.length} keyframe(s)?`))) return;
  ...
  if (e.state.cameras = e.state.cameras.filter((r) => r.id !== t), t === e.state.playblast_camera_id && (e.state.playblast_camera_id = e.state.cameras[0].id), e.cameraPreviewSignature = "", a) {
    ...
    e.state.active_camera_id = r.id, e.state.keyframes = r.keyframes, e.state.camera = E(r.camera), e.camera = A(r, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = r.keyframes.find((n) => n.frame === e.frame)?.frame ?? null, e.selectedKeyFrames = e.selectedKeyFrame != null ? /* @__PURE__ */ new Set([e.selectedKeyFrame]) : /* @__PURE__ */ new Set(), e.editingKeyFrame = null;
  }
  e.pathSelection = I(e.pathSelection, e.activeCameraTrack()), e.serialize(), e.refreshCameraSelectors(), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(p(`${o.name} deleted`));
}
function De(e, t) {
  const o = e.state.cameras.find((a) => a.id === t);
  o && (e.finishCameraEdit(), e.syncActiveCameraTrack(), e.state.active_camera_id = o.id, e.state.keyframes = o.keyframes, e.state.camera = E(o.camera), e.camera = A(o, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = o.keyframes.find((a) => a.frame === e.frame)?.frame ?? null, e.selectedKeyFrames = e.selectedKeyFrame != null ? /* @__PURE__ */ new Set([e.selectedKeyFrame]) : /* @__PURE__ */ new Set(), e.pathSelection = I(e.pathSelection, o), e.editingKeyFrame = null, e.serialize(), e.refreshCameraSelectors(), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(p(`Camera: ${o.name}`)));
}
function Oe(e, t) {
  const o = q(e.state), a = t === _ && o.length > 0, r = a ? null : e.state.cameras.find((n) => n.id === t);
  !a && !r || (e.state.playblast_camera_id = a ? _ : r.id, e.refreshCameraSelectors(), e.serialize(), e.refreshObjects(), e.renderCameraView(), e.setStatus(a ? p("Playblast: sequence ({count} shots)").replace("{count}", String(o.length)) : p(`Playblast: ${r.name}`)));
}
function $e(e) {
  ...
  }), e.setStatus(p(`Camera previews ${e.state.camera_view_visible ? "shown" : "hidden"}`));
}
function de(e, t) {
...
function ge(e, t, o = {}) {
  ...
  if (r === "replace") {
    e.checkpoint?.("Adopt reconstructed scene (replace)"), e.state = G(
