import { a, a4 as j, bK as te, bL as J, bM as w } from "./chunk-BQzFpTKV.js";
const re = "/majoor/omnicam/agent/v1/plan", ie = "/majoor/omnicam/agent/v1/apply-plan";
async function Y(e, n, i, { signal: o } = {}) {
...
function he(e, n = {}) {
  ...
  function E() {
    ...
    if (M) {
      ...
      M.textContent = `${p} · ${t.model || a("(no model set)")}`;
    }
    C && (C.textContent = me(t));
    const r = g === "planning" || g === "applying";
    if (H && (H.disabled = r), N && (N.disabled = !c || r || c.truncated || g === "stale"), U && (U.disabled = !c), d && (d.disabled = r), k && (k.innerHTML = c ? ve(c.changes) : ""), g === "planning") u(a("Planning..."));
  ...
  async function z() {
    ...
    try {
      const r = await se(o);
      if (l) return;
      y.innerHTML = K(r, t.provider);
    } catch {
      if (l) return;
      y.innerHTML = K([], t.provider);
    } finally {
      l || (y.disabled = !1);
    }
  }
  async function _() {
    ...
    try {
      ...
      if (l) return;
      let p = t.model;
      !p && r.length && (p = r[0], j(J(t.provider), p)), m.innerHTML = V(r, p), E();
    } catch {
      if (l) return;
      m.innerHTML = V([], t.model);
    } finally {
      l || (m.disabled = !1);
    }
  }
