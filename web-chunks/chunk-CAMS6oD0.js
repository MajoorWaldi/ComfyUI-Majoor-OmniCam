...
function yi(e, { mode: t, selectedIds: a } = {}) {
...
const vi = "omnicam_humanoid_v1", ht = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function Mi({ preset: e, overrides: t } = {}) {
  const a = ze(e), r = ze(t);
  return {
...
function ki(e, { startFrame: t, endFrame: a } = {}) {
...
function Yt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = q(t), l = t.position, m = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = $n(m, i);
  let p, f;
  if (t.camera_type === "orthographic") {
...
const Yn = 3;
function Zn(e, t, a, r, o) {
  const n = B(e, a, r, o), s = B(t, a, r, o);
  return !n || !s ? !1 : Math.hypot(n[0] - s[0], n[1] - s[1]) < Yn;
}
function Qn(e, { pointerX: t, pointerY: a, overHandle: r, viewCamera: o, e: n }) {
  ...
  return !m || Zn(s.position, m.camera.position, o, e.canvas.width, e.canvas.height) ? !1 : (r && n.stopImmediatePropagation?.(), e.curveHandleDrag = {
    cameraId: s.cameraId,
    frame: s.frame,
    side: s.side,
...
const es = ["maya", "blender", "simple"];
function Ct(e) {
  const t = e.state?.navigation_profile;
  return es.includes(t) ? t : "maya";
}
function ts(e, t) {
...
function ss(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function is(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  s.keyframes = rs(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = ce(s, e.frame, e.state.objects), s.camera = G(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function Jt(e, t) {
  const a = [];
...
function rl(e, t = {}) {
  ...
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), cs(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? _("Framed: all objects") : _("Framed: {name}").replace("{name}", s.name || s.type || _("Subject")));
}
function ds(e, t, a, { forceLocal: r = !1 } = {}) {
...
function Vr(e) {
  ...
  const t = ps(e);
  if (!t || e.transformControlsWiring && fs.has(t.type)) return null;
  ...
  const n = Math.max(0.7, H(A(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? ds(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
...
function ol(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : hs(e.ctx, s[0], i);
    }
  }
...
function Ie(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = ys(a), m = a.axis ? lt[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], p = q(c), f = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...A(c.position, c.target)) * 25e-4;
  let h = a.snapshots.map((y) => [...y.transform.position]);
  if (a.mode === "translate") {
    const y = l ?? (n - s) * f * i, M = m ? D(m, y) : x(D(p.right, n * f * i), D(p.up, -s * f * i));
    h = h.map((v) => x(v, M)), h = bs(e, a, h, o, t.ctrlKey || t.metaKey, m, a.snapshots.map((v) => v.transform.position));
  }
...
const oe = (e, t) => Math.round(e / t) * t, Ss = (e, t) => e.map((a) => oe(a, t));
function ws(e, t, a) {
...
function ge(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, m) => Math.abs(o.axis[m]) > 1e-6 ? oe(l, i) : o.base[m]) : Ss(t, i);
  if (s === "vertex" && a && !o) {
...
function nl(e, t) {
  ...
  if (m && (!c || p === "path_point" || p === "camera") && Qn(e, { pointerX: r, pointerY: o, overHandle: c, viewCamera: n, e: t }) || m && c || l && Xn(e, { pointerX: r, pointerY: o, shiftKey: t.shiftKey, altKey: t.altKey })) return;
  ...
  if (h) {
    ...
    if (h.entity.type === "camera_path" && ss(e, { baseDrag: k, viewCamera: n, entityPosition: h.entity.position })) return;
  ...
  const u = !!e.isNavigatingFly, w = Jn(e, t, n);
  if (!u && !w) return;
...
function sl(e, t) {
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_path") return void is(e, { pointer: i, deltaPixels: m, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
...
function ll(e, t) {
  ...
  const a = ts(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
...
function He(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Ms(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function tt(e, t) {
  return ks(t, e.interactionElement);
}
function js(e, t) {
...
function cl(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = et(e.state);
    n && (e.checkpoint("Set motion interpolation"), xs(n, o.target.value), re(e, `Motion interpolation: ${o.target.value}`));
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? He(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? Es(e, s) : n === "erase" && js(e.state, s), re(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = ta(e), l = As(e.state, n.points, s, i);
...
const ye = { t: "translate", r: "rotate", s: "scale" }, Rs = [
...
], Ls = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function Fs(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(Ls) : !1;
}
function Ks(e) {
...
function Vs(e, t) {
  return Bs(e) || t?.lastKeyZone || "viewport";
}
let ra = !1;
function Gs() {
...
function qs(e, t) {
  ...
  if (Ks(a) || (t.code === "Space" || t.key === "Enter") && Fs(a)) return !1;
...
function Qs([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * Ys), n = Math.round((window.innerHeight || t) * Zs);
  return [
...
function Js(e, t) {
  const a = Zr[t];
  return !a || !e?.setSize ? !1 : (e.setSize(Qs(a.default, a.min)), !0);
}
function ei(e, t, a) {
  const r = Zr[t];
...
function ai() {
  ...
  e.id = oa, e.textContent = ti, document.head.appendChild(e);
}
function ne(e) {
...
const sa = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", ia = "oc-help-toolbar-css", ni = "#8b7bd8";
function si() {
  ...
  e.id = ia, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${ni};color:#fff;
...
te.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Dt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-D01t7mVy.js").then((n) => n.g);
...
te.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== mt) return;
    const t = !le, a = t ? null : jt(e);
    await Dt(e, async () => (await import("./chunk-CKy5Yg0z.js")).attachExtractor), e.__majoorOmniCamExtractor && At(e, mt, t, a?.());
  }
});
te.registerExtension({
  ...
  async nodeCreated(e) {
    if (Xe(e) !== dt) return;
    const t = !le, a = t ? null : jt(e);
    await Dt(e, async () => (await import("./chunk-DnHhjPU1.js")).attachMonitor), e.__majoorOmniCamMonitor && At(e, dt, t, a?.());
  }
});
export {
  ms as $,
