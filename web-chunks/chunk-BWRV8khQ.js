import { T as ye } from "./chunk-IMiHbUfz.js";
import { ab as Fe, ae as Ye, af as Ke, ag as He, ah as Qe, W as Ze, s as be, Z as Je, i as Ee, u as ce, H as et, l as tt, G as ie, k as rt, Y as ot, a2 as at, ai as ze, aj as je, M as he, ak as We, n as ve, al as Ie, P as st, c as nt, e as Ne, D as oe, a5 as it, d as ct, V as Ge, v as lt, f as Me, J as Ce, m as De, z as dt, a1 as ut, a3 as mt, ac as ht, a8 as ft } from "./vendor-three-B8JDtKPi.js";
import { bN as pt, bO as wt, bP as gt, bc as yt, a as Mt, H as xt } from "./chunk-Cgt_u7Uc.js";
import { r as bt, q as vt, a as Pe, s as Ue, D as Ve, c as Ct, b as Bt, d as Lt } from "./chunk-CKCGudqZ.js";
import { c as St } from "./chunk-a2yd8Eqb.js";
import { o as _t } from "./chunk-DHEns4tR.js";
      ...
      if (!["grid", "point_field"].includes(n))
        for (const a of G.objects) {
          ...
          if (a.type === "glb" || a.type === "model") {
            ...
            const P = !!G.backface_culling, A = bt(a, G, o) ?? (a.material_mode || "textured");
            ...
            u = new e.Mesh(St(e), k(a, n, !!G.backface_culling));
      ...
      (G.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: G.keyframes || [] }]).forEach((h, M) => {
          ...
          if (F.renderOrder = 900, F.userData.omnicamWidget = "path", a && !h.locked && (F.userData.omnicamPathSegments = {
            ...
            if (V.renderOrder = 899, V.userData.omnicamWidget = "path", this.path.add(V), O.length >= 8) {
              ...
              for (let U = Math.floor(z / 2); U < l - 1; U += z) {
                const Y = O[U], $ = O[U + 1].clone().sub(Y).normalize(), J = new e.ConeGeometry(A * 1.5, A * 3, 8);
                J.rotateX(Math.PI / 2);
                ...
                d.quaternion.copy(Z), d.position.copy(Y), d.renderOrder = 901, d.userData.omnicamWidget = "path", this.path.add(d);
              }
            }
          }
        }
        for (const u of c) {
          ...
          if (X) {
            ...
            const U = new e.Vector3().crossVectors(z, V).normalize(), Y = e.MathUtils.clamp(P.distanceTo(A) * 0.08, 0.25, 0.8), $ = u.camera.camera_type === "orthographic" ? Y * 0.55 : Y * Math.tan(e.MathUtils.degToRad(u.camera.fov || 35) * 0.5), J = $ * (G.width || 16) / Math.max(1, G.height || 9), Z = P.clone().addScaledVector(V, Y), d = [
            ...
            const se = Ft(e, {
              ...
              up: U,
              color: B.marker,
              scale: e.MathUtils.clamp(Y * 1.15, 0.35, 1.6),
              active: a
            });
          ...
          if (X) {
            const V = _t(u, c[S - 1] || null, c[S + 1] || null);
            for (const z of ["in", "out"]) {
              const U = new e.Vector3().fromArray(V[z]), Y = new e.Line(
              ...
              Y.renderOrder = 912, Y.userData.omnicamWidget = "gizmo", this.path.add(Y);
              const $ = new e.Mesh(
    ...
    updateLiveCameras(w, t, o, n, m = "camera", s = null) {
      ...
      (w.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: w.keyframes || [] }]).forEach((g, h) => {
        ...
        if (!a) {
          ...
          Y.position.set(0, 0.07, -0.08), F.add(Y);
          ...
          for (const le of T) K.push(u, le);
          for (let le = 0; le < 4; le++) K.push(T[le], T[(le + 1) % 4]);
          const E = T[2].clone().add(T[3]).multiplyScalar(0.5).clone().addScaledVector(A, b * 0.25);
          K.push(T[2], E, E, T[3]);
          const re = new e.BufferGeometry().setFromPoints(K), Se = new e.LineSegments(re, new e.LineBasicMaterial({
        ...
        if (_ > 0.01) {
          ...
          const U = F ? 0.12 : B ? 0.11 : 0.08, Y = [
            ...
            S.clone().add(new e.Vector3(0, 0, U))
          ], $ = new e.BufferGeometry().setFromPoints(Y), J = new e.LineSegments($, new e.LineBasicMaterial({
        ...
        if (c && n !== "camera" && m === "camera") {
          ...
          U.rotateX(Math.PI / 2);
          const Y = new e.Mesh(U, new e.MeshBasicMaterial({ color: 15913067, side: e.DoubleSide, transparent: !0, opacity: 0.35 }));
          Y.position.copy(u), Y.userData.omnicamWidget = "gizmo", this.liveCameras.add(Y);
        }
      });
    },
...
const ge = ["high", "balanced", "low"], Kt = 25, ke = 30, Ht = 0.6;
...
function Zt(r, e) {
  if (!Number.isFinite(e) || e < 0 || (r.samples.push(e), r.samples.length > ke && r.samples.shift(), r.samples.length < ke) || r.samples.filter((D) => D > Kt).length / r.samples.length < Ht) return null;
  ...
  return {
    render(t, o, n, m, s, i = /* @__PURE__ */ new Map(), C = 0, g = !1, h = "camera", M = "subject", c = null, B = null, a = null) {
        ...
        if (d)
          ...
          this.bgTextureLoads.set(l, b), new e.TextureLoader().load(l, (I) => {
            ...
            for (I.colorSpace = e.SRGBColorSpace, this.bgTextureCache.set(l, I); this.bgTextureCache.size > 8; ) {
              ...
              if (!T) break;
              const K = this.bgTextureCache.get(T);
              this.bgTextureCache.delete(T), K?.dispose?.();
            }
      ...
      const _ = JSON.stringify([
        ...
        !!g,
        t.objects.map((d) => {
          const { position: b, rotation: W, keyframes: I, size: T, ...K } = d;
          return d.type === "card" && (K.size = T), K;
        })
      ...
      if (U !== this.pathKey && (this.pathKey = U, this.rebuildPath(t, h, B, V, a)), this.updateLiveCameras(t, C, g, V, h, B), this.liveCameras.visible = !g, !g) {
        ...
        for (const I of [this.path, this.liveCameras])
          I.traverse((T) => {
            const K = T.userData.omnicamWidget;
            K === "path" ? T.visible = d : K === "gizmo" ? T.visible = b : K === "lookat" && (T.visible = W);
          });
      }
      const Y = m / Math.max(1, s), $ = this.configureCamera(o, Y);
      if (this.activeCamera = $, g ? this.selectionGroup.visible = !1 : (this.updateSelection(t, h, M, c, `${t.__omnicamRevision ?? "legacy"}:${C}`, S), this.selectionGroup.visible = !0), this.studioEnabled && this.contentShadowKey !== this.sceneKey) {
        ...
        if (b) {
          const W = d.isEmpty() ? new e.Vector3() : d.getCenter(new e.Vector3()), I = d.isEmpty() ? new e.Vector3(12, 12, 12) : d.getSize(new e.Vector3()), T = Math.max(1, 0.5 * Math.max(I.x, I.y, I.z) * Math.SQRT2), K = T * 1.15 + 0.5, se = new e.Vector3(4.5, 7.5, 3.5).normalize(), E = Math.max(12, T * 4);
          b.position.copy(W).addScaledVector(se, E), b.target.position.copy(W), b.target.updateMatrixWorld(!0);
          const re = b.shadow.camera;
          re.left = -K, re.right = K, re.top = K, re.bottom = -K, re.near = Math.max(0.1, E - T - 1), re.far = E + T + 1, re.updateProjectionMatrix(), b.shadow.map?.dispose(), b.shadow.map = null;
        }
      }
    ...
    supersampleFactor() {
      return this.studioEnabled && vt(this.studio?.quality).renderScale || 1;
    },
    dispose() {
      if (!this.disposed) {
...
class nr {
  constructor(e = () => {
    ...
    y.position.set(5, 8, 4), this.scene.add(y), this.flatLights = [this.scene.children.at(-2), y], this.studio = Ct(ye, this.renderer, Lt), this.scene.add(this.studio.group), this.studioEnabled = !0, Ue(ye, this.scene, this.renderer, this.studio, !0), this.content = new ie(), this.scene.add(this.content), this.path = new ie(), this.scene.add(this.path), this.liveCameras = new ie(), this.scene.add(this.liveCameras), this.selectionGroup = new ie(), this.scene.add(this.selectionGroup), this.selectionKey = "", this.perspective = new rt(35, 16 / 9, 0.01, 1e4), this.orthographic = new ot(-5, 5, 2.8125, -2.8125, 0.01, 1e4), this.sceneKey = "", this.mediaSignature = "", this.bgImageUrl = "", this.bgTexture = null, this.bgTextureCache = /* @__PURE__ */ new Map(), this.bgTextureLoads = /* @__PURE__ */ new Map(), this.bgLoadGeneration = 0, this.disposed = !1, this.invalidate = e, this.onModelLoaded = f, this.modelUrls = /* @__PURE__ */ new Map(), this.models = /* @__PURE__ */ new Map(), this.modelLoads = /* @__PURE__ */ new Map(), this.objectNodes = /* @__PURE__ */ new Map(), this.raycaster = new at(), this.pointer = new Fe(), this.activeCamera = this.perspective;
  }
...
async function pr(r, e, f, y, D, p = "balanced") {
  ...
  return Bt(new Blob([x.target.buffer], { type: await x.getMimeType() }), {
    encoder: "webcodecs",
    requestedFrames: e,
...
export {
  ...
  pr as encodeDeterministicPlayblast,
