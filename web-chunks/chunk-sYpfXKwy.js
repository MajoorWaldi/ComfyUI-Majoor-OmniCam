import { v as i, z as H } from "./chunk-CUzpmyH7.js";
import { l as W, S as U, b as F, p as V, u as z, d as B } from "./chunk-CGAuv3Kp.js";
import "../../scripts/app.js";
import { api as m } from "../../scripts/api.js";
import { e as a, a as I, d as G, b as K, r as J } from "./chunk-KD2odbme.js";
import { M as Y, E as Q } from "./chunk-CbqXtcpr.js";
function S(t) {
  return Array.isArray(t) && t.length === 1 ? t[0] : t;
}
function X(t) {
  const e = t?.ui && typeof t.ui == "object" ? t.ui : t || {}, o = Array.isArray(e.preflight) && e.preflight.length === 1 && Array.isArray(e.preflight[0]) ? e.preflight[0] : e.preflight, r = S(e.capabilities), n = S(e.target_profile);
  return {
...
function $(t) {
  const e = I(t.state), o = t.message ? `<br><small>${a(t.message)}</small>` : "", r = t.recoverable ? ` <span class="oc-recoverable">${a(i("recoverable"))}</span>` : "";
...
function E(t, e, { live: o = !1 } = {}) {
  const r = X(e), n = r.preflight.filter((d) => d.mapping_quality), s = r.preflight.filter(C), c = r.preflight.filter((d) => !d.mapping_quality && !C(d)), l = t.querySelector('[data-role="profile-preflight"]');
  l.innerHTML = c.length ? c.map($).join("") : `<div class="oc-empty">${a(i("No preflight checks returned."))}</div>`;
  const p = t.querySelector('[data-role="profile-diff"]');
  p && (p.innerHTML = n.length ? n.map(Z).join("") : `<div class="oc-empty">${a(i("No mapping-quality diagnostics for this compile."))}</div>`);
  const x = t.querySelector('[data-role="profile-health"]');
  x && (x.innerHTML = s.length ? s.map($).join("") : `<div class="oc-empty">${a(i("No guide-health warnings."))}</div>`);
  const _ = Array.isArray(r.capabilities.capabilities) ? r.capabilities.capabilities : [], D = t.querySelector('[data-role="profile-capabilities"]');
  D.innerHTML = _.length ? _.map((d) => `<div class="oc-row"><span>${a(d.display || d.adapter)}</span><span class="oc-state" data-state="${I(d.state)}">${a(d.state)}</span></div>`).join("") : `<div class="oc-empty">${a(i("No optional downstream capability detected."))}</div>`;
  const f = r.preflight.some((d) => String(d.state).toUpperCase() === i("BLOCKED")), w = t.querySelector('[data-role="monitor-status"]');
  return w.dataset.state = f ? i("BLOCKED") : i("READY"), w.lastChild.textContent = f ? " " + i("BLOCKED") : " " + i("READY"), t.querySelector('[data-role="output-status"]').textContent = ee(f, r.targetProfile, o), r;
}
const te = "MajoorOmniCamDirector";
function oe(t) {
...
function M(t, e) {
  ...
  return o?.link == null || !t?.graph ? null : W(t.graph, o.link);
}
function se(t) {
  const e = M(t, "motion_scene"), o = M(t, "playblast_video");
  return {
    sceneConnected: !!e,
    sceneOrigin: e,
    sceneNodeClass: O(e),
...
class ce {
  ...
  poll() {
    const e = se(this.node), o = JSON.stringify([
...
const T = "majoor.omnicam.monitor.preflight", pe = 1;
function ue(t, e, o) {
  const r = (n) => {
    const s = n?.detail;
    !s || Number(s.schema_version) !== pe || s.kind === "blocked_preflight" && String(s.node) === String(e.id) && (!s.output || o.disposed || o.blockedPreflight(s.output));
  };
  return t.addEventListener(T, r), () => {
    t.removeEventListener?.(T, r);
  };
}
const N = [
  "camera_motion",
...
function me(t) {
  const e = k.includes(t.media_type) ? t.media_type : "image";
  return `<div class="oc-reference-row" data-role="reference-row">
    <input type="text" data-field="id" placeholder="${a(i("id"))}" value="${a(t.id || "")}">
    <select data-field="media_type">${k.map((o) => `<option value="${o}"${e === o ? " selected" : ""}>${a(o)}</option>`).join("")}</select>
    <input type="number" min="1" max="30" data-field="slot_hint" placeholder="1" value="${t.slot_hint || ""}">
    <select data-field="roles" multiple size="4" title="${a(i("Roles this reference is declared for"))}">${P(N, t.roles)}</select>
    <select data-field="ignore" multiple size="4" title="${a(i("Roles this reference explicitly does not carry"))}">${P(N, t.ignore)}</select>
...
function R(t, e) {
  ...
  o && (o.innerHTML = e.length ? e.map(me).join("") : `<div class="oc-empty">${a(i("No additional references declared."))}</div>`);
}
function q(t) {
...
function y(t) {
  return [...t.querySelectorAll('[data-role="reference-row"]')].map((e) => {
    const o = e.querySelector('[data-field="id"]')?.value.trim() || "", r = e.querySelector('[data-field="media_type"]')?.value || "image", n = e.querySelector('[data-field="slot_hint"]')?.value || "", s = q(e.querySelector('[data-field="roles"]')), c = q(e.querySelector('[data-field="ignore"]')), l = { id: o, media_type: r, roles: s };
...
function be() {
  return ye.map(([t, e]) => `<option value="${t}">${a(e)}</option>`).join("");
}
function ve() {
  return ge.map(([t, e]) => `<option value="${t}">${a(i(e))}</option>`).join("");
}
function xe() {
  return `<div class="majoor-omnicam oc-monitor">
    <style>${fe}</style>
    <header class="oc-header">${F("OmniCam Monitor")}
    ...
    <main class="oc-layout">
      ...
      <aside class="oc-column">
        <div class="oc-card"><div class="oc-section">${a(i("Compilation target"))}</div><div class="oc-adapter-controls">
          <label class="wide">${a(i("Profile"))}<select data-role="profile-select">${ve()}</select></label>
...
function _e(t = document) {
  ...
  return e.innerHTML = xe(), e.firstElementChild;
}
const h = [
  "base_prompt",
  "target_profile",
...
function b(t) {
  return Object.fromEntries(h.map((e) => [e, j(t, e)?.value]));
}
function v(t, e, o) {
  if (!h.includes(e)) return !1;
  const r = j(t, e);
  return r ? (r.value = Se.has(e) ? Number(o) : o, r.callback?.(r.value), !0) : !1;
}
function $e(t) {
  return String(t || "").startsWith("h3_");
}
const Ce = 250, Ee = /* @__PURE__ */ new Set(["duration_seconds", "target_fps"]);
function Oe(t) {
  we(t);
}
class Me {
  constructor(e) {
    this.node = e, this.root = _e(), this.events = new Q(), this.source = null, this.player = new ae(
      this.root.querySelector('[data-role="proxy-player"]'),
      {
        onFrame: (o) => this.showFrame(o),
        onMetadata: ({ frameCount: o }) => this.setFrameCount(o)
      }
    ), this.hasExecutedOnce = !1, this._liveUnavailableText = "", this.disposed = !1, this.connectionRefreshTimer = null, this.refreshController = new ne(m, {
      onSnapshot: (o) => this.liveSnapshotReceived(o),
      onError: (o) => this.liveRefreshFailed(o)
    }), this.bindControls(), this.syncControlsFromWidgets(), this.loadProfileInfo(), this.watcher = new ce(e, (o) => this.sourceChanged(o)), this.liveTimer = setInterval(() => this.liveTick(), Ce);
  }
  ...
  bindControls() {
    this.events.on(this.root, "wheel", V(this.root)), this.events.on(this.root.querySelector('[data-act="proxy-play"]'), "click", () => this.player.toggle()), this.events.on(this.root.querySelector('[data-role="proxy-scrubber"]'), "input", (o) => this.player.scrub(o.target.value)), this.events.on(this.root.querySelector('[data-role="proxy-loop"]'), "change", (o) => this.player.setLoop(o.target.checked)), this.events.on(this.root.querySelector('[data-role="proxy-mute"]'), "change", (o) => this.player.setMuted(o.target.checked)), this.events.on(this.root.querySelector('[data-role="profile-select"]'), "change", (o) => {
      v(this.node, "target_profile", o.target.value), this.updateH3SetupHint(o.target.value), this.settingsChanged();
    });
    for (const o of this.root.querySelectorAll("[data-setting]"))
      this.events.on(o, "change", () => {
        v(this.node, o.dataset.setting, o.value), this.settingsChanged();
      });
    ...
    this.events.on(e, "click", (o) => {
      ...
      if (!r) return;
      const n = r.closest('[data-role="reference-row"]'), c = [...this.root.querySelectorAll('[data-role="reference-row"]')].indexOf(n), l = y(this.root);
      c >= 0 && l.splice(c, 1), this.syncReferenceMatrix(l);
    }), this.events.on(e, "change", (o) => {
      o.target.closest('[data-role="reference-row"]') && this.commitReferenceMatrix(y(this.root));
    });
  }
  /** Repaints the matrix rows from `specs`, then commits. Only for add/remove. */
  syncReferenceMatrix(e) {
    R(this.root, e), this.commitReferenceMatrix(e);
  }
  ...
  commitReferenceMatrix(e) {
    v(this.node, "reference_plan_json", JSON.stringify(e)), this.settingsChanged();
  }
  /**
   ...
   */
  settingsChanged() {
    g(this.source?.sceneOrigin) ? this.liveTick() : this.markOutdated();
  }
  updateH3SetupHint(e) {
  ...
  syncControlsFromWidgets() {
    ...
    for (const r of h) {
      ...
      !n || e[r] == null || (Ee.has(r) && Number(e[r]) <= 0 ? n.value = "" : n.value = e[r]);
    }
    R(this.root, he(e.reference_plan_json)), this.reflectInheritedShot();
  }
  /**
  ...
  reflectInheritedShot() {
    const e = this.source?.sceneOrigin, o = g(e) ? L(e) : null, r = {
  ...
  liveTick() {
    ...
    const o = ie(e, b(this.node)), r = o.director, n = `${r.state_json}\0${JSON.stringify(o.monitor)}\0${r.recording_path}\0${r.card_asset}\0${r.width}x${r.height}@${r.fps}/${r.duration_seconds}:${r.render_mode}`;
    n !== this._liveKey && (this._liveKey = n, this.refreshController.schedule(o));
  }
  liveSnapshotReceived(e) {
    this.disposed || E(this.root, e, { live: !0 });
  }
  liveRefreshFailed(e) {
  ...
  refreshPlayblastPreview() {
    if (this.disposed) return;
    const e = this.root.querySelector('[data-role="proxy-upstream-preview"]'), o = this.root.querySelector(".oc-player-empty"), r = this.source?.playblastOrigin, n = G(m, r);
    ...
    this.player.setSource(""), B(s, e, 640).then((p) => {
  ...
  updateReferenceSourceLabel(e, o) {
    ...
    if (!r) return;
    const n = K(o, e);
    r.textContent = n, r.hidden = !n, r.dataset.warn = J(o, e);
  }
  setFrameCount(e) {
  ...
  dispose() {
    this.disposed || (this.disposed = !0, clearInterval(this.liveTimer), clearTimeout(this.connectionRefreshTimer), H(), this.refreshController?.dispose(), this.watcher?.dispose(), this.player.dispose(), this.events.dispose());
  }
}
function Ie(t) {
  if (t.__majoorOmniCamMonitor) return;
  Oe(t);
  const e = new Me(t), o = ue(m, t, e);
