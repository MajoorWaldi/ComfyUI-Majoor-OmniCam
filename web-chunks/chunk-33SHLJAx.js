...
let de = null;
function qr() {
  return de || typeof Image > "u" || (de = new Image(), de.src = Gr), de;
}
function Hr() {
    ...
    beforeRegisterNodeDef(t, a) {
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        if (n.save(), this.selected) {
          const d = Hr(), p = n.createRadialGradient(m, c, Y * 0.35, m, c, Y * 1.15);
...
function oo(e) {
  ...
  return e.motion_layers = (Array.isArray(e.motion_layers) ? e.motion_layers : []).slice(0, 256).map((r, o) => {
    ...
    const s = Zt.includes(r?.source_kind) ? r.source_kind : "manual_2d", i = (Array.isArray(r?.keys) ? r.keys : []).slice(0, 1e4).map((l) => ro(l, t)).sort((l, m) => l.time_seconds - m.time_seconds);
    return {
      id: n,
...
function qs(e, t, a, r) {
  ...
  if (!o || n <= 0) return 0;
  const i = (d) => o.loop ? (d % n + n) % n : Jt(d, 0, n);
...
const Do = 128, kt = "omnicam_humanoid_v1", mt = /^[a-z0-9][a-z0-9_-]*$/, To = 1e-3, jo = (e) => Array.isArray(e) && e.length === 3 && e.every((t) => Number.isFinite(Number(t)));
function dt(e) {
...
function Qs(e) {
  const t = A(e.target, e.position), a = q(t), r = a < 1e-6 ? [0, 0, -1] : k(t, 1 / a), o = Math.asin(T(r[1], -1, 1)) * 180 / Math.PI, n = Math.atan2(r[0], -r[2]) * 180 / Math.PI;
  return [o, n, e.roll || 0];
}
function Js(e, t) {
  const [a, r, o] = t, n = Math.max(1e-4, q(A(e.target, e.position))), s = a * Math.PI / 180, i = r * Math.PI / 180, l = [Math.sin(i) * Math.cos(s), Math.sin(s), -Math.cos(i) * Math.cos(s)];
  e.target = C(e.position, k(l, n)), e.roll = o;
}
function B(e, t, a, r) {
  const { right: o, up: n, forward: s } = K(t), i = A(e, t.position), l = U(i, s);
...
function le(e, t, a = null) {
  ...
  if (u && Array.isArray(f)) {
    ...
    if (g && g.enabled !== !1) {
      const M = ft(f, g, t), j = d?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
      l = (M.position?.[0] ?? 0) + (j[0] || 0), m = (M.position?.[1] ?? 1.5) + (j[1] || 0), c = (M.position?.[2] ?? 0) + (j[2] || 0);
    }
  }
...
const T = (e, t, a) => Math.max(t, Math.min(a, e)), zo = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, pe = (e, t = null) => typeof e == "string" && zo.test(e.trim()) ? e.trim() : t, C = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], A = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], k = (e, t) => [e[0] * t, e[1] * t, e[2] * t], U = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], Se = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], q = (e) => Math.sqrt(Math.max(1e-12, U(e, e))), ne = (e) => k(e, 1 / q(e));
function ra(e, t, a) {
...
function Tt(e, t, a, r, o) {
  const n = Lo(e, t), s = Ro.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", i = o ? o(e) : 0, l = a && o ? o(a) : i, m = r && o ? o(r) : i, c = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), d = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), p = (v = !1) => {
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  const s = n || pt(e, t), { leftIndex: i, left: l, right: m } = s, c = i > 0 ? e[i - 1] : null, d = i + 2 < e.length ? e[i + 2] : null, p = Math.max(1, m.frame - l.frame), u = T((t - l.frame) / p, 0, 1);
  let f = r(l), h = r(m);
  if (o) {
  ...
  if (l.interpolation === "bezier" || m.interpolation === "bezier") {
    const v = Tt(l, a, c, m, r), b = Tt(m, a, l, d, r), x = f, w = f + (v.out_y || 0), D = h + (b.in_y || 0), g = h, M = T(Number(v.out_x ?? 1 / 3), 0, 1), j = T(1 + Number(b.in_x ?? -1 / 3), 0, 1);
    let G = 0, ee = 1;
    for (let St = 0; St < 32; St++) {
      const H = (G + ee) * 0.5, He = 1 - H;
      3 * He * He * H * M + 3 * He * H * H * j + H * H * H < u ? G = H : ee = H;
    }
...
function ei(e = "balanced", t = "all_views", a = null) {
  ...
  for (let p = 0; p < o; p++) {
    ...
    if (t === "ground_focus")
      ...
      const D = u * Math.PI * 2, g = 1 - 2 * f, M = Math.sqrt(Math.max(0, 1 - g * g)), j = 1.5 + Math.cbrt(h) * 20;
      _ = Math.cos(D) * M * j, v = Math.sin(D) * M * j, y = Math.max(0.01, g * j * 0.75 + 2.5), b = 0.72, x = 0.78, w = 0.88;
    } else {
      const D = p % 4;
      if (D === 0) {
...
function ti(e) {
  ...
  n = [...new Map(n.map((c) => [c.frame, c])).values()].sort((c, d) => c.frame - d.frame), n.length || (n = [{ frame: 0, camera: V(o), interpolation: "ease" }]);
  ...
  const m = ze();
  return a.editor_views = Object.fromEntries(Object.entries(m).map(([c, d]) => [c, V(a.editor_views?.[c] || d)])), oo(a);
}
function xe(e, t) {
...
function ia(e, t = {}, a = null, r = "generic") {
  ...
  const m = qo(l, o), c = Ho(l, o), d = At(m, o), p = At(d, o), u = { speed: m, angular_speed: c, acceleration: d, jerk: p }, f = Wo(e, a), h = $o(l, f, s, i), _ = l.map((g) => g.fov), y = t.allow_framing_loss === !0, v = [], b = [];
  for (let g = 0; g < n; g++) {
    const M = [], j = [];
    for (const G of jt) {
      const ee = Et(u[G][g], t[`max_${G}`]);
      M.push(ee), ee !== "ok" && j.push(G);
    }
    !h[g] && !y && (M.push("over"), j.push("framing_loss")), v.push(Ot(M)), b.push(j);
  }
  ...
  for (const g of [...jt, "fov_drift"]) {
    const M = g === "fov_drift" ? "max_fov_change" : `max_${g}`, j = t[M];
    j != null && w[M] > Number(j) && w.violations.push({ metric: M, value: w[M], recommended_max: Number(j) });
  }
...
function Qo(e, t) {
  ...
  if (c.style.left = `${d}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${m !== 0 ? ` (${m > 0 ? "+" : ""}${m})` : ""}`, a.moving && a.moving.length > 1) {
    ...
    for (const f of a.moving) {
      let h = T(f.startFrame + m, 0, e.state.duration_frames - 1);
...
function mi(e) {
  ...
  const r = ma(e.state.keyframes, a, 0.6);
  Me(
    e,
    r,
...
function pi(e, t, a) {
  if (!me(e)) return;
  const o = ma(e.state.keyframes, [{ start: t, end: a }], 0.6);
  Me(
    e,
    o,
    "Smooth zone",
...
function Cr() {
  const e = String(F(da, "auto")), t = String(F("Comfy.Locale", "en") || "en").slice(0, 2).toLowerCase(), a = e === "auto" ? t : e, r = a !== Xr();
  if (Ur(a), !!r) {
    for (const o of $)
      if (!o.disposed)
        try {
...
function wn() {
  return {
    ...
    playblastGrid: O(va, !1),
    playblastLabels: O(Sa, !1),
    ...
    showGrid: O(ja, !0),
    showRadar: O(Aa, !0),
    showCameraPaths: O(Ea, !0),
    showCameraGizmos: O(Oa, !0),
    showLookAt: O(Ia, !0),
    showHelperAxes: O(Pa, !0),
    showGizmo: O(za, !0),
    guides: O(Na, !0),
    safeAreas: O(Ra, !1),
    resolutionGate: O(La, !1),
    aspectRatio: N(Fa, "auto", ["auto", "16:9", "4:3", "1:1", "9:16", "2.39:1"]),
    burnIn: O(Ba, !1),
    speedHeatmap: O(Va, !1),
    showWireframe: O(Ka, !1),
    showVertices: O(Ga, !1),
    ...
    loopPlayback: O(cr, !1),
    uiDensity: N(mr, "animation", ["basic", "animation", "advanced"]),
    previewLayout: N(dr, "auto", ["auto", "1", "2", "4"]),
    cameraViewVisible: O(pr, !0),
...
function Rt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = K(t), l = t.position, m = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = kn(m, i);
  let d, p;
  if (t.camera_type === "orthographic") {
...
function Le(e, t) {
  return (e.camera_type === "orthographic" ? 10 / Math.max(0.01, e.zoom || 1) : 2 * q(A(e.position, e.target)) * Math.tan((e.fov || 35) * Math.PI / 360)) / Math.max(1, t);
}
function _e(e) {
...
function In(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  ...
  return !o || o.locked || !(o.keyframes?.length >= 1) ? !1 : (e.checkpoint("Transform camera path"), e.gizmoDrag = {
    ...
    viewRight: K(a).right,
    viewUp: K(a).up,
    freeScale: a.camera_type === "orthographic" ? Le(a, e.canvas.height) : q(A(a.position, r)) * (2 * Math.tan((a.fov || 35) * Math.PI / 360)) / e.canvas.height
  }, !0);
}
...
function Nn(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => zn(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((x) => x[b]))), m = [0, 1, 2].map((b) => Math.max(...i.map((x) => x[b]))), c = l.map((b, x) => (b + m[x]) / 2), { right: d, up: p, forward: u } = K(t), f = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), h = Math.tan((t.fov || 35) * Math.PI / 360), _ = h * f;
  let y = 2, v = 0.1;
  for (const b of i) {
...
function wi(e, t = {}) {
  ...
  if (e.subSelection?.point && !t.all) {
    ...
    const i = e.subSelection.point, l = ne(A(a.position, o)), m = Number.isFinite(l[0]) && q(l) > 0.1 ? l : [0.707, 0.4, 0.707], c = 2;
...
function Ar(e) {
  ...
  const n = Math.max(0.7, q(A(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? Rn(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: i.map((m, c) => ({ index: c, axis: m, points: [o, B(C(r, k(m, n)), a, e.canvas.width, e.canvas.height)] })).filter((m) => m.points[1] && Number.isFinite(m.points[1][0]) && Number.isFinite(m.points[1][1]))
    };
  const l = i.map((m, c) => {
...
function Er(e, t) {
  ...
  for (const i of a.handles)
    for (let l = 0; l < i.points.length - 1; l++) {
      const m = i.points[l], c = i.points[l + 1], d = ra(t, m, c);
  ...
  if (e.state.view_mode !== "camera") {
    for (const s of e.state.cameras) {
      for (const c of s.keyframes || []) {
        ...
        if (!d) continue;
        const p = B(d, r, e.canvas.width, e.canvas.height);
    ...
    if (e.state.show_camera_paths !== !1)
      for (const s of e.state.cameras) {
        ...
        for (let m = 0; m < l.length - 1; m += 1)
          if (ra(t, l[m], l[m + 1]) <= 8 * Math.min(2, window.devicePixelRatio || 1))
...
function Ae(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = Kn(a), m = a.axis ? at[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], d = K(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...A(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, x = m ? k(m, b) : C(k(d.right, n * p * i), k(d.up, -s * p * i));
    u = u.map((w) => C(w, x)), u = Gn(e, a, u, o, t.ctrlKey || t.metaKey, m, a.snapshots.map((w) => w.transform.position));
  }
  ...
  a.snapshots.forEach((b, x) => {
    ...
    if (a.mode === "scale") {
      const D = y ? Fe(h, 0.1) : h, g = m ? m.map((j) => j ? D : 1) : [D, D, D], M = A(b.transform.position, a.pivot);
      w.position = C(a.pivot, M.map((j, G) => j * g[G])), w.size = b.transform.size.map((j, G) => Math.max(0.01, j * g[G]));
    }
...
function he(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, m) => Math.abs(o.axis[m]) > 1e-6 ? re(l, i) : o.base[m]) : Wn(t, i);
  if (s === "vertex" && a && !o) {
...
function Mi(e, t) {
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = X(e), s = e.state.view_mode !== "camera", i = Lt(e, t), l = t.button === 0 && !i, m = l && !t.altKey && !t.shiftKey;
  ...
  if (c) {
    ...
    if (c.entity.type === "object") {
      ...
      const g = D.reduce((M, j) => C(M, j.transform.position), [0, 0, 0]).map((M) => M / D.length);
      e.gizmoDrag = {
        ...b,
        type: "object",
        historyCheckpointed: !0,
        ...
        viewRight: K(n).right,
        viewUp: K(n).up,
        freeScale: n.camera_type === "orthographic" ? Le(n, e.canvas.height) : q(A(n.position, c.entity.position)) * (2 * Math.tan((n.fov || 35) * Math.PI / 360)) / e.canvas.height
      };
      return;
    }
  }
  const d = l ? Fn(e, [r, o]) : null;
...
function Ci(e, t) {
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_path") return void Pn(e, { pointer: i, deltaPixels: m, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
  ...
  if (e.drag.dolly) {
    ...
    const s = A(n.target, n.position), i = q(s);
    ...
    const { right: s, up: i } = K(n), l = Le(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), m = C(k(s, -a * l), k(i, r * l));
    e.drag.target.position = C(n.position, m), e.drag.target.target = C(n.target, m);
  } else {
    const s = A(n.position, n.target), i = q(s);
    let l = Math.atan2(s[0], s[2]), m = Math.asin(T(s[1] / i, -0.999, 0.999));
...
function Qn(e, t, a, r, o) {
  ...
  if (t?.object_id) {
    ...
    if (!c) return null;
    const d = Ko(e.objects, c), p = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
...
function Gt(e, t, a) {
  const r = e.viewportCamera(), o = e.state.view_mode !== "camera", n = A(r.position, r.target), s = q(n);
  if (!(s > 1e-4)) return;
...
function hs(e, t) {
  ...
  if (ds(a) || (t.code === "Space" || t.key === "Enter") && ms(a)) return !1;
...
function bs(e, t) {
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const i = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: m, up: c, forward: d } = K(i), p = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, u = { w: k(d, p), s: k(d, -p), d: k(m, p), a: k(m, -p), e: k(c, p), q: k(c, -p) }[a];
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await yt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-DcUqvPUh.js").then((n) => n.g);
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== ot) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-XKYp6bFY.js")).attachExtractor), e.__majoorOmniCamExtractor && _t(e, ot, t, a?.());
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== nt) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-BnV2y_4d.js")).attachMonitor), e.__majoorOmniCamMonitor && _t(e, nt, t, a?.());
  }
});
export {
  ya as $,
