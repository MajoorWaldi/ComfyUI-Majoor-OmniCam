import { a, a2 as j, bE as te, bF as F, bG as w } from "./chunk-DbT8L9qU.js";
...
function me(e) {
  ...
  return e.provider === "ollama" ? J(e.baseUrl) ? a("Planning stays on the configured local Ollama endpoint.") : n : e.provider === "openai_compatible" && J(e.baseUrl) ? a("Planning stays on the configured local endpoint.") : n;
}
function V(e, n) {
...
function he(e, n = {}) {
  ...
  async function _() {
    ...
    try {
      ...
      if (l) return;
      let p = t.model;
      !p && r.length && (p = r[0], j(F(t.provider), p)), m.innerHTML = Y(r, p), E();
    } catch {
      if (l) return;
  ...
  function B() {
    !m || !m.value || (j(F(w().provider), m.value), E());
  }
  function R() {
  ...
  return f?.addEventListener("click", D), y?.addEventListener("change", R), m?.addEventListener("change", B), d?.addEventListener("input", G), E(), z(), I(), _(), {
    get state() {
      return g;
    },
    dispose() {
      l = !0, L?.abort?.(), f?.removeEventListener("click", D), y?.removeEventListener("change", R), m?.removeEventListener("change", B), d?.removeEventListener("input", G);
    }
  };
}
export {
