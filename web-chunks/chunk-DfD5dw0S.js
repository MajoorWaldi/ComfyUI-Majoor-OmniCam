...
function Do({ fetchApi: e = kd } = {}) {
  ...
  return {
    list(a = {}) {
      ...
      return o && o !== "all" && (l.kind = o), t(`${_e}${dr(l)}`);
    },
    get(a) {
    ...
    importModel(a, o = {}) {
      const n = new FormData();
      return n.append("file", a, o.filename || a.name || "model.glb"), t(`${_e}/import${dr(o)}`, { method: "POST", body: n });
    },
...
function Ld(e, t = {}) {
  const a = e.root, o = t.fetchApi || ((O, H) => (e.api || e.app?.api).fetchApi(O, H)), n = t.apiClient || Do({ fetchApi: o }), s = t.store || Cd(n), i = _d(), c = t.previewQueue || Ed(), l = e.api || e.app?.api || null;
  ...
  let Q = "", k = null, _ = !0, z = !0, R = "scene", q = 0;
...
function Bd(e, t = {}) {
  ...
  function P() {
    const k = b();
    if (!k) return;
    const _ = m.map((q) => Number(q?.value) || 0), z = ns(_), R = e.directorApi?.execute({
      version: 1,
  ...
  function D() {
    const k = s?.value || "neutral", _ = u.get(k) || Qo({ preset_id: k });
  ...
  async function F() {
    ...
    const _ = (await St(e, r("Save Pose"), r("Pose name"), ""))?.trim();
    if (!_ || e.disposed) return;
    const z = _.toLowerCase().replace(qd, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "pose";
    try {
      ...
      u.set(R.pose.id, g(R.pose)), e.setStatus?.(r("Pose saved: {name}").replace("{name}", _)), x();
    } catch (R) {
...
function Yd(e, { controlsFactory: t, anchorFactory: a } = {}) {
  ...
  function d({ targetSpec: k }) {
    if (k) {
      if (l(c), k.type === "object") {
        ...
        const _ = ls(e), z = (_.length ? _ : [k.object]).map((q) => ({
          object: q,
          transform: ze(q)
        }));
      ...
      if (k.type === "path_point" || k.type === "path_group") {
        ...
        e.checkpoint(k.type === "path_point" ? "Transform path point" : "Transform path selection"), n = {
          ...
          baseKeys: _.keyframes.map((R) => ({ ...R, camera: de(R.camera) })),
          lookAtActive: Gr(_, e.state.objects)
        };
        return;
      }
  ...
  function g(k) {
    ...
    _ && (_.keyframes = Xr(n.baseKeys, h(k)), b(_));
  }
  function y(k) {
    const _ = e.state.cameras.find((R) => R.id === n.trackId);
    if (!_) return;
    const z = { ...h(k), lookAtActive: n.lookAtActive };
    _.keyframes = ms(n.baseKeys, n.selectedFrames, z), b(_);
  }
  function v(k) {
    const _ = e.state.cameras.find((z) => z.id === n.trackId);
    _ && (_.keyframes = ps(n.baseKeys, [n.frame], { delta: k.position }), b(_));
  }
  function j({ delta: k }) {
  ...
  function x(k) {
    const { group: _, pivot: z } = n, R = e.state.gizmo_mode;
    for (const q of _)
      if (R === "translate")
  ...
  function M(k) {
    if (e.state.gizmo_mode === "translate")
      ...
      e.camera.target = ye(n.position, er(_, k.rotationDeg));
    }
    e.commitCameraEdit();
  }
  function P(k) {
    const _ = ye(n.base, k.position);
    n.tracking ? ds(e, _) : e.camera.target = _, e.commitCameraEdit();
  }
  ...
  function F() {
    ...
    const k = e.recording ? null : cs(e), _ = e.state.gizmo_mode || "translate", z = !!k && !k.readOnly && Xd.has(k.type) && k.allowedModes.includes(_);
...
function om(e, t, a, o) {
  ...
  const [P, D] = j(d[0] || 0, d[2] || 0), [F, U] = j(h[0] || 0, h[2] || 0), B = 8, V = Z(P, s + B, s + c - B), Q = Z(D, i + B, i + c - B), k = Z(F, s + B, s + c - B), _ = Z(U, i + B, i + c - B), z = d[1] || 0, R = tm(z), q = am(z);
...
function im(e, t, a, o) {
  ...
  if (s.drag) {
    const [b, g] = _o(n, a, o), y = e.viewportCamera();
  ...
  const p = kn(n, a, o), f = Sn(e, n, a, o), [u, d] = _o(n, a, o);
...
function Om(e, t) {
  ...
  a.addEventListener("input", n, { signal: t }), a.addEventListener("change", () => {
    ...
    const c = Number(a.value) / 100, l = Em(s(i), c);
...
const Wa = () => import("./chunk-DdsMWPjz.js");
function Bm(e, t) {
...
function dp(e) {
  ...
  ], m = e.planeAxis === "y" ? _n(l) : En(l, [0, 0, -1]), p = e.seedPoint ? 1 : 0, f = [];
...
function Mp(e) {
  ...
  for (const g of e.state.motion_layers || []) {
    if (g.enabled === !1) continue;
    const y = Cp.includes(g.source_kind), v = y ? Ep(e.state, g, u) : _p(g);
...
function Er(e, t, { frameCount: a = 0, fps: o = 0 } = {}) {
...
function Wp(e) {
  for (const t of [...e.cardMediaById?.keys?.() || []]) _t(e, t);
}
function Re(e, t, a, o = !1, n = "") {
  ...
  s && s !== a && _t(e, t), o && In.add(a), qp(a), e.cardMediaAssetById ||= /* @__PURE__ */ new Map(), e.cardMediaById.set(t, a), e.cardMediaAssetById.set(t, n || a?.__omnicamAsset || "");
  try {
    a.__omnicamAsset = n || a.__omnicamAsset || "";
  } catch {
  }
...
function Hp(e) {
  ...
  for (const o of e.state.objects) {
    if (!o.asset) {
      (o.type === "model" || o.type === "glb") && (o.load_error = r("Not saved to the ComfyUI input folder: this model will be missing after a reload.")), o.type === "card" && _t(e, o.id);
      continue;
    }
    const n = De(o.asset);
    ...
    if (d) {
      if (u === "image" || u === "video") {
        ...
        if (h && h.value) {
          ...
          }), Re(e, "subject", b, !1, b.currentSrc || b.src || ""), Er(e, b, { frameCount: b instanceof HTMLVideoElement ? 0 : 1 }), e.upstreamImageConnected = !0, s = !0, e.render(), e.setStatus(b instanceof HTMLVideoElement ? r("Upstream video preview synced") : r("Upstream image preview synced")));
        }
      }
      if (u === "audio") {
        l = !0;
...
async function nf(e) {
  ...
  try {
    let s = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: l } = await import("./chunk-DO8pviEl.js");
    ...
    if (t.appendChild(v), !j)
      for (const { object: x, level: M } of y) {
        ...
        _.style.cssText = V ? `color:${oe.error}` : B ? "" : "opacity:.5;text-decoration:line-through", _.textContent = x.name || x.type, _.title = r("Double-click to rename"), _.addEventListener("dblclick", ($) => {
          $.preventDefault(), $.stopPropagation(), Hf(e, x, _);
        }), k.appendChild(_);
...
function Jf(e) {
  ...
  if (s) {
    const k = n.target_object_id || e.state.target_object_id || "", _ = `T${tr()}${e.state.objects.map((z) => `${z.id}\0${z.name || z.type}`).join("|")}`;
    vo(s, _, () => [
  ...
  if (!t) {
    ...
    _ && (_.textContent = `${n.name} · F${e.frame}`), Ir(o, {
  ...
  if (g) {
    ...
    const R = e.state.objects.filter(($) => !_.has($.id)), q = `P${tr()}${k}${R.map(($) => `${$.id} ${$.name || $.type}`).join("|")}`;
    vo(g, q, () => [
      vt("", r("No parent")),
  ...
  if (h && (h.hidden = !F?.animations), b) {
    ...
    vo(b, `A${k.join("|")}`, () => k.map((_, z) => vt(String(z), _)), String(t.animation_index || 0));
  }
...
function sh(e, t) {
  e.objectUrls.revoke(t), _t(e, t), e.modelUrlsById.delete(t), e.modelInfoById.delete(t), e.webgl?.removeModel(t);
}
function ih(e) {
...
function Oh(e) {
  ...
  return {
    ...
    openPathKeyContext(I, T, S) {
      ...
      const A = E ? po(E) : "auto", K = this.selectedKeyFrames?.size || 0, X = this.pathSelection?.component === "target", L = Gr(w, this.state.objects);
    ...
    openTimelineContext(I, T) {
      const S = this.selectedKeyFrames?.size || 0, w = this.selectedKeyframe(), E = w?.interpolation || "ease", A = w && po(w) || "auto", K = ["ease", "linear", "bezier", "smooth", "ease_in", "ease_out", "sine", "cubic", "quintic", "expo", "back"], X = ["auto", "clamped", "vector", "free", "aligned", "flat"];
    ...
    openCurveContext(I) {
      const T = this.selectedKeyFrames?.size || 0, S = T < 2 && !this.selectedKeyframe(), w = this.selectedKeyframe(), E = w?.interpolation || "ease", A = w && po(w) || "auto", K = ["bezier", "smooth", "linear", "ease_in", "ease_out", "ease", "sine", "cubic", "quintic", "expo", "back"], X = ["auto", "clamped", "vector", "free", "aligned", "flat"];
...
function Uh(e) {
  ...
  return {
    ...
    setPathSelectionComponent(w) {
      this.pathSelection = _i(this.pathSelection, w), this.refreshInspector(), this.render();
    },
    ...
    transformCameraPath(w) {
      ...
      const A = Xr(E.keyframes, { origin: Ur(E.keyframes), ...w });
    ...
    redistributeActiveCameraTiming() {
      ...
      const E = [...w.keyframes].sort((K, X) => K.frame - X.frame), A = ji(E, {
    ...
    async openCameraPathPresetPicker() {
      ...
      const E = ac.map((K) => ({ id: K, label: r(ar[K] || K) })), A = await an({ title: r("Camera Path Preset"), items: E, owner: this });
    ...
    applyCameraPathPreset(w, E = {}) {
      ...
      const [K, X] = Mo(this.state), L = tc({ type: w, camera: this.camera, startFrame: K, endFrame: X, params: E });
...
function Yh(e) {
  const { right: t, up: a, forward: o } = Ei(e || {});
  return Xh.map((n) => {
...
function pu(e) {
  ...
  return {
    ...
    renderViewportOnly() {
      ...
      if (this.webgl) {
        try {
          const te = this.recording ? 1 : this.webgl.supersampleFactor?.() ?? 1, Oe = te > 1 ? Math.min(te, 4096 / Math.max(1, w, E)) : 1, Na = Oe > 1 ? Math.round(w * Oe) : w, co = Oe > 1 ? Math.round(E * Oe) : E;
          this.webgl.activeCamera && this.transformControlsWiring?.sync(), this.webgl.render(qe, K, this.cardMediaById, Na, co, this.modelUrlsById, this.frame, this.recording, this.selectedEntity, this.selectedObjectId, this.subSelection, this.selectedKeyFrame ?? null, this.selectedKeyFrames ? [...this.selectedKeyFrames] : null, this.recording && this.state.guide_capture_style || "auto"), S.imageSmoothingEnabled = !0, S.imageSmoothingQuality = "high", Na !== w || co !== E ? S.drawImage(this.webgl.canvas, 0, 0, Na, co, 0, 0, w, E) : S.drawImage(this.webgl.canvas, 0, 0, w, E), J = !0;
        } catch (te) {
    ...
    requestUiUpdate(S = Ce.viewport, w = "unknown") {
      (this.renderReasons ||= /* @__PURE__ */ new Set()).add(w), this.uiDirtyMask = sc(this.uiDirtyMask, S), this.renderInvalidations = (this.renderInvalidations || 0) + 1, !this.renderScheduled && (this.renderScheduled = !0, this.renderFrame = requestAnimationFrame(() => {
        ...
        this.uiDirtyMask = 0, this.lastRenderReasons = [...this.renderReasons || []], this.renderReasons?.clear(), this.rendersCoalesced = (this.rendersCoalesced || 0) + 1, this.perf && (this.perf.renderCount = (this.perf.renderCount || 0) + 1), Ge(E, Ce.outliner) && this.refreshObjects(), Ge(E, Ce.timeline) && this.refreshKeys(), Ge(E, Ce.inspector) && this.refreshInspector(), Ge(E, Ce.viewport) && this.renderViewportOnly(), Ge(E, Ce.previews) && this.renderCameraView(), Ge(E, Ce.motion) && this.renderMotionUiOnly();
      }));
    },
    renderCameraView() {
...
class Xo {
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-DO8pviEl.js"));
    } catch (a) {
  ...
  try {
    a.assetBrowser = Ld(a, {
      ...
      onAgentFirstOpen: async () => {
        if (wo())
          try {
            const { createDirectorAgentPanel: o } = await import("./chunk-w0muu9Jx.js");
            a.agentPanel = o(a);
          } catch (o) {
