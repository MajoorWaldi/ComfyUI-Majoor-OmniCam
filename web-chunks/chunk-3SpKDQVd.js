import { T as Me } from "./chunk-__IQ4xkd.js";
import { V as ze, a as Ye, O as He, b as Qe, E as Ze, W as Je, S as ve, P as Ee, c as et, C as ce, H as tt, D as rt, G as ie, d as ot, e as at, f as st, g as je, F as We, M as he, h as Ie, i as Ce, j as Ne, k as nt, l as it, m as Ue, n as oe, o as ct, B as lt, p as De, A as dt, q as xe, r as Be, s as Pe, t as ut, u as mt, v as ht, w as ft, x as pt } from "./vendor-three-BQUrLQkn.js";
import { T as ge, bR as wt, bS as gt, bT as yt, ba as Mt, a as xt, C as bt } from "./chunk-yjHTInyb.js";
import { r as vt, q as Ct, a as Ae, s as Re, D as Ve, c as Bt, b as St, d as Lt } from "./chunk-DKzkRqD6.js";
import { c as _t } from "./chunk-a2yd8Eqb.js";
import { n as Gt } from "./chunk-CnsE-RVk.js";
import { Output as Dt, BufferTarget as Pt, WebMOutputFormat as At, CanvasSource as Vt, QUALITY_HIGH as kt, QUALITY_MEDIUM as Ot, QUALITY_LOW as Tt, canEncodeVideo as Ft } from "./vendor-mediabunny-CZ5VNE-V.js";
function zt(r, { position: e, forward: f, up: y, color: D, scale: p = 1, active: v = !0 }) {
...
function jt(r, { position: e, color: f = 15903035, radius: y = 0.28, bold: D = !1 }) {
...
function It(r, e, { color: f = null, opacity: y = null } = {}) {
  const D = f ?? qe, p = y ?? 0.65;
  if (fe(e)) {
    ...
    return x.bindMode = e.bindMode, x.bind(e.skeleton, e.bindMatrix), Xe(x, e), { overlay: pe(x), parent: e.parent || e };
  }
...
function Nt(r, e) {
  const f = new r.PointsMaterial({ color: qe, size: 0.05, sizeAttenuation: !0 });
  if (!fe(e)) {
    const N = new r.Points(e.geometry, f);
    return { overlay: pe(N), parent: e };
  }
  const y = e.geometry.getAttribute("position")?.count || 0, D = Math.max(1, Math.ceil(y / Wt)), p = Math.ceil(y / D), v = new Float32Array(p * 3), x = new r.BufferGeometry();
...
function Rt(r, e, { wireframe: f = !1, vertices: y = !1, wireframeColor: D = null, wireframeOpacity: p = null } = {}) {
  if (!f && !y) return;
  const v = [];
  e.traverse((x) => {
  ...
  for (const x of v) {
    if (f) {
      const { overlay: q, parent: j } = It(r, x, { color: D, opacity: p });
      j.add(q);
    }
    if (y) {
      const { overlay: q, parent: j } = Nt(r, x);
      j.add(q);
    }
  }
}
const qt = 16777215, ne = 0.17, ke = 3593923, Xt = 0.06;
function $t(r) {
  ...
  return {
    ...
    rebuild(G, w, t, o = !1) {
      this.content.traverse((a) => {
        for (const S of [...a.children])
          S.userData.omnicamHelper && (a.remove(S), R(S, !0));
      ...
      if (B.userData.omnicamCaptureGuide = !0, m.add(B), this.content.add(m), ["omni_ref", "point_field"].includes(n)) {
        const { points: a, colors: S } = ee(G.point_density || "balanced", G.point_spread || "all_views", G.point_color || null);
        if (a.length > 0) {
          const u = new e.BufferGeometry();
          u.setAttribute("position", new e.Float32BufferAttribute(a, 3)), u.setAttribute("color", new e.Float32BufferAttribute(S, 3));
          const L = new e.PointsMaterial({
            vertexColors: !0,
            size: 0.065,
            sizeAttenuation: !0
          }), l = new e.Points(u, L);
      ...
      if (!["grid", "point_field"].includes(n))
        for (const a of G.objects) {
          ...
          if (a.type === "glb" || a.type === "model") {
            ...
            const P = !!G.backface_culling, V = vt(a, G, o) ?? (a.material_mode || "textured");
            _?.url === l && (u = _.scene, N(u, V, a, P));
            ...
            V.userData.omnicamLightHelper = !0, V.visible = !o, l.add(V), u = l;
            ...
            const l = new e.Group(), _ = (a.cone_angle ?? 45) * Math.PI / 180, O = a.penumbra ?? 0.25, P = new e.SpotLight(a.color || 16777215, a.intensity ?? 3, 0, _, O, 2), V = (a.rotation || [0, 0, 0]).map(e.MathUtils.degToRad), X = new e.Vector3(0, 0, -1).applyEuler(new e.Euler(V[0], V[1], V[2], "YXZ"));
            ...
            u = !a.material_mode || ["textured", "wireframe_texture"].includes(a.material_mode) ? H(a, w.get(a.id), G.card_fit || "contain") : new e.Mesh(new e.PlaneGeometry(S[0], S[1]), k(a, n, !!G.backface_culling));
          else if (a.type === "null") {
          ...
          const L = ["sun_light", "point_light", "spot_light"].includes(a.type);
          if (a.type !== "card" && !L && u.scale.fromArray(S), u.userData.omnicamId = a.id, u.frustumCulled = !1, u.traverse((l) => {
            ...
            Rt(e, u, { wireframe: l, vertices: G.show_vertices });
          }
    ...
    rebuildPath(G, w = "camera", t = null, o = "", n = null) {
      ...
      (G.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: G.keyframes || [] }]).forEach((h, M) => {
        ...
        const B = h.color ? { line: new e.Color(h.color), marker: new e.Color(h.color), frustum: new e.Color(h.color) } : i[M % i.length], a = h.id === G.active_camera_id, S = a && w === "camera";
        if (c.length >= 2) {
          const u = c[0].frame, L = c[c.length - 1].frame, l = Math.max(32, Math.min(256, L - u + 1)), _ = { ...h, keyframes: c, objects: G.objects }, O = Array.from({ length: l }, (A, z) => {
            const U = u + (L - u) * z / Math.max(1, l - 1);
            return new e.Vector3().fromArray(Q(_, U, G.objects).position);
          }), P = new e.CatmullRomCurve3(O, !1, "centripetal"), V = S ? 0.06 : a ? 0.045 : 0.025, X = new e.MeshBasicMaterial({
            color: B.line,
            transparent: !0,
            opacity: a ? 1 : 0.55,
            depthTest: !1
          }), F = new e.Mesh(new e.TubeGeometry(P, Math.max(48, l), V, 8, !1), X);
          if (F.renderOrder = 900, F.userData.omnicamWidget = "path", a && !h.locked && (F.userData.omnicamPathSegments = {
            cameraId: h.id,
            firstFrame: u,
            lastFrame: L,
            frames: c.map((A) => A.frame),
            points: O.map((A) => [A.x, A.y, A.z])
          }), this.path.add(F), a) {
            const A = new e.Mesh(
              new e.TubeGeometry(P, Math.max(48, l), V * (S ? 3 : 2.4), 8, !1),
              new e.MeshBasicMaterial({ color: B.line, transparent: !0, opacity: S ? 0.3 : 0.18, depthTest: !1 })
            );
            if (A.renderOrder = 899, A.userData.omnicamWidget = "path", this.path.add(A), O.length >= 8) {
              const z = Math.max(6, Math.floor(l / 8));
              for (let U = Math.floor(z / 2); U < l - 1; U += z) {
                const K = O[U], $ = O[U + 1].clone().sub(K).normalize(), J = new e.ConeGeometry(V * 1.5, V * 3, 8);
                J.rotateX(Math.PI / 2);
        ...
        for (const u of c) {
          const L = c.indexOf(u), l = a, _ = new e.Mesh(
            new e.SphereGeometry(l ? ne : 0.085, 16, 12),
            new e.MeshBasicMaterial({ color: l ? qt : B.marker, depthTest: !1 })
          );
          ...
          const P = new e.Vector3().fromArray(u.camera.position), V = new e.Vector3().fromArray(u.camera.target || [0, 0, 0]), X = a && t != null && u.frame === t, F = a && !X && m?.has(u.frame);
          if (X) {
            const A = new e.Mesh(
            ...
            A.position.fromArray(u.camera.position), A.renderOrder = 911, A.userData.omnicamBillboard = !0, A.userData.omnicamWidget = "path", this.path.add(A);
          } else if (F) {
            const A = new e.Mesh(
          ...
          if (X) {
            const A = V.clone().sub(P).normalize();
            let z = new e.Vector3().crossVectors(A, new e.Vector3(0, 1, 0));
            z.lengthSq() < 1e-8 ? z.set(1, 0, 0) : z.normalize();
            const U = new e.Vector3().crossVectors(z, A).normalize(), K = e.MathUtils.clamp(P.distanceTo(V) * 0.08, 0.25, 0.8), $ = u.camera.camera_type === "orthographic" ? K * 0.55 : K * Math.tan(e.MathUtils.degToRad(u.camera.fov || 35) * 0.5), J = $ * (G.width || 16) / Math.max(1, G.height || 9), Z = P.clone().addScaledVector(A, K), d = [
          ...
          if (X) {
            const A = jt(e, {
              position: V,
              radius: e.MathUtils.clamp(P.distanceTo(V) * 0.05, 0.16, 0.5) * 1.4,
              bold: !0
            });
            A.userData.omnicamWidget = "lookat", this.path.add(A);
            const z = new e.Line(
              new e.BufferGeometry().setFromPoints([P.clone(), V.clone()]),
          ...
          if (X) {
            const A = Gt(u, c[L - 1] || null, c[L + 1] || null);
            for (const z of ["in", "out"]) {
              const U = new e.Vector3().fromArray(A[z]), K = new e.Line(
                new e.BufferGeometry().setFromPoints([P.clone(), U.clone()]),
                new e.LineBasicMaterial({ color: ke, transparent: !0, opacity: 0.95, depthTest: !1 })
              );
              K.renderOrder = 912, K.userData.omnicamWidget = "gizmo", this.path.add(K);
              const $ = new e.Mesh(
                new e.SphereGeometry(Xt, 12, 8),
                new e.MeshBasicMaterial({ color: ke, depthTest: !1 })
              );
      ...
      (G.objects || []).forEach((h, M) => {
        ...
        const B = h.color ? new e.Color(h.color) : g[M % g.length], a = c.map((L) => new e.Vector3().fromArray(L.transform?.position || [0, 0, 0])), S = new e.CatmullRomCurve3(a, !1, "centripetal"), u = new e.Mesh(
          new e.TubeGeometry(S, Math.max(32, c.length * 16), 0.035, 8, !1),
        ...
        for (const L of c) {
          ...
          l.position.fromArray(L.transform?.position || [0, 0, 0]), l.renderOrder = 910, l.userData.omnicamWidget = "path", this.path.add(l);
        }
      });
    }
  };
  ...
  return {
    updateLiveCameras(w, t, o, n, m = "camera", s = null) {
      ...
      (w.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: w.keyframes || [] }]).forEach((g, h) => {
        ...
        let V = new e.Vector3().crossVectors(P, l).normalize();
        if (S.roll) {
          const F = e.MathUtils.degToRad(S.roll);
          P.applyAxisAngle(l, F), V.applyAxisAngle(l, F);
        }
        ...
        if (!a) {
          ...
          const $ = new e.Matrix4().makeBasis(P, V, l.clone().negate());
          ...
          const d = e.MathUtils.clamp(_ * 0.25, 0.5, 2.5), b = S.camera_type === "orthographic" ? 5 / Math.max(0.01, S.zoom || 1) * 0.35 : d * Math.tan(e.MathUtils.degToRad(S.fov || 35) * 0.5), W = b * (w.width || 16) / Math.max(1, w.height || 9), I = u.clone().addScaledVector(l, d), T = [
            I.clone().addScaledVector(P, -W).addScaledVector(V, -b),
            I.clone().addScaledVector(P, W).addScaledVector(V, -b),
            I.clone().addScaledVector(P, W).addScaledVector(V, b),
            I.clone().addScaledVector(P, -W).addScaledVector(V, b)
          ], Y = [];
          ...
          const E = T[2].clone().add(T[3]).multiplyScalar(0.5).clone().addScaledVector(V, b * 0.25);
          Y.push(T[2], E, E, T[3]);
          const re = new e.BufferGeometry().setFromPoints(Y), _e = new e.LineSegments(re, new e.LineBasicMaterial({
          ...
          const Ge = new e.Mesh(we, new e.MeshBasicMaterial({
            color: B ? M.marker : M.frustum,
            transparent: !0,
            opacity: 0.12,
            depthTest: !1,
        ...
        if (_ > 0.01) {
          const F = c && m === "camera_target", A = new e.BufferGeometry().setFromPoints([u, L]), z = new e.Line(A, new e.LineDashedMaterial({
          ...
          const U = F ? 0.12 : B ? 0.11 : 0.08, K = [
            L.clone().add(new e.Vector3(-U, 0, 0)),
            L.clone().add(new e.Vector3(U, 0, 0)),
            L.clone().add(new e.Vector3(0, -U, 0)),
            L.clone().add(new e.Vector3(0, U, 0)),
            L.clone().add(new e.Vector3(0, 0, -U)),
            L.clone().add(new e.Vector3(0, 0, U))
          ...
          if (d.position.copy(L), d.userData = { omnicamType: "camera_target", omnicamId: g.id }, this.liveCameras.add(d), (F || B) && n !== "camera") {
            const b = new e.RingGeometry(0.14, 0.18, 24);
            b.rotateX(Math.PI / 2);
            const W = new e.MeshBasicMaterial({ color: ge.typeLookAt, side: e.DoubleSide, transparent: !0, opacity: 0.9 }), I = new e.Mesh(b, W);
            I.position.copy(L), I.userData.omnicamWidget = "lookat", this.liveCameras.add(I);
          }
        }
        if (c && n !== "camera" && m === "camera") {
          const F = new e.RingGeometry(0.19, 0.24, 32);
          F.rotateX(Math.PI / 2);
          const A = new e.MeshBasicMaterial({ color: ge.accent, side: e.DoubleSide, transparent: !0, opacity: 1 }), z = new e.Mesh(F, A);
          z.position.copy(u), z.userData.omnicamWidget = "gizmo", this.liveCameras.add(z);
          const U = new e.RingGeometry(0.28, 0.31, 32);
          U.rotateX(Math.PI / 2);
          const K = new e.Mesh(U, new e.MeshBasicMaterial({ color: ge.accent, side: e.DoubleSide, transparent: !0, opacity: 0.35 }));
          ...
          if (g) {
            ...
            if (w.show_wireframe) {
              ...
              g.traverse((M) => {
                ...
                const c = Ut(e, M, new e.MeshBasicMaterial({
                  color: 9133302,
                  transparent: !0,
                  opacity: 0.2,
                  depthTest: !0,
            ...
            if (n && n.objectId === o && n.point) {
              if (n.mode === "vertex") {
                ...
                const B = new e.RingGeometry(0.1, 0.15, 24), a = new e.MeshBasicMaterial({ color: 9133302, side: e.DoubleSide, depthTest: !1 }), S = new e.Mesh(B, a);
                S.position.fromArray(n.point), this.activeCamera && S.quaternion.copy(this.activeCamera.quaternion), S.renderOrder = 1e4, this.selectionGroup.add(S);
  ...
  return {
    ...
    pickPathSegment(w) {
      ...
      for (let u = 0; u < g.length; u += 1) {
        const [L, l, _] = g[u], O = L - n.point.x, P = l - n.point.y, V = _ - n.point.z, X = O * O + P * P + V * V;
        X < M && (M = X, h = u);
      }
      ...
      if (B === a) return null;
      const S = Math.min(1, Math.max(0, (c - B) / (a - B)));
      return { cameraId: m, leftFrame: B, rightFrame: a, t: S };
    },
    configureCamera(w, t) {
    ...
    pickSubElement(w, t, o, n, m = "vertex") {
      ...
      for (const i of s) {
        ...
        if (m === "vertex") {
          let a = -1, S = 1 / 0, u = null;
          if (i.face) {
            const L = [i.face.a, i.face.b, i.face.c];
            for (const l of L) {
              const _ = new e.Vector3(c.getX(l), c.getY(l), c.getZ(l)).applyMatrix4(B), O = _.distanceTo(i.point);
              O < S && (S = O, a = l, u = [_.x, _.y, _.z]);
            }
          } else
            for (let L = 0; L < c.count; L++) {
              const l = new e.Vector3(c.getX(L), c.getY(L), c.getZ(L)).applyMatrix4(B), _ = l.distanceTo(i.point);
              _ < S && (S = _, a = L, u = [l.x, l.y, l.z]);
            }
          if (u)
            return {
              type: "vertex",
        ...
        if (m === "edge" && i.face) {
          const a = new e.Vector3(c.getX(i.face.a), c.getY(i.face.a), c.getZ(i.face.a)).applyMatrix4(B), S = new e.Vector3(c.getX(i.face.b), c.getY(i.face.b), c.getZ(i.face.b)).applyMatrix4(B), u = new e.Vector3(c.getX(i.face.c), c.getY(i.face.c), c.getZ(i.face.c)).applyMatrix4(B), L = (V, X, F) => {
            const A = new e.Line3(X, F), z = new e.Vector3();
            return A.closestPointToPoint(V, !0, z), { dist: V.distanceTo(z), point: z, segment: [X, F] };
          }, l = L(i.point, a, S), _ = L(i.point, S, u), O = L(i.point, u, a), P = [l, _, O].reduce((V, X) => X.dist < V.dist ? X : V);
          return {
            type: "edge",
            mode: "edge",
            objectId: h,
        ...
        if (m === "face" && i.face) {
          const a = new e.Vector3(c.getX(i.face.a), c.getY(i.face.a), c.getZ(i.face.a)).applyMatrix4(B), S = new e.Vector3(c.getX(i.face.b), c.getY(i.face.b), c.getZ(i.face.b)).applyMatrix4(B), u = new e.Vector3(c.getX(i.face.c), c.getY(i.face.c), c.getZ(i.face.c)).applyMatrix4(B), L = new e.Vector3().add(a).add(S).add(u).divideScalar(3), l = i.face.normal.clone().transformDirection(B);
          return {
            type: "face",
            mode: "face",
            objectId: h,
...
const ye = ["high", "balanced", "low"], Ht = 25, Oe = 30, Qt = 0.6;
function Te(r = "balanced") {
  return { quality: r, samples: [], downgraded: !1 };
}
function Zt(r) {
  const e = ye.indexOf(r);
  return e < 0 || e >= ye.length - 1 ? null : ye[e + 1];
}
function Jt(r, e) {
  if (!Number.isFinite(e) || e < 0 || (r.samples.push(e), r.samples.length > Oe && r.samples.shift(), r.samples.length < Oe) || r.samples.filter((D) => D > Ht).length / r.samples.length < Qt) return null;
  const y = Zt(r.quality);
...
function er(r) {
  ...
  return {
    render(t, o, n, m, s, i = /* @__PURE__ */ new Map(), C = 0, g = !1, h = "camera", M = "subject", c = null, B = null, a = null) {
      const S = !g || (t.render_mode || "") === "beauty";
      if (S !== this.studioEnabled) {
        this.studioEnabled = S, Re(e, this.scene, this.renderer, this.studio, S);
        for (const d of this.flatLights || []) d.visible = !S;
      }
      const u = !!t.objects?.some((d) => d.type === "sun_light" && d.enabled !== !1);
      if (this.studio?.key && (this.studio.key.visible = !u && S), this.flatLights?.[1] && (this.flatLights[1].visible = !u && !S), this.disposed) return;
      (this.canvas.width !== m || this.canvas.height !== s) && this.renderer.setSize(m, s, !1);
      const L = (o && o.camera_type === "orthographic") === !0;
      ...
      if (l) {
        ...
        this.scene.background = this.studioEnabled && !d && !L ? this.studio.sky : new e.Color(d ? t.viewport_bg_color : this.studioEnabled && L ? 1447709 : Ve);
      }
      ...
      const V = Math.max(1, t.fps || 24), X = /* @__PURE__ */ new Map();
      ...
      for (const [d, b] of this.models) {
        ...
        const W = X.get(d);
        W ? (this.applyMotionClip?.(d, W), b.mixer.setTime(yt(W, C, V, b.duration))) : b.mixer.setTime(C / V % b.duration);
      }
      for (const d of t.objects) {
      ...
      const A = t.view_mode || "camera", z = Array.isArray(a) ? [...a].sort((d, b) => d - b).join(",") : "", U = `${A}:${h}:${B ?? ""}:${z}:${t.__omnicamRevision ?? JSON.stringify([
        t.active_camera_id,
      ...
      if (U !== this.pathKey && (this.pathKey = U, this.rebuildPath(t, h, B, A, a)), this.updateLiveCameras(t, C, g, A, h, B), this.liveCameras.visible = !g, !g) {
      ...
      if (this.activeCamera = $, g ? this.selectionGroup.visible = !1 : (this.updateSelection(t, h, M, c, `${t.__omnicamRevision ?? "legacy"}:${C}`, L), this.selectionGroup.visible = !0), this.studioEnabled && this.contentShadowKey !== this.sceneKey) {
      ...
      let Z = !1;
      if (!g && !L && h === "object" && (M || t.__selectedObjectIds?.length) && !c) {
      ...
      if (Z || this.renderer.render(this.scene, $), !g && this.adaptiveQuality !== !1) {
        ...
        const d = Jt(this.qualityMonitor, performance.now() - J);
        d && (Ae(this.studio, this.renderer, d), this.onQualityDowngrade?.(d));
      }
    },
...
const Fe = { low: Tt, balanced: Ot, high: kt }, de = new Ce({ color: 10265519, roughness: 0.48, metalness: 0.06, side: oe }), $e = new Ce({ color: 2237998, roughness: 0.95, metalness: 0, side: oe }), Se = new xe({ color: 11449792, wireframe: !0, side: oe });
function Le(r = !1) {
  ...
  return f.wrapS = f.wrapT = ht, f.repeat.set(8, 8), f.colorSpace = ve, f.needsUpdate = !0, new Ce({ map: f, roughness: 0.85, metalness: 0, side: r ? Be : oe });
}
function ar(r, e, f = !1) {
  const y = e === "wireframe" ? "wireframe" : r.material_mode || "textured", D = f ? Be : oe;
  if (y === "wireframe") {
    const v = Se.clone();
...
function Ke(r) {
  ...
  return e.colorSpace = ve, e.needsUpdate = !0, e;
}
function nr(r, e, f) {
  const [y, D] = r.size || [2, 3], p = new ie(), v = new he(new Pe(y, D), new xe({ color: 1448482, side: oe, transparent: !0, opacity: 0.85 }));
...
class ir {
  constructor(e = () => {
    ...
    }), this.renderer.setPixelRatio(1), this.renderer.outputColorSpace = ve, this.renderer.shadowMap.enabled = !0, this.renderer.shadowMap.type = Ee, this.scene = new et(), this.scene.background = new ce(1184274), this.scene.add(new tt(16777215, 3159099, 2.2));
    const y = new rt(16777215, 2.4);
    y.position.set(5, 8, 4), this.scene.add(y), this.flatLights = [this.scene.children.at(-2), y], this.studio = Bt(Me, this.renderer, Lt), this.scene.add(this.studio.group), this.studioEnabled = !0, Re(Me, this.scene, this.renderer, this.studio, !0), this.content = new ie(), this.scene.add(this.content), this.path = new ie(), this.scene.add(this.path), this.liveCameras = new ie(), this.scene.add(this.liveCameras), this.selectionGroup = new ie(), this.scene.add(this.selectionGroup), this.selectionKey = "", this.perspective = new ot(35, 16 / 9, 0.01, 1e4), this.orthographic = new at(-5, 5, 2.8125, -2.8125, 0.01, 1e4), this.sceneKey = "", this.mediaSignature = "", this.bgImageUrl = "", this.bgTexture = null, this.bgTextureCache = /* @__PURE__ */ new Map(), this.bgTextureLoads = /* @__PURE__ */ new Map(), this.bgLoadGeneration = 0, this.disposed = !1, this.invalidate = e, this.onModelLoaded = f, this.modelUrls = /* @__PURE__ */ new Map(), this.models = /* @__PURE__ */ new Map(), this.modelLoads = /* @__PURE__ */ new Map(), this.objectNodes = /* @__PURE__ */ new Map(), this.raycaster = new st(), this.pointer = new ze(), this.activeCamera = this.perspective;
  }
  async loadModel(e, f, y = "glb") {
    ...
    if (!(!f || this.modelLoads.get(e) === D)) {
      ...
      try {
        let p, v = [];
        if (y === "obj") p = await new je().loadAsync(f);
        else if (y === "fbx")
          p = await new We().loadAsync(f), v = p.animations || [];
        else if (y === "stl") p = new he(await new Ie().loadAsync(f), de.clone());
        else if (y === "ply") {
          const o = await new Ne().loadAsync(f);
          o.index ? (o.getAttribute("normal") || o.computeVertexNormals(), p = new he(o, de.clone())) : p = new nt(o, new it({ color: 11449792, size: 0.025 }));
        } else {
          const o = await new Ue().loadAsync(f);
        ...
        const te = new lt().setFromObject(R), H = te.getSize(new De()), ee = Math.max(H.x, H.y, H.z), Q = Number.isFinite(ee) && ee > 1e-6 ? 2.5 / ee : 1, ae = te.getCenter(new De());
        R.scale.setScalar(Q), R.position.set(-ae.x * Q, -te.min.y * Q, -ae.z * Q);
        const G = new ie();
        G.frustumCulled = !1, G.add(R);
        const w = v.length ? new dt(p) : null;
...
const ue = { THREE: Me, FBXLoader: We, GLTFLoader: Ue, OBJLoader: je, PLYLoader: Ne, STLLoader: Ie, neutral: de, wire: Se, checkerMaterial: Le, objectMaterial: ar, applyModelMaterial: sr, disposeObject: be, textureFor: Ke, cardMesh: nr, generatePointField: Mt, sampleCamera: xt, sampleObjectTransform: bt, hasOutlineMesh: rr, SelectionOutlineRenderer: or };
Object.assign(
  ir.prototype,
  $t(ue),
  Kt(ue),
...
async function cr(r, e) {
  ...
  for (const f of ["vp9", "vp8"])
    try {
      if (await me(Ft(f, { width: r, height: e }), 5e3, `Checking ${f} support`)) return f;
    } catch {
    }
  return null;
}
...
async function wr(r, e, f, y, D, p = "balanced") {
  const v = await cr(r.width, r.height);
  if (!v) throw new Error("No supported WebCodecs WebM encoder");
  const x = new Dt({ format: new At(), target: new Pt() }), q = new Vt(r, { codec: v, quality: Fe[p] || Fe.balanced, keyFrameInterval: 1 });
  ...
  return St(new Blob([x.target.buffer], { type: await x.getMimeType() }), {
    encoder: "webcodecs",
    requestedFrames: e,
