  ...
  e.registerExtension({
    ...
    beforeRegisterNodeDef(t, a) {
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        const s = Math.max(4, Number(this.size?.[0] || 160) - J - 6), l = -26, c = s + J / 2, d = l + J / 2;
        if (n.save(), this.selected) {
          const m = so(), p = n.createRadialGradient(c, d, J * 0.35, c, d, J * 1.15);
...
function So(e) {
  ...
  return e.motion_layers = (Array.isArray(e.motion_layers) ? e.motion_layers : []).slice(0, 256).map((r, o) => {
    ...
    const i = ua.includes(r?.source_kind) ? r.source_kind : "manual_2d", s = (Array.isArray(r?.keys) ? r.keys : []).slice(0, 1e4).map((l) => _o(l, t)).sort((l, c) => l.time_seconds - c.time_seconds);
    return {
      id: n,
...
function Ds(e, { mode: t, selectedIds: a } = {}) {
...
function js(e, t, a = "top") {
  const r = Array.isArray(e?.position) ? e.position : [0, 0, 0], o = Array.isArray(e?.size) ? e.size : [1, 1, 1], n = (Number(o[1]) || 1) * 0.5;
  let i = Io.has(t) ? 0.35 : Math.max(0.2, n + 0.2);
  return a === "center" ? i = 0 : a === "bottom" && (i = -Math.max(0.2, n + 0.2)), [Number(r[0]) || 0, (Number(r[1]) || 0) + i, Number(r[2]) || 0];
}
...
function Es(e, t, a, r) {
  ...
  if (!o || n <= 0) return 0;
  const s = (m) => o.loop ? (m % n + n) % n : fa(m, 0, n);
  if (t <= o.start_frame) return s(o.offset_seconds);
  const l = o.end_frame > o.start_frame ? o.end_frame : null, c = l !== null && !o.loop ? Math.min(t, l) : t, d = o.offset_seconds + (c - o.start_frame) * o.speed / i;
  return s(d);
}
const Is = "omnicam_humanoid_v1", gt = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function Ns({ preset: e, overrides: t } = {}) {
  const a = ze(e), r = ze(t);
  return {
...
function Ls(e, { startFrame: t, endFrame: a } = {}) {
  ...
  const s = n.map((u) => Uo(u)), l = [];
  for (let u = 1; u < n.length; u += 1) {
    const f = Xo(Lt(n[u - 1]), Lt(n[u])), w = (s[u - 1] + s[u]) / 2;
    l.push(f * w);
  }
...
function H(e) {
  ...
  Math.sqrt(Z(o, o)) < 1e-6 && (r = Math.abs(a[1]) > 0.9 ? [0, 0, a[1] > 0 ? -1 : 1] : [0, 1, 0], o = Ce(a, r)), o = se(o);
  let n = se(Ce(o, a));
  if (Math.abs(e.roll || 0) > 1e-9) {
    const i = e.roll * Math.PI / 180, s = Math.cos(i), l = Math.sin(i), c = M(T(o, s), T(n, l));
    n = M(T(n, s), T(o, -l)), o = c;
  }
...
const D = (e, t, a) => Math.max(t, Math.min(a, e)), en = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, fe = (e, t = null) => typeof e == "string" && en.test(e.trim()) ? e.trim() : t, M = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], j = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], T = (e, t) => [e[0] * t, e[1] * t, e[2] * t], Z = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], Ce = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], W = (e) => Math.sqrt(Math.max(1e-12, Z(e, e))), se = (e) => T(e, 1 / W(e));
function Sa(e, t, a) {
  const r = [a[0] - t[0], a[1] - t[1]], o = [e[0] - t[0], e[1] - t[1]], n = Math.max(1e-9, r[0] * r[0] + r[1] * r[1]), i = D((o[0] * r[0] + o[1] * r[1]) / n, 0, 1);
...
function Bt(e, t, a, r, o) {
  const n = rn(e, t), i = an.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", s = o ? o(e) : 0, l = a && o ? o(a) : s, c = r && o ? o(r) : s, d = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), m = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), p = (b = !1) => {
    const y = (s - l) / d, C = (c - s) / m;
    let S = 0;
    if (!a && !r)
      S = 0;
    else if (!a)
  ...
  return { out_x: u, out_y: f, in_x: w, in_y: v, mode: i };
}
function _t(e, t) {
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  const i = n || _t(e, t), { leftIndex: s, left: l, right: c } = i, d = s > 0 ? e[s - 1] : null, m = s + 2 < e.length ? e[s + 2] : null, p = Math.max(1, c.frame - l.frame), h = D((t - l.frame) / p, 0, 1);
  let u = r(l), f = r(c);
  if (o) {
...
function Ue(e, t) {
  ...
  const r = me(e), o = (v, b) => (v.transform?.position || r.position)[b] ?? 0, n = (v, b) => (v.transform?.rotation || r.rotation)[b] ?? 0, i = (v, b) => (v.transform?.size || r.size)[b] ?? (b === 2 ? 0.01 : 1), s = _t(a, t), l = z(a, t, "pos_x", (v) => o(v, 0), !1, s), c = z(a, t, "pos_y", (v) => o(v, 1), !1, s), d = z(a, t, "pos_z", (v) => o(v, 2), !1, s), m = z(a, t, "rot_x", (v) => n(v, 0), !0, s), p = z(a, t, "rot_y", (v) => n(v, 1), !0, s), h = z(a, t, "rot_z", (v) => n(v, 2), !0, s), u = z(a, t, "scale_x", (v) => i(v, 0), !1, s), f = z(a, t, "scale_y", (v) => i(v, 1), !1, s), w = z(a, t, "scale_z", (v) => i(v, 2), !1, s);
  return {
...
function Gs(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  const n = [], i = [];
  let s = 0.65, l = 0.72, c = 0.82;
  if (typeof a == "string" && a.startsWith("#")) {
    const p = a.replace("#", "");
    p.length === 6 && (s = parseInt(p.slice(0, 2), 16) / 255, l = parseInt(p.slice(2, 4), 16) / 255, c = parseInt(p.slice(4, 6), 16) / 255);
  }
  ...
  for (let p = 0; p < o; p++) {
    ...
    n.push(w, v, b), i.push(a ? y * s : y, a ? C * l : C, a ? S * c : S);
  }
...
function qs(e) {
  ...
  a.cameras = i.slice(0, Te.maxCameras).map((d, m) => {
    let p = String(d?.id || `camera_${m + 1}`);
    s.has(p) && (p = `camera_${m + 1}`), s.add(p);
...
function Fe(e = [0, 0, 0]) {
  const [t, a, r] = e.map((d) => d * Math.PI / 360), o = Math.cos(t), n = Math.sin(t), i = Math.cos(a), s = Math.sin(a), l = Math.cos(r), c = Math.sin(r);
  return [n * i * l + o * s * c, o * s * l - n * i * c, o * i * c + n * s * l, o * i * l - n * s * c];
}
function ln(e, t) {
...
function wa(e, [t, a, r, o]) {
  const [n, i, s] = e, l = o * n + a * s - r * i, c = o * i + r * n - t * s, d = o * s + t * i - a * n, m = -t * n - a * i - r * s;
  return [l * o - m * t - c * r + d * a, c * o - m * a - d * t + l * r, d * o - m * r - l * a + c * t];
}
function cn([e, t, a, r]) {
  const o = 1 - 2 * (t * t + a * a), n = 2 * (e * t - a * r), i = 2 * (e * a + t * r), s = 1 - 2 * (e * e + a * a), l = 2 * (t * a - e * r), c = 2 * (t * a + e * r), d = 1 - 2 * (e * e + t * t), m = Math.asin(Math.max(-1, Math.min(1, i))), [p, h] = Math.abs(i) < 0.9999999 ? [Math.atan2(-l, d), Math.atan2(-n, o)] : [Math.atan2(c, s), 0];
...
function xa(e, t = {}, a = null, r = "generic") {
  ...
  const c = un(l, o), d = pn(l, o), m = Vt(c, o), p = Vt(m, o), h = { speed: c, angular_speed: d, acceleration: m, jerk: p }, u = fn(e, a), f = hn(l, u, i, s), w = l.map((g) => g.fov), v = t.allow_framing_loss === !0, b = [], y = [];
  for (let g = 0; g < n; g++) {
    const x = [], E = [];
    for (const G of Kt) {
...
function ka(e, t, a) {
  const r = a.getBoundingClientRect(), o = Math.max(1, e.state.duration_frames - 1), n = D(Number(e.timelineZoom) || 1, 0.1, 50), i = Number(e.timelinePan) || 0, s = o / n, l = (t.clientX - r.left) / Math.max(1, r.width);
  return D(Math.round(i + l * s), 0, o);
}
function Hs(e, t) {
  ...
  if (t.shiftKey)
    ...
    const n = t.currentTarget.getBoundingClientRect(), i = (t.clientX - n.left) / Math.max(1, n.width), s = D(Number(e.timelineZoom) || 1, 0.2, 30), l = D(s * r, 0.2, 30), c = a / s, d = a / l, m = (Number(e.timelinePan) || 0) + i * c;
    e.timelinePan = D(m - i * d, -a * 0.5, a), e.timelineZoom = l;
  }
...
function Xs(e, t) {
  ...
  if (e.boxSelect && t.pointerId === e.boxSelect.pointerId) {
    t.preventDefault(), t.stopPropagation();
    const a = e.boxSelect.box.getBoundingClientRect(), r = Math.max(1, e.state.duration_frames - 1), o = D(Number(e.timelineZoom) || 1, 0.1, 50), n = Number(e.timelinePan) || 0, i = r / o, s = (m) => D(n + m / Math.max(1, a.width) * i, 0, r), l = Math.min(s(e.boxSelect.startX), s(e.boxSelect.currentX)), c = Math.max(s(e.boxSelect.startX), s(e.boxSelect.currentX));
...
function _n(e, t) {
  ...
  const r = a.box.getBoundingClientRect(), o = Math.max(1, e.state.duration_frames - 1), n = D(Number(e.timelineZoom) || 1, 0.1, 50), i = Number(e.timelinePan) || 0, s = o / n;
  let l = Math.round(D(i + (t.clientX - r.left) / Math.max(1, r.width) * s, 0, o));
  l = e.snapFrame(l);
...
function wn(e, t) {
  ...
  for (let l = 1; l < a.length - 1; l++) {
    const c = n + Math.round(i * (r[l] / o));
    a[l].frame = Math.min(t - 1, Math.max(s + 1, c)), s = a[l].frame;
  }
  return a;
}
function Aa(e, t) {
  ...
  for (let i = 1; i < r.length - 1; i++)
    if (Aa(r[i].frame ?? 0, t))
      for (const s of ["position", "target"]) {
        const l = [r[i - 1], r[i], r[i + 1]].map((m) => m.camera?.[s]).filter((m) => Array.isArray(m) && m.length >= 3), c = r[i].camera?.[s];
        if (l.length < 3 || !Array.isArray(c)) continue;
        const d = [0, 1, 2].map((m) => l.reduce((p, h) => p + Number(h[m] || 0), 0) / l.length);
        n[i].camera[s] = c.map((m, p) => Number(m) + (d[p] - Number(m)) * o);
      }
  return n;
}
function Cn(e, t, a) {
...
function he(e, t, a, r) {
  const o = a == null ? _("no limit") : `${Ht(t)} / ${Ht(a)}`, n = a != null && a > 0, i = n ? Math.min(100, Math.round(t / a * 100)) : 0, s = r === "over" ? "var(--oc-danger)" : r === "warn" ? "var(--oc-warn)" : "var(--oc-ok)";
  return `
...
function Qs(e) {
  ...
  ke(e, o, "Slow to limits", _("Speed flattened, still over: this path needs about {seconds}s to fit the limit.").replace("{seconds}", i.toFixed(1)));
}
function Js(e) {
  const t = ue(e);
  if (!t) return;
...
function Zt(e, t, a, r, o) {
  const { right: n, up: i, forward: s } = H(t), l = t.position, c = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], d = Jn(c, s);
  let m, p;
  if (t.camera_type === "orthographic") {
  ...
  return [0, 1, 2].map((h) => l[h] + s[h] * d + n[h] * m + i[h] * p);
}
function ei(e) {
...
function oi(e, { pointerX: t, pointerY: a, overHandle: r, viewCamera: o, e: n }) {
  ...
  if (!i) return !1;
  const s = (e.state.cameras || []).find((d) => d.id === i.cameraId), l = (s?.keyframes || []).findIndex((d) => d.frame === i.frame), c = l >= 0 ? s.keyframes[l] : null;
  return !c || ri(i.position, c.camera.position, o, e.canvas.width, e.canvas.height) ? !1 : (r && n.stopImmediatePropagation?.(), e.curveHandleDrag = {
    cameraId: i.cameraId,
    frame: i.frame,
    side: i.side,
...
function qe(e, { mode: t, origin: a, delta: r, factors: o, rotationDeg: n }) {
  ...
  const i = j(e, a);
  return t === "scale" ? M(a, [i[0] * o[0], i[1] * o[1], i[2] * o[2]]) : M(a, xe(i, n));
}
function ci(e, t) {
...
function ml(e, t, a) {
  ...
  return r.map((s, l) => {
    const c = { ...s.camera };
    if (!o.has(s.frame) || !Array.isArray(c.position)) return { ...s, camera: c };
    const d = c.position, m = c.target;
    if (c.position = i[l], Array.isArray(m))
      if (n)
        c.target = qe(m, a);
      else {
        const p = W(j(m, d)) || 5, h = di(i, l);
...
function pi(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  const n = e.gizmoDrag, i = e.state.cameras.find((c) => c.id === n.trackId);
  if (!i) return;
  const s = n.origin;
  let l;
  if (e.state.gizmo_mode === "translate") {
    ...
    l = { mode: "scale", origin: s, factors: c };
  } else {
    const c = o ? Math.round(a * 0.75 / 15) * 15 : a * 0.75, d = [0, 0, 0];
    d[n.axisIndex] = c, l = { mode: "rotate", origin: s, rotationDeg: d };
  }
...
function yi(e, t) {
  ...
  return ta(n.map((s) => -s), n).map((s) => M(o, wa(s, i)));
}
function bi(e, t, a, r = {}) {
  const o = e.state.objects.filter((y) => y.enabled !== !1), n = r.all ? o : o.filter((y) => e.selectedObjectIds?.has(y.id)), s = (n.length ? n : [a]).flatMap((y) => yi(e, y)), l = [0, 1, 2].map((y) => Math.min(...s.map((C) => C[y]))), c = [0, 1, 2].map((y) => Math.max(...s.map((C) => C[y]))), d = l.map((y, C) => (y + c[C]) / 2), { right: m, up: p, forward: h } = H(t), u = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), w = f * u;
  let v = 2, b = 0.1;
  for (const y of s) {
...
function bl(e, t = {}) {
  ...
  if (e.subSelection?.point && !t.all) {
    ...
    const s = e.subSelection.point, l = se(j(a.position, o)), c = Number.isFinite(l[0]) && W(l) > 0.1 ? l : [0.707, 0.4, 0.707], d = 2;
    a.target = [...s], a.position = M(a.target, T(c, d)), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const m = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(_("Focused on {value1} at [{value2}]", { value1: m, value2: s.map((p) => Math.round(p * 100) / 100).join(", ") }));
    return;
  }
  const i = e.selectedObject() || e.state.objects.find((s) => s.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] };
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), bi(e, a, i, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? _("Framed: all objects") : _("Framed: {name}").replace("{name}", i.name || i.type || _("Subject")));
}
function _i(e, t, a, { forceLocal: r = !1 } = {}) {
  const o = [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n = a?.rotation || t?.rotation || [0, 0, 0];
  return r || e.state.gizmo_space === "local" ? o.map((i) => xe(i, n)) : o;
}
function Si(e) {
  return vi(e);
}
...
function Wr(e) {
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: s.map((c, d) => ({ index: d, axis: c, points: [o, V(M(r, T(c, n)), a, e.canvas.width, e.canvas.height)] })).filter((c) => c.points[1] && Number.isFinite(c.points[1][0]) && Number.isFinite(c.points[1][1]))
    };
  const l = s.map((c, d) => {
    const m = Math.abs(c[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = se(Ce(c, m)), h = se(Ce(c, p)), u = [];
    for (let f = 0; f <= 48; f++) {
...
function $r(e, t) {
  ...
  for (const s of a.handles)
    for (let l = 0; l < s.points.length - 1; l++) {
      const c = s.points[l], d = s.points[l + 1], m = Sa(t, c, d);
      (!i || m < i.distance) && (i = { ...s, distance: m, segment: [c, d], worldLength: a.worldLength, entity: a.entity });
    }
...
function Ci(e, t) {
  ...
  if (e.state.view_mode !== "camera") {
    for (const i of e.state.cameras) {
      for (const d of i.keyframes || []) {
        ...
        if (p && Math.hypot(t[0] - p[0], t[1] - p[1]) <= 16 * Math.min(2, window.devicePixelRatio || 1))
          return { type: "camera_keyframe", camera: i, keyframe: d };
      }
      ...
      const c = V(s.position || [6, 4, 6], r, e.canvas.width, e.canvas.height);
    ...
    if (e.state.show_camera_paths !== !1)
      for (const i of e.state.cameras) {
        ...
        const l = s.map((c) => V(c.camera?.position, r, e.canvas.width, e.canvas.height)).filter((c) => c && Number.isFinite(c[0]) && Number.isFinite(c[1]));
  ...
  for (const i of e.state.objects) {
    ...
    (!n || c < n.distance) && (n = { object: i, distance: c });
  }
  return n?.distance <= 22 * Math.min(2, window.devicePixelRatio || 1) ? { type: "object", object: n.object } : null;
}
function Mi(e, t, a, r = 15) {
  const o = a[0] - t[0], n = a[1] - t[1], i = Math.hypot(o, n) || 1, s = o / i, l = n / i, c = -l, d = s, m = r * 0.42, p = a[0] - s * r, h = a[1] - l * r;
...
function vl(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((i, s) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(s[0] - 6, s[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(s[0] - 6, s[1] - 6, 12, 12), e.ctx.restore()) : Mi(e.ctx, i[0], s);
    }
  }
...
function xi(e, t) {
  ...
  const o = a.map((d) => ({ object: d, transform: me(d) })), n = o.reduce((d, m) => M(d, m.transform.position), [0, 0, 0]).map((d) => d / o.length), i = [e.canvas.width * 0.5, e.canvas.height * 0.5], s = e.lastViewportPointer || i, l = e.interactionElement.getBoundingClientRect(), c = e.lastPointerEvent || { clientX: l.left + s[0] * l.width / e.canvas.width, clientY: l.top + s[1] * l.height / e.canvas.height };
  return e.modalTransform = { mode: t, axis: null, numeric: "", start: s, lastEvent: c, snapshots: o, pivot: n }, e.setTransformMode(t), e.setStatus(`${t.toUpperCase()} · move mouse · X/Y/Z constrain · type value · Enter confirm · Esc cancel`), e.render(), !0;
}
function ki(e) {
...
function Ai(e, t, a, r, o, n, i) {
  ...
  if (s === "grid") {
    ...
    return n ? a.map((c, d) => c.map((m, p) => Math.abs(n[p]) > 1e-6 ? He(m, l) : i[d][p])) : a.map((c) => c.map((d) => He(d, l)));
  }
  if (s === "vertex" && r && !n) {
...
function Oe(e, t) {
  ...
  const n = o[0] - a.start[0], i = o[1] - a.start[1], s = t.shiftKey ? 0.1 : 1, l = ki(a), c = a.axis ? ct[a.axis] : null, d = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], m = H(d), p = d.camera_type === "orthographic" ? 10 / (Math.max(0.01, d.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...j(d.position, d.target)) * 25e-4;
  let h = a.snapshots.map((y) => [...y.transform.position]);
  if (a.mode === "translate") {
    const y = l ?? (n - i) * p * s, C = c ? T(c, y) : M(T(m.right, n * p * s), T(m.up, -i * p * s));
    h = h.map((S) => M(S, C)), h = Ai(e, a, h, o, t.ctrlKey || t.metaKey, c, a.snapshots.map((S) => S.transform.position));
  }
  const u = a.mode === "rotate" ? l ?? (n - i) * 0.5 * s : 0, f = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - i) * 0.01 * s) : 1, w = c || ct.z, v = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
...
function Ei(e, t, a) {
  ...
  if (n)
    ...
    i = p.map((h, u) => o[u] - h), s = p.map((h, u) => o[u] + h);
  }
  let l = 1 / 0, c = 1 / 0, d = -1 / 0, m = -1 / 0;
  for (const p of [i[0], s[0]]) for (const h of [i[1], s[1]]) for (const u of [i[2], s[2]]) {
...
function ye(e, t, a, r = [], o = null) {
  ...
  if (i === "grid")
    return o ? t.map((l, c) => Math.abs(o.axis[c]) > 1e-6 ? ne(l, s) : o.base[c]) : ji(t, s);
  if (i === "vertex" && a && !o) {
...
function Ii(e) {
  ...
  const t = e.boxSelection, a = Q(e), r = Math.min(t.start[0], t.current[0]), o = Math.max(t.start[0], t.current[0]), n = Math.min(t.start[1], t.current[1]), i = Math.max(t.start[1], t.current[1]), s = t.additive ? new Set(t.initial) : /* @__PURE__ */ new Set();
  for (const l of e.state.objects) {
    if (l.enabled === !1) continue;
    const c = Ei(e, l, a);
    c && c.maxX >= r && c.minX <= o && c.maxY >= n && c.minY <= i && s.add(l.id);
  }
  return e.selectedObjectIds = s, e.selectedObjectId = [...s].at(-1) || null, e.selectedEntity = s.size ? "object" : "camera", e.boxSelection = null, e.boxSelectMode = !1, Ae(e), e.interactionElement?.style && (e.interactionElement.style.cursor = ""), e.refreshObjects(), e.refreshInspector(), e.render(), e.setStatus(_("{count} object(s) selected").replace("{count}", String(s.size))), !0;
}
function Oi(e, t) {
...
function _l(e, t) {
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = Q(e), i = e.state.view_mode !== "camera", s = Jt(e, t), l = t.button === 0 && !s, c = l && !t.altKey && !t.shiftKey, d = e.transformControlsWiring?.isPointerOverHandle?.();
  !c && d && t.stopImmediatePropagation?.();
  const m = e.transformControlsWiring?.currentLiveType?.();
  if (c && (!d || m === "path_point" || m === "camera") && oi(e, { pointerX: r, pointerY: o, overHandle: d, viewCamera: n, e: t }) || c && d || l && ti(e, { pointerX: r, pointerY: o, shiftKey: t.shiftKey, altKey: t.altKey })) return;
...
function Sl(e, t) {
  ...
  if (e.pathDrag) {
    ...
    d && (d.camera.position = Zt(
      ...
      e.canvas.width,
      e.canvas.height
    ), d.interpolation = ei(d.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  ...
  if (e.targetFreeDrag) {
    ...
    const i = e.interactionElement.getBoundingClientRect(), s = (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width), l = (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height), c = s - e.targetFreeDrag.pointer[0], d = l - e.targetFreeDrag.pointer[1], m = t.shiftKey ? 0.1 : 1, p = M(T(e.targetFreeDrag.right, c * e.targetFreeDrag.scale * m), T(e.targetFreeDrag.up, -d * e.targetFreeDrag.scale * m)), h = ye(e, M(e.targetFreeDrag.target, p), [s, l]);
  ...
  if (e.gizmoDrag) {
    ...
    const i = e.interactionElement.getBoundingClientRect(), s = [
      (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width),
      (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height)
    ], l = t.shiftKey ? 0.1 : 1, c = ((s[0] - e.gizmoDrag.pointer[0]) * e.gizmoDrag.screen[0] + (s[1] - e.gizmoDrag.pointer[1]) * e.gizmoDrag.screen[1]) * l, d = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
    if (e.gizmoDrag.type === "camera_target") {
      const h = M(e.gizmoDrag.target, T(e.gizmoDrag.axis, c * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), u = ye(e, h, s, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
    ...
    if (e.gizmoDrag.type === "camera_path") return void pi(e, { pointer: s, deltaPixels: c, precision: l, snapping: d });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
        const h = (s[0] - e.gizmoDrag.pointer[0]) * l, u = (s[1] - e.gizmoDrag.pointer[1]) * l, f = M(
      ...
      if (e.gizmoDrag.free) {
        const h = (s[0] - e.gizmoDrag.pointer[0]) * l, u = (s[1] - e.gizmoDrag.pointer[1]) * l, f = (h - u) * e.gizmoDrag.freeScale, w = e.gizmoDrag.size.map((v) => {
          const b = v + f;
  ...
  if (e.drag.dolly) {
    ...
    e.drag.target.position = M(n.target, T(s, i)), e.drag.target.camera_type === "orthographic" && (e.drag.target.zoom = Math.max(0.01, (n.zoom || 1) / i));
  } else if (e.drag.fly) {
    const i = j(n.target, n.position), s = W(i);
    let l = Math.atan2(i[0], i[2]), c = Math.asin(D(i[1] / s, -0.999, 0.999));
    l -= a * 8e-3, c = D(c - r * 8e-3, -1.45, 1.45), e.drag.target.target = [
      n.position[0] + s * Math.sin(l) * Math.cos(c),
      n.position[1] + s * Math.sin(c),
      n.position[2] + s * Math.cos(l) * Math.cos(c)
    ];
  } else if (e.drag.shift) {
    const { right: i, up: s } = H(n), l = Ge(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), c = M(T(i, -a * l), T(s, r * l));
    ...
    l -= a * 8e-3, c = D(c + r * 8e-3, -1.45, 1.45), e.drag.target.position = [
      n.target[0] + s * Math.sin(l) * Math.cos(c),
      n.target[1] + s * Math.sin(c),
      n.target[2] + s * Math.cos(l) * Math.cos(c)
    ];
  }
...
function wl(e, t) {
  ...
  if (e.pathDrag) {
    ...
    e.pathDrag = null, Ae(e), i && (e.scheduleSerialize(), e.refreshKeys(), e.setStatus(_("Path key moved")));
    return;
  }
  if (e.curveHandleDrag) {
    const i = e.curveHandleDrag.moved;
    e.curveHandleDrag = null, Ae(e), i && (e.webgl && (e.webgl.pathKey = ""), e.scheduleSerialize(), e.refreshKeys(), e.setStatus(_("Curve handle updated")));
    return;
  }
  if (e.boxSelection) {
    Ii(e);
  ...
  e.gizmoDrag?.type === "camera_path" && (e.serialize?.(), e.refreshKeys?.(), e.setStatus(_("Camera path transformed"))), e.drag && (e.lastRightClickWasDrag = !!(e.drag.button === 2 && (e.drag.moved || e.drag.historyCheckpointed))), Oi(e, t), Pi(e), a && (a.badge?.remove(), e.editingKeyFrame = null, e.updateKeyVisualState(), e.root.focus({ preventScroll: !0 }), a.engaged && (e.suppressKeyClick = !0, setTimeout(() => {
...
function Cl(e, t) {
  ...
  const o = Q(e);
  r || e.beginCameraEdit();
  const n = D(a * 1e-3, -0.4, 0.4), i = j(o.position, o.target);
  o.position = M(o.target, T(i, Math.exp(n))), o.camera_type === "orthographic" && (o.zoom = Math.max(0.01, (o.zoom || 1) * Math.exp(-n))), r ? (e.scheduleSerialize(), e.render()) : (e.commitCameraEdit(), e.finishCameraEdit());
}
const Ni = (e) => {
...
function We(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Ni(e), i = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((s) => ({ visible: !0, interpolation: "linear", ...s })), source: { ...o } };
...
function Fi(e, t, a, r, o) {
  ...
  if (t?.object_id) {
    ...
    i = [m.position[0] + p[0] * m.size[0], m.position[1] + p[1] * m.size[1], m.position[2] + p[2] * m.size[2]];
  }
  if (!Array.isArray(i)) return null;
  const s = V(i, n, r, o);
  if (!s) return null;
  const l = s[0] / r, c = s[1] / o;
  return { x: l, y: c, visible: l >= 0 && l <= 1 && c >= 0 && c <= 1 };
}
function Bi(e, t = 6e-3) {
...
function at(e, t) {
  return Li(t, e.interactionElement);
}
function Vi(e, t) {
...
function Gi(e, t) {
  const a = e.state.objects.find((c) => c.id === e.selectedObjectId), r = e.motionCreationKind, n = (r === "object" || r !== "world" && !!a) && a ? "object_point" : "world_point", i = n === "object_point" ? { object_id: a.id, local_point: [0, 0, 0] } : { point: e.webgl?.intersectScenePoint?.(t.x * e.canvas.width, t.y * e.canvas.height, e.canvas.width, e.canvas.height) || [...e.camera.target] }, s = Fi(e.state, i, e.frame, e.canvas.width, e.canvas.height) || t, l = n === "object_point" ? `${a.name || a.id} Track` : "World Anchor";
...
function Ml(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    n && (e.checkpoint("Set motion interpolation"), Ri(n, o.target.value), oe(e, `Motion interpolation: ${o.target.value}`));
  }, { signal: t }), e.root.querySelector('[data-role="motion-key-visible"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    if (!n?.keys?.length) return;
    const i = e.frame / e.state.fps, s = n.keys.reduce((l, c) => Math.abs(c.time_seconds - i) < Math.abs(l.time_seconds - i) ? c : l);
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? We(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...i, interpolation: "hold" }] }) : n === "project" ? Gi(e, i) : n === "erase" && Vi(e.state, i), oe(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [i, s] = ra(e), l = Ki(e.state, n.points, i, s);
...
function xl(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => Hi(e, a.dataset.motionCreate), { signal: t });
...
function oa(e, t, a) {
  ...
  if (!(i > 1e-4)) return;
  const s = Math.atan2(n[0], n[2]) + t, l = D(Math.asin(D(n[1] / i, -0.999, 0.999)) + a, -1.45, 1.45);
...
const be = { t: "translate", r: "rotate", s: "scale" }, Xi = [
...
], Yi = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function Zi(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(Yi) : !1;
}
function Qi(e) {
...
function es(e, t) {
  return Ji(e) || t?.lastKeyZone || "viewport";
}
let na = !1;
function ts() {
...
function as(e, t) {
  ...
  if (Qi(a) || (t.code === "Space" || t.key === "Enter") && Zi(a)) return !1;
...
function ns(e, t) {
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const s = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: c, up: d, forward: m } = H(s), p = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, h = { w: T(m, p), s: T(m, -p), d: T(c, p), a: T(c, -p), e: T(d, p), q: T(d, -p) }[a];
    return l || e.beginCameraEdit(), s.position = M(s.position, h), s.target = M(s.target, h), l ? (e.serialize(), e.render()) : (e.commitCameraEdit(), e.finishCameraEdit()), !0;
  }
  return !1;
}
function is(e, t, a) {
...
function ds([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * ls), n = Math.round((window.innerHeight || t) * cs);
  return [
...
function ms(e, t) {
  const a = to[t];
  return !a || !e?.setSize ? !1 : (e.setSize(ds(a.default, a.min)), !0);
}
function us(e, t, a) {
  const r = to[t];
  if (!r || !e?.setSize) return !1;
  const o = Array.isArray(a) ? a : Array.isArray(e.size) ? e.size : [0, 0], n = Array.isArray(e.size) ? e.size : [0, 0], [i, s] = r.min, l = Math.max(Number(o[0]) || 0, i), c = Math.max(Number(o[1]) || 0, s);
...
function fs() {
  ...
  e.id = ia, e.textContent = ps, document.head.appendChild(e);
}
function ie(e) {
...
function sa(e) {
  ...
  return document.addEventListener("keydown", u, !0), s.addEventListener("click", (f) => {
    f.stopPropagation(), p();
...
const la = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", ca = "oc-help-toolbar-css", ys = "#8b7bd8";
function bs() {
  ...
  e.id = ca, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${ys};color:#fff;
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Tt(e, async () => {
      ...
      const { attachDirectorShell: o } = await import("./chunk-6djzlmmR.js").then((n) => n.q);
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== mt) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-DaNANbQ9.js")).attachExtractor), e.__majoorOmniCamExtractorRuntime && jt(e, mt, t, a?.());
  }
});
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== ut) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-CgFo05E9.js")).attachMonitor), e.__majoorOmniCamMonitor && jt(e, ut, t, a?.());
  }
});
export {
  Ts as $,
