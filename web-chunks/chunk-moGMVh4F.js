...
import { a6 as zt, F as Wt, s as jt, f as Nt } from "./chunk-CfDK73CP.js";
import { q as It, a as Le, s as Ee, D as Ve, c as Ut, b as qt, d as Rt } from "./chunk-BjvSH5sh.js";
    ...
    rebuildPath(x, y = "camera", s = null, i = "") {
      ...
      (x.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: x.keyframes || [] }]).forEach((h, g) => {
        ...
        if (d.length >= 2) {
          ...
          }), I = new e.CatmullRomCurve3(O, !1, "centripetal"), k = V ? 0.06 : u ? 0.045 : 0.025, F = new e.MeshBasicMaterial({
            color: a.line,
            transparent: !0,
            opacity: u ? 1 : 0.55,
            depthTest: !1
          }), W = new e.Mesh(new e.TubeGeometry(I, Math.max(48, f), k, 8, !1), F);
        ...
        for (const C of d) {
          ...
          if (O) {
            ...
            const F = new e.Vector3().crossVectors(k, I).normalize(), W = e.MathUtils.clamp(f.distanceTo(L) * 0.08, 0.25, 0.8), R = C.camera.camera_type === "orthographic" ? W * 0.55 : W * Math.tan(e.MathUtils.degToRad(C.camera.fov || 35) * 0.5), P = R * (x.width || 16) / Math.max(1, x.height || 9), q = f.clone().addScaledVector(I, W), $ = [
              q.clone().addScaledVector(k, -P).addScaledVector(F, -R),
              q.clone().addScaledVector(k, P).addScaledVector(F, -R),
              q.clone().addScaledVector(k, P).addScaledVector(F, R),
              q.clone().addScaledVector(k, -P).addScaledVector(F, R)
            ], c = [];
            for (const T of $) c.push(f, T);
  ...
  return {
    updateLiveCameras(y, s, i, m, r = "camera", o = null) {
      ...
      (y.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: y.keyframes || [] }]).forEach((g, d) => {
        ...
        let k = new e.Vector3(0, 1, 0), F = new e.Vector3().crossVectors(O, k);
        F.lengthSq() < 1e-6 && (k = new e.Vector3(0, 0, 1), F = new e.Vector3().crossVectors(O, k)), F.normalize();
        let W = new e.Vector3().crossVectors(F, O).normalize();
        if (M.roll) {
          const P = e.MathUtils.degToRad(M.roll);
          F.applyAxisAngle(O, P), W.applyAxisAngle(O, P);
        }
        ...
        if (!C) {
          ...
          const j = new e.Matrix4().makeBasis(F, W, O.clone().negate());
          ...
          T.position.copy(f), T.userData = { omnicamType: "camera", omnicamId: g.id }, this.liveCameras.add(T);
          const U = e.MathUtils.clamp(I * 0.25, 0.5, 2.5), Q = M.camera_type === "orthographic" ? 5 / Math.max(0.01, M.zoom || 1) * 0.35 : U * Math.tan(e.MathUtils.degToRad(M.fov || 35) * 0.5), H = Q * (y.width || 16) / Math.max(1, y.height || 9), K = f.clone().addScaledVector(O, U), te = [
            K.clone().addScaledVector(F, -H).addScaledVector(W, -Q),
            K.clone().addScaledVector(F, H).addScaledVector(W, -Q),
            K.clone().addScaledVector(F, H).addScaledVector(W, Q),
            K.clone().addScaledVector(F, -H).addScaledVector(W, Q)
          ], ae = [];
  ...
  return {
    ...
    pickSubElement(x, y, s, i, m = "vertex") {
      ...
      for (const o of r) {
        ...
        if (m === "edge" && o.face) {
          const V = new e.Vector3(a.getX(o.face.a), a.getY(o.face.a), a.getZ(o.face.a)).applyMatrix4(u), C = new e.Vector3(a.getX(o.face.b), a.getY(o.face.b), a.getZ(o.face.b)).applyMatrix4(u), M = new e.Vector3(a.getX(o.face.c), a.getY(o.face.c), a.getZ(o.face.c)).applyMatrix4(u), f = (F, W, R) => {
            const P = new e.Line3(W, R), q = new e.Vector3();
            return P.closestPointToPoint(F, !0, q), { dist: F.distanceTo(q), point: q, segment: [W, R] };
          }, L = f(o.point, V, C), O = f(o.point, C, M), I = f(o.point, M, V), k = [L, O, I].reduce((F, W) => W.dist < F.dist ? W : F);
          return {
            type: "edge",
            mode: "edge",
            objectId: g,
  ...
  return {
    render(s, i, m, r, o, w = /* @__PURE__ */ new Map(), h = 0, g = !1, d = "camera", a = "subject", u = null, V = null) {
      ...
      const L = JSON.stringify([
        ...
        !!s.show_vertices,
        s.objects.map((c) => {
          const { position: b, rotation: j, keyframes: N, size: T, ...U } = c;
      ...
      const F = s.view_mode || "camera", W = `${F}:${d}:${V ?? ""}:${s.__omnicamRevision ?? JSON.stringify([
        s.active_camera_id,
      ...
      if (W !== this.pathKey && (this.pathKey = W, this.rebuildPath(s, d, V, F)), this.updateLiveCameras(s, h, g, F, d, V), this.liveCameras.visible = !g, !g) {
        ...
        for (const N of [this.path, this.liveCameras])
          N.traverse((T) => {
            ...
            U === "path" ? T.visible = c : U === "gizmo" ? T.visible = b : U === "lookat" && (T.visible = j);
          });
      }
      ...
      if (this.activeCamera = P, g ? this.selectionGroup.visible = !1 : (this.updateSelection(s, d, a, u, `${s.__omnicamRevision ?? "legacy"}:${h}`, M), this.selectionGroup.visible = !0), this.studioEnabled && this.contentShadowKey !== this.sceneKey) {
        ...
        if (b) {
          const j = c.isEmpty() ? new e.Vector3() : c.getCenter(new e.Vector3()), N = c.isEmpty() ? new e.Vector3(12, 12, 12) : c.getSize(new e.Vector3()), T = Math.max(1, 0.5 * Math.max(N.x, N.y, N.z) * Math.SQRT2), U = T * 1.15 + 0.5, Q = new e.Vector3(4.5, 7.5, 3.5).normalize(), H = Math.max(12, T * 4);
...
class Mr {
  constructor(e = () => {
    ...
    }), this.renderer.setPixelRatio(1), this.renderer.outputColorSpace = he, this.renderer.shadowMap.enabled = !0, this.renderer.shadowMap.type = Te, this.scene = new qe(), this.scene.background = new ce(1184274), this.scene.add(new Oe(16777215, 3159099, 2.2));
