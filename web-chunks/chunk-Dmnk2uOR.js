import { a, $ as j, bA as te, bB as J, bC as w } from "./chunk-1FIr5kKM.js";
...
function he(e, n = {}) {
  ...
  async function E() {
    ...
    try {
      ...
      if (l) return;
      let p = t.model;
      !p && r.length && (p = r[0], j(J(t.provider), p)), m.innerHTML = Y(r, p), $();
    } catch {
      if (l) return;
  ...
  function B() {
    !m || !m.value || (j(J(w().provider), m.value), $());
  }
  function R() {
    !y || !y.value || (j(te, y.value), $(), I(), E());
  }
  async function Q() {
  ...
  return f?.addEventListener("click", G), y?.addEventListener("change", R), m?.addEventListener("change", B), d?.addEventListener("input", D), $(), z(), I(), E(), {
    get state() {
      return g;
    },
    dispose() {
