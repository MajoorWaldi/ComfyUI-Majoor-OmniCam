import { l as N, u as P, d as I } from "./chunk-eq1tqQ9i.js";
import "../../scripts/app.js";
import { api as u } from "../../scripts/api.js";
import { M as $, E as L } from "./chunk-CbqXtcpr.js";
import { x as R, y as A } from "./chunk-CP16H5xA.js";
import { m as j } from "./chunk-CTXwjaoT.js";
import { S as D, b as q, p as H } from "./chunk-DiX7WiXt.js";
function c(e) {
...
function F(e) {
  const t = C(e.state), o = e.message ? `<br><small>${c(e.message)}</small>` : "";
  return `<div class="oc-row"><span><strong>${c(e.label || e.id)}</strong>${o}</span><span class="oc-state" data-state="${t}">${c(e.state || "UNKNOWN")}</span></div>`;
}
function U(e, t, o = !1) {
...
function v(e, t, { live: o = !1 } = {}) {
  const r = W(t), i = e.querySelector('[data-role="profile-preflight"]');
  i.innerHTML = r.preflight.length ? r.preflight.map(F).join("") : '<div class="oc-empty">No preflight checks returned.</div>';
  ...
  n.innerHTML = a.length ? a.map((p) => `<div class="oc-row"><span>${c(p.display || p.adapter)}</span><span class="oc-state" data-state="${C(p.state)}">${c(p.state)}</span></div>`).join("") : '<div class="oc-empty">No optional downstream capability detected.</div>';
  const l = r.preflight.some((p) => String(p.state).toUpperCase() === "BLOCKED"), s = e.querySelector('[data-role="monitor-status"]');
  return s.dataset.state = l ? "BLOCKED" : "READY", s.lastChild.textContent = l ? " BLOCKED" : " READY", e.querySelector('[data-role="output-status"]').textContent = U(l, r.targetProfile, o), r;
}
const V = "MajoorOmniCamDirector";
function B(e) {
...
function X(e, t) {
  ...
  const r = R(e, o);
  if (!r) return null;
  const i = J(t), a = Y(i), n = a?.metadata?.playblast && typeof a.metadata.playblast == "object" ? a.metadata.playblast : {};
  return {
    kind: "director_playblast",
    url: r,
    fps: Number(n.fps) || void 0,
    frameCount: Number(n.frame_count) || void 0,
    width: Number(n.width) || void 0,
    height: Number(n.height) || void 0,
    durationSeconds: Number(n.duration_seconds) || void 0,
    encoder: typeof n.encoder == "string" ? n.encoder : void 0,
    outdated: Q(n, i)
  };
}
function T(e) {
  return k(e) === E && !f(e, "recording_path");
}
function Z(e, t) {
  if (e) {
  ...
  return T(t) ? "⚠ Director connected, no playblast recorded yet — showing the live viewport." : "";
}
function tt(e, t) {
  return e?.outdated ? "2" : e ? "" : T(t) ? "1" : "";
}
class et {
  constructor(t, {
    delay: o = 250,
    ...
    this.api = t, this.delay = o, this.endpoint = r, this.onSnapshot = i, this.onError = a, this.timer = null, this.abort = null, this.scheduledKey = "", this.disposed = !1, this.generation = 0;
  }
  ...
  async refresh(t) {
    ...
    try {
      ...
      return this.disposed || o !== this.generation ? null : (this.onSnapshot(i), i);
    } catch (r) {
...
function w(e, t) {
  ...
  return o?.link == null || !e?.graph ? null : N(e.graph, o.link);
}
function ot(e) {
...
class rt {
  ...
  poll() {
    const t = ot(this.node), o = JSON.stringify([
...
function at(e, t) {
  ...
  o.innerHTML = r.length ? r.map((n) => {
    const s = (n.capability || a.get(String(n.id)))?.state || "missing";
    return `<div class="oc-row"><span><strong>${c(n.display_name)}</strong><br><small>${c(n.semantic)} · ${c(n.frame_policy)}</small></span><span class="oc-state" data-state="${c(s)}">${c(s)}</span></div>`;
  }).join("") : '<div class="oc-empty">No Monitor profile is available.</div>';
}
const _ = "majoor.omnicam.monitor.preflight", nt = 1;
function st(e, t, o) {
  const r = (i) => {
    const a = i?.detail;
    !a || Number(a.schema_version) !== nt || a.kind === "blocked_preflight" && String(a.node) === String(t.id) && (!a.output || o.disposed || o.blockedPreflight(a.output));
  };
  return e.addEventListener(_, r), () => {
    e.removeEventListener?.(_, r);
  };
}
const ct = `${D}
...
function dt() {
  return lt.map(([e, t]) => `<option value="${e}">${t}</option>`).join("");
}
function pt() {
    ...
    <main class="oc-layout">
      ...
      <aside class="oc-column">
        <div class="oc-card"><div class="oc-section">Compilation target</div><div class="oc-adapter-controls">
          <label class="wide">Profile<select data-role="profile-select">${dt()}</select></label>
...
function ut(e = document) {
  ...
  return t.innerHTML = pt(), t.firstElementChild;
}
const g = [
  "base_prompt",
  "target_profile",
...
function m(e) {
  return Object.fromEntries(g.map((t) => [t, M(e, t)?.value]));
}
function S(e, t, o) {
  if (!g.includes(t)) return !1;
  const r = M(e, t);
  return r ? (r.value = ht.has(t) ? Number(o) : o, r.callback?.(r.value), !0) : !1;
}
function mt(e) {
  return String(e || "").startsWith("h3_");
}
const ft = 250, gt = /* @__PURE__ */ new Set(["duration_seconds", "target_fps"]);
function bt(e) {
...
class yt {
  constructor(t) {
    this.node = t, this.root = ut(), this.events = new L(), this.source = null, this.player = new G(
      this.root.querySelector('[data-role="proxy-player"]'),
      {
        onFrame: (o) => this.showFrame(o),
        onMetadata: ({ frameCount: o }) => this.setFrameCount(o)
      }
    ), this.hasExecutedOnce = !1, this._liveUnavailableText = "", this.disposed = !1, this.connectionRefreshTimer = null, this.refreshController = new et(u, {
      onSnapshot: (o) => this.liveSnapshotReceived(o),
      onError: (o) => this.liveRefreshFailed(o)
    }), this.bindControls(), this.syncControlsFromWidgets(), this.loadProfileInfo(), this.watcher = new rt(t, (o) => this.sourceChanged(o)), this.liveTimer = setInterval(() => this.liveTick(), ft);
  }
  ...
  bindControls() {
    this.events.on(this.root, "wheel", H(this.root)), this.events.on(this.root.querySelector('[data-act="proxy-play"]'), "click", () => this.player.toggle()), this.events.on(this.root.querySelector('[data-role="proxy-scrubber"]'), "input", (t) => this.player.scrub(t.target.value)), this.events.on(this.root.querySelector('[data-role="proxy-loop"]'), "change", (t) => this.player.setLoop(t.target.checked)), this.events.on(this.root.querySelector('[data-role="proxy-mute"]'), "change", (t) => this.player.setMuted(t.target.checked)), this.events.on(this.root.querySelector('[data-role="profile-select"]'), "change", (t) => {
      S(this.node, "target_profile", t.target.value), this.updateH3SetupHint(t.target.value), this.settingsChanged();
    });
  ...
  syncControlsFromWidgets() {
    ...
    t.target_profile != null && (o.value = String(t.target_profile)), this.updateH3SetupHint(o.value);
    for (const r of g) {
      if (r === "target_profile") continue;
      const i = this.root.querySelector(`[data-setting="${r}"]`);
      !i || t[r] == null || (gt.has(r) && Number(t[r]) <= 0 ? i.value = "" : i.value = t[r]);
    }
  ...
  reflectInheritedShot() {
    const t = this.source?.sceneOrigin, o = h(t) ? O(t) : null, r = {
  ...
  liveTick() {
    ...
    const o = z(t, m(this.node)), r = o.director, i = `${r.state_json}\0${JSON.stringify(o.monitor)}\0${r.recording_path}\0${r.card_asset}\0${r.width}x${r.height}@${r.fps}/${r.duration_seconds}:${r.render_mode}`;
    i !== this._liveKey && (this._liveKey = i, this.refreshController.schedule(o));
  }
  liveSnapshotReceived(t) {
    this.disposed || v(this.root, t, { live: !0 });
  }
  liveRefreshFailed(t) {
  ...
  refreshPlayblastPreview() {
    if (this.disposed) return;
    const t = this.root.querySelector('[data-role="proxy-upstream-preview"]'), o = this.root.querySelector(".oc-player-empty"), r = this.source?.playblastOrigin, i = X(u, r);
    ...
    const l = typeof HTMLVideoElement < "u" && a instanceof HTMLVideoElement ? String(a.currentSrc || a.src || "") : "";
    if (l) {
      t.hidden = !0, o.hidden = !0, this.player.setSource(l);
      return;
    }
    this.player.setSource(""), I(a, t, 640).then((s) => {
      this.disposed || (t.hidden = !s, o.hidden = s);
    });
  }
  updateReferenceSourceLabel(t, o) {
    ...
    if (!r) return;
    const i = Z(o, t);
    r.textContent = i, r.hidden = !i, r.dataset.warn = tt(o, t);
  }
  setFrameCount(t) {
  ...
  renderResult(t, { executed: o = !1 } = {}) {
    o && (this.hasExecutedOnce = !0);
    const r = v(this.root, t);
    r.targetProfile && m(this.node).target_profile !== r.targetProfile && this.markOutdated();
  }
  executed(t) {
    this.renderResult(t, { executed: !0 });
  }
  blockedPreflight(t) {
    this.hasExecutedOnce = !0, this.renderResult(t);
  }
  dispose() {
    this.disposed || (this.disposed = !0, clearInterval(this.liveTimer), clearTimeout(this.connectionRefreshTimer), A(), this.refreshController?.dispose(), this.watcher?.dispose(), this.player.dispose(), this.events.dispose());
  }
}
function Et(e) {
  if (e.__majoorOmniCamMonitor) return;
  bt(e);
  const t = new yt(e), o = st(u, e, t);
  t.events.add(o), e.__majoorOmniCamMonitor = t;
  const r = () => Math.max(620, t.root.scrollHeight || 0);
  ...
  e.onRemoved = function() {
    t.dispose(), i?.apply(this, arguments);
  };
  const a = e.onExecuted;
  e.onExecuted = function(s) {
    a?.apply(this, arguments), t.executed(s);
  };
  const n = e.onConfigure;
  e.onConfigure = function() {
    n?.apply(this, arguments), t.syncControlsFromWidgets();
  };
  const l = e.onConnectionsChange;
  e.onConnectionsChange = function() {
    l?.apply(this, arguments), !t.disposed && (t.watcher?.poll(), t.refreshPlayblastPreview(), clearTimeout(t.connectionRefreshTimer), t.connectionRefreshTimer = setTimeout(() => {
      t.connectionRefreshTimer = null, t.refreshPlayblastPreview();
    }, 400));
  };
}
export {
