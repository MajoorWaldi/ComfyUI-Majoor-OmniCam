...
import { d as J, u as Y, l as Q, S as Z } from "./chunk-B0ZcW-l0.js";
import { g as B, n as tt, L as et, h as rt, S as L, i as at, j as ot, r as it, p as st, T as nt, F as ct } from "./chunk-BSjCHYoo.js";
import { M as lt, t as dt, d as ut, f as ht, D as O, c as pt, q as mt, h as ft, a as bt } from "./chunk-DW815aQ8.js";
import { Z as gt } from "./chunk-CE-YeXfn.js";
function vt(r, t) {
...
async function yt(r, t) {
  ...
  r.upstreamPreviewActive = a ? await J(a, e, 960) : !1, r.disposed || r.render();
}
const wt = {
...
class kt {
  ...
  constructor(t, e = {}, a = () => !0) {
    ...
    for (const [o, i] of Object.entries(wt)) {
      const s = e[o];
...
const y = "/majoor/omnicam/extractor/jobs", Et = /* @__PURE__ */ new Set([
  "IDLE",
  "PREPARING",
  "TRACKING",
  "SOLVING",
...
function Ct(r, t = {}) {
  ...
  return !e || !Et.has(String(t.solveState || "")) ? !1 : (r.stopSolve(e).catch(() => {
  }), !0);
}
async function D(r) {
  try {
...
class Tt {
  ...
  async _request(t, { method: e = "GET", body: a } = {}) {
    ...
    if (!i.ok) throw new Error(await D(i));
    return i.json();
  }
  ...
  startSolve({ nodeId: t, source: e, settings: a }) {
    return this._request(y, {
      method: "POST",
      body: { node_id: String(t), client_id: this.identity(), source: e, settings: a }
    });
  }
  getSolveStatus(t) {
    return this._request(`${y}/${encodeURIComponent(t)}`);
  }
  stopSolve(t) {
    return this._request(`${y}/${encodeURIComponent(t)}/stop`, { method: "POST" });
  }
  refineSolve(t, e) {
    return this._request(`${y}/${encodeURIComponent(t)}/refine`, {
      method: "POST",
      body: { settings: e }
    });
  }
  getSolveResult(t) {
    return this._request(`${y}/${encodeURIComponent(t)}/result`);
  }
  deleteSolve(t) {
    return this._request(`${y}/${encodeURIComponent(t)}`, { method: "DELETE" });
  }
  ...
  async uploadSource(t) {
    ...
    if (!a.ok) throw new Error(await D(a));
    return a.json();
  }
}
const Mt = 200, V = {
...
class _t {
  constructor({ onRefine: t, delay: e = Mt, setTimer: a, clearTimer: o } = {}) {
    this.settings = { ...V }, this.alignment = _(), this.onRefine = t || (() => {
  ...
  setAlignment(t) {
    return this.alignment = { ...this.alignment, ...t }, this.update({
      global_rotation_xyzw: Nt(this.alignment),
      estimate_up: !1
    });
  }
  /** Ask the server to derive the levelling rotation from the solve itself. */
  requestEstimatedUp() {
    return this.alignment = _(), this.update({ global_rotation_xyzw: null, estimate_up: !0 });
  }
  setSpikeAction(t, e) {
  ...
  reset() {
    return this.settings = { ...V }, this.alignment = _(), this.schedule(), this.settings;
  }
  payload() {
...
function Nt({ pitch: r = 0, yaw: t = 0, roll: e = 0 } = {}) {
  if (!r && !t && !e) return null;
  const [a, o, i] = [r, t, e].map((d) => (Number(d) || 0) * (Math.PI / 180) * 0.5), [s, n, c, l, u, h] = [
    Math.cos(a),
    Math.sin(a),
    Math.cos(o),
    Math.sin(o),
...
class Ft {
  ...
  seek(t, e = "manual") {
    ...
    const a = Rt(t, this.frameCount);
    return At(e) && this.setFollow(!1), e !== "media" && this.media?.seekFrame?.(a), this.getViewer?.()?.setFrame?.(a), this.showDiagnostics(a), this.frame = a, this.dispatch({ type: "FRAME", frame: a }), e !== "playback" && (this.playbackStartFrame = a, this.playbackStartTime = null), a;
  }
  play() {
...
function $t(r, { track: t, state: e } = {}) {
  if (e !== "COMPLETED")
    throw new T("Only a completed solve can be applied to the Director.");
  const a = t?.keyframes;
  if (!Array.isArray(a) || !a.length)
    throw new T("This solve produced no camera keys to apply.");
  const o = String(t?.metadata?.extractor_fingerprint || "");
  if (!o)
    throw new T("This track carries no extractor fingerprint.");
  B(r, { track: t, fingerprint: o, confidence: Number(t?.metadata?.confidence) || 0 });
  const i = tt(r);
  return { fingerprint: o, notified: i };
}
function Lt(r, t = "") {
...
const Ot = "/majoor/omnicam/extractor/frame";
function Dt(r) {
...
function qt(r, t, e = t?.width, a = t?.height) {
  ...
  const l = Math.min(n / i, c / s), u = Math.round(i * l), h = Math.round(s * l);
  return o.clearRect(0, 0, n, c), o.drawImage(t, Math.round((n - u) / 2), Math.round((c - h) / 2), u, h), !0;
}
class Ut {
  ...
  async load(t, e, { maxDimension: a = 960 } = {}) {
    ...
    const s = Dt(e);
    try {
      const n = await this.api?.fetchApi?.(Ot, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: t, frame: s, max_dimension: a }),
        signal: i.signal
      });
      if (!n?.ok) throw new Error(await Vt(n));
      const c = await n.blob(), l = await this.decodeImage(c);
      if (o !== this.generation || i.signal.aborted)
        return l?.close?.(), !1;
      const u = E(n.headers, "X-OmniCam-Width", l?.width), h = E(n.headers, "X-OmniCam-Height", l?.height);
      let d = !1;
      try {
        d = qt(this.canvas, l, u, h);
      } finally {
        l?.close?.();
      }
      if (!d) throw new Error("The fallback preview canvas is unavailable.");
      return this.frame = E(n.headers, "X-OmniCam-Frame", s), this.frameCount = E(n.headers, "X-OmniCam-Frame-Count", this.frameCount), this.error = "", !0;
    } catch (n) {
      if (o !== this.generation || i.signal.aborted || jt(n)) return !1;
...
}, zt = /\.(mp4|mov|webm|mkv|m4v|avi)(\s|$)/i, C = {
  available: !1,
  ref: null,
  label: "",
...
function Ht(r, t) {
  ...
  return !e || e.link == null || !t ? null : Q(t, e.link);
}
function U(r) {
  const t = String(
  ...
  if (e) {
    ...
    if (!o) {
      ...
      return s ? {
        ...
        ...C,
        reason: `${q(e) || "This node"} produces its footage only while the workflow runs. Connect Load Video, or choose an Extractor source file, to track without running.`,
    ...
    return i ? zt.test(i) ? {
      ...
      originNodeId: e.id ?? null
    } : { ...C, reason: `${i} does not look like a video file.` } : { ...C, reason: "The connected Load Video node has no file selected yet." };
  }
  const a = U(r);
  return a ? {
    available: !0,
    ...
    ref: a,
    originNodeId: null
  } : { ...C, reason: "Connect Load Video, or choose a source file, to track." };
}
function Kt(r) {
...
function Xt(r) {
  const t = Bt(r.node, r.node.graph), e = t.ref ? `${t.ref.kind}:${t.ref.value}` : "", a = e !== (r.sourceKey || "");
  if (a) {
  ...
  const o = r.sourceViewer.setSource(
    t.available && t.ref ? gt(k, t.ref.value) : "",
    { source: t.available ? t.ref : null }
  );
  return a && r.coordinator.seek(0, "source"), r.dispatch({ type: "SOURCE", source: o ? { ...t, playbackError: "" } : t }), t.available && t.ref ? K(r, t) : P(r, 0), yt(r, t), t;
}
async function K(r, t) {
  ...
  try {
    ...
    const a = e?.info || null;
    return r.dispatch({ type: "SOURCE", source: { info: a } }), a && (r.coordinator.setRate(Number(a.fps) || r.sourceViewer.fps), P(r, Number(a.frame_count) || 0), xt(r, a)), a;
  } catch (e) {
...
const Jt = /* @__PURE__ */ new Set(["PREPARING", "TRACKING", "SOLVING", "REFINING", "STOPPING"]), Yt = {
  IDLE: "neutral",
  PREPARING: "info",
  TRACKING: "active",
  SOLVING: "active",
...
function Zt(r) {
  const t = r.solveState, e = Jt.has(t), a = t === "COMPLETED";
  return {
...
`, ie = '<svg class="oc-mark" viewBox="0 0 24 24" aria-hidden="true"><circle class="oc-mark-ring" cx="12" cy="12" r="8"/><circle class="oc-mark-core" cx="12" cy="12" r="3"/></svg>';
function x(r, t, { min: e = 0, max: a = 1, step: o = 0.01, value: i = 0 } = {}) {
...
function se() {
  return `<div class="majoor-omnicam oc-extractor">
    ...
    <header class="oc-header">
      <div class="oc-heading"><span class="oc-brand">${ie}</span>
      ...
      <div class="oc-columns">
        <div class="oc-card">
          ...
          <div class="oc-sliders">
            ${x("position-smoothing", "Position smooth", { value: 0.15 })}
            ${x("motion-scale", "Motion scale", { min: 0.01, max: 10, step: 0.01, value: 1 })}
          </div>
          <div class="oc-inline">
          ...
          <details class="oc-details"><summary>Advanced cleanup</summary>
            <div class="oc-sliders">
              ${x("rotation-smoothing", "Rotation smooth", { value: 0.1 })}
              ${x("position-tolerance", "Key reduction", { min: 0, max: 0.5, step: 1e-3, value: 0.01 })}
              ${x("align-pitch", "Pitch", { min: -180, max: 180, step: 0.5, value: 0 })}
              ${x("align-yaw", "Yaw", { min: -180, max: 180, step: 0.5, value: 0 })}
              ${x("align-roll", "Roll", { min: -180, max: 180, step: 0.5, value: 0 })}
            </div>
            <div class="oc-inline">
...
class ce {
  ...
  render({ track: t = null, health: e = null, quality: a = [], anomalies: o = [], frame: i = 0, frameCount: s = 0 } = {}) {
    ...
    if (!n) return null;
    const c = dt(void 0, O);
    return n.height !== c && (n.height = c), ut(n, {
      track: t,
      health: e,
      quality: a,
      anomalies: o,
  ...
  frameAt(t, e) {
    ...
    return ht(t.clientX - o.left, o.width, e, 0);
  }
...
function ue(r, t) {
  const e = I(r?.anomalies), a = I(Object.values(pt(t)).flat()), o = a.length ? a : I(t?.keyframes);
  return { anomalies: e, solved: o };
}
function R(r, t, { anomalies: e, solved: a }) {
...
function pe(r, {
  ...
  const i = (d) => r?.querySelector?.(de[d]) || null, s = () => e() || {}, n = () => ue(s(), a()), c = (d) => A(s()) < 1 ? !1 : (t?.seek?.(d, "transport"), !0), l = (d) => {
    const m = R(Number(s().frame) || 0, d, n());
    return m === null ? !1 : c(m);
  }, u = {
    "first-frame": () => c(0),
    "previous-key": () => l(-1),
    ...
    "next-key": () => l(1),
    "last-frame": () => c(A(s()) - 1),
    "toggle-loop": () => (t?.setLoop?.(!t?.loop), h(), !0)
  };
  for (const d of le) {
    const m = i(d);
    m && o(m, "click", () => u[d]());
  ...
  o(r, "keydown", (d) => {
    ...
    !m || !u[m]() || (d.preventDefault(), d.stopPropagation());
  });
  function h() {
    const d = Number(s().frame) || 0, m = n(), f = i("previous-key");
    f && (f.disabled = R(d, -1, m) === null);
    const b = i("next-key");
    b && (b.disabled = R(d, 1, m) === null);
    const p = i("toggle-loop");
    p && p.setAttribute("aria-pressed", String(!!t?.loop));
    const v = i("play");
    if (v) {
      v.classList?.toggle?.("playing", !!t?.playing);
      const g = v.querySelector?.("i");
      g && (g.className = t?.playing ? "pi pi-pause" : "pi pi-play"), v.setAttribute("aria-label", t?.playing ? "Pause playback" : "Play playback");
    }
  }
  return { render: h };
}
const me = 300, fe = 300, w = {
  accepted: "#46a758",
  weak: "#e5a23c",
  rejected: "#e5484d",
  current: "#8b7bd8"
...
function F(r, { sourceWidth: t, sourceHeight: e, width: a, height: o }) {
...
class be {
  ...
  setDiagnostics({ points: t = [], vectors: e = [], frame: a = 0, state: o = "unknown" } = {}) {
    this.points = G(t, me), this.vectors = G(e, fe), this.frame = Number(a) || 0, this.state = String(o || "unknown"), this.draw();
  }
  clear() {
  ...
  draw({ sourceWidth: t = 0, sourceHeight: e = 0 } = {}) {
    ...
    for (const n of this.vectors) {
      const [c, l] = F(n.from ?? n, s), [u, h] = F(n.to ?? n, s);
      a.strokeStyle = w[n.state] || w.accepted, a.beginPath(), a.moveTo(c, l), a.lineTo(u, h), a.stroke();
    }
    for (const n of this.points) {
      const [c, l] = F(n, s);
      a.fillStyle = w[n.state] || w.accepted, a.fillRect(c - 1.5, l - 1.5, 3, 3);
    }
    return (this.state === "weak" || this.state === "bad") && (a.strokeStyle = this.state === "bad" ? w.rejected : w.weak, a.lineWidth = 2, a.strokeRect(1, 1, o - 2, i - 2)), { points: this.points.length, vectors: this.vectors.length };
  }
  dispose() {
...
function ve(r, t, e = "Nothing to show") {
  if (!r) return 0;
  ...
  for (const [o, i] of t) r.append(ge(a, o, i));
  return t.length;
}
function xe(r, t, { onAction: e = () => {
}, onFrame: a = () => {
}, actions: o = {} } = {}) {
  if (!r) return 0;
  ...
  for (const s of t) {
    ...
    const l = i.createElement("strong"), u = Number(s.start_frame ?? s.frame), h = Number(s.end_frame ?? s.frame);
    l.textContent = u === h ? `Frame ${u}` : `Frames ${u}-${h}`, l.tabIndex = 0, l.setAttribute("role", "button"), l.addEventListener("click", () => a(s.frame)), l.addEventListener("keydown", (f) => {
      (f.key === "Enter" || f.key === " ") && (f.preventDefault(), a(s.frame));
    });
    const d = i.createElement("small");
    d.textContent = `${String(s.level || "warn").toUpperCase()} · ${s.detail || s.kind || ""}`, c.append(l, d), n.append(c);
    const m = o[String(s.frame)] || s.suggested_action || "ignore";
    for (const f of ["interpolate", "ignore", "exclude"]) {
      const b = i.createElement("button");
      b.type = "button", b.textContent = f.toUpperCase(), b.dataset.action = f, b.dataset.frame = String(s.frame), f === m && b.setAttribute("aria-selected", "true"), b.addEventListener("click", () => e(s, f)), n.append(b);
    }
    r.append(n);
  }
  return t.length;
...
function we(r) {
  return import("./chunk-CClfeOxV.js").then(({ TrackViewer: t }) => (r.viewerLoad = null, r.disposed || r.viewer || (r.viewer = new t(r.$("track-canvas")), r.pushTracksToViewer()), r.viewer)).catch((t) => (r.viewerLoad = null, console.warn("OmniCam track viewer unavailable", t), null));
}
function z(r) {
  const t = r.$("frame");
  t && (t.value = String(r.state.frame));
  const e = r.$("time");
  e && (e.textContent = j(r.state.frame, r.sourceViewer.fps));
  const a = r.$("frame-readout");
  a && (a.textContent = `${r.state.frame} / ${Math.max(0, r.state.frameCount - 1)} · ${j(r.state.frame, r.sourceViewer.fps)}`);
  const o = mt(r.state.quality, r.state.frame), i = ft(r.state.quality, r.currentHealth, r.state.frame);
  ve(r.$("quality-details"), [...o, ...i, ...ye(r.state.warnings)], "No solve yet");
}
function W(r) {
...
function Ee(r) {
  $(r), globalThis.requestAnimationFrame?.(() => $(r)), setTimeout(() => $(r), 250);
}
class Ce {
  constructor(t) {
    this.node = t, this.root = ne(), this.state = Qt(), this.disposed = !1, this.disposers = [], this.result = { raw: null, refined: null }, this.landmarks = [], this.diagnostics = new It(), this.upstreamPreviewActive = !1, this.motionLimits = null, this.client = new Tt(k), this.refine = new _t({ onRefine: (e) => this.requestRefine(e) }), this.fallbackViewer = new Ut(this.$("fallback-preview"), { api: k }), this.sourceViewer = new Pt(this.$("source-video"), {
    ...
    }), this.timeline = new ce(this.root, {
      onSeek: (e) => this.coordinator.seek(e, "timeline")
    }), this.transport = pe(this.root, {
      coordinator: this.coordinator,
      getState: () => this.state,
      getTrack: () => this.state.trackMode === "raw" ? this.result.raw : this.result.refined,
      listen: (e, a, o) => this.listen(e, a, o)
    }), this.overlay = new be(this.$("tracking-overlay")), this.viewer = null, this.viewerLoad = null, this.events = new kt(k, {
    ...
    }, St(() => ({ jobId: this.state.jobId, nodeId: this.node.id }))), this.bind(), this.loadMotionLimits(), this.refreshSource(), this.restoreCachedResult(), this.render();
  }
  // -- plumbing ----------------------------------------------------------
  $(t) {
  ...
  dispatch(t) {
    return this.state = N(this.state, t), this.disposed || this.render(), this.state;
  }
  async loadMotionLimits() {
    try {
      const t = await k.fetchApi?.("/majoor/omnicam/motion_profiles");
      if (!t?.ok) return;
      const e = await t.json();
      this.motionLimits = e?.profiles?.find((a) => a.id === "generic")?.limits || null, this.disposed || this.render();
    } catch {
    }
  }
  bind() {
  ...
  acceptSolvedResult(t, e = "interactive") {
    ...
    }), this.dispatch({ type: "REFINED", fingerprint: i }), this.pushTracksToViewer(), e === "queued" && (B(this.node, {
      track: o,
      fingerprint: i,
      confidence: Number(t?.confidence ?? o?.metadata?.confidence) || 0
    }), t?.source && at(this.node, t.source), this.node.__majoorOmniCamStatus = ot({
      track: o,
      confidence: Number(t?.confidence ?? o?.metadata?.confidence) || 0
  ...
  async estimateUp() {
    ...
    if (!e) return null;
    const [a, o, i, s] = e.map(Number), n = (u) => Math.round(u * (180 / Math.PI) * 10) / 10, c = n(Math.atan2(2 * (s * a + o * i), 1 - 2 * (a * a + o * o))), l = n(Math.atan2(2 * (s * i + a * o), 1 - 2 * (o * o + i * i)));
    for (const [u, h] of [["pitch", c], ["yaw", 0], ["roll", l]]) {
  ...
  applyRefined() {
    try {
      const { fingerprint: t } = $t(this.node, {
        track: this.result.refined,
        state: this.state.solveState
      });
      this.dispatch({ type: "APPLIED", fingerprint: t });
    } catch (t) {
      const e = t instanceof T ? t.message : String(t?.message || t);
      this.dispatch({ type: "FAILED", error: e });
    }
  }
  // -- viewer ------------------------------------------------------------
  ensureViewer() {
    return this.viewer || this.disposed ? Promise.resolve(this.viewer) : (this.viewerLoad ||= we(this), this.viewerLoad);
  }
  pushTracksToViewer() {
  ...
  render() {
    ...
    t && (t.dataset.tone = te(this.state.solveState), this.$("solve-status-text").textContent = ee(this.state));
    const e = this.$("source-strip");
    e && (e.dataset.available = String(!!this.state.source.available), this.$("source-label").textContent = Kt(this.state.source));
    const a = Zt(this.state);
    for (const [p, v] of Object.entries({
      track: a.track,
      stop: a.stop,
      apply: a.apply
    })) {
      const g = this.root.querySelector(`[data-act="${p}"]`);
      g && (g.disabled = !v);
    }
    this.$("solve-detail").textContent = re(this.state), this.$("solve-percent").textContent = `${Math.round(this.state.progress * 100)}%`, this.$("progress-bar").style.width = `${Math.round(this.state.progress * 100)}%`;
    const o = this.$("solve-error");
    o.hidden = !this.state.error, o.textContent = this.state.error || "";
    const i = ae(this.state), s = this.$("applied-state");
    ...
    const n = this.state.viewerMode, c = n === "source", l = n === "track3d", u = this.$("stage");
    u && (u.dataset.mode = n), vt(this, c), this.$("tracking-overlay").hidden = !0, this.$("track-canvas").hidden = !l, this.root.querySelector('[data-role="views"]').hidden = !l;
    const h = this.$("scrubber");
    ...
    const f = this.$("extractor-fps");
    f && (f.textContent = String(this.sourceViewer.fps || 24)), xe(this.$("anomalies"), this.state.anomalies, {
      actions: this.refine.settings.spike_actions,
      onFrame: (p) => this.coordinator.seek(p, "anomaly"),
      onAction: (p, v) => {
        const g = Number(p.start_frame ?? p.frame) || 0, X = Math.max(g, Number(p.end_frame ?? p.frame) || g);
        for (let M = g; M <= X; M += 1) this.refine.setSpikeAction(M, v);
        this.render();
      }
    }), this.renderTimeline(), this.transport.render(), z(this), W(this);
    const b = this.$("stage-notice");
    if (b) {
  ...
  renderTimeline() {
    ...
    return this.currentHealth = bt(t, this.motionLimits), this.timeline.render({
      track: t,
  ...
  // -- lifecycle ---------------------------------------------------------
  restoreCachedResult() {
    const t = it(this.node);
    t && (this.result = { raw: t.track, refined: t.track }, this.state = N(this.state, { type: "APPLIED", fingerprint: t.fingerprint }), this.state = N(this.state, { type: "REFINED", fingerprint: t.fingerprint }));
  }
  executed(t) {
    const e = st(t);
  ...
  dispose() {
    Ct(this.client, this.state), this.disposed = !0, this.events.dispose(), this.refine.dispose(), this.coordinator.dispose(), this.sourceViewer.dispose(), this.overlay.dispose(), this.diagnostics.dispose(), this.viewer?.dispose(), this.viewer = null, this.viewerLoad = null;
