...
import { V as ze, a as Ye, O as He, b as Qe, E as Ze, W as Je, S as Ce, P as Ee, c as et, C as ce, H as tt, D as rt, G as ie, d as ot, e as st, f as at, g as We, F as je, M as pe, h as Ie, i as Be, j as Ne, k as nt, l as it, m as Ue, n as se, o as ct, B as lt, p as De, A as dt, q as be, r as Le, s as Pe, t as ut, u as mt, v as ht, w as ft, x as pt } from "./vendor-three-BQUrLQkn.js";
import { T as ye, bR as wt, bS as gt, bT as yt, ba as Mt, a as xt, C as bt } from "./chunk-mvRZEinl.js";
import { r as vt, q as Ct, a as Ae, s as Re, D as Ve, c as Bt, b as Lt, d as _t } from "./chunk-DfD5dw0S.js";
import { c as St } from "./chunk-a2yd8Eqb.js";
import { n as Gt } from "./chunk-o7pry0Ne.js";
...
function $t(o) {
  const { THREE: e, FBXLoader: u, GLTFLoader: p, OBJLoader: D, PLYLoader: h, STLLoader: B, neutral: v, wire: q, checkerMaterial: j, objectMaterial: I, applyModelMaterial: N, disposeObject: R, textureFor: re, cardMesh: J, generatePointField: te, sampleCamera: E, sampleObjectTransform: ae } = o;
  return {
    removeModel(A) {
    ...
    applyMotionClip(A, f) {
      ...
      let c = t.clips.findIndex((a) => (a.name || "").toLowerCase() === r.toLowerCase());
    ...
    rebuild(A, f, t, r = !1, c = "auto") {
      this.content.traverse((s) => {
        for (const z of [...s.children])
          z.userData.omnicamHelper && (s.remove(z), R(z, !0));
      }), R(this.content), this.content.clear(), this.objectNodes.clear(), this.selectionKey = "";
      const d = A.render_mode, a = r && ["clay", "motion_proxy", "depth_rich"].includes(c), i = (s, z) => {
        const M = v.clone();
        return M.side = z ? e.FrontSide : e.DoubleSide, s.color && (M.color = new e.Color(s.color)), M;
      }, x = (s) => a ? i(s, !!A.backface_culling) : I(s, d, !!A.backface_culling), m = new e.Group();
      ...
      if (["omni_ref", "point_field"].includes(d) || S) {
        const s = A.objects.filter((b) => b.enabled !== !1 && !["sun_light", "point_light", "spot_light", "null"].includes(b.type)).length, z = S && s <= 1 && (!A.point_density || A.point_density === "none") ? "sparse" : A.point_density || "balanced", { points: M, colors: W } = te(z, A.point_spread || "all_views", A.point_color || null);
        if (M.length > 0) {
      ...
      if (!["grid", "point_field"].includes(d))
        for (const s of A.objects) {
          if (s.enabled === !1) continue;
          const z = s.size || [1, 1, 1];
          let M;
          if (s.type === "glb" || s.type === "model") {
            const b = t.get(s.id), C = this.models.get(s.id), L = s.format || (s.type === "glb" ? "glb" : "");
            b && (C?.url !== b || C?.format !== L) && this.loadModel(s.id, b, L);
            const k = !!A.backface_culling, F = r && c === "clay" ? "neutral" : vt(s, A, r) ?? (s.material_mode || "textured");
            C?.url === b && (M = C.scene, N(M, F, s, k));
          } else if (s.type === "sphere")
            M = new e.Mesh(new e.SphereGeometry(0.5, 24, 16), x(s));
          else if (s.type === "cylinder")
            M = new e.Mesh(new e.CylinderGeometry(0.5, 0.5, 1, 24), x(s));
          else if (s.type === "torus") {
            const b = new e.TorusGeometry(0.5, 0.2, 16, 32);
            b.rotateX(Math.PI / 2), M = new e.Mesh(b, x(s));
          } else if (s.type === "pyramid") {
            const b = new e.ConeGeometry(0.7, 1, 4);
            b.rotateY(Math.PI / 4), M = new e.Mesh(b, x(s));
          } else if (s.type === "sun_light") {
            const b = new e.Group(), C = new e.DirectionalLight(s.color || 16774892, s.intensity ?? 2.2);
            C.castShadow = s.cast_shadow !== !1, C.castShadow && (C.shadow.mapSize.set(1024, 1024), C.shadow.bias = -8e-4, C.shadow.normalBias = 0.02, C.shadow.radius = 2.4, C.shadow.camera.near = 0.5, C.shadow.camera.far = 70, C.shadow.camera.left = C.shadow.camera.bottom = -14, C.shadow.camera.right = C.shadow.camera.top = 14);
            const L = (s.rotation || [0, 0, 0]).map(e.MathUtils.degToRad), k = new e.Vector3(0, 0, -1).applyEuler(new e.Euler(L[0], L[1], L[2], "YXZ"));
            ...
            const b = new e.Group(), C = new e.PointLight(s.color || 16777215, s.intensity ?? 2, 0, 2);
            b.add(C);
            const L = new e.Mesh(
            ...
            const b = new e.Group(), C = (s.cone_angle ?? 45) * Math.PI / 180, L = s.penumbra ?? 0.25, k = new e.SpotLight(s.color || 16777215, s.intensity ?? 3, 0, C, L, 2), F = (s.rotation || [0, 0, 0]).map(e.MathUtils.degToRad), $ = new e.Vector3(0, 0, -1).applyEuler(new e.Euler(F[0], F[1], F[2], "YXZ"));
            ...
            M = new e.Mesh(St(e), x(s));
          else if (s.type === "ground") M = new e.Mesh(new e.BoxGeometry(1, 1, 1), x(s));
          else if (s.type === "card")
            M = !s.material_mode || ["textured", "wireframe_texture"].includes(s.material_mode) ? J(s, f.get(s.id), A.card_fit || "contain") : new e.Mesh(new e.PlaneGeometry(z[0], z[1]), x(s));
          else if (s.type === "null") {
            const b = new e.AxesHelper(0.5);
            b.position.fromArray(s.position || [0, 0, 0]), b.userData.omnicamId = s.id, b.frustumCulled = !1, this.objectNodes.set(s.id, b), this.content.add(b);
            continue;
          } else
            M = new e.Mesh(new e.BoxGeometry(1, 1, 1), x(s));
          if (!M) continue;
          M.position.fromArray(s.position || [0, 0, 0]), M.rotation.set(...(s.rotation || [0, 0, 0]).map(e.MathUtils.degToRad));
          const W = ["sun_light", "point_light", "spot_light"].includes(s.type);
          if (s.type !== "card" && !W && M.scale.fromArray(z), M.userData.omnicamId = s.id, M.frustumCulled = !1, M.traverse((b) => {
            b.frustumCulled = !1, b.userData.omnicamId = s.id;
          }), !W) {
            const b = !!(A.show_wireframe || A.render_mode === "wireframe_texture" || s.material_mode === "wireframe_texture" || s.material_mode === "wireframe_neutral");
            Rt(e, M, { wireframe: b, vertices: A.show_vertices });
          }
          this.objectNodes.set(s.id, M), this.content.add(M);
        }
    },
    rebuildPath(A, f = "camera", t = null, r = "", c = null) {
      const d = Array.isArray(c) ? new Set(c) : null;
      R(this.path), this.path.clear();
      const a = r === "camera" ? A.active_camera_id : null, i = [
      ...
      (A.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: A.keyframes || [] }]).forEach((l, g) => {
        ...
        if (n.length >= 2) {
          const y = n[0].frame, P = n[n.length - 1].frame, S = Math.max(32, Math.min(256, P - y + 1)), s = { ...l, keyframes: n, objects: A.objects }, z = Array.from({ length: S }, (L, k) => {
            const F = y + (P - y) * k / Math.max(1, S - 1);
            return new e.Vector3().fromArray(E(s, F, A.objects).position);
        ...
        for (const y of n) {
          const P = n.indexOf(y), S = T, s = new e.Mesh(
            new e.SphereGeometry(S ? ne : 0.085, 16, 12),
            new e.MeshBasicMaterial({ color: S ? qt : G.marker, depthTest: !1 })
          );
          s.position.fromArray(y.camera.position), s.renderOrder = 910, s.userData.omnicamPathKey = { cameraId: l.id, frame: y.frame }, s.userData.omnicamWidget = "path", this.path.add(s);
          const z = new e.Mesh(
...
function Kt(o) {
  const { THREE: e, FBXLoader: u, GLTFLoader: p, OBJLoader: D, PLYLoader: h, STLLoader: B, neutral: v, wire: q, checkerMaterial: j, objectMaterial: I, applyModelMaterial: N, disposeObject: R, textureFor: re, cardMesh: J, generatePointField: te, sampleCamera: E, sampleObjectTransform: ae, hasOutlineMesh: A } = o;
  return {
    updateLiveCameras(f, t, r, c, d = "camera", a = null) {
      ...
      (f.cameras || [{ id: "camera_1", name: "Camera 1", keyframes: f.keyframes || [] }]).forEach((m, l) => {
        const g = m.color ? { line: new e.Color(m.color), marker: new e.Color(m.color), frustum: new e.Color(m.color), body: new e.Color(m.color).multiplyScalar(0.35) } : i[l % i.length], n = m.id === f.active_camera_id, G = n && d === "camera", T = c === "camera" && n, O = E(m, t, f.objects), y = new e.Vector3().fromArray(O.position || [0, 0, 0]), P = new e.Vector3().fromArray(O.target || [0, 0, 0]), S = P.clone().sub(y), s = S.length();
        s < 1e-4 ? S.set(0, 0, -1) : S.normalize();
        ...
        if (!T) {
          ...
          const U = e.MathUtils.clamp(s * 0.25, 0.5, 2.5), Z = O.camera_type === "orthographic" ? 5 / Math.max(0.01, O.zoom || 1) * 0.35 : U * Math.tan(e.MathUtils.degToRad(O.fov || 35) * 0.5), w = Z * (f.width || 16) / Math.max(1, f.height || 9), _ = y.clone().addScaledVector(S, U), V = [
    ...
    updateSelection(f, t, r, c = null, d = "", a = !1) {
      const i = c ? `${c.mode || ""}:${c.objectId || ""}:${(c.point || []).join(",")}` : "", x = `${t}:${r || ""}:${(f.__selectedObjectIds || []).join(",")}:${d}:${i}:${a ? "ortho" : "persp"}`;
          ...
          if (m) {
            ...
            try {
              ...
              if (m.traverse((n) => {
                ...
                l.setFromObject(m);
              if ((a || !A(m)) && !l.isEmpty() && Number.isFinite(l.min.x) && Number.isFinite(l.max.x) && Number.isFinite(l.min.y) && Number.isFinite(l.max.y) && Number.isFinite(l.min.z) && Number.isFinite(l.max.z)) {
          ...
          for (const m of f.__selectedObjectIds || []) {
            ...
            if (l) {
              ...
              try {
                ...
                if ((a || !A(l)) && !g.isEmpty() && Number.isFinite(g.min.x)) {
    ...
    listObjectBones(f) {
      ...
      return t.traverse((d) => {
        const a = d.isBone ? d.name : "";
        !a || c.has(a) || r.length >= 256 || (c.add(a), r.push(a));
      }), r;
    },
    /**
    ...
    sampleModelPoint(f, t, r, c = 24) {
      ...
      if (!d) return null;
      const a = this.models.get(f), i = a?.mixer && a.duration > 0, x = i ? a.mixer.time : null;
      i && (a.mixer.setTime(Math.max(0, r) / Math.max(1, c) % a.duration), d.updateMatrixWorld(!0));
      let m = null;
      if (t) {
        let l = null;
        if (d.traverse((g) => {
      ...
      return i && Number.isFinite(x) && (a.mixer.setTime(x), d.updateMatrixWorld(!0)), m;
    },
    ...
    sampleCharacterBonePose(f, t) {
      ...
      r.traverse((a) => {
        a.isBone && a.name && c.set(a.name, a);
      });
      const d = {};
      for (const [a, i] of Object.entries(t)) {
        ...
        m && x.quaternion.angleTo(m) < 1e-4 || (d[a] = x.quaternion.toArray());
      }
      return d;
    }
  };
  ...
  return {
    ...
    pickPathKey(f) {
      ...
      for (const i of this.path.children) {
        ...
        if (!x || (a.copy(i.position).project(this.activeCamera), a.z < -1 || a.z > 1)) continue;
        const m = (a.x * 0.5 + 0.5) * t, l = (1 - (a.y * 0.5 + 0.5)) * r, g = Math.hypot(f[0] - m, f[1] - l);
    ...
    pickPathSegment(f) {
      ...
      if (!c) return null;
      const { cameraId: d, firstFrame: a, lastFrame: i, frames: x, points: m } = c.object.userData.omnicamPathSegments;
      if (!m?.length || x.length < 2) return null;
      let l = 0, g = 1 / 0;
      for (let y = 0; y < m.length; y += 1) {
        const [P, S, s] = m[y], z = P - c.point.x, M = S - c.point.y, W = s - c.point.z, b = z * z + M * M + W * W;
        b < g && (g = b, l = y);
      }
      const n = a + (i - a) * l / Math.max(1, m.length - 1);
    ...
    configureCamera(f, t) {
      ...
      if (r.camera_type === "orthographic") {
        ...
        a.left = -n * t, a.right = n * t, a.top = n, a.bottom = -n, a.near = c, a.far = d, a.updateProjectionMatrix();
      } else
        a = this.perspective, a.fov = e.MathUtils.clamp(Number(r.fov) || 35, 1, 175), a.aspect = t, a.near = c, a.far = d, a.updateProjectionMatrix();
      ...
      return a.position.copy(i), a.up.copy(l), a.lookAt(x), a.updateMatrixWorld(), a;
    },
    pick(f, t, r, c) {
      ...
      if (this.liveCameras && this.liveCameras.visible)
        for (const a of this.raycaster.intersectObjects(this.liveCameras.children, !0))
          a.object?.userData?.omnicamType && d.push({
            distance: a.distance,
      ...
      return d.length ? (d.sort((a, i) => a.distance - i.distance), { type: d[0].type, id: d[0].id }) : null;
    },
    /**
      ...
      for (const i of a) {
        ...
        if (d === "vertex") {
          ...
          if (i.face) {
            ...
            for (const S of P) {
              const s = new e.Vector3(n.getX(S), n.getY(S), n.getZ(S)).applyMatrix4(G), z = s.distanceTo(i.point);
              z < O && (O = z, T = S, y = [s.x, s.y, s.z]);
            }
          } else
            for (let P = 0; P < n.count; P++) {
              const S = new e.Vector3(n.getX(P), n.getY(P), n.getZ(P)).applyMatrix4(G), s = S.distanceTo(i.point);
              s < O && (O = s, T = P, y = [S.x, S.y, S.z]);
            }
          if (y)
            return {
              type: "vertex",
        ...
        if (d === "edge" && i.face) {
          ...
          }, S = P(i.point, T, O), s = P(i.point, O, y), z = P(i.point, y, T), M = [S, s, z].reduce((W, b) => b.dist < W.dist ? b : W);
          return {
            type: "edge",
            mode: "edge",
            objectId: l,
...
function er(o) {
  const { THREE: e, FBXLoader: u, GLTFLoader: p, OBJLoader: D, PLYLoader: h, STLLoader: B, neutral: v, wire: q, checkerMaterial: j, objectMaterial: I, applyModelMaterial: N, disposeObject: R, textureFor: re, cardMesh: J, generatePointField: te, sampleCamera: E, sampleObjectTransform: ae, hasOutlineMesh: A, SelectionOutlineRenderer: f } = o;
  return {
    render(t, r, c, d, a, i = /* @__PURE__ */ new Map(), x = 0, m = !1, l = "camera", g = "subject", n = null, G = null, T = null, O = "auto") {
      ...
      (this.canvas.width !== d || this.canvas.height !== a) && this.renderer.setSize(d, a, !1);
      const S = (r && r.camera_type === "orthographic") === !0;
      this.renderer.setClearColor(0, 1);
      const s = t.viewport_bg_sequence && t.viewport_bg_sequence.length ? t.viewport_bg_sequence[x % t.viewport_bg_sequence.length] : t.viewport_bg_image || "";
      if (s) {
        this.bgImageUrl = s;
        const w = this.bgTextureCache.get(s);
        if (w)
          this.bgTextureCache.delete(s), this.bgTextureCache.set(s, w), this.bgTexture = w, this.scene.background = w;
        else if (!this.bgTextureLoads.has(s)) {
          const _ = this.bgLoadGeneration;
          this.bgTextureLoads.set(s, _), new e.TextureLoader().load(s, (X) => {
            if (this.bgTextureLoads.delete(s), this.disposed || _ !== this.bgLoadGeneration) {
              X.dispose();
              return;
            }
            for (X.colorSpace = e.SRGBColorSpace, this.bgTextureCache.set(s, X); this.bgTextureCache.size > 8; ) {
            ...
            this.bgImageUrl === s && (this.bgTexture = X, this.scene.background = X), this.invalidate();
          }, void 0, () => {
            this.bgTextureLoads.delete(s);
          });
        }
      } else {
      ...
      for (const w of t.objects) {
        ...
        if (!_) continue;
        const V = w.keyframes?.length ? ae(w, x) : w;
      ...
      const ee = d / Math.max(1, a), Q = this.configureCamera(r, ee);
      ...
      }), this.renderer.setScissorTest(!1), this.renderer.setViewport(0, 0, d, a);
      ...
      if (!m && !S && l === "object" && (g || t.__selectedObjectIds?.length) && !n) {
        ...
        _.length && (this.outlineRenderer || (this.outlineRenderer = new f(this.renderer, this.scene, void 0, Q)), this.outlineRenderer.render(Q, d, a, _), Z = !0);
      }
...
const Fe = { low: Tt, balanced: kt, high: Ot }, me = new Be({ color: 10265519, roughness: 0.48, metalness: 0.06, side: se }), $e = new Be({ color: 2237998, roughness: 0.95, metalness: 0, side: se }), _e = new be({ color: 11449792, wireframe: !0, side: se });
function Se(o = !1) {
  ...
  return u.wrapS = u.wrapT = ht, u.repeat.set(8, 8), u.colorSpace = Ce, u.needsUpdate = !0, new Be({ map: u, roughness: 0.85, metalness: 0, side: o ? Le : se });
}
function sr(o, e, u = !1) {
  const p = e === "wireframe" ? "wireframe" : o.material_mode || "textured", D = u ? Le : se;
  if (p === "wireframe") {
    const B = _e.clone();
...
function nr(o, e, u) {
  const [p, D] = o.size || [2, 3], h = new ie(), B = new pe(new Pe(p, D), new be({ color: 1448482, side: se, transparent: !0, opacity: 0.85 }));
...
class ir {
  constructor(e = () => {
    ...
    p.position.set(5, 8, 4), this.scene.add(p), this.flatLights = [this.scene.children.at(-2), p], this.studio = Bt(xe, this.renderer, _t), this.scene.add(this.studio.group), this.studioEnabled = !0, Re(xe, this.scene, this.renderer, this.studio, !0), this.content = new ie(), this.scene.add(this.content), this.path = new ie(), this.scene.add(this.path), this.liveCameras = new ie(), this.scene.add(this.liveCameras), this.selectionGroup = new ie(), this.scene.add(this.selectionGroup), this.selectionKey = "", this.perspective = new ot(35, 16 / 9, 0.01, 1e4), this.orthographic = new st(-5, 5, 2.8125, -2.8125, 0.01, 1e4), this.sceneKey = "", this.mediaSignature = "", this.bgImageUrl = "", this.bgTexture = null, this.bgTextureCache = /* @__PURE__ */ new Map(), this.bgTextureLoads = /* @__PURE__ */ new Map(), this.bgLoadGeneration = 0, this.disposed = !1, this.invalidate = e, this.onModelLoaded = u, this.modelUrls = /* @__PURE__ */ new Map(), this.models = /* @__PURE__ */ new Map(), this.modelLoads = /* @__PURE__ */ new Map(), this.objectNodes = /* @__PURE__ */ new Map(), this.raycaster = new at(), this.pointer = new ze(), this.activeCamera = this.perspective;
  }
  async loadModel(e, u, p = "glb") {
    ...
    if (!(!u || this.modelLoads.get(e) === D)) {
      ...
      try {
        ...
        const re = new lt().setFromObject(R), J = re.getSize(new De()), te = Math.max(J.x, J.y, J.z), E = Number.isFinite(te) && te > 1e-6 ? 2.5 / te : 1, ae = re.getCenter(new De());
        R.scale.setScalar(E), R.position.set(-ae.x * E, -re.min.y * E, -ae.z * E);
        const A = new ie();
...
const he = { THREE: xe, FBXLoader: je, GLTFLoader: Ue, OBJLoader: We, PLYLoader: Ne, STLLoader: Ie, neutral: me, wire: _e, checkerMaterial: Se, objectMaterial: sr, applyModelMaterial: ar, disposeObject: ve, textureFor: Ke, cardMesh: nr, generatePointField: Mt, sampleCamera: xt, sampleObjectTransform: bt, hasOutlineMesh: rr, SelectionOutlineRenderer: or };
Object.assign(
  ir.prototype,
  $t(he),
  Kt(he),
