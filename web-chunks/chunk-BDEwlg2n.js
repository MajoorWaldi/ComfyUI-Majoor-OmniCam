import { c as k, bn as x, e as w, aR as A, bo as ae, y as O, s as L, bp as oe, a as b, g as $, aS as I, bq as re } from "./chunk-B2Nzrc8E.js";
...
async function ot(e, t) {
  ...
  if (e.state.cameras = e.state.cameras.filter((r) => r.id !== t), t === e.state.playblast_camera_id && (e.state.playblast_camera_id = e.state.cameras[0].id), e.cameraPreviewSignature = "", o) {
    ...
    e.state.active_camera_id = r.id, e.state.keyframes = r.keyframes, e.state.camera = w(r.camera), e.camera = L(r, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = r.keyframes.find((s) => s.frame === e.frame)?.frame ?? null, e.editingKeyFrame = null;
  }
...
function Le(e, t, a = {}) {
  ...
  if (r === "replace") {
    ...
    for (const l of e.state.objects || [])
      F(l, s), (l.type === "glb" || l.type === "model") && l.asset && e.modelUrlsById?.set(l.id, I(l.asset));
  } else {
    ...
    for (const m of c)
      m.parent_id && i.has(m.parent_id) && (m.parent_id = i.get(m.parent_id)), F(m, s), e.state.objects.push(m), (m.type === "glb" || m.type === "model") && m.asset && e.modelUrlsById?.set(m.id, I(m.asset));
    for (const m of ee(o)) {
      const f = D(d, m.id);
...
function vt(e) {
  ...
  return t ? (e.checkpoint("Import extracted camera"), ye(e, t.track, { label: t.label }), e.pendingExtractorImport = null, W(e), e.setStatus?.(b("Imported {count} camera keys from {name}").replace("{count}", String(t.keyCount)).replace("{name}", t.label)), e.scheduleSerialize(), e.render(), !0) : !1;
}
function xt(e) {
  return e.pendingExtractorImport ? (e.pendingExtractorImport = null, W(e), e.render(), e.setStatus?.(b("Extracted camera preview dismissed")), !0) : !1;
}
function yt(e) {
  const t = e?.graph;
  if (!t) return 0;
