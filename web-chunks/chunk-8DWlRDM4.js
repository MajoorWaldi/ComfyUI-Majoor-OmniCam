...
import { v as h } from "./chunk-d8qZJB0s.js";
...
import { w as K } from "./chunk-CKLKPyqB.js";
import { c as z, w, W as B } from "./chunk-DsoefGi0.js";
...
function re(e, t) {
  ...
  }, s = () => String(e.node?.id ?? ""), i = (o) => {
    const c = String(e.queuePromptId || "");
    return c !== "" && String(o ?? "") === c;
  }, a = (o, c = {}) => e.dispatch({ type: "QUEUE_LIFECYCLE", state: o, ...c }), u = () => {
    e.queuePromptId = "";
  };
  return n("execution_start", (o) => {
    i(o.prompt_id) && a("PREPARING");
  }), n("executing", (o) => {
    if (!i(o.prompt_id)) return;
    const c = o.node ?? o.display_node ?? null;
    c != null && String(c) === s() && a(e.extractMode === "scene_reconstruct" ? "RECONSTRUCTING" : "TRACKING");
  }), n("progress", (o) => {
    if (!i(o.prompt_id) || o.node != null && String(o.node) !== s()) return;
    const c = Number(o.max) || 0;
    c > 0 && a(null, { progress: (Number(o.value) || 0) / c });
  }), n("executed", (o) => {
    String(o.node ?? o.display_node ?? "") === s() && (!i(o.prompt_id) && e.queuePromptId || (u(), e.executed(o.output ?? o)));
  }), n("execution_error", (o) => {
    i(o.prompt_id) && (a("FAILED", {
      error: String(o.exception_message || o.error || "The queued solve failed")
    }), u());
  }), n("execution_interrupted", (o) => {
    i(o.prompt_id) && (a("CANCELLED"), u());
  }), n("execution_success", (o) => {
    i(o.prompt_id) && (a("FINALIZING"), u());
  }), () => {
...
async function ie(e, t, { frontendVersion: r = oe(), intent: n } = {}) {
...
async function ue(e, {
  ...
  intervalMs: r = 16,
  now: n = () => Date.now(),
  sleep: s = (i) => new Promise((a) => setTimeout(a, i))
} = {}) {
...
async function Ge(e, t = "camera_track", { idle: r } = {}) {
  ...
  const { accepted: i, promptId: a } = await le(
    e.app,
    e.api,
    [s],
    { intent: { trigger_source: ae[t] || "omnicam_track" } }
  );
  return e.queuePromptId = i ? String(a || "") : "", { accepted: i, promptId: e.queuePromptId };
}
async function le(e, t, r, n) {
  const s = r.map(String).sort(), i = t.fetchApi;
  let a = "";
  t.fetchApi = async (u, o = {}) => {
    const c = await i.call(t, u, o);
    try {
      const l = String(u).split("?")[0];
      if (String(o.method || "GET").toUpperCase() === "POST" && (l === "/prompt" || l.endsWith("/prompt")) && c.ok && !a) {
        let f = {};
        try {
          f = JSON.parse(o.body || "{}");
        } catch {
        ...
        if (d && d.length === s.length && d.every((g, R) => g === s[R])) {
          ...
          typeof g?.prompt_id == "string" && (a = g.prompt_id);
        }
      }
    } catch {
    }
  ...
  try {
    return { accepted: !!await ie(e, r, n), promptId: a };
  } finally {
    t.fetchApi = i;
  }
}
...
function Se(e, t = e?.graph, r = "camera_track") {
  const n = r === "scene_reconstruct", s = n ? pe : de, i = n ? he : fe, a = n ? "Load Image" : "Load Video", u = n ? "an image" : "a video", o = n ? "reconstruct" : "track", c = me(n), l = Ee(e, t);
  if (l) {
    const f = s[_(l)];
    if (!f) {
      const m = N(e);
      return m ? {
        ...
        ...c,
        reason: `${_(l) || "This node"} produces its footage only while the workflow runs. Connect ${a}, or choose an Extractor source file, to ${o} without running.`,
    ...
    return d ? i.test(d) ? {
      ...
      originNodeId: l.id ?? null
    } : { ...c, reason: `${d} does not look like ${u} file.` } : { ...c, reason: `The connected ${a} node has no file selected yet.` };
  }
  const p = N(e);
  return p ? {
    available: !0,
    ...
    ref: p,
    originNodeId: null
  } : { ...c, reason: `Connect ${a}, or choose a source file, to ${o}.` };
}
function Me(e) {
...
class we extends EventTarget {
  ...
  acceptSolvedResult(t) {
    ...
    const i = Number(t?.confidence ?? n?.metadata?.confidence) || 0, a = t?.motionScene || U(n);
    return $(this.node, { motionScene: a, fingerprint: s }), t?.source && q(this.node, t.source), this.node.__majoorOmniCamStatus = W({ track: n, confidence: i }), this.dispatch({ type: "APPLIED", fingerprint: s }), this.workbench?.pushTracksToViewer?.(), t?.source && (this.workbench ? this.workbench.refreshSource() : this.pendingSourceResync = !0), !0;
  }
...
async function ve(e, t) {
  return w.open({
    ...
    opener: t,
    createSession: async () => {
      const r = ++e.workbenchGeneration, { openExtractorWorkbench: n, closeExtractorWorkbench: s } = await import("./chunk-DLBTD7Mm.js");
      if (e.disposed || r !== e.workbenchGeneration) return null;
      const i = n(e), a = T(e.node), u = new B({
        kind: "extractor",
        nodeId: e.node.id,
        ...
        onRequestClose: (o) => w.close(a, o),
        onResize: () => i.viewer?.resize?.()
      });
      return u.mount(i.root), {
        key: a,
        nodeId: e.node.id,
        host: u,
        close: async () => (s(i), u.dispose(), !0),
        dispose: () => {
          s(i), u.dispose();
        }
      };
...
function _e(e) {
  ...
  const i = K(e, () => setTimeout(n, 0)), a = e.onAfterGraphConfigured;
  e.onAfterGraphConfigured = function(...o) {
    a?.apply(this, o), n(), t.workbench?.reconstruction?.syncFromWidgets?.();
  };
  const u = e.onRemoved;
  return e.onRemoved = function(...o) {
    w.disposeForNode(e.id), i(), t.dispose(), u?.apply(this, o);
  }, t;
}
