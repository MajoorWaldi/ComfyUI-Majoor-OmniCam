...
function Hs(e, { mode: t, selectedIds: a } = {}) {
...
const Us = "omnicam_humanoid_v1", ct = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function Zs({ preset: e, overrides: t } = {}) {
  const a = Pe(e), r = Pe(t);
  return {
...
function ri(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
...
function na(e, t) {
  const a = t.quaternion || Ne(t.rotation), r = Vo(a, e.quaternion || Ne(e.rotation));
  return { position: C(oa(e.position.map((o, n) => o * t.size[n]), a), t.position), rotation: Ko(r), quaternion: r, size: e.size.map((o, n) => o * t.size[n]) };
}
function Go(e, t) {
...
const jt = ["speed", "angular_speed", "acceleration", "jerk"], $e = ["ok", "warn", "over"], sa = 0.8, qo = [0, 1.5, 0];
function At(e, t) {
  const a = [0];
...
function Yo(e, t) {
  ...
  for (let r = 0; r < e.length; r++) {
    ...
    if (n && n.grade === e[r] && Xo(n.metrics, o)) {
      n.end = r;
      continue;
    }
...
function ia(e, t = {}, a = null, r = "generic") {
  ...
  const d = Ho(l, o), c = Wo(l, o), m = At(d, o), p = At(m, o), u = { speed: d, angular_speed: c, acceleration: m, jerk: p }, h = $o(e, a), f = Uo(l, h, s, i), _ = l.map((g) => g.fov), y = t.allow_framing_loss === !0, v = [], b = [];
  for (let g = 0; g < n; g++) {
    const M = [], A = [];
    for (const G of jt) {
...
function Jo(e, t) {
  ...
  if (!a) return;
  if (!a.engaged) {
    if (Math.hypot(t.clientX - (a.startClientX ?? t.clientX), t.clientY - (a.startClientY ?? t.clientY)) < Qo) return;
    a.engaged = !0;
  }
...
function pn(e) {
  ...
  if (a.className = `oc-health-badge ${o.grade}`, a.textContent = nn(o.grade), r) {
  ...
  t.innerHTML = `
    ...
    <div class="oc-health-zones" data-role="health-zones">${mn(o)}</div>
...
function Me(e, t, a, r) {
  ...
  o && (e.checkpoint(a), o.keyframes = t, e.state.keyframes = t, e.syncActiveCameraTrack(), e.refreshKeys(), e.setFrame(e.frame, !1, !1), e.setStatus(r), pn(e));
}
function pi(e) {
  const t = de(e);
  if (!t) return;
  ...
  const r = Math.max(1, e.state.duration_frames - 1), o = tn(e.state.keyframes, r), n = ia({ ...e.state, keyframes: o }, t.limits, null, t.profile);
  if (n.max_speed <= a) {
...
function ui(e) {
  const t = de(e);
  if (!t) return;
  const a = rn(t, "framing_loss");
  if (!a.length) {
    e.setStatus(S("The subject stays in frame on this shot."));
    return;
  }
  const r = an(e.state.keyframes, a, t.subject);
  Me(
    e,
    r,
    "Recenter subject",
...
function yi() {
  for (const e of xr)
    e.defaultValue !== void 0 && (yn(e.id, e.defaultValue), e.onChange?.(e.defaultValue));
}
...
function Rt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = K(t), l = t.position, d = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = jn(d, i);
  let m, p;
  if (t.camera_type === "orthographic") {
...
const On = ["maya", "blender", "simple"];
function ht(e) {
  const t = e.state?.navigation_profile;
  return On.includes(t) ? t : "maya";
}
function In(e, t) {
...
function Nn(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function Rn(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  s.keyframes = Pn(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = le(s, e.frame, e.state.objects), s.camera = V(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function Bt(e, t) {
  const a = [];
...
function Fn(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => Ln(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((x) => x[b]))), d = [0, 1, 2].map((b) => Math.max(...i.map((x) => x[b]))), c = l.map((b, x) => (b + d[x]) / 2), { right: m, up: p, forward: u } = K(t), h = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), _ = f * h;
  let y = 2, v = 0.1;
  for (const b of i) {
...
function Ci(e, t = {}) {
  ...
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), Fn(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? S("Framed: all objects") : S("Framed: {name}").replace("{name}", s.name || s.type || S("Subject")));
}
function Bn(e, t, a, { forceLocal: r = !1 } = {}) {
...
function jr(e) {
  ...
  const n = Math.max(0.7, H(E(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? Bn(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
...
function ki(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : Gn(e.ctx, s[0], i);
    }
  }
...
function Ae(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = Hn(a), d = a.axis ? at[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], m = K(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...E(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, x = d ? k(d, b) : C(k(m.right, n * p * i), k(m.up, -s * p * i));
    u = u.map((w) => C(w, x)), u = Wn(e, a, u, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((w) => w.transform.position));
  }
...
const re = (e, t) => Math.round(e / t) * t, Xn = (e, t) => e.map((a) => re(a, t));
function Yn(e, t, a) {
...
function he(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, d) => Math.abs(o.axis[d]) > 1e-6 ? re(l, i) : o.base[d]) : Xn(t, i);
  if (s === "vertex" && a && !o) {
...
function Di(e, t) {
  ...
  if (c) {
    ...
    if (c.entity.type === "camera_path" && Nn(e, { baseDrag: b, viewCamera: n, entityPosition: c.entity.position })) return;
  ...
  const p = !!e.isNavigatingFly, u = En(e, t, n);
  if (!p && !u) return;
...
function Ti(e, t) {
  ...
  if (e.pathDrag) {
    ...
    c && (c.camera.position = Rt(
      ...
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = An(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_path") return void Rn(e, { pointer: i, deltaPixels: d, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
...
function Ai(e, t) {
  ...
  const a = In(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
...
function Be(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Zn(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function ts(e, t, a, r, o) {
  const n = le(e, a, e.objects);
  let s = t?.point;
  if (t?.object_id) {
    const c = e.objects.find((u) => u.id === t.object_id);
    if (!c) return null;
    const m = Go(e.objects, c), p = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
...
function Ye(e, t) {
  return es(t, e.interactionElement);
}
function os(e, t) {
...
function Ei(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = Xe(e.state);
    n && (e.checkpoint("Set motion interpolation"), Qn(n, o.target.value), ae(e, `Motion interpolation: ${o.target.value}`));
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? Be(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? ns(e, s) : n === "erase" && os(e.state, s), ae(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = Kt(e), l = rs(e.state, n.points, s, i);
...
function Oi(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => is(e, a.dataset.motionCreate), { signal: t });
...
const ge = { t: "translate", r: "rotate", s: "scale" }, ms = [
...
], ps = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function fs(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(ps) : !1;
}
function us(e) {
...
function gs(e, t) {
  return hs(e) || t?.lastKeyZone || "viewport";
}
let qt = !1;
function ys() {
...
function bs(e, t) {
  ...
  if (us(a) || (t.code === "Space" || t.key === "Enter") && fs(a)) return !1;
...
function ks([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * Ms), n = Math.round((window.innerHeight || t) * Cs);
  return [
...
function Ds(e, t) {
  const a = Lr[t];
  return !a || !e?.setSize ? !1 : (e.setSize(ks(a.default, a.min)), !0);
}
function Ts(e, t, a) {
  const r = Lr[t];
...
function As() {
  ...
  e.id = Ht, e.textContent = js, document.head.appendChild(e);
}
function oe(e) {
...
const $t = "MajoorOmniCam.ShowHelp", it = "oc-help-toolbar-icon", Ut = "oc-help-toolbar-css", Is = "#8b7bd8";
function Ps() {
  ...
  e.id = Ut, e.textContent = `
    .${it}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${Is};color:#fff;
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await yt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-0NFE0eFY.js").then((n) => n.g);
      return je("director:import:resolved", e), o;
    });
    const r = e.__majoorOmniCam;
    r && (je("director:attach:complete", e), Dn(r), t && Tn(r), _t(e, rt, t, a?.()));
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== ot) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-3teFXchc.js")).attachExtractor), e.__majoorOmniCamExtractor && _t(e, ot, t, a?.());
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== nt) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-BTHoeukd.js")).attachMonitor), e.__majoorOmniCamMonitor && _t(e, nt, t, a?.());
  }
});
export {
  ha as $,
