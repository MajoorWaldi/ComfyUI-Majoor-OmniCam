  ...
  e.registerExtension({
    ...
    beforeRegisterNodeDef(t, a) {
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        const s = Math.max(4, Number(this.size?.[0] || 160) - J - 6), l = -26, d = s + J / 2, c = l + J / 2;
        if (n.save(), this.selected) {
          const u = so(), p = n.createRadialGradient(d, c, J * 0.35, d, c, J * 1.15);
          p.addColorStop(0, `rgba(136, 115, 253, ${u})`), p.addColorStop(1, "rgba(136, 115, 253, 0)"), n.fillStyle = p, n.beginPath(), n.arc(d, c, J * 1.15, 0, Math.PI * 2), n.fill();
        }
        n.globalAlpha = 0.96, n.drawImage(i, s, l, J, J), n.restore();
      };
    }
  });
}
...
function ho(e, t = []) {
  const a = e && typeof e == "object" ? e : {}, r = new Set(t), o = /* @__PURE__ */ new Set(), n = (Array.isArray(a.cuts) ? a.cuts : []).filter((i) => i && typeof i == "object" && r.has(String(i.camera_id))).map((i) => ({
    camera_id: String(i.camera_id),
    start: Math.max(0, Math.round(Number(i.start) || 0))
  })).sort((i, s) => i.start - s.start).filter((i) => o.has(i.start) ? !1 : (o.add(i.start), !0));
...
function go(e) {
  ...
  if (!t.length) return [];
  const r = (a + 1) / t.length, o = t.map((i, s) => ({
    camera_id: i.id,
    start: s === 0 ? 0 : Math.round(s * r)
  })), n = /* @__PURE__ */ new Set();
  return o.filter((i) => i.start > a || n.has(i.start) ? !1 : (n.add(i.start), !0));
}
function ws(e, t, a) {
  ...
  if (n < o) return !1;
  const i = Math.max(o, Math.min(n, Math.round(Number(a) || 0)));
  return i === r[t].start ? !1 : (r[t].start = i, !0);
}
function yo(e, t) {
  const a = e?.cameras || [];
...
function bo(e, t, a = null) {
  ...
  if (!r.length || o <= 0 || r.some((s) => s.start === o)) return !1;
  const i = da(e, o)?.camera_id || r[0].camera_id;
  return r.push({ camera_id: a || yo(e, i), start: o }), r.sort((s, l) => s.start - l.start), !0;
}
function vo(e, t) {
  const a = e?.sequence?.cuts || [];
  return t < 0 || t >= a.length || a.length === 1 ? !1 : (a.splice(t, 1), a.length && (a[0].start = 0), !0);
}
const ma = Object.freeze(["select", "track", "anchor", "project", "erase"]), ua = Object.freeze(["manual_2d", "static_anchor", "world_point", "object_point", "camera_field"]), pa = Object.freeze(["linear", "smooth", "hold"]), Ce = (e, t = 0) => Number.isFinite(Number(e)) ? Number(e) : t, Ot = (e) => Math.max(0, Math.min(1, Ce(e)));
function _o(e, t) {
  return {
    time_seconds: Math.max(0, Math.min(t, Ce(e?.time_seconds))),
    x: Ot(e?.x),
    y: Ot(e?.y),
    visible: e?.visible !== !1,
...
function So(e) {
  const t = Math.max(1 / Math.max(1, Ce(e.fps, 24)), Ce(e.duration_frames, 120) / Math.max(1, Ce(e.fps, 24))), a = /* @__PURE__ */ new Set();
  return e.motion_layers = (Array.isArray(e.motion_layers) ? e.motion_layers : []).slice(0, 256).map((r, o) => {
    ...
    const i = ua.includes(r?.source_kind) ? r.source_kind : "manual_2d", s = (Array.isArray(r?.keys) ? r.keys : []).slice(0, 1e4).map((l) => _o(l, t)).sort((l, d) => l.time_seconds - d.time_seconds);
    return {
      id: n,
...
const wo = 32, Co = 64, xo = 128, Mo = Object.freeze(["off", "selected", "all"]), ko = Object.freeze(["annotation", "name", "tag"]), Pt = Object.freeze({ mode: "selected", content: "annotation" }), Ao = Object.freeze(["top", "center", "bottom"]), Do = /^[a-z0-9][a-z0-9_-]*$/, To = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/, jo = /[<>]|:\/\/|javascript:|expression\(|&#/i;
function nt(e) {
  ...
  for (const r of e) {
    ...
    if (!(!o || o.length > Co || !Do.test(o) || a.has(o)) && (a.add(o), t.push(o), t.length >= wo))
      break;
  }
  return t;
}
...
function Nt(e) {
  ...
  if (!t || t.length > xo || jo.test(t)) return null;
  const a = typeof e.color == "string" ? e.color.trim() : "", r = To.test(a) ? a.toLowerCase() : "#8d7ee8", o = Ao.includes(e.anchor) ? e.anchor : "top";
  return { text: t, visible: e.visible !== !1, color: r, anchor: o };
}
function xs(e) {
  return {
    mode: Mo.includes(e?.mode) ? e.mode : Pt.mode,
...
function ks(e, { mode: t, selectedIds: a } = {}) {
...
function As(e, t) {
  const a = Array.isArray(e?.position) ? e.position : [0, 0, 0], r = Array.isArray(e?.size) ? e.size : [1, 1, 1], o = Io.has(t) ? 0.35 : Math.max(0.2, (Number(r[1]) || 1) * 0.5 + 0.2);
...
function ha(e) {
  ...
  const a = Math.max(0, Math.round(Number(e.start_frame) || 0)), r = Math.round(Number(e.end_frame) || 0), o = r > a ? r : 0, n = Number(e.speed), i = fa(Number.isFinite(n) && n > 0 ? n : 1, Rt[0], Rt[1]), s = Number(e.offset_seconds);
  return {
    clip_id: t,
    start_frame: a,
    end_frame: o,
    speed: i,
    loop: e.loop !== !1,
    offset_seconds: Number.isFinite(s) ? s : 0
  };
}
function Ds(e, t, a, r) {
  const o = ha(e), n = Number(r) || 0, i = Math.max(1, Number(a) || 24);
  if (!o || n <= 0) return 0;
  const s = (u) => o.loop ? (u % n + n) % n : fa(u, 0, n);
  if (t <= o.start_frame) return s(o.offset_seconds);
  const l = o.end_frame > o.start_frame ? o.end_frame : null, d = l !== null && !o.loop ? Math.min(t, l) : t, c = o.offset_seconds + (d - o.start_frame) * o.speed / i;
  return s(c);
}
const Ts = "omnicam_humanoid_v1", gt = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function Bo(e, t, a, r) {
  for (const [o, n] of t)
    if (!a.has(o)) {
      ...
      } else if (n.some((i) => i.includes(e) || e.includes(i)))
        return o;
    }
  return null;
}
function js(e) {
  ...
  for (const n of [!0, !1])
    for (const [i, s] of Object.entries(zo))
      if (!o[i])
        for (const l of s) {
          const d = Bo(l, a, r, n);
          if (d != null) {
...
function Is({ preset: e, overrides: t } = {}) {
  const a = ze(e), r = ze(t);
  return {
...
function Ns(e, { startFrame: t, endFrame: a } = {}) {
  ...
  const n = [...e].sort((m, f) => m.frame - f.frame);
  if (Math.floor(o) - Math.floor(r) + 1 < n.length)
    return { ok: !1, reason: "insufficient_frame_slots" };
  const s = n.map((m) => Uo(m)), l = [];
  for (let m = 1; m < n.length; m += 1) {
    const f = Xo(Lt(n[m - 1]), Lt(n[m])), w = (s[m - 1] + s[m]) / 2;
    l.push(f * w);
  }
  const d = l.reduce((m, f) => m + f, 0), c = o - r, u = [0];
  if (d > 0) {
    let m = 0;
    for (const f of l)
      m += f, u.push(m / d);
  } else
    for (let m = 1; m < n.length; m += 1) u.push(m / (n.length - 1));
  const p = u.map((m) => Math.round(r + m * c));
  p[0] = r, p[p.length - 1] = o;
  for (let m = 1; m < p.length; m += 1)
    p[m] <= p[m - 1] && (p[m] = p[m - 1] + 1);
  for (let m = p.length - 1; m > 0; m -= 1)
    p[m] > o - (p.length - 1 - m) && (p[m] = o - (p.length - 1 - m));
  return p[p.length - 1] = o, p[0] = r, { ok: !0, keys: n.map((m, f) => ({ ...m, frame: p[f] })) };
}
function Yo(e) {
  ...
  if (!r || /^[A-Za-z]:/.test(r) || r.split("/").some((s) => s === "..")) return null;
  const o = r.lastIndexOf("/"), n = o >= 0 ? r.slice(o + 1) : r, i = o >= 0 ? r.slice(0, o) : "";
  return !n || n === "." ? null : { filename: n, subfolder: i, type: a[2] || "input" };
}
function Zo(e, t) {
  const a = Yo(t);
  if (!a) return "";
...
function Qo(e, t, a) {
  ...
  if (o?.source === r && a >= o.frame && o.index < t.length - 1) {
    let i = o.index;
    for (; i + 1 < t.length - 1 && a >= t[i + 1].frame; ) i += 1;
    if (t[i].frame < a && a < t[i + 1].frame)
      return Qe.set(e, { source: r, frame: a, index: i }), { leftIndex: i, left: t[i], right: t[i + 1] };
  }
  const n = _t(t, a);
  return Qe.set(e, { source: r, frame: a, index: n?.leftIndex ?? 0 }), n;
}
function H(e) {
  const t = j(e.target, e.position), a = Math.sqrt(Z(t, t)) < 1e-6 ? [0, 0, -1] : se(t);
  let r = e.up || [0, 1, 0], o = xe(a, r);
  Math.sqrt(Z(o, o)) < 1e-6 && (r = Math.abs(a[1]) > 0.9 ? [0, 0, a[1] > 0 ? -1 : 1] : [0, 1, 0], o = xe(a, r)), o = se(o);
  let n = se(xe(o, a));
  if (Math.abs(e.roll || 0) > 1e-9) {
    const i = e.roll * Math.PI / 180, s = Math.cos(i), l = Math.sin(i), d = x(D(o, s), D(n, l));
    n = x(D(n, s), D(o, -l)), o = d;
  }
  return { right: o, up: n, forward: a };
}
function Ls(e) {
  const t = j(e.target, e.position), a = W(t), r = a < 1e-6 ? [0, 0, -1] : D(t, 1 / a), o = Math.asin(T(r[1], -1, 1)) * 180 / Math.PI, n = Math.atan2(r[0], -r[2]) * 180 / Math.PI;
  return [o, n, e.roll || 0];
}
function Fs(e, t) {
  const [a, r, o] = t, n = Math.max(1e-4, W(j(e.target, e.position))), i = a * Math.PI / 180, s = r * Math.PI / 180, l = [Math.sin(s) * Math.cos(i), Math.sin(i), -Math.cos(s) * Math.cos(i)];
  e.target = x(e.position, D(l, n)), e.roll = o;
}
function V(e, t, a, r) {
  const { right: o, up: n, forward: i } = H(t), s = j(e, t.position), l = Z(s, i);
  if (l <= Math.max(1e-4, t.near || 0.01) || l >= (t.far || 1e4)) return null;
  const d = Z(s, o), c = Z(s, n);
  if (t.camera_type === "orthographic") {
    const p = 5 / Math.max(0.01, t.zoom || 1), h = p * a / Math.max(1, r);
    return [a * (0.5 + d / (2 * h)), r * (0.5 - c / (2 * p)), l];
  }
  const u = 0.5 * r / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
  return [a * 0.5 + d * u / l, r * 0.5 - c * u / l, l];
}
function de(e, t, a = null) {
  ...
  const o = Qo(e, r, t), n = z(r, t, "pos_x", (g) => (g.camera || g).position[0], !1, o), i = z(r, t, "pos_y", (g) => (g.camera || g).position[1], !1, o), s = z(r, t, "pos_z", (g) => (g.camera || g).position[2], !1, o);
  let l = z(r, t, "target_x", (g) => (g.camera || g).target[0], !1, o), d = z(r, t, "target_y", (g) => (g.camera || g).target[1], !1, o), c = z(r, t, "target_z", (g) => (g.camera || g).target[2], !1, o);
  const u = e.constraints?.look_at, h = u?.status === void 0 || u?.status === "active" ? u?.object_id || e.target_object_id || e.camera?.target_object_id : null, m = a || e.objects;
  if (h && Array.isArray(m)) {
    const g = m.find((M) => M.id === h);
    if (g && g.enabled !== !1) {
      const M = St(m, g, t), E = u?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
      l = (M.position?.[0] ?? 0) + (E[0] || 0), d = (M.position?.[1] ?? 1.5) + (E[1] || 0), c = (M.position?.[2] ?? 0) + (E[2] || 0);
    }
  }
  const f = z(r, t, "fov", (g) => Number((g.camera || g).fov ?? 35), !1, o), w = z(r, t, "roll", (g) => Number((g.camera || g).roll ?? 0), !0, o), b = z(r, t, "zoom", (g) => Number((g.camera || g).zoom ?? 1), !1, o), S = z(r, t, "near", (g) => Number((g.camera || g).near ?? 0.01), !1, o), y = z(r, t, "far", (g) => Number((g.camera || g).far ?? 1e4), !1, o), C = r[0]?.camera || r[0] || le();
  let _ = r[0];
  for (const g of r)
    if ((g.frame ?? 0) <= t) _ = g;
    else break;
  const A = (_.camera || _).camera_type;
  return {
    position: [n, i, s],
    target: [l, d, c],
    fov: T(f, 5, 150),
    ...
    zoom: Math.max(0.01, b),
    near: Math.max(1e-4, S),
    far: Math.max(S + 1e-4, y),
    ...C.up ? { up: [...C.up] } : {}
  };
}
function Jo(e) {
...
const T = (e, t, a) => Math.max(t, Math.min(a, e)), en = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, fe = (e, t = null) => typeof e == "string" && en.test(e.trim()) ? e.trim() : t, x = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], j = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], D = (e, t) => [e[0] * t, e[1] * t, e[2] * t], Z = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], xe = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], W = (e) => Math.sqrt(Math.max(1e-12, Z(e, e))), se = (e) => D(e, 1 / W(e));
function Sa(e, t, a) {
  const r = [a[0] - t[0], a[1] - t[1]], o = [e[0] - t[0], e[1] - t[1]], n = Math.max(1e-9, r[0] * r[0] + r[1] * r[1]), i = T((o[0] * r[0] + o[1] * r[1]) / n, 0, 1);
  return Math.hypot(e[0] - t[0] - r[0] * i, e[1] - t[1] - r[1] * i);
}
...
function Bt(e, t, a, r, o) {
  const n = rn(e, t), i = an.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", s = o ? o(e) : 0, l = a && o ? o(a) : s, d = r && o ? o(r) : s, c = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), u = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), p = (S = !1) => {
    const y = (s - l) / c, C = (d - s) / u;
    let _ = 0;
    if (!a && !r)
      _ = 0;
    else if (!a)
      ...
      const A = u / (c + u), g = c / (c + u);
      if (_ = y * A + C * g, S) {
        const M = 3 * Math.min(Math.abs(y), Math.abs(C));
        _ = T(_, -M, M);
      }
    }
    return {
  ...
  if (i === "flat")
    return { out_x: 1 / 3, out_y: 0, in_x: -1 / 3, in_y: 0, mode: i };
  if (i === "clamped")
    return { ...p(!0), mode: i };
  if (i === "auto")
    return { ...p(!1), mode: i };
  const h = p(!1), m = T(Number(n.out_x ?? h.out_x), 0.01, 0.99), f = Number(n.out_y ?? h.out_y);
  let w = T(Number(n.in_x ?? h.in_x), -0.99, -0.01), b = Number(n.in_y ?? h.in_y);
  if (i === "aligned") {
    const S = Math.hypot(m, f) || 1e-6, y = Math.hypot(w, b) || 1e-6;
    w = -m / S * y, b = -f / S * y;
  }
  return { out_x: m, out_y: f, in_x: w, in_y: b, mode: i };
}
function _t(e, t) {
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  const i = n || _t(e, t), { leftIndex: s, left: l, right: d } = i, c = s > 0 ? e[s - 1] : null, u = s + 2 < e.length ? e[s + 2] : null, p = Math.max(1, d.frame - l.frame), h = T((t - l.frame) / p, 0, 1);
  let m = r(l), f = r(d);
  if (o) {
    const S = ((f - m + 540) % 360 + 360) % 360 - 180;
    f = m + S;
  }
  if (l.interpolation === "bezier" || d.interpolation === "bezier") {
    const S = Bt(l, a, c, d, r), y = Bt(d, a, l, u, r), C = m, _ = m + (S.out_y || 0), A = f + (y.in_y || 0), g = f, M = T(Number(S.out_x ?? 1 / 3), 0, 1), E = T(1 + Number(y.in_x ?? -1 / 3), 0, 1);
    let G = 0, F = 1;
    for (let It = 0; It < 32; It++) {
      const Y = (G + F) * 0.5, Ze = 1 - Y;
      3 * Ze * Ze * Y * M + 3 * Ze * Y * Y * E + Y * Y * Y < h ? G = Y : F = Y;
    }
    const X = (G + F) * 0.5, re = 1 - X;
    return re * re * re * C + 3 * re * re * X * _ + 3 * re * X * X * A + X * X * X * g;
  }
  const b = tn(h, l.interpolation);
  return m + (f - m) * b;
}
...
function Ue(e, t) {
  ...
  const r = me(e), o = (b, S) => (b.transform?.position || r.position)[S] ?? 0, n = (b, S) => (b.transform?.rotation || r.rotation)[S] ?? 0, i = (b, S) => (b.transform?.size || r.size)[S] ?? (S === 2 ? 0.01 : 1), s = _t(a, t), l = z(a, t, "pos_x", (b) => o(b, 0), !1, s), d = z(a, t, "pos_y", (b) => o(b, 1), !1, s), c = z(a, t, "pos_z", (b) => o(b, 2), !1, s), u = z(a, t, "rot_x", (b) => n(b, 0), !0, s), p = z(a, t, "rot_y", (b) => n(b, 1), !0, s), h = z(a, t, "rot_z", (b) => n(b, 2), !0, s), m = z(a, t, "scale_x", (b) => i(b, 0), !1, s), f = z(a, t, "scale_y", (b) => i(b, 1), !1, s), w = z(a, t, "scale_z", (b) => i(b, 2), !1, s);
  return {
    position: [Number.isFinite(l) ? l : r.position[0], Number.isFinite(d) ? d : r.position[1], Number.isFinite(c) ? c : r.position[2]],
    rotation: [Number.isFinite(u) ? u : r.rotation[0], Number.isFinite(p) ? p : r.rotation[1], Number.isFinite(h) ? h : r.rotation[2]],
    size: [
      Math.max(0.01, Number.isFinite(m) ? m : r.size[0]),
      Math.max(0.01, Number.isFinite(f) ? f : r.size[1]),
      Math.max(0.01, Number.isFinite(w) ? w : r.size[2])
    ]
  };
}
function Bs(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  const n = [], i = [];
  let s = 0.65, l = 0.72, d = 0.82;
  if (typeof a == "string" && a.startsWith("#")) {
    const p = a.replace("#", "");
    p.length === 6 && (s = parseInt(p.slice(0, 2), 16) / 255, l = parseInt(p.slice(2, 4), 16) / 255, d = parseInt(p.slice(4, 6), 16) / 255);
  }
  const c = 0.618033988749895, u = 0.324717957244746;
  for (let p = 0; p < o; p++) {
    const h = p * c % 1, m = p * u % 1, f = (p + 0.5) * 0.7548776662466927 % 1;
    let w = 0, b = 0, S = 0, y = 0.65, C = 0.72, _ = 0.82;
    if (t === "ground_focus")
      if (h < 0.6) {
        const A = 0.4 + Math.sqrt(m) * 24, g = f * Math.PI * 2 + p * 2.399963229728653;
        w = Math.cos(g) * A, S = Math.sin(g) * A, b = 0.01 + h * 0.75, y = 0.86, C = 0.9, _ = 0.98;
      } else {
        const A = 1 + Math.sqrt(m) * 18, g = f * Math.PI * 2 + p * 2.399963229728653;
        w = Math.cos(g) * A, S = Math.sin(g) * A, b = 0.75 + (h - 0.6) * 8.5, y = 0.62, C = 0.7, _ = 0.82;
      }
    else if (t === "dome") {
      const A = h * Math.PI * 2, g = 1 - 2 * m, M = Math.sqrt(Math.max(0, 1 - g * g)), E = 1.5 + Math.cbrt(f) * 20;
      w = Math.cos(A) * M * E, S = Math.sin(A) * M * E, b = Math.max(0.01, g * E * 0.75 + 2.5), y = 0.72, C = 0.78, _ = 0.88;
    } else {
      const A = p % 4;
      if (A === 0) {
        const g = 0.3 + Math.sqrt(m) * 28, M = p * 2.399963229728653;
        w = Math.cos(M) * g, S = Math.sin(M) * g, b = 0.01 + f * 0.34, y = 0.9, C = 0.94, _ = 1;
      } else if (A === 1) {
        const g = 0.6 + Math.sqrt(m) * 18, M = p * 2.399963229728653;
        w = Math.cos(M) * g, S = Math.sin(M) * g, b = 0.35 + f * 3.15, y = 0.68, C = 0.76, _ = 0.86;
      } else if (A === 2) {
        const g = 2 + Math.sqrt(m) * 24, M = p * 2.399963229728653;
        w = Math.cos(M) * g, S = Math.sin(M) * g, b = 3.5 + f * 11.5, y = 0.55, C = 0.65, _ = 0.78;
      } else {
        const g = 0.5 + m * 6.5, M = p * 2.399963229728653;
        w = Math.cos(M) * g, S = Math.sin(M) * g, b = 0.05 + f * 4.95, y = 0.8, C = 0.86, _ = 0.94;
      }
    }
    n.push(w, b, S), i.push(a ? y * s : y, a ? C * l : C, a ? _ * d : _);
  }
  return { points: n, colors: i };
}
function sn() {
...
function Ks(e) {
  ...
  if (!e || typeof e != "object") return t;
  const a = { ...t, ...e };
  a.fps = Math.round($(a.fps, 24, 1, 120)), a.duration_frames = Math.round($(a.duration_frames, 120, 1, De.maxDurationFrames)), a.width = Math.round($(a.width, 1280, 64, 4096)), a.height = Math.round($(a.height, 720, 64, 4096));
  const r = (c, u) => (Array.isArray(c) ? c : []).slice(0, De.maxKeysPerTrack).map((p) => ({
    frame: Math.max(0, Math.round(Number(p.frame || 0))),
    camera: q(p.camera || p || u),
  ...
  n = [...new Map(n.map((c) => [c.frame, c])).values()].sort((c, u) => c.frame - u.frame), n.length || (n = [{ frame: 0, camera: q(o), interpolation: "ease" }]);
  const i = Array.isArray(a.cameras) && a.cameras.length ? a.cameras : [{ id: "camera_1", name: "Camera 1", color: "#4aa3ef", camera: o, keyframes: n }], s = /* @__PURE__ */ new Set();
  a.cameras = i.slice(0, De.maxCameras).map((c, u) => {
    let p = String(c?.id || `camera_${u + 1}`);
    s.has(p) && (p = `camera_${u + 1}`), s.add(p);
    const h = q(c?.camera || c?.keyframes?.[0]?.camera || o);
    let m = r(c?.keyframes, h);
    return m = [...new Map(m.map((f) => [f.frame, f])).values()].sort((f, w) => f.frame - w.frame), m.length || (m = [{ frame: 0, camera: q(h), interpolation: "ease" }]), {
      id: p,
      name: String(c?.name || `Camera ${u + 1}`),
      color: fe(c?.color),
      camera: h,
      keyframes: m,
  ...
  }), a.active_camera_id = a.cameras.some((c) => c.id === a.active_camera_id) ? a.active_camera_id : a.cameras[0].id, a.sequence = ho(a.sequence, a.cameras.map((c) => c.id)), a.playblast_camera_id = a.playblast_camera_id === po && a.sequence.cuts.length || a.cameras.some((c) => c.id === a.playblast_camera_id) ? a.playblast_camera_id : a.active_camera_id;
  ...
  a.camera = l.camera, a.keyframes = l.keyframes, a.target_object_id = l.target_object_id || null, a.target_offset = l.target_offset || [0, 0, 0], a.aim_bone = l.aim_bone || null, a.objects = (Array.isArray(a.objects) ? a.objects : t.objects).slice(0, De.maxObjects).map((c) => ({
    ...
    ...c?.tags !== void 0 ? { tags: nt(c.tags) } : {},
    ...Nt(c?.annotation) ? { annotation: Nt(c.annotation) } : {},
...
function wa(e, [t, a, r, o]) {
  const [n, i, s] = e, l = o * n + a * s - r * i, d = o * i + r * n - t * s, c = o * s + t * i - a * n, u = -t * n - a * i - r * s;
  return [l * o - u * t - d * r + c * a, d * o - u * a - c * t + l * r, c * o - u * r - l * a + d * t];
}
function cn([e, t, a, r]) {
  const o = 1 - 2 * (t * t + a * a), n = 2 * (e * t - a * r), i = 2 * (e * a + t * r), s = 1 - 2 * (e * e + a * a), l = 2 * (t * a - e * r), d = 2 * (t * a + e * r), c = 1 - 2 * (e * e + t * t), u = Math.asin(Math.max(-1, Math.min(1, i))), [p, h] = Math.abs(i) < 0.9999999 ? [Math.atan2(-l, c), Math.atan2(-n, o)] : [Math.atan2(d, s), 0];
  return [p, u, h].map((m) => m * 180 / Math.PI);
}
function Ca(e, t) {
  const a = t.quaternion || Fe(t.rotation), r = ln(a, e.quaternion || Fe(e.rotation));
  return { position: x(wa(e.position.map((o, n) => o * t.size[n]), a), t.position), rotation: cn(r), quaternion: r, size: e.size.map((o, n) => o * t.size[n]) };
}
function dn(e, t) {
  const a = new Map(e.map((o) => [o.id, o])), r = (o, n = /* @__PURE__ */ new Set()) => {
    const i = { ...me(o), quaternion: Fe(o.rotation) };
    if (!o?.id || n.has(o.id)) return i;
    const s = o.parent_id ? a.get(o.parent_id) : null;
    if (!s) return i;
    const l = new Set(n);
    return l.add(o.id), Ca(i, r(s, l));
  };
  return r(t);
}
function St(e, t, a, r = /* @__PURE__ */ new Set()) {
  ...
  const n = new Set(r);
  n.add(t.id);
  const i = t.parent_id ? e.find((l) => l.id === t.parent_id) : null;
  if (!i) return o;
  const s = St(e, i, a, n);
  return Ca(o, s);
}
const Kt = ["speed", "angular_speed", "acceleration", "jerk"], Je = ["ok", "warn", "over"], xa = 0.8, mn = [0, 1.5, 0];
function Vt(e, t) {
  const a = [0];
...
function pn(e, t) {
  const a = [0];
  for (let r = 1; r < e.length; r++) {
    const o = H(e[r - 1]), n = H(e[r]), i = ["right", "up", "forward"].reduce(
      (l, d) => l + o[d][0] * n[d][0] + o[d][1] * n[d][1] + o[d][2] * n[d][2],
      0
    ), s = Math.max(-1, Math.min(1, (i - 1) * 0.5));
    a.push(Math.acos(s) * 180 / Math.PI * t);
  }
  return a;
}
...
function Gt(e, t) {
  return t == null || t <= 0 ? "ok" : e > t ? "over" : e > t * xa ? "warn" : "ok";
}
function qt(e) {
...
function Ma(e, t = {}, a = null, r = "generic") {
  const o = Math.max(1, Number(e.fps) || 24), n = Math.max(1, Number(e.duration_frames) || 1), i = Math.max(1, Number(e.width) || 1280), s = Math.max(1, Number(e.height) || 720), l = [];
  for (let g = 0; g < n; g++) l.push(de(e, g, e.objects));
  const d = un(l, o), c = pn(l, o), u = Vt(d, o), p = Vt(u, o), h = { speed: d, angular_speed: c, acceleration: u, jerk: p }, m = fn(e, a), f = hn(l, m, i, s), w = l.map((g) => g.fov), b = t.allow_framing_loss === !0, S = [], y = [];
  for (let g = 0; g < n; g++) {
    const M = [], E = [];
    for (const G of Kt) {
      const F = Gt(h[G][g], t[`max_${G}`]);
      M.push(F), F !== "ok" && E.push(G);
    }
    !f[g] && !b && (M.push("over"), E.push("framing_loss")), S.push(qt(M)), y.push(E);
  }
  const C = f.filter((g) => !g).length, _ = {
    profile: r,
    warn_ratio: xa,
    limits: t,
    subject: m,
    duration_frames: n,
    fps: o,
    max_speed: Math.max(...d),
    max_angular_speed: Math.max(...c),
    max_acceleration: Math.max(...u),
    max_jerk: Math.max(...p),
  ...
  for (const g of [...Kt, "fov_drift"]) {
    const M = g === "fov_drift" ? "max_fov_change" : `max_${g}`, E = t[M];
    E != null && _[M] > Number(E) && _.violations.push({ metric: M, value: _[M], recommended_max: Number(E) });
  }
  C && !b && _.violations.push({ metric: "framing_loss_frames", value: C, recommended_max: 0 });
  const A = Gt(_.max_fov_change, t.max_fov_change);
  return _.track_grades = { fov_drift: A }, _.grade = qt([...S, A]), _.trajectory_valid = _.violations.length === 0, _.ok = _.trajectory_valid, _;
}
function bn(e) {
...
function ka(e, t, a) {
  const r = a.getBoundingClientRect(), o = Math.max(1, e.state.duration_frames - 1), n = T(Number(e.timelineZoom) || 1, 0.1, 50), i = Number(e.timelinePan) || 0, s = o / n, l = (t.clientX - r.left) / Math.max(1, r.width);
  return T(Math.round(i + l * s), 0, o);
}
function Vs(e, t) {
  ...
  if (t.shiftKey)
    ...
    const n = t.currentTarget.getBoundingClientRect(), i = (t.clientX - n.left) / Math.max(1, n.width), s = T(Number(e.timelineZoom) || 1, 0.2, 30), l = T(s * r, 0.2, 30), d = a / s, c = a / l, u = (Number(e.timelinePan) || 0) + i * d;
    e.timelinePan = T(u - i * c, -a * 0.5, a), e.timelineZoom = l;
  }
  e.refreshKeys(), e.setStatus(v("Timeline zoom: {value1}%", { value1: (e.timelineZoom * 100).toFixed(0) }));
}
function Gs(e) {
...
function Ws(e, t) {
  ...
  if (e.boxSelect && t.pointerId === e.boxSelect.pointerId) {
    t.preventDefault(), t.stopPropagation();
    const a = e.boxSelect.box.getBoundingClientRect(), r = Math.max(1, e.state.duration_frames - 1), o = T(Number(e.timelineZoom) || 1, 0.1, 50), n = Number(e.timelinePan) || 0, i = r / o, s = (u) => T(n + u / Math.max(1, a.width) * i, 0, r), l = Math.min(s(e.boxSelect.startX), s(e.boxSelect.currentX)), d = Math.max(s(e.boxSelect.startX), s(e.boxSelect.currentX));
    e.boxSelect.overlay?.remove(), e.boxSelect = null;
    const c = e.timelineKeyframes().filter((u) => u.frame >= l && u.frame <= d).map((u) => u.frame);
    c.length && (e.selectedKeyFrames = new Set(c), e.selectedKeyFrame = c[0], e.updateKeyVisualState(), e.refreshKeyEditor(), e.setStatus(v("{value1} keys selected", { value1: c.length })));
    return;
  }
...
function _n(e, t) {
  ...
  const r = a.box.getBoundingClientRect(), o = Math.max(1, e.state.duration_frames - 1), n = T(Number(e.timelineZoom) || 1, 0.1, 50), i = Number(e.timelinePan) || 0, s = o / n;
  let l = Math.round(T(i + (t.clientX - r.left) / Math.max(1, r.width) * s, 0, o));
  l = e.snapFrame(l);
  const d = l - a.startPointerFrame;
  let c = a.badge;
  c || (c = document.createElement("div"), c.className = "floating-retime-badge", a.box.appendChild(c), a.badge = c);
  const u = it(e, l);
  if (c.style.left = `${u}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${d !== 0 ? ` (${d > 0 ? "+" : ""}${d})` : ""}`, a.moving && a.moving.length > 1) {
    if (d === a.lastDelta) return;
    a.lastDelta = d;
    const p = e.timelineKeyframes(), h = new Set(p.filter((m) => !e.selectedKeyFrames.has(m.frame)).map((m) => m.frame));
    for (const m of a.moving) {
      let f = T(m.startFrame + d, 0, e.state.duration_frames - 1);
      for (; h.has(f) && f > 0 && f < e.state.duration_frames - 1; ) f += Math.sign(d || 1);
      m.key.frame = h.has(f) ? m.key.frame : f;
    }
    p.sort((m, f) => m.frame - f.frame), e.editingKeyFrame = a.key.frame, e.scheduleSerialize(), e.setFrame(a.key.frame, !1, !0);
    return;
  }
...
function wn(e, t) {
  ...
  if (o <= 1e-9) return a;
  const n = a[0].frame ?? 0, i = (a[a.length - 1].frame ?? t) - n;
  if (i <= 0) return a;
  let s = n;
  for (let l = 1; l < a.length - 1; l++) {
    const d = n + Math.round(i * (r[l] / o));
    a[l].frame = Math.min(t - 1, Math.max(s + 1, d)), s = a[l].frame;
  }
  return a;
}
function Aa(e, t) {
...
function Da(e, t, a = 0.6) {
  const r = Be(e), o = Math.min(1, Math.max(0, Number(a) || 0));
  if (!o || r.length < 3 || !t?.length) return r;
  const n = Be(r);
  for (let i = 1; i < r.length - 1; i++)
    if (Aa(r[i].frame ?? 0, t))
      for (const s of ["position", "target"]) {
        const l = [r[i - 1], r[i], r[i + 1]].map((u) => u.camera?.[s]).filter((u) => Array.isArray(u) && u.length >= 3), d = r[i].camera?.[s];
        if (l.length < 3 || !Array.isArray(d)) continue;
        const c = [0, 1, 2].map((u) => l.reduce((p, h) => p + Number(h[u] || 0), 0) / l.length);
        n[i].camera[s] = d.map((u, p) => Number(u) + (c[p] - Number(u)) * o);
      }
  return n;
}
function Cn(e, t, a) {
...
function jn(e) {
  ...
  let a = 0;
  for (const { val: n, limit: i } of t) {
    const s = (n || 0) / i;
    let l = 100;
    s <= 0.75 ? l = 100 : s <= 1 ? l = 100 - (s - 0.75) / 0.25 * 20 : s <= 1.5 ? l = 80 - (s - 1) / 0.5 * 40 : l = Math.max(0, 40 - (s - 1.5) * 40), a += l;
  }
...
function he(e, t, a, r) {
  const o = a == null ? v("no limit") : `${Ht(t)} / ${Ht(a)}`, n = a != null && a > 0, i = n ? Math.min(100, Math.round(t / a * 100)) : 0, s = r === "over" ? "var(--oc-danger)" : r === "warn" ? "var(--oc-warn)" : "var(--oc-ok)";
  return `
    <div class="oc-health-metric" data-grade="${r}">
      ...
      <div class="oc-health-bar-track">
        <div class="oc-health-bar-fill" style="width:${i}%;background:${s}"></div>
      </div>` : ""}
    </div>`;
}
function je(e, t, a) {
...
function En(e) {
  ...
  return t.length ? t.slice(0, 6).map((a) => {
    const r = a.metrics.map((n) => Xe(n)).join(", "), o = a.start === a.end ? v("Frame {frame}").replace("{frame}", String(a.start)) : v("Frames {start}-{end}").replace("{start}", String(a.start)).replace("{end}", String(a.end));
    return `
...
function In(e) {
  ...
  if (a.className = `oc-health-badge ${o.grade}`, a.textContent = kn(o.grade), r) {
    const { score: l, letter: d } = jn(o);
    r.textContent = `${l}% (${d})`, r.className = `oc-health-score-badge grade-${d.toLowerCase()}`;
  }
  const i = [
    he(
      "speed",
  ...
  ].join(""), s = o.framing_loss_frames ? `<div class="oc-health-metric" data-grade="over">
         <div class="oc-health-metric-row">
           <span class="oc-health-dot"></span>
  ...
  t.innerHTML = `
    <div class="oc-health-metrics">${i}${s}</div>
    ...
    <div class="oc-card-actions oc-health-actions">
      ...
      <button data-act="health-timing" title="${v("Open the Graph Editor on camera timing weights")}"><i class="pi pi-sliders-h"></i> ${v("Inspect Timing")}</button>
    </div>
...
function Us(e, t) {
  ...
  if (a) {
    ...
    for (const r of a.segments) {
      ...
      const o = it(e, r.start), n = it(e, r.end + 1);
      if (n < -5 || o > 105) continue;
      const i = document.createElement("div");
      i.className = "oc-health-band", i.dataset.grade = r.grade, i.style.left = `${o}%`, i.style.width = `${Math.max(0.4, n - o)}%`, i.title = r.metrics.map((s) => Xe(s)).join(", "), t.appendChild(i);
    }
  }
}
function Ae(e, t, a, r) {
...
function Xs(e) {
  ...
  const r = Math.max(1, e.state.duration_frames - 1), o = wn(e.state.keyframes, r), n = Ma({ ...e.state, keyframes: o }, t.limits, null, t.profile);
  if (n.max_speed <= a) {
    Ae(e, o, "Slow to limits", v("Speed flattened; the shot keeps its length."));
    return;
  }
  const i = n.max_speed / a * (e.state.duration_frames / Math.max(1, e.state.fps));
  Ae(e, o, "Slow to limits", v("Speed flattened, still over: this path needs about {seconds}s to fit the limit.").replace("{seconds}", i.toFixed(1)));
}
function Ys(e) {
  const t = ue(e);
  if (!t) return;
  ...
  const r = Da(e.state.keyframes, a, 0.6);
  Ae(
    e,
    r,
...
function Qs(e, t, a) {
  if (!ue(e)) return;
  const o = Da(e.state.keyframes, [{ start: t, end: a }], 0.6);
  Ae(
    e,
    o,
    "Smooth zone",
...
function L(e, t) {
  try {
    const a = Mt?.extensionManager?.setting?.get(e);
    return a ?? t;
  } catch {
    return t;
  }
...
function ve(e, t) {
  try {
    Mt?.extensionManager?.setting?.set?.(e, t);
  } catch (a) {
...
function K(e, t, a, r, o = !1) {
  const n = Number(L(e, t)), i = Number.isFinite(n) ? Math.min(r, Math.max(a, n)) : t;
...
function Kr() {
  const e = String(L(Ta, "auto")), t = String(L("Comfy.Locale", "en") || "en").slice(0, 2).toLowerCase(), a = e === "auto" ? t : e, r = a !== uo();
  if (mo(a), !!r) {
    for (const o of U)
      if (!o.disposed)
        try {
...
function Zn(e, { cameraId: t, frame: a, additive: r = !1 } = {}) {
  ...
  const i = new Set(o.frames);
  if (i.has(a)) {
    i.delete(a);
    let s = o.primaryFrame;
    return i.size ? s === a && (s = Math.max(...i)) : s = null, { ...o, frames: i, primaryFrame: s };
  }
  return i.add(a), { ...o, cameraId: t, frames: i, primaryFrame: a };
}
function ol(e, t) {
  const a = e || Ve();
...
function Zt(e, t, a, r, o) {
  const { right: n, up: i, forward: s } = H(t), l = t.position, d = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = Jn(d, s);
  let u, p;
  if (t.camera_type === "orthographic") {
    const h = 5 / Math.max(0.01, t.zoom || 1), m = h * r / Math.max(1, o);
    u = (e[0] / Math.max(1, r) - 0.5) * 2 * m, p = (0.5 - e[1] / Math.max(1, o)) * 2 * h;
  } else {
    const h = 0.5 * o / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
    u = (e[0] - r / 2) * c / h, p = (o / 2 - e[1]) * c / h;
  }
  return [0, 1, 2].map((h) => l[h] + s[h] * c + n[h] * u + i[h] * p);
}
function ei(e) {
...
function ti(e, { pointerX: t, pointerY: a, shiftKey: r, altKey: o }) {
  ...
  const i = (e.state.cameras || []).find((l) => l.id === n.cameraId), s = (i?.keyframes || []).find((l) => l.frame === n.frame);
  return s ? (i.id !== e.state.active_camera_id && e.activateCamera(i.id), r ? (Qt(e, { cameraId: i.id, frame: s.frame, additive: !0 }), e.render(), !0) : (e.pathDrag = { cameraId: n.cameraId, frame: n.frame, anchor: [...s.camera.position], startX: t, startY: a, moved: !1, historyCheckpointed: !1 }, e.interactionElement.style && (e.interactionElement.style.cursor = "grabbing"), Qt(e, { cameraId: i.id, frame: s.frame, additive: !1 }), e.render(), !0)) : !1;
}
const ai = 3;
function ri(e, t, a, r, o) {
  const n = V(e, a, r, o), i = V(t, a, r, o);
  return !n || !i ? !1 : Math.hypot(n[0] - i[0], n[1] - i[1]) < ai;
}
function oi(e, { pointerX: t, pointerY: a, overHandle: r, viewCamera: o, e: n }) {
  if (!e.webgl?.pickCurveHandle) return !1;
  const i = e.webgl.pickCurveHandle([t, a]);
  if (!i) return !1;
  const s = (e.state.cameras || []).find((c) => c.id === i.cameraId), l = (s?.keyframes || []).findIndex((c) => c.frame === i.frame), d = l >= 0 ? s.keyframes[l] : null;
  return !d || ri(i.position, d.camera.position, o, e.canvas.width, e.canvas.height) ? !1 : (r && n.stopImmediatePropagation?.(), e.curveHandleDrag = {
    cameraId: i.cameraId,
    frame: i.frame,
    side: i.side,
    anchor: [...d.camera.position],
    prevKey: s.keyframes[l - 1] || null,
    nextKey: s.keyframes[l + 1] || null,
    startX: t,
    startY: a,
    moved: !1,
    historyCheckpointed: !1
  }, e.interactionElement.style && (e.interactionElement.style.cursor = "grabbing"), e.selectKeyframe?.(d), !0);
}
...
const ii = ["maya", "blender", "simple"];
function kt(e) {
  const t = e.state?.navigation_profile;
  return ii.includes(t) ? t : "maya";
}
function si(e, t) {
...
function li(e, t) {
  ...
  const n = t.find((i) => i.id === o);
  return !!(n && n.enabled !== !1);
}
function lt(e) {
  const t = Array.isArray(e) ? e.filter((r) => r?.camera?.position) : [];
  if (!t.length) return [0, 0, 0];
  const a = t.reduce((r, o) => x(r, o.camera.position), [0, 0, 0]);
  return D(a, 1 / t.length);
}
function qe(e, { mode: t, origin: a, delta: r, factors: o, rotationDeg: n }) {
  if (t === "translate") return x(e, r);
  const i = j(e, a);
  return t === "scale" ? x(a, [i[0] * o[0], i[1] * o[1], i[2] * o[2]]) : x(a, ke(i, n));
}
function ci(e, t) {
...
function di(e, t) {
  ...
  const r = a ? W(a) : 0;
  return r > 1e-6 ? D(a, 1 / r) : [0, 0, -1];
}
function ll(e, t, a) {
  const r = Array.isArray(e) ? e : [], o = t instanceof Set ? t : new Set(t || []), n = !!a?.lookAtActive, i = r.map((s) => {
    const l = s?.camera?.position;
    return Array.isArray(l) ? o.has(s.frame) ? qe(l, a) : [...l] : l;
  });
  return r.map((s, l) => {
    const d = { ...s.camera };
    if (!o.has(s.frame) || !Array.isArray(d.position)) return { ...s, camera: d };
    const c = d.position, u = d.target;
    if (d.position = i[l], Array.isArray(u))
      if (n)
        d.target = qe(u, a);
      else {
        const p = W(j(u, c)) || 5, h = di(i, l);
        d.target = x(d.position, D(h, p));
      }
    return { ...s, camera: d };
  });
}
function cl(e, t, { delta: a = [0, 0, 0] } = {}) {
  const r = Array.isArray(e) ? e : [], o = t instanceof Set ? t : new Set(t || []);
  return r.map((n) => !o.has(n.frame) || !Array.isArray(n?.camera?.target) ? n : { ...n, camera: { ...n.camera, target: x(n.camera.target, a) } });
}
function mi(e, t) {
...
function ui(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function pi(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  const n = e.gizmoDrag, i = e.state.cameras.find((d) => d.id === n.trackId);
  if (!i) return;
  const s = n.origin;
  let l;
  if (e.state.gizmo_mode === "translate") {
    let d;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, u = (t[1] - n.pointer[1]) * r;
      d = x(D(n.viewRight, c * n.freeScale), D(n.viewUp, -u * n.freeScale));
    } else
      d = D(n.axis, a * n.worldLength / n.screenLength);
    l = { mode: "translate", delta: d };
  } else if (e.state.gizmo_mode === "scale") {
    let d;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, u = (t[1] - n.pointer[1]) * r, p = Math.max(0.01, 1 + (c - u) * n.freeScale * 0.35);
      d = [p, p, p];
    } else {
      const c = Math.max(0.01, 1 + a * n.worldLength / n.screenLength * 0.5);
      d = [1, 1, 1], d[n.axisIndex] = o ? Math.max(0.01, Math.round(c / 0.1) * 0.1) : c;
    }
    l = { mode: "scale", origin: s, factors: d };
  } else {
    const d = o ? Math.round(a * 0.75 / 15) * 15 : a * 0.75, c = [0, 0, 0];
    c[n.axisIndex] = d, l = { mode: "rotate", origin: s, rotationDeg: c };
  }
  i.keyframes = ci(n.baseKeys, l), i.id === e.state.active_camera_id && (e.state.keyframes = i.keyframes), e.camera = de(i, e.frame, e.state.objects), i.camera = q(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
...
const hi = [
  ...
  },
  {
    ...
    label: "Look At",
    color: k.typeLookAt,
    changed: (e, t) => fi(e?.target, t?.target),
    read: (e) => e?.target
  },
  {
    id: "focal_length",
...
function gi(e, t) {
  const a = [...e || []].sort((n, i) => n.frame - i.frame), r = [];
  let o = null;
  for (const n of a) {
    const i = n.camera || n.transform || {};
    (o === null || t.changed(o, i)) && r.push(n.frame), o = i;
  }
  return r;
}
function ml(e, t = null) {
  return hi.filter((a) => !t || t.has(a.id)).map((a) => ({
    id: a.id,
    label: a.label,
    color: a.color,
    frames: gi(e, a)
...
function yi(e, t) {
  ...
  const r = St(e.state.objects, t, e.frame || 0), o = (t.type === "model" || t.type === "glb") && e.webgl?.getObjectWorldCenter?.(t.id) || r.position, n = r.size.map((s) => Math.max(0.01, Math.abs(s)) / 2), i = r.quaternion || Fe(r.rotation);
  return ta(n.map((s) => -s), n).map((s) => x(o, wa(s, i)));
}
function bi(e, t, a, r = {}) {
  const o = e.state.objects.filter((y) => y.enabled !== !1), n = r.all ? o : o.filter((y) => e.selectedObjectIds?.has(y.id)), s = (n.length ? n : [a]).flatMap((y) => yi(e, y)), l = [0, 1, 2].map((y) => Math.min(...s.map((C) => C[y]))), d = [0, 1, 2].map((y) => Math.max(...s.map((C) => C[y]))), c = l.map((y, C) => (y + d[C]) / 2), { right: u, up: p, forward: h } = H(t), m = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), w = f * m;
  let b = 2, S = 0.1;
  for (const y of s) {
    const C = j(y, c), _ = Math.abs(Z(C, u)), A = Math.abs(Z(C, p)), g = Z(C, h);
    b = Math.max(b, 1.15 * _ / w - g, 1.15 * A / f - g, (t.near || 0.01) * 2 - g), S = Math.max(S, 1.15 * A, 1.15 * _ / m);
  }
  t.target = c, t.position = j(c, D(h, b)), t.camera_type === "orthographic" && (t.zoom = Math.max(0.01, 5 / S));
}
const te = {
...
function ul(e, t) {
  if (["camera", "perspective", "iso", "front", "back", "top", "right", "left", "bottom"].includes(t)) {
    ...
    e.serialize(), e.render(), e.setStatus(v("View: {value1}{value2}", { value1: t[0].toUpperCase(), value2: t.slice(1) }));
  }
}
function pl(e, t) {
  if (["translate", "rotate", "scale"].includes(t)) {
    ...
    e.serialize(), e.render(), e.setStatus(v("{value1}{value2} · {value3}", { value1: t[0].toUpperCase(), value2: t.slice(1), value3: t === "translate" ? "W" : t === "rotate" ? "E" : "R" }));
  }
}
function fl(e, t) {
...
function hl(e, t = {}) {
  ...
  if (e.subSelection?.point && !t.all) {
    ...
    const s = e.subSelection.point, l = se(j(a.position, o)), d = Number.isFinite(l[0]) && W(l) > 0.1 ? l : [0.707, 0.4, 0.707], c = 2;
    a.target = [...s], a.position = x(a.target, D(d, c)), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const u = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(v("Focused on {value1} at [{value2}]", { value1: u, value2: s.map((p) => Math.round(p * 100) / 100).join(", ") }));
    return;
  }
  const i = e.selectedObject() || e.state.objects.find((s) => s.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] };
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), bi(e, a, i, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? v("Framed: all objects") : v("Framed: {name}").replace("{name}", i.name || i.type || v("Subject")));
}
function _i(e, t, a, { forceLocal: r = !1 } = {}) {
  const o = [[1, 0, 0], [0, 1, 0], [0, 0, 1]], n = a?.rotation || t?.rotation || [0, 0, 0];
  return r || e.state.gizmo_space === "local" ? o.map((i) => ke(i, n)) : o;
}
function Si(e) {
  return vi(e);
}
...
function Wr(e) {
  ...
  const t = Si(e);
  if (!t || e.transformControlsWiring && wi.has(t.type)) return null;
  ...
  const n = Math.max(0.7, W(j(a.position, r)) * 0.12), i = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", s = t.type === "object" ? _i(e, t.object, t, { forceLocal: i }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: s.map((d, c) => ({ index: c, axis: d, points: [o, V(x(r, D(d, n)), a, e.canvas.width, e.canvas.height)] })).filter((d) => d.points[1] && Number.isFinite(d.points[1][0]) && Number.isFinite(d.points[1][1]))
    };
  const l = s.map((d, c) => {
    const u = Math.abs(d[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = se(xe(d, u)), h = se(xe(d, p)), m = [];
    for (let f = 0; f <= 48; f++) {
      const w = f / 48 * Math.PI * 2, b = V(x(r, x(D(p, Math.cos(w) * n), D(h, Math.sin(w) * n))), a, e.canvas.width, e.canvas.height);
      b && Number.isFinite(b[0]) && Number.isFinite(b[1]) && m.push(b);
    }
    return { index: c, axis: d, points: m };
  });
...
function $r(e, t) {
  ...
  let i = null;
  for (const s of a.handles)
    for (let l = 0; l < s.points.length - 1; l++) {
      const d = s.points[l], c = s.points[l + 1], u = Sa(t, d, c);
      (!i || u < i.distance) && (i = { ...s, distance: u, segment: [d, c], worldLength: a.worldLength, entity: a.entity });
    }
  return i?.distance <= 18 * r ? i : null;
}
function Ci(e, t) {
  const a = e.webgl?.pick?.(t[0], t[1], e.canvas.width, e.canvas.height);
  if (a) {
    if (typeof a == "string") {
      const s = e.state.objects.find((l) => l.id === a);
      return s ? { type: "object", object: s } : null;
    }
    if (a.type === "camera" || a.type === "camera_target") {
      const s = e.state.cameras.find((l) => l.id === a.id);
      return s ? { type: a.type, camera: s } : null;
    }
    const i = e.state.objects.find((s) => s.id === a.id);
  ...
  if (e.state.view_mode !== "camera") {
    for (const i of e.state.cameras) {
      for (const c of i.keyframes || []) {
        const u = c.camera?.position;
        if (!u) continue;
        const p = V(u, r, e.canvas.width, e.canvas.height);
        if (p && Math.hypot(t[0] - p[0], t[1] - p[1]) <= 16 * Math.min(2, window.devicePixelRatio || 1))
          return { type: "camera_keyframe", camera: i, keyframe: c };
      }
      const s = de(i, e.frame, e.state.objects), l = V(s.target || [0, 1.5, 0], r, e.canvas.width, e.canvas.height);
      if (l && Math.hypot(t[0] - l[0], t[1] - l[1]) <= 18 * Math.min(2, window.devicePixelRatio || 1))
        return { type: "camera_target", camera: i };
      const d = V(s.position || [6, 4, 6], r, e.canvas.width, e.canvas.height);
      if (d && Math.hypot(t[0] - d[0], t[1] - d[1]) <= 22 * Math.min(2, window.devicePixelRatio || 1))
    ...
    if (e.state.show_camera_paths !== !1)
      for (const i of e.state.cameras) {
        ...
        const l = s.map((d) => V(d.camera?.position, r, e.canvas.width, e.canvas.height)).filter((d) => d && Number.isFinite(d[0]) && Number.isFinite(d[1]));
        for (let d = 0; d < l.length - 1; d += 1)
          if (Sa(t, l[d], l[d + 1]) <= 8 * Math.min(2, window.devicePixelRatio || 1))
            return { type: "camera_path", camera: i };
      }
    for (const i of e.state.objects)
      if (i.enabled !== !1)
        for (const s of i.keyframes || []) {
          const l = s.transform?.position;
          if (!l) continue;
          const d = V(l, r, e.canvas.width, e.canvas.height);
          if (d && Math.hypot(t[0] - d[0], t[1] - d[1]) <= 16 * Math.min(2, window.devicePixelRatio || 1))
            return { type: "object_keyframe", object: i, keyframe: s };
        }
  }
  let n = null;
  for (const i of e.state.objects) {
    if (i.enabled === !1) continue;
    const s = i.keyframes?.length ? Ue(i, e.frame) : i, l = V(s.position || [0, 0, 0], r, e.canvas.width, e.canvas.height);
    if (!l) continue;
    const d = Math.hypot(t[0] - l[0], t[1] - l[1]);
    (!n || d < n.distance) && (n = { object: i, distance: d });
  }
  return n?.distance <= 22 * Math.min(2, window.devicePixelRatio || 1) ? { type: "object", object: n.object } : null;
}
function xi(e, t, a, r = 15) {
  const o = a[0] - t[0], n = a[1] - t[1], i = Math.hypot(o, n) || 1, s = o / i, l = n / i, d = -l, c = s, u = r * 0.42, p = a[0] - s * r, h = a[1] - l * r;
  e.beginPath(), e.moveTo(a[0], a[1]), e.lineTo(p + d * u, h + c * u), e.lineTo(p - d * u, h - c * u), e.closePath(), e.fill(), e.save(), e.strokeStyle = "rgba(15, 23, 42, 0.65)", e.lineWidth = 1, e.stroke(), e.restore();
}
function gl(e) {
  const t = Wr(e);
  if (!t || !t.handles) return;
  if (t.entity?.type === "camera_path") {
    const o = Q(e), n = (t.entity.track?.keyframes || []).map((i) => V(i.camera?.position, o, e.canvas.width, e.canvas.height)).filter((i) => i && Number.isFinite(i[0]) && Number.isFinite(i[1]));
    n.length >= 2 && (e.ctx.save(), e.ctx.strokeStyle = "rgba(139, 125, 227, 0.9)", e.ctx.lineWidth = 2, e.ctx.setLineDash([6, 4]), e.ctx.beginPath(), n.forEach((i, s) => s ? e.ctx.lineTo(i[0], i[1]) : e.ctx.moveTo(i[0], i[1])), e.ctx.stroke(), e.ctx.restore());
  }
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((i, s) => {
      i && (s ? e.ctx.lineTo(i[0], i[1]) : e.ctx.moveTo(i[0], i[1]));
    }), e.ctx.stroke(), e.state.gizmo_mode !== "rotate" || t.entity?.type === "camera_target") {
      const i = o.points.filter((l) => l && Number.isFinite(l[0]) && Number.isFinite(l[1]));
      if (!i.length) continue;
      const s = i[i.length - 1];
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(s[0] - 6, s[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(s[0] - 6, s[1] - 6, 12, 12), e.ctx.restore()) : xi(e.ctx, i[0], s);
    }
  }
...
function Mi(e, t) {
  ...
  const o = a.map((c) => ({ object: c, transform: me(c) })), n = o.reduce((c, u) => x(c, u.transform.position), [0, 0, 0]).map((c) => c / o.length), i = [e.canvas.width * 0.5, e.canvas.height * 0.5], s = e.lastViewportPointer || i, l = e.interactionElement.getBoundingClientRect(), d = e.lastPointerEvent || { clientX: l.left + s[0] * l.width / e.canvas.width, clientY: l.top + s[1] * l.height / e.canvas.height };
  return e.modalTransform = { mode: t, axis: null, numeric: "", start: s, lastEvent: d, snapshots: o, pivot: n }, e.setTransformMode(t), e.setStatus(`${t.toUpperCase()} · move mouse · X/Y/Z constrain · type value · Enter confirm · Esc cancel`), e.render(), !0;
}
function ki(e) {
...
function Ai(e, t, a, r, o, n, i) {
  const s = o ? "grid" : e.state.spatial_snap_mode;
  if (s === "grid") {
    const l = e.state.spatial_grid_size || 0.5;
    return n ? a.map((d, c) => d.map((u, p) => Math.abs(n[p]) > 1e-6 ? He(u, l) : i[c][p])) : a.map((d) => d.map((c) => He(c, l)));
  }
  if (s === "vertex" && r && !n) {
    const l = e.webgl?.pickSubElement?.(r[0], r[1], e.canvas.width, e.canvas.height, "vertex");
    if (l?.point && !t.snapshots.some((d) => d.object.id === l.objectId)) {
      const d = a.reduce((u, p) => x(u, p), [0, 0, 0]).map((u) => u / a.length), c = j(l.point, d);
      return a.map((u) => x(u, c));
    }
  }
  return a;
}
function Oe(e, t) {
  ...
  const n = o[0] - a.start[0], i = o[1] - a.start[1], s = t.shiftKey ? 0.1 : 1, l = ki(a), d = a.axis ? ct[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], u = H(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...j(c.position, c.target)) * 25e-4;
  let h = a.snapshots.map((y) => [...y.transform.position]);
  if (a.mode === "translate") {
    const y = l ?? (n - i) * p * s, C = d ? D(d, y) : x(D(u.right, n * p * s), D(u.up, -i * p * s));
    h = h.map((_) => x(_, C)), h = Ai(e, a, h, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((_) => _.transform.position));
  }
  const m = a.mode === "rotate" ? l ?? (n - i) * 0.5 * s : 0, f = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - i) * 0.01 * s) : 1, w = d || ct.z, b = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
  a.snapshots.forEach((y, C) => {
    const _ = y.object;
    if (a.mode === "translate" && (_.position = h[C]), a.mode === "rotate") {
      const A = b ? He(m, 15) : m, g = D(w, A);
      _.position = x(a.pivot, ke(j(y.transform.position, a.pivot), g)), _.rotation = x(y.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const A = b ? He(f, 0.1) : f, g = d ? d.map((E) => E ? A : 1) : [A, A, A], M = j(y.transform.position, a.pivot);
      _.position = x(a.pivot, M.map((E, G) => E * g[G])), _.size = y.transform.size.map((E, G) => Math.max(0.01, E * g[G]));
    }
    e.commitObjectEdit(_);
...
const ne = (e, t) => Math.round(e / t) * t, ji = (e, t) => e.map((a) => ne(a, t));
function Ei(e, t, a) {
  ...
  if (n)
    ...
    i = p.map((h, m) => o[m] - h), s = p.map((h, m) => o[m] + h);
  }
  let l = 1 / 0, d = 1 / 0, c = -1 / 0, u = -1 / 0;
  for (const p of [i[0], s[0]]) for (const h of [i[1], s[1]]) for (const m of [i[2], s[2]]) {
    const f = V([p, h, m], a, e.canvas.width, e.canvas.height);
    f && (l = Math.min(l, f[0]), c = Math.max(c, f[0]), d = Math.min(d, f[1]), u = Math.max(u, f[1]));
  }
  return Number.isFinite(l) ? { minX: l, minY: d, maxX: c, maxY: u } : null;
}
function ye(e, t, a, r = [], o = null) {
  const i = e.currentTransformEvent?.ctrlKey || e.currentTransformEvent?.metaKey ? "grid" : e.state.spatial_snap_mode, s = e.state.spatial_grid_size || 0.5;
  if (i === "grid")
    return o ? t.map((l, d) => Math.abs(o.axis[d]) > 1e-6 ? ne(l, s) : o.base[d]) : ji(t, s);
  if (i === "vertex" && a && !o) {
...
function yl(e, t) {
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = Q(e), i = e.state.view_mode !== "camera", s = Jt(e, t), l = t.button === 0 && !s, d = l && !t.altKey && !t.shiftKey, c = e.transformControlsWiring?.isPointerOverHandle?.();
  !d && c && t.stopImmediatePropagation?.();
  const u = e.transformControlsWiring?.currentLiveType?.();
  if (d && (!c || u === "path_point" || u === "camera") && oi(e, { pointerX: r, pointerY: o, overHandle: c, viewCamera: n, e: t }) || d && c || l && ti(e, { pointerX: r, pointerY: o, shiftKey: t.shiftKey, altKey: t.altKey })) return;
  const h = d ? $r(e, [r, o]) : null;
  if (h) {
    const [y, C] = h.segment, _ = Math.max(1, Math.hypot(C[0] - y[0], C[1] - y[1])), A = {
      pointer: [r, o],
      axis: h.axis,
      axisIndex: h.index,
      screen: [(C[0] - y[0]) / _, (C[1] - y[1]) / _],
    ...
    if (e.interactionElement.style && (e.interactionElement.style.cursor = "grabbing"), h.entity.type === "camera_target") {
      ...
      const g = e.activeCameraTrack?.(), M = !!g?.target_object_id;
      e.gizmoDrag = {
        ...A,
        type: "camera_target",
        historyCheckpointed: !0,
        tracking: M,
        target: M ? [...g.target_offset || [0, 0, 0]] : [...h.entity.position || e.camera.target]
      };
      return;
    }
    ...
    if (h.entity.type === "camera_path" && ui(e, { baseDrag: A, viewCamera: n, entityPosition: h.entity.position })) return;
    if (h.entity.type === "object") {
      const g = h.entity.object;
      e.checkpoint("Transform object");
      const M = Ur(e), E = (M.length ? M : [g]).map((F) => ({ object: F, transform: me(F) }));
      for (const F of E) e.beginObjectEdit(F.object);
      const G = E.reduce((F, X) => x(F, X.transform.position), [0, 0, 0]).map((F) => F / E.length);
      e.gizmoDrag = {
        ...A,
        type: "object",
        historyCheckpointed: !0,
  ...
  const m = l ? Ci(e, [r, o]) : null;
  if (e.pointerHit = !!(h || m), m) {
    if (m.type === "camera_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.editingKeyFrame = null, e.activateCamera(m.camera.id), e.setFrame(m.keyframe.frame), e.selectKeyframe(m.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v("{value1} · Keyframe @ F{value2} selected", { value1: m.camera.name, value2: m.keyframe.frame }));
      return;
    }
    if (m.type === "object_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectId = m.object.id, e.editingKeyFrame = null, e.setFrame(m.keyframe.frame), e.selectKeyframe(m.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v("{value1} · Keyframe @ F{value2} selected", { value1: m.object.name || m.object.type, value2: m.keyframe.frame }));
      return;
    }
    if (m.type === "camera_target") {
      if (e.finishCameraEdit(), e.selectedEntity = "camera_target", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(m.camera.id), m.camera.locked) {
        e.setStatus(v("{name} is locked").replace("{name}", m.camera.name)), e.refreshObjects(), e.refreshInspector(), e.render();
        return;
      }
      e.checkpoint("Move camera target"), e.beginCameraEdit();
      const { right: y, up: C } = H(n), _ = [...e.camera.target], A = e.activeCameraTrack?.(), g = !!A?.target_object_id;
      e.targetFreeDrag = {
        pointer: [r, o],
        target: g ? [...A.target_offset || [0, 0, 0]] : _,
        tracking: g,
        right: y,
        up: C,
        ...
        historyCheckpointed: !0
      }, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v("{value1} · Target aim selected", { value1: m.camera.name }));
      return;
    }
    if (m.type === "camera") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(m.camera.id), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v("{value1} selected", { value1: m.camera.name }));
      return;
    }
    ...
    if (m.type === "object" && m.object) {
      if (e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectIds ||= /* @__PURE__ */ new Set(), t.shiftKey || t.ctrlKey || t.metaKey ? e.selectedObjectIds.has(m.object.id) ? e.selectedObjectIds.delete(m.object.id) : e.selectedObjectIds.add(m.object.id) : e.selectedObjectIds = /* @__PURE__ */ new Set([m.object.id]), e.selectedObjectId = e.selectedObjectIds.has(m.object.id) ? m.object.id : [...e.selectedObjectIds].at(-1) || null, e.selectedKeyFrame = m.object.keyframes?.find((y) => y.frame === e.frame)?.frame ?? null, e.editingKeyFrame = null, e.state.select_mode && e.state.select_mode !== "object") {
        const y = e.webgl?.pickSubElement?.(r, o, e.canvas.width, e.canvas.height, e.state.select_mode);
        if (y) {
          e.subSelection = y;
          const C = y.point.map((A) => Math.round(A * 100) / 100).join(", "), _ = y.mode === "vertex" ? "Vertex" : y.mode === "edge" ? "Edge" : "Face";
          e.setStatus(v("{value1} selected at [{value2}] · Press F to focus", { value1: _, value2: C }));
        } else
          e.subSelection = null;
      } else
        e.subSelection = null, e.setStatus(v("{value1} selected", { value1: m.object.name || m.object.type }));
      e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render();
      return;
    }
  }
  if (!m && l && !t.ctrlKey && !t.metaKey && kt(e) !== "simple") {
    e.boxSelection = {
      start: [r, o],
      current: [r, o],
      additive: t.shiftKey,
  ...
  const f = !!e.isNavigatingFly, w = ni(e, t, n);
  if (!f && !w) return;
  if (!i && e.state.camera_lock) {
    e.setStatus?.(v("Camera View is locked (click 🔒 to unlock)"));
    return;
  }
  const b = !f && w === "pan", S = !f && w === "dolly";
  i && !e.state.editor_views && (e.state.editor_views = Le()), e.drag = {
    x: t.clientX,
    y: t.clientY,
    shift: b,
    dolly: S,
    fly: f,
    camera: q(n),
    target: i ? e.state.editor_views[e.state.view_mode] || (e.state.editor_views[e.state.view_mode] = Le()[e.state.view_mode]) : e.camera,
    editorView: i,
    navigationOnly: s,
    historyCheckpointed: !1
  }, e.interactionElement.style && (e.interactionElement.style.cursor = S ? "ns-resize" : b ? "move" : "grabbing"), e.setStatus?.(v(f ? "Fly" : S ? "Dolly" : b ? "Pan" : "Orbit"));
}
function bl(e, t) {
  ...
  if (e.pathDrag) {
    const i = e.interactionElement.getBoundingClientRect(), s = (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width), l = (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height);
    if (!e.pathDrag.moved && Math.hypot(s - e.pathDrag.startX, l - e.pathDrag.startY) < 3) return;
    e.pathDrag.moved = !0, ge(e, e.pathDrag, "Move path key");
    const c = ((e.state.cameras || []).find((u) => u.id === e.pathDrag.cameraId)?.keyframes || []).find((u) => u.frame === e.pathDrag.frame);
    c && (c.camera.position = Zt(
      [s, l],
      Q(e),
      e.pathDrag.anchor,
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = ei(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  if (e.curveHandleDrag) {
    const i = e.interactionElement.getBoundingClientRect(), s = (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width), l = (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height);
    if (!e.curveHandleDrag.moved && Math.hypot(s - e.curveHandleDrag.startX, l - e.curveHandleDrag.startY) < 3) return;
    e.curveHandleDrag.moved = !0, ge(e, e.curveHandleDrag, "Edit curve handle");
    const c = ((e.state.cameras || []).find((u) => u.id === e.curveHandleDrag.cameraId)?.keyframes || []).find((u) => u.frame === e.curveHandleDrag.frame);
    if (c) {
      const u = Zt(
        [s, l],
        Q(e),
      ...
      e.dragCurveHandle?.(c, e.curveHandleDrag.side, u, {
  ...
  if (e.boxSelection) {
    const i = e.interactionElement.getBoundingClientRect();
    e.boxSelection.current = [
      (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width),
      (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height)
    ], e.render();
    return;
  }
  ...
  if (e.targetFreeDrag) {
    ...
    const i = e.interactionElement.getBoundingClientRect(), s = (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width), l = (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height), d = s - e.targetFreeDrag.pointer[0], c = l - e.targetFreeDrag.pointer[1], u = t.shiftKey ? 0.1 : 1, p = x(D(e.targetFreeDrag.right, d * e.targetFreeDrag.scale * u), D(e.targetFreeDrag.up, -c * e.targetFreeDrag.scale * u)), h = ye(e, x(e.targetFreeDrag.target, p), [s, l]);
  ...
  if (e.gizmoDrag) {
    ...
    const i = e.interactionElement.getBoundingClientRect(), s = [
      (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width),
      (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height)
    ], l = t.shiftKey ? 0.1 : 1, d = ((s[0] - e.gizmoDrag.pointer[0]) * e.gizmoDrag.screen[0] + (s[1] - e.gizmoDrag.pointer[1]) * e.gizmoDrag.screen[1]) * l, c = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
    if (e.gizmoDrag.type === "camera_target") {
      const h = x(e.gizmoDrag.target, D(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), m = ye(e, h, s, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
      e.gizmoDrag.tracking ? aa(e, m) : e.camera.target = m, e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        const h = x(e.gizmoDrag.position, D(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.camera.position = ye(e, h, s, [], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      } else {
        const h = c ? ne(d * 0.015, Math.PI / 12) : d * 0.015, m = j(e.gizmoDrag.target, e.gizmoDrag.position), f = ke(m, D(e.gizmoDrag.axis, h * (180 / Math.PI)));
        e.camera.target = x(e.gizmoDrag.position, f);
      }
      e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera_path") return void pi(e, { pointer: s, deltaPixels: d, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
        const h = (s[0] - e.gizmoDrag.pointer[0]) * l, m = (s[1] - e.gizmoDrag.pointer[1]) * l, f = x(
          e.gizmoDrag.position,
          x(D(e.gizmoDrag.viewRight, h * e.gizmoDrag.freeScale), D(e.gizmoDrag.viewUp, -m * e.gizmoDrag.freeScale))
        );
        e.gizmoDrag.object.position = ye(e, f, s, [e.gizmoDrag.object.id]);
      } else {
        const h = x(e.gizmoDrag.position, D(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.gizmoDrag.object.position = ye(e, h, s, [e.gizmoDrag.object.id], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      }
    else if (e.state.gizmo_mode === "scale")
      if (e.gizmoDrag.free) {
        const h = (s[0] - e.gizmoDrag.pointer[0]) * l, m = (s[1] - e.gizmoDrag.pointer[1]) * l, f = (h - m) * e.gizmoDrag.freeScale, w = e.gizmoDrag.size.map((b) => {
          const S = b + f;
          return Math.max(0.01, c ? ne(S, 0.1) : S);
        });
        e.gizmoDrag.object.size = w;
      } else {
        const h = [...e.gizmoDrag.size], m = h[e.gizmoDrag.axisIndex] + d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength;
        h[e.gizmoDrag.axisIndex] = Math.max(0.01, c ? ne(m, 0.1) : m), e.gizmoDrag.object.size = h;
      }
    else {
      const h = [...e.gizmoDrag.rotation], m = h[e.gizmoDrag.axisIndex] + d * 0.75;
      h[e.gizmoDrag.axisIndex] = c ? ne(m, 15) : m, e.gizmoDrag.object.rotation = h;
    }
    const u = e.gizmoDrag.group || [], p = u.find((h) => h.object === e.gizmoDrag.object)?.transform;
    if (u.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        const h = j(e.gizmoDrag.object.position, p.position);
        for (const m of u) m.object.position = x(m.transform.position, h);
      } else if (e.state.gizmo_mode === "rotate") {
        const h = j(e.gizmoDrag.object.rotation, p.rotation);
        for (const m of u)
          m.object.position = x(e.gizmoDrag.groupPivot, ke(j(m.transform.position, e.gizmoDrag.groupPivot), h)), m.object.rotation = x(m.transform.rotation, h);
      } else {
        const h = e.gizmoDrag.object.size.map((m, f) => m / Math.max(0.01, p.size[f]));
        for (const m of u) {
          const f = j(m.transform.position, e.gizmoDrag.groupPivot);
          m.object.position = x(e.gizmoDrag.groupPivot, f.map((w, b) => w * h[b])), m.object.size = m.transform.size.map((w, b) => Math.max(0.01, w * h[b]));
        }
      }
    for (const h of u.length ? u : [{ object: e.gizmoDrag.object }]) e.commitObjectEdit(h.object);
    e.refreshInspector(), e.render();
    return;
  }
  if (!e.drag) {
    const i = e.interactionElement.getBoundingClientRect(), s = $r(e, [
      (t.clientX - i.left) * e.canvas.width / Math.max(1, i.width),
      (t.clientY - i.top) * e.canvas.height / Math.max(1, i.height)
    ]), l = s ? s.free ? "free" : s.index : null;
    l !== e.hoveredGizmoHandle && (e.hoveredGizmoHandle = l, e.interactionElement.style && (e.interactionElement.style.cursor = s ? "grab" : "default"), e.render());
    return;
  }
  ...
  if (e.drag.dolly) {
    const i = Math.exp(r * 5e-3 * (e.dollySensitivity ?? 1)), s = j(n.position, n.target);
    e.drag.target.position = x(n.target, D(s, i)), e.drag.target.camera_type === "orthographic" && (e.drag.target.zoom = Math.max(0.01, (n.zoom || 1) / i));
  } else if (e.drag.fly) {
    const i = j(n.target, n.position), s = W(i);
    let l = Math.atan2(i[0], i[2]), d = Math.asin(T(i[1] / s, -0.999, 0.999));
    l -= a * 8e-3, d = T(d - r * 8e-3, -1.45, 1.45), e.drag.target.target = [
      n.position[0] + s * Math.sin(l) * Math.cos(d),
      n.position[1] + s * Math.sin(d),
      n.position[2] + s * Math.cos(l) * Math.cos(d)
    ];
  } else if (e.drag.shift) {
    const { right: i, up: s } = H(n), l = Ge(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), d = x(D(i, -a * l), D(s, r * l));
    e.drag.target.position = x(n.position, d), e.drag.target.target = x(n.target, d);
  } else {
    const i = j(n.position, n.target), s = W(i);
    let l = Math.atan2(i[0], i[2]), d = Math.asin(T(i[1] / s, -0.999, 0.999));
    l -= a * 8e-3, d = T(d + r * 8e-3, -1.45, 1.45), e.drag.target.position = [
      n.target[0] + s * Math.sin(l) * Math.cos(d),
      n.target[1] + s * Math.sin(d),
      n.target[2] + s * Math.cos(l) * Math.cos(d)
    ];
  }
...
function Zr(e) {
  ...
  return e.keyDrag?.badge?.remove?.(), e.boxSelect?.overlay?.remove?.(), e.drag?.camera && e.drag?.target && (Object.assign(e.drag.target, e.drag.camera), e.drag.editorView && e.scheduleSerialize()), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.boxSelection = null, e.pathDrag = null, e.keyDrag = null, e.curveDrag = null, e.timelineDrag = null, e.timelinePanDrag = null, e.boxSelect = null, e.curvePanDrag = null, e.curveScrub = null, e.curveBoxSelect = null, we(e), t && e.undo(), e.finishCameraEdit(), e.refreshInspector(), e.render(), e.setStatus(v("Interaction cancelled")), !0;
}
function vl(e, t) {
  ...
  if (e.pathDrag) {
    const i = e.pathDrag.moved;
    e.pathDrag = null, we(e), i && (e.scheduleSerialize(), e.refreshKeys(), e.setStatus(v("Path key moved")));
    return;
  }
  if (e.curveHandleDrag) {
    const i = e.curveHandleDrag.moved;
    e.curveHandleDrag = null, we(e), i && (e.webgl && (e.webgl.pathKey = ""), e.scheduleSerialize(), e.refreshKeys(), e.setStatus(v("Curve handle updated")));
    return;
  }
  if (e.boxSelection) {
    const i = e.boxSelection, s = Q(e), l = Math.min(i.start[0], i.current[0]), d = Math.max(i.start[0], i.current[0]), c = Math.min(i.start[1], i.current[1]), u = Math.max(i.start[1], i.current[1]), p = i.additive ? new Set(i.initial) : /* @__PURE__ */ new Set();
    for (const h of e.state.objects) {
      if (h.enabled === !1) continue;
      const m = Ei(e, h, s);
      m && m.maxX >= l && m.minX <= d && m.maxY >= c && m.minY <= u && p.add(h.id);
    }
    e.selectedObjectIds = p, e.selectedObjectId = [...p].at(-1) || null, e.selectedEntity = p.size ? "object" : "camera", e.boxSelection = null, we(e), e.refreshObjects(), e.refreshInspector(), e.render(), e.setStatus(v("{count} object(s) selected").replace("{count}", String(p.size)));
    return;
  }
  const a = e.keyDrag, r = !!(e.drag && !e.drag.editorView || e.targetFreeDrag), o = !!e.gizmoDrag;
  e.gizmoDrag?.type === "camera_path" && (e.serialize?.(), e.refreshKeys?.(), e.setStatus(v("Camera path transformed"))), !e.pointerHit && !e.gizmoDrag && !e.targetFreeDrag && e.drag && !e.drag.navigationOnly && t && Math.hypot(t.clientX - e.drag.x, t.clientY - e.drag.y) < 5 && (t.button === 0 || t.button === void 0) && (e.selectedEntity === "object" || e.selectedObjectId !== null || e.selectedEntity === "camera_target" || e.selectedEntity === "camera_path") && (e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = null, e.subSelection = null, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v("Deselected"))), we(e), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.keyDrag = null, e.pointerHit = !1, e.canvas.classList.remove("dragging"), e.interactionElement.style && (e.interactionElement.style.cursor = "default"), a && (a.badge?.remove(), e.editingKeyFrame = null, e.updateKeyVisualState(), e.root.focus({ preventScroll: !0 })), r && e.finishCameraEdit(), o && (e.editingKeyFrame = null, e.updateKeyVisualState(), e.drawCurveEditor());
}
function _l(e, t) {
  ...
  const a = si(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
  if (e.isNavigatingFly) {
    e.cameraSpeed = T(e.cameraSpeed * Math.exp(-a * 1e-3), 0.05, 20), e.setStatus(v("Fly speed: {value1}x", { value1: e.cameraSpeed.toFixed(2) }));
    return;
  }
  Ti(e);
  ...
  const o = Q(e);
  r || e.beginCameraEdit();
  const n = T(a * 1e-3, -0.4, 0.4), i = j(o.position, o.target);
  o.position = x(o.target, D(i, Math.exp(n))), o.camera_type === "orthographic" && (o.zoom = Math.max(0.01, (o.zoom || 1) * Math.exp(-n))), r ? (e.scheduleSerialize(), e.render()) : (e.commitCameraEdit(), e.finishCameraEdit());
}
const Ii = (e) => {
...
function We(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Ii(e), i = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((s) => ({ visible: !0, interpolation: "linear", ...s })), source: { ...o } };
...
function Ri(e, t, a, r, o) {
  ...
  if (t?.object_id) {
    ...
    if (!c) return null;
    const u = dn(e.objects, c), p = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
    i = [u.position[0] + p[0] * u.size[0], u.position[1] + p[1] * u.size[1], u.position[2] + p[2] * u.size[2]];
  }
  if (!Array.isArray(i)) return null;
  const s = V(i, n, r, o);
  if (!s) return null;
  const l = s[0] / r, d = s[1] / o;
  return { x: l, y: d, visible: l >= 0 && l <= 1 && d >= 0 && d <= 1 };
}
function zi(e, t = 6e-3) {
...
function Jr(e, t, a = 0.035) {
  ...
  for (const n of e || []) for (const i of n.keys || []) {
    const s = Math.hypot(i.x - t.x, i.y - t.y);
    s <= o && (r = n, o = s);
  }
  return r;
}
function Li(e, t, a, r) {
  ...
  return We(e, {
    ...
    keys: o.map((i, s) => ({ ...i, time_seconds: a + n * s / (o.length - 1) }))
  });
}
function at(e, t) {
  return Ni(t, e.interactionElement);
}
function Fi(e, t) {
...
function Bi(e, t) {
  const a = e.state.objects.find((d) => d.id === e.selectedObjectId), r = e.motionCreationKind, n = (r === "object" || r !== "world" && !!a) && a ? "object_point" : "world_point", i = n === "object_point" ? { object_id: a.id, local_point: [0, 0, 0] } : { point: e.webgl?.intersectScenePoint?.(t.x * e.canvas.width, t.y * e.canvas.height, e.canvas.width, e.canvas.height) || [...e.camera.target] }, s = Ri(e.state, i, e.frame, e.canvas.width, e.canvas.height) || t, l = n === "object_point" ? `${a.name || a.id} Track` : "World Anchor";
  return We(e.state, { sourceKind: n, label: l, keys: [{ time_seconds: e.frame / e.state.fps, x: s.x, y: s.y, visible: s.visible !== !1 }], source: i });
}
function Sl(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    n && (e.checkpoint("Set motion interpolation"), Oi(n, o.target.value), oe(e, `Motion interpolation: ${o.target.value}`));
  }, { signal: t }), e.root.querySelector('[data-role="motion-key-visible"]')?.addEventListener("change", (o) => {
    const n = tt(e.state);
    if (!n?.keys?.length) return;
    const i = e.frame / e.state.fps, s = n.keys.reduce((l, d) => Math.abs(d.time_seconds - i) < Math.abs(l.time_seconds - i) ? d : l);
    e.checkpoint("Set motion visibility"), s.visible = o.target.checked, oe(e, `Motion key ${s.visible ? "visible" : "hidden"}`);
  }, { signal: t });
  for (const o of e.root.querySelectorAll("[data-motion-layer-action]"))
    o.addEventListener("click", () => {
      const n = tt(e.state);
      if (!n) return;
      const i = o.dataset.motionLayerAction;
      if (e.checkpoint(i === "delete" ? "Delete motion layer" : i === "retime" ? "Retime motion layer" : "Toggle motion layer"), i === "delete") Qr(e.state, n.id);
      else if (i === "retime") {
        const [s, l] = ra(e);
        Pi(n, s, l);
      } else n.enabled = !n.enabled;
      oe(e, i === "delete" ? "Motion layer deleted" : i === "retime" ? "Motion layer retimed" : `Motion layer ${n.enabled ? "enabled" : "disabled"}`);
    }, { signal: t });
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    const i = at(e, o);
    if (n === "track") {
      e.checkpoint("Draw motion track"), e.motionTrackDraft = { pointerId: o.pointerId, points: [i] };
      return;
    }
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? We(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...i, interpolation: "hold" }] }) : n === "project" ? Bi(e, i) : n === "erase" && Fi(e.state, i), oe(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [i, s] = ra(e), l = Li(e.state, n.points, i, s);
...
function wl(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => Vi(e, a.dataset.motionCreate), { signal: t });
...
function oa(e, t, a) {
  const r = e.viewportCamera(), o = e.state.view_mode !== "camera", n = j(r.position, r.target), i = W(n);
  if (!(i > 1e-4)) return;
  const s = Math.atan2(n[0], n[2]) + t, l = T(Math.asin(T(n[1] / i, -0.999, 0.999)) + a, -1.45, 1.45);
  o || e.beginCameraEdit(), r.position = x(r.target, D([
    Math.sin(s) * Math.cos(l),
    Math.sin(l),
    Math.cos(s) * Math.cos(l)
  ], i)), o ? (e.scheduleSerialize(), e.render()) : (e.commitCameraEdit(), e.finishCameraEdit());
}
const be = { t: "translate", r: "rotate", s: "scale" }, Wi = [
...
], $i = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function Ui(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.($i) : !1;
}
function Xi(e) {
...
function Zi(e, t) {
  return Yi(e) || t?.lastKeyZone || "viewport";
}
let na = !1;
function Qi() {
  na || typeof window > "u" || (na = !0, window.addEventListener("keydown", (e) => {
    ...
    !a || a.disposed || Ji(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function Ji(e, t) {
  if (!qn()) return !1;
  const a = t.composedPath?.()[0] || t.target;
  if (Xi(a) || (t.code === "Space" || t.key === "Enter") && Ui(a)) return !1;
...
function as(e, t) {
  ...
  if (e.selectedEntity === "camera_path" && !e.isNavigatingFly) {
    ...
    const s = e.state.spatial_grid_size || 0.5, l = { ArrowLeft: [-s, 0, 0], ArrowRight: [s, 0, 0], ArrowUp: [0, 0, -s], ArrowDown: [0, 0, s], PageUp: [0, s, 0], PageDown: [0, -s, 0] }[t.key];
    if (l)
  ...
  if (be[a] && !e.isNavigatingFly)
    return t.repeat || Mi(e, be[a]), !0;
  if (a === "tab") {
    const s = e.state.select_mode === "object" ? "vertex" : "object";
    return e.setSelectMode(s), e.setStatus(s === "object" ? "Object Mode" : "Component Mode: Vertex"), !0;
  }
  ...
  if (r === "Numpad9") {
    const s = Hi(e.state.view_mode);
    return s ? e.setViewMode(s) : oa(e, Math.PI, 0), !0;
  }
  const n = Math.PI / 12, i = { Numpad4: [n, 0], Numpad6: [-n, 0], Numpad8: [0, n], Numpad2: [0, -n] };
  if (i[r])
    return oa(e, i[r][0], i[r][1]), !0;
  if (r === "Numpad5")
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const s = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: d, up: c, forward: u } = H(s), p = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, h = { w: D(u, p), s: D(u, -p), d: D(d, p), a: D(d, -p), e: D(c, p), q: D(c, -p) }[a];
    return l || e.beginCameraEdit(), s.position = x(s.position, h), s.target = x(s.target, h), l ? (e.serialize(), e.render()) : (e.commitCameraEdit(), e.finishCameraEdit()), !0;
  }
  return !1;
}
function rs(e, t, a) {
...
function os(e, t) {
  ...
  if (t.key === "Delete" || t.key === "Backspace") {
    ...
    const r = Re(e.state), o = da(e.state, e.frame), n = o ? r.findIndex((i) => i.start === o.start) : -1;
...
function Dt(e, t) {
  let a = !1;
  const r = e.onRemoved, o = function(...i) {
    a = !0, r?.apply(this, i);
  };
  ...
  return t().then((i) => {
    ...
    n(), console.error("OmniCam: node UI failed to load", i);
  });
}
...
function ss([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * ns), n = Math.round((window.innerHeight || t) * is);
  return [
...
function ls(e, t) {
  const a = to[t];
  return !a || !e?.setSize ? !1 : (e.setSize(ss(a.default, a.min)), !0);
}
function cs(e, t, a) {
  const r = to[t];
  if (!r || !e?.setSize) return !1;
  const o = Array.isArray(a) ? a : Array.isArray(e.size) ? e.size : [0, 0], n = Array.isArray(e.size) ? e.size : [0, 0], [i, s] = r.min, l = Math.max(Number(o[0]) || 0, i), d = Math.max(Number(o[1]) || 0, s);
  return l === n[0] && d === n[1] ? !1 : (e.setSize([l, d]), !0);
}
const ia = "oc-help-css", Ee = "#8b7bd8", ao = /* @__PURE__ */ new Map();
function Tt(e, t) {
  e && t && ao.set(e, t);
}
function pt(e) {
...
function ms() {
  ...
  e.id = ia, e.textContent = ds, document.head.appendChild(e);
}
function ie(e) {
...
function us(e) {
  ...
  if (e.body)
    for (const a of String(e.body).split(/\n\s*\n/)) {
      ...
      r.className = "oc-help-p", r.innerHTML = ie(a), t.appendChild(r);
    }
  if (Array.isArray(e.bullets) && e.bullets.length) {
    ...
    for (const r of e.bullets) {
      ...
      o.innerHTML = ie(r), a.appendChild(o);
    }
    t.appendChild(a);
  }
  if (Array.isArray(e.defs) && e.defs.length) {
    ...
    for (const r of e.defs) {
      const [o, n] = Array.isArray(r) ? r : [r, ""], i = document.createElement("dt");
      i.innerHTML = ie(o);
      const s = document.createElement("dd");
      s.innerHTML = ie(n), a.appendChild(i), a.appendChild(s);
    }
    t.appendChild(a);
  }
  return t;
...
function sa(e) {
  ...
  const i = document.createElement("div");
  i.className = "oc-help-h-title", i.id = r, i.textContent = e.title || "Help";
  const s = document.createElement("button");
  s.className = "oc-help-close", s.type = "button", s.textContent = "✕", s.title = "Close (Esc)", o.appendChild(n), o.appendChild(i), o.appendChild(s), a.appendChild(o);
  const l = document.createElement("div");
  if (l.className = "oc-help-body", e.tagline) {
    const f = document.createElement("p");
    f.className = "oc-help-p", f.style.color = "#e6e6e6", f.innerHTML = ie(e.tagline), l.appendChild(f);
  }
  ...
  if (e.footer) {
    ...
    f.className = "oc-help-tip", f.innerHTML = ie(e.footer), l.appendChild(f);
  }
  a.appendChild(l);
  let c = !1;
  const u = document.activeElement instanceof HTMLElement ? document.activeElement : null, p = () => {
    document.removeEventListener("keydown", m, !0), t.remove(), Me === p && (Me = null), u && u.isConnected && typeof u.focus == "function" && u.focus({ preventScroll: !0 });
  };
  Me = p;
  const h = () => Array.from(
    a.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
  ).filter((f) => !f.disabled && f.offsetParent !== null), m = (f) => {
    if (f.key === "Escape") {
    ...
    if (f.key === "Tab") {
      ...
      const b = w[0], S = w[w.length - 1], y = document.activeElement;
      f.shiftKey && (y === b || !a.contains(y)) ? (f.preventDefault(), S.focus()) : !f.shiftKey && (y === S || !a.contains(y)) && (f.preventDefault(), b.focus());
    }
  };
  return document.addEventListener("keydown", m, !0), s.addEventListener("click", (f) => {
    f.stopPropagation(), p();
  }), t.addEventListener("mousedown", (f) => {
    c = f.target === t;
  }), t.addEventListener("click", (f) => {
    f.target === t && c && p(), c = !1;
  }), a.addEventListener("mousedown", (f) => f.stopPropagation()), document.body.appendChild(t), (s.isConnected ? s : a).focus({ preventScroll: !0 }), p;
}
...
const la = "MajoorOmniCam.ShowHelp", ft = "oc-help-toolbar-icon", ca = "oc-help-toolbar-css", fs = "#8b7bd8";
function hs() {
  ...
  e.id = ca, e.textContent = `
    .${ft}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${fs};color:#fff;
...
ae.registerExtension({
  ...
  getNodeMenuItems(e) {
    const t = pt(e?.comfyClass);
    return t ? [null, { content: "? Help", callback: () => sa(t) }] : [];
  },
  setup() {
    hs();
  }
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Dt(e, async () => {
      ...
      const { attachDirectorShell: o } = await import("./chunk-Cp6PWKYg.js").then((n) => n.o);
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== mt) return;
    const t = !ce, a = t ? null : Et(e);
    await Dt(e, async () => (await import("./chunk-78YUZ4YI.js")).attachExtractor), e.__majoorOmniCamExtractorRuntime && jt(e, mt, t, a?.());
  }
});
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== ut) return;
    const t = !ce, a = t ? null : Et(e);
    await Dt(e, async () => (await import("./chunk-DsL-YxLs.js")).attachMonitor), e.__majoorOmniCamMonitor && jt(e, ut, t, a?.());
  }
});
export {
  Ms as $,
