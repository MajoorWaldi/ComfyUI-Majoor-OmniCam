...
import { p as cc, a as lc, s as bo, S as pn, c as dc, C as pr, b as mc, i as pc, w as fc, d as hc, U as je, m as uc, h as Ge, e as bc, f as gc, g as yc, j as vc, k as xc, r as kc, l as wc, n as Sc, E as jc } from "./chunk-6djzlmmR.js";
import { L as Cc, h as _c, i as jt, j as Vo, k as Ec, l as $c, o as fr, q as Mc, t as Tc, u as Ac, v as Pc, w as Ic, C as hr, c as Ct, x as fn, y as zc, z as Fc, A as Lc, B as hn, D as Oc, E as Kc, G as Dc, H as Rc, I as Nc, J as qc, K as Bc, M as Wc, N as Vc, O as Hc, P as Uc, Q as Gc } from "./chunk-CUk9VyT6.js";
import { S as Xc, b as Yc, p as Zc, l as Jc, u as Qc } from "./chunk-DKIdeco7.js";
import { T as el } from "./chunk-IMiHbUfz.js";
import { T as tl, R as al } from "./vendor-three-B8JDtKPi.js";
import { b as ol, p as un, a as Ao, c as Po, d as rl, r as nl, s as sl, e as il } from "./chunk-Bbeoi1Pz.js";
import { m as cl } from "./chunk-_f2VoZbc.js";
function gt(e, t = 0) {
...
function ur(e, { type: t = "handheld_subtle", intensity: a = 1, duration_frames: o = null, subdivide: n = !0 } = {}) {
  ...
  return [...x].sort((w, v) => w - v).map((w) => {
    const v = xe(b, w), $ = gt(w * u, 1) * p, E = gt(w * u, 2) * p, F = gt(w * u, 3) * p * 0.5, T = gt(w * u, 4) * f, L = gt(w * u, 5) * (f * 0.35), O = [...v.position], A = [...v.target];
    return O[0] += $, O[1] += E, O[2] += F, A[0] += $ * 0.35, A[1] += E * 0.35, {
      frame: w,
      camera: {
        ...v,
        position: O,
...
function Sl(e, t) {
  ...
  if (e.curvePanDrag && t.pointerId === e.curvePanDrag.pointerId) {
    t.preventDefault();
    const g = t.clientX - e.curvePanDrag.startX, w = t.clientY - e.curvePanDrag.startY, $ = Math.max(1, e.state.duration_frames - 1) / (Number(e.curveZoomX) || 1), E = Math.max(1, a.clientWidth - 58), F = a.clientHeight || 180, T = Math.max(1, F - 38);
    e.curvePanX = e.curvePanDrag.origPanX - g / E * $, e.curvePanY = e.curvePanDrag.origPanY + w / T * 10 / (Number(e.curveZoom) || 1), e.drawCurveEditor();
    return;
  }
  if (e.curveScrub && t.pointerId === e.curveScrub.pointerId) {
    t.preventDefault();
    const g = Math.max(1, e.state.duration_frames - 1), w = g / (Number(e.curveZoomX) || 1), v = Number(e.curvePanX) || 0, $ = Math.round(U(v + (o - 44) / Math.max(1, a.clientWidth - 58) * w, 0, g));
    e.setFrame($);
    return;
  }
  if (e.curveBoxSelect && t.pointerId === e.curveBoxSelect.pointerId) {
    ...
    const g = Math.min(e.curveBoxSelect.startX, o), w = Math.max(e.curveBoxSelect.startX, o), v = Math.min(e.curveBoxSelect.startY, n), $ = Math.max(e.curveBoxSelect.startY, n), E = (e.curveHitPoints || []).filter((T) => !T.handle && T.x >= g && T.x <= w && T.y >= v && T.y <= $).map((T) => T.key.frame), F = new Set(e.curveBoxSelect.additive ? e.curveBoxSelect.initial : []);
    for (const T of E) F.add(T);
    e.selectedKeyFrames = F, F.size && (e.selectedKeyFrame = [...F].at(-1)), e.updateKeyVisualState(), e.drawCurveEditor();
    return;
  }
  if (!e.curveDrag || t.pointerId !== e.curveDrag.pointerId) {
    const g = (e.curveHitPoints || []).map((L) => ({ point: L, distance: Math.hypot(o - L.x, n - L.y) })).sort((L, O) => L.distance - O.distance)[0], w = Math.max(1, e.state.duration_frames - 1), v = w / (Number(e.curveZoomX) || 1), $ = Number(e.curvePanX) || 0, E = Math.max(1, a.clientWidth - 58), F = U(Math.round($ + (o - 44) / E * v), 0, w);
    let T = null;
    if (g && g.distance <= 14) {
    ...
    } else n >= 20 && n <= 165 && o >= 44 && o <= a.clientWidth - 14 && (T = { x: o, y: n, frame: F });
    (!!e.curveHover != !!T || T && (e.curveHover?.frame !== T.frame || e.curveHover?.channelName !== T.channelName)) && (e.curveHover = T, e.drawCurveEditor());
    return;
  }
  if (e.curveHover = null, t.preventDefault(), t.stopPropagation(), !e.curveDrag.moved && Math.hypot(o - e.curveDrag.startX, n - e.curveDrag.startY) > 3 && (e.curveDrag.moved = !0), e.curveDrag.historyCheckpointed || (e.checkpoint?.(e.curveDrag.handle ? "Edit curve tangent" : "Edit curve"), e.curveDrag.historyCheckpointed = !0), e.curveDrag.handle) {
    const g = e.curveDrag.key, w = e.curveDrag.channel, v = e.curveDrag.handle, $ = e.curveDrag.pixelPerSegment, E = e.curveDrag.valuePerPixel, F = e.curveDrag.keyX, T = e.curveDrag.keyY;
    ...
    const L = g.tangents.channels[w.id] || {}, O = L.mode || (g.tangents.mode === "aligned" ? "aligned" : "free"), A = {
    ...
    if (v === "in") {
      if (A.in_x = U((o - F) / Math.max(1, $), -0.99, -0.01), A.in_y = (T - n) * E, O === "aligned") {
        const B = Math.hypot(A.in_x, A.in_y) || 1e-6, k = Math.hypot(e.curveDrag.startHandles.out_x, e.curveDrag.startHandles.out_y) || 1e-6;
        A.out_x = -A.in_x / B * k, A.out_y = -A.in_y / B * k;
      }
    } else if (A.out_x = U((o - F) / Math.max(1, $), 0.01, 0.99), A.out_y = (T - n) * E, O === "aligned") {
      const B = Math.hypot(A.out_x, A.out_y) || 1e-6, k = Math.hypot(e.curveDrag.startHandles.in_x, e.curveDrag.startHandles.in_y) || 1e-6;
      A.in_x = -A.out_x / B * k, A.in_y = -A.out_y / B * k;
    }
    g.tangents.channels[w.id] = A, e.scheduleSerialize(), e.camera = xe(e.state, e.frame), e.applyObjectAnimationFrame(), e.render(), e.drawCurveEditor();
    return;
  }
  ...
  if (e.curveDrag.group) {
    ...
    let w = x ? b - e.curveDrag.startFrame : 0;
    const v = new Set(e.curveDrag.group.map((E) => E.key)), $ = e.timelineKeyframes().filter((E) => !v.has(E)).map((E) => E.frame);
    if (w) {
      const E = f;
      let F = 0;
      if (w > 0) {
        ...
        for (const L of e.curveDrag.group) {
          T = Math.min(T, E - L.startFrame);
          for (const O of $)
            O > L.startFrame && (T = Math.min(T, O - 1 - L.startFrame));
        }
        F = Math.max(0, Math.min(w, T));
      } else if (w < 0) {
        let T = 1 / 0;
        for (const L of e.curveDrag.group) {
          T = Math.min(T, L.startFrame - 0);
          for (const O of $)
            O < L.startFrame && (T = Math.min(T, L.startFrame - (O + 1)));
        }
        F = Math.min(0, Math.max(w, -Math.max(0, T)));
      }
      e.curveDrag.group.forEach((T) => {
        T.key.frame = T.startFrame + F;
      });
    }
    for (const E of e.curveDrag.group)
      e.curveDrag.channel.set(E.backing, E.startValue + g);
    e.timelineKeyframes().sort((E, F) => E.frame - F.frame), e.selectedKeyFrames = new Set(e.curveDrag.group.map((E) => E.key.frame)), e.selectedKeyFrame = e.curveDrag.key.frame, e.editingKeyFrame = x ? null : e.curveDrag.key.frame, e.frame = e.curveDrag.key.frame;
  } else if (e.curveDrag.channel.set(p, d), x) {
    const g = e.timelineKeyframes();
    let w = 0, v = f;
    for (const E of g)
      E !== e.curveDrag.key && (E.frame < e.curveDrag.startFrame && E.frame >= w && (w = E.frame + 1), E.frame > e.curveDrag.startFrame && E.frame <= v && (v = E.frame - 1));
    const $ = U(b, w, v);
    $ !== e.curveDrag.key.frame && (e.curveDrag.key.frame = $, e.selectedKeyFrame = $, e.selectedKeyFrames = /* @__PURE__ */ new Set([$]), e.frame = $, e.timelineKeyframes().sort((E, F) => E.frame - F.frame));
  } else
...
function wn(e, { selectedOnly: t = !1 } = {}) {
  ...
  if (m.length > 0) {
    const y = Math.min(...m), b = Math.max(...m), x = Math.max(1e-4, b - y), g = (y + b) / 2, w = (A) => n ? so(n, A) : xe(e.state, A), v = [], $ = Math.max(1, Math.floor(a / 40));
    for (let A = 0; A <= a; A += $) {
      const B = w(A);
      for (const k of s) {
        const _ = k.sample ? k.sample(A) : k.get(B);
        Number.isFinite(_) && v.push(_);
      }
    }
    let E = Math.min(...v), F = Math.max(...v);
    (!Number.isFinite(E) || !Number.isFinite(F)) && (E = -1, F = 1), Math.abs(F - E) < 1e-6 && (E -= 1, F += 1);
    const T = (F - E) * 0.1;
    E -= T, F += T;
    const L = F - E, O = (E + F) / 2;
...
function Il(e) {
  ...
  const i = e.timelineObject(), c = e.timelineKeyframes(), l = tt(e), d = 44, p = 14, f = 16, u = 22, m = Math.max(1, a - d - p), h = Math.max(1, o - f - u), y = Math.max(1, e.state.duration_frames - 1), b = U(Number(e.curveZoomX) || 1, 0.1, 50), x = Number(e.curvePanX) || 0, g = y / b, w = x, v = x + g, $ = [], E = Math.max(1, Math.ceil(g / Math.max(80, m))), F = (M) => i ? so(i, M) : xe(e.state, M);
  for (let M = 0; M <= y; M += E) $.push({ frame: M, value: F(M) });
  $[$.length - 1]?.frame !== y && $.push({ frame: y, value: F(y) });
  const T = $.flatMap((M) => l.map((W) => W.sample ? W.sample(M.frame) : W.get(M.value)));
  let L = Math.min(...T), O = Math.max(...T);
  (!Number.isFinite(L) || !Number.isFinite(O)) && (L = -1, O = 1), Math.abs(O - L) < 1e-6 && (L -= 1, O += 1);
  const A = (O - L) * 0.1;
  L -= A, O += A;
  const B = U(Number(e.curveZoom) || 1, 0.1, 50), k = (O + L) / 2 + (Number(e.curvePanY) || 0), _ = (O - L) / B;
  L = k - _ / 2, O = k + _ / 2;
  const I = (M) => d + (M - w) / Math.max(1e-6, v - w) * m, R = (M) => f + h * (O - M) / Math.max(1e-6, O - L);
  if (s.fillStyle = "#111114", s.fillRect(0, 0, a, o), s.strokeStyle = "#222228", s.lineWidth = 1, s.font = "9px system-ui, -apple-system, sans-serif", s.fillStyle = "#6e727a", xl(s, {
    ...
    const M = R(0);
    s.strokeStyle = "#383842", s.lineWidth = 1.2, s.beginPath(), s.moveTo(d, M), s.lineTo(a - p, M), s.stroke();
  }
  e.curveHitPoints = [];
  for (const M of l) {
    ...
    let W = !1;
    $.forEach((C) => {
      const N = I(C.frame), H = R(M.sample ? M.sample(C.frame) : M.get(C.value));
      N >= d - 50 && N <= a - p + 50 && (W ? s.lineTo(N, H) : (s.moveTo(N, H), W = !0));
    }), s.stroke();
    for (const C of c) {
      const N = i ? C.transform || i : C.camera || C, H = I(C.frame), te = R(M.get(N)), oe = C.frame === e.selectedKeyFrame || e.selectedKeyFrames?.has(C.frame);
      oe && (s.fillStyle = "rgba(242, 208, 107, 0.35)", s.beginPath(), s.arc(H, te, 8.5, 0, Math.PI * 2), s.fill()), s.fillStyle = oe ? "#ffd75e" : M.color, s.strokeStyle = "#0d0d10", s.lineWidth = 1.6, s.beginPath(), s.arc(H, te, oe ? 5.2 : 3.8, 0, Math.PI * 2), s.fill(), s.stroke(), e.curveHitPoints.push({
        x: H,
        y: te,
        key: C,
        channel: M,
    ...
    if (e.showCurveHandles && !M.keyBased)
      for (let C = 0; C < c.length; C++) {
        ...
        const oe = i ? N.transform || i : N.camera || N, J = I(N.frame), se = R(M.get(oe)), pe = c[C - 1], de = c[C + 1], be = Math.max(1, N.frame - (pe?.frame ?? N.frame - 1)), ge = Math.max(1, (de?.frame ?? N.frame + 1) - N.frame), K = ds(
          N,
          M.id,
          pe,
          de,
          (re) => M.get(i ? re.transform || i : re.camera || N)
        ...
        for (const re of me) {
          if (s.strokeStyle = M.color, s.lineWidth = H ? 1.5 : 1, s.beginPath(), s.moveTo(J, se), s.lineTo(re.x, re.y), s.stroke(), s.fillStyle = H ? "#2a2233" : "#171720", s.strokeStyle = H ? "#ffd75e" : M.color, s.lineWidth = H ? 2 : 1.2, s.beginPath(), re.side === "in")
  ...
  if (e.curveBoxSelect) {
    const M = Math.min(e.curveBoxSelect.startX, e.curveBoxSelect.currentX), W = Math.min(e.curveBoxSelect.startY, e.curveBoxSelect.currentY), C = Math.abs(e.curveBoxSelect.currentX - e.curveBoxSelect.startX), N = Math.abs(e.curveBoxSelect.currentY - e.curveBoxSelect.startY);
    s.fillStyle = "rgba(56, 189, 248, 0.15)", s.fillRect(M, W, C, N), s.strokeStyle = "#38bdf8", s.lineWidth = 1, s.setLineDash([4, 4]), s.strokeRect(M, W, C, N), s.setLineDash([]);
  }
  const q = I(e.frame);
  if (q >= d && q <= a - p && (s.strokeStyle = "#a78bfa", s.lineWidth = 1.5, s.beginPath(), s.moveTo(q, f), s.lineTo(q, f + h), s.stroke(), s.fillStyle = "#a78bfa", s.beginPath(), s.moveTo(q - 4, f), s.lineTo(q + 4, f), s.lineTo(q, f + 6), s.closePath(), s.fill()), e.curveDrag || e.curveHover) {
    ...
    if (e.curveDrag)
      if (e.curveDrag.handle) {
        ...
        M = `${e.curveDrag.group.length} keys · ΔF: ${te} · ΔVal: ${oe}`, W = `${e.curveDrag.channel.name}: ${N.toFixed(2)}`;
      } else {
        const C = e.curveDrag.key.frame - e.curveDrag.startFrame, N = e.curveDrag.channel.get(e.curveDrag.object ? e.curveDrag.key.transform : e.curveDrag.key.camera), H = N - e.curveDrag.startValue, te = C >= 0 ? `+${C}` : `${C}`, oe = H >= 0 ? `+${H.toFixed(2)}` : `${H.toFixed(2)}`;
        M = `F${e.curveDrag.key.frame} (${te}) · ${e.curveDrag.channel.name}: ${N.toFixed(2)} (${oe})`;
      }
    ...
    if (M) {
      ...
      const C = s.measureText(M), N = W ? s.measureText(W) : { width: 0 }, H = Math.max(C.width, N.width) + 16, te = W ? 32 : 20, oe = a - p - H - 6, J = f + 6;
      s.fillStyle = "rgba(18, 18, 24, 0.88)", s.strokeStyle = "#38384a", s.lineWidth = 1, s.beginPath(), s.roundRect ? s.roundRect(oe, J, H, te, 4) : s.rect(oe, J, H, te), s.fill(), s.stroke(), s.fillStyle = "#e2e8f0", s.fillText(M, oe + 8, J + (W ? 13 : 14)), W && (s.fillStyle = "#94a3b8", s.font = "9.5px system-ui, -apple-system, sans-serif", s.fillText(W, oe + 8, J + 26)), s.restore();
    }
  }
...
function Wl(e) {
  ...
  if (e.audioWaveformPeaks && e.audioWaveformPeaks.length) {
    ...
    const w = g.getContext("2d"), v = e.audioWaveformPeaks, $ = g.width, E = g.height, F = E / 2;
    w.fillStyle = ae.warning;
    for (let T = 0; T < v.length; T++) {
      const O = (T / (v.length - 1) * n - l) / Math.max(1e-6, c) * $;
      if (O >= -5 && O <= $ + 5) {
        const A = v[T] * (E * 0.85);
        w.fillRect(O, F - A / 2, Math.max(1, $ / v.length * s - 0.5), A);
      }
    }
    t.appendChild(g);
  }
  if (e.state.playback_range) {
    ...
    const w = fe(e, e.state.playback_range[0]), v = fe(e, e.state.playback_range[1]);
    g.style.left = `${w}%`, g.style.width = `${Math.max(0, v - w)}%`, t.appendChild(g);
  }
  hs(e, t);
  for (const g of e.state.markers || []) {
    const w = fe(e, g.frame);
    if (w < -5 || w > 105) continue;
    const v = document.createElement("span");
    v.className = "timeline-marker", v.style.left = `${w}%`, v.style.setProperty("--marker-color", g.color), v.title = g.name, t.appendChild(v);
  }
  if (o.length > 1) {
    const g = fe(e, o[0].frame), w = fe(e, o[o.length - 1].frame), v = document.createElement("span");
    v.className = "oc-dope-rail", v.style.left = `${Math.min(g, w)}%`, v.style.width = `${Math.abs(w - g)}%`, v.style.setProperty("--channel-color", ae.typeCamera), t.appendChild(v);
  }
  ...
  for (const g of o) {
    ...
    v.type = "button", v.className = `key${g.frame === e.frame ? " at-playhead" : ""}${d.has(g.frame) ? " selected" : ""}${g.frame === e.editingKeyFrame ? " editing" : ""}`, v.dataset.keyFrame = String(g.frame), v.dataset.interp = g.interpolation || "ease", v.setAttribute("aria-label", r("{value1} keyframe at frame {value2}", { value1: a?.name || "Camera", value2: g.frame })), v.title = r("Frame {value1} · {value2} · Drag: Retime · Alt+Drag: Duplicate", { value1: g.frame, value2: g.interpolation }), v.style.left = `${w}%`;
    const $ = document.createElement("span");
    $.className = "key-label", $.textContent = String(g.frame), v.appendChild($), v.addEventListener("pointerdown", (E) => {
      if (E.preventDefault(), E.stopPropagation(), v.focus({ preventScroll: !0 }), E.altKey) {
        e.checkpoint("Duplicate keyframe");
        const T = a ? { frame: g.frame, transform: ze(g.transform), interpolation: g.interpolation } : { frame: g.frame, camera: le(g.camera), interpolation: g.interpolation }, L = e.timelineKeyframes();
        L.push(T), L.sort((O, A) => O.frame - A.frame), e.selectedKeyFrame = T.frame, e.selectedKeyFrames = /* @__PURE__ */ new Set([T.frame]), e.keyDrag = { key: T, box: t, isDuplicate: !0, historyCheckpointed: !0, moving: [{ key: T, startFrame: T.frame }], startPointerFrame: g.frame, startClientX: E.clientX, startClientY: E.clientY }, e.setFrame(T.frame, !1, !1), e.setStatus(r("Duplicating key from {value1}...", { value1: g.frame }));
        return;
      }
      if (E.shiftKey) {
        e.selectedKeyFrames = new Set(e.selectedKeyFrames || [e.selectedKeyFrame].filter((T) => T !== null)), e.selectedKeyFrames.has(g.frame) ? e.selectedKeyFrames.delete(g.frame) : e.selectedKeyFrames.add(g.frame), e.selectedKeyFrame = e.selectedKeyFrames.has(g.frame) ? g.frame : [...e.selectedKeyFrames].at(-1) ?? null, e.setFrame(g.frame, !1, !1), e.updateKeyVisualState(), e.refreshKeyEditor();
        return;
      }
      e.selectedKeyFrames?.has(g.frame) || (e.selectedKeyFrames = /* @__PURE__ */ new Set([g.frame])), e.selectedKeyFrame = g.frame;
      const F = e.timelineKeyframes().filter((T) => e.selectedKeyFrames.has(T.frame));
      e.keyDrag = { key: g, box: t, historyCheckpointed: !1, moving: F.map((T) => ({ key: T, startFrame: T.frame })), startPointerFrame: g.frame, startClientX: E.clientX, startClientY: E.clientY }, e.setFrame(g.frame, !1, !1);
    }), v.addEventListener("click", (E) => {
      E.preventDefault(), E.stopPropagation(), !(E.shiftKey || e.suppressKeyClick) && e.selectKeyframe(g);
    }), t.appendChild(v);
  }
  ...
  if (f) {
    ...
    const w = e.selectedKeyFrames?.size || 0;
    w > 1 && f.append(document.createTextNode(` · ${w} selected`));
  ...
  if (h) {
    ...
    for (const g of e.state.cameras) {
      ...
      const $ = document.createElement("span");
      $.textContent = `${g.name} · ${g.keyframes.length} key${g.keyframes.length === 1 ? "" : "s"}${g.id === e.state.playblast_camera_id ? " · PLAYBLAST" : ""}`, w.append(v, $), w.addEventListener("click", () => {
...
function Vd(e, t = {}) {
  ...
  const b = (K) => a.querySelector(`[data-role="${K}"]`), x = b("assets-panel"), g = b("asset-grid"), w = b("asset-kinds"), v = b("asset-search"), $ = b("asset-status"), E = b("asset-import-file"), F = b("scene-tab"), T = b("assets-tab"), L = b("agent-tab"), O = a.querySelector('[data-asset-view="agent"]'), A = { assets: T, agent: L };
  let B = "", k = null, _ = !0, I = !0, R = "scene", q = 0;
  function M(K, { sticky: V = !1 } = {}) {
  ...
  let C = !1;
  function N() {
    if (w && (w.innerHTML = qd(s.state.filter.kind, s.state.kinds)), g) {
      const K = {};
  ...
  async function te(K) {
    if (K) {
      M(r("Importing {name}...").replace("{name}", K.name));
      try {
        const V = await n.importModel(K, { kind: "prop", name: K.name.replace(/\.[^.]+$/, "") });
        V.asset && s.upsert(V.asset), M(r("Imported {name}").replace("{name}", V.asset?.name || K.name));
      } catch (V) {
        M(V.message || r("Import failed"));
      }
    }
  }
  function oe(K) {
    K === "agent" && !$o() && (K = "scene"), R = K, F && (F.hidden = K !== "scene");
    ...
    K === "assets" && _ && (_ = !1, s.refresh()), K === "agent" && I && (I = !1, t.onAgentFirstOpen?.());
  }
  function J() {
    const K = $o();
  ...
  function de() {
    clearTimeout(k), k = setTimeout(() => s.setFilter({ search: v.value }), 200);
  }
  function be(K) {
  ...
  return x?.addEventListener("click", se), x?.addEventListener("dblclick", pe), a.querySelector('[data-role="left-tabs"]')?.addEventListener("click", se), v?.addEventListener("input", de), E?.addEventListener("change", be), N(), J(), {
    store: s,
    switchView: oe,
    syncAgentAvailability: J,
    refresh: () => s.refresh(),
    dispose() {
      ge(), clearTimeout(k), x?.removeEventListener("click", se), x?.removeEventListener("dblclick", pe), a.querySelector('[data-role="left-tabs"]')?.removeEventListener("click", se), v?.removeEventListener("input", de), E?.removeEventListener("change", be), c.clear(), d?.dispose?.(), i.clear();
    }
  };
}
function Hd(e, t = {}) {
  ...
  function l() {
    ...
    o.hidden = !1;
    const u = c(), m = Number(e.frame) || 0, h = Array.isArray(e.state?.objects) ? e.state.objects : [];
    let y = 0;
    for (const b of h) {
      if (!Zr(b, { mode: s.mode, selectedIds: u })) continue;
      const x = Jr(b, s.content);
      if (!x) continue;
      const g = Qr(h, b, m) || {
        position: b.position,
        size: b.size
      }, w = e.webgl.projectWorldToScreen(en(g, b.type, b.annotation?.anchor));
      if (!w || w.behind) continue;
      const v = i(y);
      y += 1, v.hidden = !1, v.textContent = x, v.style.transform = `translate(-50%, -100%) translate(${Math.round(w.x)}px, ${Math.round(w.y)}px)`;
      const $ = s.content === "annotation" ? b.annotation?.color : "";
      v.style.setProperty("--oc-label-accent", $ || ""), v.classList.toggle("is-annotation", s.content === "annotation" && !!$);
    }
    for (let b = y; b < n.length; b += 1) n[b].hidden = !0;
  }
  function d(p) {
...
function Xd(e, t = {}) {
  ...
  function m() {
    ...
    x.id !== c && (c = x.id, n.open = !0, l = {}, x.asset_id ? o.get(x.asset_id).then((w) => {
      c === x.id && (l = { ...w?.asset?.rig?.bone_map || {} }, u());
    }).catch(() => u()) : u());
  }
  function h(x) {
    const g = x.target.closest("[data-rig-joint]");
    if (!g) return;
    const w = g.dataset.rigJoint;
    g.value ? l[w] = g.value : delete l[w], u();
  }
  function y(x) {
...
function Yd(e, t = {}) {
  ...
  function i() {
    ...
    o.hidden = !1;
    const { objectId: l, boneMap: d, selectedJoint: p } = c, f = e.characterRuntime, u = /* @__PURE__ */ new Set();
    for (const m of an) {
      const h = d?.[m];
      if (!h) continue;
      const y = f?.getJointWorldTransform?.(l, m, d) || e.webgl.resolveModelBone?.(l, h);
      if (!y?.world) continue;
      const b = e.webgl.projectWorldToScreen(y.world);
      if (!b || b.behind) continue;
      const x = s(m);
      x.hidden = !1, x.classList.toggle("selected", m === p), x.style.transform = `translate(-50%, -50%) translate(${Math.round(b.x)}px, ${Math.round(b.y)}px)`, u.add(m);
    }
    for (const [m, h] of n) u.has(m) || (h.hidden = !0);
  }
  return i(), {
    update: i,
    dispose() {
...
function Jd(e, t = {}) {
  ...
  const s = n.querySelector('[data-role="pose-preset"]'), i = n.querySelector('[data-pose-act="edit"]'), c = n.querySelector('[data-role="pose-joint-row"]'), l = n.querySelector('[data-role="pose-joint-name"]'), d = ["x", "y", "z"].map((k) => n.querySelector(`[data-role="pose-rot-${k}"]`));
  let p = null, f = !1, u = /* @__PURE__ */ new Map();
  const m = () => e.state?.objects?.find((k) => k.id === p) || null, h = () => e.rigMapper?.boneMap && Object.keys(e.rigMapper.boneMap).length ? e.rigMapper.boneMap : null, y = () => e.subSelection?.type === "character_joint" && e.subSelection.objectId === p ? e.subSelection.jointId : null;
  function b(k) {
    const _ = lr({ ...k, preset_id: k.id });
    return { id: k.id, name: k.name || k.id, root_offset: _.root_offset, joints: _.joints };
  }
  function x(k) {
    const _ = u.get(k?.pose?.preset_id) || null;
    return gs({ preset: _, overrides: k?.pose }).joints;
  }
  function g() {
    const k = m();
    !k?.character || !h() || e.webgl?.applyCharacterPose?.(p, h(), f || k.character.pose?.joints ? x(k.character) : {});
  }
  function w() {
    const k = y(), _ = f && !!k;
    if (c && (c.hidden = !_), !_) return;
    l && (l.textContent = k);
    const I = m(), R = x(I?.character)[k] || [0, 0, 0, 1], q = ys(R);
    d.forEach((M, W) => {
      M && document.activeElement !== M && (M.value = String(Math.round(q[W] * 100) / 100));
    });
  }
  function v() {
    const k = m(), _ = !!k?.character?.motion;
    if (i && (i.classList.toggle("active", f), i.disabled = _, i.title = _ ? r("Clear the motion clip to edit the pose") : r("Toggle FK pose editing")), s) {
      const I = [["neutral", r("Standing Neutral")]].concat([...u.values()].filter((q) => q.id !== "neutral").map((q) => [q.id, q.name || q.id])), R = I.map((q) => q.join(":")).join("|");
      s.dataset.sig !== R && (s.dataset.sig = R, s.replaceChildren(...I.map(([q, M]) => {
  ...
  async function $() {
    try {
      ...
      u = new Map((k.poses || []).filter((_) => _?.id).map((_) => [_.id, b(_)]));
    } catch {
      u = /* @__PURE__ */ new Map();
    }
  ...
  function E() {
    const k = y();
    if (!k) return;
    const _ = d.map((q) => Number(q?.value) || 0), I = bs(_), R = e.directorApi?.execute({
      version: 1,
      id: `tx_pose_${Date.now().toString(36)}`,
      description: "Pose joint",
      operations: [{ type: "character.set_joint_rotation", objectId: p, joint: k, rotation: I }]
    });
    R && !R.ok && e.setStatus?.(R.error?.message || r("Could not set the joint")), v();
  }
  function F() {
    const k = s?.value || "neutral", _ = u.get(k) || lr({ preset_id: k });
  ...
  async function T() {
    ...
    const _ = (await jt(e, r("Save Pose"), r("Pose name"), ""))?.trim();
    if (!_ || e.disposed) return;
    const I = _.toLowerCase().replace(Zd, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "pose";
    try {
      const R = await o.savePose({
        ...
        joints: x(k.character)
      });
      u.set(R.pose.id, b(R.pose)), e.setStatus?.(r("Pose saved: {name}").replace("{name}", _)), v();
    } catch (R) {
  ...
  function L() {
    const k = m();
    !k?.character || k.character.motion || (f = !f, !f && e.subSelection?.type === "character_joint" && (e.subSelection = null), v(), e.rigOverlay?.update?.());
  }
  function O(k) {
    f && (e.subSelection = { type: "character_joint", objectId: p, jointId: k }, v(), e.rigOverlay?.update?.());
  }
  const A = Yd(e, {
    onPick: O,
  ...
  function B() {
    ...
    k.id !== p && (p = k.id, f = !1, e.subSelection?.type === "character_joint" && (e.subSelection = null), u.size || $()), v(), A.update();
  }
  s?.addEventListener("change", F), i?.addEventListener("click", L), n.querySelector('[data-pose-act="save"]')?.addEventListener("click", T);
  for (const k of d)
    k?.addEventListener("change", E), k?.addEventListener("input", E);
  return $(), {
    sync: B,
    update() {
      A.update(), g();
    ...
    dispose() {
      A.dispose(), s?.removeEventListener("change", F), i?.removeEventListener("click", L);
      for (const k of d)
        k?.removeEventListener("change", E), k?.removeEventListener("input", E);
    }
  };
}
function Cr(e, t) {
...
function Qd(e) {
  ...
  function m() {
    const x = u(), g = x ? { type: "character.set_motion", objectId: d, motion: x } : { type: "character.clear_motion", objectId: d }, w = e.directorApi?.execute({
      version: 1,
      id: `tx_motion_${Date.now().toString(36)}`,
      description: "Character motion",
      operations: [g]
    });
    w && !w.ok && e.setStatus?.(w.error?.message || r("Could not set the motion")), e.poseEditor?.sync?.(), h();
  }
  function h() {
    const x = p(), g = f(), w = x?.character?.motion || null;
    if (o) {
      const $ = g.join("|");
      o.dataset.sig !== $ && (o.dataset.sig = $, o.replaceChildren(
        Cr("", g.length ? r("No motion (static)") : r("No clips in this model")),
        ...g.map((E) => Cr(E, E))
      )), document.activeElement !== o && (o.value = w?.clip_id || ""), o.disabled = !g.length;
    }
    yo(n, w?.start_frame ?? 0), yo(s, w?.end_frame ?? 0), yo(i, w?.speed ?? 1), c && document.activeElement !== c && (c.checked = w ? w.loop !== !1 : !0);
    const v = !!w;
  ...
  function y() {
    ...
    const w = e.characterRuntime?.sampleCanonicalPose?.(d, g) || {};
    e.directorApi?.execute({
      version: 1,
      ...
      operations: [
        { type: "character.clear_motion", objectId: d },
        {
          ...
          objectId: d,
          pose: { preset_id: "neutral", root_offset: x.character.pose?.root_offset || [0, 0, 0], joints: w }
        }
      ]
...
function om({
  ...
  f.addEventListener?.("mouseDown", E), f.addEventListener?.("objectChange", F), f.addEventListener?.("mouseUp", T), f.addEventListener?.("dragging-changed", L);
  function O(C) {
  ...
  function W() {
    f.removeEventListener?.("mouseDown", E), f.removeEventListener?.("objectChange", F), f.removeEventListener?.("mouseUp", T), f.removeEventListener?.("dragging-changed", L), o?.remove && u && o.remove(u), o?.remove && o.remove(p), f.dispose?.(), m = null, h = null;
  }
  return {
    attach: O,
    detach: A,
...
function nm(e, { controlsFactory: t, anchorFactory: a } = {}) {
  ...
  function m({ targetSpec: k }) {
    if (k) {
      if (l(c), k.type === "object") {
        e.checkpoint("Transform object");
        const _ = xs(e), I = (_.length ? _ : [k.object]).map((q) => ({
          object: q,
          transform: ze(q)
        }));
        for (const q of I) e.beginObjectEdit(q.object);
        const R = I.reduce((q, M) => ve(q, M.transform.position), [0, 0, 0]).map((q) => q / I.length);
        n = { type: "object", group: I, pivot: R };
        return;
      }
      if (k.type === "camera") {
      ...
      if (k.type === "camera_path") {
        ...
        e.checkpoint("Transform camera path"), n = {
          type: "camera_path",
          trackId: _.id,
          origin: on(_.keyframes),
          baseKeys: _.keyframes.map((I) => ({ ...I, camera: le(I.camera) }))
        };
        return;
      }
      if (k.type === "path_point" || k.type === "path_group") {
        ...
        e.checkpoint(k.type === "path_point" ? "Transform path point" : "Transform path selection"), n = {
          ...
          baseKeys: _.keyframes.map((R) => ({ ...R, camera: le(R.camera) })),
          lookAtActive: rn(_, e.state.objects)
        };
        return;
      }
  ...
  function b(k) {
    const _ = e.state.cameras.find((I) => I.id === n.trackId);
    _ && (_.keyframes = nn(n.baseKeys, h(k)), y(_));
  }
  function x(k) {
    const _ = e.state.cameras.find((R) => R.id === n.trackId);
    if (!_) return;
    const I = { ...h(k), lookAtActive: n.lookAtActive };
    _.keyframes = ws(n.baseKeys, n.selectedFrames, I), y(_);
  }
  function g(k) {
    const _ = e.state.cameras.find((I) => I.id === n.trackId);
    _ && (_.keyframes = Ss(n.baseKeys, [n.frame], { delta: k.position }), y(_));
  }
  function w({ delta: k }) {
    n && (n.type === "object" ? v(k) : n.type === "camera" ? $(k) : n.type === "camera_target" ? E(k) : n.type === "camera_path" ? b(k) : n.type === "path_point" || n.type === "path_group" ? x(k) : n.type === "path_point_target" && g(k), e.render());
  }
  function v(k) {
    const { group: _, pivot: I } = n, R = e.state.gizmo_mode;
    for (const q of _)
      if (R === "translate")
        q.object.position = ve(q.transform.position, k.position);
      else if (R === "rotate")
        q.object.position = ve(I, dr($e(q.transform.position, I), k.rotationDeg)), q.object.rotation = ve(q.transform.rotation, k.rotationDeg);
      else if (R === "scale") {
        const M = $e(q.transform.position, I);
        q.object.position = ve(I, M.map((W, C) => W * k.scaleFactors[C])), q.object.size = q.transform.size.map((W, C) => Math.max(0.01, W * k.scaleFactors[C]));
      }
    for (const q of _) e.commitObjectEdit(q.object);
  }
  function $(k) {
    if (e.state.gizmo_mode === "translate")
      e.camera.position = ve(n.position, k.position);
    else {
      const _ = $e(n.target, n.position);
      e.camera.target = ve(n.position, dr(_, k.rotationDeg));
    }
    e.commitCameraEdit();
  }
  function E(k) {
    const _ = ve(n.base, k.position);
    n.tracking ? ks(e, _) : e.camera.target = _, e.commitCameraEdit();
  }
  function F({ cancelled: k }) {
    if (!n) return;
    const _ = n.type;
  ...
  function T() {
    ...
    const k = e.recording ? null : vs(e), _ = e.state.gizmo_mode || "translate", I = !!k && !k.readOnly && rm.has(k.type) && k.allowedModes.includes(_);
    if (i = I ? k.type : null, !I) {
      o?.detach();
      return;
    }
  ...
  return { sync: T, isPointerOverHandle: L, cancelDrag: O, currentLiveType: A, dispose: B };
}
function sm(e, t) {
...
function co(e, t, a) {
  ...
  const o = at(e), n = o.expanded ? 220 : 138, s = Math.min(n, Math.max(80, Math.min(t, a) - 20)), i = 10, c = Math.max(0, t - s - i), l = Math.max(0, a - s - i), d = e.viewportCamera(), p = d?.position || [0, 1.5, 5], f = d?.target || [0, 0, 0], u = e.activeCameraTrack?.(), m = e.state.cameras?.length ? e.state.cameras : u ? [u] : [], h = m.flatMap(
    ($) => ($.keyframes || []).map((E) => E.camera?.position).filter(Boolean)
  );
  let y = 12;
  if (o.rangeIndex >= 0 && o.rangeIndex < De.length)
    ...
    const $ = Math.max(
      ...
      Math.abs(f[0] || 0),
      Math.abs(f[2] || 0),
      ...h.flatMap((E) => [Math.abs(E[0] || 0), Math.abs(E[2] || 0)]),
      4
    );
    y = Math.max(6, Math.ceil(($ + 1.5) / 4) * 4);
  }
  const b = c + s / 2, x = l + s / 2, g = s / 2 - 12, w = g / y, v = o.centerMode === "camera" ? [p[0] || 0, p[2] || 0] : [0, 0];
  return {
    rx: c,
    ry: l,
    radarSize: s,
...
function km(e, t, a, o) {
  const n = co(e, a, o);
  if (!n) return;
  const { rx: s, ry: i, radarSize: c, cx: l, cy: d, innerRadius: p, range: f, scale: u, camPos: m, camTgt: h, cameras: y, activeTrack: b } = n, x = at(e), g = e.state.active_camera_id || b?.id, w = (J, se) => An(n, J, se);
  ...
  for (const J of e.state.objects || []) {
    ...
    const se = No(e.state.objects, J).position || [0, 0, 0], [pe, de] = w(se[0], se[2]);
  ...
  const [E, F] = w(m[0] || 0, m[2] || 0), [T, L] = w(h[0] || 0, h[2] || 0), O = 8, A = U(E, s + O, s + c - O), B = U(F, i + O, i + c - O), k = U(T, s + O, s + c - O), _ = U(L, i + O, i + c - O), I = m[1] || 0, R = vm(I), q = xm(I);
  t.strokeStyle = "rgba(255, 255, 255, 0.40)", t.lineWidth = 1, t.setLineDash([3, 3]), t.beginPath(), t.moveTo(A, B), t.lineTo(k, _), t.stroke(), t.setLineDash([]), t.fillStyle = "#ffffff", t.beginPath(), t.arc(k, _, 2.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "rgba(255, 255, 255, 0.7)", t.lineWidth = 1, t.beginPath(), t.arc(k, _, 4.5, 0, Math.PI * 2), t.stroke();
  const M = h[0] - m[0], W = h[2] - m[2], C = Math.atan2(W, M), H = (e.viewportCamera().fov || 35) * Math.PI / 360, te = U(24 * (u / (p / 8)), 16, 38), oe = t.createRadialGradient(A, B, 2, A, B, te);
  oe.addColorStop(0, R + "55"), oe.addColorStop(1, R + "08"), t.fillStyle = oe, t.strokeStyle = R, t.lineWidth = 1.2, t.beginPath(), t.moveTo(A, B), t.lineTo(A + Math.cos(C - H) * te, B + Math.sin(C - H) * te), t.arc(A, B, te, C - H, C + H), t.closePath(), t.fill(), t.stroke(), t.fillStyle = R + "44", t.beginPath(), t.arc(A, B, 6.5, 0, Math.PI * 2), t.fill(), t.fillStyle = R, t.beginPath(), t.arc(A, B, 3.5, 0, Math.PI * 2), t.fill(), t.strokeStyle = "#ffffff", t.lineWidth = 1.5, t.beginPath(), t.moveTo(A, B), t.lineTo(A + Math.cos(C) * 7, B + Math.sin(C) * 7), t.stroke(), wm(e, t, n, R, q, C, Math.hypot(M, W)), x.hover && Sm(t, n, x.hover), t.restore();
}
...
function In(e, t, a, o) {
  ...
  if (y)
    for (const b of y.keyframes || []) {
      ...
      if (!x) continue;
      const [g, w] = p(x[0], x[2]);
      if (Math.hypot(a - g, o - w) <= 6)
        return { type: "keyframe", key: b, frame: b.frame };
    }
  for (const b of e.state.objects || []) {
    if (b.enabled === !1) continue;
    const x = No(e.state.objects, b).position || [0, 0, 0], [g, w] = p(x[0], x[2]);
    if (Math.hypot(a - g, o - w) <= 7)
...
function Em(e, t = {}) {
  ...
  const p = (m) => {
    ...
    const w = parseFloat(a.min), v = parseFloat(a.max);
    isNaN(w) || (x = Math.max(w, x)), isNaN(v) || (x = Math.min(v, x)), parseFloat(a.value) !== x && (a.value = x, a.dispatchEvent(new Event("input", { bubbles: !0 })));
  }, u = () => {
...
function Tm(e, t, a) {
  ...
  t('[data-role="duration-seconds"]')?.addEventListener("change", (m) => {
    e.durationWidget && Number(e.durationWidget.value) !== Number(m.target.value) && e.checkpoint("Change duration"), e.durationWidget && (e.durationWidget.value = Number(m.target.value)), e.syncFromWidgets();
    ...
    if (!(!y || h.button === 2 || m.closest(".scene-action-btn")))
      if (y.dataset.objectId) {
        ...
        if (e.finishCameraEdit(), e.selectedObjectIds ||= /* @__PURE__ */ new Set(), h.ctrlKey || h.metaKey)
          ...
          const g = e.state.objects.map(($) => $.id), w = g.indexOf(e.outlinerAnchorId), v = g.indexOf(b.id);
          e.selectedObjectIds = new Set(g.slice(Math.min(w, v), Math.max(w, v) + 1));
        } else
        ...
        for (const g of e.root.querySelectorAll(".scene-item")) {
          const w = !!(g.dataset.objectId && e.selectedObjectIds.has(g.dataset.objectId)), v = !!(g.dataset.objectId && g.dataset.objectId === e.selectedObjectId);
          g.classList.toggle("selected", w), g.classList.toggle("primary", v), g.setAttribute("aria-selected", String(w));
        }
        const x = e.root.querySelector('[data-role="outliner-batch-bar"]');
        if (x) {
          const g = e.selectedObjectIds?.size || 0;
          x.hidden = g < 2;
          const w = x.querySelector('[data-role="batch-count"]');
          w && (w.textContent = `${g} ${r("selected")}`);
        }
...
const Ua = () => import("./chunk-B1zSO_At.js");
function ip(e, t) {
...
function Gu(e, t, a = Fn) {
  ...
  return b.traverse((g) => {
    g.geometry?.dispose?.();
    const w = Array.isArray(g.material) ? g.material : [g.material];
    for (const v of w) v?.dispose?.();
  }), {
    group: n,
    key: s,
    fill: c,
...
function Up(e) {
  ...
  for (const b of e.state.motion_layers || []) {
    ...
    h += 1;
    const w = b.id === e.state.selected_motion_layer_id;
    if (c.strokeStyle = w ? "#ffcc4d" : "rgba(65,217,197,0.6)", c.lineWidth = (w ? 2.4 : 1.5) * n, c.beginPath(), g.forEach(($, E) => {
      const F = d($.x), T = p($.y);
      E ? c.lineTo(F, T) : c.moveTo(F, T);
    }), c.stroke(), !x) {
      c.fillStyle = w ? "#ffcc4d" : "#41d9c5";
      for (const $ of g)
        c.beginPath(), c.arc(d($.x), p($.y), (w ? 3.4 : 2.4) * n, 0, Math.PI * 2), c.fill();
    }
    const v = Hp(g, m);
    v && (c.fillStyle = w ? "#ffcc4d" : "#41d9c5", c.strokeStyle = "#fff", c.lineWidth = 1.4 * n, c.beginPath(), c.arc(d(v.x), p(v.y), 4.4 * n, 0, Math.PI * 2), c.fill(), c.stroke());
  }
...
function Qp(e = 512, t = 768) {
  ...
  if (!o) return a;
  const n = (A, B, k, _, I) => {
    typeof o.roundRect == "function" ? (o.beginPath(), o.roundRect(A, B, k, _, I)) : (o.beginPath(), o.moveTo(A + I, B), o.lineTo(A + k - I, B), o.quadraticCurveTo(A + k, B, A + k, B + I), o.lineTo(A + k, B + _ - I), o.quadraticCurveTo(A + k, B + _, A + k - I, B + _), o.lineTo(A + I, B + _), o.quadraticCurveTo(A, B + _, A, B + _ - I), o.lineTo(A, B + I), o.quadraticCurveTo(A, B, A + I, B), o.closePath());
  ...
  o.restore(), o.save();
  const v = 180, $ = 28, E = i - v / 2, F = p + 10;
  n(E, F, v, $, 14), o.fillStyle = "rgba(15, 23, 42, 0.9)", o.fill(), o.strokeStyle = "rgba(56, 189, 248, 0.45)", o.lineWidth = 1.2, o.stroke(), o.fillStyle = "#38bdf8", o.beginPath(), o.arc(E + 18, F + $ / 2, 4, 0, Math.PI * 2), o.fill(), o.fillStyle = "#f1f5f9", o.font = "bold 11px system-ui, -apple-system, sans-serif", o.textAlign = "left", o.fillText("SUBJECT PROXY", E + 30, F + 18), o.restore(), o.save(), o.textAlign = "center", o.fillStyle = "#f8fafc", o.font = "bold 22px system-ui, -apple-system, sans-serif", o.fillText("SUBJECT CARD", i, t * 0.73), o.fillStyle = "#94a3b8", o.font = "italic 13px system-ui, -apple-system, sans-serif", o.fillText("No input image connected", i, t * 0.775);
  const T = t * 0.85, L = 250, O = 32;
  return n(i - L / 2, T - O / 2, L, O, 6), o.fillStyle = "rgba(30, 41, 59, 0.75)", o.fill(), o.strokeStyle = "rgba(56, 189, 248, 0.3)", o.lineWidth = 1, o.stroke(), o.fillStyle = "#38bdf8", o.font = "12px system-ui, -apple-system, sans-serif", o.fillText("Connect IMAGE node or load file", i, T + 4), o.strokeStyle = "rgba(56, 189, 248, 0.3)", o.lineWidth = 1.5, o.strokeRect(1, 1, e - 2, t - 2), o.restore(), a;
}
...
function Ju(e) {
  ...
  return a.colorSpace = e.SRGBColorSpace, a.needsUpdate = !0, wo = a, a;
}
function Or(e, t, { frameCount: a = 0, fps: o = 0 } = {}) {
  ...
  if (!n || !s) return !1;
  e.widthWidget && (e.widthWidget.value = n), e.heightWidget && (e.heightWidget.value = s), i && e.fpsWidget && (e.fpsWidget.value = i), l && i && e.durationWidget && (e.durationWidget.value = Math.max(0.25, l / i)), e.syncFromWidgets();
  const d = e.state?.objects?.find((p) => p.id === "subject");
  return d && Ne(d, t), !0;
}
  ...
  for (const f of i) {
    ...
    if (m) {
      if (u === "image" || u === "video") {
        ...
        if (h && h.value) {
          const y = String(h.value), b = /\.(mp4|mov|webm|mkv|m4v|avi)(?:\s|$)/i.test(y), x = m.widgets?.find((v) => String(v.name).toLowerCase() === "subfolder")?.value || "", g = Re(Xe(y, x)), w = e.state.objects.find((v) => v.id === "subject");
          if (w) {
            const v = await Hn(e, w, g, n, b);
            if (!n()) return;
            w.asset = Xe(y, x);
            let $ = null;
            if (b)
              try {
                ...
                console.warn("Failed to describe upstream video:", E);
              }
            n() && Or(e, v, {
              fps: $?.fps,
        ...
        if (h && h.value) {
          ...
          try {
            ...
            if (g.ok) {
              ...
              if (!n()) return;
              const v = new File([w], y, { type: w.type || "audio/wav" });
      ...
      if (u === "scene_3d" || u === "model" || u === "mesh") {
        ...
        if (h && h.value) {
          ...
          if (["glb", "gltf", "obj", "fbx", "stl", "ply"].includes(b)) {
            const x = m.widgets?.find(($) => String($.name).toLowerCase() === "subfolder")?.value || "", g = Re(Xe(y, x)), w = `upstream_scene_${m.id}`;
            d.add(w);
            let v = e.state.objects.find(($) => $.id === w);
            v ? (v.asset = Xe(y, x), v.format = b === "gltf" ? "glb" : b) : (v = {
              ...
              enabled: !0,
              asset: Xe(y, x)
            }, e.state.objects.push(v)), e.modelUrlsById.set(w, g), e.serialize(), e.refreshObjects(), e.render(), s = !0, e.setStatus(r("Upstream 3D model: {value1}", { value1: y }));
          }
        }
      }
    }
...
async function kf({
  ...
  try {
    for (const w of vf)
      if (!(s.isTypeSupported && !s.isTypeSupported(w)))
        try {
          f = new s(p, { mimeType: w, videoBitsPerSecond: xf(n) });
          break;
        } catch {
        }
    ...
    const u = [];
    f.ondataavailable = (w) => {
      w.data.size && u.push(w.data);
    };
    const m = new Promise((w, v) => {
      f.addEventListener("stop", w, { once: !0 }), f.addEventListener("error", () => v(f.error || new Error("MediaRecorder failed")), { once: !0 });
    });
    f.start(100);
    const h = c();
    for (let w = 0; w < a; w++) {
      if (i?.aborted) throw new DOMException("Playblast cancelled", "AbortError");
      await o(w), await l(1e3 / t);
    }
    f.stop(), await m;
...
async function Cf(e) {
  ...
  try {
    let s = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: l } = await import("./chunk-BKFX1vDy.js");
...
function Tf(e) {
  ...
  for (const p of a) {
    ...
    if (e.webgl?.projectWorldToScreen && e.webgl.activeCamera) {
      const L = e.webgl.projectWorldToScreen(h);
    ...
    const x = t.content === "annotation" && p.annotation?.color || "", g = 6 * c, w = 4 * c, $ = n.measureText(f).width + g * 2, E = 12 * c + w * 2, F = Math.round(y - $ / 2), T = Math.round(b - E - 6 * c);
    n.fillStyle = "rgba(16,17,22,0.88)", Af(n, F, T, $, E, 4 * c), n.fill(), x && (n.strokeStyle = x, n.lineWidth = Math.max(1, 1.5 * c), n.stroke()), n.fillStyle = x ? "#f3f4f6" : "#e6e6ec", n.fillText(f, F + g, T + E - w - 2 * c);
  }
  n.restore();
}
...
function Kf(e, t) {
  const [a, o, n] = t.position || [0, 0, 0], s = t.size?.[1] || 1.8, i = (t.size?.[0] || 0.7) * 0.5, c = [a, o + s * 0.88, n], l = [a, o + s * 0.76, n], d = [a - i * 0.55, o + s * 0.73, n], p = [a + i * 0.55, o + s * 0.73, n], f = [a - i * 0.72, o + s * 0.52, n], u = [a + i * 0.72, o + s * 0.52, n], m = [a - i * 0.82, o + s * 0.34, n], h = [a + i * 0.82, o + s * 0.34, n], y = [a, o + s * 0.44, n], b = [a - i * 0.28, o + s * 0.44, n], x = [a + i * 0.28, o + s * 0.44, n], g = [a - i * 0.28, o + s * 0.22, n], w = [a + i * 0.28, o + s * 0.22, n], v = [a - i * 0.28, o, n + 0.05], $ = [a + i * 0.28, o, n + 0.05];
  ne(e, c, l, "#aaa", 2), ne(e, l, y, "#aaa", 2), ne(e, d, p, "#aaa", 2), ne(e, d, f, "#aaa", 2), ne(e, f, m, "#aaa", 2), ne(e, p, u, "#aaa", 2), ne(e, u, h, "#aaa", 2), ne(e, b, x, "#aaa", 2), ne(e, b, g, "#aaa", 2), ne(e, g, v, "#aaa", 2), ne(e, x, w, "#aaa", 2), ne(e, w, $, "#aaa", 2);
  const E = Ie(c, e.viewportCamera(), e.canvas.width, e.canvas.height);
  E && (e.ctx.strokeStyle = "#aaa", e.ctx.beginPath(), e.ctx.arc(E[0], E[1], U(28 / E[2], 3, 12), 0, Math.PI * 2), e.ctx.stroke());
}
function Df(e, t) {
...
function qf(e, t) {
  ...
  if (a)
    try {
      const b = Math.max(1, m - u), x = Math.max(1, y - h), g = a.videoWidth || a.naturalWidth || a.width, w = a.videoHeight || a.naturalHeight || a.height, v = e.state.card_fit || "contain";
      if (e.ctx.fillStyle = "#111", e.ctx.fillRect(u, h, b, x), v === "stretch" || !g || !w)
        e.ctx.drawImage(a, u, h, b, x);
      else if (v === "contain") {
        const $ = Math.min(b / g, x / w), E = g * $, F = w * $;
        e.ctx.drawImage(a, u + (b - E) / 2, h + (x - F) / 2, E, F);
      } else {
        const $ = Math.max(b / g, x / w), E = b / $, F = x / $;
        e.ctx.drawImage(a, (g - E) / 2, (w - F) / 2, E, F, u, h, b, x);
      }
    } catch {
    }
  else {
...
function Qn(e) {
  ...
  if (d) {
    ...
    if (t.appendChild(m), !h)
      for (const y of u) {
        ...
        const x = y.id === e.state.active_camera_id, g = y.id === e.state.playblast_camera_id, w = e.selectedEntity === "camera" && x;
        b.setAttribute("aria-selected", String(w)), b.className = `scene-item${w ? " selected" : ""}${x && !w ? " active-view" : ""}`;
        const v = document.createElement("i");
        v.className = "pi pi-video", v.style.cssText = `color:${ae.typeCamera}`;
        const $ = document.createElement("span");
        if ($.className = "scene-item-label", w || x) {
          const T = document.createElement("span");
          T.style.cssText = `color:${w ? ae.warning : ae.success};font-weight:700`, T.textContent = w ? "● " : "○ ", $.appendChild(T);
        }
        if ($.appendChild(document.createTextNode(y.name)), g) {
          const T = document.createElement("span");
          T.style.cssText = `color:${ae.warning};font-size:10px`, T.title = "Playblast Output", T.textContent = " ★", $.appendChild(T);
        }
        if (y.muted) {
          const T = document.createElement("span");
          T.style.opacity = ".6", T.textContent = " (muted)", $.appendChild(T);
        }
        const E = document.createElement("div");
        E.className = "scene-item-actions", E.appendChild(n("pi-star", "Solo track", y.solo, () => {
          e.checkpoint("Solo track"), y.solo = !y.solo, e.serialize(), e.refreshObjects(), e.renderCameraView();
        }, `color:${ae.warning};border-color:${ae.warning};background:${ae.warningSoft}`)), E.appendChild(n("pi-volume-off", "Mute track", y.muted, () => {
          e.checkpoint("Mute track"), y.muted = !y.muted, e.serialize(), e.refreshObjects(), e.renderCameraView();
        }, `color:${ae.error};border-color:${ae.error};background:${ae.errorSoft}`)), E.appendChild(n("pi-lock", "Lock track", y.locked, () => {
          e.checkpoint("Lock track"), y.locked = !y.locked, e.serialize(), e.refreshObjects(), e.renderCameraView();
        })), (y.keyframes || []).length >= 1 && E.appendChild(n(
          "pi-arrows-alt",
        ...
        )), E.appendChild(n("pi-ellipsis-v", "Camera actions", !1, (T) => e.openCameraContext(T, y.id, !1))), b.append(v, $, E), b.title = w ? r("Currently selected for editing") : g ? r("Active playblast camera") : r("Click to select & activate this camera");
        const F = () => {
        ...
        b.addEventListener("contextmenu", (T) => {
          T.preventDefault(), T.stopPropagation(), e.openCameraContext(T, y.id, !1);
  ...
  if (p) {
    ...
    }), { header: g, isCollapsed: w } = c(a === "lights" ? r("Lights") : r("Objects"), x.length, "objects");
    if (t.appendChild(g), !w)
      for (const { object: v, level: $ } of x) {
        const E = document.createElement("div");
        E.role = "button", E.tabIndex = 0, E.dataset.objectId = v.id;
        const F = e.selectedEntity === "object" && (v.id === e.selectedObjectId || e.selectedObjectIds?.has?.(v.id)), T = e.selectedEntity === "object" && v.id === e.selectedObjectId;
        E.setAttribute("aria-selected", String(F)), E.className = `scene-item${F ? " selected" : ""}${T ? " primary" : ""}${$ > 0 && !s ? " scene-item-child" : ""}`, $ > 0 && !s && (E.style.paddingLeft = `${$ * 16 + 6}px`);
        ...
        _.style.cssText = A ? `color:${ae.error}` : O ? "" : "opacity:.5;text-decoration:line-through", _.textContent = v.name || v.type, _.title = r("Double-click to rename"), _.addEventListener("dblclick", (M) => {
          M.preventDefault(), M.stopPropagation(), mh(e, v, _);
        }), k.appendChild(_);
        const I = Array.isArray(v.tags) ? v.tags : [];
        if (I.length) {
          const M = document.createElement("span");
          M.className = "scene-item-tags";
          for (const W of I.slice(0, 2)) {
            const C = document.createElement("span");
            C.className = "scene-item-tag", C.textContent = W, M.appendChild(C);
          }
          if (I.length > 2) {
            const W = document.createElement("span");
            W.className = "scene-item-tag scene-item-tag-more", W.textContent = `+${I.length - 2}`, M.appendChild(W);
          }
          k.appendChild(M);
        }
        if (A) {
          const M = document.createElement("span");
          M.style.cssText = `color:${ae.error};font-size:9px;font-weight:700`, M.textContent = " [Format!]", k.appendChild(M);
        }
        const R = document.createElement("div");
        R.className = "scene-item-actions", R.appendChild(n(O ? "pi-eye" : "pi-eye-slash", O ? "Hide object (Alt+Click to Isolate)" : "Show object (Alt+Click to Isolate)", !O, (M) => {
          if (M?.altKey) {
            ...
            e.toggleObject(v.id);
        }, `color:${ae.error};opacity:.7`)), R.appendChild(n(v.locked ? "pi-lock" : "pi-lock-open", "Lock object", v.locked, () => Zo(e, v))), R.appendChild(n("pi-copy", "Duplicate object", !1, () => e.duplicateObject?.(v.id))), v.id !== "subject" && R.appendChild(n("pi-trash", "Delete object", !1, () => e.deleteObject?.(v.id))), R.appendChild(n("pi-ellipsis-v", "Object actions", !1, (M) => e.openObjectContext(M, v.id))), E.append(B, k, R), E.title = r("Click to select · Double-click to toggle visibility · Right-click for actions");
        const q = (M = {}) => {
          if (M.altKey && v.id !== "subject") return void e.deleteObject(v.id);
        ...
        E.addEventListener("dblclick", () => e.toggleObject(v.id)), E.addEventListener("contextmenu", (M) => {
          M.preventDefault(), M.stopPropagation(), e.openObjectContext(M, v.id);
...
function ph(e, t) {
  ...
  let h = [0, 0, 0], y = [0, 0, 0], b = "#8c929b", x, g, w, v;
  d ? (h = [5, 8.5, 4], y = [-55, 35, 0], b = "#fff6ec", x = 2.2, g = !0) : p ? (h = [0, 3, 0], b = "#ffffff", x = 2, g = !1) : f && (h = [0, 4, 0], y = [-60, 0, 0], b = "#ffffff", x = 3, w = 45, v = 0.25, g = !0);
  const $ = {
    id: a,
    type: t,
    name: u,
...
function gh(e) {
  ...
  if (s) {
    const k = n.target_object_id || e.state.target_object_id || "", _ = `T${mr()}${e.state.objects.map((I) => `${I.id}\0${I.name || I.type}`).join("|")}`;
    jo(s, _, () => [
      yt("", r("Manual Target (No Tracking)")),
      ...e.state.objects.map((I) => yt(I.id, `${r("Track:")} ${I.name || I.type}`))
    ], k);
  }
  dh(e);
  ...
  ["camera-px", "camera-py", "camera-pz", "camera-tx", "camera-ty", "camera-tz", "camera-fov", "camera-roll", "camera-near", "camera-far", "camera-rx", "camera-ry", "camera-rz"].forEach((k, _) => {
    for (const I of e.root.querySelectorAll(`[data-role="${k}"]`))
      document.activeElement !== I && (I.value = String(Math.round(i[_] * 1e4) / 1e4));
  });
  ...
  if (!t) {
    ...
    _ && (_.textContent = `${n.name} · F${e.frame}`), Br(o, {
  ...
  for (const [k, _] of Object.entries(m))
    for (const I of e.root.querySelectorAll(`[data-role="${k}"]`))
      document.activeElement !== I && (I.value = String(Math.round(_ * 1e4) / 1e4));
  ...
  if (b) {
    ...
    for (; I; ) {
      ...
      for (const M of e.state.objects)
        !_.has(M.id) && M.parent_id && _.has(M.parent_id) && (_.add(M.id), I = !0);
    }
    const R = e.state.objects.filter((M) => !_.has(M.id)), q = `P${mr()}${k}${R.map((M) => `${M.id} ${M.name || M.type}`).join("|")}`;
    jo(b, q, () => [
      yt("", r("No parent")),
      ...R.map((M) => yt(M.id, M.name || M.type))
    ], t.parent_id || "");
  }
  const x = ["sun_light", "point_light", "spot_light"].includes(t.type), g = t.type === "spot_light", w = o('[data-role="light-props-row"]');
  w && (w.hidden = !x);
  const v = o('[data-role="spot-props-row"]');
  v && (v.hidden = !g);
  const $ = o('[data-role="material-row"]');
  $ && ($.hidden = x);
  const E = o('[data-role="scale-row"]');
  E && (E.hidden = x);
  const F = o('[data-role="rotation-row"]');
  if (F && (F.hidden = t.type === "point_light"), x) {
    ...
    if (_ && (_.checked = t.cast_shadow !== !1), g) {
      const I = o('[data-role="object-cone-angle"]');
      I && document.activeElement !== I && (I.value = String(t.cone_angle ?? 45));
  ...
  if (h && (h.hidden = !T?.animations), y) {
    const k = T?.animationNames || [];
    jo(y, `A${k.join("|")}`, () => k.map((_, I) => yt(String(I), _)), String(t.animation_index || 0));
  }
...
function Xh(e) {
  ...
  if (f) {
    const m = Math.max(1, e.state?.fps || 24), h = a ? a.frame : e.frame, y = Math.floor(h / m), b = h % Math.round(m), x = String(Math.floor(y / 3600)).padStart(2, "0"), g = String(Math.floor(y % 3600 / 60)).padStart(2, "0"), w = String(y % 60).padStart(2, "0"), v = String(b).padStart(2, "0");
    f.textContent = `${x}:${g}:${w}:${v} (${h}f)`;
  }
  if (Gh(e), !a) return;
  if (t) {
    ...
    onContextMenu(c) {
      ...
      if (l.closest?.(".curve-editor")) {
        ...
        if (m && this.curveHitPoints) {
          const h = m.getBoundingClientRect(), y = c.clientX - h.left, b = c.clientY - h.top, x = this.curveHitPoints.map((g) => ({ point: g, distance: Math.hypot(y - g.x, b - g.y) })).sort((g, w) => g.distance - w.distance)[0];
          if (x && x.distance <= 14) {
...
function ou(e) {
  ...
  return {
    ...
    restoreHistorySnapshot(G) {
      ...
      const z = new Set(this.state.objects.map((Q) => Q.id));
      for (const Q of P) z.has(Q) || this.removeObjectResources(Q);
    ...
    resizeCanvas() {
      ...
      if (!G) return;
      const j = Math.min(2, window.devicePixelRatio || 1), S = G.clientWidth || 320, P = G.clientHeight || 180, z = Math.max(320, Math.round(S * j)), D = Math.max(180, Math.round(P * j));
      (this.canvas.width !== z || this.canvas.height !== D) && (this.canvas.width = z, this.canvas.height = D);
...
function fu(e) {
  ...
  return {
    ...
    setSpatialHandleMode(S) {
      ...
      const z = this.activeCameraTrack()?.keyframes || [], D = z.findIndex((X) => X.frame === this.selectedKeyFrame);
      if (D < 0) {
        this.setStatus(r("Select a camera keyframe first"));
        return;
      }
      this.checkpoint(r("Camera path handle: {mode}").replace("{mode}", S)), hc(z[D], S, {
        prevKey: z[D - 1] || null,
        nextKey: z[D + 1] || null
    ...
    insertPathKeyAtCursor(S) {
      ...
      const P = this.interactionElement.getBoundingClientRect(), z = (S.clientX - P.left) * this.canvas.width / Math.max(1, P.width), D = (S.clientY - P.top) * this.canvas.height / Math.max(1, P.height), X = this.webgl.pickPathSegment([z, D]);
      if (!X) return !1;
    ...
    transformCameraPath(S) {
      ...
      const z = nn(P.keyframes, { origin: on(P.keyframes), ...S });
      return P.keyframes = z, P.id === this.state.active_camera_id && (this.state.keyframes = z), this.camera = Se(P, this.frame, this.state.objects), P.camera = Le(this.camera), this.serialize(), this.refreshKeys(), this.refreshInspector(), this.render(), this.renderCameraView?.(), !0;
    },
    ...
    async openCameraPathPresetPicker() {
      ...
      const P = mc.map((D) => ({ id: D, label: r(pr[D] || D) })), z = await fn({ title: r("Camera Path Preset"), items: P, owner: this });
      return z ? this.applyCameraPathPreset(z) : !1;
    },
    applyCameraPathPreset(S, P = {}) {
      const z = this.activeCameraTrack();
      if (!z || z.locked)
        return this.setStatus(r("{name} is locked").replace("{name}", z?.name || r("Camera"))), !1;
      const [D, X] = Oo(this.state), Z = dc({ type: S, camera: this.camera, startFrame: D, endFrame: X, params: P });
      return Z.ok ? (this.checkpoint(r("Apply camera path preset")), z.keyframes = Z.keyframes, z.id === this.state.active_camera_id && (this.state.keyframes = Z.keyframes), this.camera = Se(z, this.frame, this.state.objects), z.camera = Le(this.camera), this.serialize(), this.refreshObjects(), this.refreshKeys(), this.refreshInspector(), this.render(), this.setStatus(r("{preset} camera path generated").replace("{preset}", r(pr[S] || S))), !0) : (this.setStatus(Z.reason === "insufficient_frame_slots" ? r("Not enough frames in the playback range for this preset") : r("Could not generate that camera path preset")), !1);
    },
    toggleCurveHandles() {
      pt(this);
    },
    ...
    toggleInspector(S) {
      ...
      if (!P) return;
      const z = S !== void 0 ? S : P.dataset.collapsed !== "true";
      P.dataset.collapsed = String(z);
      for (const D of this.root.querySelectorAll('[data-act="toggle-inspector"]'))
        D.classList.toggle("active", !z), D.setAttribute("aria-pressed", String(!z));
      this.setStatus(z ? "Inspector hidden (N)" : "Inspector shown");
    },
    refreshKeys() {
      it(this);
    },
    ...
    selectHierarchy(S = this.selectedObjectId) {
      ...
      for (; z; ) {
        ...
        for (const D of this.state.objects)
          D.parent_id && P.has(D.parent_id) && !P.has(D.id) && (P.add(D.id), z = !0);
      }
    ...
    lookAtObject(S) {
      const P = this.state.objects.find((z) => z.id === S);
      if (P) {
        this.checkpoint("Look-at constraint");
        for (const z of this.state.cameras)
          for (const D of z.keyframes) D.camera.target = [...P.position || [0, 1.5, 0]];
    ...
    aimAtSelectedObject(S) {
      ...
      const P = this.activeCameraTrack(), z = S && this.state.objects.find((Z) => Z.id === S) || this.selectedObject() || this.state.objects.find((Z) => Z.id === "subject") || this.state.objects[0];
      if (!z) return;
      P.target_object_id !== z.id && (P.aim_bone = null), P.target_object_id = z.id, P.id === this.state.active_camera_id && (this.state.target_object_id = z.id, this.state.aim_bone = P.aim_bone);
      const X = (z.type === "model" || z.type === "glb" ? this.webgl?.getObjectWorldCenter?.(z.id) : null) || (z.keyframes?.length ? Ue(z, this.frame).position : z.position || [0, 1.5, 0]);
      this.camera.target = [...X], this.beginCameraEdit(), this.commitCameraEdit(), this.finishCameraEdit(), this.serialize(), this.refreshInspector(), this.updateHudCamera(), this.render(), this.setStatus(`Camera tracking locked to ${z.name || z.id}`);
    },
    setAimBone(S) {
      ch(this, S);
    },
    ...
    bakeAimToKeyframes() {
      ...
      const S = this.activeCameraTrack(), P = S.target_object_id || this.state.target_object_id || "subject", z = this.state.objects.find((X) => X.id === P) || this.state.objects[0];
      if (!z || !S.keyframes?.length) return;
      const D = z.type === "model" || z.type === "glb" ? this.webgl?.getObjectWorldCenter?.(z.id) : null;
      for (const X of S.keyframes) {
        const Z = (z.type === "model" || z.type === "glb") && D && !z.keyframes?.length ? D : z.keyframes?.length ? Ue(z, X.frame).position : z.position || [0, 1.5, 0];
        X.camera.target = [...Z];
      }
      S.id === this.state.active_camera_id && (this.state.keyframes = S.keyframes), this.serialize(), this.refreshKeys(), this.refreshInspector(), this.render(), this.setStatus(`Aim baked across all keyframes following ${z.name || z.id}`);
    }
  };
}
function hu(e) {
  ...
  return {
    setTargetAtCursor(j) {
      if (!j) return;
      const S = this.interactionElement.getBoundingClientRect(), P = (j.clientX - S.left) * this.canvas.width / Math.max(1, S.width), z = (j.clientY - S.top) * this.canvas.height / Math.max(1, S.height), D = this.webgl?.intersectScenePoint?.(P, z, this.canvas.width, this.canvas.height);
    ...
    async loadMediaUrl(j, S, P, z) {
      return Zt(this, j, S, P, z);
    },
    restoreAssets() {
      ta(this);
    },
...
function Pu(e) {
  ...
  return {
    ...
    renderViewportOnly() {
      ...
      if (!Y) {
        (!this.recording && ["omni_ref", "card_grid", "graybox", "grid", "wireframe"].includes(z) || this.recording && this.state.playblast_grid) && this.drawGrid(), ["omni_ref", "point_field"].includes(z) && this.drawPointField();
        for (const ee of X)
          ee.enabled !== !1 && (ee.type === "card" && ["omni_ref", "card_grid", "graybox", "wireframe"].includes(z) ? this.drawCard(ee) : ["cube", "ground", "glb", "model"].includes(ee.type) && z !== "grid" && z !== "point_field" ? this.drawCube(ee) : ee.type === "sphere" && z !== "grid" && z !== "point_field" ? this.drawSphere(ee) : ee.type === "cylinder" && z !== "grid" && z !== "point_field" ? this.drawCylinder(ee) : ee.type === "torus" && z !== "grid" && z !== "point_field" ? this.drawTorus(ee) : ee.type === "human" && z !== "grid" && z !== "point_field" ? this.drawHuman(ee) : ee.type === "null" && this.drawNull(ee));
    ...
    renderCameraView() {
      if (this.perf && (this.perf.previewRenderCount = (this.perf.previewRenderCount || 0) + 1), this.state.camera_view_visible) {
        ...
        if (P) {
          ...
          z = X.length ? X[this.cameraPreviewTick % X.length] : null;
        }
        for (const D of S) {
          ...
          if (Me && (Me.textContent = `F${this.frame}`), P && D.id !== this.state.active_camera_id && D !== z) continue;
  ...
  return {
    ...
    simplifyActiveKeys({ mode: e = "simplify", tolerance: t = 0, target: a = 0, scope: o = "camera", fromKeys: n = null, silent: s = !1 } = {}) {
      ...
      const l = this.resolveSelectedFrames(), d = o !== "all_cameras" && l.length >= 2 ? [l[0], l.at(-1)] : null, p = d ? [] : l, f = (h) => {
        const y = [...h].sort((F, T) => F.frame - T.frame), b = d ? d[0] : -1 / 0, x = d ? d[1] : 1 / 0, g = y.filter((F) => F.frame < b), w = y.filter((F) => F.frame >= b && F.frame <= x), v = y.filter((F) => F.frame > x);
        let $ = w, E = 0;
        if (w.length > 2) {
          const F = e === "reduce" ? kc(w, i, { target: a || Math.ceil(w.length / 2), keepFrames: p }) : e === "clean" ? wc(w, i, { keepFrames: p }) : Sc(w, i, { tolerance: t * Iu, keepFrames: p });
          $ = F.keys, E = F.removed;
        }
        return { keys: [...g, ...$, ...v].sort((F, T) => F.frame - T.frame), removed: E };
      };
      let u = 0;
      const m = c.map((h) => {
...
class nr {
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-BKFX1vDy.js"));
    } catch (a) {
  ...
  try {
    a.assetBrowser = Vd(a, {
      ...
      onAgentFirstOpen: async () => {
        if ($o())
          try {
            const { createDirectorAgentPanel: o } = await import("./chunk-OtfIitw8.js");
            a.agentPanel = o(a);
          } catch (o) {
