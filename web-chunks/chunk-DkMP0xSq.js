...
const Hs = "omnicam_humanoid_v1", ct = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
function Us({ preset: e, overrides: t } = {}) {
  const a = Pe(e), r = Pe(t);
  return {
...
function le(e, t, a = null) {
  ...
  const m = e.constraints?.look_at, u = m?.status === void 0 || m?.status === "active" ? m?.object_id || e.target_object_id || e.camera?.target_object_id : null, f = a || e.objects;
  if (u && Array.isArray(f)) {
    const g = f.find((M) => M.id === u);
    if (g && g.enabled !== !1) {
      const M = ft(f, g, t), A = m?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
...
function Tt(e, t, a, r, o) {
  ...
  if (s === "auto")
    return { ...p(!1), mode: s };
  const u = p(!1), f = T(Number(n.out_x ?? u.out_x), 0.01, 0.99), h = Number(n.out_y ?? u.out_y);
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  if (l.interpolation === "bezier" || d.interpolation === "bezier") {
    const v = Tt(l, a, c, d, r), b = Tt(d, a, l, m, r), x = f, w = f + (v.out_y || 0), D = h + (b.in_y || 0), g = h, M = T(Number(v.out_x ?? 1 / 3), 0, 1), A = T(1 + Number(b.in_x ?? -1 / 3), 0, 1);
    let G = 0, ee = 1;
    for (let St = 0; St < 32; St++) {
      const W = (G + ee) * 0.5, He = 1 - W;
      3 * He * He * W * M + 3 * He * W * W * A + W * W * W < u ? G = W : ee = W;
    }
...
function Ke(e, t) {
  ...
  const r = ce(e), o = (y, v) => (y.transform?.position || r.position)[v] ?? 0, n = (y, v) => (y.transform?.rotation || r.rotation)[v] ?? 0, s = (y, v) => (y.transform?.size || r.size)[v] ?? (v === 2 ? 0.01 : 1), i = pt(a, t), l = z(a, t, "pos_x", (y) => o(y, 0), !1, i), d = z(a, t, "pos_y", (y) => o(y, 1), !1, i), c = z(a, t, "pos_z", (y) => o(y, 2), !1, i), m = z(a, t, "rot_x", (y) => n(y, 0), !0, i), p = z(a, t, "rot_y", (y) => n(y, 1), !0, i), u = z(a, t, "rot_z", (y) => n(y, 2), !0, i), f = z(a, t, "scale_x", (y) => s(y, 0), !1, i), h = z(a, t, "scale_y", (y) => s(y, 1), !1, i), _ = z(a, t, "scale_z", (y) => s(y, 2), !1, i);
  return {
...
function ei(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  for (let p = 0; p < o; p++) {
    const u = p * c % 1, f = p * m % 1, h = (p + 0.5) * 0.7548776662466927 % 1;
    ...
    if (t === "ground_focus")
      ...
      const D = u * Math.PI * 2, g = 1 - 2 * f, M = Math.sqrt(Math.max(0, 1 - g * g)), A = 1.5 + Math.cbrt(h) * 20;
...
function ti(e) {
  ...
  a.cameras = s.slice(0, Ce.maxCameras).map((c, m) => {
    ...
    return f = [...new Map(f.map((h) => [h.frame, h])).values()].sort((h, _) => h.frame - _.frame), f.length || (f = [{ frame: 0, camera: V(u), interpolation: "ease" }]), {
      id: p,
...
function Vo([e, t, a, r]) {
  ...
  return [p, m, u].map((f) => f * 180 / Math.PI);
}
function na(e, t) {
...
function ia(e, t = {}, a = null, r = "generic") {
  ...
  const d = qo(l, o), c = Ho(l, o), m = At(d, o), p = At(m, o), u = { speed: d, angular_speed: c, acceleration: m, jerk: p }, f = Wo(e, a), h = $o(l, f, s, i), _ = l.map((g) => g.fov), y = t.allow_framing_loss === !0, v = [], b = [];
  for (let g = 0; g < n; g++) {
    const M = [], A = [];
    for (const G of jt) {
    ...
    !h[g] && !y && (M.push("over"), A.push("framing_loss")), v.push(Ot(M)), b.push(A);
  }
...
function Qo(e, t) {
  ...
  if (c.style.left = `${m}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${d !== 0 ? ` (${d > 0 ? "+" : ""}${d})` : ""}`, a.moving && a.moving.length > 1) {
    ...
    for (const f of a.moving) {
      let h = T(f.startFrame + d, 0, e.state.duration_frames - 1);
      for (; u.has(h) && h > 0 && h < e.state.duration_frames - 1; ) h += Math.sign(d || 1);
...
function hn({
  ...
  return [
    ...
    O(
      ...
      "Enables the OmniCam Director Agent panel.",
      !0
    ),
    P(
      yr,
...
function Rt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = K(t), l = t.position, d = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = kn(d, i);
  let m, p;
  if (t.camera_type === "orthographic") {
    const u = 5 / Math.max(0.01, t.zoom || 1), f = u * r / Math.max(1, o);
    m = (e[0] / Math.max(1, r) - 0.5) * 2 * f, p = (0.5 - e[1] / Math.max(1, o)) * 2 * u;
  } else {
...
const jn = ["maya", "blender", "simple"];
function ht(e) {
  const t = e.state?.navigation_profile;
  return jn.includes(t) ? t : "maya";
}
function An(e, t) {
...
function In(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
...
function Pn(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  s.keyframes = En(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = le(s, e.frame, e.state.objects), s.camera = V(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function Bt(e, t) {
  const a = [];
...
function Nn(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => zn(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((x) => x[b]))), d = [0, 1, 2].map((b) => Math.max(...i.map((x) => x[b]))), c = l.map((b, x) => (b + d[x]) / 2), { right: m, up: p, forward: u } = K(t), f = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), h = Math.tan((t.fov || 35) * Math.PI / 360), _ = h * f;
  let y = 2, v = 0.1;
  for (const b of i) {
...
function wi(e, t = {}) {
  ...
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), Nn(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? S("Framed: all objects") : S("Framed: {name}").replace("{name}", s.name || s.type || S("Subject")));
}
function Rn(e, t, a, { forceLocal: r = !1 } = {}) {
...
function Ar(e) {
  ...
  const n = Math.max(0.7, H(E(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? Rn(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  const l = i.map((d, c) => {
    const m = Math.abs(d[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = ne(Se(d, m)), u = ne(Se(d, p)), f = [];
    for (let h = 0; h <= 48; h++) {
      const _ = h / 48 * Math.PI * 2, y = B(C(r, C(k(p, Math.cos(_) * n), k(u, Math.sin(_) * n))), a, e.canvas.width, e.canvas.height);
      y && Number.isFinite(y[0]) && Number.isFinite(y[1]) && f.push(y);
    }
    return { index: c, axis: d, points: f };
  });
...
function xi(e) {
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : Bn(e.ctx, s[0], i);
    }
  }
...
function Ae(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = Kn(a), d = a.axis ? at[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], m = K(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...E(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, x = d ? k(d, b) : C(k(m.right, n * p * i), k(m.up, -s * p * i));
    u = u.map((w) => C(w, x)), u = Gn(e, a, u, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((w) => w.transform.position));
  }
  const f = a.mode === "rotate" ? l ?? (n - s) * 0.5 * i : 0, h = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - s) * 0.01 * i) : 1, _ = d || at.z, y = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
  a.snapshots.forEach((b, x) => {
    ...
    if (a.mode === "scale") {
      const D = y ? Fe(h, 0.1) : h, g = d ? d.map((A) => A ? D : 1) : [D, D, D], M = E(b.transform.position, a.pivot);
...
const re = (e, t) => Math.round(e / t) * t, Wn = (e, t) => e.map((a) => re(a, t));
function $n(e, t, a) {
  ...
  for (const p of [s[0], i[0]]) for (const u of [s[1], i[1]]) for (const f of [s[2], i[2]]) {
    const h = B([p, u, f], a, e.canvas.width, e.canvas.height);
    h && (l = Math.min(l, h[0]), c = Math.max(c, h[0]), d = Math.min(d, h[1]), m = Math.max(m, h[1]));
  }
...
function he(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, d) => Math.abs(o.axis[d]) > 1e-6 ? re(l, i) : o.base[d]) : Wn(t, i);
  if (s === "vertex" && a && !o) {
...
function Mi(e, t) {
  ...
  if (c) {
    ...
    if (c.entity.type === "camera_path" && In(e, { baseDrag: b, viewCamera: n, entityPosition: c.entity.position })) return;
  ...
  const p = !!e.isNavigatingFly, u = Tn(e, t, n);
  if (!p && !u) return;
...
function Ci(e, t) {
  ...
  if (e.pathDrag) {
    ...
    c && (c.camera.position = Rt(
      ...
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = Dn(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_target") {
      const u = C(e.gizmoDrag.target, k(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), f = he(e, u, i, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
    ...
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        ...
        const u = c ? re(d * 0.015, Math.PI / 12) : d * 0.015, f = E(e.gizmoDrag.target, e.gizmoDrag.position), h = xe(f, k(e.gizmoDrag.axis, u * (180 / Math.PI)));
        e.camera.target = C(e.gizmoDrag.position, h);
      }
      e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera_path") return void Pn(e, { pointer: i, deltaPixels: d, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
      ...
      if (e.gizmoDrag.free) {
        ...
        const u = [...e.gizmoDrag.size], f = u[e.gizmoDrag.axisIndex] + d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength;
        u[e.gizmoDrag.axisIndex] = Math.max(0.01, c ? re(f, 0.1) : f), e.gizmoDrag.object.size = u;
      }
    else {
      const u = [...e.gizmoDrag.rotation], f = u[e.gizmoDrag.axisIndex] + d * 0.75;
    ...
    if (m.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        ...
        for (const f of m) f.object.position = C(f.transform.position, u);
      } else if (e.state.gizmo_mode === "rotate") {
        const u = E(e.gizmoDrag.object.rotation, p.rotation);
        for (const f of m)
          f.object.position = C(e.gizmoDrag.groupPivot, xe(E(f.transform.position, e.gizmoDrag.groupPivot), u)), f.object.rotation = C(f.transform.rotation, u);
      } else {
        const u = e.gizmoDrag.object.size.map((f, h) => f / Math.max(0.01, p.size[h]));
        for (const f of m) {
          const h = E(f.transform.position, e.gizmoDrag.groupPivot);
...
function ki(e, t) {
  ...
  if (e.boxSelection) {
    ...
    for (const u of e.state.objects) {
      ...
      const f = $n(e, u, i);
      f && f.maxX >= l && f.minX <= d && f.maxY >= c && f.minY <= m && p.add(u.id);
    }
...
function Di(e, t) {
  ...
  const a = An(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
...
function Be(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  ...
  const n = Un(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function Ye(e, t) {
  return Zn(t, e.interactionElement);
}
function ts(e, t) {
...
function Ti(e, t) {
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = Xe(e.state);
    n && (e.checkpoint("Set motion interpolation"), Xn(n, o.target.value), ae(e, `Motion interpolation: ${o.target.value}`));
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? Be(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? as(e, s) : n === "erase" && ts(e.state, s), ae(e, `Motion tool: ${n}`);
  ...
  const r = (o) => {
    ...
    const [s, i] = Kt(e), l = es(e.state, n.points, s, i);
...
function ji(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => os(e, a.dataset.motionCreate), { signal: t });
...
const ge = { t: "translate", r: "rotate", s: "scale" }, ls = [
...
], cs = 'button,summary,a[href],[role="button"],[role="menuitem"],[role="tab"],[role="option"],[role="checkbox"],[role="switch"]';
function ds(e) {
  return e instanceof HTMLElement || e instanceof SVGElement ? !!e.closest?.(cs) : !1;
}
function ms(e) {
...
function fs(e, t) {
  return ps(e) || t?.lastKeyZone || "viewport";
}
let qt = !1;
function us() {
  qt || typeof window > "u" || (qt = !0, window.addEventListener("keydown", (e) => {
    ...
    !a || a.disposed || hs(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function hs(e, t) {
  if (!Sn()) return !1;
  const a = t.composedPath?.()[0] || t.target;
  if (ms(a) || (t.code === "Space" || t.key === "Enter") && ds(a)) return !1;
...
function xs([e, t], [a, r]) {
  ...
  const o = Math.round((window.innerWidth || e) * Ss), n = Math.round((window.innerHeight || t) * ws);
  return [
...
function Ds() {
  ...
  e.id = Ht, e.textContent = ks, document.head.appendChild(e);
}
function oe(e) {
...
function Wt(e) {
  ...
  const m = document.activeElement instanceof HTMLElement ? document.activeElement : null, p = () => {
    document.removeEventListener("keydown", f, !0), t.remove(), we === p && (we = null), m && m.isConnected && typeof m.focus == "function" && m.focus({ preventScroll: !0 });
  };
  we = p;
  const u = () => Array.from(
...
const $t = "MajoorOmniCam.ShowHelp", it = "oc-help-toolbar-icon", Ut = "oc-help-toolbar-css", As = "#8b7bd8";
function Es() {
  ...
  e.id = Ut, e.textContent = `
    .${it}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${As};color:#fff;
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await yt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-BK4uZSlu.js").then((n) => n.g);
      return je("director:import:resolved", e), o;
    });
    const r = e.__majoorOmniCam;
    r && (je("director:attach:complete", e), Mn(r), t && Cn(r), _t(e, rt, t, a?.()));
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== ot) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-BmV56wKd.js")).attachExtractor), e.__majoorOmniCamExtractor && _t(e, ot, t, a?.());
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== nt) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-Bq7rc35q.js")).attachMonitor), e.__majoorOmniCamMonitor && _t(e, nt, t, a?.());
  }
});
export {
  ya as $,
