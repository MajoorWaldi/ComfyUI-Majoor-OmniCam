...
import { R as $l } from "./vendor-three-AeKB2-k3.js";
function Pt(e, t = 0) {
...
function rp(e, t) {
  ...
  try {
    ...
    if (m instanceof I) return Wo(e, t?.id, m);
    throw m;
  }
  const o = sr(e);
  ...
  let n = 0;
  const i = [], c = [];
  for (let m = 0; m < a.operations.length; m += 1)
    try {
      const f = Bm({ ui: e, state: r, operation: a.operations[m] });
      n |= f?.dirtyMask || 0, f?.warning && i.push(f.warning), f?.outcome && c.push({ index: m, ...f.outcome });
    } catch (f) {
      if (f instanceof I)
        return (f.operationIndex === null || f.operationIndex === void 0) && (f.operationIndex = m), Wo(e, a.id, f);
      throw f;
    }
  if (a.validateOnly) {
    const { changes: m, truncated: f } = Um(e.state, r);
    return {
      ok: !0,
      version: Je,
      revision: o,
...
const ho = () => import("./chunk-BcC6JFjV.js");
function Ef(e, t) {
...
async function qh(e) {
  ...
  try {
    let n = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: l } = await import("./chunk-D5XBurXg.js");
...
class ai {
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-D5XBurXg.js"));
    } catch (a) {
  ...
  try {
    t.assetBrowser = Ap(t, {
      ...
      onAgentFirstOpen: async () => {
        if (Zo())
          try {
            const { createDirectorAgentPanel: m } = await import("./chunk-DF2yFThn.js");
            t.agentPanel = m(t);
          } catch (m) {
