...
import { s as te, c as P, t as H, a as s, o as vo, b as Ba, d as _e, e as R, r as cr, f as Oe, g as tt, h as xo, i as lr, j as dr, k as mr, S as je, l as pr, m as Wa, n as fr, p as hr, q as Ka, u as br, v as ur, w as yr, M as gr, x as vr, y as xr, z as Ha, A as Q, B as Ce, C as he, D as wr, E as ja, F as _a, G as kr, H as $a, I as Je, J as Sr, K as ao, L as Cr, N as wo, O as jr, P as _r, Q as $r, R as Er, T as Mr, U as Ar, V as Pr, W as Fr, X as zr, Y as Lr, Z as Tr, _ as Kr, $ as ko } from "./chunk-Q1Odh_h6.js";
import { L as Ir, a as Or, f as qr, b as So, c as Dr, d as Nr, e as Rr, s as Br } from "./chunk-BS9OBHCL.js";
import { S as Wr, l as Hr, u as Vr } from "./chunk-CYXHK_as.js";
import { b as Ur } from "./chunk-D2Hci8OZ.js";
function Ze(e, t = 0) {
  return Math.sin(e * 1.7 + t * 3.1) * 0.5 + Math.sin(e * 3.3 + t * 5.7) * 0.3 + Math.sin(e * 7.9 + t * 11.3) * 0.2;
}
function Gr(e, { type: t = "handheld_subtle", intensity: a = 1, duration_frames: o = null, subdivide: r = !0 } = {}) {
  const n = Array.isArray(e) ? e : e?.keyframes || [];
  if (!n || n.length === 0) return n;
  const i = t === "turbulence", c = t === "handheld_heavy", d = (i ? 0.12 : c ? 0.18 : 0.06) * a, l = (i ? 2 : c ? 2.8 : 0.9) * a, p = i ? 0.45 : c ? 0.22 : 0.12, h = n[n.length - 1]?.frame ?? 119, f = Math.max(h + 1, Number(o || (e?.duration_frames ?? h + 1))), u = i ? 4 : c ? 6 : 8, g = Array.isArray(e) ? { keyframes: n, duration_frames: f } : e, S = new Set(n.map((y) => y.frame));
  if (r && f > u) {
    for (let y = 0; y < f; y += u)
      S.add(y);
    S.add(f - 1);
  ...
  return [...S].sort((y, w) => y - w).map((y) => {
    const w = te(g, y), x = Ze(y * p, 1) * d, j = Ze(y * p, 2) * d, _ = Ze(y * p, 3) * d * 0.5, q = Ze(y * p, 4) * l, A = Ze(y * p, 5) * (l * 0.35), B = [...w.position], z = [...w.target];
...
function Xr(e, { duration_frames: t = 120, target: a = [0, 1.5, 0], radius: o = 6, height: r = 3.5 } = {}) {
  const n = [], i = Math.max(2, t), [c, d, l] = a;
  if (e === "orbit_360") {
    const p = Math.max(17, Math.min(65, Math.ceil(i / 4) + 1));
    for (let h = 0; h < p; h++) {
      const f = Math.round(h / (p - 1) * (i - 1)), u = h / (p - 1) * Math.PI * 2, g = h === p - 1 ? [c, d + r, l + o] : [c + Math.sin(u) * o, d + r, l + Math.cos(u) * o];
      n.push({
        frame: f,
        camera: {
          position: g,
...
const Co = 1e-4;
function Yr(e, t) {
  return !Array.isArray(e) || !Array.isArray(t) ? e !== t : e.some((a, o) => Math.abs(Number(a) - Number(t[o])) > Co);
}
function oo(e, t) {
  return Math.abs(Number(e) - Number(t)) > Co;
}
const jo = [
  {
    id: "camera",
  ...
  },
  {
    ...
    label: "Look At",
    color: "#f0a742",
    changed: (e, t) => Yr(e?.target, t?.target),
    read: (e) => e?.target
  },
  {
    id: "focal_length",
...
function Jr(e, t = null) {
  return jo.filter((a) => !t || t.has(a.id)).map((a) => ({
    id: a.id,
    label: a.label,
    color: a.color,
    frames: Zr(e, a)
...
function Ea(e) {
  ...
  for (const a of [".oc-playhead-head", '[data-role="dope-playhead"]', ".oc-gdope-playhead", ".oc-sequence-playhead"])
    for (const o of e.root.querySelectorAll(a)) Qr(o, t);
}
function en(e, t, a) {
...
function _o(e) {
  ...
  a.delete("camera");
  const o = e.timelineKeyframes() || [], r = Jr(o, a), n = an(e, r);
  if (t.dataset.signature !== n) {
    t.dataset.signature = n, t.replaceChildren();
    const i = (c) => H(e, c);
    for (const c of r) {
      const d = document.createElement("div");
      d.className = "oc-dope-row", d.dataset.channel = c.id, d.style.setProperty("--channel-color", c.color), en(d, c.frames, i), tn(e, d, c, o, i), t.appendChild(d);
    }
  }
...
const Ia = [1, 2, 5, 10, 20, 25, 50, 100, 200, 250, 500, 1e3, 2e3, 5e3], $o = 46, ro = 5, Eo = 640;
function Mo(e, t) {
  const a = t > 0 ? t : Eo, o = Math.max(2, Math.floor(a / $o)), r = Math.max(1e-6, e / o);
  return Ia.find((n) => n >= r) ?? Ia[Ia.length - 1];
}
function on(e, t) {
  const a = Math.max(1, e.state.duration_frames - 1), o = P(Number(e.timelineZoom) || 1, 0.1, 50), r = Number(e.timelinePan) || 0, n = a / o, i = Mo(n, t), d = (i >= ro ? i / ro : 0) || i, l = [], p = Math.max(0, Math.floor(r / d) * d);
  for (let f = p; f <= a + 1e-6; f += d) {
    const u = Math.round(f), g = H(e, u);
    if (!(g < -1)) {
      if (g > 101) break;
      l.push({ frame: u, percent: g, major: Math.abs(u % i) < 1e-6 });
    }
  }
  const h = H(e, a);
  if (h <= 101 && !l.some((f) => f.major && f.frame === a)) {
    const f = $o / Math.max(1, t) * 100;
...
function rn(e) {
  return e.clientWidth || e.parentElement?.clientWidth || Eo;
}
function Ao(e) {
  const t = e.root.querySelector('[data-role="ruler"]');
  if (!t) return;
  t.replaceChildren();
  for (const o of on(e, rn(t))) {
...
function nn(e, t) {
  ...
  a.addEventListener("pointerup", i, { signal: t }), a.addEventListener("pointercancel", i, { signal: t }), a.addEventListener("wheel", (c) => vo(e, c), { passive: !1, signal: t });
}
const sn = [1, 2, 2.5, 5, 10];
function cn(e, t = 5) {
  const a = Math.abs(e) / Math.max(1, t);
  if (!(a > 0) || !Number.isFinite(a)) return 1;
  const o = 10 ** Math.floor(Math.log10(a)), r = a / o;
  return (sn.find((n) => n >= r) ?? 10) * o;
}
function ln(e, t) {
  const a = Math.max(0, Math.min(4, Math.ceil(-Math.log10(t))));
  return e.toFixed(a);
}
function dn(e, { left: t, right: a, top: o, width: r, graphWidth: n, graphHeight: i, height: c, timeMin: d, timeMax: l, totalDuration: p, xFor: h, frame: f }) {
  const u = Mo(l - d, n);
  ...
  const g = h(f);
  g >= t && g <= r - a && (e.fillStyle = "#a78bfa", e.fillText(String(f), g, c - 6)), e.textAlign = "left";
}
function mn(e, { left: t, right: a, top: o, width: r, graphHeight: n, minimum: i, maximum: c, yFor: d }) {
  const l = cn(c - i, 4);
  e.strokeStyle = "#222228", e.lineWidth = 1, e.fillStyle = "#6e727a";
  for (let p = Math.ceil(i / l) * l; p <= c; p += l) {
    const h = d(p);
    h < o - 1 || h > o + n + 1 || (e.beginPath(), e.moveTo(t, h), e.lineTo(r - a, h), e.stroke(), e.fillText(ln(p, l), 4, h + 3));
  }
}
function Po(e, t) {
...
function pn(e, t) {
  ...
  const a = t.currentTarget, { x: o, y: r } = Po(a, t);
...
function fn(e, t) {
  ...
  if (e.curvePanDrag && t.pointerId === e.curvePanDrag.pointerId) {
    t.preventDefault();
    const p = t.clientX - e.curvePanDrag.startX, h = t.clientY - e.curvePanDrag.startY, u = Math.max(1, e.state.duration_frames - 1) / (Number(e.curveZoomX) || 1), g = Math.max(1, a.clientWidth - 58), S = 142;
    e.curvePanX = e.curvePanDrag.origPanX - p / g * u, e.curvePanY = e.curvePanDrag.origPanY + h / S * 10 / (Number(e.curveZoom) || 1), e.drawCurveEditor();
    return;
  }
  ...
  if (e.curveBoxSelect && t.pointerId === e.curveBoxSelect.pointerId) {
    ...
    const p = Math.min(e.curveBoxSelect.startX, o), h = Math.max(e.curveBoxSelect.startX, o), f = Math.min(e.curveBoxSelect.startY, r), u = Math.max(e.curveBoxSelect.startY, r), g = (e.curveHitPoints || []).filter((S) => !S.handle && S.x >= p && S.x <= h && S.y >= f && S.y <= u).map((S) => S.key.frame);
    e.selectedKeyFrames = new Set(g), g.length && (e.selectedKeyFrame = g[0]), e.updateKeyVisualState(), e.drawCurveEditor();
    return;
  }
  if (!e.curveDrag || t.pointerId !== e.curveDrag.pointerId) return;
  if (t.preventDefault(), t.stopPropagation(), e.curveDrag.handle) {
    const p = e.curveDrag.key, h = e.curveDrag.channel, f = e.curveDrag.handle, u = e.curveDrag.pixelPerSegment, g = e.curveDrag.valuePerPixel, S = e.curveDrag.keyX, v = e.curveDrag.keyY;
    ...
    if (f === "in") {
      if (x.in_x = P((o - S) / Math.max(1, u), -0.99, -0.01), x.in_y = (v - r) * g, w === "aligned") {
    ...
    } else if (x.out_x = P((o - S) / Math.max(1, u), 0.01, 0.99), x.out_y = (v - r) * g, w === "aligned") {
    ...
    p.tangents.channels[h.id] = x, e.scheduleSerialize(), e.camera = te(e.state, e.frame), e.applyObjectAnimationFrame(), e.render(), e.drawCurveEditor();
    return;
  }
  ...
  if (!t.shiftKey && Math.abs(o - e.curveDrag.startX) > 8 && l !== e.curveDrag.key.frame ? (e.curveDrag.key.frame = l, e.selectedKeyFrame = l, e.frame = l) : (e.editingKeyFrame = e.curveDrag.key.frame, e.frame = e.curveDrag.key.frame), e.curveDrag.object) {
    const p = _e(e.curveDrag.key.transform);
...
function kn(e) {
  ...
  const i = e.timelineObject(), c = e.timelineKeyframes(), d = at(e), l = 44, p = 14, h = 16, f = 22, u = Math.max(1, a - l - p), g = Math.max(1, o - h - f), S = Math.max(1, e.state.duration_frames - 1), v = P(Number(e.curveZoomX) || 1, 0.1, 50), y = Number(e.curvePanX) || 0, w = S / v, x = y, j = y + w, _ = [], q = Math.max(1, Math.ceil(w / Math.max(80, u))), A = (E) => i ? Oe(i, E) : te(e.state, E);
  for (let E = 0; E <= S; E += q) _.push({ frame: E, value: A(E) });
  ...
  z = ne - se / 2, I = ne + se / 2;
  const me = (E) => l + (E - x) / Math.max(1e-6, j - x) * u, pe = (E) => h + g * (I - E) / Math.max(1e-6, I - z);
  if (n.fillStyle = "#111114", n.fillRect(0, 0, a, o), n.strokeStyle = "#222228", n.lineWidth = 1, n.font = "9px system-ui, -apple-system, sans-serif", n.fillStyle = "#6e727a", dn(n, {
    left: l,
    right: p,
    top: h,
    width: a,
    ...
    xFor: me,
    frame: e.frame
  }), mn(n, { left: l, right: p, top: h, width: a, graphHeight: g, minimum: z, maximum: I, yFor: pe }), z <= 0 && I >= 0) {
    const E = pe(0);
  ...
  for (const E of d) {
    ...
    for (const T of c) {
      const F = i ? T.transform : T.camera, N = me(T.frame), ie = pe(E.get(F)), O = T.frame === e.selectedKeyFrame || e.selectedKeyFrames?.has(T.frame);
      O && (n.fillStyle = "rgba(242, 208, 107, 0.35)", n.beginPath(), n.arc(N, ie, 8.5, 0, Math.PI * 2), n.fill()), n.fillStyle = O ? "#ffd75e" : E.color, n.strokeStyle = "#0d0d10", n.lineWidth = 1.6, n.beginPath(), n.arc(N, ie, O ? 5.2 : 3.8, 0, Math.PI * 2), n.fill(), n.stroke(), e.curveHitPoints.push({
        x: N,
        y: ie,
        key: T,
        channel: E,
    ...
    if (e.showCurveHandles)
      for (let T = 0; T < c.length; T++) {
        ...
        const O = i ? F.transform || i : F.camera || F, W = me(F.frame), X = pe(E.get(O)), V = c[T - 1], be = c[T + 1], ue = Math.max(1, F.frame - (V?.frame ?? F.frame - 1)), we = Math.max(1, (be?.frame ?? F.frame + 1) - F.frame), fe = cr(
          F,
          E.id,
          V,
          be,
          (U) => E.get(i ? U.transform || i : U.camera || F)
        ), J = (I - z) / Math.max(1, g), oe = u * we / Math.max(1, w), ce = u * ue / Math.max(1, w), re = [];
  ...
  const Z = me(e.frame);
  Z >= l && Z <= a - p && (n.strokeStyle = "#a78bfa", n.lineWidth = 1.5, n.beginPath(), n.moveTo(Z, h), n.lineTo(Z, h + g), n.stroke(), n.fillStyle = "#a78bfa", n.beginPath(), n.moveTo(Z - 4, h), n.lineTo(Z + 4, h), n.lineTo(Z, h + 6), n.closePath(), n.fill());
...
function no(e, { filter: t, label: a, color: o, title: r }) {
...
function Fo(e) {
  ...
  if (!t) return;
  const a = e.timelineObject(), o = a ? a.name || a.type : e.activeCameraTrack().name, r = Sn(e), n = `${o}\0${r.map((c) => `${c.id}:${c.color}`).join("|")}`;
  if (t.dataset.signature !== n) {
    t.dataset.signature = n, t.replaceChildren();
    const c = document.createElement("span");
    c.className = "oc-graph-legend-title", c.textContent = o, t.appendChild(c), t.appendChild(no(e, {
      filter: "all",
      label: s("All"),
      color: null,
...
function so(e, t, a) {
  ...
  for (const n of [...e].sort((i, c) => i.frame - c.frame)) {
    ...
    (r === null || !(Math.abs(c - r) <= Cn)) && o.push(n.frame), r = c;
  }
  return o;
}
function Va(e) {
  ...
  for (const c of n) {
    ...
    for (const f of so(a, c, o)) {
      const u = H(e, f);
      if (u < -5 || u > 105) continue;
      const g = document.createElement("button");
      g.type = "button", g.className = `oc-dope-key${f === e.frame ? " at-playhead" : ""}${r.has(f) ? " selected" : ""}`, g.style.left = `${u}%`, g.dataset.frame = String(f), g.title = s("{channel} changes at frame {frame}").replace("{channel}", s(c.name)).replace("{frame}", String(f)), g.addEventListener("click", (S) => {
        S.preventDefault(), S.stopPropagation();
        const v = a.find((y) => y.frame === f);
...
const io = ["#4aa3ef", "#f2a93b", "#48c774", "#b565d8", "#ec4899"];
function zo(e, t) {
  const a = e.state.cameras.findIndex((r) => r.id === t), o = a >= 0 ? e.state.cameras[a] : null;
  return { camera: o, color: o?.color || io[Math.max(0, a) % io.length] };
}
function Le(e) {
...
function _n(e, t) {
  ...
  if (a.appendChild(wa(
    ...
    () => {
      e.checkpoint("Split shot"), xo(e.state, e.frame, null) ? Le(e) : e.setStatus(s("Move the playhead inside a shot first"));
    },
    { disabled: o }
  )), a.appendChild(wa(
    s("Clear edit"),
...
function En(e, t, a, o, r) {
  t.preventDefault(), t.stopPropagation();
  const { camera: n } = zo(e, a.camera_id);
  e.contextMenu?.show(t, n?.name || s("Shot"), [
    ...
    null,
    {
      ...
      run: () => {
        e.checkpoint("Split shot"), xo(e.state, e.frame, null) && Le(e);
      }
    },
    {
      label: s("Remove shot"),
      ...
      disabled: r === 1,
      run: () => {
        e.checkpoint("Remove shot"), lr(e.state, o) && Le(e);
      }
    }
  ]);
}
function Mn(e, t, a, o) {
  const { camera: r, color: n } = zo(e, t.camera_id), i = document.createElement("div");
  ...
  if (c.className = "oc-sequence-name", c.textContent = r?.name || t.camera_id, i.appendChild(c), a > 0) {
    ...
    d.className = "oc-sequence-handle", d.title = s("Drag to trim the cut"), d.addEventListener("pointerdown", (l) => $n(e, l, d, o, a)), i.appendChild(d);
  }
...
function An(e) {
  ...
  return o.className = "oc-sequence-waveform", t.appendChild(o), requestAnimationFrame(() => {
    ...
    for (let h = 0; h < a.length; h++) {
      ...
      const g = a[h] * n * 0.9;
      i.fillRect(u, (n - g) / 2, Math.max(1, r / a.length * d - 0.5), g);
    }
  }), t;
}
function To(e, t) {
...
const Pn = ['[data-act="curve-zoom-in"]', '[data-act="curve-zoom-out"]', '[data-act="curve-fit"]', '[data-act="curve-handles"]'], co = { curves: "Graph Editor", dope: "Dope Sheet", sequence: "Sequence" };
function Fn(e, t) {
  const a = t in co ? t : "curves";
  e.graphTab = a;
  ...
  a === "dope" ? Va(e) : a === "sequence" ? (To(e, n), n?.focus?.({ preventScroll: !0 })) : e.drawCurveEditor(), e.setStatus(s(co[a]));
}
function zn(e) {
  e.graphTab === "sequence" && To(e, e.root.querySelector('[data-role="graph-sequence"]'));
}
function Ln(e, t) {
  for (const a of e.root.querySelectorAll("[data-graph-tab]"))
    a.addEventListener("click", (o) => {
      o.preventDefault(), o.stopPropagation(), Fn(e, a.dataset.graphTab);
    }, { signal: t });
}
class Ko {
    ...
    for (const l of o) {
      ...
      p.addEventListener("pointerdown", (u) => u.stopPropagation()), p.addEventListener("mousedown", (u) => u.stopPropagation()), p.addEventListener("click", (u) => {
        ...
        try {
          ...
          console.error("Context menu action failed:", g);
        }
...
async function Ga(e, { route: t, field: a = "file", file: o }) {
...
function Bn() {
  const e = Or.map((t) => `<button data-lens="${t}">${t}mm</button>`).join("");
  return `
  ...
  return `
    <div class="oc-dope">
      <div class="oc-dope-body">
        <div class="oc-dope-labels">${jo.map((t) => `
...
function Xa(e) {
  if (!e?.state?.cameras?.length)
    return Qe(e);
  if (e.state.playblast_camera_id === je) {
    const t = pr(e.state, e.frame), a = t && e.state.cameras.find((o) => o.id === t.camera_id);
    if (a) return a;
  }
...
function is(e) {
  ...
  e.renderRevision = (e.renderRevision || 0) + 1, No(e);
  const t = e.state.playblast_camera_id === je && fr(e.state), a = Xa(e);
...
function ls(e, t = !0) {
  ...
  for (const f of e.state.cameras) {
    ...
    f.keyframes = [...new Map(f.keyframes.map((u) => [u.frame, u])).values()].sort((u, g) => u.frame - g.frame);
  }
  e.state.keyframes = Qe(e).keyframes;
  for (const f of e.state.objects)
    f.keyframes = [...new Map((f.keyframes || []).map((u) => {
      const g = Math.max(0, Math.round(u.frame));
      return [g, { ...u, frame: g }];
    })).values()].sort((u, g) => u.frame - g.frame);
...
function xs(e, t) {
  ...
  for (let r = 1; r < a.length - 1; r++) {
    ...
    for (const d of ys) {
      const l = i.camera?.[d], p = gs(n, i, c, d);
...
function ks(e, t) {
  ...
  !a || !o || (a.addEventListener("input", () => {
    ...
    document.activeElement !== a && (a.value = So(o.value));
  }, { signal: t }));
}
function Ss(e, t) {
  ...
  if (a.length) {
    ...
    for (const o of a)
      o.addEventListener("change", () => {
        e.dopeChannels = new Set(a.filter((r) => r.checked).map((r) => r.dataset.dopeChannel)), _o(e);
      }, { signal: t });
  }
}
function _s(e, t) {
...
const ka = () => import("./chunk-qAgu99h1.js");
function Es(e, t) {
  ka().then(({ loadExchangeFormats: o }) => o(e)), e.root.querySelector('[data-act="import-camera"]')?.addEventListener("click", async () => {
    (await ka()).pickCameraFile(e);
  }, { signal: t }), e.root.querySelector('[data-act="export-camera"]')?.addEventListener("click", async () => {
    (await ka()).exportCamera(e);
  }, { signal: t }), e.root.querySelector('[data-role="camera-file"]')?.addEventListener("change", async (o) => {
    const r = o.target.files?.[0];
    o.target.value = "", await (await ka()).importCameraFile(e, r);
  }, { signal: t });
}
function Ms(e, t) {
  ...
  hr().then((r) => {
    ...
    i === "health-slow" ? br(e) : i === "health-smooth" ? ur(e) : i === "health-recenter" && yr(e);
  }, { signal: t });
  for (const r of e.root.querySelectorAll('[data-tab="health"]'))
    r.addEventListener("click", () => Ka(e), { signal: t });
}
function As(e, t) {
  Es(e, t), ks(e, t), Ss(e, t), Cs(e, t), js(e, t), $s(e, t), _s(e, t), Ms(e, t);
}
const lo = {
...
}, Bo = "balanced", mo = "#121212";
function Ya(e) {
  return lo[e] || lo[Bo];
}
function Ps(e, t = "#2a2d38", a = "#16171d", o = "#0b0c10") {
...
function hl(e, t, a = Bo) {
  const o = Ya(a), r = new e.Group();
  ...
  const f = Ps(e), u = new e.PMREMGenerator(t);
  u.compileEquirectangularShader();
  const g = u.fromEquirectangular(f).texture;
  return {
    group: r,
    key: n,
    fill: c,
    ...
    quality: a,
    dispose() {
      p.geometry.dispose(), p.material.dispose(), h.geometry.dispose(), h.material.dispose(), l.dispose(), f.dispose(), g.dispose(), u.dispose();
      for (const S of [n, c, d]) S.dispose?.();
    }
  };
}
function bl(e, t, a) {
...
function ul(e, t, a, o, r) {
  o.group.visible = r, t.environment = r ? o.environment : null, t.background = r ? o.sky : new e.Color(1184274), a.toneMapping = r ? e.ACESFilmicToneMapping : e.NoToneMapping, a.toneMappingExposure = r ? Ya(o.quality).toneExposure : 1, t.traverse((n) => {
...
function Ma(e, { sourceKind: t = "manual_2d", label: a, keys: o, source: r = {} }) {
  if (!vr.includes(t)) throw new Error(`Unsupported motion source: ${t}`);
  const n = Ks(e), i = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: o.map((c) => ({ visible: !0, interpolation: "linear", ...c })), source: { ...r } };
...
function Is(e, t) {
  return e.motion_tool = gr.includes(t) ? t : "select", e.motion_tool;
}
function Os(e, t) {
  if (xr.includes(t))
...
function Ho(e, t, a, o, r) {
  const n = te(e, a, e.objects);
  let i = t?.point;
  if (t?.object_id) {
    const p = e.objects.find((u) => u.id === t.object_id);
    if (!p) return null;
    const h = Ha(e.objects, p), f = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
...
function qa(e, t) {
  return Ds(t, e.interactionElement);
}
function Bs(e, t) {
  const a = Vo(e.motion_layers, t);
  return a ? (Wo(e, a.id), a) : null;
}
const Da = (e) => {
...
function Ws(e, t) {
  const a = e.state.objects.find((i) => i.id === e.selectedObjectId), o = a ? "object_point" : "world_point", r = a ? { object_id: a.id, local_point: [0, 0, 0] } : { point: e.webgl?.intersectScenePoint?.(t.x * e.canvas.width, t.y * e.canvas.height, e.canvas.width, e.canvas.height) || [...e.camera.target] }, n = Ho(e.state, r, e.frame, e.canvas.width, e.canvas.height) || t;
  return Ma(e.state, { sourceKind: o, label: a ? `${a.name || a.id} Track` : "World Anchor", keys: [{ time_seconds: e.frame / e.state.fps, x: n.x, y: n.y, visible: n.visible !== !1 }], source: r });
}
function Hs(e, t) {
  for (const r of e.root.querySelectorAll("[data-motion-tool]"))
    r.addEventListener("click", () => {
      Is(e.state, r.dataset.motionTool), fo(e), e.render();
    }, { signal: t });
  for (const r of e.root.querySelectorAll("[data-motion-preset]"))
    r.addEventListener("click", () => {
      e.checkpoint("Add camera field"), Ma(e.state, { sourceKind: "camera_field", label: `${r.dataset.motionPreset} Field`, keys: [{ time_seconds: 0, x: 0.5, y: 0.5 }], source: { preset: r.dataset.motionPreset, point: [...e.camera.target] } }), Ae(e, `Camera field: ${r.dataset.motionPreset}`);
    }, { signal: t });
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (r) => {
    const n = Oa(e.state);
    n && (e.checkpoint("Set motion interpolation"), Os(n, r.target.value), Ae(e, `Motion interpolation: ${r.target.value}`));
    ...
    e.checkpoint("Set motion visibility"), c.visible = r.target.checked, Ae(e, `Motion key ${c.visible ? "visible" : "hidden"}`);
  }, { signal: t });
  for (const r of e.root.querySelectorAll("[data-motion-layer-action]"))
    r.addEventListener("click", () => {
      ...
      if (e.checkpoint(i === "delete" ? "Delete motion layer" : i === "retime" ? "Retime motion layer" : "Toggle motion layer"), i === "delete") Wo(e.state, n.id);
      else if (i === "retime") {
        const [c, d] = po(e);
        qs(n, c, d);
      } else n.enabled = !n.enabled;
      Ae(e, i === "delete" ? "Motion layer deleted" : i === "retime" ? "Motion layer retimed" : `Motion layer ${n.enabled ? "enabled" : "disabled"}`);
    }, { signal: t });
  const a = e.interactionElement;
  a.addEventListener("pointerdown", (r) => {
    const n = e.state.motion_tool;
    if (n === "select" || r.button !== 0) return;
    Da(r), a.setPointerCapture?.(r.pointerId);
    const i = qa(e, r);
    if (n === "track") {
      e.checkpoint("Draw motion track"), e.motionTrackDraft = { pointerId: r.pointerId, points: [i] };
      return;
    }
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? Ma(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...i, interpolation: "hold" }] }) : n === "project" ? Ws(e, i) : n === "erase" && Bs(e.state, i), Ae(e, `Motion tool: ${n}`);
  }, { capture: !0, signal: t }), a.addEventListener("pointermove", (r) => {
    e.motionTrackDraft?.pointerId === r.pointerId && (Da(r), e.motionTrackDraft.points.push(qa(e, r)), e.render());
  }, { capture: !0, signal: t });
  const o = (r) => {
    const n = e.motionTrackDraft;
    if (n?.pointerId !== r.pointerId) return;
    Da(r), e.motionTrackDraft = null;
    const [i, c] = po(e), d = Rs(e.state, n.points, i, c);
    Ae(e, d ? `Motion track: ${d.label}` : "Motion track needs a longer stroke");
  };
  a.addEventListener("pointerup", o, { capture: !0, signal: t }), a.addEventListener("pointercancel", o, { capture: !0, signal: t }), a.addEventListener("click", (r) => {
    ...
    const n = Vo(e.state.motion_layers, qa(e, r));
...
function Vs(e, t, a) {
  ...
  }, { signal: a }), nn(e, a), Ln(e, a), t('[data-role="curve-group"]')?.addEventListener("change", () => {
    e.setChannelFilter("all"), Fo(e), e.drawCurveEditor(), Va(e);
    ...
    if (!(!h || p.button === 2 || l.closest(".scene-action-btn")))
      if (h.dataset.objectId) {
        ...
        for (const u of e.root.querySelectorAll(".scene-item")) {
          const g = !!(u.dataset.objectId && e.selectedObjectIds.has(u.dataset.objectId));
          u.classList.toggle("selected", g), u.setAttribute("aria-selected", String(g));
        }
  ...
  n && (n.addEventListener("pointerdown", (l) => e.onTimelinePointerDown(l), { signal: a }), n.addEventListener("pointermove", (l) => e.onTimelinePointerMove(l), { signal: a }), n.addEventListener("pointerup", (l) => e.onTimelinePointerUp(l), { signal: a }), n.addEventListener("pointercancel", (l) => e.onTimelinePointerUp(l), { signal: a }), n.addEventListener("wheel", (l) => vo(e, l), { passive: !1, signal: a }));
  const i = (l) => {
...
function Us(e) {
  ...
  Hs(e, t), us(e, a, t), Ts(e, a, t), Vs(e, a, t), As(e, t);
}
const Te = [
  "#4aa3ef",
  // Camera 1 - Blue/Cyan
...
function Gs(e) {
  ...
  for (const a of e.root.querySelectorAll('[data-role="playblast-camera"]')) {
    ...
    o.value = je, o.textContent = t.length ? s("Sequence ({count} shots)").replace("{count}", String(t.length)) : s("Sequence (no shots yet)"), o.disabled = t.length === 0, a.appendChild(o), a.value = e.state.playblast_camera_id;
  }
...
function Za(e) {
  ...
  const i = Xs(e), c = i.map((l) => `${l.id}:${l.name}:${l.muted ? 1 : 0}:${l.solo ? 1 : 0}:${l.color || ""}`).join("|");
  let d = !1;
  c !== e.cameraPreviewSignature && (d = !0, e.cameraPreviewSignature = c, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), i.forEach((l, p) => {
    ...
    x.className = "camera-view-badge", x.textContent = s("CAMERA PREVIEW"), u.append(g, S, v, y), h.append(w, u, x), t.appendChild(h), h.addEventListener("click", () => {
...
function Ys(e) {
  ...
  const t = Uo(e.state), a = e.state.cameras.length, o = `Camera ${a + 1}`, r = R(e.camera), n = [
...
async function Zs(e, t) {
  ...
  if (!a) return;
  const o = (await Ua(s("Rename camera"), s("Camera name"), a.name))?.trim();
...
async function Qs(e, t) {
  ...
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a || !await Oo(s("Delete camera"), s(`Delete ${a.name} and its ${a.keyframes.length} keyframe(s)?`))) return;
...
function ti(e, t) {
  const a = tt(e.state), o = t === je && a.length > 0, r = o ? null : e.state.cameras.find((n) => n.id === t);
  !o && !r || (e.state.playblast_camera_id = o ? je : r.id, e.refreshCameraSelectors(), e.serialize(), e.refreshObjects(), e.renderCameraView(), e.setStatus(o ? s("Playblast: sequence ({count} shots)").replace("{count}", String(a.length)) : s(`Playblast: ${r.name}`)));
}
function ai(e) {
...
function Go(e, t) {
  e.state.maximized_camera_id = e.state.maximized_camera_id === t ? null : t, e.serialize(), Za(e), requestAnimationFrame(() => {
    e.resizeCanvas(), e.renderCameraView();
...
const ii = ["video/mp4;codecs=avc1.42E01E", "video/webm;codecs=vp9", "video/webm;codecs=vp8", "video/webm"];
async function ci({
  canvas: e,
  fps: t,
  frameCount: a,
  ...
  try {
    ...
    p.stop(), await f;
    const g = Math.max(0, i() - u), S = a / t * 1e3, v = {
...
async function Xo(e) {
  await di(e.cardMediaById.values());
}
async function Yo(e) {
  return ci({
    canvas: e.canvas,
...
async function Zo(e, t) {
  const a = await li(Ie, t);
  if (si(e, t), e.state.playblast_camera_id === je)
...
async function pi(e) {
  ...
  const t = e.frame, a = e.canvas.width, o = e.canvas.height, r = mi(e);
  (r.width !== e.canvas.width || r.height !== e.canvas.height) && (e.canvas.width = r.width, e.canvas.height = r.height, e.render());
  try {
    let n = null;
    const i = e.root.querySelector('[data-role="encoder"]').value, { encodeDeterministicPlayblast: c, supportsDeterministicEncoding: d } = await import("./chunk-CJPsn7W5.js");
    i !== "realtime" && await d(e.canvas.width, e.canvas.height) && (n = await c(e.canvas, e.state.duration_frames, e.state.fps, async (l) => {
      e.setFrame(l, !0), e.setStatus(s(`Encoding frame ${l + 1}/${e.state.duration_frames}…`)), await Xo(e), await new Promise((p) => requestAnimationFrame(p));
...
function fi(e) {
  ...
  const p = (h) => {
    if (e.playing) {
      for (l += h - d, d = h; l >= c; )
        if (l -= c, n += 1, n > r) {
          if (!e.state.loop_playback) return void Na(e);
          if (n = o, e.audioBuffer && e.audioContext)
            try {
              ...
              const u = o / Math.max(1, e.state.fps), g = Math.max(0, Math.min(t - u, (r - o) / Math.max(1, e.state.fps)));
              u < t && u < e.audioBuffer.duration && g > 0 && (f.start(0, u, g), e.audioSource = f);
            } catch {
            }
        }
...
function bi(e, t) {
  ...
  const a = e.activeCameraTrack(), o = Xr(t, {
...
function ui(e, t) {
  ...
  const o = Gr(a, { type: t, intensity: 1, duration_frames: e.state.duration_frames });
...
async function Qo(e) {
  ...
  return Ga(et, { route: "/majoor/omnicam/upload_asset", field: "asset", file: e });
}
async function Aa(e) {
...
async function vi(e, t) {
  ...
  try {
    ...
    if (o && await Aa([o]), a !== e.backgroundRequestId || e.disposed) return;
...
async function xi(e, t) {
  ...
  try {
    ...
    if (await Aa(o), a !== e.backgroundRequestId || e.disposed) return;
    e.setStatus(`Failed to load BG sequence: ${r.message || r}`);
  }
}
function wi(e) {
  Ja(e), e.state.viewport_bg_image = "", e.state.viewport_bg_sequence = [], e.viewportBgImage = null, e.viewportBgSequenceImages = [], e.serialize(), e.render(), e.setStatus("Background cleared");
}
...
function Si(e) {
  const { points: t, colors: a } = wr(e.state.point_density || "balanced", e.state.point_spread || "all_views", e.state.point_color || null);
  if (!t.length) return;
...
function Ei(e, t) {
  const [a, o, r] = t.position || [0, 1.5, 0], [n, i] = t.size || [2, 3], c = e.viewportCamera(), d = [
    ...
    [a + n / 2, o + i / 2, r],
    [a - n / 2, o + i / 2, r]
  ].map((v) => Q(v, c, e.canvas.width, e.canvas.height));
  if (d.some((v) => !v)) return;
  const l = d.map((v) => v[0]), p = d.map((v) => v[1]), h = Math.min(...l), f = Math.max(...l), u = Math.min(...p), g = Math.max(...p);
  e.ctx.save(), e.ctx.beginPath(), e.ctx.moveTo(d[0][0], d[0][1]);
  for (let v = 1; v < 4; v++) e.ctx.lineTo(d[v][0], d[v][1]);
  e.ctx.closePath(), e.ctx.clip();
  const S = e.cardMediaById.get(t.id) || (t.id === "subject" ? e.cardMedia : null);
  if (S)
    try {
      const v = Math.max(1, f - h), y = Math.max(1, g - u), w = S.videoWidth || S.naturalWidth || S.width, x = S.videoHeight || S.naturalHeight || S.height, j = e.state.card_fit || "contain";
      if (e.ctx.fillStyle = "#111", e.ctx.fillRect(h, u, v, y), j === "stretch" || !w || !x)
        e.ctx.drawImage(S, h, u, v, y);
      else if (j === "contain") {
        const _ = Math.min(v / w, y / x), q = w * _, A = x * _;
        e.ctx.drawImage(S, h + (v - q) / 2, u + (y - A) / 2, q, A);
      } else {
        const _ = Math.max(v / w, y / x), q = v / _, A = y / _;
        e.ctx.drawImage(S, (w - q) / 2, (x - A) / 2, q, A, h, u, v, y);
      }
    } catch {
    }
  else
    e.ctx.fillStyle = "#3a414b", e.ctx.fillRect(h, u, f - h, g - u), e.ctx.fillStyle = "#d8d8d8", e.ctx.textAlign = "center", e.ctx.font = `${Math.max(12, Math.min(28, (f - h) * 0.08))}px system-ui`, e.ctx.fillText("SUBJECT CARD", (h + f) / 2, (u + g) / 2);
...
function Ai(e) {
  ...
  for (let o = 0; o < e.state.keyframes.length - 1; o++) {
    ...
    t.push(ja(_a(n.camera.position, r.camera.position)) * e.state.fps / Math.max(1, n.frame - r.frame));
  }
...
function Li(e, t, a, o) {
  ...
  const r = Math.min(138, Math.max(80, Math.min(a, o) - 20)), n = 10, i = Math.max(0, a - r - n), c = Math.max(0, o - r - n), d = e.viewportCamera(), l = d?.position || [0, 1.5, 5], p = d?.target || [0, 0, 0], h = ["#4aa3ef", "#f2a93b", "#48c774", "#b565d8", "#ec4899"], f = e.activeCameraTrack?.(), u = e.state.cameras?.length ? e.state.cameras : f ? [f] : [], g = e.state.active_camera_id || f?.id, S = u.flatMap(
    (O) => (O.keyframes || []).map((W) => W.camera?.position).filter(Boolean)
  ), v = Math.max(
    Math.abs(l[0] || 0),
    Math.abs(l[2] || 0),
    Math.abs(p[0] || 0),
    Math.abs(p[2] || 0),
    ...S.flatMap((O) => [Math.abs(O[0] || 0), Math.abs(O[2] || 0)]),
    4
  ), y = Math.max(6, Math.ceil((v + 1.5) / 4) * 4), w = i + r / 2, x = c + r / 2, j = r / 2 - 10, _ = j / y, q = (O, W) => [w + O * _, x + W * _];
  ...
  for (const O of e.state.objects || []) {
    ...
    const W = Ha(e.state.objects, O).position || [0, 0, 0], [X, V] = q(W[0], W[2]);
    X < i + 2 || X > i + r - 2 || V < c + 2 || V > c + r - 2 || (t.fillStyle = O.type === "card" ? "#48dbfb" : O.type === "human" ? "#ff9ff3" : "#feca57", t.beginPath(), t.arc(X, V, 2.5, 0, Math.PI * 2), t.fill());
  }
  for (let O = 0; O < u.length; O++) {
    const W = u[O], X = W.keyframes || [], V = W.id === g, be = W.color || h[O % h.length];
  ...
  const I = w + l[0] * _, ye = x + l[2] * _, ae = 8, ne = P(I, i + ae, i + r - ae), se = P(ye, c + ae, c + r - ae), me = w + p[0] * _, pe = x + p[2] * _, Z = P(me, i + ae, i + r - ae), E = P(pe, c + ae, c + r - ae);
...
function Ti(e) {
  ...
  if (e.audioWaveformPeaks && e.audioWaveformPeaks.length) {
    ...
    const w = y.getContext("2d"), x = e.audioWaveformPeaks, j = y.width, _ = y.height, q = _ / 2;
    w.fillStyle = "#f2d06b";
    for (let A = 0; A < x.length; A++) {
      const z = (A / (x.length - 1) * r - d) / Math.max(1e-6, c) * j;
      if (z >= -5 && z <= j + 5) {
        const I = x[A] * (_ * 0.85);
        w.fillRect(z, q - I / 2, Math.max(1, j / x.length * n - 0.5), I);
      }
    }
    t.appendChild(y);
  }
  ...
  for (const y of o) {
    ...
    j.className = "key-label", j.textContent = String(y.frame), x.appendChild(j), x.addEventListener("pointerdown", (_) => {
      if (_.preventDefault(), _.stopPropagation(), x.focus({ preventScroll: !0 }), _.altKey) {
        e.checkpoint("Duplicate keyframe");
        const A = a ? { frame: y.frame, transform: _e(y.transform), interpolation: y.interpolation } : { frame: y.frame, camera: R(y.camera), interpolation: y.interpolation }, B = e.timelineKeyframes();
      ...
      const q = e.timelineKeyframes().filter((A) => e.selectedKeyFrames.has(A.frame));
      e.keyDrag = { key: y, box: t, moving: q.map((A) => ({ key: A, startFrame: A.frame })), startPointerFrame: y.frame, startClientX: _.clientX, startClientY: _.clientY }, e.setFrame(y.frame, !1, !1);
  ...
  v && (v.textContent = p.name), Ao(e), _o(e), Fo(e), Va(e), zn(e), e.refreshCameraSelectors(), e.refreshKeyEditor(), e.updateEditState(), e.drawCurveEditor();
}
function Pa(e) {
...
function qi(e) {
  const t = Pa(e), a = e.state.view_mode !== "camera", o = [...t.target];
  if (e.subSelection?.point) {
    e.checkpoint("Frame selection");
    const g = e.subSelection.point, S = $a(_a(t.position, o)), v = Number.isFinite(S[0]) && ja(S) > 0.1 ? S : [0.707, 0.4, 0.707], y = 2;
    t.target = [...g], t.position = he(t.target, Je(v, y)), a ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const w = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(s(`Focused on ${w} at [${g.map((x) => Math.round(x * 100) / 100).join(", ")}]`));
    return;
  }
  const n = e.selectedObject() || e.state.objects.find((g) => g.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] }, i = n.size || [1, 1, 1], c = Math.max(i[0] || 1, i[1] || 1, i[2] || 1), d = (t.fov || 35) * Math.PI / 360, l = Math.max(2, c / Math.max(0.1, Math.tan(d)) * 0.9), p = $a(_a(t.position, o)), h = Number.isFinite(p[0]) && ja(p) > 0.1 ? p : [0.707, 0.4, 0.707], u = (n.type === "model" || n.type === "glb" ? e.webgl?.getObjectWorldCenter?.(n.id) : null) || (n.keyframes?.length ? Oe(n, e.frame).position : n.position || [0, 1.5, 0]);
...
function er(e, t, a) {
  ...
  return e.state.gizmo_space === "local" ? o.map((n) => Sr(n, r)) : o;
}
function Di(e) {
  if (e.selectedEntity === "object") {
    const t = e.selectedObject();
    if (!t || t.locked) return null;
    const a = t.type === "model" || t.type === "glb" ? e.webgl?.getObjectWorldCenter?.(t.id) : null, o = t.keyframes?.length ? Oe(t, e.frame) : t, r = a || o.position || [0, 0, 0];
    return {
      type: "object",
      object: t,
      position: r,
...
function Qa(e) {
  ...
  const n = Math.max(0.7, ja(_a(a.position, o)) * 0.12), i = t.type === "object" ? er(e, t.object, t) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      entity: t,
      center: r,
      worldLength: n,
      handles: i.map((d, l) => ({ index: l, axis: d, points: [r, Q(he(o, Je(d, n)), a, e.canvas.width, e.canvas.height)] })).filter((d) => d.points[1] && Number.isFinite(d.points[1][0]) && Number.isFinite(d.points[1][1]))
    };
  const c = i.map((d, l) => {
    const p = Math.abs(d[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], h = $a(ao(d, p)), f = $a(ao(d, h)), u = [];
    for (let g = 0; g <= 48; g++) {
      const S = g / 48 * Math.PI * 2, v = Q(he(o, he(Je(h, Math.cos(S) * n), Je(f, Math.sin(S) * n))), a, e.canvas.width, e.canvas.height);
...
function Ni(e, t) {
  ...
  for (const c of a.handles)
    for (let d = 0; d < c.points.length - 1; d++) {
      const l = c.points[d], p = c.points[d + 1], h = Cr(t, l, p);
...
function Ri(e, t) {
  ...
  for (const i of e.state.objects) {
    ...
    const c = i.keyframes?.length ? Oe(i, e.frame) : i, d = Q(c.position || [0, 0, 0], o, e.canvas.width, e.canvas.height);
    if (!d) continue;
...
function Wi(e) {
  ...
  for (const r of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 7 : 4, e.ctx.strokeStyle = n ? "#ffffff" : a[r.index] || "#ffffff", e.ctx.fillStyle = a[r.index] || "#ffffff", e.ctx.beginPath(), r.points.forEach((i, c) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? e.ctx.fillRect(c[0] - 6, c[1] - 6, 12, 12) : Bi(e.ctx, i[0], c);
    }
  }
...
function ho(e, t, { frameCount: a = 0, fps: o = 0 } = {}) {
...
async function Gi(e, t) {
  ...
  try {
    const i = await Ga(Ke, { route: "/majoor/omnicam/upload_model", field: "asset", file: t });
...
async function Xi(e, t) {
  ...
  try {
    const o = await Ga(Ke, { route: "/majoor/omnicam/upload_asset", field: "asset", file: t });
...
async function Ji(e) {
  ...
  for (const h of i) {
    ...
    if (u) {
      if (f === "image" || f === "video") {
        c = !0;
        const g = u.widgets?.find(
          (S) => ["image", "image_path", "upload", "file", "filename", "video", "video_path"].includes(String(S.name).toLowerCase())
        );
        if (g && g.value) {
          const S = String(g.value), v = /\.(mp4|mov|webm|mkv|m4v|avi)(?:\s|$)/i.test(S), y = u.widgets?.find((j) => String(j.name).toLowerCase() === "subfolder")?.value || "", w = Ce(Pe(S, y)), x = e.state.objects.find((j) => j.id === "subject");
          if (x) {
            const j = await ar(e, x, w, r, v);
            if (!r()) return;
            x.asset = Pe(S, y);
            let _ = null;
            if (v)
              try {
                _ = await Hi(S, o.signal);
              } catch (q) {
                if (q?.name === "AbortError") return;
                console.warn("Failed to describe upstream video:", q);
              }
            r() && ho(e, j, {
              fps: _?.fps,
          ...
          }), e.cardMediaById.set("subject", S), e.cardMedia = S, ho(e, S, { frameCount: S instanceof HTMLVideoElement ? 0 : 1 }), e.upstreamImageConnected = !0, n = !0, e.render(), e.setStatus(S instanceof HTMLVideoElement ? s("Upstream video preview synced") : s("Upstream image preview synced")));
        }
      }
      if (f === "audio") {
        d = !0;
        ...
        if (g && g.value) {
          const S = String(g.value), v = u.widgets?.find((w) => String(w.name).toLowerCase() === "subfolder")?.value || "", y = Ce(Pe(S, v));
          try {
      ...
      if (f === "scene_3d" || f === "model" || f === "mesh") {
        ...
        if (g && g.value) {
          const S = String(g.value), v = S.split(".").pop()?.toLowerCase();
          if (["glb", "gltf", "obj", "fbx", "stl", "ply"].includes(v)) {
            const y = u.widgets?.find((_) => String(_.name).toLowerCase() === "subfolder")?.value || "", w = Ce(Pe(S, y)), x = `upstream_scene_${u.id}`;
            l.add(x);
            let j = e.state.objects.find((_) => _.id === x);
            j ? (j.asset = Pe(S, y), j.format = v === "gltf" ? "glb" : v) : (j = {
              id: x,
              type: "model",
              format: v === "gltf" ? "glb" : v,
  ...
  Br(e) && (n = !0), n && (e.serialize(), e.render());
}
async function Qi(e, { signal: t } = {}) {
...
async function tc(e) {
  ...
  const r = o.diagnostic?.issues || [], n = ec(r);
...
function oc(e, t, a) {
  ...
  return o || (t.keyframes?.length ? Oe(t, a).position : t.position || [0, 1.5, 0]);
}
function Ra(e, t, a, o) {
  if (!a) return a;
  const r = rr(e, t, o);
...
function nc(e, { perFrame: t = !1 } = {}) {
  const a = e.activeCameraTrack(), o = eo(e, a), r = to(e, a);
...
function ic(e) {
  ...
  for (const n of e.state.cameras) {
    ...
    if (h.className = "scene-item-label", l || c) {
      ...
      g.style.cssText = `color:${l ? "#f59e0b" : "#58cc6b"};font-weight:700`, g.textContent = l ? "● " : "○ ", h.appendChild(g);
    }
    if (h.appendChild(document.createTextNode(n.name)), d) {
      const g = document.createElement("span");
      g.style.cssText = "color:#f2d06b;font-size:10px", g.title = "Playblast Output", g.textContent = " ★", h.appendChild(g);
    }
    if (n.muted) {
      const g = document.createElement("span");
      g.style.opacity = ".6", g.textContent = " (muted)", h.appendChild(g);
    }
    ...
    })), f.appendChild(a("pi-ellipsis-v", "Camera actions", !1, (g) => e.openCameraContext(g, n.id, !1))), i.append(p, h, f), i.title = l ? s("Currently selected for editing") : d ? s("Active playblast camera") : s("Click to select & activate this camera");
    const u = () => {
    ...
    i.addEventListener("contextmenu", (g) => {
      g.preventDefault(), g.stopPropagation(), e.openCameraContext(g, n.id, !1);
  ...
  for (const n of e.state.objects) {
    ...
    g.className = "scene-item-actions", g.appendChild(a(l ? "pi-eye" : "pi-eye-slash", l ? "Hide object" : "Show object", !l, () => e.toggleObject(n.id), "color:#ef4444;opacity:.7")), g.appendChild(a("pi-lock", "Lock object", n.locked, () => {
      e.checkpoint("Lock object"), n.locked = !n.locked, e.serialize(), e.refreshObjects();
    })), g.appendChild(a("pi-ellipsis-v", "Object actions", !1, (v) => e.openObjectContext(v, n.id))), i.append(h, f, g), i.title = s("Click to select · Double-click to toggle visibility · Right-click for actions");
    const S = (v = {}) => {
...
async function lc(e, t) {
  ...
  if (!a) return;
  const o = (await Ua(e.app, s("Rename object"), s("Object name"), a.name || a.type))?.trim();
...
async function pc(e, t) {
  ...
  const a = e.state.objects.find((o) => o.id === t);
  if (a && await Oo(e.app, s("Delete object"), s(`Delete ${a.name || a.type} and its ${(a.keyframes || []).length} keyframe(s)?`))) {
...
function hc(e) {
  ...
  ["camera-px", "camera-py", "camera-pz", "camera-tx", "camera-ty", "camera-tz", "camera-fov", "camera-roll", "camera-near", "camera-far"].forEach((v, y) => {
  ...
  if (!t) {
    ...
    v && (v.textContent = `${r.name} · F${e.frame}`), bo(o, {
  ...
  d && (d.textContent = t.name || t.type), bo(o, {
  ...
  const f = o('[data-role="animation-row"]'), u = o('[data-role="animation-select"]'), g = o('[data-role="object-parent"]');
  if (g) {
    const v = t.id;
    g.innerHTML = "";
    const y = document.createElement("option");
    y.value = "", y.textContent = s("No parent"), g.appendChild(y);
    ...
    for (const j of e.state.objects) {
      ...
      _.value = j.id, _.textContent = j.name || j.type, g.appendChild(_);
    }
    g.value = t.parent_id || "";
  }
  ...
  if (f && (f.hidden = !S?.animations), u) {
    u.innerHTML = "";
    for (const [v, y] of (S?.animationNames || []).entries()) {
      const w = document.createElement("option");
      w.value = String(v), w.textContent = y, u.appendChild(w);
    }
...
function Sc(e) {
  ...
  const t = e.root.querySelector('[data-role="key-interp"]')?.value || e.root.querySelector('[data-role="interp"]')?.value || "ease", a = ve(e), o = xe(e), r = a ? { frame: e.frame, transform: _e(a), interpolation: t } : { frame: e.frame, camera: R(e.camera), interpolation: t }, n = o.findIndex((i) => i.frame === e.frame);
...
function jc(e) {
  ...
  const o = $e(e) || a.find((i) => i.frame === e.frame);
...
function _c(e) {
  const t = ve(e), a = $e(e) || xe(e).find((o) => o.frame === e.frame);
  e.copiedKeyframe = t ? { kind: "object", transform: _e(a?.transform || t), interpolation: a?.interpolation || e.root.querySelector('[data-role="interp"]')?.value || "ease" } : { kind: "camera", camera: R(a?.camera || e.camera), interpolation: a?.interpolation || e.root.querySelector('[data-role="interp"]')?.value || "ease" }, e.setStatus(s(`Keyframe copied @ ${a?.frame ?? e.frame}`));
}
function $c(e) {
  ...
  const o = t ? { frame: e.frame, transform: _e(e.copiedKeyframe.transform), interpolation: e.copiedKeyframe.interpolation } : { frame: e.frame, camera: R(e.copiedKeyframe.camera), interpolation: e.copiedKeyframe.interpolation }, r = xe(e), n = r.findIndex((i) => i.frame === e.frame);
...
const Lc = ["guides", "safe-areas", "resolution-gate", "aspect-ratio"];
function Tc(e) {
...
function Ic(e) {
  const t = ve(e), a = $e(e), o = e.root.querySelector('[data-role="key-editor"]');
...
function qc(e) {
  ...
  if (e.editingKeyFrame = t.frame, ve(e)) {
    t.interpolation = e.root.querySelector('[data-role="key-interp"]').value, t.transform = _e(ve(e)), e.serialize(), e.setFrame(t.frame), e.setStatus(s(`Object keyframe updated @ ${t.frame}`));
    return;
  }
  const a = (o, r) => {
...
function Bc(e) {
  const { app: t, api: a, EditorHistory: o, ContextMenuController: r, initializeTooltips: n, promptText: i, ObjectUrlRegistry: c, buildRoot: d, dispatchDirectorKey: l, activeCameraTrack: p, bindWidgetCallbacks: h, playblastCameraTrack: f, restoreFromWidgets: u, serializeEditorState: g, syncActiveCameraTrack: S, syncFromWidgets: v, bind: y, activateCamera: w, addCamera: x, deleteCamera: j, drawPreviewOverlays: _, duplicateCamera: q, maximizeCameraPreview: A, refreshCameraPreviews: B, refreshCameraSelectors: z, renameCamera: I, setPlayblastCamera: ye, toggleCameraView: ae, captureRealtime: ne, makePlayblast: se, uploadDirectorPlayblast: me, waitForMediaFrame: pe, computeAudioPeaks: Z, loadAudioFile: E, stopPlay: Y, togglePlay: T, applyCameraPreset: F, applyCameraShake: N, applyProxyPreset: ie, clearViewportBgImage: O, loadViewportBgFile: W, loadViewportBgSequence: X, drawCameraPath: V, drawCard: be, drawCube: ue, drawGrid: we, drawHuman: fe, drawLine3D: J, drawNull: oe, drawOverlays: ce, drawPointField: re, drawSpeedHeatmap: U, drawSphere: ge, curveChannels: rt, drawCurveEditor: qe, onCurvePointerDown: De, onCurvePointerMove: Ne, onCurvePointerUp: Re, onTimelinePointerDown: nt, onTimelinePointerMove: st, onTimelinePointerUp: it, refreshKeys: Be, resetCurveZoom: We, resetTimelineZoom: Me, setChannelFilter: He, setCurveInterpolation: Ve, setTangentMode: Ue, timelineFrameFromEvent: ct, toggleCurveHandles: Ge, zoomCurve: lt, drawTransformGizmo: dt, frameTarget: mt, gizmoAxes: pt, gizmoGeometry: ft, onPointerDown: ht, onPointerMove: bt, onPointerUp: ut, onWheel: yt, pickGizmo: gt, pickSceneObject: vt, resetCamera: xt, setTransformMode: wt, setViewMode: kt, viewportCamera: St, loadCardFile: Ct, loadExecutionPreview: jt, loadMediaUrl: _t, loadModelFile: $t, loadSelectedReference: Et, onModelLoaded: Mt, restoreAssets: At, syncUpstreamInputs: Pt, configureDomMedia: Fa, refreshSetupDiagnostic: Ft, addMediaCard: zt, addPrimitive: Lt, applyObjectAnimationFrame: Tt, beginCameraEdit: Kt, beginObjectEdit: It, commitCameraEdit: Ot, commitObjectEdit: qt, copyKeyframe: Dt, deleteKeyframe: Nt, deleteObject: Rt, duplicateObject: Bt, exitKeyEdit: Wt, finishCameraEdit: Ht, goToAdjacentKey: Vt, insertKeyframe: Ut, loadSelectedKeyView: Gt, pasteKeyframe: Xt, playblastCameraAtFrame: Yt, refreshInspector: Zt, refreshKeyEditor: Jt, refreshObjects: Qt, removeObjectResources: ea, renameObject: ta, retimeSelectedKey: aa, selectKeyframe: oa, selectedKeyframe: ra, selectedObject: na, selectObjectAnimation: sa, setKeyInterpolation: ia, setObjectParent: ca, timelineKeyframes: la, timelineObject: da, toggleAutoKey: ma, toggleObject: pa, updateCameraFromHud: fa, updateEditState: ha, updateKeyVisualState: ba, updateSelectedKey: ua, updateSelectedObject: ya, clamp: Xe, cloneCamera: za, configureCore: La, defaultCamera: ga, sampleCamera: ke, sampleObjectTransform: Ee, sanitizeState: va, worldTransform: xa } = e;
  return {
    setSelectMode(m) {
    ...
    restoreHistorySnapshot(m) {
      const b = JSON.parse(m), k = uo(this.state), C = new Set(this.state.objects.map(($) => $.id));
    ...
    resizeCanvas() {
      ...
      for (const G of this.cameraPreviewCanvases.values()) {
        const le = G.clientWidth || 220, L = G.clientHeight || 124, $ = Math.max(b, Rc / Math.max(1, le)), Se = Math.max(1, Math.round(le * $)), Ye = Math.max(1, Math.round(L * $));
...
function Vc(e) {
  ...
  return {
    ...
    setFrame(m, b = !1, k = !0) {
      this.frame = Xe(Math.round(m), 0, this.state.duration_frames - 1), this.editingKeyFrame !== this.frame && (this.editingKeyFrame = null), this.camera = ke(this.activeCameraTrack(), this.frame, this.state.objects), Ra(this, this.activeCameraTrack(), this.camera, this.frame), this.applyObjectAnimationFrame();
      ...
      for (const $ of this.root.querySelectorAll('[data-role="camera-focal"]')) document.activeElement !== $ && ($.value = So(this.camera.fov));
    ...
    playblastCameraAtFrame() {
      return Ra(this, f(this), Yt(this, ke), this.frame);
    },
    viewportCamera() {
      return St(this);
    },
    ...
    aimAtSelectedObject(m) {
      ...
      const M = (k.type === "model" || k.type === "glb" ? this.webgl?.getObjectWorldCenter?.(k.id) : null) || (k.keyframes?.length ? Ee(k, this.frame).position : k.position || [0, 1.5, 0]);
    ...
    bakeAimToKeyframes() {
      ...
      for (const M of m.keyframes) {
        const K = (k.type === "model" || k.type === "glb") && C && !k.keyframes?.length ? C : k.keyframes?.length ? Ee(k, M.frame).position : k.position || [0, 1.5, 0];
        M.camera.target = [...K];
      }
...
function Xc(e) {
  const { right: t, up: a, forward: o } = jr(e || {});
  return Gc.map((r) => {
...
const Jc = "http://www.w3.org/2000/svg", Fe = 26, yo = 17, Qc = 5.4;
function ze(e, t) {
...
function el(e) {
  ...
  for (const n of Yc(Xc(a))) {
    const i = Fe + n.x * yo, c = Fe + n.y * yo, d = Zc(n.depth);
...
function al(e) {
  ...
  for (const r of e.state.motion_layers || []) {
    ...
    const n = tl(e.state, r, e.frame, a, o);
    if (n.length) {
  ...
  return {
    render() {
      ...
      if (m.fillStyle = this.state.viewport_bg_color || "#121212", m.fillRect(0, 0, b, k), this.viewportBgSequenceImages && this.viewportBgSequenceImages.length) {
        const D = this.frame % this.viewportBgSequenceImages.length, Ta = this.viewportBgSequenceImages[D];
        if (Ta?.complete && Ta.naturalWidth)
          try {
            m.drawImage(Ta, 0, 0, b, k);
          } catch {
          }
      ...
      const C = this.state.render_mode, M = this.viewportCamera(), K = this.state.objects.some((D) => D.parent_id) ? this.state.objects.map((D) => D.parent_id ? { ...D, ...xa(this.state.objects, D) } : D) : this.state.objects, G = (this.viewportBgSequenceImages || []).map((D) => D.src), le = this.viewportBgImage?.src || "", L = this.pendingExtractorImport, $ = L ? [...this.state.cameras, {
      ...
      if (this.webgl)
        try {
          this.webgl.render(Se, M, this.cardMediaById, b, k, this.modelUrlsById, this.frame, this.recording, this.selectedEntity, this.selectedObjectId, this.subSelection, this.selectedKeyFrame ?? null), m.drawImage(this.webgl.canvas, 0, 0, b, k), Ye = !0;
        } catch (D) {
      ...
      !this.recording && this.state.speed_heatmap && this.drawSpeedHeatmap(), this.drawOverlays(), al(this), this.state.show_gizmo && el(this), ol(this), rl(this), this.renderCameraView();
    },
    renderCameraView() {
      if (this.state.camera_view_visible) {
        ...
        for (const m of this.state.cameras) {
          ...
          const C = b.width, M = b.height, K = Ra(this, m, ke(m, this.frame, this.state.objects), this.frame);
...
class ir {
  constructor(t) {
    this.app = go, this.node = t, this.root = Do(), this.root.tabIndex = -1, this.canvas = this.root.querySelector(".viewport-wrap > canvas"), this.cameraPreviewCanvases = /* @__PURE__ */ new Map(), this.cameraPreviewContexts = /* @__PURE__ */ new Map(), this.cameraPreviewSignature = "", this.interactionElement = this.canvas, this.interactionElement.tabIndex = 0, this.interactionElement.dataset.captureWheel = "true", this.ctx = this.canvas.getContext("2d", { alpha: !1 }), this.disposed = !1, this.renderRevision = 0, this.webgl = null, this.cameraWebgl = null, this.webglReady = this.loadWebGLViewports(), this.stateWidget = t.widgets?.find((o) => o.name === "state_json"), this.recordingWidget = t.widgets?.find((o) => o.name === "recording_path"), this.cardWidget = t.widgets?.find((o) => o.name === "card_asset"), this.widthWidget = t.widgets?.find((o) => o.name === "width"), this.heightWidget = t.widgets?.find((o) => o.name === "height"), this.fpsWidget = t.widgets?.find((o) => o.name === "fps"), this.durationWidget = t.widgets?.find((o) => o.name === "duration_seconds"), this.modeWidget = t.widgets?.find((o) => o.name === "render_mode");
    let a = null;
    try {
      a = JSON.parse(this.stateWidget?.value || "{}");
    } catch {
    }
    this.state = Wa(a), this.frame = 0, this.camera = te(this.state, 0), this.playing = !1, this.drag = null, this.cameraEditActive = !1, this.cameraEditKey = null, this.keyDrag = null, this.timelineDrag = null, this.curveDrag = null, this.selectedKeyFrame = this.state.keyframes[0]?.frame ?? null, this.editingKeyFrame = null, this.copiedKeyframe = null, this.cameraSpeed = 1, this.cardMedia = null, this.cardMediaById = /* @__PURE__ */ new Map(), this.objectUrls = new qo(), this.cardUrlsById = this.objectUrls.urls, this.modelUrlsById = /* @__PURE__ */ new Map(), this.modelInfoById = /* @__PURE__ */ new Map(), this.executionReferences = [], this.selectedObjectId = null, this.selectedEntity = "camera", this.subSelection = null, this.cardUrl = null, this.recording = !1, this.gizmoDrag = null, this.playTimer = null, this.previewClickTimer = null, this.showCurveHandles = !0, this.contextMenu = new Io(this.root), this.history = new Ko({ capture: () => JSON.stringify({ state: this.state, frame: this.frame, selectedEntity: this.selectedEntity, selectedObjectId: this.selectedObjectId, selectedObjectIds: [...this.selectedObjectIds || []], selectedKeyFrame: this.selectedKeyFrame, selectedKeyFrames: [...this.selectedKeyFrames || []], subSelection: this.subSelection }), restore: (o) => this.restoreHistorySnapshot(o) }), this.refreshCameraPreviews(), this.initializeTooltips(), this.bind(), this.bindWidgetCallbacks(), this.syncFromWidgets(), this.resizeCanvas(), this.render(), this.refreshKeys(), this.refreshObjects(), this.restoreAssets(), this.syncUpstreamInputs(), this.refreshSetupDiagnostic(), // Seed every frame-derived readout (timecode, lens millimetres, viewport
  ...
  async loadWebGLViewports() {
    let t;
    try {
      ({ OmniWebGLViewport: t } = await import("./chunk-CJPsn7W5.js"));
    } catch (a) {
    ...
    if (!this.disposed) {
      ...
      $r(this), this.resizeCanvas(), this.render(), this.renderCameraView();
    }
  }
}
const Sa = { app: go, api: Ie, EditorHistory: Ko, ContextMenuController: Io, initializeTooltips: Tn, promptText: Ua, ObjectUrlRegistry: qo, buildRoot: Do, dispatchDirectorKey: Er, activeCameraTrack: Qe, bindWidgetCallbacks: cs, playblastCameraTrack: Xa, restoreFromWidgets: ds, serializeEditorState: is, syncActiveCameraTrack: No, syncFromWidgets: ls, bind: Us, activateCamera: ei, addCamera: Ys, deleteCamera: Qs, drawPreviewOverlays: oi, duplicateCamera: Js, maximizeCameraPreview: Go, refreshCameraPreviews: Za, refreshCameraSelectors: Gs, renameCamera: Zs, setPlayblastCamera: ti, toggleCameraView: ai, captureRealtime: Yo, makePlayblast: pi, uploadDirectorPlayblast: Zo, waitForMediaFrame: Xo, computeAudioPeaks: Jo, loadAudioFile: hi, stopPlay: Na, togglePlay: fi, applyCameraPreset: bi, applyCameraShake: ui, applyProxyPreset: yi, clearViewportBgImage: wi, loadViewportBgFile: vi, loadViewportBgSequence: xi, drawCameraPath: Mi, drawCard: Ei, drawCube: Ci, drawGrid: ki, drawHuman: _i, drawLine3D: ee, drawNull: $i, drawOverlays: Pi, drawPointField: Si, drawSpeedHeatmap: Ai, drawSphere: ji, curveChannels: at, drawCurveEditor: kn, onCurvePointerDown: pn, onCurvePointerMove: fn, onCurvePointerUp: hn, onTimelinePointerDown: Mr, onTimelinePointerMove: Ar, onTimelinePointerUp: Pr, refreshKeys: Ti, resetCurveZoom: wn, resetTimelineZoom: Fr, setChannelFilter: un, setCurveInterpolation: bn, setTangentMode: yn, timelineFrameFromEvent: Ba, toggleCurveHandles: gn, zoomCurve: xn, drawTransformGizmo: Wi, frameTarget: qi, gizmoAxes: er, gizmoGeometry: Qa, onPointerDown: zr, onPointerMove: Lr, onPointerUp: Tr, onWheel: Kr, pickGizmo: Ni, pickSceneObject: Ri, resetCamera: Oi, setTransformMode: Ii, setViewMode: Ki, viewportCamera: Pa, loadCardFile: Xi, loadExecutionPreview: Yi, loadMediaUrl: ar, loadModelFile: Gi, loadSelectedReference: Zi, onModelLoaded: Ui, restoreAssets: Vi, syncUpstreamInputs: Ji, configureDomMedia: tr, refreshSetupDiagnostic: tc, addMediaCard: fc, addPrimitive: cc, applyObjectAnimationFrame: kc, beginCameraEdit: Mc, beginObjectEdit: nr, commitCameraEdit: Ac, commitObjectEdit: uc, copyKeyframe: _c, deleteKeyframe: jc, deleteObject: pc, duplicateObject: dc, exitKeyEdit: Fc, finishCameraEdit: Pc, goToAdjacentKey: Nc, insertKeyframe: Sc, loadSelectedKeyView: Dc, pasteKeyframe: $c, playblastCameraAtFrame: wc, refreshInspector: hc, refreshKeyEditor: Ic, refreshObjects: ic, removeObjectResources: xc, renameObject: lc, retimeSelectedKey: Oc, selectKeyframe: Ec, selectedKeyframe: $e, selectedObject: ot, selectObjectAnimation: vc, setKeyInterpolation: Cc, setObjectParent: gc, timelineKeyframes: xe, timelineObject: ve, toggleAutoKey: zc, toggleObject: mc, updateCameraFromHud: yc, updateEditState: Tc, updateKeyVisualState: Kc, updateSelectedKey: qc, updateSelectedObject: bc, clamp: P, cloneCamera: R, configureCore: ko, defaultCamera: wo, sampleCamera: te, sampleObjectTransform: Oe, sanitizeState: Wa, worldTransform: Ha };
Object.assign(
  ir.prototype,
  Bc(Sa),
  Vc(Sa),
...
function sl(e) {
  ...
  Ca("director:attach:start", e), Ca("director:constructor:start", e);
  const t = new ir(e);
  Ca("director:constructor:complete", e), e.__majoorOmniCam = t, Ca("director:marker:assigned", e), t.hideInternalWidgets();
  ...
  const d = e.onConnectionsChange;
  e.onConnectionsChange = function() {
    d?.apply(this, arguments), clearTimeout(t.connectionTimer), t.connectionTimer = setTimeout(() => {
