import { c as w, bI as g, e as k, b5 as A, bJ as re, K as $, s as L, bK as ne, bf as H, a as v, g as K, b6 as I, bL as se } from "./chunk-DFlWjFta.js";
import { m as ie, g as B, l as ce } from "./chunk-DjvDVpt_.js";
function Oe(e, t) {
    ...
    for (const r of a) {
      ...
      if (Array.isArray(d) && d.length) {
        ...
        const y = () => {
          ...
          const p = s.getBoundingClientRect(), b = c.getBoundingClientRect(), h = 8;
          let x = p.right + 2;
          x + b.width > window.innerWidth - h && (x = Math.max(h, p.left - b.width - 2));
          let u = p.top - 4;
          u + b.height > window.innerHeight - h && (u = Math.max(h, window.innerHeight - b.height - h)), c.style.left = `${x}px`, c.style.top = `${u}px`;
        }, n = () => {
        ...
        s.addEventListener("pointerenter", () => {
          clearTimeout(f), m = setTimeout(y, 60);
          ...
          p.preventDefault(), p.stopPropagation(), c.hidden ? y() : n();
        });
      } else {
        if (r.shortcut) {
...
function V({ title: e, message: t, withInput: a = !1, defaultValue: o = "", owner: r = null }) {
  return typeof document > "u" || !document.body ? Promise.resolve(a ? null : !1) : new Promise((s) => {
    ...
    y.type = "button", y.textContent = "Cancel";
    ...
    n.style.background = "var(--oc-accent, #4c6ef5)", n.style.borderColor = "transparent", n.style.color = "#fff", f.append(y, n), d.append(i, c), m && d.append(m), d.append(f), l.append(d);
    let p = !1;
    const b = (u) => {
      p || (p = !0, document.removeEventListener("keydown", x, !0), r && typeof r == "object" && S.get(r)?.delete(h), l.remove(), s(u));
    }, h = () => b(a ? null : !1);
    if (r && typeof r == "object") {
      let u = S.get(r);
      u || S.set(r, u = /* @__PURE__ */ new Set()), u.add(h);
    }
    const x = (u) => {
      u.key === "Escape" ? (u.stopPropagation(), b(a ? null : !1)) : u.key === "Enter" && (u.stopPropagation(), b(a ? m.value : !0));
    };
    y.addEventListener("click", () => b(a ? null : !1)), n.addEventListener("click", () => b(a ? m.value : !0)), l.addEventListener("mousedown", (u) => {
      u.target === l && b(a ? null : !1);
    }), document.addEventListener("keydown", x, !0), document.body.appendChild(l), (m || n).focus();
  });
}
function Ke({ title: e, items: t = [], onDelete: a = null, owner: o = null }) {
  return typeof document > "u" || !document.body ? Promise.resolve(null) : new Promise((r) => {
    ...
    let c = !1;
    const m = (h) => {
      c || (c = !0, document.removeEventListener("keydown", b, !0), o && typeof o == "object" && S.get(o)?.delete(f), s.remove(), r(h));
    }, f = () => m(null), y = (h) => {
      const x = document.createElement("div");
      Object.assign(x.style, { display: "flex", alignItems: "stretch", gap: "4px" });
      ...
      const C = document.createElement("div");
      C.textContent = h.label || h.id;
      const E = document.createElement("div");
      if (E.textContent = h.sublabel || "", Object.assign(E.style, { opacity: "0.6", fontSize: "11px" }), u.append(C, E), u.addEventListener("click", () => m(h.id)), x.appendChild(u), a) {
        const _ = document.createElement("button");
        _.type = "button", _.title = "Delete", _.textContent = "✕", Object.assign(_.style, {
          width: "34px",
          borderRadius: "6px",
          cursor: "pointer",
          border: "1px solid var(--oc-line, #34363f)",
          background: "transparent",
          color: "inherit"
        }), _.addEventListener("click", async (oe) => {
          oe.stopPropagation(), _.disabled = !0;
          try {
            await a(h.id), x.remove(), i.children.length || m(null);
          } catch (T) {
            _.disabled = !1, console.warn("[OmniCam] delete failed", T), o?.setStatus?.(String(T?.message || T).slice(0, 120));
          }
        }), x.appendChild(_);
      }
      return x;
    };
    for (const h of t) i.appendChild(y(h));
    ...
    if (p.type = "button", p.textContent = "Cancel", Object.assign(p.style, {
      ...
      let h = S.get(o);
      h || S.set(o, h = /* @__PURE__ */ new Set()), h.add(f);
    }
    const b = (h) => {
    ...
    s.addEventListener("mousedown", (h) => {
      h.target === s && m(null);
    }), document.addEventListener("keydown", b, !0), document.body.appendChild(s), i.querySelector("button")?.focus({ preventScroll: !0 });
  });
}
async function le(e, t, a, o) {
  let r, s, l, d, i;
  ...
  return c?.prompt ? c.prompt({ title: l, message: d, defaultValue: i }) : V({ title: l, message: d, withInput: !0, defaultValue: i, owner: s });
}
async function de(e, t, a) {
  ...
  return d?.confirm ? d.confirm({ title: s, message: l }) : V({ title: s, message: l, withInput: !1, owner: r });
}
const Be = `
...
`, J = 24, G = 5, Y = 150, X = Math.PI / 180;
function Q(e, t, a) {
  return Math.min(a, Math.max(t, e));
}
function me(e, t = J) {
  const a = Q(Number(e) || 0, G, Y);
...
const Je = [14, 18, 24, 35, 50, 85, 135], Ge = {
...
function Z(e) {
  ...
  const t = w(
    Number(e.state.outliner_height) || g.outlinerHeight.default,
    g.outlinerHeight.min,
    g.outlinerHeight.max
  ), a = w(
    Number(e.state.preview_width) || g.previewWidth.default,
    g.previewWidth.min,
    g.previewWidth.max
  ), o = w(
    Number(e.state.side_width) || g.sideWidth.default,
    g.sideWidth.min,
    g.sideWidth.max
  ), r = w(
    Number(e.state.left_width) || g.leftWidth.default,
    g.leftWidth.min,
    g.leftWidth.max
  ), s = w(
    Number(e.state.graph_height) || g.graphHeight.default,
    g.graphHeight.min,
    g.graphHeight.max
  ), l = w(
    Number(e.state.assets_height) || g.assetsHeight.default,
    g.assetsHeight.min,
    g.assetsHeight.max
  ), d = w(
    Number(e.state.agent_height) || g.agentHeight.default,
    g.agentHeight.min,
    g.agentHeight.max
  );
  e.root.style.setProperty("--oc-outliner-h", `${Math.round(t)}px`), e.root.style.setProperty("--oc-preview-w", `${Math.round(a)}px`), e.root.style.setProperty("--oc-side-w", `${Math.round(o)}px`), e.root.style.setProperty("--oc-left-w", `${Math.round(r)}px`), e.root.style.setProperty("--oc-graph-h", `${Math.round(s)}px`), e.root.style.setProperty("--oc-assets-h", `${Math.round(l)}px`), e.root.style.setProperty("--oc-agent-h", `${Math.round(d)}px`);
}
const ue = {
  "outliner-resize": { axis: "y", direction: 1, stateKey: "outliner_height", bounds: g.outlinerHeight, cssVar: "--oc-outliner-h" },
  "preview-resize": { axis: "x", direction: 1, stateKey: "preview_width", bounds: g.previewWidth, cssVar: "--oc-preview-w" },
  "side-resize": { axis: "x", direction: -1, stateKey: "side_width", bounds: g.sideWidth, cssVar: "--oc-side-w" },
  "left-resize": { axis: "x", direction: 1, stateKey: "left_width", bounds: g.leftWidth, cssVar: "--oc-left-w" },
  "graph-resize": { axis: "y", direction: 1, stateKey: "graph_height", bounds: g.graphHeight, cssVar: "--oc-graph-h" },
  "assets-resize": { axis: "y", direction: 1, stateKey: "assets_height", bounds: g.assetsHeight, cssVar: "--oc-assets-h" },
  "agent-resize": { axis: "y", direction: 1, stateKey: "agent_height", bounds: g.agentHeight, cssVar: "--oc-agent-h" }
};
function Xe(e, t) {
  Z(e);
  for (const [a, o] of Object.entries(ue)) {
    ...
    let d = !1;
    const i = (n) => {
      e.root.style.setProperty(o.cssVar, `${Math.round(w(n, o.bounds.min, o.bounds.max))}px`), !d && (d = !0, l(() => {
        d = !1, e.refitNode?.();
      }));
    }, c = (n) => {
      const p = Math.round(w(n, o.bounds.min, o.bounds.max));
    ...
    r.addEventListener("pointerup", y, { signal: t }), r.addEventListener("pointercancel", y, { signal: t }), r.addEventListener("dblclick", (n) => {
      n.preventDefault(), c(o.bounds.default);
    }, { signal: t }), r.addEventListener("keydown", (n) => {
      const p = (n.shiftKey ? 48 : 16) * s, b = Number(e.state[o.stateKey]) || o.bounds.default;
      n.key === "ArrowDown" || n.key === "ArrowRight" ? (n.preventDefault(), c(b + p)) : n.key === "ArrowUp" || n.key === "ArrowLeft" ? (n.preventDefault(), c(b - p)) : n.key === "Home" && (n.preventDefault(), c(o.bounds.default));
    }, { signal: t });
  }
}
function N(e) {
  return e?.state?.cameras?.length || (e.state.cameras = [{ id: "camera_1", name: "Camera 1", color: "#4aa3ef", camera: k(e?.camera), keyframes: e?.state?.keyframes || [] }]), e.state.cameras.find((t) => t.id === e.state.active_camera_id) || e.state.cameras[0];
}
function ge(e) {
  if (!e?.state?.cameras?.length)
    return N(e);
  if (e.state.playblast_camera_id === A) {
    const t = re(e.state, e.frame), a = t && e.state.cameras.find((o) => o.id === t.camera_id);
    if (a) return a;
  }
  return e.state.cameras.find((t) => t.id === e.state.playblast_camera_id) || N(e);
}
function be(e) {
  const t = N(e);
  t && (t.camera = k(e.camera), t.keyframes = e.state.keyframes, e.state.camera = k(e.camera));
}
function Qe(e) {
  if (e.disposed) return;
  e.directorRevision = (Number.isInteger(e.directorRevision) ? Math.max(0, e.directorRevision) : 0) + 1, e.renderRevision = (e.renderRevision || 0) + 1, be(e);
  const t = e.state.playblast_camera_id === A && ne(e.state), a = ge(e);
  ...
  const o = { ...e.state, camera: k(a.camera), keyframes: a.keyframes };
  o.metadata = { ...o.metadata, motion_scene_fingerprint_live: ie(e.state) }, e.stateWidget && (e.stateWidget.value = JSON.stringify(o)), e.widthWidget && (e.widthWidget.value = e.state.width), e.heightWidget && (e.heightWidget.value = e.state.height), e.fpsWidget && (e.fpsWidget.value = e.state.fps), e.durationWidget && (e.durationWidget.value = e.state.duration_frames / e.state.fps), e.modeWidget && (e.modeWidget.value = e.state.render_mode), e.cardWidget && (e.cardWidget.value = e.state.card_asset || ""), e.node.graph?.setDirtyCanvas?.(!0, !0);
}
function Ze(e) {
...
function Ue(e, t = !0) {
  ...
  for (const n of e.state.cameras) {
    ...
    n.keyframes = [...new Map(n.keyframes.map((p) => [p.frame, p])).values()].sort((p, b) => p.frame - b.frame);
  }
  e.state.keyframes = N(e).keyframes;
  for (const n of e.state.objects)
    n.keyframes = [...new Map((n.keyframes || []).map((p) => {
      const b = Math.max(0, Math.round(p.frame));
      return [b, { ...p, frame: b }];
    })).values()].sort((p, b) => p.frame - b.frame);
  ...
  e.root.dataset.density = e.state.ui_density || "animation", Z(e);
  ...
  const y = r('[data-role="timeline-fps"]');
  y && (y.value = String(e.state.fps)), e.frame = w(e.frame, 0, e.state.duration_frames - 1), t && e.serialize(), (a !== e.state.duration_frames || o !== e.state.fps) && (e.computeAudioPeaks?.(), e.setFrame(e.frame, !1, !0), e.setStatus(`Timeline: ${e.state.duration_frames} frames · ${(e.state.duration_frames / e.state.fps).toFixed(2)} s`));
}
function et(e) {
  let t = null;
  try {
  ...
  e.timelineKeyframes().some((r) => r.frame === e.selectedKeyFrame) || (e.selectedKeyFrame = e.timelineKeyframes()[0]?.frame ?? null), e.camera = L(e.state, Math.min(e.frame, e.state.duration_frames - 1)), e.syncFromWidgets(!1), e.root.querySelector('[data-role="gizmo-space"]').value = e.state.gizmo_space, e.restoreAssets(), e.refreshKeys(), e.refreshObjects(), e.render(), e.history?.clear(), e.sceneBaseline = e.stateWidget?.value ?? e.sceneBaseline, e.sceneName = e.state.metadata?.scene_name || "";
}
const ve = { "16:9": 16 / 9, "4:3": 4 / 3, "1:1": 1, "9:16": 9 / 16, "2.39:1": 2.39 };
function ye(e) {
  if (!e) return null;
  const t = ve[e.aspect_ratio];
  if (t) return t;
...
function ke(e, t, { label: a = "Extracted Camera" } = {}) {
  const o = Array.isArray(t?.keyframes) ? t.keyframes : [];
  if (!o.length) throw new Error(v("no camera keys in this solve"));
  const r = Number(e.state.fps) || 24, s = Number(t.fps) || r, l = s > 0 ? r / s : 1, d = R(e.state), i = e.state.cameras.length, c = we(e.state, a || "Extracted Camera"), m = j[i % j.length], f = o.map((y) => ({
    ...y,
    frame: Math.round((Number(y.frame) || 0) * l),
    camera: k(y.camera)
  }));
  if (e.state.cameras.push({ id: d, name: c, color: m, camera: k(f[0].camera), keyframes: f }), Number.isFinite(Number(t.duration_frames))) {
    const y = Math.round(Number(t.duration_frames) * l);
    e.state.duration_frames = Math.max(e.state.duration_frames || 1, y);
  }
...
function tt(e) {
  ...
  for (const a of e.root.querySelectorAll('[data-role="playblast-camera"]')) {
    ...
    o.value = A, o.textContent = t.length ? v("Sequence ({count} shots)").replace("{count}", String(t.length)) : v("Sequence (no shots yet)"), o.disabled = t.length === 0, a.appendChild(o), a.value = e.state.playblast_camera_id;
  }
...
function U(e) {
  ...
  const l = _e(e), d = l.map((c) => `${c.id}:${c.name}:${c.muted ? 1 : 0}:${c.solo ? 1 : 0}:${c.color || ""}`).join("|");
  let i = !1;
  d !== e.cameraPreviewSignature && (i = !0, e.cameraPreviewSignature = d, t.innerHTML = "", e.cameraPreviewCanvases.clear(), e.cameraPreviewContexts.clear(), l.forEach((c, m) => {
    ...
    const y = c.color || j[m % j.length];
    f.style.setProperty("--camera-color", y), f.title = v(`Click: set ${c.name} as primary · Double-click: edit · Right-click: preview actions`);
    const n = document.createElement("div");
    ...
    x.className = "pi pi-circle-fill output-mark", x.title = v("Playblast camera");
    const u = document.createElement("canvas");
    u.dataset.cameraPreview = c.id;
    const C = document.createElement("span");
    C.className = "camera-view-badge", C.textContent = v("CAMERA PREVIEW"), n.append(p, b, h, x), f.append(u, n, C), t.appendChild(f), f.addEventListener("click", () => {
    ...
    }), f.addEventListener("auxclick", (E) => {
      E.button === 1 && (E.preventDefault(), Se(e, c.id));
...
function at(e) {
  ...
  const t = R(e.state), a = e.state.cameras.length, o = `Camera ${a + 1}`, r = k(e.camera), s = [
    (r.target?.[0] ?? 0) - (r.position?.[0] ?? 0),
    (r.target?.[1] ?? 0) - (r.position?.[1] ?? 0),
    (r.target?.[2] ?? -1) - (r.position?.[2] ?? 0)
  ], l = Math.hypot(...s) || 1;
  r.position = [0, 0, 0], r.target = s.map((c) => c / l);
  const d = j[a % j.length], i = e.root.querySelector('[data-role="key-interp"]')?.value || e.root.querySelector('[data-role="interp"]')?.value || "ease";
  e.state.cameras.push({
    id: t,
    name: o,
    color: d,
    camera: r,
    keyframes: [{ frame: 0, camera: k(r), interpolation: i }]
  }), e.cameraPreviewSignature = "", e.activateCamera(t), e.setStatus(v(`${o} added`));
}
async function ot(e, t) {
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a) return;
  const o = (await le(e.app, v("Rename camera"), v("Camera name"), a.name))?.trim();
  !o || o === a.name || (e.checkpoint("Rename camera"), a.name = o.slice(0, 80), e.cameraPreviewSignature = "", e.serialize(), e.refreshObjects(), e.refreshKeys(), e.setStatus(v(`Camera renamed: ${a.name}`)));
}
function rt(e, t) {
  ...
  if (o.color = j[r % j.length], o.camera?.position && (o.camera.position = [
  ...
  e.state.cameras.push(o), e.cameraPreviewSignature = "", e.activateCamera(o.id), e.setStatus(v(`${o.name} added`));
}
async function nt(e, t) {
  if (e.state.cameras.length <= 1) return e.setStatus(v("At least one camera is required"));
  const a = e.state.cameras.find((r) => r.id === t);
  if (!a || !await de(e.app, v("Delete camera"), v(`Delete ${a.name} and its ${a.keyframes.length} keyframe(s)?`))) return;
  ...
  if (e.state.cameras = e.state.cameras.filter((r) => r.id !== t), t === e.state.playblast_camera_id && (e.state.playblast_camera_id = e.state.cameras[0].id), e.cameraPreviewSignature = "", o) {
    ...
    e.state.active_camera_id = r.id, e.state.keyframes = r.keyframes, e.state.camera = k(r.camera), e.camera = L(r, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = r.keyframes.find((s) => s.frame === e.frame)?.frame ?? null, e.selectedKeyFrames = e.selectedKeyFrame != null ? /* @__PURE__ */ new Set([e.selectedKeyFrame]) : /* @__PURE__ */ new Set(), e.editingKeyFrame = null;
  }
  e.pathSelection = H(e.pathSelection, e.activeCameraTrack()), e.serialize(), e.refreshCameraSelectors(), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v(`${a.name} deleted`));
}
function st(e, t) {
  const a = e.state.cameras.find((o) => o.id === t);
  a && (e.finishCameraEdit(), e.syncActiveCameraTrack(), e.state.active_camera_id = a.id, e.state.keyframes = a.keyframes, e.state.camera = k(a.camera), e.camera = L(a, e.frame, e.state.objects), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = a.keyframes.find((o) => o.frame === e.frame)?.frame ?? null, e.selectedKeyFrames = e.selectedKeyFrame != null ? /* @__PURE__ */ new Set([e.selectedKeyFrame]) : /* @__PURE__ */ new Set(), e.pathSelection = H(e.pathSelection, a), e.editingKeyFrame = null, e.serialize(), e.refreshCameraSelectors(), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(v(`Camera: ${a.name}`)));
}
function it(e, t) {
  const a = K(e.state), o = t === A && a.length > 0, r = o ? null : e.state.cameras.find((s) => s.id === t);
  !o && !r || (e.state.playblast_camera_id = o ? A : r.id, e.refreshCameraSelectors(), e.serialize(), e.refreshObjects(), e.renderCameraView(), e.setStatus(o ? v("Playblast: sequence ({count} shots)").replace("{count}", String(a.length)) : v(`Playblast: ${r.name}`)));
}
function ct(e) {
  ...
  }), e.setStatus(v(`Camera previews ${e.state.camera_view_visible ? "shown" : "hidden"}`));
}
function Se(e, t) {
  e.state.maximized_camera_id = e.state.maximized_camera_id === t ? null : t, e.serialize(), U(e), requestAnimationFrame(() => {
    e.resizeCanvas(), e.renderCameraView();
  }), e.setStatus(e.state.maximized_camera_id ? v("Preview maximized") : v("Preview restored"));
}
function lt(e, t, a, o) {
  if (e.state.guides !== !1) {
...
function dt(e, t) {
  ...
  for (const o of e.root.querySelectorAll('[data-role="camera-focal"]')) o.value = fe(a);
  e.setStatus(`Lens: ${t}mm (FOV ${a.toFixed(1)}°)`);
}
const P = "upstream_camera_track";
function mt(e, t, {
  ...
  if (!Array.isArray(c) || !c.length)
    throw new Error(v("no camera keys in this file"));
  d && e.checkpoint(a);
  const m = N(e);
  ...
  }), e.syncActiveCameraTrack(), e.setFrame(0), e.refreshKeys(), e.render(), e.scheduleSerialize(), i && e.setStatus(v("Imported {count} camera keys from {name}").replace("{count}", String(c.length)).replace("{name}", a)), c.length;
}
const je = "omnicam_extractor_result_v2", M = "omnicam_extracted_motion_scene_json", z = "omnicam_extracted_track_fingerprint", ee = "omnicam_extractor_source";
function te(e) {
...
function ze(e, t, a = {}) {
  ...
  const r = a.mode || (Me(e) ? "replace" : "merge"), s = a.reconMode || Ne(t);
  if (r === "replace") {
    e.checkpoint?.("Adopt reconstructed scene (replace)"), e.state = $(
      Le(JSON.parse(JSON.stringify(o)))
    ), e.camera = L(e.state, e.frame || 0);
    for (const l of e.state.objects || [])
      D(l, s), (l.type === "glb" || l.type === "model") && l.asset && e.modelUrlsById?.set(l.id, I(l.asset));
  } else {
    ...
    for (const m of c)
      m.parent_id && i.has(m.parent_id) && (m.parent_id = i.get(m.parent_id)), D(m, s), e.state.objects.push(m), (m.type === "glb" || m.type === "model") && m.asset && e.modelUrlsById?.set(m.id, I(m.asset));
    for (const m of ae(o)) {
      const f = O(d, m.id);
...
function Pe(e) {
  ...
  for (const o of t.inputs || []) {
    if (String(o?.name || "").toLowerCase() !== Te || o.link == null) continue;
    const r = ce(a, o.link);
    if (r && Re(r) === se) return r;
  }
  return null;
}
function We(e) {
  return String(e?.state?.metadata?.[P]?.fingerprint || "");
}
function Ie(e, t, a) {
  e.state.metadata = {
    ...e.state.metadata,
...
function W(e) {
  ...
  o && (o.textContent = v("{count} camera keys ready from {name} — import as a new camera?").replace("{count}", String(a.keyCount)).replace("{name}", a.label));
}
function yt(e) {
  const t = Pe(e), a = t ? Ae(t) : null;
  let o = !1;
  return a ? a.fingerprint !== We(e) && e.pendingExtractorImport?.fingerprint !== a.fingerprint && (e.pendingExtractorImport = {
    track: a.track,
    fingerprint: a.fingerprint,
    originNodeId: t.id,
    label: String(t.title || v("OmniCam Extractor")),
    keyCount: a.track.keyframes?.length || 0
  }, Ie(e, a.fingerprint, t), o = !0) : e.pendingExtractorImport && (e.pendingExtractorImport = null, o = !0), W(e), o;
}
function xt(e) {
  const t = e.pendingExtractorImport;
  return t ? (e.checkpoint("Import extracted camera"), ke(e, t.track, { label: t.label }), e.pendingExtractorImport = null, W(e), e.setStatus?.(v("Imported {count} camera keys from {name}").replace("{count}", String(t.keyCount)).replace("{name}", t.label)), e.scheduleSerialize(), e.render(), !0) : !1;
}
function wt(e) {
  return e.pendingExtractorImport ? (e.pendingExtractorImport = null, W(e), e.render(), e.setStatus?.(v("Extracted camera preview dismissed")), !0) : !1;
}
function kt(e) {
  const t = e?.graph;
  if (!t) return 0;
