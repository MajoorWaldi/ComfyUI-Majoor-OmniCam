...
function Yr(e) {
  e.registerExtension({
    ...
    beforeRegisterNodeDef(t, a) {
      if (!String(a?.name || a?.node_id || t?.comfyClass || t?.type || "").startsWith(Wr)) return;
      ...
      t.prototype.onDrawForeground = function(n) {
        ...
        const i = Math.max(4, Number(this.size?.[0] || 160) - Y - 6), l = -26, d = i + Y / 2, c = l + Y / 2;
        if (n.save(), this.selected) {
          const m = Xr(), p = n.createRadialGradient(d, c, Y * 0.35, d, c, Y * 1.15);
          p.addColorStop(0, `rgba(136, 115, 253, ${m})`), p.addColorStop(1, "rgba(136, 115, 253, 0)"), n.fillStyle = p, n.beginPath(), n.arc(d, c, Y * 1.15, 0, Math.PI * 2), n.fill();
        }
...
const dt = "en", Oe = /* @__PURE__ */ new Map([[dt, {}]]);
function Zr(e, t) {
  Oe.set(e, { ...Oe.get(e) || {}, ...t || {} });
}
let Ie = dt;
...
function S(e) {
  return Ie === dt ? e : Oe.get(Ie)?.[e] || e;
}
const eo = "__sequence__";
function to() {
...
function Hs(e) {
  return !!e?.sequence?.enabled && Pe(e).length > 0;
}
function aa(e, t) {
  const a = Pe(e);
...
function no(e, t, a = null) {
  const r = e?.sequence?.cuts || [], o = Math.max(0, Math.round(Number(t) || 0));
  if (!r.length || o <= 0 || r.some((i) => i.start === o)) return !1;
  const s = aa(e, o)?.camera_id || r[0].camera_id;
  return r.push({ camera_id: a || oo(e, s), start: o }), r.sort((i, l) => i.start - l.start), !0;
}
function so(e, t) {
  const a = e?.sequence?.cuts || [];
  return t < 0 || t >= a.length || a.length === 1 ? !1 : (a.splice(t, 1), a.length && (a[0].start = 0), !0);
}
const ra = Object.freeze(["select", "track", "anchor", "project", "erase"]), oa = Object.freeze(["manual_2d", "static_anchor", "world_point", "object_point", "camera_field"]), na = Object.freeze(["linear", "smooth", "hold"]), Se = (e, t = 0) => Number.isFinite(Number(e)) ? Number(e) : t, Dt = (e) => Math.max(0, Math.min(1, Se(e)));
function io(e, t) {
  return {
...
function lo(e) {
  ...
  return e.motion_layers = (Array.isArray(e.motion_layers) ? e.motion_layers : []).slice(0, 256).map((r, o) => {
    ...
    const s = oa.includes(r?.source_kind) ? r.source_kind : "manual_2d", i = (Array.isArray(r?.keys) ? r.keys : []).slice(0, 1e4).map((l) => io(l, t)).sort((l, d) => l.time_seconds - d.time_seconds);
    return {
      id: n,
  ...
  }).filter((r) => r.keys.length), e.motion_tool = ra.includes(e.motion_tool) ? e.motion_tool : "select", e.selected_motion_layer_id = e.motion_layers.some((r) => r.id === e.selected_motion_layer_id) ? e.selected_motion_layer_id : e.motion_layers[0]?.id || null, e;
}
const co = 32, mo = 64, po = 128, fo = Object.freeze(["off", "selected", "all"]), uo = Object.freeze(["annotation", "name", "tag"]), Tt = Object.freeze({ mode: "selected", content: "annotation" }), ho = Object.freeze(["top", "center", "bottom"]), go = /^[a-z0-9][a-z0-9_-]*$/, yo = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/, bo = /[<>]|:\/\/|javascript:|expression\(|&#/i;
function tt(e) {
  ...
  for (const r of e) {
    ...
    if (!(!o || o.length > mo || !go.test(o) || a.has(o)) && (a.add(o), t.push(o), t.length >= co))
      break;
  }
  return t;
}
function $s(e) {
  return Array.isArray(e) ? tt(e) : tt(String(e || "").split(/[,\n]/));
}
function jt(e) {
  if (!e || typeof e != "object") return null;
  const t = typeof e.text == "string" ? e.text.trim() : "";
  if (!t || t.length > po || bo.test(t)) return null;
  const a = typeof e.color == "string" ? e.color.trim() : "", r = yo.test(a) ? a.toLowerCase() : "#8d7ee8", o = ho.includes(e.anchor) ? e.anchor : "top";
  return { text: t, visible: e.visible !== !1, color: r, anchor: o };
}
function Us(e) {
  return {
    mode: fo.includes(e?.mode) ? e.mode : Tt.mode,
    content: uo.includes(e?.content) ? e.content : Tt.content
  };
}
function _o(e) {
...
function Xs(e, t) {
  ...
  if (t === "tag") return _o(e);
  const a = e.annotation;
  return a && a.visible !== !1 ? String(a.text || "") : "";
}
function Ys(e, { mode: t, selectedIds: a } = {}) {
  return !e || e.enabled === !1 || t === "off" ? !1 : t === "all" ? !0 : (a instanceof Set ? a : new Set(a || [])).has(e.id);
}
const vo = /* @__PURE__ */ new Set(["camera", "null", "sun_light", "point_light", "spot_light"]);
function Zs(e, t) {
  const a = Array.isArray(e?.position) ? e.position : [0, 0, 0], r = Array.isArray(e?.size) ? e.size : [1, 1, 1], o = vo.has(t) ? 0.35 : Math.max(0.2, (Number(r[1]) || 1) * 0.5 + 0.2);
  return [Number(a[0]) || 0, (Number(a[1]) || 0) + o, Number(a[2]) || 0];
}
const At = Object.freeze([0.05, 8]), So = 80, sa = (e, t, a) => Math.max(t, Math.min(a, e));
function ia(e) {
  if (!e || typeof e != "object") return null;
  const t = String(e.clip_id ?? "").trim();
  if (!t || t.length > So) return null;
  const a = Math.max(0, Math.round(Number(e.start_frame) || 0)), r = Math.round(Number(e.end_frame) || 0), o = r > a ? r : 0, n = Number(e.speed), s = sa(Number.isFinite(n) && n > 0 ? n : 1, At[0], At[1]), i = Number(e.offset_seconds);
  return {
    clip_id: t,
    start_frame: a,
    end_frame: o,
...
function Qs(e, t, a, r) {
  const o = ia(e), n = Number(r) || 0, s = Math.max(1, Number(a) || 24);
  if (!o || n <= 0) return 0;
  const i = (m) => o.loop ? (m % n + n) % n : sa(m, 0, n);
...
const Js = "omnicam_humanoid_v1", mt = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
  ...
  "foot_r",
  "toe_r"
]), wo = Object.freeze(["eye_l", "eye_r", "hand_tip_l", "hand_tip_r"]);
Object.freeze([...mt, ...wo]);
new Map(mt.map((e, t) => [e, t]));
const xo = /^mixamorig[:_ ]?/i, Mo = /[\s_\-.:|]+/g, Co = {
...
function ko(e) {
  return String(e || "").trim().replace(xo, "").replace(Mo, "").toLowerCase();
}
function Do(e) {
  const t = [e];
...
function ei(e) {
  const a = (e || []).map(String).filter((n) => n.trim()).map((n) => [n, Do(ko(n))]), r = /* @__PURE__ */ new Set(), o = {};
  for (const n of [!0, !1])
    for (const [s, i] of Object.entries(Co))
      if (!o[s])
        for (const l of i) {
          const d = To(l, a, r, n);
          if (d != null) {
...
function jo(e) {
  ...
  return mt.filter((a) => !t.has(a));
}
function Ao(e) {
  return jo(e).length === 0;
}
function ti(e) {
  const t = e?.bone_map || e?.rig?.bone_map || null;
  return !t || !Object.keys(t).length ? "none" : Ao(t) ? "rigged" : "incomplete";
}
const Eo = 128, Et = "omnicam_humanoid_v1", pt = /^[a-z0-9][a-z0-9_-]*$/, Oo = 1e-3, Io = (e) => Array.isArray(e) && e.length === 3 && e.every((t) => Number.isFinite(Number(t)));
function ft(e) {
  ...
  return a <= 1e-8 || Math.abs(a - 1) > Oo && !(a > 0.5 && a < 2) ? null : t.map((r) => r / a);
}
function la(e, t = 1e-4) {
  const a = ft(e);
...
function Po(e) {
  ...
  for (const [r, o] of Object.entries(e)) {
    ...
    if (!pt.test(n) || n.length > 64) continue;
    const s = ft(o);
    !s || la(s) || (t[n] = s, a += 1);
  }
  return t;
...
function ze(e) {
  ...
  return {
    preset_id: pt.test(a) && a.length <= 80 ? a : "neutral",
    root_offset: Io(t.root_offset) ? t.root_offset.map(Number) : [0, 0, 0],
    joints: Po(t.joints)
  };
}
function ai({ preset: e, overrides: t } = {}) {
  const a = ze(e), r = ze(t);
  return {
...
function ri(e, t, a) {
  ...
  if (!pt.test(o)) return r;
  const n = ft(a);
  return !n || la(n) ? delete r.joints[o] : r.joints[o] = n, r;
}
function zo(e) {
  return !e || typeof e != "object" ? null : {
    rig_profile: e.rig_profile === Et ? Et : null,
    pose: ze(e.pose),
    motion: ia(e.motion)
  };
}
...
function oi(e) {
  return Ro({ apiURL: ca }, e);
}
let ca = (e) => e;
const Ue = /* @__PURE__ */ new WeakMap();
function ni({ api: e }) {
  ca = (t) => e.apiURL ? e.apiURL(t) : t;
}
function Lo(e, t, a) {
  const r = e.keyframes, o = Ue.get(e);
  if (o?.source === r && a >= o.frame && o.index < t.length - 1) {
    let s = o.index;
    for (; s + 1 < t.length - 1 && a >= t[s + 1].frame; ) s += 1;
    if (t[s].frame < a && a < t[s + 1].frame)
      return Ue.set(e, { source: r, frame: a, index: s }), { leftIndex: s, left: t[s], right: t[s + 1] };
  }
  const n = ut(t, a);
  return Ue.set(e, { source: r, frame: a, index: n?.leftIndex ?? 0 }), n;
}
function V(e) {
  ...
  if (Math.abs(e.roll || 0) > 1e-9) {
    const s = e.roll * Math.PI / 180, i = Math.cos(s), l = Math.sin(s), d = C(k(o, i), k(n, l));
    n = C(k(n, i), k(o, -l)), o = d;
  }
...
function B(e, t, a, r) {
  ...
  const d = U(i, o), c = U(i, n);
  if (t.camera_type === "orthographic") {
    const p = 5 / Math.max(0.01, t.zoom || 1), u = p * a / Math.max(1, r);
    return [a * (0.5 + d / (2 * u)), r * (0.5 - c / (2 * p)), l];
  }
  const m = 0.5 * r / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
  return [a * 0.5 + d * m / l, r * 0.5 - c * m / l, l];
}
function le(e, t, a = null) {
  const r = (e.keyframes || []).map((g) => ({
    ...g,
    camera: K(g.camera || g || e.camera || se())
  }));
  if (!r.length) return K(e.camera || se());
  const o = Lo(e, r, t), n = N(r, t, "pos_x", (g) => (g.camera || g).position[0], !1, o), s = N(r, t, "pos_y", (g) => (g.camera || g).position[1], !1, o), i = N(r, t, "pos_z", (g) => (g.camera || g).position[2], !1, o);
  let l = N(r, t, "target_x", (g) => (g.camera || g).target[0], !1, o), d = N(r, t, "target_y", (g) => (g.camera || g).target[1], !1, o), c = N(r, t, "target_z", (g) => (g.camera || g).target[2], !1, o);
  const m = e.constraints?.look_at, u = m?.status === void 0 || m?.status === "active" ? m?.object_id || e.target_object_id || e.camera?.target_object_id : null, h = a || e.objects;
  if (u && Array.isArray(h)) {
    const g = h.find((M) => M.id === u);
    if (g && g.enabled !== !1) {
      const M = ht(h, g, t), A = m?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
      l = (M.position?.[0] ?? 0) + (A[0] || 0), d = (M.position?.[1] ?? 1.5) + (A[1] || 0), c = (M.position?.[2] ?? 0) + (A[2] || 0);
    }
  }
  const f = N(r, t, "fov", (g) => Number((g.camera || g).fov ?? 35), !1, o), _ = N(r, t, "roll", (g) => Number((g.camera || g).roll ?? 0), !0, o), y = N(r, t, "zoom", (g) => Number((g.camera || g).zoom ?? 1), !1, o), v = N(r, t, "near", (g) => Number((g.camera || g).near ?? 0.01), !1, o), b = N(r, t, "far", (g) => Number((g.camera || g).far ?? 1e4), !1, o), x = r[0]?.camera || r[0] || se();
  let w = r[0];
  for (const g of r)
    if ((g.frame ?? 0) <= t) w = g;
    else break;
...
const Ot = [
  ...
  "ease_expo",
  "ease_back"
], Ko = ["auto", "clamped", "vector", "free", "aligned", "flat"];
function Vo(e, t) {
  const a = e?.tangents;
  return !a || typeof a != "object" ? {} : a.channels && typeof a.channels == "object" && a.channels[t] ? a.channels[t] : a;
}
function It(e, t, a, r, o) {
  const n = Vo(e, t), s = Ko.includes(n.mode) ? n.mode : e?.tangents?.mode || "auto", i = o ? o(e) : 0, l = a && o ? o(a) : i, d = r && o ? o(r) : i, c = Math.max(1e-6, e.frame - (a?.frame ?? e.frame - 1)), m = Math.max(1e-6, (r?.frame ?? e.frame + 1) - e.frame), p = (v = !1) => {
    const b = (i - l) / c, x = (d - i) / m;
    let w = 0;
    if (!a && !r)
      w = 0;
    else if (!a)
  ...
  if (s === "aligned") {
    const v = Math.hypot(h, f) || 1e-6, b = Math.hypot(_, y) || 1e-6;
    _ = -h / v * b, y = -f / v * b;
  }
...
function N(e, t, a, r, o = !1, n = null) {
  ...
  const s = n || ut(e, t), { leftIndex: i, left: l, right: d } = s, c = i > 0 ? e[i - 1] : null, m = i + 2 < e.length ? e[i + 2] : null, p = Math.max(1, d.frame - l.frame), u = T((t - l.frame) / p, 0, 1);
  let h = r(l), f = r(d);
  if (o) {
    const v = ((f - h + 540) % 360 + 360) % 360 - 180;
    f = h + v;
  }
  if (l.interpolation === "bezier" || d.interpolation === "bezier") {
    const v = It(l, a, c, d, r), b = It(d, a, l, m, r), x = h, w = h + (v.out_y || 0), D = f + (b.in_y || 0), g = f, M = T(Number(v.out_x ?? 1 / 3), 0, 1), A = T(1 + Number(b.in_x ?? -1 / 3), 0, 1);
    let G = 0, ee = 1;
    for (let kt = 0; kt < 32; kt++) {
      const $ = (G + ee) * 0.5, $e = 1 - $;
      3 * $e * $e * $ * M + 3 * $e * $ * $ * A + $ * $ * $ < u ? G = $ : ee = $;
    }
    const Q = (G + ee) * 0.5, te = 1 - Q;
    return te * te * te * x + 3 * te * te * Q * w + 3 * te * Q * Q * D + Q * Q * Q * g;
  }
  const y = Bo(u, l.interpolation);
  return h + (f - h) * y;
}
const Go = 0.01, qo = 1e4;
function se() {
  return {
    position: [6, 4, 6],
    target: [0, 1.5, 0],
...
function qe(e, t) {
  ...
  const r = ce(e), o = (y, v) => (y.transform?.position || r.position)[v] ?? 0, n = (y, v) => (y.transform?.rotation || r.rotation)[v] ?? 0, s = (y, v) => (y.transform?.size || r.size)[v] ?? (v === 2 ? 0.01 : 1), i = ut(a, t), l = N(a, t, "pos_x", (y) => o(y, 0), !1, i), d = N(a, t, "pos_y", (y) => o(y, 1), !1, i), c = N(a, t, "pos_z", (y) => o(y, 2), !1, i), m = N(a, t, "rot_x", (y) => n(y, 0), !0, i), p = N(a, t, "rot_y", (y) => n(y, 1), !0, i), u = N(a, t, "rot_z", (y) => n(y, 2), !0, i), h = N(a, t, "scale_x", (y) => s(y, 0), !1, i), f = N(a, t, "scale_y", (y) => s(y, 1), !1, i), _ = N(a, t, "scale_z", (y) => s(y, 2), !1, i);
  return {
    position: [Number.isFinite(l) ? l : r.position[0], Number.isFinite(d) ? d : r.position[1], Number.isFinite(c) ? c : r.position[2]],
    rotation: [Number.isFinite(m) ? m : r.rotation[0], Number.isFinite(p) ? p : r.rotation[1], Number.isFinite(u) ? u : r.rotation[2]],
    size: [
...
function li(e = "balanced", t = "all_views", a = null) {
  const r = {
    none: 0,
    0: 0,
    sparse: 300,
  ...
  const c = 0.618033988749895, m = 0.324717957244746;
  for (let p = 0; p < o; p++) {
    const u = p * c % 1, h = p * m % 1, f = (p + 0.5) * 0.7548776662466927 % 1;
    ...
    if (t === "ground_focus")
      ...
      _ = Math.cos(D) * M * A, v = Math.sin(D) * M * A, y = Math.max(0.01, g * A * 0.75 + 2.5), b = 0.72, x = 0.78, w = 0.88;
    } else {
      const D = p % 4;
      if (D === 0) {
        const g = 0.3 + Math.sqrt(h) * 28, M = p * 2.399963229728653;
        _ = Math.cos(M) * g, v = Math.sin(M) * g, y = 0.01 + f * 0.34, b = 0.9, x = 0.94, w = 1;
      } else if (D === 1) {
        const g = 0.6 + Math.sqrt(h) * 18, M = p * 2.399963229728653;
        _ = Math.cos(M) * g, v = Math.sin(M) * g, y = 0.35 + f * 3.15, b = 0.68, x = 0.76, w = 0.86;
      } else if (D === 2) {
        const g = 2 + Math.sqrt(h) * 24, M = p * 2.399963229728653;
        _ = Math.cos(M) * g, v = Math.sin(M) * g, y = 3.5 + f * 11.5, b = 0.55, x = 0.65, w = 0.78;
      } else {
        const g = 0.5 + h * 6.5, M = p * 2.399963229728653;
        _ = Math.cos(M) * g, v = Math.sin(M) * g, y = 0.05 + f * 4.95, b = 0.8, x = 0.86, w = 0.94;
      }
    }
    n.push(_, y, v), s.push(a ? b * i : b, a ? x * l : x, a ? w * d : w);
  }
  return { points: n, colors: s };
}
function Ho() {
  const e = se(), t = [{ frame: 0, camera: K(e), interpolation: "ease" }];
  return {
    schema_version: 1,
    fps: 24,
    duration_frames: 120,
    ...
    camera: e,
    keyframes: t,
    cameras: [{ id: "camera_1", name: "Camera 1", color: "#4aa3ef", camera: K(e), keyframes: t }],
...
function ci(e) {
  ...
  const r = (c, m) => (Array.isArray(c) ? c : []).slice(0, ke.maxKeysPerTrack).map((p) => ({
    frame: Math.max(0, Math.round(Number(p.frame || 0))),
    camera: K(p.camera || p || m),
    interpolation: Ot.includes(p.interpolation) ? p.interpolation : "ease",
    ...p.tangents && typeof p.tangents == "object" ? { tangents: { ...p.tangents } } : {},
    ...Array.isArray(p.references) ? { references: p.references.map((u) => ({ ...u })) } : {}
  })), o = K(a.camera || t.camera);
  let n = r(a.keyframes, o);
  n = [...new Map(n.map((c) => [c.frame, c])).values()].sort((c, m) => c.frame - m.frame), n.length || (n = [{ frame: 0, camera: K(o), interpolation: "ease" }]);
  ...
  a.cameras = s.slice(0, ke.maxCameras).map((c, m) => {
    let p = String(c?.id || `camera_${m + 1}`);
    i.has(p) && (p = `camera_${m + 1}`), i.add(p);
    const u = K(c?.camera || c?.keyframes?.[0]?.camera || o);
    let h = r(c?.keyframes, u);
    return h = [...new Map(h.map((f) => [f.frame, f])).values()].sort((f, _) => f.frame - _.frame), h.length || (h = [{ frame: 0, camera: K(u), interpolation: "ease" }]), {
      id: p,
      name: String(c?.name || `Camera ${m + 1}`),
      color: pe(c?.color),
      camera: u,
      keyframes: h,
  ...
  }), a.active_camera_id = a.cameras.some((c) => c.id === a.active_camera_id) ? a.active_camera_id : a.cameras[0].id, a.sequence = ao(a.sequence, a.cameras.map((c) => c.id)), a.playblast_camera_id = a.playblast_camera_id === eo && a.sequence.cuts.length || a.cameras.some((c) => c.id === a.playblast_camera_id) ? a.playblast_camera_id : a.active_camera_id;
  ...
  a.camera = l.camera, a.keyframes = l.keyframes, a.target_object_id = l.target_object_id || null, a.target_offset = l.target_offset || [0, 0, 0], a.aim_bone = l.aim_bone || null, a.objects = (Array.isArray(a.objects) ? a.objects : t.objects).slice(0, ke.maxObjects).map((c) => ({
    ...
    ...c?.tags !== void 0 ? { tags: tt(c.tags) } : {},
    ...jt(c?.annotation) ? { annotation: jt(c.annotation) } : {},
...
function ma(e, [t, a, r, o]) {
  const [n, s, i] = e, l = o * n + a * i - r * s, d = o * s + r * n - t * i, c = o * i + t * s - a * n, m = -t * n - a * s - r * i;
  return [l * o - m * t - d * r + c * a, d * o - m * a - c * t + l * r, c * o - m * r - l * a + d * t];
}
function $o([e, t, a, r]) {
  const o = 1 - 2 * (t * t + a * a), n = 2 * (e * t - a * r), s = 2 * (e * a + t * r), i = 1 - 2 * (e * e + a * a), l = 2 * (t * a - e * r), d = 2 * (t * a + e * r), c = 1 - 2 * (e * e + t * t), m = Math.asin(Math.max(-1, Math.min(1, s))), [p, u] = Math.abs(s) < 0.9999999 ? [Math.atan2(-l, c), Math.atan2(-n, o)] : [Math.atan2(d, i), 0];
  return [p, m, u].map((h) => h * 180 / Math.PI);
}
function pa(e, t) {
  const a = t.quaternion || Re(t.rotation), r = Wo(a, e.quaternion || Re(e.rotation));
  return { position: C(ma(e.position.map((o, n) => o * t.size[n]), a), t.position), rotation: $o(r), quaternion: r, size: e.size.map((o, n) => o * t.size[n]) };
}
function Uo(e, t) {
  const a = new Map(e.map((o) => [o.id, o])), r = (o, n = /* @__PURE__ */ new Set()) => {
    const s = { ...ce(o), quaternion: Re(o.rotation) };
    if (!o?.id || n.has(o.id)) return s;
    const i = o.parent_id ? a.get(o.parent_id) : null;
    if (!i) return s;
    const l = new Set(n);
    return l.add(o.id), pa(s, r(i, l));
  };
  return r(t);
}
function ht(e, t, a, r = /* @__PURE__ */ new Set()) {
  const o = qe(t, a);
...
const Pt = ["speed", "angular_speed", "acceleration", "jerk"], Xe = ["ok", "warn", "over"], fa = 0.8, Xo = [0, 1.5, 0];
function zt(e, t) {
  const a = [0];
...
function Qo(e, t = null) {
  ...
  return Array.isArray(a?.position) ? a.position.slice(0, 3).map(Number) : [...Xo];
}
function Jo(e, t, a, r) {
  return e.map((o) => {
    const n = B(t, o, a, r);
...
function Nt(e, t) {
  return t == null || t <= 0 ? "ok" : e > t ? "over" : e > t * fa ? "warn" : "ok";
}
function Rt(e) {
  for (let t = Xe.length - 1; t >= 0; t--) if (e.includes(Xe[t])) return Xe[t];
  return "ok";
}
function en(e, t) {
...
function tn(e, t) {
  ...
  for (let r = 0; r < e.length; r++) {
    ...
    if (n && n.grade === e[r] && en(n.metrics, o)) {
      n.end = r;
      continue;
    }
...
function ua(e, t = {}, a = null, r = "generic") {
  ...
  const d = Yo(l, o), c = Zo(l, o), m = zt(d, o), p = zt(m, o), u = { speed: d, angular_speed: c, acceleration: m, jerk: p }, h = Qo(e, a), f = Jo(l, h, s, i), _ = l.map((g) => g.fov), y = t.allow_framing_loss === !0, v = [], b = [];
  for (let g = 0; g < n; g++) {
    const M = [], A = [];
    for (const G of Pt) {
      const ee = Nt(u[G][g], t[`max_${G}`]);
      M.push(ee), ee !== "ok" && A.push(G);
    }
    !f[g] && !y && (M.push("over"), A.push("framing_loss")), v.push(Rt(M)), b.push(A);
  }
  ...
  const D = Nt(w.max_fov_change, t.max_fov_change);
  return w.track_grades = { fov_drift: D }, w.grade = Rt([...v, D]), w.trajectory_valid = w.violations.length === 0, w.ok = w.trajectory_valid, w;
}
function an(e) {
...
function di(e, t) {
  ...
  if (t.shiftKey)
    ...
    const n = t.currentTarget.getBoundingClientRect(), s = (t.clientX - n.left) / Math.max(1, n.width), i = T(Number(e.timelineZoom) || 1, 0.2, 30), l = T(i * r, 0.2, 30), d = a / i, c = a / l, m = (Number(e.timelinePan) || 0) + s * d;
    e.timelinePan = T(m - s * c, -a * 0.5, a), e.timelineZoom = l;
  }
...
function pi(e, t) {
  ...
  e.selectedKeyFrames = null, e.timelineDrag = { box: a, pointerId: t.pointerId }, e.setFrame(ha(e, t, a));
}
function fi(e, t) {
  ...
  !e.timelineDrag || t.pointerId !== e.timelineDrag.pointerId || (t.preventDefault(), t.stopPropagation(), e.setFrame(ha(e, t, e.timelineDrag.box), !1, !1));
}
function ui(e, t) {
  if (e.timelinePanDrag && t.pointerId === e.timelinePanDrag.pointerId) {
  ...
  if (e.boxSelect && t.pointerId === e.boxSelect.pointerId) {
    t.preventDefault(), t.stopPropagation();
    const a = e.boxSelect.box.getBoundingClientRect(), r = Math.max(1, e.state.duration_frames - 1), o = T(Number(e.timelineZoom) || 1, 0.1, 50), n = Number(e.timelinePan) || 0, s = r / o, i = (m) => T(n + m / Math.max(1, a.width) * s, 0, r), l = Math.min(i(e.boxSelect.startX), i(e.boxSelect.currentX)), d = Math.max(i(e.boxSelect.startX), i(e.boxSelect.currentX));
    e.boxSelect.overlay?.remove(), e.boxSelect = null;
    const c = e.timelineKeyframes().filter((m) => m.frame >= l && m.frame <= d).map((m) => m.frame);
...
function on(e, t) {
  ...
  if (!a) return;
  if (!a.engaged) {
    if (Math.hypot(t.clientX - (a.startClientX ?? t.clientX), t.clientY - (a.startClientY ?? t.clientY)) < rn) return;
    a.engaged = !0;
  }
  ...
  const m = at(e, l);
  if (c.style.left = `${m}%`, c.textContent = a.isDuplicate ? `+Copy F${l}` : `F${l}${d !== 0 ? ` (${d > 0 ? "+" : ""}${d})` : ""}`, a.moving && a.moving.length > 1) {
    ...
    for (const h of a.moving) {
      let f = T(h.startFrame + d, 0, e.state.duration_frames - 1);
      for (; u.has(f) && f > 0 && f < e.state.duration_frames - 1; ) f += Math.sign(d || 1);
...
function ya(e, t, a = 0.6) {
  const r = Le(e), o = Math.min(1, Math.max(0, Number(a) || 0));
  if (!o || r.length < 3 || !t?.length) return r;
  const n = Le(r);
  for (let s = 1; s < r.length - 1; s++)
    if (ga(r[s].frame ?? 0, t))
      for (const i of ["position", "target"]) {
        const l = [r[s - 1], r[s], r[s + 1]].map((m) => m.camera?.[i]).filter((m) => Array.isArray(m) && m.length >= 3), d = r[s].camera?.[i];
        if (l.length < 3 || !Array.isArray(d)) continue;
        const c = [0, 1, 2].map((m) => l.reduce((p, u) => p + Number(u[m] || 0), 0) / l.length);
        n[s].camera[i] = d.map((m, p) => Number(m) + (c[p] - Number(m)) * o);
      }
  return n;
}
function ln(e, t, a) {
  ...
  for (const n of r)
    ga(n.frame ?? 0, t) && (n.camera.target = [...o]);
  return r;
}
function cn(e, t) {
...
async function hi() {
  ...
  try {
    if (!rt) return null;
    const e = await rt.fetchApi("/majoor/omnicam/motion_profiles");
...
function de(e) {
  const t = fn(e), a = un(e, t);
  return a ? ua(e.state, a, null, t) : null;
}
function Lt(e) {
...
function fe(e, t, a, r) {
  const o = a == null ? S("no limit") : `${Lt(t)} / ${Lt(a)}`, n = a != null && a > 0, s = n ? Math.min(100, Math.round(t / a * 100)) : 0, i = r === "over" ? "#ef4444" : r === "warn" ? "#f59e0b" : "#22c55e";
  return `
    <div class="oc-health-metric" data-grade="${r}">
      <div class="oc-health-metric-row">
        <span class="oc-health-dot"></span>
        <span class="oc-health-metric-name">${He(e)}</span>
...
function gn(e) {
  ...
  return t.length ? t.slice(0, 6).map((a) => {
    const r = a.metrics.map((n) => He(n)).join(", "), o = a.start === a.end ? S("Frame {frame}").replace("{frame}", String(a.start)) : S("Frames {start}-{end}").replace("{start}", String(a.start)).replace("{end}", String(a.end));
    return `
...
function yn(e) {
  ...
  if (a.className = `oc-health-badge ${o.grade}`, a.textContent = mn(o.grade), r) {
    const { score: l, letter: d } = hn(o);
  ...
  const s = [
    ...
    fe("fov_drift", o.max_fov_change, o.limits.max_fov_change, o.track_grades.fov_drift)
         ...
         <div class="oc-health-metric-row">
           <span class="oc-health-dot"></span>
           <span class="oc-health-metric-name">${He("framing_loss")}</span>
  ...
  t.innerHTML = `
    ...
    <div class="oc-health-zones" data-role="health-zones">${gn(o)}</div>
...
function gi(e, t) {
  ...
  if (a) {
    ...
    for (const r of a.segments) {
      ...
      const o = at(e, r.start), n = at(e, r.end + 1);
      if (n < -5 || o > 105) continue;
      const s = document.createElement("div");
      s.className = "oc-health-band", s.dataset.grade = r.grade, s.style.left = `${o}%`, s.style.width = `${Math.max(0.4, n - o)}%`, s.title = r.metrics.map((i) => He(i)).join(", "), t.appendChild(s);
    }
  }
}
function Ce(e, t, a, r) {
  const o = e.activeCameraTrack();
  o && (e.checkpoint(a), o.keyframes = t, e.state.keyframes = t, e.syncActiveCameraTrack(), e.refreshKeys(), e.setFrame(e.frame, !1, !1), e.setStatus(r), yn(e));
}
function yi(e) {
  const t = de(e);
  if (!t) return;
  ...
  const r = Math.max(1, e.state.duration_frames - 1), o = sn(e.state.keyframes, r), n = ua({ ...e.state, keyframes: o }, t.limits, null, t.profile);
  if (n.max_speed <= a) {
    Ce(e, o, "Slow to limits", S("Speed flattened; the shot keeps its length."));
    return;
  }
  const s = n.max_speed / a * (e.state.duration_frames / Math.max(1, e.state.fps));
  Ce(e, o, "Slow to limits", S("Speed flattened, still over: this path needs about {seconds}s to fit the limit.").replace("{seconds}", s.toFixed(1)));
}
function bi(e) {
  const t = de(e);
  if (!t) return;
  ...
  const r = ya(e.state.keyframes, a, 0.6);
  Ce(
    e,
    r,
...
function _i(e) {
  ...
  const r = ln(e.state.keyframes, a, t.subject);
  Ce(
    e,
    r,
    "Recenter subject",
...
function vi(e, t, a) {
  if (!de(e)) return;
  const o = ya(e.state.keyframes, [{ start: t, end: a }], 0.6);
  Ce(
    e,
    o,
    "Smooth zone",
...
const bn = {
  ...
  ...bn,
  ..._n
}, Z = ["OmniCam"], ba = "MajoorOmniCam.Locale", _a = "MajoorOmniCam.Defaults.Fps", va = "MajoorOmniCam.Defaults.DurationSeconds", Sa = "MajoorOmniCam.Defaults.Width", wa = "MajoorOmniCam.Defaults.Height", xa = "MajoorOmniCam.Defaults.RenderMode", Ma = "MajoorOmniCam.Defaults.Encoder", Ca = "MajoorOmniCam.Defaults.PlayblastResolution", ka = "MajoorOmniCam.Playblast.Quality", Da = "MajoorOmniCam.Defaults.PlayblastGrid", Ta = "MajoorOmniCam.Defaults.PlayblastLabels", ja = "MajoorOmniCam.Proxy.PointDensity", Aa = "MajoorOmniCam.Proxy.PointSpread", Ea = "MajoorOmniCam.Proxy.PointColor", Oa = "MajoorOmniCam.Proxy.CardFit", Ia = "MajoorOmniCam.Viewport.Quality", Pa = "MajoorOmniCam.Viewport.Adaptive", za = "MajoorOmniCam.Viewport.BackgroundColor", Na = "MajoorOmniCam.Display.Grid", Ra = "MajoorOmniCam.Display.Radar", La = "MajoorOmniCam.Display.CameraPaths", Fa = "MajoorOmniCam.Display.CameraGizmos", Ba = "MajoorOmniCam.Display.LookAt", Ka = "MajoorOmniCam.Display.HelperAxes", Va = "MajoorOmniCam.Display.Gizmo", Ga = "MajoorOmniCam.Display.Guides", qa = "MajoorOmniCam.Display.SafeAreas", Ha = "MajoorOmniCam.Display.ResolutionGate", Wa = "MajoorOmniCam.Display.AspectRatio", $a = "MajoorOmniCam.Display.BurnIn", Ua = "MajoorOmniCam.Display.SpeedHeatmap", Xa = "MajoorOmniCam.Display.Wireframe", Ya = "MajoorOmniCam.Display.Vertices", Za = "MajoorOmniCam.Tools.SelectMode", Qa = "MajoorOmniCam.Tools.GizmoMode", Ja = "MajoorOmniCam.Tools.GizmoSpace", er = "MajoorOmniCam.Tools.SpatialSnapMode", tr = "MajoorOmniCam.Tools.SpatialGridSize", ar = "MajoorOmniCam.Navigation.Profile", rr = "MajoorOmniCam.Navigation.FlySpeed", or = "MajoorOmniCam.Navigation.InvertOrbitY", nr = "MajoorOmniCam.Navigation.ZoomSensitivity", sr = "MajoorOmniCam.Navigation.OrbitSensitivity", ir = "MajoorOmniCam.Navigation.PanSensitivity", lr = "MajoorOmniCam.Navigation.DollySensitivity", cr = "MajoorOmniCam.Navigation.ViewMode", dr = "MajoorOmniCam.Controls.EnableShortcuts", mr = "MajoorOmniCam.Timeline.SnapEnabled", pr = "MajoorOmniCam.Timeline.SnapFrames", fr = "MajoorOmniCam.Timeline.AutoKey", ur = "MajoorOmniCam.Timeline.DefaultInterpolation", hr = "MajoorOmniCam.Timeline.TimecodeMode", gr = "MajoorOmniCam.Timeline.LoopPlayback", yr = "MajoorOmniCam.Interface.Density", br = "MajoorOmniCam.Interface.PreviewLayout", _r = "MajoorOmniCam.Interface.CameraPreviews", vr = "MajoorOmniCam.History.Limit", Sr = "MajoorOmniCam.Extractor.DefaultBackend", wr = "MajoorOmniCam.Monitor.DefaultProfile", xr = "MajoorOmniCam.Agent.Enabled", gt = "MajoorOmniCam.Agent.Provider", Ft = "MajoorOmniCam.Agent.Model", Bt = "MajoorOmniCam.Agent.BaseUrl", Mr = "MajoorOmniCam.Agent.MaxOutputTokens", Cr = "MajoorOmniCam.Agent.MaxPlannerSteps", kr = "MajoorOmniCam.Agent.RequestTimeoutSeconds", Fe = [
...
function Dr(e) {
  return Fe.find((t) => t.id === e) || Fe[0];
}
function yt(e) {
  return `MajoorOmniCam.Agent.${Dr(e).settingKey}.Model`;
}
function bt(e) {
  return `MajoorOmniCam.Agent.${Dr(e).settingKey}.BaseUrl`;
}
...
const Tr = Sn({
  onLocaleChange: () => jr(),
  onQualityChange: (e) => Gt(e),
  onAdaptiveChange: () => Gt(),
  onAgentEnabledChange: () => Dn()
});
let _t = null;
function R(e, t) {
  try {
    const a = _t?.extensionManager?.setting?.get(e);
    return a ?? t;
  } catch {
    return t;
  }
...
function ye(e, t) {
  try {
    _t?.extensionManager?.setting?.set?.(e, t);
  } catch (a) {
...
function jr() {
  const e = String(R(ba, "auto")), t = String(R("Comfy.Locale", "en") || "en").slice(0, 2).toLowerCase(), a = e === "auto" ? t : e, r = a !== Jr();
  if (Qr(a), !!r) {
    for (const o of W)
      if (!o.disposed)
        try {
...
function Gt(e = Ar()) {
  for (const t of W)
    t.disposed || (Er(t), t.requestRender ? t.requestRender("quality") : t.render?.(), t.renderCameraView?.());
}
function Mi() {
  return Tn().enabled;
}
...
function Tn() {
  const e = z(gt, "ollama", ["openai", "openai_compatible", "anthropic", "ollama"]);
  return {
    enabled: I(xr, !0),
    provider: e,
    ...
    model: String(R(yt(e), "") || "").trim(),
    baseUrl: String(R(bt(e), "") || "").trim(),
    maxOutputTokens: F(Mr, 4096, 512, 32768, !0),
    maxPlannerSteps: F(Cr, 6, 1, 12, !0),
    requestTimeoutSeconds: F(kr, 120, 15, 300, !0)
  };
}
function jn() {
  return {
    fps: F(_a, 24, 1, 120, !0),
    durationSeconds: F(va, 5, 1, 120, !0),
    width: F(Sa, 1280, 64, 4096, !0),
    height: F(wa, 720, 64, 4096, !0),
    renderMode: String(R(xa, "omni_ref")),
    encoder: String(R(Ma, "auto")),
    playblastResolution: z(Ca, "output", ["viewport", "half", "output", "double"]),
    playblastQuality: z(ka, "balanced", ["low", "balanced", "high"]),
    playblastGrid: I(Da, !1),
    playblastLabels: I(Ta, !1),
    pointDensity: z(ja, "balanced", ["none", "sparse", "balanced", "dense", "ultra"]),
    pointSpread: z(Aa, "all_views", ["all_views", "ground_focus", "dome"]),
    pointColor: Vt(Ea, "#cbd5e1"),
    cardFit: z(Oa, "contain", ["contain", "cover", "stretch"]),
    backgroundColor: Vt(za, "#121212"),
    showGrid: I(Na, !0),
    showRadar: I(Ra, !0),
    showCameraPaths: I(La, !0),
    showCameraGizmos: I(Fa, !0),
    showLookAt: I(Ba, !0),
    showHelperAxes: I(Ka, !0),
    showGizmo: I(Va, !0),
    guides: I(Ga, !0),
    safeAreas: I(qa, !1),
    resolutionGate: I(Ha, !1),
    aspectRatio: z(Wa, "auto", ["auto", "16:9", "4:3", "1:1", "9:16", "2.39:1"]),
    burnIn: I($a, !1),
    speedHeatmap: I(Ua, !1),
    showWireframe: I(Xa, !1),
    showVertices: I(Ya, !1),
    selectMode: z(Za, "object", ["object", "vertex", "edge", "face"]),
    gizmoMode: z(Qa, "translate", ["translate", "rotate", "scale"]),
    gizmoSpace: z(Ja, "world", ["world", "local"]),
    spatialSnapMode: z(er, "none", ["none", "grid", "vertex"]),
    spatialGridSize: F(tr, 0.5, 0.01, 100),
    navigationProfile: z(ar, "simple", ["maya", "blender", "simple"]),
    flySpeed: F(rr, 1, 0.05, 5),
    invertOrbitY: I(or, !1),
    zoomSensitivity: F(nr, 1, 0.2, 3),
    orbitSensitivity: F(sr, 1, 0.2, 3),
    panSensitivity: F(ir, 1, 0.2, 3),
    dollySensitivity: F(lr, 1, 0.2, 3),
    viewMode: z(cr, "perspective", ["camera", "perspective", "front", "back", "top", "bottom", "right", "left"]),
    snapEnabled: I(mr, !0),
    snapFrames: F(pr, 1, 1, 24, !0),
    autoKey: I(fr, !1),
    defaultInterpolation: z(ur, "ease", ["ease", "smooth", "bezier", "linear", "ease_in", "ease_out", "hold"]),
    timecodeMode: z(hr, "time", ["time", "timecode"]),
    loopPlayback: I(gr, !1),
    uiDensity: z(yr, "animation", ["basic", "animation", "advanced"]),
    previewLayout: z(br, "auto", ["auto", "1", "2", "4"]),
    cameraViewVisible: I(_r, !0),
    undoLimit: F(vr, 100, 10, 500, !0),
    extractorBackend: z(Sr, "dpvo", ["dpvo", "pycolmap"]),
    monitorProfile: z(wr, "wan_camera_native", ["wan_camera_native", "minimax_h3", "ltx_motion", "generic_video"])
  };
}
function An() {
  const e = String(R(Ft, "") || "").trim(), t = String(R(Bt, "") || "").trim();
  if (!e && !t) return;
  const a = z(
    gt,
    "ollama",
  ...
  e && !String(R(r, "") || "").trim() && ye(r, e), t && !String(R(o, "") || "").trim() && ye(o, t), ye(Ft, ""), ye(Bt, "");
}
function En(e) {
  _t = e, Zr("fr", vn), jr(), An();
}
...
function qt(e, t, a, r, o) {
  const { right: n, up: s, forward: i } = V(t), l = t.position, d = [a[0] - l[0], a[1] - l[1], a[2] - l[2]], c = Pn(d, i);
  let m, p;
  if (t.camera_type === "orthographic") {
    const u = 5 / Math.max(0.01, t.zoom || 1), h = u * r / Math.max(1, o);
    m = (e[0] / Math.max(1, r) - 0.5) * 2 * h, p = (0.5 - e[1] / Math.max(1, o)) * 2 * u;
  } else {
    const u = 0.5 * o / Math.tan(Math.max(1e-3, t.fov) * Math.PI / 360);
    m = (e[0] - r / 2) * c / u, p = (o / 2 - e[1]) * c / u;
  }
  return [0, 1, 2].map((u) => l[u] + i[u] * c + n[u] * m + s[u] * p);
}
function zn(e) {
...
const be = "orbit", _e = "pan", Ye = "dolly";
function Or(e, t, { includeCtrlFallback: a, includeSimpleLeft: r = a }) {
  const o = !!(t.ctrlKey || t.metaKey);
  if (e === "simple" && !t.altKey && !o && !t.shiftKey) {
    if (t.button === 2) return _e;
    if (t.button === 0) return r ? be : null;
  }
  return t.button === 1 ? o ? Ye : t.shiftKey || t.altKey ? _e : be : t.button === 2 ? t.altKey && e === "maya" ? Ye : null : t.button !== 0 ? null : t.altKey ? o ? Ye : t.shiftKey ? _e : be : o && a ? t.shiftKey ? _e : be : null;
}
function Nn(e, t, a) {
  const r = vt(e), o = Or(r, t, { includeCtrlFallback: !0 });
  return o === be && a?.camera_type === "orthographic" ? _e : o;
}
function Ht(e, t) {
  return e.isNavigatingFly ? !0 : Or(vt(e), t, { includeCtrlFallback: !1 }) !== null;
}
const Rn = ["maya", "blender", "simple"];
function vt(e) {
  const t = e.state?.navigation_profile;
  return Rn.includes(t) ? t : "maya";
}
function Ln(e, t) {
...
function Wt(e, { mode: t, origin: a, delta: r, factors: o, rotationDeg: n }) {
...
function Fn(e, t) {
  return (Array.isArray(e) ? e : []).map((r) => {
    const o = { ...r.camera };
    return Array.isArray(o.position) && (o.position = Wt(o.position, t)), Array.isArray(o.target) && (o.target = Wt(o.target, t)), { ...r, camera: o };
  });
}
function Bn(e, t) {
...
function Kn(e, { baseDrag: t, viewCamera: a, entityPosition: r }) {
  const o = e.activeCameraTrack?.();
  return !o || o.locked || !(o.keyframes?.length >= 1) ? !1 : (e.checkpoint("Transform camera path"), e.gizmoDrag = {
    ...
    trackId: o.id,
    origin: Ir(o.keyframes),
    baseKeys: o.keyframes.map((n) => ({ ...n, camera: K(n.camera) })),
    viewRight: V(a).right,
    viewUp: V(a).up,
...
function Vn(e, { pointer: t, deltaPixels: a, precision: r, snapping: o }) {
  ...
  if (e.state.gizmo_mode === "translate") {
    let d;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, m = (t[1] - n.pointer[1]) * r;
      d = C(k(n.viewRight, c * n.freeScale), k(n.viewUp, -m * n.freeScale));
    } else
      d = k(n.axis, a * n.worldLength / n.screenLength);
    l = { mode: "translate", delta: d };
  } else if (e.state.gizmo_mode === "scale") {
    let d;
    if (n.free) {
      const c = (t[0] - n.pointer[0]) * r, m = (t[1] - n.pointer[1]) * r, p = Math.max(0.01, 1 + (c - m) * n.freeScale * 0.35);
      d = [p, p, p];
    } else {
  ...
  s.keyframes = Fn(n.baseKeys, l), s.id === e.state.active_camera_id && (e.state.keyframes = s.keyframes), e.camera = le(s, e.frame, e.state.objects), s.camera = K(e.camera), e.refreshKeys(), e.refreshInspector(), e.render(), e.renderCameraView?.();
}
function $t(e, t) {
  const a = [];
...
function Gn(e, t) {
  ...
  if (a) return $t(a.min, a.max);
  const r = ht(e.state.objects, t, e.frame || 0), o = (t.type === "model" || t.type === "glb") && e.webgl?.getObjectWorldCenter?.(t.id) || r.position, n = r.size.map((i) => Math.max(0.01, Math.abs(i)) / 2), s = r.quaternion || Re(r.rotation);
  return $t(n.map((i) => -i), n).map((i) => C(o, ma(i, s)));
}
function qn(e, t, a, r = {}) {
  const o = e.state.objects.filter((b) => b.enabled !== !1), n = r.all ? o : o.filter((b) => e.selectedObjectIds?.has(b.id)), i = (n.length ? n : [a]).flatMap((b) => Gn(e, b)), l = [0, 1, 2].map((b) => Math.min(...i.map((x) => x[b]))), d = [0, 1, 2].map((b) => Math.max(...i.map((x) => x[b]))), c = l.map((b, x) => (b + d[x]) / 2), { right: m, up: p, forward: u } = V(t), h = Math.max(1, e.canvas?.width || e.state.width || 1280) / Math.max(1, e.canvas?.height || e.state.height || 720), f = Math.tan((t.fov || 35) * Math.PI / 360), _ = f * h;
  let y = 2, v = 0.1;
  for (const b of i) {
    const x = E(b, c), w = Math.abs(U(x, m)), D = Math.abs(U(x, p)), g = U(x, u);
    y = Math.max(y, 1.15 * w / _ - g, 1.15 * D / f - g, (t.near || 0.01) * 2 - g), v = Math.max(v, 1.15 * D, 1.15 * w / h);
  }
  t.target = c, t.position = E(c, k(u, y)), t.camera_type === "orthographic" && (t.zoom = Math.max(0.01, 5 / v));
}
function X(e) {
...
function Ai(e, t = {}) {
  ...
  if (e.subSelection?.point && !t.all) {
    ...
    const i = e.subSelection.point, l = ne(E(a.position, o)), d = Number.isFinite(l[0]) && H(l) > 0.1 ? l : [0.707, 0.4, 0.707], c = 2;
    a.target = [...i], a.position = C(a.target, k(d, c)), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit());
    const m = e.subSelection.mode === "vertex" ? "Vertex" : e.subSelection.mode === "edge" ? "Edge" : "Face";
    e.setStatus(S(`Focused on ${m} at [${i.map((p) => Math.round(p * 100) / 100).join(", ")}]`));
    return;
  }
  const s = e.selectedObject() || e.state.objects.find((i) => i.id === "subject") || e.state.objects[0] || { position: [0, 1.5, 0], size: [2, 3] };
  e.checkpoint(t.all ? "Frame all" : "Frame subject"), qn(e, a, s, t), r ? (e.serialize(), e.render()) : (e.beginCameraEdit(), e.commitCameraEdit(), e.finishCameraEdit()), e.setStatus(t.all ? S("Framed: all objects") : S("Framed: {name}").replace("{name}", s.name || s.type || S("Subject")));
}
function Hn(e, t, a, { forceLocal: r = !1 } = {}) {
...
function Wn(e) {
  if (e.selectedEntity === "object") {
    ...
    const a = t.keyframes?.length ? qe(t, e.frame) : t, r = a.position || [0, 0, 0];
    return {
      type: "object",
      object: t,
      position: r,
...
function Pr(e) {
  ...
  const o = B(r, a, e.canvas.width, e.canvas.height);
  if (!o || !Number.isFinite(o[0]) || !Number.isFinite(o[1])) return null;
  const n = Math.max(0.7, H(E(a.position, r)) * 0.12), s = e.state.gizmo_mode === "scale" || e.state.gizmo_mode === "rotate", i = t.type === "object" ? Hn(e, t.object, t, { forceLocal: s }) : [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
  ...
  if (e.state.gizmo_mode !== "rotate" || t.type === "camera_target")
    return {
      ...
      center: o,
      worldLength: n,
      handles: i.map((d, c) => ({ index: c, axis: d, points: [o, B(C(r, k(d, n)), a, e.canvas.width, e.canvas.height)] })).filter((d) => d.points[1] && Number.isFinite(d.points[1][0]) && Number.isFinite(d.points[1][1]))
    };
  const l = i.map((d, c) => {
    const m = Math.abs(d[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0], p = ne(we(d, m)), u = ne(we(d, p)), h = [];
    for (let f = 0; f <= 48; f++) {
      const _ = f / 48 * Math.PI * 2, y = B(C(r, C(k(p, Math.cos(_) * n), k(u, Math.sin(_) * n))), a, e.canvas.width, e.canvas.height);
      y && Number.isFinite(y[0]) && Number.isFinite(y[1]) && h.push(y);
    }
    return { index: c, axis: d, points: h };
  });
...
function zr(e, t) {
  ...
  for (const i of a.handles)
    for (let l = 0; l < i.points.length - 1; l++) {
      const d = i.points[l], c = i.points[l + 1], m = da(t, d, c);
      (!s || m < s.distance) && (s = { ...i, distance: m, segment: [d, c], worldLength: a.worldLength, entity: a.entity });
    }
...
function $n(e, t) {
  ...
  if (e.state.view_mode !== "camera") {
    ...
    if (e.state.show_camera_paths !== !1)
      for (const s of e.state.cameras) {
        ...
        const l = i.map((d) => B(d.camera?.position, r, e.canvas.width, e.canvas.height)).filter((d) => d && Number.isFinite(d[0]) && Number.isFinite(d[1]));
        for (let d = 0; d < l.length - 1; d += 1)
          if (da(t, l[d], l[d + 1]) <= 8 * Math.min(2, window.devicePixelRatio || 1))
    ...
    for (const s of e.state.objects)
      if (s.enabled !== !1)
        for (const i of s.keyframes || []) {
          ...
          if (!l) continue;
          const d = B(l, r, e.canvas.width, e.canvas.height);
  ...
  for (const s of e.state.objects) {
    ...
    const i = s.keyframes?.length ? qe(s, e.frame) : s, l = B(i.position || [0, 0, 0], r, e.canvas.width, e.canvas.height);
    if (!l) continue;
...
function Un(e, t, a, r = 15) {
  const o = a[0] - t[0], n = a[1] - t[1], s = Math.hypot(o, n) || 1, i = o / s, l = n / s, d = -l, c = i, m = r * 0.42, p = a[0] - i * r, u = a[1] - l * r;
  e.beginPath(), e.moveTo(a[0], a[1]), e.lineTo(p + d * m, u + c * m), e.lineTo(p - d * m, u - c * m), e.closePath(), e.fill(), e.save(), e.strokeStyle = "rgba(15, 23, 42, 0.65)", e.lineWidth = 1, e.stroke(), e.restore();
}
function Ei(e) {
  const t = Pr(e);
  if (!t || !t.handles) return;
  if (t.entity?.type === "camera_path") {
    const o = X(e), n = (t.entity.track?.keyframes || []).map((s) => B(s.camera?.position, o, e.canvas.width, e.canvas.height)).filter((s) => s && Number.isFinite(s[0]) && Number.isFinite(s[1]));
  ...
  for (const o of t.handles) {
    ...
    if (e.ctx.lineWidth = n ? 6.5 : 3.8, e.ctx.strokeStyle = n ? "#ffffff" : a[o.index] || "#ffffff", e.ctx.fillStyle = a[o.index] || "#ffffff", e.ctx.beginPath(), o.points.forEach((s, i) => {
      ...
      e.state.gizmo_mode === "scale" && t.entity?.type === "object" ? (e.ctx.fillRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.save(), e.ctx.strokeStyle = "rgba(15, 23, 42, 0.65)", e.ctx.lineWidth = 1, e.ctx.strokeRect(i[0] - 6, i[1] - 6, 12, 12), e.ctx.restore()) : Un(e.ctx, s[0], i);
    }
  }
...
const ot = { x: [1, 0, 0], y: [0, 1, 0], z: [0, 0, 1] }, Ke = (e, t) => Math.round(e / t) * t;
function Nr(e) {
...
function Zn(e, t, a, r, o, n, s) {
  const i = o ? "grid" : e.state.spatial_snap_mode;
  if (i === "grid") {
    const l = e.state.spatial_grid_size || 0.5;
    return n ? a.map((d, c) => d.map((m, p) => Math.abs(n[p]) > 1e-6 ? Ke(m, l) : s[c][p])) : a.map((d) => d.map((c) => Ke(c, l)));
  }
  if (i === "vertex" && r && !n) {
    ...
    if (l?.point && !t.snapshots.some((d) => d.object.id === l.objectId)) {
      const d = a.reduce((m, p) => C(m, p), [0, 0, 0]).map((m) => m / a.length), c = E(l.point, d);
      return a.map((m) => C(m, c));
    }
  }
  return a;
}
function Ee(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = Yn(a), d = a.axis ? ot[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], m = V(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...E(c.position, c.target)) * 25e-4;
  let u = a.snapshots.map((b) => [...b.transform.position]);
  if (a.mode === "translate") {
    const b = l ?? (n - s) * p * i, x = d ? k(d, b) : C(k(m.right, n * p * i), k(m.up, -s * p * i));
    u = u.map((w) => C(w, x)), u = Zn(e, a, u, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((w) => w.transform.position));
  }
  const h = a.mode === "rotate" ? l ?? (n - s) * 0.5 * i : 0, f = a.mode === "scale" ? Math.max(0.01, l ?? 1 + (n - s) * 0.01 * i) : 1, _ = d || ot.z, y = t.ctrlKey || t.metaKey || e.state.spatial_snap_mode === "grid";
  a.snapshots.forEach((b, x) => {
    const w = b.object;
    if (a.mode === "translate" && (w.position = u[x]), a.mode === "rotate") {
      const D = y ? Ke(h, 15) : h, g = k(_, D);
      w.position = C(a.pivot, Me(E(b.transform.position, a.pivot), g)), w.rotation = C(b.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const D = y ? Ke(f, 0.1) : f, g = d ? d.map((A) => A ? D : 1) : [D, D, D], M = E(b.transform.position, a.pivot);
...
function Qn(e, t) {
  ...
  return r === "escape" ? Lr(e) : r === "enter" || r === " " ? Rr(e) : ot[r] ? (a.axis = a.axis === r ? null : r, Ee(e, a.lastEvent), !0) : /^[0-9]$/.test(r) || r === "." || r === "," || r === "-" && !a.numeric ? (a.numeric += r === "," ? "." : r, Ee(e, a.lastEvent), !0) : (r === "backspace" && (a.numeric = a.numeric.slice(0, -1), Ee(e, a.lastEvent)), !0);
}
function ue(e, t, a) {
...
const re = (e, t) => Math.round(e / t) * t, es = (e, t) => e.map((a) => re(a, t));
function ts(e, t, a) {
  const r = t.keyframes?.length ? qe(t, e.frame) : t, o = r.position || [0, 0, 0], n = e.webgl?.getObjectWorldBounds?.(t.id);
  let s, i;
  if (n)
    ({ min: s, max: i } = n);
  else {
  ...
  let l = 1 / 0, d = 1 / 0, c = -1 / 0, m = -1 / 0;
  for (const p of [s[0], i[0]]) for (const u of [s[1], i[1]]) for (const h of [s[2], i[2]]) {
    const f = B([p, u, h], a, e.canvas.width, e.canvas.height);
    f && (l = Math.min(l, f[0]), c = Math.max(c, f[0]), d = Math.min(d, f[1]), m = Math.max(m, f[1]));
  }
  return Number.isFinite(l) ? { minX: l, minY: d, maxX: c, maxY: m } : null;
}
function he(e, t, a, r = [], o = null) {
  ...
  if (s === "grid")
    return o ? t.map((l, d) => Math.abs(o.axis[d]) > 1e-6 ? re(l, i) : o.base[d]) : es(t, i);
  if (s === "vertex" && a && !o) {
...
function Oi(e, t) {
  if (e.modalTransform) {
    t.preventDefault?.(), t.stopPropagation?.(), t.button === 0 ? Rr(e) : t.button === 2 && Lr(e);
    return;
  }
  if (t.target?.closest?.("button,input,select")) return;
  if (t.button === 2 && !Ht(e, t)) {
  ...
  const a = e.interactionElement.getBoundingClientRect(), r = (t.clientX - a.left) * e.canvas.width / Math.max(1, a.width), o = (t.clientY - a.top) * e.canvas.height / Math.max(1, a.height), n = X(e), s = e.state.view_mode !== "camera", i = Ht(e, t), l = t.button === 0 && !i, d = l && !t.altKey && !t.shiftKey;
  if (d && e.webgl?.pickCurveHandle) {
  ...
  if (c) {
    ...
    if (c.entity.type === "camera_path" && Kn(e, { baseDrag: b, viewCamera: n, entityPosition: c.entity.position })) return;
    if (c.entity.type === "object") {
      const x = c.entity.object;
      e.checkpoint("Transform object");
      const w = Nr(e), D = (w.length ? w : [x]).map((M) => ({ object: M, transform: ce(M) }));
  ...
  const m = l ? $n(e, [r, o]) : null;
  if (e.pointerHit = !!(c || m), m) {
    if (m.type === "camera_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.editingKeyFrame = null, e.activateCamera(m.camera.id), e.setFrame(m.keyframe.frame), e.selectKeyframe(m.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(S(`${m.camera.name} · Keyframe @ F${m.keyframe.frame} selected`));
      return;
    }
    if (m.type === "object_keyframe") {
      e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectId = m.object.id, e.editingKeyFrame = null, e.setFrame(m.keyframe.frame), e.selectKeyframe(m.keyframe), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(S(`${m.object.name || m.object.type} · Keyframe @ F${m.keyframe.frame} selected`));
      return;
    }
    if (m.type === "camera_target") {
      if (e.finishCameraEdit(), e.selectedEntity = "camera_target", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(m.camera.id), m.camera.locked) {
        e.setStatus(S("{name} is locked").replace("{name}", m.camera.name)), e.refreshObjects(), e.refreshInspector(), e.render();
        return;
      }
      e.checkpoint("Move camera target"), e.beginCameraEdit();
      const { right: _, up: y } = V(n), v = [...e.camera.target], b = e.activeCameraTrack?.(), x = !!b?.target_object_id;
      e.targetFreeDrag = {
        pointer: [r, o],
        ...
        historyCheckpointed: !0
      }, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(S(`${m.camera.name} · Target aim selected`));
      return;
    }
    if (m.type === "camera") {
      e.finishCameraEdit(), e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.editingKeyFrame = null, e.activateCamera(m.camera.id), e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(S(`${m.camera.name} selected`));
      return;
    }
    ...
    if (m.type === "object" && m.object) {
      if (e.finishCameraEdit(), e.selectedEntity = "object", e.selectedObjectIds ||= /* @__PURE__ */ new Set(), t.shiftKey || t.ctrlKey || t.metaKey ? e.selectedObjectIds.has(m.object.id) ? e.selectedObjectIds.delete(m.object.id) : e.selectedObjectIds.add(m.object.id) : e.selectedObjectIds = /* @__PURE__ */ new Set([m.object.id]), e.selectedObjectId = e.selectedObjectIds.has(m.object.id) ? m.object.id : [...e.selectedObjectIds].at(-1) || null, e.selectedKeyFrame = m.object.keyframes?.find((_) => _.frame === e.frame)?.frame ?? null, e.editingKeyFrame = null, e.state.select_mode && e.state.select_mode !== "object") {
        ...
        e.subSelection = null, e.setStatus(S(`${m.object.name || m.object.type} selected`));
      e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render();
      return;
    }
  }
  if (!m && l && !t.ctrlKey && !t.metaKey && vt(e) !== "simple") {
    e.boxSelection = {
      start: [r, o],
      current: [r, o],
      additive: t.shiftKey,
  ...
  const p = !!e.isNavigatingFly, u = Nn(e, t, n);
  if (!p && !u) return;
  ...
  s && !e.state.editor_views && (e.state.editor_views = Ne()), e.drag = {
    x: t.clientX,
    y: t.clientY,
    shift: h,
    dolly: f,
    fly: p,
    camera: K(n),
    target: s ? e.state.editor_views[e.state.view_mode] || (e.state.editor_views[e.state.view_mode] = Ne()[e.state.view_mode]) : e.camera,
    editorView: s,
    navigationOnly: i,
    historyCheckpointed: !1
  }, e.interactionElement.style && (e.interactionElement.style.cursor = f ? "ns-resize" : h ? "move" : "grabbing"), e.setStatus?.(S(p ? "Fly" : f ? "Dolly" : h ? "Pan" : "Orbit"));
}
function Ii(e, t) {
  ...
  if (e.pathDrag) {
    ...
    const c = ((e.state.cameras || []).find((m) => m.id === e.pathDrag.cameraId)?.keyframes || []).find((m) => m.frame === e.pathDrag.frame);
    c && (c.camera.position = qt(
      [i, l],
      X(e),
      e.pathDrag.anchor,
      e.canvas.width,
      e.canvas.height
    ), c.interpolation = zn(c.interpolation), e.webgl && (e.webgl.pathKey = ""), e.setFrame(e.frame, !1, !1), e.render());
    return;
  }
  if (e.curveHandleDrag) {
    ...
    const c = ((e.state.cameras || []).find((m) => m.id === e.curveHandleDrag.cameraId)?.keyframes || []).find((m) => m.frame === e.curveHandleDrag.frame);
    if (c) {
      const m = qt(
        [i, l],
        X(e),
      ...
      e.dragCurveHandle?.(c, e.curveHandleDrag.side, m, {
  ...
  if (e.targetFreeDrag) {
    ...
    const s = e.interactionElement.getBoundingClientRect(), i = (t.clientX - s.left) * e.canvas.width / Math.max(1, s.width), l = (t.clientY - s.top) * e.canvas.height / Math.max(1, s.height), d = i - e.targetFreeDrag.pointer[0], c = l - e.targetFreeDrag.pointer[1], m = t.shiftKey ? 0.1 : 1, p = C(k(e.targetFreeDrag.right, d * e.targetFreeDrag.scale * m), k(e.targetFreeDrag.up, -c * e.targetFreeDrag.scale * m)), u = he(e, C(e.targetFreeDrag.target, p), [i, l]);
    e.targetFreeDrag.tracking ? Ut(e, u) : e.camera.target = u, e.commitCameraEdit(), e.refreshInspector(), e.render();
    return;
  }
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_target") {
      const u = C(e.gizmoDrag.target, k(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), h = he(e, u, i, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
      e.gizmoDrag.tracking ? Ut(e, h) : e.camera.target = h, e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        const u = C(e.gizmoDrag.position, k(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength));
        e.camera.position = he(e, u, i, [], { base: e.gizmoDrag.position, axis: e.gizmoDrag.axis });
      } else {
        const u = c ? re(d * 0.015, Math.PI / 12) : d * 0.015, h = E(e.gizmoDrag.target, e.gizmoDrag.position), f = Me(h, k(e.gizmoDrag.axis, u * (180 / Math.PI)));
        e.camera.target = C(e.gizmoDrag.position, f);
      }
      e.commitCameraEdit(), e.refreshInspector(), e.render();
      return;
    }
    if (e.gizmoDrag.type === "camera_path") return void Vn(e, { pointer: i, deltaPixels: d, precision: l, snapping: c });
    if (e.state.gizmo_mode === "translate")
      if (e.gizmoDrag.free) {
      ...
      if (e.gizmoDrag.free) {
        ...
        const u = [...e.gizmoDrag.size], h = u[e.gizmoDrag.axisIndex] + d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength;
        u[e.gizmoDrag.axisIndex] = Math.max(0.01, c ? re(h, 0.1) : h), e.gizmoDrag.object.size = u;
      }
    else {
      const u = [...e.gizmoDrag.rotation], h = u[e.gizmoDrag.axisIndex] + d * 0.75;
      u[e.gizmoDrag.axisIndex] = c ? re(h, 15) : h, e.gizmoDrag.object.rotation = u;
    }
    const m = e.gizmoDrag.group || [], p = m.find((u) => u.object === e.gizmoDrag.object)?.transform;
    if (m.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        const u = E(e.gizmoDrag.object.position, p.position);
        for (const h of m) h.object.position = C(h.transform.position, u);
    ...
    for (const u of m.length ? m : [{ object: e.gizmoDrag.object }]) e.commitObjectEdit(u.object);
    e.refreshInspector(), e.render();
    return;
  }
  if (!e.drag) {
    const s = e.interactionElement.getBoundingClientRect(), i = zr(e, [
      (t.clientX - s.left) * e.canvas.width / Math.max(1, s.width),
      (t.clientY - s.top) * e.canvas.height / Math.max(1, s.height)
  ...
  if (e.drag.dolly) {
    ...
    let l = Math.atan2(s[0], s[2]), d = Math.asin(T(s[1] / i, -0.999, 0.999));
    ...
    const { right: s, up: i } = V(n), l = Be(n, e.interactionElement.getBoundingClientRect().height) * (e.panSensitivity ?? 1), d = C(k(s, -a * l), k(i, r * l));
    e.drag.target.position = C(n.position, d), e.drag.target.target = C(n.target, d);
  } else {
...
function Fr(e) {
  ...
  return e.keyDrag?.badge?.remove?.(), e.boxSelect?.overlay?.remove?.(), e.drag?.camera && e.drag?.target && (Object.assign(e.drag.target, e.drag.camera), e.drag.editorView && e.scheduleSerialize()), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.boxSelection = null, e.pathDrag = null, e.keyDrag = null, e.curveDrag = null, e.timelineDrag = null, e.timelinePanDrag = null, e.boxSelect = null, e.curvePanDrag = null, e.curveScrub = null, e.curveBoxSelect = null, ve(e), t && e.undo(), e.finishCameraEdit(), e.refreshInspector(), e.render(), e.setStatus(S("Interaction cancelled")), !0;
}
function Pi(e, t) {
  if (t?.type === "pointercancel" || t?.type === "lostpointercapture") {
    t.pointerId === e.activePointerId && Fr(e);
    return;
  }
  if (e.pathDrag) {
    const s = e.pathDrag.moved;
    e.pathDrag = null, ve(e), s && (e.scheduleSerialize(), e.refreshKeys(), e.setStatus(S("Path key moved")));
    return;
  }
  if (e.curveHandleDrag) {
    const s = e.curveHandleDrag.moved;
    e.curveHandleDrag = null, ve(e), s && (e.webgl && (e.webgl.pathKey = ""), e.scheduleSerialize(), e.refreshKeys(), e.setStatus(S("Curve handle updated")));
    return;
  }
  if (e.boxSelection) {
    const s = e.boxSelection, i = X(e), l = Math.min(s.start[0], s.current[0]), d = Math.max(s.start[0], s.current[0]), c = Math.min(s.start[1], s.current[1]), m = Math.max(s.start[1], s.current[1]), p = s.additive ? new Set(s.initial) : /* @__PURE__ */ new Set();
    for (const u of e.state.objects) {
      if (u.enabled === !1) continue;
      const h = ts(e, u, i);
      h && h.maxX >= l && h.minX <= d && h.maxY >= c && h.minY <= m && p.add(u.id);
    }
    e.selectedObjectIds = p, e.selectedObjectId = [...p].at(-1) || null, e.selectedEntity = p.size ? "object" : "camera", e.boxSelection = null, ve(e), e.refreshObjects(), e.refreshInspector(), e.render(), e.setStatus(S("{count} object(s) selected").replace("{count}", String(p.size)));
    return;
  }
  const a = e.keyDrag, r = !!(e.drag && !e.drag.editorView || e.targetFreeDrag), o = !!e.gizmoDrag;
  e.gizmoDrag?.type === "camera_path" && (e.serialize?.(), e.refreshKeys?.(), e.setStatus(S("Camera path transformed"))), !e.pointerHit && !e.gizmoDrag && !e.targetFreeDrag && e.drag && !e.drag.navigationOnly && t && Math.hypot(t.clientX - e.drag.x, t.clientY - e.drag.y) < 5 && (t.button === 0 || t.button === void 0) && (e.selectedEntity === "object" || e.selectedObjectId !== null || e.selectedEntity === "camera_target" || e.selectedEntity === "camera_path") && (e.selectedEntity = "camera", e.selectedObjectId = null, e.selectedObjectIds = /* @__PURE__ */ new Set(), e.selectedKeyFrame = null, e.subSelection = null, e.refreshObjects(), e.refreshKeys(), e.refreshInspector(), e.render(), e.setStatus(S("Deselected"))), ve(e), e.drag = null, e.gizmoDrag = null, e.targetFreeDrag = null, e.keyDrag = null, e.pointerHit = !1, e.canvas.classList.remove("dragging"), e.interactionElement.style && (e.interactionElement.style.cursor = "default"), a && (a.badge?.remove(), e.editingKeyFrame = null, e.updateKeyVisualState(), e.root.focus({ preventScroll: !0 })), r && e.finishCameraEdit(), o && (e.editingKeyFrame = null, e.updateKeyVisualState(), e.drawCurveEditor());
}
function zi(e, t) {
  ...
  const a = Ln(t, e.interactionElement.getBoundingClientRect().height);
  if (!a) return;
  if (e.isNavigatingFly) {
    e.cameraSpeed = T(e.cameraSpeed * Math.exp(-a * 1e-3), 0.05, 20), e.setStatus(S(`Fly speed: ${e.cameraSpeed.toFixed(2)}x`));
    return;
  }
  Jn(e);
...
function Ve(e, { sourceKind: t = "manual_2d", label: a, keys: r, source: o = {} }) {
  if (!oa.includes(t)) throw new Error(`Unsupported motion source: ${t}`);
  const n = as(e), s = { id: n, label: a || `Motion ${n.split("_").at(-1)}`, enabled: !0, semantic: "screen_point", source_kind: t, keys: r.map((i) => ({ visible: !0, interpolation: "linear", ...i })), source: { ...o } };
...
function St(e, t) {
  return e.motion_tool = ra.includes(t) ? t : "select", e.motion_tool;
}
function rs(e, t) {
  if (na.includes(t))
...
function ss(e, t, a, r, o) {
  const n = le(e, a, e.objects);
  let s = t?.point;
  if (t?.object_id) {
    const c = e.objects.find((u) => u.id === t.object_id);
    if (!c) return null;
    const m = Uo(e.objects, c), p = Array.isArray(t.local_point) ? t.local_point : [0, 0, 0];
...
function Qe(e, t) {
  return ns(t, e.interactionElement);
}
function cs(e, t) {
  const a = Kr(e.motion_layers, t);
  return a ? (Br(e, a.id), a) : null;
}
const Je = (e) => {
...
function Ni(e, t) {
  for (const o of e.root.querySelectorAll("[data-motion-tool]"))
    o.addEventListener("click", () => {
      e.motionCreationKind = "", St(e.state, o.dataset.motionTool), Ge(e), e.render();
    }, { signal: t });
  ...
  e.root.querySelector('[data-role="motion-interpolation"]')?.addEventListener("change", (o) => {
    const n = Ze(e.state);
    n && (e.checkpoint("Set motion interpolation"), rs(n, o.target.value), ae(e, `Motion interpolation: ${o.target.value}`));
  ...
  for (const o of e.root.querySelectorAll("[data-motion-layer-action]"))
    o.addEventListener("click", () => {
      ...
      if (e.checkpoint(s === "delete" ? "Delete motion layer" : s === "retime" ? "Retime motion layer" : "Toggle motion layer"), s === "delete") Br(e.state, n.id);
      else if (s === "retime") {
        const [i, l] = Xt(e);
        os(n, i, l);
  ...
  a.addEventListener("pointerdown", (o) => {
    ...
    Je(o), a.setPointerCapture?.(o.pointerId);
    const s = Qe(e, o);
    if (n === "track") {
      e.checkpoint("Draw motion track"), e.motionTrackDraft = { pointerId: o.pointerId, points: [s] };
      return;
    }
    e.checkpoint(n === "erase" ? "Erase motion track" : "Add motion anchor"), n === "anchor" ? Ve(e.state, { sourceKind: "static_anchor", label: `Anchor ${(e.state.motion_layers || []).length + 1}`, keys: [{ time_seconds: e.frame / e.state.fps, ...s, interpolation: "hold" }] }) : n === "project" ? ds(e, s) : n === "erase" && cs(e.state, s), ae(e, `Motion tool: ${n}`);
  }, { capture: !0, signal: t }), a.addEventListener("pointermove", (o) => {
    e.motionTrackDraft?.pointerId === o.pointerId && (Je(o), e.motionTrackDraft.points.push(Qe(e, o)), e.render());
  }, { capture: !0, signal: t });
  const r = (o) => {
    const n = e.motionTrackDraft;
    if (n?.pointerId !== o.pointerId) return;
    Je(o), e.motionTrackDraft = null;
    const [s, i] = Xt(e), l = ls(e.state, n.points, s, i);
  ...
  a.addEventListener("pointerup", r, { capture: !0, signal: t }), a.addEventListener("pointercancel", r, { capture: !0, signal: t }), a.addEventListener("click", (o) => {
    ...
    const n = Kr(e.state.motion_layers, Qe(e, o));
...
function ps(e, t) {
  ...
  if (a) {
    ...
    e.checkpoint?.(`Motion: ${a.label}`), St(e.state, a.tool), e.motionCreatingLabel = a.label, e.motionCreationKind = t, Ge(e), e.render(), e.setStatus(a.hint);
  }
}
function Vr(e) {
  return (e.state.motion_tool || "select") === "select" && !e.motionTrackDraft ? !1 : (St(e.state, "select"), e.motionTrackDraft = null, e.motionCreatingLabel = "", e.motionCreationKind = "", Ge(e), e.render(), e.setStatus("Motion creation cancelled."), !0);
}
function Ri(e, t) {
  for (const a of e.root.querySelectorAll("[data-motion-create]"))
    a.addEventListener("click", () => ps(e, a.dataset.motionCreate), { signal: t });
  e.root.querySelector("[data-motion-create-cancel]")?.addEventListener("click", () => Vr(e), { signal: t });
}
...
const ge = { t: "translate", r: "rotate", s: "scale" }, gs = [
...
function Ss(e, t) {
  return vs(e) || t?.lastKeyZone || "viewport";
}
let Zt = !1;
function ws() {
  Zt || typeof window > "u" || (Zt = !0, window.addEventListener("keydown", (e) => {
    if (!xn()) return;
    const t = e.composedPath?.()[0] || e.target, a = Mn(t);
    !a || a.disposed || xs(a, e) && (e.preventDefault(), e.stopImmediatePropagation?.(), e.stopPropagation());
  }, { capture: !0 }));
}
function xs(e, t) {
  if (!kn()) return !1;
  const a = t.composedPath?.()[0] || t.target;
  if (_s(a) || (t.code === "Space" || t.key === "Enter") && bs(a)) return !1;
...
function ks(e, t) {
  ...
  if (r === "Numpad9") {
    const i = hs(e.state.view_mode);
    return i ? e.setViewMode(i) : Yt(e, Math.PI, 0), !0;
  }
  const n = Math.PI / 12, s = { Numpad4: [n, 0], Numpad6: [-n, 0], Numpad8: [0, n], Numpad2: [0, -n] };
  if (s[r])
    return Yt(e, s[r][0], s[r][1]), !0;
  if (r === "Numpad5")
  ...
  if (["w", "a", "s", "d", "q", "e"].includes(a) && e.isNavigatingFly) {
    const i = e.viewportCamera(), l = e.state.view_mode !== "camera", { right: d, up: c, forward: m } = V(i), p = (t.shiftKey ? 0.6 : 0.18) * e.cameraSpeed, u = { w: k(m, p), s: k(m, -p), d: k(d, p), a: k(d, -p), e: k(c, p), q: k(c, -p) }[a];
...
function Ts(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!Pe(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: ro(e.state) }, et(e)) : (e.checkpoint("Split shot"), no(e.state, e.frame, null) ? et(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
    if (t.repeat) return !0;
    const r = Pe(e.state), o = aa(e.state, e.frame), n = o ? r.findIndex((s) => s.start === o.start) : -1;
    return n >= 0 && (e.checkpoint("Remove shot"), so(e.state, n) && et(e)), !0;
  }
  return !1;
}
function wt(e, t) {
...
const nt = "MajoorOmniCamDirector", st = "MajoorOmniCamExtractor", it = "MajoorOmniCamMonitor";
function We(e) {
  return String(e?.comfyClass || e?.type || e?.constructor?.comfyClass || e?.constructor?.type || "");
}
const Gr = {
  [nt]: { default: [1313, 1633], min: [760, 760] },
  [st]: { default: [761, 1458], min: [700, 760] },
  [it]: { default: [798, 1634], min: [640, 680] }
}, js = 0.92, As = 0.88;
function Es([e, t], [a, r]) {
  if (typeof window > "u") return [e, t];
  const o = Math.round((window.innerWidth || e) * js), n = Math.round((window.innerHeight || t) * As);
  return [
...
function Os(e, t) {
  const a = Gr[t];
  return !a || !e?.setSize ? !1 : (e.setSize(Es(a.default, a.min)), !0);
}
function Is(e, t, a) {
  const r = Gr[t];
...
const Qt = "oc-help-css", je = "#8b7bd8", qr = /* @__PURE__ */ new Map();
function xt(e, t) {
  e && t && qr.set(e, t);
}
function lt(e) {
...
function zs() {
  ...
  e.id = Qt, e.textContent = Ps, document.head.appendChild(e);
}
function oe(e) {
...
function Jt(e) {
  ...
  const m = document.activeElement instanceof HTMLElement ? document.activeElement : null, p = () => {
    document.removeEventListener("keydown", h, !0), t.remove(), xe === p && (xe = null), m && m.isConnected && typeof m.focus == "function" && m.focus({ preventScroll: !0 });
  };
  xe = p;
  const u = () => Array.from(
    ...
    if (f.key === "Tab") {
      ...
      f.shiftKey && (b === y || !a.contains(b)) ? (f.preventDefault(), v.focus()) : !f.shiftKey && (b === v || !a.contains(b)) && (f.preventDefault(), y.focus());
    }
  };
...
const ea = "MajoorOmniCam.ShowHelp", ct = "oc-help-toolbar-icon", ta = "oc-help-toolbar-css", Ls = "#8b7bd8";
function Fs() {
  if (document.getElementById(ta)) return;
  const e = document.createElement("style");
  e.id = ta, e.textContent = `
    .${ct}{display:inline-flex;align-items:center;justify-content:center;
      width:16px;height:16px;border-radius:50%;background:${Ls};color:#fff;
...
J.registerExtension({
  ...
  getNodeMenuItems(e) {
    const t = lt(e?.comfyClass);
    return t ? [null, { content: "? Help", callback: () => Jt(t) }] : [];
  },
  setup() {
    Fs();
  }
...
function Ae(e, t) {
  ...
  Array.isArray(a) && a.push({ stage: e, nodeId: t?.id ?? null, nodeClass: We(t), configuringGraph: ie });
}
ws();
Yr(J);
En(J);
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (We(e) !== nt) return;
    Ae("director:nodeCreated", e);
    const t = !ie, a = t ? null : Ct(e);
    await wt(e, async () => {
      Ae("director:import:start", e);
      const { attachDirector: o } = await import("./chunk-HWHEB5St.js").then((n) => n.g);
      return Ae("director:import:resolved", e), o;
    });
    const r = e.__majoorOmniCam;
    r && (Ae("director:attach:complete", e), On(r), t && In(r), Mt(e, nt, t, a?.()));
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (We(e) !== st) return;
    const t = !ie, a = t ? null : Ct(e);
    await wt(e, async () => (await import("./chunk-mFTUwfW_.js")).attachExtractor), e.__majoorOmniCamExtractor && Mt(e, st, t, a?.());
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (We(e) !== it) return;
    const t = !ie, a = t ? null : Ct(e);
    await wt(e, async () => (await import("./chunk-Z9mpGHMZ.js")).attachMonitor), e.__majoorOmniCamMonitor && Mt(e, it, t, a?.());
  }
});
export {
  wa as $,
