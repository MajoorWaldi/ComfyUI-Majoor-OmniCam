...
const zt = Object.freeze([0.05, 8]), Oo = 80, fa = (e, t, a) => Math.max(t, Math.min(a, e));
function ha(e) {
  if (!e || typeof e != "object") return null;
...
const Di = "omnicam_humanoid_v1", gt = Object.freeze([
  "root",
  "pelvis",
  "spine",
  "chest",
...
const No = /^mixamorig[:_ ]?/i, zo = /[\s_\-.:|]+/g, Ro = {
...
function Lo(e) {
  return String(e || "").trim().replace(No, "").replace(zo, "").toLowerCase();
}
function Fo(e) {
  const t = [e];
...
function ji(e) {
  ...
  for (const n of [!0, !1])
    for (const [s, i] of Object.entries(Ro))
      if (!o[s])
        for (const l of i) {
          const d = Bo(l, a, r, n);
          if (d != null) {
...
const Go = 128, Rt = "omnicam_humanoid_v1", yt = /^[a-z0-9][a-z0-9_-]*$/, qo = 1e-3, Ho = (e) => Array.isArray(e) && e.length === 3 && e.every((t) => Number.isFinite(Number(t)));
function bt(e) {
...
function de(e, t, a = null) {
  ...
  const o = Qo(e, r, t), n = R(r, t, "pos_x", (g) => (g.camera || g).position[0], !1, o), s = R(r, t, "pos_y", (g) => (g.camera || g).position[1], !1, o), i = R(r, t, "pos_z", (g) => (g.camera || g).position[2], !1, o);
  ...
  const f = R(r, t, "fov", (g) => Number((g.camera || g).fov ?? 35), !1, o), w = R(r, t, "roll", (g) => Number((g.camera || g).roll ?? 0), !0, o), b = R(r, t, "zoom", (g) => Number((g.camera || g).zoom ?? 1), !1, o), S = R(r, t, "near", (g) => Number((g.camera || g).near ?? 0.01), !1, o), y = R(r, t, "far", (g) => Number((g.camera || g).far ?? 1e4), !1, o), C = r[0]?.camera || r[0] || le();
  let _ = r[0];
  for (const g of r)
    if ((g.frame ?? 0) <= t) _ = g;
    else break;
...
const D = (e, t, a) => Math.max(t, Math.min(a, e)), en = /^#(?:[0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, fe = (e, t = null) => typeof e == "string" && en.test(e.trim()) ? e.trim() : t, M = (e, t) => [e[0] + t[0], e[1] + t[1], e[2] + t[2]], j = (e, t) => [e[0] - t[0], e[1] - t[1], e[2] - t[2]], T = (e, t) => [e[0] * t, e[1] * t, e[2] * t], Z = (e, t) => e[0] * t[0] + e[1] * t[1] + e[2] * t[2], Me = (e, t) => [e[1] * t[2] - e[2] * t[1], e[2] * t[0] - e[0] * t[2], e[0] * t[1] - e[1] * t[0]], W = (e) => Math.sqrt(Math.max(1e-12, Z(e, e))), ie = (e) => T(e, 1 / W(e));
function Sa(e, t, a) {
...
function Bt(e, t, a, r, o) {
  ...
  let w = D(Number(n.in_x ?? h.in_x), -0.99, -0.01), b = Number(n.in_y ?? h.in_y);
  if (s === "aligned") {
...
function R(e, t, a, r, o = !1, n = null) {
  ...
  if (l.interpolation === "bezier" || d.interpolation === "bezier") {
    const S = Bt(l, a, c, d, r), y = Bt(d, a, l, u, r), C = m, _ = m + (S.out_y || 0), A = f + (y.in_y || 0), g = f, x = D(Number(S.out_x ?? 1 / 3), 0, 1), E = D(1 + Number(y.in_x ?? -1 / 3), 0, 1);
    let G = 0, F = 1;
...
function Ue(e, t) {
  ...
  const r = me(e), o = (b, S) => (b.transform?.position || r.position)[S] ?? 0, n = (b, S) => (b.transform?.rotation || r.rotation)[S] ?? 0, s = (b, S) => (b.transform?.size || r.size)[S] ?? (S === 2 ? 0.01 : 1), i = _t(a, t), l = R(a, t, "pos_x", (b) => o(b, 0), !1, i), d = R(a, t, "pos_y", (b) => o(b, 1), !1, i), c = R(a, t, "pos_z", (b) => o(b, 2), !1, i), u = R(a, t, "rot_x", (b) => n(b, 0), !0, i), p = R(a, t, "rot_y", (b) => n(b, 1), !0, i), h = R(a, t, "rot_z", (b) => n(b, 2), !0, i), m = R(a, t, "scale_x", (b) => s(b, 0), !1, i), f = R(a, t, "scale_y", (b) => s(b, 1), !1, i), w = R(a, t, "scale_z", (b) => s(b, 2), !1, i);
  return {
...
function Bi(e = "balanced", t = "all_views", a = null) {
  ...
  for (let p = 0; p < o; p++) {
    ...
    if (t === "ground_focus")
      ...
      w = Math.cos(A) * x * E, S = Math.sin(A) * x * E, b = Math.max(0.01, g * E * 0.75 + 2.5), y = 0.72, C = 0.78, _ = 0.88;
    } else {
      const A = p % 4;
      if (A === 0) {
        const g = 0.3 + Math.sqrt(m) * 28, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, b = 0.01 + f * 0.34, y = 0.9, C = 0.94, _ = 1;
      } else if (A === 1) {
        const g = 0.6 + Math.sqrt(m) * 18, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, b = 0.35 + f * 3.15, y = 0.68, C = 0.76, _ = 0.86;
      } else if (A === 2) {
        const g = 2 + Math.sqrt(m) * 24, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, b = 3.5 + f * 11.5, y = 0.55, C = 0.65, _ = 0.78;
      } else {
        const g = 0.5 + m * 6.5, x = p * 2.399963229728653;
        w = Math.cos(x) * g, S = Math.sin(x) * g, b = 0.05 + f * 4.95, y = 0.8, C = 0.86, _ = 0.94;
      }
    }
...
function xa(e, t = {}, a = null, r = "generic") {
  ...
  for (let g = 0; g < n; g++) {
    ...
    !f[g] && !b && (x.push("over"), E.push("framing_loss")), S.push(qt(x)), y.push(E);
  }
...
function Wn() {
  ...
  return {
    ...
    maxOutputTokens: K(zr, 4096, 512, 32768, !0),
    maxPlannerSteps: K(Rr, 6, 1, 12, !0),
...
function $n() {
  return {
    ...
    guideCaptureStyle: z(Ba, "auto", ["auto", "motion_proxy", "clay"]),
    pointDensity: z(Ka, "balanced", ["none", "sparse", "balanced", "dense", "ultra"]),
    pointSpread: z(Va, "all_views", ["all_views", "ground_focus", "dome"]),
    pointColor: Xt(Ga, "#cbd5e1"),
    cardFit: z(qa, "contain", ["contain", "cover", "stretch"]),
    ...
    aspectRatio: z(or, "auto", ["auto", "16:9", "4:3", "1:1", "9:16", "2.39:1"]),
    burnIn: N(nr, !1),
    speedHeatmap: N(sr, !1),
    showWireframe: N(ir, !1),
    showVertices: N(lr, !1),
    selectMode: z(cr, "object", ["object", "vertex", "edge", "face"]),
    gizmoMode: z(dr, "translate", ["translate", "rotate", "scale"]),
    gizmoSpace: z(mr, "world", ["world", "local"]),
    spatialSnapMode: z(ur, "none", ["none", "grid", "vertex"]),
    spatialGridSize: K(pr, 0.5, 0.01, 100),
    navigationProfile: z(fr, "simple", ["maya", "blender", "simple"]),
    ...
    viewMode: z(Sr, "perspective", ["camera", "perspective", "front", "back", "top", "bottom", "right", "left"]),
    snapEnabled: N(Cr, !0),
    snapFrames: K(Mr, 1, 1, 24, !0),
    autoKey: N(xr, !1),
    defaultInterpolation: z(kr, "ease", ["ease", "smooth", "bezier", "linear", "ease_in", "ease_out", "hold"]),
    timecodeMode: z(Ar, "time", ["time", "timecode"]),
    loopPlayback: N(Tr, !1),
    uiDensity: z(Dr, "animation", ["basic", "animation", "advanced"]),
    previewLayout: z(jr, "auto", ["auto", "1", "2", "4"]),
    cameraViewVisible: N(Er, !0),
    undoLimit: K(Ir, 100, 10, 500, !0),
    extractorBackend: z(Or, "dpvo", ["dpvo", "pycolmap"]),
    monitorProfile: z(Pr, "wan_camera_native", ["wan_camera_native", "minimax_h3", "ltx_motion", "generic_video"])
  };
}
function Un() {
...
function bs(e, t, a, r = {}) {
  ...
  t.target = c, t.position = j(c, T(h, b)), t.camera_type === "orthographic" && (t.zoom = Math.max(0.01, 5 / S));
}
const te = {
...
function Wr(e) {
  ...
  const l = i.map((d, c) => {
    ...
    for (let f = 0; f <= 48; f++) {
      const w = f / 48 * Math.PI * 2, b = V(M(r, M(T(p, Math.cos(w) * n), T(h, Math.sin(w) * n))), a, e.canvas.width, e.canvas.height);
...
function Oe(e, t) {
  ...
  if (a.mode === "translate") {
    const y = l ?? (n - s) * p * i, C = d ? T(d, y) : M(T(u.right, n * p * i), T(u.up, -s * p * i));
    h = h.map((_) => M(_, C)), h = As(e, a, h, o, t.ctrlKey || t.metaKey, d, a.snapshots.map((_) => _.transform.position));
  }
  ...
  a.snapshots.forEach((y, C) => {
    ...
    if (a.mode === "translate" && (_.position = h[C]), a.mode === "rotate") {
      const A = b ? He(m, 15) : m, g = T(w, A);
      _.position = M(a.pivot, ke(j(y.transform.position, a.pivot), g)), _.rotation = M(y.transform.rotation, g);
    }
    if (a.mode === "scale") {
      const A = b ? He(f, 0.1) : f, g = d ? d.map((E) => E ? A : 1) : [A, A, A], x = j(y.transform.position, a.pivot);
      _.position = M(a.pivot, x.map((E, G) => E * g[G])), _.size = y.transform.size.map((E, G) => Math.max(0.01, E * g[G]));
    }
...
function bl(e, t) {
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera_target") {
      const h = M(e.gizmoDrag.target, T(e.gizmoDrag.axis, d * e.gizmoDrag.worldLength / e.gizmoDrag.screenLength)), m = ye(e, h, i, [], { base: e.gizmoDrag.target, axis: e.gizmoDrag.axis });
    ...
    if (u.length > 1 && p)
      if (e.state.gizmo_mode === "translate") {
        ...
        for (const m of u) {
          ...
          m.object.position = M(e.gizmoDrag.groupPivot, f.map((w, b) => w * h[b])), m.object.size = m.transform.size.map((w, b) => Math.max(0.01, w * h[b]));
        }
      }
...
function oi(e, t) {
  ...
  if (a === "s" || a === "a")
    return t.repeat || (!ze(e.state).length || a === "a" ? (e.checkpoint("Auto-split shots"), e.state.sequence = { ...e.state.sequence || { recording_path: "" }, enabled: !0, cuts: go(e.state) }, ot(e)) : (e.checkpoint("Split shot"), bo(e.state, e.frame, null) ? ot(e) : e.setStatus("Move the playhead inside a shot first"))), !0;
  if (t.key === "Delete" || t.key === "Backspace") {
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await Tt(e, async () => {
      ...
      const { attachDirectorShell: o } = await import("./chunk-DXxCJosK.js").then((n) => n.o);
...
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== mt) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-C_MpHHsR.js").then((r) => r.g)).attachExtractorShell), e.__majoorOmniCamExtractorRuntime && jt(e, mt, t, a?.());
  }
});
ae.registerExtension({
  ...
  async nodeCreated(e) {
    if (Ye(e) !== ut) return;
    const t = !ce, a = t ? null : Et(e);
    await Tt(e, async () => (await import("./chunk-C0ioVhpL.js").then((r) => r.s)).attachMonitorShell), !(!e.__majoorOmniCamMonitorShell && !e.__majoorOmniCamMonitor) && jt(e, ut, t, a?.());
  }
});
export {
  Fe as $,
