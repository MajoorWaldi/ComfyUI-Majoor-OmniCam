import { u as a, af as j, bQ as te, bR as J, bS as w } from "./chunk-Cgt_u7Uc.js";
...
async function K(e, n, i, { signal: o } = {}) {
  ...
  if (s.ok === !1) {
    const P = p?.error?.code || `HTTP_${s.status || 0}`, C = p?.error?.message || `OmniCam Agent plan request failed (${s.status})`, d = new Error(C);
...
async function A(e, n, i, o) {
  ...
  if (s.ok === !1) {
    const P = p?.error?.code || `HTTP_${s.status || 0}`, C = p?.error?.message || `OmniCam Agent provider request failed (${s.status})`, d = new Error(C);
    throw d.code = P, d.status = s.status || 0, d;
  }
  return p ?? {};
}
async function se(e) {
  return (await A(e, "GET", "/majoor/omnicam/agent/v1/providers")).providers || [];
}
async function le(e, n) {
  return A(e, "GET", $(n, "/status"));
}
async function ce(e, n, i) {
  try {
    return await A(e, "PUT", $(n, "/credential"), { secret: i });
  } finally {
    i = null;
  }
}
async function de(e, n) {
  return A(e, "DELETE", $(n, "/credential"));
}
async function ue(e, n, i) {
  return A(e, "POST", $(n, "/test"), i || {});
}
async function fe(e, n, i) {
  return (await A(e, "POST", $(n, "/models"), i || {})).models || [];
}
const pe = {
  ollama: "Ollama",
  openai: "OpenAI",
...
function ve(e) {
  return !e || !e.length ? `<li class="oc-asset-empty">${a("No visible changes")}</li>` : e.map((n) => `<li>${S(n.entity)} · ${S(n.field)}</li>`).join("");
}
function F(e) {
...
function V(e, n) {
  ...
  return i.length ? i.map((o) => `<option value="${S(o.id)}"${o.id === n ? " selected" : ""}>${S(o.label)}</option>`).join("") : `<option value="">${a("No providers found")}</option>`;
}
function Y(e, n) {
  const i = Array.isArray(e) ? [...e] : [];
  return n && !i.includes(n) && i.unshift(n), i.length ? i.map((o) => `<option value="${S(o)}"${o === n ? " selected" : ""}>${S(o)}</option>`).join("") : `<option value="">${a("No models found")}</option>`;
}
function he(e, n = {}) {
  const i = e.root, o = n.api || e.api || e.app?.api, s = (t) => i.querySelector(`[data-role="${t}"]`), p = s("agent-panel"), P = s("agent-hint"), C = s("agent-privacy-note"), d = s("agent-describe"), k = s("agent-plan"), y = s("agent-provider-select"), m = s("agent-model-select"), H = s("agent-provider-label"), O = s("agent-credential-status"), q = s("agent-credential-form"), T = s("agent-credential-input"), M = i.querySelector('[data-agent-act="preview"]'), N = i.querySelector('[data-agent-act="apply"]'), U = i.querySelector('[data-agent-act="cancel"]');
  ...
  function E() {
    const t = w();
    if (H) {
      const f = pe[t.provider] || t.provider;
      H.textContent = `${f} · ${t.model || a("(no model set)")}`;
    }
    C && (C.textContent = me(t));
    const r = g === "planning" || g === "applying";
    if (M && (M.disabled = r), N && (N.disabled = !c || r || c.truncated || g === "stale"), U && (U.disabled = !c), d && (d.disabled = r), k && (k.innerHTML = c ? ve(c.changes) : ""), g === "planning") u(a("Planning..."));
      ...
      const f = c?.description || "", b = c?.warnings || [];
      u(b.length ? [f, ...b.map((v) => `⚠ ${v}`)].join(" ") : f);
    }
  }
  function h(t) {
    g = t, E();
  ...
  async function _() {
    ...
    try {
      const r = await fe(o, t.provider, { base_url: t.baseUrl });
      if (l) return;
      let f = t.model;
      !f && r.length && (f = r[0], j(J(t.provider), f)), m.innerHTML = Y(r, f), E();
    } catch {
      if (l) return;
  ...
  function R() {
    !m || !m.value || (j(J(w().provider), m.value), E());
  }
  function B() {
    !y || !y.value || (j(te, y.value), E(), I(), _());
  }
  async function z() {
    ...
    L?.abort?.();
    const f = typeof AbortController == "function" ? new AbortController() : null;
    L = f, c = null, h("planning");
    const b = w();
    try {
      const v = await ae(o, {
        sessionId: r,
  ...
  function x(t) {
    q && (q.hidden = !t), t ? T?.focus?.() : T && (T.value = "");
  }
  async function Z() {
    const t = w(), r = T?.value || "";
    if (T && (T.value = ""), !r) {
    ...
    try {
      ...
      l || u(f?.message || a("Could not save the credential"));
    } finally {
      x(!1), await I(), await _();
    }
  }
  ...
  function G(t) {
    ...
    if (r) {
      ...
      if (r.action === "credential-replace") return x(q?.hidden !== !1);
  ...
  return p?.addEventListener("click", G), y?.addEventListener("change", B), m?.addEventListener("change", R), d?.addEventListener("input", D), E(), Q(), I(), _(), {
    get state() {
      return g;
    },
    dispose() {
      l = !0, L?.abort?.(), p?.removeEventListener("click", G), y?.removeEventListener("change", B), m?.removeEventListener("change", R), d?.removeEventListener("input", D);
    }
  };
}
export {
