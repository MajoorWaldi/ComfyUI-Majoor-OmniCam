...
function le(e, t, a = null) {
  ...
  if (u && Array.isArray(f)) {
    ...
    if (g && g.enabled !== !1) {
      const M = ft(f, g, t), A = d?.offset || e.target_offset || e.camera?.target_offset || [0, 0, 0];
...
function z(e, t, a, r, o = !1, n = null) {
  ...
  if (l.interpolation === "bezier" || m.interpolation === "bezier") {
    const v = Tt(l, a, c, m, r), b = Tt(m, a, l, d, r), x = f, w = f + (v.out_y || 0), D = h + (b.in_y || 0), g = h, M = T(Number(v.out_x ?? 1 / 3), 0, 1), A = T(1 + Number(b.in_x ?? -1 / 3), 0, 1);
    let G = 0, ee = 1;
...
function Ae(e, t) {
  ...
  const n = o[0] - a.start[0], s = o[1] - a.start[1], i = t.shiftKey ? 0.1 : 1, l = Kn(a), m = a.axis ? at[a.axis] : null, c = e.state.view_mode === "camera" ? e.camera : e.state.editor_views[e.state.view_mode], d = K(c), p = c.camera_type === "orthographic" ? 10 / (Math.max(0.01, c.zoom || 1) * Math.max(1, e.canvas.height)) : Math.hypot(...E(c.position, c.target)) * 25e-4;
  ...
  a.snapshots.forEach((b, x) => {
    ...
    if (a.mode === "scale") {
      const D = y ? Fe(h, 0.1) : h, g = m ? m.map((A) => A ? D : 1) : [D, D, D], M = E(b.transform.position, a.pivot);
  ...
  if (e.gizmoDrag) {
    ...
    if (e.gizmoDrag.type === "camera") {
      if (e.state.gizmo_mode === "translate") {
        ...
        const u = c ? re(m * 0.015, Math.PI / 12) : m * 0.015, f = E(e.gizmoDrag.target, e.gizmoDrag.position), h = xe(f, k(e.gizmoDrag.axis, u * (180 / Math.PI)));
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    ...
    await yt(e, async () => {
      ...
      const { attachDirector: o } = await import("./chunk-OJjI39uU.js").then((n) => n.g);
...
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== ot) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-C9RG8G3e.js")).attachExtractor), e.__majoorOmniCamExtractor && _t(e, ot, t, a?.());
  }
});
J.registerExtension({
  ...
  async nodeCreated(e) {
    if (qe(e) !== nt) return;
    const t = !ie, a = t ? null : vt(e);
    await yt(e, async () => (await import("./chunk-AgFqthg8.js")).attachMonitor), e.__majoorOmniCamMonitor && _t(e, nt, t, a?.());
  }
});
export {
  ya as $,
